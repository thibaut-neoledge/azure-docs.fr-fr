...
Cette rubrique montre comment utiliser des scripts serveur pour autoriser les utilisateurs authentifiés à accéder aux données dans Azure Mobile Services à partir d'une application iOS.  Dans ce didacticiel, vous inscrivez les scripts dans Mobile Services pour filtrer les requêtes en fonction de l'ID utilisateur d'un utilisateur authentifié, garantissant à chaque utilisateur de voir uniquement ses propres données.
