...
Cet article s'appuie sur ces articles précédents en montrant comment vos applications HTML peuvent utiliser JavaScript pour accéder à vos applications API de backend.
...
1. Cliquez avec le bouton droit sur le fichier *index.html* et sélectionnez l'élément de menu **Définir comme page de démarrage**. Ensuite, déboguez le projet web afin qu'il s'ouvre dans votre navigateur par défaut.

	![apiapp.JSON et les métadonnées dans l'Explorateur de solutions](./media/app-service-api-javascript-client/08-run-the-web-app.png)

1. Notez les barres de manipulation du modèle dans la sortie HTML. Vous allez lier ces éléments HTML aux données en utilisant AngularJS à l'étape suivante.
...
1. Ajoutez le code JavaScript ci-dessous au fichier *index.html* pour appeler l'API et lier l'interface utilisateur HTML aux données dans la sortie de l'API.
	...
	> **Remarque** : votre numéro de port peut être différent. Par conséquent, si votre projet d'API s'exécute sur un autre port, modifiez simplement le code JavaScript ci-dessus de façon à refléter votre environnement.

1. Assurez-vous que le projet d'application API est également en cours d'exécution, sans quoi le HTML JavaScript ne fonctionnera pas correctement. Cliquez avec le bouton droit sur la solution et sélectionnez **Propriétés**. Spécifiez que les deux projets web doivent s'**Exécuter sans débogage** et que le projet d'API doit s'exécuter en premier.
...
1. Déboguez la solution : vous verrez que le client HTML/JavaScript peut se connecter au projet d'application API et afficher des données à partir de celui-ci.
...
## Résumé 
Cet exemple a montré comment vous pouvez utiliser AngularJS comme plateforme JavaScript pour accéder aux backends d'applications API. Vous pouvez changer la fonctionnalité d'accès REST et utiliser une autre infrastructure JavaScript. <!--HONumber=52-->
