...
* L’accès réseau entrant vers les ports nécessaires aux environnements App Service doit être autorisé, comme décrit dans cet [article][requiredports].
...
Les conditions d’accès au réseau entrant peuvent être remplies en configurant un [groupe de sécurité réseau][NetworkSecurityGroups] sur le sous-réseau de l’environnement App Service pour autoriser l’accès nécessaire, comme décrit dans cet [article][requiredports].
