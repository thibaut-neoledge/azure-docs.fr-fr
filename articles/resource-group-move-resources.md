...
Cette rubrique vous montre comment déplacer des ressources vers un nouvel abonnement ou un nouveau groupe de ressources dans le même abonnement. Lors du déplacement vers un nouvel abonnement, l’abonnement doit exister dans le même [locataire Active Directory](active-directory/active-directory-howto-tenant.md) que l’abonnement d’origine. Vous pouvez utiliser le portail, PowerShell, Azure CLI ou l’API REST pour déplacer des ressources. Les opérations de déplacement de cette rubrique sont disponibles sans assistance du support technique Azure.
...
1. Le service doit activer la possibilité de déplacer des ressources. Pour plus d’informations sur les [services permettant le déplacement de ressources](#services-that-enable-move), consultez la liste ci-dessous.
...
3. Si vous déplacez une application App Service, vous avez lu attentivement les [limitations d’App Service](#app-service-limitations).
4. Si vous déplacez des ressources associées à Recovery Services, vous avez consulté les [limitations de Recovery Services](#recovery-services-limitations)
...
Actuellement, si votre application web inclut un certificat SSL que vous avez acheté en externe et chargé sur l’application, vous devez supprimer le certificat avant de déplacer l’application web. Par exemple, vous pouvez effectuer les opérations suivantes :
...
Si votre application web utilise un certificat App Service, vous pouvez déplacer sans problème l’application web et le certificat vers un nouveau groupe de ressources. Pour déplacer un certificat App Service vers un nouvel abonnement, contactez le support technique Azure.
...
Pour déplacer des ressources existantes vers un autre groupe de ressources ou un autre abonnement, exécutez la commande **azure resource move** . Vous devez fournir les ID des ressources à déplacer. Vous pouvez obtenir les ID des ressources avec la commande suivante :
