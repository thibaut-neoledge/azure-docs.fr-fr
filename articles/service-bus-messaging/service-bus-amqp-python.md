...
---
# <a name="using-service-bus-from-python-with-amqp-10"></a>Utilisation de Service Bus à partir de Python avec AMQP 1.0
...
Dans l’API .NET Service Bus, les propriétés de l’application de messagerie sont exécutées dans la collection **Properties** de [BrokeredMessage][BrokeredMessage]. Le code suivant montre comment lire les propriétés d’application d’un message reçu d’un client Python.
...
Les tables suivantes montrent le mappage entre les propriétés de message standard Proton-Python et les propriétés de message standard [BrokeredMessage][BrokeredMessage].
