...
Le mécanisme de configuration de service vous permet de modifier les paramètres de configuration de façon dynamique depuis le [portail Azure][portail Azure], sans avoir à redéployer votre application. Par exemple, ajoutez une étiquette `Setting` à votre fichier de définition de service (**.csdef**), comme indiqué dans l’exemple suivant.
...
Utilisez le nom et les valeurs de clé SAP que vous avez récupérées sur le [portail Azure][portail Azure], comme décrit précédemment.
...
Le type de filtre le plus flexible pris en charge par les abonnements est la classe [SqlFilter][SqlFilter], qui implémente un sous-ensemble de SQL92. Les filtres SQL opèrent au niveau des propriétés des messages publiés dans la rubrique. Pour plus d’informations sur les expressions utilisables avec un filtre SQL, examinez la syntaxe [SqlFilter.SqlExpression][SqlFilter.SqlExpression].

Dans l’exemple suivant, l’abonnement **HighMessages** est créé avec un objet [SqlFilter][SqlFilter] qui sélectionne uniquement les messages dont la propriété personnalisée **MessageNumber** présente une valeur supérieure à 3.
...
De même, l’exemple suivant crée l’abonnement **LowMessages** avec un filtre [SqlFilter][SqlFilter] qui sélectionne uniquement les messages dont la propriété **MessageNumber** présente une valeur inférieure ou égale à 3.
...
* Consultez [Files d’attente, rubriques et abonnements Service Bus][Files d’attente, rubriques et abonnements Service Bus].
* [Exemple de filtres de rubrique][Exemple de filtres de rubrique]
...
* Exemples Service Bus : téléchargement depuis la page [Exemples Azure][Exemples Azure] ou consultation de la [présentation](service-bus-samples.md).
