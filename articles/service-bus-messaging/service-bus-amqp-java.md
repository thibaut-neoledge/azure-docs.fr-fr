...
| `[username]`  | Nom de la clé SAP Service Bus obtenue auprès du [portail Azure][].                    |   |   |   |   |
| `[password]`  | Forme encodée dans une URL de la clé SAP Service Bus obtenue auprès du [portail Azure][]. |   |   |   |   |
...
Le code suivant montre comment créer un objet [BrokeredMessage][] dans .NET qui est reçu par un client JMS comme un objet **BytesMessage**.
...
Le code suivant montre comment créer un objet [BrokeredMessage][] dans .NET qui est reçu par un client JMS comme un objet **StreamMessage**.
...
Dans les API .NET Service Bus, les propriétés de l’application de messagerie sont exécutées dans la collection **Properties** de [BrokeredMessage][]. Le code suivant montre comment lire les propriétés d’application d’un message reçu d’un client JMS.
...
Les tableaux suivants montrent comment les en-têtes standard JMS et les propriétés standard [BrokeredMessage][] sont mappés à l’aide d’AMQP 1.0.
