...
1.  Dans Xamarin Studio, ouvrez le projet que vous avez créé en suivant le didacticiel [Prise en main des données][Prise en main des données].

2.  Dans le fichier TodoActivity.cs, recherchez la méthode **AddItem** et remplacez l'appel à la méthode CreateAndShowDialog par le code qui suit :
...
1.  Ajoutez le code suivant aux définitions de champ privé dans la classe **TodoItem** :

        [DataMember(Name = "createdAt")]
        public DateTime? CreatedAt { get; set; }

    <div class="dev-callout"><b>Remarque</b>
<p>L'annotation <code data-inline="1">DataMember's Name</code> demande au client de mapper la nouvelle propri&eacute;t&eacute; <code data-inline="1">CreatedAt</code> dans l'application vers la colonne <code data-inline="1">createdAt</code> d&eacute;finie dans la table TodoItem, qui a un nom diff&eacute;rent. En utilisant cette annotation, votre application peut avoir des noms de propri&eacute;t&eacute;s pour les objets qui diff&egrave;rent des noms de colonnes dans la base de donn&eacute;es SQL. Sans cette annotation, une erreur peut se produire en raison de la diff&eacute;rence de casse.</p>
</div>

2.  Dans la méthode GetView, ajoutez le code suivant juste au-dessus du code actuel qui définit `checkBox.Text` sur `currentItem.Text` :
...
5.  Dans **TodoActivity.cs**, remplacez la requête existante dans **RefreshItemsFromTableAsync** par la suivante :
