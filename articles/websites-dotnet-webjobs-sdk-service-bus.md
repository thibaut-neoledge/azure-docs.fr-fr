...
Ce guide fournit des exemples de code c# qui montrent comment déclencher un processus pendant la création ou la mise à jour d'un objet blob Azure. Les exemples de code utilisent le [Kit de développement logiciel (SDK) WebJobs](../websites-dotnet-webjobs-sdk/) version 1.x.

Le guide suppose que vous savez [comment créer un projet WebJob dans Visual Studio avec des chaînes de connexion qui pointent vers votre compte de stockage](../websites-dotnet-webjobs-sdk-get-started/).
...
Pour plus d'informations sur les scénarios de SDK WebJobs non spécifiques à Service Bus, consultez la rubrique [Utilisation du stockage de file d'attente Azure avec le Kit de développement logiciel (SDK) WebJobs](../websites-dotnet-webjobs-sdk-storage-queues-how-to/). 
