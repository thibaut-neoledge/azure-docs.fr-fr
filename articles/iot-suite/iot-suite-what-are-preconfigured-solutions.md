<properties
 pageTitle="Que sont les solutions préconfigurées IoT Azure ? | Microsoft Azure"
...
Vous pouvez déployer des solutions préconfigurées qui implémentent des scénarios IoT (Internet des objets) courants dans Microsoft Azure en utilisant votre abonnement Azure. Vous pouvez utiliser ces solutions préconfigurées : - Comme point de départ pour vos propres solutions IoT. - Pour en savoir plus sur les modèles courants rencontrés lors du développement et de la conception de solution IoT.
...
Les solutions préconfigurées disponibles sont : - Surveillance à distance - Maintenance prédictive (prochainement)
...
Le portail de la solution est une interface utilisateur web qui vous permet d’effectuer les opérations suivantes : - Afficher l’historique des alertes et de télémétrie dans le tableau de bord. - Configurer de nouveaux appareils. - Gérer et surveiller des appareils. - Envoyer des commandes à des appareils spécifiques. - Gérer des règles et des actions.

Remarque : l’affichage d’appareils du portail de la solution assure également la synchronisation du Registre d’identités d’appareils IoT Hub avec la banque d’informations d’état d’appareils enrichie dans la base de données DocumentDB.


## Étapes suivantes
Pour commencer à utiliser les solutions préconfigurées IoT Azure, explorez ces ressources : - [Vue d’ensemble des solutions préconfigurées Azure IoT](iot-suite-overview.md) - [Prise en main des solutions préconfigurées IoT](iot-suite-getstarted-preconfigured-solutions.md)
