...
# Architecture de sécurité de l’Internet des objets (IoT)
...
**Usurpation d’identité (S)** : une personne malveillante peut extraire les éléments de clé de chiffrement d’un périphérique, au niveau logiciel ou matériel, et par la suite, accéder au système avec un autre périphérique physique ou virtuel sous l’identité du périphérique dont elle a pris les éléments de clé. Les télécommandes qui peuvent allumer toutes les TV et qui sont des outils de plaisanterie populaires en sont une bonne illustration.

**Déni de service (D)** : un périphérique peut être dans l’incapacité de fonctionner ou de communiquer en interférant avec des fréquences radio ou en coupant des câbles. Par exemple, une caméra de surveillance dont l’alimentation ou la connexion réseau ont été intentionnellement mises hors-service ne signalera pas de données du tout.

**Falsification (T)** : une personne malveillante peut remplacer partiellement ou totalement les logiciels s’exécutant sur le périphérique, permettant ainsi potentiellement aux logiciels de remplacement de tirer parti de l’identité véritable du périphérique si les éléments de clé ou les fonctions de chiffrement détenant les éléments de clé ont été mis à la disposition des programmes illicites. Par exemple, une personne malveillante peut tirer parti des éléments de clé extraits pour intercepter et supprimer des données du périphérique sur le chemin de communication et les remplacer par des données erronées authentifiées avec les éléments de clé volés.

**Divulgation d’informations (I)** : si le périphérique exécute des logiciels manipulés, de tels logiciels peuvent potentiellement créer une fuite des données vers des tiers non autorisés. Par exemple, une personne malveillante peut tirer parti des éléments de clé extraits pour les injecter dans le chemin de communication entre le périphérique et un contrôleur, une passerelle de champ ou une passerelle cloud afin de détourner les informations.

**Élévation de privilège (E)** : un périphérique qui exécute une fonction spécifique peut être contraint d’en exécuter une autre. Par exemple, une vanne programmée pour s’ouvrir à mi-chemin peut à tort s’ouvrir tout le temps.
...
| Passerelle de champ | S | Authentification de la passerelle de champ auprès de la passerelle cloud (basée sur les certificats, clé prépartagée, basée sur les revendications...) | Si quelqu’un peut usurper l’identité de la passerelle de champ, elle peut alors se présenter comme un périphérique. | RSA/PSK TLS, IPSec, [RFC 4279](https://tools.ietf.org/html/rfc4279). Mêmes préoccupations de stockage et d’attestation de clés des périphériques en général ; la meilleure approche consiste à utiliser le module de plateforme sécurisée. Extension 6LowPAN pour IPSec afin de prendre en charge des réseaux de capteurs sans fil (WSN). |
...
**Déni de service** : le périphérique peut être désactivé ou activé dans un mode dans lequel la communication n’est pas possible (ce qui est intentionnel sur de nombreuses machines industrielles).

**Falsification** : le périphérique peut être reconfiguré pour fonctionner dans un état inconnu pour le système de contrôle (en dehors des paramètres d’étalonnage connus), et donc fournir des données qui peuvent être interprétées à tort comme une élévation de privilège : un périphérique qui exécute une fonction spécifique peut être contraint d’en exécuter une autre. Par exemple, une vanne programmée pour s’ouvrir à mi-chemin peut à tort s’ouvrir tout le temps.

**Déni de service** : le périphérique peut être activé dans un état dans lequel la communication n’est pas possible.

**Falsification** : le périphérique peut être reconfiguré pour fonctionner dans un état inconnu pour le système de contrôle (en dehors des paramètres d’étalonnage connus), et donc fournir des données qui peuvent être interprétées à tort.
 
**Usurpation d’identité/Falsification/Répudiation** : si le périphérique n’est pas sécurisé (ce qui est souvent le cas avec les télécommandes grand public), une personne malveillante peut manipuler l’état d’un périphérique de façon anonyme. Les télécommandes qui peuvent allumer toutes les TV et qui sont des outils de plaisanterie populaires en sont une bonne illustration.
...
**Déni de service** : les périphériques limités sont généralement sous la menace d’un déni de service s’ils écoutent activement les connexions entrantes ou les datagrammes non sollicités sur un réseau, car une personne malveillante peut ouvrir plusieurs connexions en parallèle et ne pas les traiter ou les traiter très lentement, ou le périphérique peut être saturé par un trafic non sollicité. Dans les deux cas, le périphérique peut être effectivement rendu inutilisable sur le réseau.

**Usurpation d’identité, divulgation d’informations** : les périphériques limités et à usage spécifique possèdent souvent des fonctionnalités de sécurité de type un pour tous comme la protection par mot de passe ou par code confidentiel, ou ils reposent entièrement sur l’approbation du réseau, ce qui signifie qu’ils accordent l’accès aux informations si un périphérique se trouve sur le même réseau, lequel est souvent uniquement protégé par une clé partagée. En d’autres termes, si le secret partagé pour le périphérique ou le réseau est divulgué, il est possible de contrôler le périphérique ou d’observer les données émises par celui-ci.
...
- [SDL Threat Modeling Tool](https://www.microsoft.com/sdl/adopt/threatmodeling.aspx) (Outil SDL de modélisation des menaces)
