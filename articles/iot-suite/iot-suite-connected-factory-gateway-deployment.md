...
Les étapes décrites dans cet article montrent comment déployer une passerelle à l’aide de Docker sur Windows ou Linux. La passerelle permet de se connecter à la solution préconfigurée d’usine connectée.
...
1. Configurez la passerelle pour votre IoT Hub en exécutant **une seule fois** les deux modules de passerelle à partir d’une invite de commandes, avec :
    ...
    * **&lt;IoTHubOwnerConnectionString&gt;** est la chaîne de connexion **iothubowner** que vous avez copiée à l’étape précédente. Cette chaîne de connexion est utilisée uniquement dans cette étape ; elle vous sera inutile par la suite.

    Le dossier mappé D:\\docker (l’argument `-v`) sera utilisé ultérieurement pour conserver les deux certificats X.509 utilisés par les modules de passerelle.
...
1. Vérifiez la connectivité du réseau. Essayez d’exécuter une commande ping sur votre passerelle. Dans une invite de commandes, entrez la commande `ping publisher.<your fully qualified domain name>`. Si la destination est inaccessible, ajoutez l’adresse IP et le nom de votre passerelle au fichier hosts de votre passerelle. Le fichier hosts se trouve dans le dossier « Windows\\System32\\drivers\\etc ».
...
1. Configurez la passerelle pour votre IoT Hub en exécutant **une seule fois** les deux modules de passerelle à partir d’un interpréteur de commandes, avec :
    ...
    * **&lt;IoTHubOwnerConnectionString&gt;** est la chaîne de connexion **iothubowner** que vous avez copiée à l’étape précédente. Cette chaîne de connexion est utilisée uniquement dans cette étape ; elle vous sera inutile par la suite.
    ...
    Le dossier mappé /shared (l’argument `-v`) sera utilisé ultérieurement pour conserver les deux certificats X.509 utilisés par les modules de passerelle.
...
1. Pour des raisons de sécurité, les deux certificats X.509 conservés dans le dossier /shared contiennent la clé privée. L’accès à ce dossier doit être limité aux informations d’identification utilisées pour exécuter le conteneur Docker. Pour définir les autorisations en tant que **root** uniquement, utilisez l’interpréteur de commandes `chmod` sur le dossier.

1. Vérifiez la connectivité du réseau. Essayez d’exécuter une commande ping sur votre passerelle. À partir d’un interpréteur de commandes, entrez la commande `ping publisher.<your fully qualified domain name>`. Si la destination est inaccessible, ajoutez l’adresse IP et le nom de votre passerelle au fichier hosts de votre passerelle. Le fichier hosts se trouve dans /etc.
