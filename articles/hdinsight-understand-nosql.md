<properties linkid="manage-hdinsight-understand-nosql" urlDisplayName="Understand NoSQL technologies" pageTitle="Understanding NoSQL Technologies on Azure | Azure" metaKeywords="" description="Learn how NoSQL technologies on HDInsight can help you manage data not suited to relational databases, such as big data sets and JSON documents or graphs." umbracoNaviHide="0" disqusComments="1" editor="cgronlun" manager="paulettm" services="storage,hdinsight" documentationCenter="" title="Data management: Understanding NoSQL technologies on Azure" authors="dchappell" />

Gestion des données : compréhension des technologies NoSQL sur Azure
====================================================================

**Résumé :** Azure propose un vaste choix d'options pour vos charges de travail NoSQL, mais il peut être difficile de déterminer celle qui vous convient. Ce document présente les technologies NoSQL sur Azure et explique comment choisir le service approprié pour gérer vos données non relationnelles, qui incluent des ensembles de données non structurés ou particulièrement volumineux, souvent désignés par le terme « Big Data ».

La technologie relationnelle a été pendant longtemps l'approche prédominante pour l'exploitation des données. Toutefois, lorsque vous utilisez de très grosses quantités de données de types différents, la mise à l'échelle sur de nombreux serveurs est une opération difficile. De plus, la technologie relationnelle n'est pas la meilleure solution pour gérer certains types de données tels que les documents JSON ou les graphiques. Ce guide passe en revue les différentes options, en décrivant le rôle de chacune et en expliquant dans quelles circonstances vous pouvez les utiliser.

**Contenu :**

-   Données sur Azure : les grandes lignes
-   Technologies relationnelles Azure : aperçu
-   Technologies NoSQL Azure : données opérationnelles
    -   Magasins de clés/valeurs
    -   Magasins de familles de colonnes
    -   Magasins de documents
    -   Bases de données de graphiques
-   Technologies NoSQL Azure : données analytiques
    -   Hadoop MapReduce
    -   HDInsight

**Auteur :** David Chappell, directeur de Chappell & Associates

**Date de publication :** novembre 2013

**Révision :** 1.0

**Téléchargement :** [Compréhension des technologies NoSQL sur Azure (fichier PDF)](http://go.microsoft.com/fwlink/p/?LinkId=330292)

