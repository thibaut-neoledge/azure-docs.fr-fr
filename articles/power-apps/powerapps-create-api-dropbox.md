...
# Créer une API Dropbox dans l’environnement App Service Environment de votre organisation
...
2. Dans la barre des tâches, sélectionnez **Parcourir** : ![][12]

3. Dans la liste, vous pouvez faire défiler pour sélectionner PowerApps ou taper *powerapps* : ![][13]

4. Dans **PowerApps**, sélectionnez **Gérer les API** : ![Accès aux API inscrites][4]

5. Dans **Gérer les API**, sélectionnez **Ajouter** pour ajouter la nouvelle API : ![Add API][5]

6. Tapez un **nom** descriptif pour votre API.
	
7. Dans **Source**, sélectionnez **API disponibles** pour voir les API prédéfinies, puis sélectionnez **Dropbox** : ![sélection d’une api dropbox][6]

8. Sélectionnez **Paramètres – Configurer les paramètres requis** : ![configuration des paramètres d’une API dropbox][7]
...
4. Dans **Create a new app on the Dropbox platform** (Créer une application sur la plateforme Dropbox) :
	...
	a) Dans **Choose API** (Choisir une API), sélectionnez **Dropbox API** (API Dropbox). b) Dans **Choose the type of access you need** (Choisir le type d’accès dont vous avez besoin), sélectionnez **Full Dropbox...** (Dropbox complet...). c) Tapez un nom pour votre application.

	![Création d’application Dropbox page 1][10]

5. Dans la page des paramètres de l’application :

	a) Dans **OAuth 2**, définissez la valeur **Redirect URL** (URL de redirection) sur l’URL que vous avez reçue lorsque vous avez ajouté la nouvelle API Dropbox dans le portail Azure (dans cette rubrique). Sélectionnez **Add** (Ajouter). b) Sélectionnez **Show link** (Afficher le lien) pour révéler la **question secrète de l’application** :
