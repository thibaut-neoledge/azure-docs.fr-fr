...
# Création d’une API Salesforce dans l’App Service Environment de votre organisation
...
4. Dans **New Connected App** :
	...
	a) Entrez la valeur pour **Connected App Name**. b) Entrez la valeur pour **API Name**. c) Entrez la valeur **Contact Email**. d) Sous _API (Enable OAuth Settings)_, sélectionnez **Enable OAuth Settings**, et définissez **Callback URL** sur l’URL de redirection reçue lorsque vous avez ajouté la nouvelle API Salesforce dans le portail Azure (dans cette rubrique).
