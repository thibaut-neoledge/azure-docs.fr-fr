...
[Étape 5 : établissement d'une prévision sur de nouvelles données]: #step-5-predict-over-new-data 
...
Plusieurs exemples de jeux de données sont inclus avec ML Studio et vous pouvez importer des données à partir de différentes sources. Pour cet exemple, nous allons utiliser le jeu de données inclus, **Automobile price data (Raw)**, qui représente les données des prix automobiles.
...
2. Sélectionnez le module **Colonnes de projet** et cliquez sur **Lancer le sélecteur de colonne** dans le volet Propriétés. 
	...
	- À la ligne suivante, sélectionnez **Exclure** et **Noms des colonnes**, puis cliquez dans la zone de texte. Une liste des colonnes s'affiche. Sélectionnez " normalized-losses ", qui s'ajoute dans la zone de texte. 
...
Pour l'instant, nous avons juste nettoyé les données de l'expérience. Pour afficher le jeu de données nettoyé, cliquez sur le port de sortie du module **Programme de nettoyage des valeurs manquantes** et sélectionnez **Visualiser**. Notez que la colonne " normalized-losses " n'est plus là et qu'il ne manque plus de données.
...
2. Pour sélectionner l'algorithme d'apprentissage, développez la catégorie **Machine Learning** dans la palette des modules à gauche du canevas, puis développez **Initialiser le modèle**. Ceci affiche différentes catégories de modules permettant d'initialiser un algorithme d'apprentissage. 
...
3. Recherchez et faites glisser le module **Effectuer le traitement de données pour apprentissage du modèle** vers l'expérience. Cliquez sur **Lancer le sélecteur de colonne**, puis sélectionnez la colonne *price*. Il s'agit de la valeur que notre modèle va prévoir.

	![Select "price" column][screen7]

4. Connectez le port d'entrée gauche à la sortie du module **Régression linéaire** et le port d'entrée droit à la sortie des données de formation (port gauche) du module **Fractionner**.  
...
4. Exécutez l'expérience et affichez la sortie du module **Évaluer le modèle** (cliquez sur le port de sortie et sélectionnez **Visualiser**). Les statistiques suivantes s'affichent pour notre modèle.
