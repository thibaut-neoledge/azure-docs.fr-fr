...
2. Vérifier votre numéro de téléphone en tant qu'ID d'appelant sortant avec Twilio. Pour plus d'informations sur la vérification d'un numéro de téléphone, consultez la page [https://www.twilio.com/user/account/phone-numbers/verified#][verify_phone]. Si vous souhaitez utiliser un autre numéro, vous pouvez acheter un numéro de téléphone Twilio.
Pour cet exemple, utilisez le numéro de téléphone vérifié comme valeur **From** de callform.jsp (décrit plus bas).
3. Obtenir le JAR Twilio. Sur [https://github.com/twilio/twilio-java][twilio_java_github], vous pouvez télécharger les sources GitHub et créer votre propre JAR, ou télécharger un JAR préconstruit (avec ou sans dépendances).
...
Voici les étapes générales pour exécuter votre application. Ces étapes sont détaillées sur la page [Création d'une application simple avec le plug-in Azure pour Eclipse avec Java (par Microsoft Open Technologies)][azure_java_eclipse_hello_world].
...
6. Entrez les valeurs dans le formulaire, cliquez sur **Make this call** (Effectuer cet appel), puis consultez les résultats dans makecall.jsp.
