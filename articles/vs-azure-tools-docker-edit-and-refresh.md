<properties
   pageTitle="Modifier une application web ASP.NET 5 dynamique en cours d’exécution dans un conteneur Docker local à l’aide de la modification et de l’actualisation | Microsoft Azure"
...
# Modifier une application web ASP.NET 5 dynamique en cours d’exécution dans un conteneur Docker local à l’aide de la modification et de l’actualisation
...
## Configuration et test du client Docker 
Cette section vous aide à vérifier que l’instance par défaut de Docker Machine est configurée et en cours d’exécution.

1. Créez une instance d’hôte Docker par défaut en exécutant la commande suivante à l’invite de commande.

		docker-machine create --driver virtualbox default
 
1. Vérifiez que l’instance par défaut est configurée et en cours d’exécution en exécutant la commande suivante à l’invite de commande. Vous devriez voir une instance nommée « par défaut » en cours d’exécution.

		docker-machine ls 
		
	![][0]
 
1. Définissez la valeur par défaut sur l’hôte actuel en exécutant la commande suivante à l’invite de commande.

		docker-machine env default

1. Exécutez la commande suivante pour configurer votre interpréteur de commandes.

		FOR /f "tokens=*" %i IN ('docker-machine env default') DO %i

1. La commande suivante doit afficher une réponse vide concernant les conteneurs actifs en cours d’exécution.
...
- **Génération : échec de génération de l’image, erreur lors de la vérification de la connexion TLS : l’hôte n’est pas en cours d’exécution.**
	...
	Vérifiez que l’hôte Docker par défaut est en cours d’exécution. Consultez la section [Configuration du client Docker](#configuring-the-docker-client).
...
- **Utiliser Microsoft Edge en tant que navigateur par défaut**
	...
	Si vous utilisez le navigateur Microsoft Edge, le site risque de ne pas s’ouvrir, car Edge considère l’adresse IP comme non sécurisée. Pour résoudre ce problème, procédez comme suit : 1. Dans la zone Exécuter, tapez `Internet Options`. 2. Cliquez sur **Options Internet**. 2. Cliquez sur l’onglet **Sécurité**. 3. Sélectionnez la zone **Intranet local**. 4. Cliquez sur **Sites**. 5. Ajoutez l’IP de votre machine virtuelle (dans ce cas, l’hôte Docker) à la liste. 6. Actualisez la page dans Edge et vérifiez que le site est opérationnel. 7. Pour plus d’informations sur ce problème, reportez-vous au billet de blog de Scott Hanselman [Microsoft Edge can’t see or open VirtualBox-hosted local web sites (Microsoft Edge ne peut pas voir ou ouvrir des sites web locaux hébergés sur VirtualBox)](http://www.hanselman.com/blog/FixedMicrosoftEdgeCantSeeOrOpenVirtualBoxhostedLocalWebSites.aspx).
