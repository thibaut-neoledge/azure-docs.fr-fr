...
## Bien plus que de simples applications web##
...
- **Mise à l'échelle automatique** : Web Apps permet de réaliser rapidement une montée ou une descente en puissance pour gérer la charge entrante des clients. Sélectionnez manuellement le nombre et la taille des machines virtuelles, ou configurez la mise à l'échelle automatique de vos serveurs suivant la charge ou la planification.
- **Intégration continue** : configurez l'intégration continue et les flux de travail de déploiement avec Visual Studio Online, GitHub, TeamCity, Hudson ou BitBucket. Vous pourrez ainsi générer, tester et déployer automatiquement votre application web lors de chaque archivage du code ou test d'intégration réussi.
...
- **Tests dans un environnement de production** : allez au-delà du déploiement intermédiaire en effectuant des [tests A/B][Tip] pour vérifier votre nouveau code avec une fraction configurable de votre trafic. 
- **Tâches web** : exécutez n'importe quel programme ou script sur des machines virtuelles Web Apps. Exécutez des tâches en continu ou selon un planning, et effectuez une mise à l'échelle pour une exécution sur plusieurs machines virtuelles. Pour intégrer le stockage Azure ou Service Bus, utilisez le [Kit de développement logiciel (SDK) pour les tâches Web][Webjobs] Azure.
