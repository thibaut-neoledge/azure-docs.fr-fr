<properties urlDisplayName="Using Soft Delete" pageTitle="Utilisation de la suppression r&eacute;versible dans Mobile Services (Windows Store) | Centre de d&eacute;veloppement mobile" metaKeywords="" description="D&eacute;couvrez comment utiliser la fonctionnalit&eacute; de suppression r&eacute;versible Azure Mobile Services dans votre application" metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Utilisation de la suppression r&eacute;versible dans Mobile Services" authors="wesmc" manager="dwrede" />
...
-   La suppression réversible peut permettre d'implémenter une fonctionnalité d'annulation de suppression qui permet de récupérer les données supprimées par accident.
    Toutefois, les enregistrements supprimés par ce biais prennent de l'espace dans la base de données. Vous devez donc penser à créer une tâche planifiée récurrente visant à supprimer définitivement les enregistrements supprimés de manière réversible. Pour voir un exemple, consultez les sections [Utilisation de la suppression réversible avec le serveur principal .NET][Utilisation de la suppression réversible avec le serveur principal .NET] et [Utilisation de la suppression réversible avec le serveur principal JavaScript][Utilisation de la suppression réversible avec le serveur principal JavaScript]. Votre code client doit également régulièrement appeler `PurgeAsync()` afin que les enregistrements supprimés de manière définitive ne soient pas conservés dans le magasin de données local de l'appareil.

Vue d'ensemble de cette rubrique :

1.  [Activation de la suppression réversible avec le serveur principal .NET][Activation de la suppression réversible avec le serveur principal .NET]
2.  [Activation de la suppression réversible avec le serveur principal JavaScript][Activation de la suppression réversible avec le serveur principal JavaScript]
3.  [Utilisation de la suppression réversible avec le serveur principal .NET][Utilisation de la suppression réversible avec le serveur principal .NET]
4.  [Utilisation de la suppression réversible avec le serveur principal JavaScript][Utilisation de la suppression réversible avec le serveur principal JavaScript]
...
3.  Dans la boîte de dialogue du gestionnaire de package, cliquez sur **Nuget.org** sous les mises à jour et installez la version 1.0.402 ou ultérieure des packages NuGet du [serveur principal .NET de Microsoft Azure Mobile Services][Serveur principal .NET de Microsoft Azure Mobile Services].
4.  Dans l'Explorateur de solutions de Visual Studio, développez le nœud **Contrôleurs** sous votre projet principal .NET et ouvrez la source du contrôleur. Par exemple, *TodoItemController.cs*.
5.  Dans la méthode `Initialize()` du contrôleur, transmettez le paramètre `enableSoftDelete: true` au constructeur EntityDomainManager.
...
1.  Dans le [portail de gestion][portail de gestion], cliquez sur votre service mobile. Cliquez ensuite sur l'onglet Données.
...
Pour en savoir plus sur les tâches planifiées avec le serveur principal .NET de Mobiles services, consultez la page suivante : [Planification de tâches récurrentes avec le serveur principal JavaScript de Mobile Services][Planification de tâches récurrentes avec le serveur principal JavaScript de Mobile Services]
...
<!-- Anchors. --> <!-- Images --> <!-- URLs. -->
  ...
  [Serveur principal .NET de Microsoft Azure Mobile Services]: http://go.microsoft.com/fwlink/?LinkId=513165
  ...
  [Planification de tâches récurrentes avec le serveur principal JavaScript de Mobile Services]: /fr-fr/documentation/articles/mobile-services-dotnet-backend-schedule-recurring-tasks/
  [3]: /fr-fr/documentation/articles/mobile-services-schedule-recurring-tasks/
