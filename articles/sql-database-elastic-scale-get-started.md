<properties title="Get Started with  Azure SQL Database Elastic Scale" pageTitle="Prise en main de l'infrastructure élastique de Base de données SQL Azure" description="Explication élémentaire de la fonctionnalité d'infrastructure élastique de Base de données SQL Azure avec un exemple d'application facile à exécuter." metaKeywords="sharding scaling, Azure SQL DB sharding, elastic scale" services="sql-database" documentationCenter="" manager="jhubbard" authors="sidneyh@microsoft.com"/>
...
Pour plus d'informations sur le fonctionnement de l'infrastructure élastique, consultez la rubrique [Vue d'ensemble de l'infrastructure élastique](http://go.microsoft.com/?linkid=9862592).
...
L'exemple crée une application partitionnée et explore les fonctionnalités clés de l'infrastructure élastique. Pour télécharger et exécuter l'application, suivez les étapes indiquées ci-dessous ou dans la vidéo intitulée [Infrastructure élastique - Prise en main](http://go.microsoft.com/?linkid=9862983). 

###Conditions préalables
Pour exécuter l'exemple d'application, vous devez utiliser Visual Studio et accéder à une base de données SQL Azure exécutée sur Azure Si vous n'êtes pas encore abonné à Azure, inscrivez-vous pour vous abonner à une [version d'évaluation](http://azure.microsoft.com/fr-fr/pricing/free-trial/).
####Visual Studio et Nuget

1. Visual Studio 2012 ou ultérieur et C# sont requis. Téléchargez une version gratuite à la page [Téléchargements Visual Studio](http://www.visualstudio.com/fr-fr/downloads/download-visual-studio-vs.aspx).
2. Nuget 2.7 ou ultérieur. Pour obtenir la dernière version, consultez la page traitant de l'[installation de NuGet](http://docs.nuget.org/docs/start-here/installing-nuget)
####Création d'une base de données SQL Azure

* Consultez la rubrique [Prise en main de Base de données SQL Microsoft](http://azure.microsoft.com/fr-fr/documentation/articles/sql-database-get-started/).

##Téléchargement et exécution de l'exemple d'application
L'exemple d'application **Elastic Scale with Azure SQL Database - Getting Started** (Infrastructure élastique avec Base de données SQL Azure - Prise en main) présente les aspects les plus importants du développement d'applications partitionnées à l'aide de l'infrastructure élastique de Base de données SQL Azure. Il s'intéresse aux principaux cas d'utilisation pour la [gestion des cartes de partitions](http://go.microsoft.com/?linkid=9862595), le [routage dépendant des données](http://go.microsoft.com/?linkid=9862596) et l'[interrogation de plusieurs partitions](http://go.microsoft.com/?linkid=9862597). Pour télécharger et exécuter les exemples, procédez comme suit : 
...
3. Cliquez ensuite sur **Visual C#** sous **Exemples**.
    ...
    ![Click Visual C#][3]
4. Dans la zone de recherche, tapez **Elastic Scale** pour rechercher l'exemple. Le titre **Elastic Scale with Azure SQL Database-Getting Started** s'affiche.
...
6. Dans la solution, ouvrez le fichier **app.config** de l'exemple de projet et suivez les instructions contenues dans le fichier pour ajouter le nom du serveur de base de données SQL Azure et vos informations de connexion (nom d'utilisateur et mot de passe).
...
**Remarque**   Si vous ne possédez pas SQL Server Management Studio, consultez la rubrique [Gestion de la base de données SQL Azure au moyen de SQL Server Management Studio](http://azure.microsoft.com/fr-fr/documentation/articles/sql-database-manage-azure-ssms/), où vous trouverez la marche à suivre pour vous procurer l'outil.  
...
3. **Interrogation de plusieurs partitions** : l'interrogation de partitions est illustrée dans le fichier **MultiShardQuerySample.cs**. Pour plus d'informations, consultez la rubrique [Interrogation de plusieurs partitions](http://go.microsoft.com/?linkid=9862597).
...
1. **Fractionnement d'une partition existante** : la fonctionnalité de fractionnement de partitions est fournie via le **service de fractionnement/fusion**. Vous trouverez des informations supplémentaires sur ce service ici : [Service de fractionnement/fusion](http://go.microsoft.com/?linkid=9862795).
...
Pour plus d'informations sur la tarification, consultez la page [Tarification - Base de données SQL](http://azure.microsoft.com/fr-fr/pricing/details/sql-database/).
...
-    Forum de discussion : [Forum Base de données SQL Azure](http://social.msdn.microsoft.com/forums/azure/en-US/home?forum=ssdsgetstarted)
