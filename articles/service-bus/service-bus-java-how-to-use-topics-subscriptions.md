<properties 
	pageTitle="Utilisation des rubriques Service Bus (Java) - Azure" 
...
Notez que vous pouvez utiliser la méthode **listTopics** sur les objets **ServiceBusContract** pour vérifier si une rubrique portant un nom spécifié existe déjà dans un espace de noms de service.
...
Le type de filtre le plus flexible pris en charge par les abonnements est **SqlFilter**, qui implémente un sous-ensemble de SQL92. Les filtres SQL opèrent au niveau des propriétés des messages publiés dans la rubrique. Pour plus de détails sur les expressions utilisables avec un filtre SQL, examinez la syntaxe SqlFilter.SqlExpression.
...
Pour envoyer un message à une rubrique Service Bus, votre application doit obtenir un objet **ServiceBusContract**. Le code ci-dessous montre comment créer un message pour la rubrique " TestTopic " créée précédemment à l'aide d'un espace de noms du service " HowToSample " :
...
Maintenant que vous avez appris les principes de base des files d'attente Service Bus, consultez la rubrique MSDN [Files d'attente, rubriques et abonnements Service Bus][] pour plus d'informations.
  ...
  [Files d'attente, rubriques et abonnements Service Bus]: http://msdn.microsoft.com/library/hh367516.aspx
