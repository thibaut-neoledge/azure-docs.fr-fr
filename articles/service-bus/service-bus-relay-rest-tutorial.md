<properties
   pageTitle="Didacticiel Service Bus REST | Microsoft Azure"
...
Ce didacticiel explique comment créer une simple application hôte Service Bus qui expose une interface de type REST. REST permet à un client web, par exemple un navigateur web, d’accéder à l'API Service Bus via des requêtes HTTP.
...
1. Pour créer un espace de noms dans le portail Azure, suivez les étapes de la rubrique [Création ou modification d’un espace de noms de service Service Bus](https://msdn.microsoft.com/library/hh690931.aspx).

2. Dans la fenêtre principale du portail Azure, cliquez sur le nom d’espace de noms du service que vous avez créé à l’étape précédente.
...
5. Répétez l’étape précédente pour ajouter une référence à l’assemblage **System.ServiceModel.Web.dll**.
...
5. Ajoutez des références aux assemblages **System.Drawing.dll**, **System.Runtime.Serialization.dll** et **Microsoft.ServiceBus.dll** au projet, et ajoutez également les instructions connexes `using` suivantes.
...
1. Cliquez avec le bouton droit sur le projet **ImageListener**. Cliquez ensuite sur **Ajouter**, puis sur **Nouvel élément**.
...
Maintenant que vous avez créé une application qui utilise le service de relais Service Bus, consultez les articles suivantes pour en savoir plus sur la messagerie par relais.
