<properties 
   pageTitle="Didacticiel Service Bus REST"
   description="Créez une simple application hôte Service Bus qui expose une interface de type REST."
...
Cette rubrique explique comment créer une simple application hôte Service Bus qui expose une interface de type REST. REST permet à un client web, par exemple un navigateur web, d’accéder à l'API Service Bus via des requêtes HTTP.
...
1. Pour créer un espace de noms dans le portail de gestion Azure, suivez les étapes de la rubrique [Création ou modification d’un espace de noms de service Service Bus](https://msdn.microsoft.com/library/hh690931.aspx).

2. Dans la fenêtre principale du portail de gestion Azure, cliquez sur le nom d'espace de noms du service que vous avez créé à l'étape précédente.
...
4. Ajoutez une référence à **System.ServiceModel.dll** au projet :
	1. Dans l'**Explorateur de solutions**, cliquez avec le bouton droit sur le dossier **Références**, puis cliquez sur **Ajouter une référence**. 
	2. Cliquez sur l’onglet **.NET** dans la boîte de dialogue **Ajouter une référence** et faites défiler la fenêtre jusqu'à l’élément **System.ServiceModel**. Sélectionnez cet élément, puis cliquez sur **OK**.

5. Répétez l'étape précédente pour ajouter une référence à l’assembly **System.ServiceModel.Web.dll**.

6. Ajoutez les instructions `using` aux espaces de noms **System.ServiceModel**, **System.ServiceModel.Channels**, **System.ServiceModel.Web** et **System.IO**.
...
12. Dans le menu **Générer**, cliquez sur **Générer la solution** pour confirmer si votre travail est correct.
...
5. Ajoutez des références aux assemblys **System.Drawing.dll**, **System.Runtime.Serialization.dll** et **Microsoft.ServiceBus.dll** au projet, et ajoutez également les instructions connexes `using` suivantes :
...
7. À partir du menu **Build**, cliquez sur **Générer la solution** pour générer la solution complète.
...
1. Créez l'hôte de service web en utilisant l'adresse URI créée précédemment dans cette section.
...
Maintenant que vous avez créé une application qui utilise le service de relais Service Bus, consultez les rubriques suivantes pour en savoir plus sur la messagerie par relais :

- [Présentation de l'architecture d'Azure Service Bus](fundamentals-service-bus-hybrid-solutions.md#relays)
