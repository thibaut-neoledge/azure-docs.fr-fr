...
* [Connecter votre application Chrome au concentrateur de notification](#connect-app)
...
1. Accédez au site web [Google Cloud Console], connectez-vous avec vos informations d’identification de compte Google, puis cliquez sur le bouton **Créer un projet**. Saisissez un **nom de projet** adéquat et cliquez sur le bouton **Créer**.

   	![][1]

2. Notez le **numéro du projet** que vous venez de créer, affiché sur la page des projets. Il vous servira d’**ID d’expéditeur GCM** dans l’application Chrome pour votre inscription auprès de GCM.

   	![][2]

3. Dans le volet gauche, cliquez sur **API & authentification**, puis faites défiler la page et cliquez sur la touche bascule pour activer **Google Cloud Messaging pour Android**. Vous n’avez pas besoin d’activer *Google Cloud Messaging pour Chrome*. Il est également possible que le nom passe simplement à *Google Cloud Messaging* à l’avenir.

   	![][3]

4. Dans le volet gauche, cliquez sur **Informations d'identification** -> **Créer une nouvelle clé** -> **Clé de serveur** -> **Créer**

   	![][4]

5. Notez la valeur **clé API** du serveur. Vous configurerez ensuite cela dans votre Notification Hub afin qu’il puisse envoyer des notifications Push vers GCM.
...
2. Cliquez sur **Services d’application** -> **Service Bus** -> **Concentrateur de notification** -> **Création rapide**. Tapez un nom pour votre concentrateur de notification, sélectionnez la région souhaitée, puis cliquez sur **Créer un concentrateur de notification**.
...
7. Dans l’onglet **Configuration**, faites défiler jusqu’à la section **Paramètres de Google Cloud Messaging**, entrez la valeur de **clé API** obtenue dans la section précédente, puis cliquez sur **Enregistrer**.
...
##<a id="connect-app"></a>Connexion de votre application Chrome au concentrateur de notification
...
Dans cet exemple simple, vous avez envoyé des notifications à votre application Chrome. En savoir plus sur les concentrateurs de notification dans la [Vue d’ensemble des concentrateurs de notification]. Afin de cibler des utilisateurs spécifiques, consultez le didacticiel [Notification des utilisateurs via Azure Notification Hubs], et, si vous souhaitez segmenter vos utilisateurs par groupes d’intérêt, vous pouvez lire [Dernières nouvelles via Azure Notification Hubs].
