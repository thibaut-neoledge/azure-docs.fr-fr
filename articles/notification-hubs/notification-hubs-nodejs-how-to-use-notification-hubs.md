<properties
	pageTitle="Envoi de notifications Push avec Azure Notification Hubs et Node.js"
...
2.  Dans la fenêtre de commande, tapez **npm install azure**. Cette opération doit générer un résultat similaire à celui ci-dessous (les numéros de version peuvent différer) :
...
>[AZURE.NOTE] Plus d’informations sur l’installation de NPM, voir le [Blog NPM](http://blog.npmjs.org/post/85484771375/how-to-install-npm) officiel.
...
2. Sélectionnez **Notification Hubs**, puis recherchez le hub à utiliser pour l’exemple. Si vous avez besoin d’aide pour créer un hub de notification, vous pouvez consulter le [Didacticiel de prise en main de Windows Store](notification-hubs-windows-store-dotnet-get-started.md).
...
> [AZURE.NOTE] Vous pouvez également extraire la chaîne de connexion à l’aide de l’applet de commande **Get-AzureSbNamespace** fournie par [Azure PowerShell](../powershell-install-configure.md) ou de la commande **azure sb namespace show** avec les [outils en ligne de commande Azure (Azure CLI)](../xplat-cli-install.md).
...
Pour plus d’informations sur le format de charge utile, voir la section **Charge utile** du document relatif à l’[implémentation du serveur GCM](http://developer.android.com/google/gcm/server.html#payload).
...
### Envoi de notifications Push à des applications iOS
...
### Envoi de notifications Push à des applications Windows Phone
...
* **NotificationClass** : priorité de la notification. Pour les valeurs valides, voir la section **Éléments d’en-tête HTTP** de l’article [Notifications Push d’un serveur](http://msdn.microsoft.com/library/hh221551.aspx).
* **Options** : en-têtes de demande facultatifs.
* **Callback** : fonction de rappel.

Pour la liste des options **TargetName**, **NotificationClass** et d’en-tête valides, voir [Notifications Push](http://msdn.microsoft.com/library/hh221551.aspx).
...
### Envoi de notifications Push à des applications de la plate-forme Windows universelle (UWP)
...
Pour obtenir la liste des types et en-têtes de demande valides, voir [En-têtes de demande et de réponse des services de notifications Push](http://msdn.microsoft.com/library/windows/apps/hh465435.aspx).

Le code suivant utilise l’instance **WnsService** exposée par **NotificationHubService** pour envoyer une notification toast à une application UWP :
...
-   Voir la référence MSDN pour [Azure Notification Hubs](https://msdn.microsoft.com/library/azure/jj927170.aspx).
-   Pour d’autres exemples et des détails d’implémentation, visitez le dépôt [Azure SDK pour Node] sur GitHub.

  [Azure SDK pour Node]: https://github.com/WindowsAzure/azure-sdk-for-node
