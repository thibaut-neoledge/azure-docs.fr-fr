<properties pageTitle="Autorisation côté service (HTML) | Centre de développement mobile" description="Découvrez comment autoriser des utilisateurs sur le serveur principal JavaScript d'Azure Mobile Services." services="mobile-services" documentationCenter="" authors="ggailey777" manager="dwrede" editor=""/>
...
4. Remplacez le script existant par la fonction suivante, puis cliquez sur **Enregistrer**.
    ...
    > [AUZRE.IMPORTANT] Le schéma dynamique doit être activé la première fois que ce script d'insertion s'exécute. Avec le schéma dynamique activé, Mobile Services ajoute automatiquement la colonne **userId** à la table **TodoItem** lors de la première exécution. Par défaut, le schéma dynamique est activé pour un nouveau service mobile. Il doit être désactivé avant que l'application ne soit publiée.
...
1. Dans un navigateur Web, accédez à la page index.html de l'application, puis connectez-vous au fournisseur d'identité choisi. 
   	...
   	Notez que cette fois, bien que certains éléments provenant des didacticiels précédents figurent déjà dans la table TodoItem, aucun élément n'est renvoyé. Cela est dû au fait que les éléments précédents ont été insérés sans la colonne userId et comportent maintenant des valeurs null.

3. Dans l'application, entrez du texte dans **Entrer une nouvelle tâche**, puis cliquez sur **Ajouter**.

   	![][3]

   	Cette action insère à la fois le texte et la valeur userId dans la table TodoItem du service mobile. Le nouvel élément disposant de la valeur userId correcte, il est renvoyé par le service mobile et affiché dans la seconde colonne.
