...
### <a name="cors"></a>CORS
En lieu et place d’un paramètre d’application **MS_CrossDomainOrigins** séparé par des virgules, il existe désormais dans le portail de gestion Azure un panneau qui permet de configurer la fonctionnalité CORS. Il est aussi possible de la configurer à l’aide des outils Resource Manager, tels que Microsoft Azure PowerShell, CLI ou l’ [Explorateur de ressources](https://resources.azure.com/). Définissez la propriété **cors** du type de ressource **Microsoft.Web/sites/config** pour votre ressource **&lt;nom du site&gt;/web**. Par exemple :
