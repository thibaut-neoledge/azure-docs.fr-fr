<properties title="Troubleshooting Azure Websites in Visual Studio" pageTitle="Dépannage des sites web Azure dans Visual Studio" metaKeywords="dépannage déboguer site web azure journalisation de suivi" description="Découvrez comment dépanner un site web Azure en utilisant des outils de débogage, de suivi et de journalisation à distance intégrés à Visual Studio 2013." metaCanonical="" services="web-sites" documentationCenter=".NET" authors="tdykstra" manager="wpickett" solutions="" />
...
Ce didacticiel fonctionne avec l'environnement de développement, le projet web et le site web Azure que vous avez configurés dans le didacticiel [Prise en main d'Azure et d'ASP.NET][GetStarted]. Pour les sections WebJobs, vous aurez besoin de l'application que vous créez dans [Prise en main du Kit de développement logiciel (SDK) WebJobs Azure][GetStartedWJ].
...
Cette section montre comment procéder au débogage distant en utilisant le projet que vous créez dans le didacticiel [Prise en main d'Azure et d'ASP.NET][GetStarted].
...
6. Pour Visual Studio 2012 avec Update 4 :<a id="vs2012"></a>
	...
	* Dans la zone **Qualificateur**, enter entrez l'URL de votre site web, sans le préfixe `http://`. 
...
### <a name="remotedebugwj"></a> Débogage distant des tâches web
...
* Vérifiez que l'attribut `debug` de l'élément `compilation` du fichier *Web.config* a la valeur true. Il est défini sur true par défaut lorsque vous publiez une configuration de version de débogage.
...
* ajout d'instructions de traçage au projet web que vous avez créé dans [Prise en main d'Azure et d'ASP.NET][GetStarted].
...
3. Ajoutez un <a href="http://msdn.microsoft.com/fr-fr/library/vstudio/6915t83k(v=vs.100).aspx">élément trace</a> sous `<system.web>` dans le fichier Web.config, comme dans l'exemple suivant :
...
4. Dans la barre d'adresses de la fenêtre de navigateur, ajoutez un caractère supplémentaire à l'URL pour provoquer une erreur 404 (par exemple, `http://localhost:53370/Home/Contactx`), puis appuyez sur Entrée.
...
* [How to Monitor Web Sites](/fr-fr/manage/services/web-sites/how-to-monitor-websites/)
...
* [Présentation du débogage distant de Sites Web Azure](/blog/2014/05/06/introduction-to-remote-debugging-on-azure-web-sites/).
