...
Vous pouvez attacher des disques qui soient vides ou non. Dans les deux cas, les disques sont en réalité des fichiers .vhd conservés dans un compte de stockage Azure. Après avoir attaché le disque, vous devrez également l'initialiser pour le rendre opérationnel. 
...
3. Dans la fenêtre SSH, tapez la commande suivante pour créer un périphérique, puis entrez le mot de passe du compte :

		# sudo fdisk /dev/sdc

	>[AZURE.NOTE] Dans cet exemple, vous devrez peut-être utiliser  `sudo -i` avec certaines distributions si /sbin ou /usr/sbin ne sont pas dans votre `$PATH`.
