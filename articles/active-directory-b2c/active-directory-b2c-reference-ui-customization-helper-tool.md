...
---
# Azure Active Directory B2C : un outil d’assistance utilisé pour illustrer la fonctionnalité de personnalisation de l’interface utilisateur de la page
...
2. Cliquez sur **Sign-up or sign-in policies** (Stratégies d’inscription ou de connexion), puis sur votre stratégie (par exemple, « b2c\_1\_sign\_up\_sign\_in »).
3. Cliquez sur **Page UI customization** (Personnalisation de l’interface utilisateur de la page) puis sur **Unified sign-up or sign-in page** (Page d’inscription ou de connexion unifiée).
...
6. Suivez la même procédure pour la **page d’inscription à un compte social**. Cliquez deux fois sur **OK** pour fermer les panneaux de personnalisation de l'interface utilisateur.
...
3. Fournissez un **Nom** pour le compte de stockage (par exemple, « contoso ») et sélectionnez les options appropriées pour **Niveau tarifaire**, **Groupe de ressources** et **Abonnement**. Assurez-vous que l’option **Épingler au tableau d’accueil** est activée. Cliquez sur **Create**.
...
9. Connectez-vous au [portail Azure](https://portal.azure.com/).
10. Cliquez sur **+ Nouveau** -> **Données et stockage** -> **Compte de stockage**. Vous avez besoin d’un abonnement Azure pour créer un compte Azure Blob Storage. Vous pouvez vous inscrire gratuitement sur le [site web Azure](https://azure.microsoft.com/pricing/free-trial/).
11. Sélectionnez **Stockage d’objets blob** sous **Type de compte** et conservez les autres valeurs par défaut. Si vous le souhaitez, vous pouvez modifier le groupe de ressources et l’emplacement. Cliquez sur **Create**.
12. Revenez au tableau d’accueil et cliquez sur le compte de stockage que vous venez de créer.
13. Dans la section **Résumé**, cliquez sur **+ Conteneur**.
14. Fournissez un **Nom** pour le conteneur (par exemple, « b2c ») et sélectionnez **Blob** comme le **type d’accès**. Cliquez sur **OK**.
15. Ouvrez les **propriétés** du conteneur et notez l’URL du conteneur, qui doit ressembler par exemple à `https://contoso.blob.core.windows.net/b2c`. Fermez le panneau du conteneur.
16. Dans le panneau du compte de stockage, cliquez sur l’icône **Clé** et notez les valeurs des champs **Nom de compte de stockage** et **Clé d’accès primaire**.
