...
Avant de demander un jeton d’accès, vous devez inscrire une API web et publier les autorisations qui peuvent être accordées à l’application cliente. Démarrez en suivant les étapes décrites dans la section [Inscrire une API web](active-directory-b2c-app-registration.md#register-a-web-api).
...
1. Inscrivez une application cliente ([application web](active-directory-b2c-app-registration.md#register-a-web-application) ou [cliente native](active-directory-b2c-app-registration.md#register-a-mobilenative-application)) si vous ne l’avez pas déjà fait.
1. Dans le panneau Paramètres de votre application, sélectionnez **Accès API**.
...
> [!NOTE]
> À ce stade, les domaines personnalisés ne sont pas pris en charge avec les jetons d’accès. Vous devez utiliser votre domaine ID_votre_locataire.onmicrosoft.com dans l’URL de la demande.
