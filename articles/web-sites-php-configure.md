...
Ce guide vous explique comment configurer le runtime PHP intégré dans Sites Web Azure, fournir un runtime PHP personnalisé et activer des extensions dans Sites Web Azure. Pour utiliser Sites Web Azure, inscrivez-vous pour bénéficier d'une [évaluation gratuite][évaluation gratuite]. Pour tirer pleinement parti de ce guide, vous devez d'abord créer un site PHP dans Sites Web Azure (voir la page [Didacticiels du Centre de développement PHP][Didacticiels du Centre de développement PHP]). Pour obtenir des informations générales sur la configuration de sites dans Sites Web Azure, consultez la page [Configuration de sites web][Configuration de sites web].

## Sommaire

-   [Présentation de Sites Web Azure][Présentation de Sites Web Azure]
-   [Modification de la configuration PHP par défaut][Modification de la configuration PHP par défaut]
-   [Activation des extensions dans le runtime PHP intégré][Activation des extensions dans le runtime PHP intégré]
-   [Utilisation d'un runtime PHP personnalisé][Utilisation d'un runtime PHP personnalisé]
...
Par défaut, PHP 5.4 est installé et immédiatement disponible lorsque vous créez un site web Azure. Pour afficher la révision de version disponible, sa configuration par défaut et les extensions activées, la méthode idéale consiste à déployer un script qui appelle la fonction [phpinfo()][phpinfo()].
...
1.  Accédez au tableau de bord de votre site web dans le portail Azure, puis cliquez sur **Configurer**.
    ...
    ![Onglet Configurer sur le tableau de bord des sites Web][Onglet Configurer sur le tableau de bord des sites Web]

2.  Cliquez sur PHP 5.5.

    ![Sélectionner la version PHP][Sélectionner la version PHP]

3.  Cliquez sur **Enregistrer** au bas de la page.

    ![Enregistrer les paramètres de configuration][Enregistrer les paramètres de configuration]

Quelle que soit l'exécution PHP intégrée, vous pouvez modifier toute option de configuration qui n'est pas une directive de niveau système uniquement en procédant comme suit. Pour plus d'informations sur les directives de niveau système uniquement, consultez la page [Liste des directives du php.ini][Liste des directives du php.ini].

1.  Ajoutez un fichier [.user.ini][.user.ini] à votre répertoire racine.
...
Au lieu d'utiliser le fichier `.user.ini`, vous pouvez utiliser la fonction [ini\_set()][ini\_set()] dans des scripts afin de définir des options de configuration qui ne sont pas des directives de niveau système.

## <a name="EnableExtDefaultPHP"></a> Activation des extensions dans l'exécution PHP par défaut

Comme indiqué dans la section précédente, la méthode idéale pour afficher la version PHP par défaut, sa configuration par défaut et les extensions activées consiste à déployer un script qui appelle [phpinfo()][phpinfo()]. Pour activer des extensions supplémentaires, procédez comme suit.
...
5.  Dans la section **App Settings**, créez une clé **PHP\_EXTENSIONS** et une valeur **bin\\your-ext-file**. Pour activer plusieurs extensions, insérez une liste de fichiers `.dll` séparés par des virgules.
    ...
    ![Activer une extension dans les paramètres d'application][Activer une extension dans les paramètres d'application]
...
7.  Dans la section **Mappages de gestionnaires**, ajoutez `*.php` à EXTENSION et ajoutez le chemin d'accès à l'exécutable `php-cgi.exe`. Si vous avez placé votre runtime PHP dans le répertoire `bin` situé à la racine de votre application, le chemin sera `D:\home\site\wwwroot\bin\php\php-cgi.exe`.
    ...
    ![Indiquer le gestionnaire dans les mappages][Indiquer le gestionnaire dans les mappages]
...
-   [Configuration, surveillance et mise à l'échelle de vos sites Web dans Azure][Configuration, surveillance et mise à l'échelle de vos sites Web dans Azure]
