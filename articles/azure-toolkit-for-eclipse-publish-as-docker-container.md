...
# <a name="how-to-publish-a-web-app-as-a-docker-container-using-the-azure-toolkit-for-eclipse"></a>Comment publier une application web en tant que conteneur Docker à l’aide du kit de ressources Azure pour Eclipse

Les conteneurs Docker sont une méthode largement utilisée pour déployer des applications web dans lesquelles les développeurs peuvent consolider tous leurs fichiers de projet et dépendances dans un seul package de déploiement sur un serveur. Le kit de ressources Azure pour Eclipse simplifie ce processus pour les développeurs Java en ajoutant des fonctionnalités *Publish as Docker Container* (Publier en tant que conteneur Docker) pour le déploiement dans Microsoft Azure, et les étapes décrites dans cet article vous permettront de publier vos applications dans Azure en tant que conteneurs Docker.
...
1. Utilisez l’une des méthodes suivantes pour lancer l’Assistant Publish as Docker Container (Publier en tant que conteneur Docker) :
   ...
   * Cliquez avec le bouton droit sur votre projet dans la vue **Navigateur**, puis cliquez sur **Azure** et **Publish as Docker Container** (Publier en tant que conteneur Docker) :  ![Publish as Docker Container][PUB01] (Publier en tant que conteneur Docker)

   * Cliquez sur le menu **Publier** dans la barre d’outils Eclipse, puis cliquez sur **Publish as Docker Container** (Publier en tant que conteneur Docker) :  ![Publish as Docker Container][PUB02] (Publier en tant que conteneur Docker)

1. Lorsque l’Assistant **Deploy Docker Container on Azure** (Déployer le conteneur Docker dans Azure) s’affiche, vous verrez une boîte de dialogue similaire à la suivante : ![Assistant Deploy Docker Container on Azure (Déployer le conteneur Docker dans Azure)][PUB03]

   a. Entrez un nom unique dans la zone de texte pour votre hôte Docker dans **Docker image name** (Nom de l’image Docker). (L’Assistant va créer automatiquement un nom pour vous, mais vous pouvez le modifier que si vous voulez).
...
1. Pour créer un hôte Docker, cliquez sur le bouton **Ajouter** ; la boîte de dialogue **Create Docker Host** (Créer un hôte Docker) s’affiche.
      ![Assistant Deploy Docker Container on Azure Déployer le conteneur Docker dans Azure)][PUB04]
   ...
   c. Choisissez l’une des options suivantes pour la connexion de machine virtuelle de votre hôte Docker :    ![Create Docker Host][PUB05] (Créer un hôte Docker)
      ...
      * **Import credentials from Azure Key Vault** (Importer des informations d’identification Azure Key Vault) : vous permet de spécifier un jeu d’informations d’identification précédemment enregistré stocké dans votre abonnement Azure.

      > [!NOTE]
      > Un Azure Key Vault créé avec un compte ou un principal de service spécifique ne sera pas automatiquement accessible par un autre compte ou principal de service qui partage le même abonnement. Pour permettre à un autre compte ou principal de service d’utiliser le Key Vault, vous devrez utiliser le portail Azure pour ajouter le compte ou le principal de service.
...
1. Après avoir indiqué toutes les options ci-dessus, cliquez sur **Terminer**.
