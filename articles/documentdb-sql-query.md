<properties title="Interrogation avec le langage&nbsp;SQL de DocumentDB" pageTitle="Interrogation avec le langage&nbsp;SQL de DocumentDB | Azure" description="DocumentDB prend en charge l'interrogation de documents au moyen d'une syntaxe semblable &agrave; SQL sur des documents JSON hi&eacute;rarchiques sans n&eacute;cessiter de sch&eacute;ma explicite ou de cr&eacute;ation d'index secondaires." metaKeywords="" services="documentdb"  documentationCenter="" solutions="data-management" authors="bradsev" manager="jhubbard" editor="cgronlun" videoId="" scriptId="" />

<tags ms.service="documentdb" ms.workload="data-services" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="08/20/2014" ms.author="spelluru" />

# Interrogation de DocumentDB

Azure DocumentDB prend en charge l'interrogation de documents à l'aide d'un langage SQL convivial sur des documents JSON hiérarchiques. DocumentDB n'utilise pas de schéma. Grâce à son engagement au modèle de données JSON directement dans son moteur de base de données, DocumentDB permet une indexation automatique des documents JSON sans nécessiter de schéma explicite ou de création d'index secondaires.
Durant la conception du langage de requête pour DocumentDB, nous avions deux objectifs en tête :

-   **Exploiter le SQL** : au lieu d'inventer un langage de requête, nous voulions exploiter le langage SQL existant. Après tout, le SQL est l'un des langages de requête les plus conviviaux et populaires. Le langage SQL d'Azure DocumentDB fournit un modèle de programmation formel pour développer des requêtes enrichies destinées aux documents JSON.
-   **Étendre le SQL** : comme une base de données de document JSON peut exécuter JavaScript directement dans le moteur de base de données, nous avons voulu utiliser le modèle de programmation de JavaScript comme base pour notre langage de requête SQL. Le langage de requête SQL d'Azure DocumentDB est inclus dans le système de type, dans l'évaluation d'expression et dans l'appel de fonction de JavaScript. En retour, cela fournit un modèle de programmation naturel pour les projections relationnelles, la navigation hiérarchique entre les documents JSON, les jointures réflexives, l'appel de fonctions définies par l'utilisateur, le tout écrit entièrement en JavaScript, parmi d'autres fonctionnalités.
...
Pour voir comment le langage SQL de DocumentDB fonctionne, nous allons commencer par quelques documents JSON simples sur lesquels nous allons appliquer certaines requêtes simples. Prenez ces deux documents JSON relatifs à deux familles. Notez qu'avec DocumentDB, nous n'avons pas besoin de créer de schéma ou d'index secondaire de façon explicite. Nous allons simplement insérer les documents JSON dans une collection et une requête DocumentDB.
Voici un document JSON simple pour la famille Andersen, les parents, les enfants (et leurs animaux), leur adresse et leurs informations d'enregistrement. Le document se compose de chaînes, de nombres, d'opérateurs booléens, de tableaux et de propriétés imbriquées.
...
-   Comme le langage SQL de DocumentDB fonctionne avec les valeurs JSON, il traite des entités d'arborescence au lieu des lignes et des colonnes. C'est pourquoi ce langage vous permet de faire référence aux nœuds de l'arborescence à n'importe quel niveau arbitraire, comme `Node1.Node2.Node3…..Nodem`, tout comme le SQL relationnel se rattachant à une référence en deux parties de `<table>.<column>`.
...
-   DocumentDB prend uniquement en charge les documents JSON stricts. Cela signifie que le système de type et les expressions peuvent uniquement traiter des types JSON. Veuillez consulter [spécification de JSON] (http://www.json.org/) pour plus d'informations.
-   Une collection DocumentDB est un conteneur sans schéma pour vos documents JSON. Les relations des entités de données dans et entre les documents d'une collection sont capturées de façon implicite par le contenant et non par les relations PK-FK. Cet aspect est important dans le cadre des liaisons entre documents (ce sujet est abordé plus loin dans ce didacticiel).
...
-   Indexation des documents sans requérir de schéma : le sous-système d'indexation ne requiert pas d'information de schéma ou n'établit pas de supposition a priori sur les schémas des documents.

-   Prise en charge de requêtes hiérarchiques et relationnelles enrichies et efficaces : l'index prend en charge le langage de requête DocumentDB de manière efficace, notamment la prise en charge des projections hiérarchiques et relationnelles.

-   Prise en charge des requêtes cohérentes face à des volumes soutenus d'écritures : dans le cas des charges de travail à débits d'écriture élevés avec des requêtes cohérentes, l'index est mis à jour de manière incrémentielle, efficacement et en ligne, face à des volumes soutenus d'écritures. La mise à jour d'index cohérente est cruciale pour servir les requêtes en respectant le niveau de cohérence défini par l'utilisateur pour le service du document.

-   Prise en charge de l'infrastructure mutualisée : étant donné le modèle basé sur la réservation pour la gouvernance des ressources sur les clients, les mises à jour d'index sont effectuées dans le budget des ressources système (processeur, mémoire, IOPS) allouées par réplica.
...
Consultez la page [exemples de DocumentDB] (http://code.msdn.microsoft.com/Azure-DocumentDB-NET-Code-6b3da8af\#content) sur MSDN pour apprendre à configurer la stratégie d'indexation d'une collection. Nous allons à présent détailler davantage le langage SQL de DocumentDB.
...
La clause `FROM <from_specification>` est facultative, sauf si la source est filtrée/projetée ultérieurement dans la requête. L'objectif de cette clause est de spécifier la source des données à partir de laquelle la requête doit fonctionner. Généralement, l'intégralité de la collection est la source, mais parfois, il peut s'agir plutôt d'un sous-ensemble de la collection.

Une requête telle que `SELECT * FROM Families` indique que l'intégralité de la collection Families est la source de l'énumération. Vous pouvez utiliser un identificateur **ROOT** spécial pour représenter la collection au lieu d'utiliser le nom de la collection.
Les règles de liaison appliquées par requête sont les suivantes :

-   Un alias peut s'appliquer à la collection, comme dans `SELECT f.id FROM Families AS f` ou simplement `SELECT f.id FROM Families f`. Ici, `f` est l'équivalent de `Families`. `AS` est un mot clé facultatif pour appliquer un alias à l'identificateur.

-   Notez qu'une fois l'alias appliqué, vous ne pouvez plus lier la source d'origine. Par exemple, `SELECT Familes.id FROM Families f` n'a pas une syntaxe valide, car l'identificateur « Families » ne peut plus être résolu.

-   Toutes les propriétés à référencer doivent être **entièrement qualifiées**. Si le schéma strict n'est pas respecté, ceci est renforcé pour éviter toute liaison ambiguë. Par conséquent, `SELECT id FROM Families f` n'a pas une syntaxe valide, car l'`id` de propriété n'est pas lié.

## Sous-documents

Vous pouvez également réduire la source à un sous-ensemble. Par exemple, si vous voulez uniquement énumérer une sous-arborescence dans chaque document, la sous-racine peut devenir la source, comme dans l'exemple suivant.
...
Même si la source est un tableau dans l'exemple précédent, il est possible d'utiliser un objet en tant que source, comme indiqué dans l'exemple suivant. Toute valeur JSON valide (définie) pouvant être trouvée dans la source est incluse dans les résultats de la requête. Si certaines familles n'ont pas de valeur `address.state`, elles sont exclues du résultat de la requête.
...
La requête suivante demande les documents contenant une propriété name dont la valeur est `AndersenFamily`. Tous les documents n'ayant pas la propriété name ou ceux dont la valeur de la propriété name ne correspond pas à `AndersenFamily` sont exclus.
...
Les opérateurs unaires **+,-, ~ et NOT** sont également pris en charge et peuvent être utilisés dans des requêtes comme indiqué ci-dessous :
...
En plus des opérateurs binaires et unaires, les références de propriétés sont également autorisées. Par exemple, `SELECT * FROM Families f WHERE f.isRegistered` renvoie les documents JSON contenant la propriété `isRegistered` et la valeur de la propriété est égale à la valeur JSON`true`. Toutes les autres valeurs (false, null, Undefined, <number>, <string>, <object>, <array> etc.) entraînent l'exclusion du document source des résultats.
...
Pour les autres opérateurs de comparaison tels que \>, \>=, !=, \< et \<=
...
Ces opérateurs s'appliquent aux valeurs booléennes. Les tableaux de vérités logiques de ces opérateurs sont indiqués ci-dessous.
...
La clause SELECT (**`SELECT <select_list>`**) est obligatoire et indique les valeurs qui seront récupérées dans la requête, comme dans ANSI-SQL. Le sous-ensemble filtré au début des documents source est transmis à la phase de projection, où les valeurs JSON spécifiées sont récupérées et un nouvel objet JSON est construit, pour chaque entrée qui lui est transmise.
...
﻿À présent, nous allons développer l'exemple précédent en appliquant des alias de valeurs de façon explicite. **AS** est le mot clé utilisé pour l'application d'alias. Notez que cela est facultatif, comme indiqué lors de la projection de la seconde valeur en tant que `NameInfo`.
...
En plus des références de propriété, la clause **SELECT** prend également en charge des expressions scalaires telles que les constantes, les expressions arithmétiques, les expressions logiques, etc. Par exemple, voici une simple requête « Hello World ».
...
Une autre fonctionnalité clé du langage SQL de DocumentDB est la possibilité de créer un tableau ou un objet. Dans l'exemple précédent, notez que nous avons créé un objet JSON. De même, vous pouvez également construire des tableaux comme indiqué ci-dessous.
...
Le mot clé **VALUE** fournit une méthode pour renvoyer une valeur JSON. Par exemple, la requête indiquée ci-dessous renvoie le `"Hello World"` scalaire au lieu de `{$1: "Hello World"}`.
...
Nous prenons en charge l'opérateur spécial (\*) pour projeter le document en l'état. Une fois utilisé, il doit être le seul champ projeté. Si une requête comme `SELECT * FROM Families f` est valide, `SELECT VALUE * FROM Families f` et `SELECT *, f.id FROM Families f` ne le sont pas.
...
Nous avons ajouté une nouvelle construction via le mot clé **IN** dans le langage SQL de DocumentDB pour fournir une prise en charge pour l'itération via les tableaux JSON. La source FROM fournit une prise en charge pour l'itération. Commençons par l'exemple suivant :
...
﻿À présent, recherchons une autre requête effectuant une itération sur les enfants de la collection. Notez la différence dans le tableau de sortie. Cet exemple fractionne `children` et regroupe les résultats en un seul tableau.
...
Dans une base de données relationnelle, il est très important de joindre les tables. Ceci est la conséquence logique de la conception de schémas normalisés. Au contraire, DocumentDB traite des modèles de données dénormalisés de documents sans schéma. Il s'agit de l'équivalent logique d'une « jointure réflexive ».

La syntaxe prise en charge dans le langage est <from_source1> JOIN <from_source2> JOIN ... JOIN <from_sourcen>. D'une façon générale, ceci renvoie un ensemble de **N** tuples (un tuple avec les valeurs **N**). Les valeurs de chaque tuple sont produites par l'itération de tous les alias de la collection sur leurs ensembles respectifs. En d'autres termes, il s'agit d'un produit croisé complet des ensembles participants à la jointure.

Les exemples suivants illustrent le fonctionnement de la syntaxe JOIN. Vous y constaterez que le résultat est vide, car le produit croisé de chaque document depuis la source et un ensemble vide est vide.
...
L'exemple suivant est une jointure plus conventionnelle :
...
La première chose à noter est que le `from_source` de **JOIN** est ici un itérateur. Dans ce cas, le flux est le suivant

-   Développez chaque élément enfant **c** dans le tableau.
-   Appliquez un produit croisé avec la racine du document **f** avec chaque élément enfant **c** arrondi à la première étape.
-   Finalement, la propriété du nom de l'objet racine **f** est projetée seule.
...
`AndersenFamily` a un enfant mais pas d'animal. Le produit croisé renvoie une ligne (1*1*1) à partir de cette famille. Cependant, WakefieldFamily a deux enfants, mais seul l'un d'eux, « Jesse », a des animaux. Elle a deux animaux. Le produit croisé renvoie donc 1*1*2 = 2 lignes à partir de cette famille.

L'exemple suivant ajoute un filtre sur `pet`. Ceci exclut tous les tuples où le nom de l'animal n'est pas « Shadow ». Notez que nous pouvons développer des tuples à partir de tableaux, filtrer n'importe quel élément d'un tuple et projeter n'importe quelle combinaison d'éléments.
...
L'exemple ci-dessus crée une fonction définie par l'utilisateur dont le nom est `SQRT`. Il accepte une seule valeur JSON `number` et calcule la racine carrée du nombre en utilisant la bibliothèque Math.
...
Vous pouvez également utiliser les fonctions définies par l'utilisateur dans un filtre, comme indiqué dans l'exemple suivant :
...
Le langage SQL de DocumentDB n'effectue pas de conversions implicites, contrairement à JavaScript. Par exemplen une requête `SELECT * FROM Person p WHERE p.Age = 21` correspond aux documents contenant la propriété Age ayant la valeur 21. Tous les autres documents dont la propriété Age correspond à la chaîne « 21 », ou toute autre variante possible comme « 021 », « 21.0 », « 0021 », « 00021 » etc. ne correspondra pas. Ceci est contraire au JavaScript où les valeurs de chaîne sont appelées de manière implicite sur les nombres (en fonction d'un opérateur, par exemple : ==). Ce choix est crucial pour une correspondance d'Index efficace dans le langage SQL de DocumentDB.
...
-   Expressions arithmétiques : elles incluent des expressions arithmétiques communes sur des valeurs numériques et booléennes. Pour obtenir une liste complète, veuillez consulter les spécifications SQL suivantes.
...
Les opérateurs suivants peuvent être composés pour former des requêtes plus puissantes. Comme DocumentDB prend en charge les collections imbriquées, de telles compositions peuvent être concaténées ou imbriquées.
...
Le client .NET effectue une itération automatique à travers l'ensemble des pages des résultats de la requête dans les blocs foreach, comme indiqué précédemment. Les options de requête présentées dans la section sur l'API REST sont également disponibles dans le Kit de développement logiciel (SDK) .NET à l'aide des classes `FeedOptions` and `FeedResponse` dans la méthode CreateDocumentQuery. Vous pouvez aussi contrôler le nombre de pages en utilisant le paramètre `MaxItemCount`.

Les développeurs peuvent également contrôler la pagination de façon explicite en créant un `IDocumentQueryable` à l'aide de l'objet `IQueryable`, puis en lisant les valeurs `ResponseContinuationToken` avant de les transmettre en tant que `RequestContinuationToken` in `FeedOptions`. `EnableScanInQuery` peut être défini pour autoriser les analyses lorsque la stratégie d'indexation configurée ne peut pas prendre en charge la requête.

Consultez la page [Exemples DocumentDB .NET] (http://code.msdn.microsoft.com/Azure-DocumentDB-NET-Code-6b3da8af\#content) pour obtenir plus d'exemples de requêtes.
...
1.  [Présentation d'Azure DocumentDB][Présentation d'Azure DocumentDB]
2.  [Spécification de langage pour DocumentDB SQL] (http://go.microsoft.com/fwlink/p/?LinkID=510612)
3.  [Exemples .NET pour DocumentDB] (http://code.msdn.microsoft.com/Azure-DocumentDB-NET-Code-6b3da8af\#content)
4.  [Niveaux de cohérence DocumentDB][Niveaux de cohérence DocumentDB]
5.  ANSI SQL 2011 - <http://www.iso.org/iso/iso_catalogue/catalogue_tc/catalogue_detail.htm?csnumber=53681>
6.  JSON <http://json.org/>
7.  Spécifications Javascript <http://www.ecma-international.org/publications/standards/Ecma-262.htm>
8.  LINQ <http://msdn.microsoft.com/fr-fr/library/bb308959.aspx>
9.  Techniques d'évaluation des requêtes pour les bases de données volumineuses <http://dl.acm.org/citation.cfm?id=152611>
...
13. G. Graefe. The Cascades framework for query optimization. IEEE Data Eng. Bull., 18(3): 1995.
  ...
  [Niveaux de cohérence DocumentDB]: ../documentdb-consistency-levels
  [Présentation d'Azure DocumentDB]: ../documentdb-introduction
