...
Nous avons récemment introduit une version préliminaire du Gestionnaire de ressources comme nouveau moyen de gestion des ressources Microsoft Azure. Cet article explique comment utiliser l'interface de ligne de commande interplateforme Azure (xplat-cli) pour travailler avec le Gestionnaire de ressources. 
...
>[WACOM.NOTE] Si vous n'avez pas encore installé ni configuré l'interface xplat-cli, consultez la rubrique [Installation et configuration de l'interface de ligne de commande interplateforme Microsoft Azure][xplatsetup] pour connaître les procédures d'installation, de configuration et d'utilisation de xplat-cli.
...
>[WACOM.NOTE] Les spécificités du langage des modèles ne sont pas documentées ici. Quand la documentation sera disponible, cette rubrique sera mise à jour avec un lien vers la documentation de référence.
>
...
2. Quand vous utilisez des modèles, vous pouvez créer vos modèles ou utiliser ceux de la galerie. Pour répertorier les modèles disponibles dans la galerie, utilisez la commande suivante.
...
1. Après avoir enregistré le fichier **params.json**, utilisez la commande suivante pour créer un groupe de ressources basé sur le modèle. Le paramètre " -e " spécifie le fichier **params.json** créé à l'étape précédente.

		azure group create MyGroupName "MyDataCenter" -y Microsoft.WebSiteSQLDatabase.0.1.0-preview1 -d MyDeployment -e params.json

	Remplacez **MyGroupName** par le nom du groupe que vous voulez utiliser et **MyDataCenter** par la valeur **siteLocation** spécifiée dans le modèle.
