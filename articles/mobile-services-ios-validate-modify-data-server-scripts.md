<properties pageTitle="Utilisation de scripts serveur pour valider et modifier des données (iOS) | Centre de développement mobile" description="Découvrez comment valider et modifier les données envoyées à l'aide de scripts serveurs à partir de votre application iOS." services="mobile-services" documentationCenter="ios" authors="krisragh" manager="dwrede" editor=""/>
...
Ce didacticiel s'appuie sur la procédure et l'exemple d'application présentés dans le didacticiel précédent intitulé [Prise en main des données]. Avant de commencer ce didacticiel, vous devez suivre [Prise en main des données].  
...
4. Remplacez le script existant par la fonction suivante, puis cliquez sur **Enregistrer**.
    ...
    > [AZURE.TIP] Pour supprimer un script inscrit de l'onglet **Script**, cliquez sur **Clear**, puis sur **Save**.
...
1. Sous l'onglet **Scripts** du [portail de gestion], remplacez le script **Insert** actuel par la fonction qui suit, puis cliquez sur **Save**.
