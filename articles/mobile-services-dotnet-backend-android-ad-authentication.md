<properties linkid="develop-mobile-tutorials-sso-android" urlDisplayName="Active Directory SSO Authentication with ADAL" pageTitle="Connexion et acc&egrave;s aux ressources avec Active Directory (Android) | Centre de d&eacute;veloppement mobile" metaKeywords="" description="Apprenez &agrave; ajouter l'authentification &agrave; une application avec Active Directory" metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Authentification de votre application avec Active Directory" authors="wesmc,mahender" />

<tags ms.service="mobile-services" ms.workload="mobile" ms.tgt_pltfrm="mobile-android" ms.topic="article" ms.date="01/01/1900" ms.author="yuaxu" />

# Authentification de votre application avec Active Directory

<div class="dev-center-tutorial-selector sublanding">
<a href="/fr-fr/documentation/articles/mobile-services-windows-store-dotnet-adal-sso-authentication" title="Windows Store&nbsp;C#" >Windows Store&nbsp;C#</a>
<a href="/fr-fr/documentation/articles/mobile-services-dotnet-backend-ios-adal-sso-authentication" title="iOS">iOS</a>
<a href="/fr-fr/documentation/articles/mobile-services-dotnet-backend-xamarin-ios-adal-sso-authentication" title="Xamarin.iOS">Xamarin.iOS</a>
<a href="/fr-fr/documentation/articles/mobile-services-dotnet-backend-android-ad-authentication" title="Android" class="current">Android</a>
</div>

Apprenez à authentifier votre application et à accéder à des ressources avec Active Directory en suivant le didacticiel de Chris Risner, [Accès à des ressources sécurisées par Azure Active Directory avec iOS et Android][Accès à des ressources sécurisées par Azure Active Directory avec iOS et Android].

<!-- URLs. -->

  [Accès à des ressources sécurisées par Azure Active Directory avec iOS et Android]: http://chrisrisner.com/Accessing-Resources-Secured-by-Azure-Active-Directory-with-iOS-and-Android
