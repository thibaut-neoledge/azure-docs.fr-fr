...
# Recherches de journal dans Log Analytics
...
- Dans le champ de requête de recherche, tapez `EventLog=System TimeGenerated>NOW-24HOURS"` ![Recherche avec des opérateurs boléens](./media/log-analytics-log-searches/oms-search-boolean.png)
...
Cela signifie que, si deux périodes ne se chevauchent pas, par exemple dans le portail OMS où vous choisissez **Cette semaine** et dans la requête où vous définissez **La semaine dernière**, il n’existe aucune intersection et vous ne recevrez aucun résultat.
...
- Dans le champ de requête de recherche, tapez `Type=Event EventID=2110 | Top 1` ![Recherche top](./media/log-analytics-log-searches/oms-search-top.png)
...
3. Sélectionnez certains d’entre eux de façon explicite ; la requête devient alors `Type=Event | Select Computer,EventID,RenderedDescription`. ![Recherche select](./media/log-analytics-log-searches/oms-search-select.png)
...
La première chose que vous remarquerez est que Log Analytics vous offre deux points de vue. Vous avez d’une part le point de vue Mesures, qui affiche les graphiques des compteurs de performance et d’autre part le point de vue Journaux, qui affiche les enregistrements réels dont sont issus les graphiques.
...
- **CounterValue** est la valeur réelle du compteur.
...
![Recherche du groupement de la fonciton avg](./media/log-analytics-log-searches/oms-search-avg04.png)


Vous pouvez facilement utiliser cela dans un tableau de bord. Pour en savoir plus sur l’utilisation de tableaux de bord, consultez [Créer un tableau de bord personnalisé dans Log Analytics](log-analytics-dashboards.md).
...
### Utilisation de la fonction percentile avec la commande measure
...
Vous pouvez utiliser la requête sous forme de vignette dans **Mon tableau de bord**, comme un moniteur de tri, afin de voir à quel moment les processeurs de l'ordinateur sont surexploités. Pour en savoir plus sur les tableaux de bord, consultez [Créer un tableau de bord personnalisé dans Log Analytics](log-analytics-dashboards.md). Vous pouvez également créer et utiliser des tableaux de bord à l'aide de l'application mobile. Pour plus d’informations, consultez [Application mobile OMS](http://www.windowsphone.com/fr-FR/store/app/operational-insights/4823b935-83ce-466c-82bb-bd0a3f58d865). En bas des deux vignettes de l'image suivante, vous pouvez voir que le moniteur a affiché une liste et un nombre. Essentiellement, le nombre doit toujours être zéro et la liste doit toujours être vide. Dans le cas contraire, il indique une condition d'alerte. Au besoin, vous pouvez l'utiliser pour voir quels ordinateurs sont sous pression.

![Tableau de bord mobile](./media/log-analytics-log-searches/oms-search-mobile.png)

## Utilisation de l’opérateur in

Les opérateurs *IN* et *NOT IN* vous permettent d’effectuer des sous-recherches, c’est-à-dire des recherches incluant une autre recherche en tant qu’argument. Elles sont contenues dans des accolades {} dans une autre recherche *principale* ou *externe*. Le résultat d’une sous-recherche, souvent une liste de résultats distincts, sert ensuite d’argument pour la recherche principale.

Vous pouvez utiliser les sous-recherches pour faire correspondre des sous-ensembles de données que vous ne pouvez pas décrire directement dans une expression de recherche, mais qui peuvent être générés à partir d’une recherche. Par exemple, si vous souhaitez utiliser une recherche pour trouver tous les événements sur des *ordinateurs avec mises à jour de sécurité manquantes*, vous devez concevoir une sous-recherche qui identifie d’abord les *ordinateurs avec mises à jour de sécurité manquantes*, avant de rechercher les événements appartenant à ces hôtes.
...
**Quels autres comptes ont ouvert une session sur les ordinateurs sur lesquels le compte BACONLAND\\jochan s’est connecté ?**

```
Type=SecurityEvent EventID=4624   Account!="BACONLAND\\jochan" Computer IN { Type=SecurityEvent EventID=4624   Account="BACONLAND\\jochan" | measure count() by Computer } | measure count() by Account
```

## Utilisation de la commande distinct
...
## Utilisation de la fonction countdistinct avec la commande measure
...
## Utilisation de la commande measure interval
...
### Utilisation de plusieurs agrégats dans une même requête
