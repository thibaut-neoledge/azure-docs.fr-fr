<properties 
   pageTitle="Alertes dans Log Analytics | Microsoft Azure"
...
Les actions de runbook démarrent le runbook en utilisant un [webhook](../automation/automation-webhooks.md). Quand vous créez la règle d’alerte, elle crée automatiquement un webhook pour le runbook sous le nom **Correction d’alerte OMS** suivi d’un GUID. La règle d’alerte n’envoie pas de charge utile, et vous ne pouvez pas remplir les paramètres du runbook ; vous devez donc utiliser un runbook qui n’exige pas de paramètres.

Vous pouvez également démarrer un runbook à partir d’une alerte avec une action de webhook, comme décrit dans la section précédente. Les actions de webhook ont l’avantage de fournir une charge utile pour envoyer des données au runbook. Grâce aux actions de runbook, vous n’êtes pas obligé de créer le webhook vous-même ou de savoir comment l’utiliser.
...
4.	Cliquez sur **Enregistrer** pour terminer la règle d’alerte. Son exécution démarre immédiatement.
...
## Définition des fenêtres de temps 
