...
# API REST d’alerte Log Analytics
...
| QueryTimeSpan | Intervalle de temps pendant lequel les critères sont évalués. Doit être égal ou supérieur à Interval. Exprimé en minutes. |
...
Utilisez la méthode Get pour extraire une planification particulière d’une recherche enregistrée.
...
### Création ou modification d’une planification
Utilisez la méthode Put avec un ID de planification unique pour la recherche enregistrée pour créer une planification. Si vous utilisez un ID de planification existante, cette planification sera modifiée. Quand vous créez une planification dans la console OMS, un GUID est créé pour l’ID de la planification.
...
### Création ou modification des actions
Utilisez la méthode Put avec un ID d’activité unique pour la planification pour créer une activité. Si vous utilisez un ID d’activité existant, l’activité sera modifiée. Quand vous créez une activité dans la console OMS, un GUID est défini pour l’ID d’activité.

Étant donné que le format de la demande de création d’une activité varie selon le type d’activité, les exemples correspondants sont fournis dans les sections ci-dessous.
...
Utilisez la méthode Put avec l’ID d’action pour créer une action de seuil pour une planification. Si vous utilisez un nouvel ID pour l’action, une action est créée. Si vous utilisez un ID qui existe déjà pour la planification, cette action est modifiée.
...
Utilisez la méthode Put avec l’ID d’action pour créer une action d’e-mail pour une planification. Si vous utilisez un nouvel ID pour l’action, une action est créée. Si vous utilisez un ID qui existe déjà pour la planification, cette action est modifiée.

L’exemple suivant crée une notification par courrier électronique avec un seuil afin que l’e-mail soit envoyé quand les résultats de la recherche enregistrée dépassent le seuil.
...
Utilisez la méthode Put avec l’ID d’action pour créer une action de correction pour une planification. Si vous utilisez un nouvel ID pour l’action, une action est créée. Si vous utilisez un ID qui existe déjà pour la planification, cette action est modifiée.

L’exemple suivant crée une correction avec un seuil afin que le runbook démarre quand les résultats de la recherche enregistrée dépassent le seuil.
...
#### Créer ou modifier une action de webhook
Utilisez la méthode Put avec l’ID d’action pour créer une action webhook pour une planification. Si vous utilisez un nouvel ID pour l’action, une action est créée. Si vous utilisez un ID qui existe déjà pour la planification, cette action est modifiée. L’exemple suivant crée une action de webhook et une action d’alerte avec un seuil pour que le webhook soit déclenché quand les résultats de la recherche enregistrée dépassent le seuil.
