...
| *aggregatedField* |Champ en cours d’agrégation. Ce champ est facultatif pour la fonction d’agrégation COUNT, mais il doit s’agir d’un champ numérique existant pour les fonctions SUM, MAX, MIN, AVG STDDEV ou PERCENTILE## ou PCT## (## est un nombre compris entre 1 et 99) aggregatedField peut être également n’importe laquelle des fonctions d’extension prises en charge. |
...
Calcule que le nombre maximal d’écritures disque par minute pour chaque disque sur votre ordinateur
...
**Description** vous permet de créer des champs d’exécution dans les requêtes. Vous pouvez également utiliser la commande de mesure après extension si vous souhaitez effectuer l’agrégation.
