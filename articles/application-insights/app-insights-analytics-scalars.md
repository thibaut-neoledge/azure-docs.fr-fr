...
Les expressions scalaires sont distinctes des [requêtes](app-insights-analytics-queries.md), dont les résultats sont des tables.
...
Arrondit les valeurs à l’entier inférieur multiple d’une taille bin donnée. Très utilisé dans la requête [`summarize by`](app-insights-analytics-queries.md#summarize-operator). Si vous avez un ensemble de valeurs dispersées, elles seront regroupées dans un plus petit ensemble de valeurs spécifiques.
...
* *text :* une chaîne.
* *search :* chaîne simple ou [expression régulière](app-insights-analytics-reference.md#regular-expressions) à faire correspondre au *text*.
...
Obtient une correspondance pour une [expression régulière](app-insights-analytics-reference.md#regular-expressions) à partir d’une chaîne de texte. Éventuellement, il convertit ensuite la sous-chaîne extraite dans le type indiqué.
...
* *regex :* une [expression régulière](app-insights-analytics-reference.md#regular-expressions).
...
* *regex :* l’[expression régulière](https://github.com/google/re2/wiki/Syntax) pour rechercher *text*. Elle peut contenir des groupes de capture entre '('parenthèses')'. 
...
|[`mvexpand` listColumn](app-insights-analytics-queries.md#mvexpand-operator) | Réplique une ligne pour chaque valeur d’une liste dans une cellule spécifiée.
|[`summarize buildschema(`column`)`](app-insights-analytics-queries.md#summarize-operator) |Déduit le schéma du type à partir du contenu de la colonne
|[`summarize makelist(`column`)` ](app-insights-analytics-queries.md#summarize-operator)| Aplatit des groupes de lignes et place les valeurs de la colonne dans un tableau.
|[`summarize makeset(`column`)`](app-insights-analytics-queries.md#summarize-operator) | Aplatit des groupes de lignes et place les valeurs de la colonne dans un tableau, sans duplication.

### Objets dynamiques dans les clauses let


Les [clauses let](app-insights-analytics-queries.md#let-clause) stockent des valeurs dynamiques sous forme de chaînes, afin que ces deux clauses soient équivalentes, et elles nécessitent toutes deux `parsejson` (ou `todynamic`) avant d’être utilisées :
...
Lorsque [sampling](app-insights-sampling.md) fonctionne, seule une fraction des données générées par le Kit de développement logiciel (SDK) est réceptionnée dans le portail Application Insights. L’échantillonnage peut être appliqué soit au Kit de développement logiciel (SDK) soit lors de l’ingestion au point de terminaison du portail.

`itemCount` indique le nombre d’événements générés dans le Kit de développement logiciel (SDK) pour chaque événement qui s’affiche dans le flux. Par exemple, si le taux d’échantillonnage actuel est 25 %, itemCount == 4.

Lorsque vous souhaitez compter des événements, utilisez `sum(itemCount)` au lieu de `count()` pour obtenir une estimation précise des nombres réels. Par exemple :

* Le nombre d’exceptions qui se sont produites au cours des deux dernières heures :
