<properties
	pageTitle="Applications web J2EE de détection, de tri et de diagnostic"
	description="Analyser les pannes et détecter et diagnostiquer les problèmes de performances dans vos applications web Java"
	authors="alancameronwills"
	services="application-insights"
    documentationCenter=""
	manager="douge"/>

<tags
	ms.service="application-insights"
	ms.workload="tbd"
	ms.tgt_pltfrm="ibiza"
	ms.devlang="na"
	ms.topic="article" 
	ms.date="02/04/2016"
	ms.author="awills"/>

# Applications web de détection, de tri et de diagnostic avec Application Insights

Veuillez consulter l’[article consolidé](app-insights-detect-triage-diagnose.md).
 

<!---HONumber=AcomDC_0211_2016-->