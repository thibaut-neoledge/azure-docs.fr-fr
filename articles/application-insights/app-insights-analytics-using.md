...
Écrivez une requête commençant par le nom de n’importe quelle table répertoriée sur la gauche. Utilisez `|` pour créer un pipeline d’[opérateurs](app-insights-analytics-queries.md).
...
Par défaut, les résultats sont initialement affichés sous forme de table. Vous pouvez ensuite sélectionner le diagramme manuellement. Mais vous pouvez également utiliser la [directive render](app-insights-analytics-queries.md#render-directive) à la fin d’une requête pour sélectionner un diagramme.
