...
3. Dans une fenêtre de navigateur distincte, ouvrez (ou créez) le modèle de version qui gère vos déploiements à partir de Visual Studio Team Services.
...
4. Définissez une variable `$(ApiKey)` dans le champ **APIKey**.
5. Dans le panneau Accès API, créez une clé d’API et copiez-la.
