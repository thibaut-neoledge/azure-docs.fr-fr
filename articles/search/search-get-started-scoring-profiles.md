---
title: Utilisation des profils de score dans la Recherche Azure | Microsoft Docs
...
---
# <a name="how-to-use-scoring-profiles-in-azure-search"></a>Utilisation des profils de score dans Azure Search
...
Vous avez besoin d’un abonnement Azure et d’un service Azure Search pour suivre ce didacticiel. Reportez-vous à la rubrique [Création d’un service Search dans le portail](search-create-service-portal.md) pour obtenir de l’aide sur la configuration du service.
 ...
 [Pour suivre ce didacticiel, vous avez besoin d’un compte Azure :](../../includes/free-trial-note.md)]
...
## <a name="edit-appconfig"></a>Modification du fichier app.config
1. Après avoir extrait les fichiers, ouvrez la solution dans Visual Studio pour modifier le fichier de configuration.
2. Dans l'Explorateur de solutions, double-cliquez sur **app.config**. Ce fichier spécifie le point de terminaison de service et une `api-key` utilisée pour authentifier la demande. Ces valeurs sont disponibles dans le portail Classic.
3. Connectez-vous au [portail Azure](https://portal.azure.com).
4. Accédez au tableau de bord du service pour Azure Search.
5. Cliquez sur la vignette **Propriétés** pour copier l'URL du service.
6. Cliquez sur la vignette **Clés** pour copier l'`api-key`.
