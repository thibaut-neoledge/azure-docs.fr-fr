...
Pour configurer un indexeur de table :
...
Les noms de champ figurant dans votre index existant diffèrent généralement des noms de propriétés dans votre table. Dans ce cas, vous pouvez utiliser les **mappages de champs** pour mapper les noms de propriété aux noms de champ de votre index de recherche. Pour en savoir plus sur les mappages de champs, consultez la rubrique [Personnalisation des indexeurs Azure Search](search-indexers-customization.md).

## Indexation incrémentielle et détection des suppressions
 
Lorsque vous configurez un indexeur de table pour s’exécuter de manière planifiée, cet indexeur répertorie uniquement les lignes nouvelles ou mises à jour, comme déterminé par la valeur `Timestamp` de la ligne. Vous n’êtes pas contraint de spécifier une stratégie de détection des modifications ; l’indexation incrémentielle est activée automatiquement à votre intention.
