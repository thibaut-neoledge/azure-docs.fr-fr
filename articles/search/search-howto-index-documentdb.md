...
Si vous souhaitez mettre en place des expériences de recherche sur vos données DocumentDB, vous pouvez configurer et exécuter un indexeur Recherche Azure qui extrait les données et les place dans un index Recherche Azure. Dans cet article, nous vous montrons comment intégrer Recherche Azure à Azure DocumentDB sans avoir à écrire le moindre code pour mettre l’infrastructure d’indexation à jour.
...
Émettez une demande HTTP POST pour créer une source de données dans votre service Recherche Azure avec les en-têtes de requête suivants.

    POST https://[Search service name].search.windows.net/datasources?api-version=[api-version]
    Content-Type: application/json
    api-key: [Search service admin key]

Le paramètre `api-version` est obligatoire. Les valeurs valables incluent `2015-02-28` ou une version ultérieure. Consultez [Versions d’API dans Azure Search](search-api-versions.md) pour voir toutes les versions d’API prises en charge dans Search.
...
Créez un index Azure Search cible si vous n'en possédez pas déjà un. Pour ce faire, vous pouvez utiliser l’[interface utilisateur du portail Azure](search-create-index-portal.md) ou l’[API Création d’index](/rest/api/searchservice/create-index) ou la [classe d’index](/dotnet/api/microsoft.azure.search.models.index).
...
Vous pouvez émettre une demande HTTP GET pour récupérer l’historique d’état et d’exécution d’un indexeur :
