...
[Kit de développement logiciel (SDK) .NET 1.0.1-version préliminaire](https://msdn.microsoft.com/library/azure/dn951165.aspx)|Janvier 2016|VERSION PRÉLIMINAIRE|Il s'agit de la troisième itération de la bibliothèque cliente .NET, Microsoft.Azure.Search.dll. Cette version introduit des changements majeurs. Consultez [Mise à niveau vers la version préliminaire du Kit de développement logiciel (SDK) .NET version 1.0.1 d'Azure Search](search-dotnet-sdk-migration.md) pour obtenir des conseils sur la migration.
