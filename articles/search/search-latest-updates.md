...
-------|--------|------|-------
[Mise à jour des références](search-limits-quotas-capacity.md)|Juin 2016|Aperçu et disponibilité générale|Les références De base et Standard 2 (S2), en version préliminaire en mars 2016, sont désormais en disponibilité générale. <br/><br/>Les versions préliminaires de nouvelles références Standard 3 (S3) et Haute densité S3 (HD S3) sont introduites dans cette mise à jour. Consultez [Choisir un niveau ou une référence pour Azure Search ](search-sku-tier.md) pour les comparer.
...
[moreLikeThis=](search-api-2015-02-28-preview.md)|Mars 2015|VERSION PRÉLIMINAIRE|Un paramètre de recherche qui s’exclut mutuellement avec `search=` et déclenche un chemin d’exécution de requête différent. Au lieu d’effectuer la recherche en texte intégral de `search=` à partir de l’entrée d’un terme de recherche, `moreLikeThis=` recherche les documents semblables à un document donné en comparant leurs champs de recherche.
