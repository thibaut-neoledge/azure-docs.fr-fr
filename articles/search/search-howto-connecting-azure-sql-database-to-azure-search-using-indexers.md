<properties 
    pageTitle="Connexion d’Azure SQL Database à Azure Search à l’aide d’indexeurs | Microsoft Azure | Indexeurs" 
...
Pour installer et configurer l’indexeur Azure SQL, vous pouvez appeler [l’API REST de Recherche Azure](http://go.microsoft.com/fwlink/p/?LinkID=528173)pour créer et gérer les **indexeurs** et les **sources de données**. 

Vous pouvez également utiliser la [classe Indexer](https://msdn.microsoft.com/library/azure/microsoft.azure.search.models.indexer.aspx) du [SDK .NET](https://msdn.microsoft.com/library/azure/dn951165.aspx), ou l’Assistant d’importation de données du [portail Azure](https://portal.azure.com) pour créer et planifier un indexeur.
...
1. Il est maintenant 2 h 00, heure du début de la troisième exécution. Cependant, comme la deuxième exécution démarrée à 1 heure est toujours en cours d’exécution, la troisième exécution est ignorée. Elle commence à 3 h 00.

Vous pouvez ajouter, modifier ou supprimer une planification d’indexeur en utilisant une requête **PUT indexer** . 

<a name="CaptureChangedRows">,/a>
## <a name="capturing-new,-changed-and-deleted-rows"></a>Capture des lignes nouvelles, modifiées et supprimées
...
Lorsque la stratégie de suivi intégré des modifications SQL est recommandée, vous ne pourrez pas l'utiliser si vos données sont dans une vue ou si vous utilisez une version antérieure d’Azure SQL Database. Dans ce cas, essayez la stratégie de limite supérieure. Celle-ci est applicable si votre table contient une colonne qui répond aux critères suivants :
...
| char, nchar, varchar, nvarchar | Edm.String<br/>Collection(Edm.String)|La transformation d’une colonne de chaîne en Collection(Edm.String) requiert l’utilisation d’une API d’aperçu version 2015-02-28-Preview. Pour plus d’informations, consultez [cet article](search-api-indexers-2015-02-28-Preview.md#CreateIndexer).| 
...
R. : Oui. Toutefois, vous devez autoriser votre service de recherche à se connecter à votre base de données en effectuant les deux tâches suivantes. Pour plus d’informations, consultez l’article [Configurer une connexion d’un indexeur de Recherche Azure à SQL Server sur une machine virtuelle Azure](search-howto-connecting-azure-sql-iaas-to-azure-search-using-indexers.md).

1. Vous devrez peut-être configurer votre base de données avec un certificat approuvé afin que le service de recherche puisse ouvrir des connexions SSL à la base de données.
2. Configurez le pare-feu pour autoriser l’accès à l’adresse IP de votre service de recherche.
