...
- [Stockage d’objets blob Azure](search-howto-indexing-azure-blob-storage.md) (actuellement en version préliminaire. Extrait le texte des documents PDF et Office, et de code HTML ou XML.)
...
Le [Kit de développement logiciel (SDK) .NET](https://msdn.microsoft.com/library/azure/microsoft.azure.search.iindexersoperations.aspx) et l’[API REST du service](https://msdn.microsoft.com/library/azure/dn946891.aspx) prennent en charge la gestion des indexeurs et des sources de données. Vous pouvez également configurer un indexeur dans le portail lorsque vous utilisez l’Assistant **Importation de données**.
