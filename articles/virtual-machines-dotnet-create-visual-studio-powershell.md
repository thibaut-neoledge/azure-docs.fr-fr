<properties urlDisplayName="Create a virtual machine for a website" pageTitle="Cr&eacute;ation d'une machine virtuelle pour un projet web avec Visual&nbsp;Studio" metaKeywords="Visual Studio, ASP.NET, web project, virtual machine" description="Cr&eacute;ation d'une machine virtuelle pour un site web" metaCanonical="" services="" documentationCenter="" title="Cr&eacute;ation d'une machine virtuelle pour un site web avec Visual Studio" authors="ghogen" solutions="" manager="douge" editor="" />
...
12. Lorsque la machine virtuelle est approvisionnée, des scripts de publication sont créés dans un nœud **PublishScripts** de votre solution. Le script de publication exécute et approvisionne une machine virtuelle dans Azure. La fenêtre **Sortie** affiche le statut. Le script effectue les opérations ci-après pour configurer la machine virtuelle :
    ...
    -   Création d'un compte de stockage avec un nom commençant par `devtest`, mais seulement s'il n'y a pas déjà de compte de stockage dans la région spécifiée.
...
1.  (Facultatif) Dans l'**Explorateur de serveurs**, développez le nœud **Machines virtuelles**. Choisissez le nœud de la machine virtuelle que vous avez créée, puis sélectionnez **Se connecter avec le Bureau à distance** pour vous connecter à la machine virtuelle.

# Étapes suivantes

Si vous voulez personnaliser les scripts de publication créés, consultez des informations détaillées [ici][ici].
  ...
  [ici]: http://msdn.microsoft.com/library/dn642480.aspx
