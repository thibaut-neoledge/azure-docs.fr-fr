...
Un projet avec l'état terminé après ce didacticiel est disponible [ici](https://github.com/Azure/mobile-services-samples/tree/master/TodoOffline/Xamarin.Android).
...
* [Kit de développement logiciel (SDK) Azure Mobile Services version 1.3.0-beta2][Mobile Services SDK Nuget]
* [Azure Mobile Services SQLite Store version 1.0.0-beta2][SQLite store nuget]

>[WACOM.NOTE] Les instructions qui suivent partent du principe que vous utilisez Visual Studio 2012 ou version ultérieure avec l'extension Xamarin. Si vous utilisez Xamarin Studio, utilisez la prise en charge du Gestionnaire de package NuGet intégrée.
...
2. Changez le type de membre `ToDoActivity.toDoTable` de `IMobileServiceTable<>` en `IMobileServiceSyncTable<>`
...
4. Dans la même méthode, changez la ligne qui initialise `toDoTable` pour qu'elle utilise la méthode `GetSyncTable<>` au lieu de `GetTable<>` :
...
5. Dans l'application, appuyez sur le bouton **Actualiser**. L'application appelle `MobileServiceClient.SyncContext.PushAsync` et `IMobileServiceSyncTable.PullAsync()`, puis `RefreshTodoItems` pour actualiser l'application avec les éléments du magasin local. 
...
Vous pouvez télécharger la version complète de ce didacticiel dans notre [référentiel d'exemples GitHub](https://github.com/Azure/mobile-services-samples/tree/master/TodoOffline/Xamarin.Android).
