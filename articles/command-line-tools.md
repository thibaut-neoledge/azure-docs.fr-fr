...
Pour installer l'outil sur un Mac, téléchargez et exécutez le [programme d'installation du SDK Azure](http://go.microsoft.com/fwlink/?LinkId=252249).
...
**vm create &lt;dns-name> &lt;image> &lt;userName> [options] [password]**
...
Notez qu'à l'instar du portail Azure, la commande vm create ne crée les machines virtuelles que dans l'environnement de déploiement de production. Il n'existe pour l'instant aucune option permettant de créer une machine virtuelle dans l'environnement de déploiement intermédiaire d'un service cloud. À noter que cette commande ne crée un compte de stockage Azure que s'il n'en existe pas déjà un pour votre abonnement.
 ...
 **-s** représente l'abonnement.<br />
 **-o, --community** : l'image spécifiée est une image de communauté.<br />
 **-w** représente le nom du réseau virtuel.<br />
 **-l, --location** spécifie l'emplacement (par exemple, « États-Unis - partie centrale septentrionale »).<br />
...
Cette commande permet de mettre à jour un point de terminaison de machine virtuelle avec de nouvelles valeurs en utilisant ces options.
    ...
    -n, --endpoint-name           the new endpoint name
    -t, --lb-port                 the new load balancer port
    -t, --vm-port                 the new local port port
    -o, --endpoint-protocol   the new transport layer protocol for port (tcp or udp) 
...
Cette commande permet de détacher un disque de données attaché à une machine virtuelle Azure. identifie le disque à détacher. Pour obtenir la liste des disques associés à un disque avant de le détacher, utilisez la commande vm disk-list .
...
Les certificats Azure sont les certificats (c'est-à-dire, des certificats SSL) connectés à votre compte Azure.
...
Cette commande permet de répertorier vos sites Web.
...
Cette commande permet d'ouvrir le portail dans un navigateur pour vous permettre de gérer vos sites Web.
...
Cette commande permet de répertorier vos emplacements de site Web.
...
Cette commande permet d'afficher la liste des certificats de site Web.
...
### Commandes pour gérer vos diagnostics de site Web

**site log download [options] [name]**

Permet de télécharger un fichier .zip de vos diagnostics de site Web.
...
Cette commande permet de configurer les options de diagnostic.
...
### Commandes pour gérer vos référentiels de site Web
...
### Commandes pour gérer la mise à l'échelle de site Web
...
-   **-l `<location>`** ou **--location `<location>`** : crée le service à un emplacement spécifique, spécifié sous la forme `<location>`. Exécutez azure mobile locations pour obtenir les emplacements disponibles.
...
Le paramètre \*\*--query\*\* a la priorité sur \*\*--type\*\*, \*\*--skip\*\* et \*\*--top\*\*.
...
Les nouveaux travaux créés sont désactivés, car il reste encore à télécharger un script. Utilisez la commande **mobile script upload** pour télécharger un script et la commande **mobile job update** pour activer le travail.
...
Lorsque vous attribuez au mode de calcul la valeur « Reserved » (réservé), tous vos services mobiles d'une même région s'exécutent en mode premium.
...
Affiche des détails sur la base de données.
    ...
    info:    Commande sql db show OK

**sql db list [options] &lt;serverName> &lt;databaseName> &lt;administratorPassword>**
...
**sql firewallrule create [options] &lt;serverName> &lt;databaseName> &lt;administratorPassword>**
...
Affiche les détails concernant le réseau virtuel spécifié.
