...
<div class="dev-callout"><b>Remarque</b>
   <p>Le fichier publishsettings peut contenir les d&eacute;tails (c'est-&agrave;-dire, le nom et l'ID d'abonnement) concernant plusieurs abonnements. Lorsque vous importez le fichier publishsettings, le premier abonnement est utilis&eacute; comme description par d&eacute;faut. Pour utiliser un autre abonnement, ex&eacute;cutez la commande suivante&nbsp;:</p>
...
**-c, --connect** crée la machine virtuelle dans un déploiement déjà créé dans un service d'hébergement. Si -vmname n'est pas utilisé avec cette option, le nom de la nouvelle machine virtuelle sera généré automatiquement.
**-n, --vm-name** spécifie le nom de la machine virtuelle. Ce paramètre prend le nom du service d'hébergement par défaut. Si -vmname n'est pas spécifié, le nom de la nouvelle machine virtuelle est généré sous la forme \<nom-service\>\<id\>, où \<id\> correspond au nombre de machines virtuelles existant dans le service plus 1. Par exemple, si vous utilisez cette commande pour ajouter une nouvelle machine virtuelle au service d'hébergement MyService, la nouvelle machine virtuelle est nommée MyService2.

**-u, --blob-url** spécifie l'URL du stockage d'objets blob à partir duquel le disque système de la machine virtuelle est créé.

**-z, --vm-size** spécifie la taille de la machine virtuelle. Les valeurs valides sont « extrasmall » (très petite), « small » (petite), « medium » (moyenne), « large » (grande), « extralarge » (très grande). La valeur par défaut est « small » (petite).

**-r** ajoute la connectivité RDP à une machine virtuelle Windows.

**-e, --ssh** ajoute la connectivité SSH à une machine virtuelle Windows.

**-t, --ssh-cert** spécifie le certificat SSH.

**-s** représente l'abonnement.

**-o, --community** : l'image spécifiée est une image de communauté.

**-w** représente le nom du réseau virtuel.

**-l, --location** spécifie l'emplacement (par exemple, « États-Unis - partie centrale septentrionale »).

**-a, --affinity-group** spécifie le groupe d'affinités.

**-w, --virtual-network-name** spécifie le réseau virtuel sur lequel la nouvelle machine virtuelle doit être ajoutée. Les réseaux virtuels peuvent être configurés et gérés à partir du portail Azure.
...
La capture d'une image de machine virtuelle n'est possible que si la machine virtuelle est à l'arrêt (état **Arrêté**).
...
<div class="dev-callout"><b>Remarque</b>
   <p>Notez que vous devez toujours d&eacute;tacher les disques de donn&eacute;es dans l'ordre inverse des num&eacute;ros d'unit&eacute; logique attribu&eacute;s, c'est-&agrave;-dire en commen&ccedil;ant par le num&eacute;ro le plus &eacute;lev&eacute;. En effet, la couche SCSI Linux ne permet pas de d&eacute;tacher un num&eacute;ro d'unit&eacute; logique inf&eacute;rieur &agrave; un autre toujours attach&eacute;. Par exemple, vous ne devez pas d&eacute;tacher LUN&nbsp;0 si LUN&nbsp;1 est toujours attach&eacute;.</p>
</div>
...
## <a name="Commands_to_manage_your_web_sites"></a>Commandes pour gérer vos sites Web
...
<div class="dev-callout"><b>Remarque</b>
   <p>Le nom du site doit &ecirc;tre unique. Vous ne pouvez pas cr&eacute;er un site avec le m&ecirc;me nom DNS qu'un site existant.</p>
</div>
...
<div class="dev-callout"><b>Remarque</b>
   <p>Le param&egrave;tre <b>--query</b> a la priorit&eacute; sur <b>--type</b>, <b>--skip</b> et <b>--top</b>.</p>
</div>
...
<div class="dev-callout"><b>Remarque</b>
   <p>Lorsque vous r&eacute;g&eacute;n&eacute;rez une cl&eacute;, les clients qui utilisent l'ancienne cl&eacute; risquent de ne pas pouvoir acc&eacute;der &agrave; votre service mobile. Lorsque vous r&eacute;g&eacute;n&eacute;rez la cl&eacute; d'application, vous devez mettre &agrave; jour votre application avec la nouvelle valeur de cl&eacute;. </p>
</div>
...
-   **-p `&lt;permissions>`** ou **--permissions `&lt;permissions>`** : liste de paires `<operation>`=`<permission>` séparée par des virgules, où `<operation>` correspond à `insert`, `read`, `update` ou `delete` et `&lt;permissions>` représente `public`, `application` (par défaut), `user` ou `admin`.
...
-   **-u `<unit>`** ou **--intervalUnit `<unit>`** : unité du paramètre *interval*, qui peut être l'une des valeurs suivantes :
...
<div class="dev-callout"><b>Remarque</b>
   <p>Les nouveaux travaux cr&eacute;&eacute;s sont d&eacute;sactiv&eacute;s, car il reste encore &agrave; t&eacute;l&eacute;charger un script. Utilisez la commande <strong>mobile script upload</strong> pour t&eacute;l&eacute;charger un script et la commande <strong>mobile job update</strong> pour activer le travail.</p>
</div>
...
-   **-u `<unit>`** ou **--intervalUnit `<unit>`** : unité du paramètre *interval*, qui peut être l'une des valeurs suivantes :
    ...
    -   **none** (travaux à la demande)
-   **-t `<time>`** **--startTime `<time>`** Heure de début de la première exécution du script, au format ISO ; la valeur par défaut est `now`.
-   **-a `<status>`** ou **--status `<status>`** : statut du travail, qui peut être `enabled` ou `disabled`.
...
<div class="dev-callout"><b>Remarque</b>
   <p>La suppression d'un travail entra&icirc;ne celle du script t&eacute;l&eacute;charg&eacute;.</p>
</div>
...
<div class="dev-callout"><b>Remarque</b>
   <p>Lorsque vous attribuez au mode de calcul la valeur &laquo;&nbsp;Reserved&nbsp;&raquo; (r&eacute;serv&eacute;), tous vos services mobiles d'une m&ecirc;me r&eacute;gion s'ex&eacute;cutent en mode premium.</p>
</div>
...
**-p** ou **--permissions** \<permissions\> : liste de paires \<method\>=\<permission\> séparée par des virgules.
...
-   **-a** ou **--affinity-group** \<name\> : groupe d'affinités auquel associer le compte d stockage.
...
-   **--container** \<container\> : nom du conteneur de stockage à créer.
-   **-b** ou **--blob** \<blobName\> : nom de l'objet blob de stockage à mettre à jour.
-   **-t** ou **--blobtype** \<blobtype\> : type d'objet blob : page ou bloc.
...
Supprime une entrée de serveur DNS de la configuration réseau.
  ...
  [programme d'installation du SDK Azure]: http://go.microsoft.com/fwlink/?LinkId=252249
  ...
  [Schéma technique Azure]: ./media/command-line-tools/architecturediagram.jpg
  [azurenetworkdiagram]: ./media/command-line-tools/networkdiagram.jpg
  [Gérer les certificats]: http://msdn.microsoft.com/fr-fr/library/azure/gg981929.aspx
  ...
  [Utilisation des scripts serveur dans Mobile Services]: http://www.windowsazure.com/fr-fr/develop/mobile/how-to-guides/work-with-server-scripts/
  [Planifier les travaux]: http://msdn.microsoft.com/fr-fr/library/windowsazure/jj860528.aspx
  [Augmenter l'extensibilité d'un service mobile]: http://msdn.microsoft.com/fr-fr/library/windowsazure/jj193178.aspx
