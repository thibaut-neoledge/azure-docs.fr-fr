...
4. Après l’élément `<services>`, créez un élément `<behaviors>` avec le contenu suivant, en remplaçant « SAS_KEY » par la clé avec *signature d’accès partagé* (SAP) obtenue précédemment à partir du [portail Azure][].
