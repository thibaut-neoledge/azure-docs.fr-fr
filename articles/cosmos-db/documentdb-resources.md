...
* Qu’est-ce que le modèle de ressource Azure Cosmos DB ?
...
Les API REST prennent en charge l’adressage des ressources et le routage des requêtes par les propriétés id et _rid.
...
En plus d’approvisionner, de configurer et de gérer votre compte de base de données à partir du portail Azure, vous pouvez créer et gérer des comptes de base de données Cosmos DB par programme en utilisant les [API REST Azure Cosmos DB](/rest/api/documentdb/) ainsi que des [Kits de développement logiciel (SDK) clients](documentdb-sdk-dotnet.md).  
...
### <a name="elastic-scale-of-a-azure-cosmos-db-database"></a>Mise à l’échelle élastique d’une base de données Azure Cosmos DB
...
Dans l’API DocumentDB, les collections peuvent être facilement créées, supprimées, lues ou répertoriées avec les [API REST](/rest/api/documentdb/) ou l’un des [kits SDK clients](documentdb-sdk-dotnet.md). L’API DocumentDB fournit toujours une cohérence forte pour la lecture ou l’interrogation des métadonnées d’une collection. La suppression d'une collection garantit automatiquement que vous ne pouvez pas accéder aux documents, pièces jointes, procédures stockées, déclencheurs et fonctions définies par l'utilisateur qu'elle contient.   
...
Vous pouvez spécifier une fonction définie par l’utilisateur dans le cadre d’une requête SQL et l’utiliser pour étendre le [langage de requête SQL pour l’API DocumentDB](https://msdn.microsoft.com/library/azure/dn782250.aspx).
...
Bien que les extraits de code précédents aient montré l’enregistrement (POST), l’annulation de l’enregistrement (PUT), la lecture ou le tri (GET) et l’exécution (POST) via le [kit SDK JavaScript](https://github.com/Azure/azure-documentdb-js), vous pouvez également utiliser les [API REST](/rest/api/documentdb/) ou d’autres [kits SDK clients](documentdb-sdk-dotnet.md). 
...
Pour les éléments multimédias gérés par Azure Cosmos DB, la propriété _media de la pièce jointe référence les éléments multimédias en suivant leur URI. Azure Cosmos DB veille à nettoyer les éléments multimédias lorsque toutes les références en suspens sont supprimées. Azure Cosmos DB génère automatiquement les pièces jointes lorsque vous chargez les nouveaux éléments multimédias et renseigne la propriété _media pour pointer vers l’élément multimédia récemment ajouté. Si vous choisissez de stocker l'élément multimédia dans un magasin d'objets blob distant que vous gérez (comme OneDrive, Azure Storage, DropBox, etc.), vous pouvez toujours utiliser les pièces jointes pour le référencer. Dans ce cas, vous créez la pièce jointe vous-même et renseignez sa propriété _media manuellement.   
...
Un utilisateur d’Azure Cosmos DB correspond à un espace de noms logique pour le regroupement des autorisations. Un utilisateur d’Azure Cosmos DB peut correspondre à un utilisateur dans un système de gestion d’identité ou à un rôle d’application prédéfini. Pour Azure Cosmos DB, un utilisateur représente simplement une donnée abstraite visant à regrouper un ensemble d’autorisations dans une base de données.   

Pour implémenter une architecture multilocataire dans votre application, vous pouvez créer des utilisateurs dans Azure Cosmos DB qui correspondent à vos utilisateurs réels ou aux locataires de votre application. Vous pouvez ensuite créer des autorisations pour un utilisateur donné correspondant au contrôle d'accès de divers collections, documents, pièces jointes, etc.   
...
Indépendamment de la stratégie de partition que vous choisissez, vous pouvez modéliser vos utilisateurs actuels en tant qu’utilisateurs dans la base de données Azure Cosmos DB, et associer des autorisations affinées à chacun d’eux.  

![Regroupements d’utilisateurs][3]  
**Stratégies de partitionnement et modélisation des utilisateurs**

Comme avec les autres ressources, vous pouvez créer, remplacer, supprimer, lire ou énumérer facilement les utilisateurs dans Azure Cosmos DB en utilisant des API REST ou l’un des kits SDK clients. Azure Cosmos DB fournit toujours une cohérence forte pour la lecture ou l’interrogation des métadonnées d’une ressource utilisateur. Il est intéressant de mentionner que la suppression d'un utilisateur garantit automatiquement que vous ne pouvez pas accéder à l'une des autorisations qu'il contient. Même si Azure Cosmos DB réclame le quota d’autorisations dans le cadre de la suppression de l’utilisateur en arrière-plan, les autorisations supprimées sont de nouveau disponibles immédiatement pour utilisation.  
...
Comme avec les autres ressources, vous pouvez créer, remplacer, supprimer, lire ou énumérer facilement les autorisations dans Azure Cosmos DB en utilisant des API REST ou l’un des kits SDK clients. Azure Cosmos DB fournit toujours une cohérence forte pour la lecture ou l’interrogation des métadonnées d’une autorisation. 
