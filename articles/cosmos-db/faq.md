...
description: "Obtenez des réponses aux questions fréquemment posées sur Azure Cosmos DB, service de base de données multimodèle distribué mondialement. Découvrez la capacité, les niveaux de performances et la mise à l’échelle."
...
Azure Cosmos DB est un service de base de données multimodèle répliqué mondialement qui offre des requêtes enrichies sur des données sans schéma et permet d’obtenir des performances configurables et fiables, ainsi qu’un développement rapide. Tout cela s’effectue via une plateforme gérée s’appuyant sur la puissance et l’étendue de Microsoft Azure. 
...
* La réplication mondiale est disponible dans les [régions Azure](https://azure.microsoft.com/regions/) suivantes : Centre des États-Unis, Europe du Nord et Asie du Sud-Est.
...
* Les appels CreateIfNotExists sont restreints par une limitation de gestion qui est fixe et distincte des autres opérations de table couvertes par les unités de requête. Cela signifie que ceux qui effectuent beaucoup d’appels CreateIfNotExists se voient imposer une limite qu’ils ne peuvent pas contrôler puisqu’elle ne provient pas de leurs propres unités de requête.
...
| OPTIONS | /<nom-ressource-table-> | [Requête de table CORS préliminaire](https://docs.microsoft.com/en-us/rest/api/storageservices/preflight-table-request) | Cette option fait partie de CORS qui n’est pas pris en charge par Azure Cosmos DB pour l’instant. |
...
Vous pouvez utiliser [l’Explorateur Stockage Azure](https://docs.microsoft.com/en-us/azure/vs-azure-tools-storage-manage-with-storage-explorer).
...
[AzCopy](https://docs.microsoft.com/en-us/azure/storage/common/storage-use-azcopy) et [l‘outil de migration de données Azure Cosmos DB](import-data.md) sont tous les deux pris en charge.
...
Vous pouvez utiliser les [paramètres de réplication mondiale](tutorial-global-distribution-documentdb.md#portal) du portail Azure Cosmos DB pour ajouter des régions appropriées à votre application. Pour développer une application distribuée mondialement, vous devez également ajouter votre application avec les informations PreferredLocation définies sur la région locale afin de bénéficier d’une faible latence de lecture. 
...
Vous pouvez utiliser le volet du portail de réplication mondiale d’Azure Cosmos DB pour ajouter une région, puis basculer vers la région requise. Pour des instructions, voir [Développement avec des comptes Azure Cosmos DB multirégions](regional-failover.md). 
...
Azure Cosmos DB valide les données durablement dans la région locale, et envoie les données vers les autres régions immédiatement, en quelques millisecondes. Cette réplication dépend uniquement de la durée des boucles (RTT) du centre de données. Pour en savoir plus sur la fonctionnalité de distribution mondiale d’Azure Cosmos DB, consultez [Azure Cosmos DB : service de base de données distribué mondialement sur Azure](distribute-data-globally.md).
...
Azure Cosmos DB est conçu pour être un système distribué mondialement basé sur un contrat de niveau de service (SLA) offrant des garanties en matière de disponibilité, de latence et de débit. Le débit que vous réservez dans Azure Cosmos DB est garanti, à la différence du débit d’autres systèmes. Azure Cosmos DB offre des fonctionnalités supplémentaires qui ont été demandées par les clients, telles que les index secondaires et la distribution mondiale.  
...
### <a name="it-looks-like-you-support-the-gremlin-graph-traversal-language-do-you-plan-to-add-more-forms-of-query"></a>Vous semblez prendre en charge le langage de traversée de graphe Gremlin. Prévoyez-vous d’ajouter d’autres formes de requête ?
...
### <a name="what-is-the-protocol-version-supported-in-the-preview-do-you-plan-to-support-other-protocols"></a>Quelle est la version du protocole prise en charge dans la préversion ? Prévoyez-vous de prendre en charge d’autres protocoles ?
L’API Apache Cassandra pour Azure Cosmos DB prend actuellement en charge CQL version 4. Pour donner votre avis sur la prise en charge de modèles supplémentaires, créez une requête dans les [commentaires UserVoice](https://feedback.azure.com/forums/263030-azure-cosmos-db) ou envoyez un e-mail à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 
...
Azure Cosmos DB est un système de gouvernance des ressources pour les activités liées au plan de données et au plan de contrôle. Les conteneurs comme les collections et les tables sont des entités de runtime qui sont provisionnées pour une certaine capacité de débit. La création de ces conteneurs en succession rapide n’est pas une activité prévue et peut être limitée. Si vous avez des tests qui suppriment/créent des tables sans délai, essayez de les espacer ou de réutiliser des tables existantes après suppression des données.
...
Il n’existe aucune limite physique concernant le nombre de tables qui peuvent être créées. Si vous devez créer un très grand nombre de tables (taille stable totale dépassant 10 To), bien au-delà des dizaines ou des centaines de tables habituelles, envoyez un e-mail à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

### <a name="what-is-the-maximum-number-of-keyspaces-that-can-be-created"></a>Quel est le nombre maximal d’espaces de clés qu’il est possible de créer ? 
Les espaces de clés étant des conteneurs de métadonnées, il n’existe aucune limite physique concernant le nombre d’espaces de clés que vous pouvez créer. Si vous devez créer un très grand nombre d’espaces de clés, envoyez un e-mail à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

### <a name="is-it-possible-to-bring-in-a-lot-of-data-after-starting-from-a-normal-table"></a>Puis-je importer un volume important de données si j’ai commencé dans une table normale ? 
La capacité de stockage est gérée automatiquement et augmente à mesure que vous envoyez (push) des données supplémentaires. Le débit est suffisant pour prendre en charge cette quantité de données, ce qui vous permet donc d’importer toutes les données nécessaires en toute confiance.
...
L’API Apache Cassandra est un service de plateforme qui simplifie les processus de planification de la capacité et de réponse aux demandes d’élasticité pour le débit et le stockage. Azure Cosmos DB provisionne le débit dont vous avez besoin. Vous pouvez ensuite augmenter ou réduire le débit autant de fois par jour que vous le souhaitez, sans vous soucier des tâches d’ajout, de suppression ou de gestion des nœuds. Cela signifie également que vous n’avez pas besoin de recourir à un outil de gestion des nœuds ou des clusters. 

### <a name="what-happens-with-respect-to-various-config-settings-for-keyspace-creation"></a>Qu’en est-il des différents paramètres de configuration relatifs à la création des espaces clés ?
Azure Cosmos DB offre des fonctionnalités de distribution mondiale prêtes à l’emploi à haute disponibilité et faible latence. Il est inutile de configurer des réplicas. Toutes les écritures sont toujours validées durablement dans un quorum dans toute région où vous écrivez ou distribuez des données, le tout avec des garanties de performance. Pour cette raison, les paramètres de configuration tels que `Simple Strategy` et `Network Topology Strategy` sont ignorés. 

### <a name="what-happens-with-respect-to-various-settings-for-table-metadata-like-bloom-filters-caching-read-repair-changes-gcgrace-and-compression-memtableflushperiod"></a>Qu’en est-il des différents paramètres relatifs aux métadonnées de table comme les filtres de Bloom, la mise en cache, les changements de réparation des lectures, la période de grâce du garbage collection et la période de vidage des memtables pour la compression ?
Azure Cosmos DB garantit des opérations en lecture/écriture et des débits performants sans aucun changement des paramètres de configuration, réduisant ainsi les risques liés à toute manipulation accidentelle. Il en résulte l’élimination d’un grand nombre de paramètres de configuration qui sont difficiles à gérer et à tenir à jour.

### <a name="what-is-the-default-consistency-of-an-apache-cassandra-api-account"></a>Quelle est la cohérence par défaut d’un compte d’API Apache Cassandra ?
Par défaut, Azure Cosmos DB prend en charge cinq niveaux de cohérence, comme décrit dans [Niveaux de cohérence des données paramétrables dans Azure Cosmos DB](consistency-levels.md). Pour l’instant, l’API Apache Cassandra ne prend en charge que la cohérence de la session. 
...
Oui, les tables prennent en charge la durée de vie. 

### <a name="what-is-the-default-throughput-of-a-table-when-created-through-cql-what-if-i-need-to-change-it"></a>Quel est le débit par défaut d’une table créée par le biais de CQL ? Comment faire pour le changer ?
Azure Cosmos DB utilise les unités de requête par seconde (RU/s) comme devise pour fournir le débit. Les tables créées par le biais de CQL ont une provision par défaut de 400 RU/s. Vous pouvez changer le débit d’une table dans le portail Azure et monter en puissance jusqu‘à 10 000 RU/s. Si vous avez besoin de plus de 10 000 RU/s pendant la période de préversion, créez votre table dans le portail Azure. Si vous avez besoin de cette fonctionnalité pour CQL, contactez-nous à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

###  <a name="is-it-possible-to-monitor-node-status-replica-status-gc-and-os-parameters-what-should-i-monitor"></a>Est-il possible de surveiller l’état du nœud, l’état du réplica, le garbage collection et les paramètres du système d’exploitation ? Quels sont les éléments que je dois surveiller ?
Azure Cosmos DB est un service de plateforme qui vous permet d’augmenter votre productivité sans vous soucier des opérations de gestion et de surveillance de l’infrastructure. Le débit est la métrique à surveiller. Celle-ci est disponible sous l’onglet Débit de la page des métriques d’Azure Cosmos DB dans le portail Azure. Les graphiques indiquent si vous faites l’objet d’une limitation et si vous devez augmenter ou réduire le débit. Pour plus d’informations, consultez [Surveiller Azure Cosmos DB](monitor-accounts.md) et [Surveillance et débogage à l’aide de métriques dans Azure Cosmos DB](use-metrics.md).
...
L’API Apache Cassandra a été testée avec les pilotes C#, Java, Python et Node Apache Cassandra. Si vous avez besoin d’aide sur d’autres pilotes, envoyez-nous un e-mail à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com).

### <a name="how-do-i-get-ru-capacity-planning-for-the-apache-cassandra-api"></a>Comment obtenir la planification de capacité en RU pour l’API Apache Cassandra ?
Azure Cosmos DB fournit un [planificateur de capacité](https://www.documentdb.com/capacityplanner). Vous fournissez une représentation JSON de l’entité et le planificateur obtient les RU nécessaires pour les lectures/écriture de points. Pour les requêtes et d’autres opérations, les pilotes existants retournent les métadonnées qui contiennent les informations de consommation de RU. Vous pouvez utiliser ces informations pour planifier avec précision. 

### <a name="is-composite-primary-key-supported"></a>Les clés primaires composites sont-elles prises en charge ?
...
Non, le chargeur sstable n’est pas pris en charge pour l’instant. 

### <a name="can-an-on-premise-cassandra-cluster-be-paired-with-the-azure-cosmos-db-apache-cassandra-api"></a>Est-il possible d’associer un cluster Cassandra local à l’API Apache Cassandra d’Azure Cosmos DB ?
Envoyez-nous un e-mail à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com) et décrivez vos scénarios. Le service offre une expérience optimisée pour un environnement cloud, sans la surcharge engendrée par les opérations.

### <a name="do-i-need-to-take-snapshots-and-incremental-backups"></a>Dois-je effectuer des instantanés et des sauvegardes incrémentielles ? 
Azure Cosmos DB réalise deux sauvegardes complètes gratuites toutes les quatre heures, sur toutes les API. Il est donc inutile de créer une planification de sauvegarde. Si vous souhaitez modifier la rétention et la fréquence des sauvegardes, envoyez-nous un e-mail à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

### <a name="where-can-i-provide-feedback-if-a-feature-in-the-regular-cassandra-api-does-not-work"></a>Où puis-je fournir des commentaires si une fonctionnalité de l’API Cassandra normale ne fonctionne pas ?
Envoyez-nous un e-mail à l’adresse [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com).

### <a name="where-can-i-suggest-new-features-for-the-apache-cassandra-api"></a>Où puis-je suggérer de nouvelles fonctionnalités pour l’API de Cassandra Apache ?
Envoyez-nous un e-mail à l’adresse [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). Vous pouvez également donner votre avis sur le site des [commentaires UserVoice](https://feedback.azure.com/forums/263030-azure-cosmos-db). 

### <a name="why-do-i-need-to-choose-a-throughput-level"></a>Pourquoi est-il nécessaire de choisir un niveau de débit ?
Azure Cosmos DB définit le débit par défaut pour votre conteneur en fonction de l’endroit où vous créez la table (portail ou CQL). 

Azure Cosmos DB fournit des garanties de performances et de latence avec des limites supérieures pour les opérations. Cette garantie est possible car le moteur peut assurer la gouvernance des opérations du locataire. La définition du débit est pour vous l’assurance de bénéficier du débit et de la latence garantis, car la plateforme réserve cette capacité et garantit le succès des opérations. Pour plus d’informations, voir [Unités de requête dans Azure Cosmos DB](request-units.md).

La spécification du débit vous permet de changer celui-ci en souplesse pour tirer parti du caractère saisonnier de votre application, répondre à vos besoins de débit et réduire les coûts.

### <a name="i-never-get-a-quota-full-notification-indicating-that-a-partition-is-full-when-i-ingest-data-into-regular-cassandra-with-the-azure-cosmos-db-cassandra-api-i-do-get-this-message-is-this-offering-limiting-me-and-forcing-me-to-change-my-existing-application"></a>Je ne reçois jamais de notification de « quota complet » (indiquant qu’une partition est pleine) lorsque j’ingère des données dans l’API Cassandra normale. Avec l’API Cassandra d’Azure Cosmos DB, je reçois ce message. Cette offre me limite-t-elle et m’oblige-t-elle à modifier mon application existante ?

Azure Cosmos DB est un système basé sur un contrat de niveau de service (SLA) permettant une mise à l’échelle illimitée, avec des garanties en matière de latence, de débit, de disponibilité et de cohérence. Pour être certain de bénéficier des performances premium garanties, assurez-vous que la taille de vos données et votre index sont gérables et extensibles. La limite de 10 Go appliquée au nombre d’entités ou d’éléments par clé de partition est conçue pour qu’Azure Cosmos DB fournisse des performances de recherche et de requête exceptionnelles. Pour vous assurer que la mise à l’échelle de votre application fonctionne correctement, nous vous recommandons de ne *pas* créer de partition sensible en stockant toutes les informations dans une seule partition et en interrogeant celle-ci. 

### <a name="how-does-the-cassandra-api-handle-failover-if-a-region-goes-down"></a>Comment l’API Cassandra gère-t-elle le basculement en cas de défaillance d’une région ? 
L’API Cassandra d’Azure Cosmos DB utilise la plateforme distribuée mondialement d’Azure Cosmos DB. Pour vérifier que votre application peut tolérer des temps d’arrêt de centre de données, activez au moins une région supplémentaire pour le compte dans le portail Azure Cosmos DB. Vous pouvez définir la priorité de la région à l’aide du portail, comme décrit dans [Développement avec des comptes Azure Cosmos DB multirégions](regional-failover.md). 

Vous pouvez ajouter autant de régions que vous le souhaitez pour le compte, et contrôler l’emplacement de basculement du compte en indiquant une priorité. Bien entendu, pour utiliser la base de données, vous devez également fournir une application à cet emplacement. Si vous procédez de la sorte, vos clients ne subissent aucun temps d’arrêt.  

### <a name="is-the-apache-cassandra-api-enabled-for-backups"></a>L’API Apache Cassandra prend-elle en charge les sauvegardes ?
Oui, l’API Apache Cassandra d’Azure Cosmos DB propose les mêmes sauvegardes qu’Azure Cosmos. Les sauvegardes sont effectuées automatiquement. Pour plus d’informations, voir [Sauvegarde et restauration en ligne automatiques avec Azure Cosmos DB](online-backup-and-restore.md).
...
Oui, tous les attributs d’une entité sont indexés par défaut par Azure Cosmos DB. Pour plus d’informations, voir [Comment Azure Cosmos DB indexe-t-il les données ?](indexing-policies.md). Vous bénéficiez de performances garanties avec une indexation cohérente et des écritures durables validées dans un quorum. 
...
### <a name="can-i-change-the-indexing-policy"></a>Puis-je modifier la stratégie d’indexation ?
Oui, vous pouvez modifier la stratégie d’indexation en fournissant la définition d’index. Pour plus d’informations, voir [Fonctionnalités d’Azure Cosmos DB](../cosmos-db/tutorial-develop-table-dotnet.md#azure-cosmos-db-capabilities). Vous devez correctement encoder et placer dans une séquence d’échappement les paramètres. Pour plus d‘informations, envoyez un e-mail à [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

Voici un exemple du format de chaîne JSON dans le fichier app.config :
...
L’[émulateur local](local-emulator.md) ne prend pas en charge l’API Cassandra pour l’instant. 
...
Un autre témoin de notre travail continu est la base étendue pour une extensibilité mondiale et flexible du débit et du stockage. Nous avons apporté plusieurs améliorations fondamentales au sous-système de distribution mondiale. Une de ces nombreuses fonctionnalités côté développeur est le modèle de cohérence Préfixe cohérent, qui regroupe en fait cinq modèles de cohérence bien définis. Nous publierons beaucoup plus de fonctionnalités intéressantes à mesure qu’elles arriveront à maturité. 
