...
<ol start="6">
<li><p>Accédez au <code>\services\mobileServices\scripts</code> dossier du projet, copiez le fichier de script &lt;<em>your_service_name</em>&gt;.push.register.js généré dans le dossier partagé, <code>\js</code> puis supprimez ce fichier des deux projets d'application Windows et WindowsPhone.</p></li> 
<li><p>Ouvrez le fichier de script dans le dossier de projet partagé, <code>\js</code> recherchez le code dans l'écouteur d'événements <em>activé</em> qui enregistre l'URL de canal d'appareil avec le concentrateur de notification, puis supprimez la fonction de promesse <strong>done</strong>.</p>
...
<li><p>Dans le projet d'application Windows, ouvrez le fichier default.html et modifiez le chemin d'accès de la référence du fichier de script vers le dossier de projet partagé, <code>\js</code> pour qu'il se présente comme suit :</p><pre><code>&lt;script src="/js/your_service_name.push.register.js"&gt;&lt;/script&gt;</code></pre></li>
...
<ol start="5">
<li><p>Accédez au <code>\services\mobileServices\settings</code> dossier du projet, copiez le fichier de script &lt;<em>your_service_name</em>&gt;.js généré dans le dossier de projet partagé, <code>\js</code> puis supprimez ce fichier des deux projets d'application Windows et WindowsPhone. Supprimez également ce fichier du <code>\services\mobileServices\scripts</code> dossier dans chaque projet d'application s'il existe également dans ce dernier.</p></li> 
<li><p>Ouvrez le fichier de script dans le dossier de projet partagé <code>\js</code> et placez en commentaire le code existant qui définit l'objet <a href="http://msdn.microsoft.com/fr-fr/library/azure/jj554219.aspx">MobileServiceClient object</a> utilisé pour accéder au service mobile exécuté dans Azure.</p></li>
...
var todolistClient = new WindowsAzure.MobileServiceClient(
	"http://localhost:4584");
</code></pre><p>En utilisant cet objet <strong>MobileServiceClient</strong>, l'application se connecte au service local plutôt qu'à la version hébergée dans Azure. Si vous souhaitez revenir à une exécution de l'application sur le service mobile hébergé dans Azure, réintégrez les définitions d'objet <strong>MobileServiceClient</strong> d'origine.</p></li>
</ol>
