<properties pageTitle="Utilisation des données hors connexion dans Mobile Services (Xamarin iOS) | Centre de développement mobile" description="Learn how to use Azure Mobile Services to cache and sync offline data in your Xamarin iOS application" documentationCenter="xamarin" authors="lindydonna" editor="wesmc" manager="dwrede" services=""/>
...
Cette section décrit le code relatif à la synchronisation hors dans  `QSTodoService.cs`.

1. Dans Visual Studio, ouvrez le projet que vous avez terminé dans le didacticiel [Prise en main de Mobile Services]. Ouvrez le fichier  `QSTodoService.cs`.
...
1. Ouvrez  `QSTodoService.cs`. Modifiez les méthodes  `InsertTodoItemAsync()` et  `CompleteItemAsync()` et commentez les appels vers  `SyncAsync()`.
...
3. Pour modifier l'initialisation de l'interface utilisateur afin qu'elle ne se synchronise pas lors du démarrage de l'application, modifiez la méthode  `QSTodoListViewController.ViewDidLoad()`. Supprimez l'appel vers  `RefreshAsync()` situé la fin de la méthode et remplacez-le par un appel vers  `ReloadData()` :
...
4. Dans l'application, effectuez le geste d'actualisation en tirant la liste des éléments vers le bas. Ainsi, l'application appelle  `RefreshDataAsync()`, qui appelle à son tour  `SyncAsync()`. Les opérations push et pull sont effectuées, tout d'abord pour envoyer les éléments du magasin local vers le service mobile, puis pour récupérer les nouvelles données à partir du service.
