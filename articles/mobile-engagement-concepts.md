...
Mobile Engagement définit quelques concepts communs à toutes les plateformes prises en charge. Cette page décrit brièvement ces concepts.

Cette page est également un bon point de départ si vous débutez avec Mobile Engagement. Assurez-vous de lire la documentation propre à la plateforme que vous utilisez, car elle définira plus en détail les concepts décrits ici, avec plus d'exemples et en indiquant les éventuelles limitations.
...
Les sessions sont calculées automatiquement à partir de la séquence d'activités effectuées par les utilisateurs : une session démarre quand l'utilisateur commence sa première activité et s'arrête quand il termine sa dernière activité. Cela signifie que les sessions ne doivent pas explicitement être démarrées ou arrêtées. Ce qui n'est pas le cas des activités : si aucune activité n'est signalée, aucune session n'est signalée.
...
##Blocage
Les blocages sont émis automatiquement par le SDK Mobile Engagement pour signaler les échecs de l'application (par ex., les problèmes non détectés par l'application qui entraînent son blocage).

##Informations de l'application
Les informations de l'application (ou `app info`) sont utilisées pour baliser les utilisateurs, c'est-à-dire pour associer des données aux utilisateurs d'une application (elles sont assimilables aux cookies web, sauf qu'elles sont stockées côté serveur sur la plateforme Mobile Engagement).
