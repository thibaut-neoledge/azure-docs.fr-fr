<properties 
	pageTitle="Utilisation d'Apache Giraph avec Azure HDInsight" 
	description="Découvrez comment utiliser Apache Giraph pour traiter des graphiques avec Azure HDInsight" 
	services="hdinsight" 
	documentationCenter="" 
	authors="blackmist" 
	manager="paulettm" 
...
* [Exécution de l'exemple SimpleShortestPathsComputation](#run)
	...
	Pour obtenir une liste d'autres exemples fournis avec Giraph, consultez la section [Package org.apache.giraph.examples](https://giraph.apache.org/apidocs/org/apache/giraph/examples/package-summary.html).
...
* [Maven](http://maven.apache.org/) 3 ou version ultérieure
...
1. À cette date (14/07/2014), Giraph nécessite un correctif pour fonctionner avec le système de stockage de fichiers WASB utilisé par HDInsight. Ce correctif a été soumis au projet Apache Giraph, mais n'a pas encore été accepté. Téléchargez-le dans la section __attachments__ de [GIRAPH-930](https://issues.apache.org/jira/browse/GIRAPH-930) et enregistrez-le dans le lecteur local sous __giraph-930.diff__.

1. À partir d'une ligne de commande, utilisez la commande Git ci-après pour créer un clone du référentiel Giraph.

		git clone https://github.com/apache/giraph.git

2. Modifiez les répertoires dans le répertoire __giraph__ créé par l'opération de clonage à l'étape 2.
...
3. Fusionnez le correctif au référentiel local à l'aide de la commande ci-après.
	...
	Remplacez __giraph-930.diff__ par le chemin d'accès au fichier créé à l'étape 1.
...
4. Téléchargez l'exemple de fichier JAR vers le stockage principal de votre cluster HDInsight avec [Azure PowerShell][aps] et [HDInsight-Tools][tools].
	...
	Remplacez __giraph-examples-1.1.0-SNAPSHOT-for-hadoop-0.23.1-jar-with-dependencies.jar__ par le chemin d'accès et le nom du fichier JAR produit à l'étape précédente, et __clustername__ par le nom de votre cluster HDInsight. Par exemple, si vous générez le package avec le paramètre `-Phadoop_0.20.203`, le nom du fichier JAR comprend __hadoop-0.20.203__.
...
SimpleShortestPathsComputation montre l'implémentation de base de [Pregel](http://people.apache.org/~edwardyoon/documents/pregel.pdf) pour trouver le chemin le plus court entre des objets d'un graphique. Procédez comme suit pour télécharger les exemples de données, exécuter une tâche avec l'exemple SimpleShortestPathsComputation et afficher les résultats.

> [AZURE.NOTE] La source de cet exemple (ainsi que d'autres exemples) est disponible dans la [branche de la version 1.1](https://github.com/apache/giraph/tree/release-1.1) du [référentiel GitHub](https://github.com/apache/giraph).

1. Créez un fichier nommé __tiny\_graph.txt__. Il doit contenir les lignes suivantes.
...
2. Téléchargez le fichier __tiny\_graph.txt__ vers le stockage principal de votre cluster HDInsight à l'aide d'[Azure PowerShell][aps] et de [HDInsight-Tools][tools].
...
3. Utilisez la commande PowerShell suivante pour exécuter l'exemple __SimpleShortstPathsComputation__ en utilisant le fichier __tiny\_graph.txt__ comme entrée. Pour cela, il est nécessaire que vous ayez installé et configuré [Azure PowerShell][aps].
...
Une fois la tâche terminée, les résultats sont stockés dans le dossier __wasb:///example/out/shotestpaths__ sous la forme de fichiers __part-m-#####__. Utilisez [Azure PowerShell][aps] et [HDInsight-Tools][tools] pour télécharger les fichiers de sortie.
...
Cette opération crée la structure de répertoires __example/output/shortestpaths__ dans le répertoire actuel et télécharge les fichiers commençant par __part__. La cmdlet __Cat__ affiche alors le contenu des fichiers, qui doivent avoir l'aspect suivant.
...
> [AZURE.NOTE] La ligne de commande Hadoop n'est disponible qu'en cas de connexion au cluster HDInsight avec le Bureau à distance.
> 
...
1. À partir du [portail de gestion Azure](https://manage.windowsazure.com), sélectionnez votre cluster HDInsight, puis sélectionnez __Configuration__.

2. Au bas de la page, sélectionnez __Activer Distant__ et fournissez le nom d'utilisateur, le mot de passe et la date d'expiration de la connexion Bureau à distance.

3. Une fois la demande d'activation du Bureau à distance traitée, une nouvelle entrée apparaît dans __Connecter__ au bas de la page. Sélectionnez-la pour télécharger le fichier .RDP pour la session Bureau à distance.
...
Maintenant que vous êtes capable d'utiliser Giraph avec HDInsight, essayez [Pig][] et [Hive][] avec HDInsight.
