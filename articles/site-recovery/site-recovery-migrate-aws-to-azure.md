...
Cet article explique comment utiliser Site Recovery pour migrer ou basculer des instances de Windows s’exécutant dans AWS sur Azure. Cet article fait appel à la plupart des étapes décrites dans la rubrique [Configurer la protection entre des machines virtuelles ou des serveurs physiques VMWare locaux et Azure](site-recovery-vmware-to-azure-classic-legacy.md). Nous vous suggérons de lire cet article pour obtenir des instructions détaillées sur chaque étape du déploiement.
...
- En savoir plus sur ces composants dans la rubrique [De quoi ai-je besoin ?](site-recovery-vmware-to-azure-classic-legacy.md#what-do-i-need)
- Vous devez également lire les instructions de [planification de capacité](site-recovery-vmware-to-azure-classic-legacy.md#capacity-planning) et vous assurer que vous disposez de toutes les [conditions préalables au déploiement](site-recovery-vmware-to-azure-classic-legacy.md#before-you-start) en place avant de commencer.

## Étapes du déploiement

1. [création d'un coffre](site-recovery-vmware-to-azure-classic-legacy.md#step-1-create-a-vault)
2. [Déploiement du serveur de configuration](site-recovery-vmware-to-azure-classic-legacy.md#step-2-deploy-a-configuration-server) en tant que machine virtuelle Azure.
3. [déploiement du serveur cible principal](site-recovery-vmware-to-azure-classic-legacy.md#step-2-deploy-a-configuration-server) en tant que machine virtuelle Azure.
4. [Déploiement d’un serveur de processus](site-recovery-vmware-to-azure-classic-legacy.md#step-4-deploy-the-on-premises-process-server). Notez les points suivants :
...
5. [Installer les dernières mises à jour](site-recovery-vmware-to-azure-classic-legacy.md#step-5-install-latest-updates). Assurez-vous que tous les serveurs de composant que vous avez installés sont à jour.
6. [Créer un groupe de protection](site-recovery-vmware-to-azure-classic-legacy.md#step-7-create-a-protection-group). Pour commencer à protéger les instances migrées à l’aide de Site Recovery, vous devez configurer un groupe de protection. Vous spécifiez les paramètres de réplication pour un groupe et elles seront appliquées à toutes les instances que vous ajoutez à ce groupe. 
7. [Configurer des machines virtuelles](site-recovery-vmware-to-azure-classic-legacy.md#step-8-set-up-machines-you-want-to-protect). Vous devrez accéder au service de mobilité installé sur chaque instance (automatiquement ou manuellement).
8. [Activer la protection des machines virtuelles](site-recovery-vmware-to-azure-classic-legacy.md#step-9-enable-protection). Activez la protection des instances en les ajoutant à un groupe de protection. Notez les points suivants :
