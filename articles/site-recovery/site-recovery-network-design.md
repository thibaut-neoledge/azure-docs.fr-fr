<properties
	pageTitle="Considérations sur l’infrastructure réseau pour Site Recovery | Microsoft Azure" 
	description="Cet article aborde les considérations de conception de réseau pratiques pour le basculement avec Site Recovery" 
...
#  Considérations d'infrastructure réseau pour Site Recovery

Le service Azure Site Recovery participe à une solution fiable de continuité d’activité et de récupération d’urgence (Business Continuity and Disaster Recovery, BCDR) en protégeant et en effectuant la récupération de vos machines virtuelles et de vos serveurs physiques locaux au moyen de l’orchestration et de l’automatisation de la réplication et du basculement vers Azure ou vers un centre de données secondaire local.

Cet article s’adresse à l’équipe de virtualisation responsable de l’architecture, de l’implémentation et de la prise en charge d’une infrastructure et d’une solution BCDR qui comprend System Center VMM et Azure Site Recovery.

## Vue d'ensemble

L'objectif de votre stratégie BCDR est de conserver vos applications d'entreprise en cours d'exécution et de restaurer les charges de travail et les services ayant échoué afin que l'organisation puisse rapidement reprendre ses opérations habituelles. Le développement de stratégies de récupération d'urgence représente un défi en raison de la difficulté à prévoir des événements imprévus et le coût élevé de l'implémentation d'une protection adéquate contre les défaillances importantes. Azure Site Recovery vous permet de mettre en œuvre la protection et le basculement de votre centre de données principal vers un centre de données secondaire (ou vers Azure) en effectuant une copie (réplication) initiale de vos données primaires, puis en actualisant régulièrement les réplicas.
...
Si vous avez décidé d'utiliser le service Azure Site Recovery pour faciliter la mise en place de la protection et du basculement, deux problèmes essentiels se présentent dans la conception du réseau pour la solution BCDR :

- **Conception VMM** : j'utilise System Center VMM. Comment dois-je gérer la conception du réseau pour intégrer VMM et Site Recovery ?
- **Connectivité après basculement** : j'utilise Site Recovery pour exécuter un basculement. Comment puis-je m'assurer que mes applications et mes charges de travail sont disponibles et accessibles une fois le basculement effectué ?

## Conception VMM

Vous pouvez superposer Site Recovery à votre conception VMM existante, que cette conception utilise ou non un seul ou plusieurs serveurs VMM.

### Serveur VMM autonome
...
![Cluster VMM étiré](./media/site-recovery-network-design/ASR_NetworkDesign1.png)

## Connectivité après un basculement
...
![Avant le basculement](./media/site-recovery-network-design/ASR_NetworkDesign2.png)

**Après le basculement**

![Après le basculement](./media/site-recovery-network-design/ASR_NetworkDesign3.png)
...
2. Si l'administrateur définit le même pool d'adresses IP qui est utilisé sur le site principal pour le réseau du site de récupération, Site Recovery alloue la même adresse IP allouée à la machine virtuelle principale à la machine virtuelle à l'adresse IP de machine virtuelle de réplication. L’adresse IP est réservée dans VMM mais n'est pas définie en tant qu'adresse IP de basculement. L'adresse IP de basculement est définie juste avant le basculement. Cette capture d'écran montre les paramètres TCP/IP de basculement de la machine virtuelle de réplication (sur la console Hyper-V). Ces paramètres sont répliqués juste avant le démarrage du basculement de la machine virtuelle.
	...
	![Configurer l’adresse IP](./media/site-recovery-network-design/ASR_NetworkDesign4.png)
...
- Un réseau logique basé sur VLAN (réseau d’application) a été créé sur le serveur VMM. ![Réseau logique](./media/site-recovery-network-design/ASR_NetworkDesign5.png)
- Un réseau de machines virtuelles (réseau de machines virtuelles d’application) a été créé à l'aide du réseau logique. ![Réseau de machines virtuelles](./media/site-recovery-network-design/ASR_NetworkDesign6.png)
...
- Nous allons devoir créer un réseau supplémentaire dans Azure (réseau de récupération) dans lequel les machines virtuelles basculées seront créées.
	...
	![Réseaux Azure](./media/site-recovery-network-design/ASR_NetworkDesign7.png)
...
![Réseaux Azure](./media/site-recovery-network-design/ASR_NetworkDesign10.png)
...
- Un basculement et certaines mises à jour DNS conserveront l’accès aux applications. Si le serveur DNS est configuré pour autoriser les mises à jour dynamiques, les machines virtuelles peuvent alors s'enregistrer à l'aide de la nouvelle adresse IP une fois qu'elles démarrent après un basculement.
	...
	![Réseaux Azure](./media/site-recovery-network-design/ASR_NetworkDesign11.png)
