...
- **Même adresse IP** : vous souhaiterez peut-être utiliser la même plage d’adresses IP dans Azure que celle du site principal local après le basculement. Dans un scénario normal, vous devriez mettre à jour les itinéraires avec le nouvel emplacement des adresses IP. Toutefois, si un VLAN étiré est déployé entre le site principal et Azure, il est possible de conserver les adresses IP pour les machines virtuelles. Conserver les mêmes adresses IP simplifie la récupération en réduisant les problèmes liés au réseau après le basculement.


## <a name="retain-ip-addresses"></a>Conserver les adresses IP

Du point de vue de la récupération d’urgence, l’utilisation d’adresses IP fixes semble être la méthode la plus simple, mais il existe un certain nombre de difficultés potentielles. Site Recovery offre la possibilité de conserver les adresses IP lors du basculement vers Azure en basculant les sous-réseaux.


### <a name="subnet-failover"></a>Basculement de sous-réseau

Dans ce scénario, un sous-réseau spécifique est présent sur le Site 1 ou sur le Site 2, mais jamais sur les deux sites simultanément. Pour conserver l’espace d’adressage IP en cas de basculement, vous pouvez mettre en place un programme pour que l’infrastructure du routeur déplace les sous-réseaux d’un site à un autre. Pendant le basculement, les sous-réseaux se déplacent avec les machines virtuelles protégées associées. Le principal inconvénient de cette approche est que vous devez déplacer l’ensemble du sous-réseau en cas de défaillance, ce qui peut affecter les considérations relatives à la granularité du basculement.
...
- Woodgrove souhaite utiliser Site Recovery pour répliquer les charges de travail locales vers Azure.
 - Woodgrove doit gérer des applications et des configurations qui dépendent d’adresses IP codées en dur. Elle doit donc conserver les adresses IP de ses applications après le basculement vers Azure.
...
3. Lorsque vous configurez la connexion de site à site, dans le réseau Azure, vous pouvez acheminer le trafic vers l’emplacement local (réseau local) uniquement si la plage d’adresses IP est différente de la plage d’adresses IP locales.
    - Cela est dû au fait qu’Azure ne prend pas en charge les sous-réseaux étirés. Par conséquent, si vous possédez un sous-réseau 192.168.1.0/24 local, il est impossible d’ajouter un réseau local 192.168.1.0/24 dans le réseau Azure.
    - Cela est normal, car Azure ignore le fait que le sous-réseau est dépourvu de machines virtuelles actives, et que ce sous-réseau est créé uniquement à des fins de récupération d’urgence.
    - Pour que le trafic réseau puisse être acheminé correctement en dehors d’un réseau Azure, les sous-réseaux et le réseau local ne doivent pas être en conflit.
...
3. Lorsque la machine virtuelle est basculée, Azure Site Recovery lui assigne l’adresse IP fournie.
