...
---
# <a name="replicate-vmware-virtual-machines-to-azure-with-azure-site-recovery"></a>Répliquer des machines virtuelles VMware sur Azure avec Azure Site Recovery
...
Cet article explique comment répliquer des machines virtuelles VMware locales sur Azure à l’aide du service [Azure Site Recovery](site-recovery-overview.md) dans le portail Azure.
 ...
 Si vous souhaitez uniquement migrer des machines virtuelles VMware avec un basculement simple, sans réplication complète (réplication, basculement, restauration automatique), consultez [cet article](site-recovery-migrate-to-azure.md).
...
- **Préparer un compte pour la découverte automatique** : le serveur de traitement Site Recovery découvre automatiquement les machines virtuelles. Pour cela, Site Recovery a besoin des informations d’identification permettant d’accéder aux serveurs vCenter/hôtes vSphere ESXi.
...
4. [Créez un groupe de ressources](../azure-resource-manager/resource-group-template-deploy-portal.md)ou sélectionnez-en un. Spécifiez une région Azure. Pour découvrir les régions prises en charge, référez-vous à la disponibilité géographique de la page [Détails des prix d'Azure Site Recovery](https://azure.microsoft.com/pricing/details/site-recovery/)
...
1. Dans Programme d’installation unifiée > **Avant de commencer**, sélectionnez **Installer le serveur de configuration et le serveur de processus**.

   ![Avant de commencer](./media/site-recovery-vmware-to-azure/combined-wiz1.png)
2. Dans **Licence de logiciel tiers**, cliquez sur **J’accepte le contrat de licence tiers** pour télécharger et installer MySQL.

    ![Logiciel tiers](./media/site-recovery-vmware-to-azure/combined-wiz105.PNG)
3. Dans **Inscription**, recherchez et sélectionnez la clé d’inscription que vous avez téléchargée à partir du coffre.

    ![Inscription](./media/site-recovery-vmware-to-azure/combined-wiz3.png)
4. Dans **Paramètres Internet**, indiquez de quelle manière le fournisseur qui s’exécute sur le serveur de configuration doit se connecter à Site Recovery par le biais d’Internet (443).
   ...
   * Si vous souhaitez utiliser un proxy, sélectionnez **Se connecter à Azure Site Recovery à l’aide d’un serveur proxy** et spécifiez les paramètres.

     ![Pare-feu](./media/site-recovery-vmware-to-azure/combined-wiz4.png)
5. Dans **Vérification de la configuration requise**, le programme d’installation vérifie les paramètres.  Si un avertissement de temps s’affiche, vérifiez que l’heure dans les paramètres **Date et heure** est identique au fuseau horaire.

    ![Composants requis](./media/site-recovery-vmware-to-azure/combined-wiz5.png)
6. Dans **Configuration MySQL**, spécifiez les informations d’identification utilisées pour se connecter à l’instance de serveur MySQL qui sera installée.
...
8. Dans **Emplacement d’installation**, sélectionnez l’emplacement où vous voulez installer les fichiers binaires et stocker le cache. Vous pouvez sélectionner tout lecteur disposant de 5 Go de stockage disponible. Toutefois, nous vous recommandons d’utiliser un lecteur de cache présentant au moins 600 Go d’espace disponible.

    ![Emplacement d’installation](./media/site-recovery-vmware-to-azure/combined-wiz8.png)
9. Dans **Sélection du réseau**, spécifiez l’écouteur (carte réseau et port SSL) sur lequel le serveur de configuration envoie et reçoit les données de réplication. Vous pouvez modifier la valeur par défaut du port (9443). En plus de ce port, le port 443 est utilisé pour orchestrer la réplication. N’utilisez pas ce port pour le trafic de réplication.

    ![Sélection du réseau](./media/site-recovery-vmware-to-azure/combined-wiz9.png)


10. Dans **Résumé**, passez en revue les informations, puis cliquez sur **Installer**. Une fois l’installation terminée, une phrase secrète est générée. Vous en avez besoin lorsque vous activez la réplication : copiez-la et conservez-la en lieu sûr. Une fois l’inscription terminée, le serveur s’affiche dans **Serveurs** dans le coffre.
...
### <a name="add-the-account-for-automatic-discovery"></a>Ajouter le compte pour la découverte automatique
 ...
 Ajoutez le compte que vous avez créé pour la détection automatique des machines virtuelles VMware.

1. Sur le serveur de configuration, exécutez **CSPSConfigtool.exe**. Il est disponible sous forme de raccourci sur le bureau ou dans le dossier **[EMPLACEMENT D’INSTALLATION]\home\svsystems\bin**.
...
- Si vous ajoutez le serveur vCenter ou des hôtes vSphere avec un compte ne disposant pas de droits d’administrateur sur le serveur, ces droits doivent être activés pour le compte :
    ...
    - Le serveur vCenter a besoin du privilège d’affichage de stockage.
...
2. Cliquez sur **Préparer l’infrastructure** > **Source**. Dans le panneau **Préparer la source**, sélectionnez le serveur de configuration. Cliquez sur **+vCenter** pour ajouter un hôte vSphere ou un serveur vCenter.
3. Dans **Ajouter un serveur vCenter**, spécifiez un nom convivial pour le serveur et indiquez l’adresse IP ou le nom de domaine complet public. Conservez le port 443, sauf si les serveurs VMware sont configurés pour écouter les demandes sur un port différent. Ensuite, sélectionnez le compte que vous avez créé pour la découverte automatique, puis cliquez sur **OK**.
...
1. Dans **Calcul et réseau** > **Propriétés de calcul**, vous pouvez spécifier la taille de la cible et le nom de la machine virtuelle Azure. Au besoin, modifiez ce nom en fonction de la [configuration Azure requise](site-recovery-support-matrix-to-azure.md#failed-over-azure-vm-requirements) .
2. Modifiez les paramètres du réseau cible, du sous-réseau et de l’adresse IP qui seront affectés à la machine virtuelle Azure :
...
1. Dans **Disques**, vous pouvez voir le système d’exploitation de la machine virtuelle et les disques de données qui seront répliqués.

## <a name="run-a-test-failover"></a>Exécution d’un test de basculement

Après avoir terminé la configuration, [exécutez un test de basculement](site-recovery-test-failover-to-azure.md) afin de vérifier que tout fonctionne bien.
