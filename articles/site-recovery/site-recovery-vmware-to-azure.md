---
title: Répliquer des machines virtuelles VMware et des serveurs physiques sur Azure avec Azure Site Recovery dans le portail Azure | Microsoft Docs
...
> [!NOTE]
> Azure dispose de deux [modèles de déploiement](../resource-manager-deployment-model.md) pour créer et utiliser des ressources : Azure Resource Manager et le modèle classique. De plus, Azure propose deux portails : le portail Azure Classic, qui prend en charge le modèle de déploiement classique, et le portail Azure, qui gère les deux modèles de déploiement.
> 
> 
...
## <a name="business-advantages"></a>Avantages pour l’entreprise
...
2. Créez un utilisateur sur l’hôte vSphere ou vCenter et attribuez le rôle à l’utilisateur. Vous communiquerez ensuite à Site Recovery ces informations d’identification pour permettre la découverte automatique.
   ...
   > [!NOTE]
   > Un compte d’utilisateur vCenter doté d’un rôle en lecture seule peut exécuter le basculement mais il ne peut pas arrêter les machines source protégées. Si vous souhaitez arrêter les machines virtuelles, vous aurez besoin du rôle [Azure_Site_Recovery](#vmware-account-permissions). Si vous ne faites que migrer des machines virtuelles de VMware vers Azure sans restauration automatique, le rôle en lecture seule est suffisant.
   > 
   > 
...
4. Si vous n’avez pas créé de compte de stockage et que vous souhaitez le faire par le biais d’ARM, cliquez sur **+Compte de stockage** afin de procéder à cette opération en ligne.  Dans le panneau **Créer un compte de stockage** , saisissez le nom, le type, l’abonnement associé et l’emplacement du compte de stockage. Ce compte doit se trouver dans la même région que le coffre Recovery Services.
   ...
   * Si vous utilisez un compte Premium Storage pour les données répliquées, vous devez configurer un compte de stockage standard supplémentaire, afin de stocker les journaux de réplication qui capturent les modifications apportées en continu aux données locales.
   ...
   > 
   > 
5. Sélectionnez un réseau Azure. Si vous n’avez pas créé de réseau et que vous souhaitez le faire par le biais d’ARM, cliquez sur **+Réseau** afin de procéder à cette opération en ligne. Dans le panneau **Créer un réseau virtuel** , spécifiez le nom, la plage d’adresses, les informations sur le sous-réseau associé, l’abonnement et l’emplacement du réseau virtuel. Ce réseau doit se trouver au même emplacement que le coffre Recovery Services.
...
1. Cliquez sur **Télécharger** pour télécharger l’outil, puis exécutez-le. [l’article relatif à cet outil](site-recovery-capacity-planner.md) .
2. Lorsque vous avez terminé, sélectionnez **Yes** in **Avez-vous terminé la planification de la capacité ?**
...
| CentOS 6.5, 6.6, 6.7 (64 bits uniquement) |Microsoft-ASR_UA_9.*.0.0_RHEL6-64_*release.tar.gz |
| CentOS 7.0, 7.1, 7.2 (64 bits uniquement) |Microsoft-ASR_UA_9.*.0.0_RHEL7-64_*release.tar.gz |
| Red Had Enterprise Linux 6.7 (64 bits uniquement) |Microsoft-ASR_UA_9.*.0.0_RHEL6-64_*release.tar.gz |
| Red Had Enterprise Linux 7.1, 7.2 (64 bits uniquement) |Microsoft-ASR_UA_9.*.0.0_RHEL7-64_*release.tar.gz |
...
2. Ouvrez un interpréteur de commandes et extrayez l’archive tar compressée dans un emplacement local en exécutant la commande suivante : `tar -xvzf Microsoft-ASR_UA_9.3.0.0*`
...
6. Dans **Cible**, sélectionnez l’abonnement du coffre, puis dans **Modèle de déploiement post-basculement**, sélectionnez le modèle (Classic ou Azure Resource Manager) que vous souhaitez utiliser dans Azure après le basculement.
...
2. Dans **Server Type** click **Serveur de processus (local)**.
...
4. Dans la zone **Avant de commencer**, sélectionnez **Ajouter des serveurs de traitement supplémentaires pour effectuer un déploiement avec montée en puissance**.
...
* [Découvrez plus d’informations sur la restauration automatique](site-recovery-failback-azure-to-vmware.md) qui permet de ramener dans votre environnement local vos machines basculées s’exécutant dans Azure.
