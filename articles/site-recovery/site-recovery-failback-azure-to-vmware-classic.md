...
- Si vous procédez à une restauration automatique vers la machine virtuelle d’origine, les conditions suivantes doivent être respectées :
	- Si la machine virtuelle est gérée par un serveur vCenter, la machine virtuelle du site local doit utiliser la même banque de données que la machine virtuelle qui s’exécute sur le serveur cible maître local. Si tel n’est pas le cas, vous devez la migrer afin de procéder à une restauration automatique sur la même machine virtuelle.
	- Si la machine virtuelle se trouve sur un ordinateur hôte ESX, mais n’est pas gérée par vCenter, son disque dur doit se situer dans la même banque de données que la machine virtuelle cible principale.
	- Si votre machine virtuelle se trouve sur un ordinateur hôte ESX et n’utilise pas vCenter, vous devez exécuter la détection de l’ordinateur hôte ESX avant d’assurer la reprotection. Cela s’applique également si vous restaurez automatiquement des serveurs physiques.
