...
3. Dans la zone **Avant de commencer**, sélectionnez **Installer le serveur de configuration et le serveur de processus**. Selon la taille de votre déploiement, il se peut que vous ayez besoin de serveurs de traitement supplémentaire ultérieurement, mais pas lorsque vous configurez ce déploiement pour la première fois.
...
5. Dans **Paramètres Internet**, indiquez de quelle manière le fournisseur à installer sur le serveur doit se connecter à Azure Site Recovery par le biais d’Internet.
...
>[AZURE.WARNING] Si un message d’avertissement lié à la vérification de la configuration requise de **Synchronisation globale de l’heure** s’affiche, vérifiez que l’heure de l’horloge système est la même que celle du fuseau horaire.
...
- Si vous avez déjà créé un groupe de protection et que vous ajoutez ensuite un serveur vCenter ou un hôte VMware ESXi, l’actualisation du portail Azure Site Recovery et l’affichage des machines virtuelles dans la boîte de dialogue **Ajouter des ordinateurs à un groupe de protection** peuvent prendre plus de 15 minutes.
