---
title: "Répliquer des machines virtuelles Hyper-V dans un cloud VMM sur un site secondaire avec Azure Site Recovery à l’aide d’un SAN | Documents Microsoft"
...
---
# <a name="replicate-hyper-v-vms-in-a-vmm-cloud-to-a-secondary-site-with-azure-site-recovery-using-san"></a>Répliquer des machines virtuelles Hyper-V dans un cloud VMM sur un site secondaire avec Azure Site Recovery à l’aide d’un SAN
Dans cet article, vous allez apprendre à déployer Site Recovery pour orchestrer et automatiser la réplication et le basculement SAN des machines virtuelles Hyper-V situées dans les clouds VMM System Center vers un site VMM secondaire.

Après la lecture de cet article, publiez des commentaires ou des questions au bas de cet article ou sur le [Forum Azure Recovery Services](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr). 
...
Cet article contient des instructions pour configurer la réplication des machines virtuelles Hyper-V à partir d’un site VMM vers un autre à l’aide de la réplication SAN. Il fournit une vue d’ensemble de l’architecture, ainsi que les instructions et les conditions préalables au déploiement. Vous allez découvrir et classer le stockage SAN dans VMM, approvisionner des LUN et allouer de l'espace de stockage aux clusters Hyper-V. Pour finir, vous pourrez tester le basculement pour vous assurer que tout fonctionne comme prévu.

## <a name="why-replicate-with-san"></a>Pourquoi effectuer une réplication avec un SAN ?
...
### <a name="ensure-vmm-clouds-are-set-up"></a>Vérifier que les clouds VMM sont configurés
...
### <a name="integrate-and-classify-san-storage-in-vmm"></a>Intégrez et classez le stockage SAN dans VMM.
...
1. Une fois le stockage SAN intégré à VMM, vous créez (approvisionnez) des LUN :
   ...
   * [Comment approvisionner les unités logiques de stockage dans VMM](https://technet.microsoft.com/library/gg696973.aspx)
     ...
     > [!NOTE]
     > Une fois que vous avez activé la réplication sur une machine, vous ne devez ajouter de disques durs virtuels pour celle-ci aux numéros d’unités logiques qui ne se trouvent pas dans un groupe de réplication Site Recovery. Si vous le faites, ils ne seront pas détectés par Site Recovery.
     > 
     > 
2. Ensuite, allouez de l'espace de stockage au cluster hôte Hyper-V pour que VMM puisse déployer les données des machines virtuelles vers le stockage approvisionné :
   ...
   * Avant d'allouer de l'espace de stockage au cluster, vous devez en allouer au groupe hôte VMM sur lequel réside le cluster. Consultez [Comment allouer des unités logiques de stockage à un groupe hôte](https://technet.microsoft.com/library/gg610686.aspx) et [Comment allouer des pools de stockage à un groupe hôte](https://technet.microsoft.com/library/gg610635.aspx).</a>.
   * Ensuite, allouez la capacité de stockage au cluster comme décrit dans [Comment configurer du stockage sur un cluster hôte Hyper-V dans VMM](https://technet.microsoft.com/library/gg610692.aspx).</a>.
...
1. [cliquez ici](site-recovery-network-mapping.md) .
...
7. L’emplacement de l’installation est **<SystemDrive>\Program Files\Microsoft System Center 2012 R2\Virtual Machine Manager\bin**. Cliquez sur le bouton Installer pour démarrer l’installation du fournisseur.
...
2. Dans l’onglet **Éléments protégés**, sélectionnez le cloud que vous souhaitez configurer, puis accédez à l’onglet **Configuration**. Notez les points suivants :
3. Dans **Cible**, sélectionnez **VMM**.
4. Dans **Emplacement cible**, sélectionnez le serveur VMM local qui gère le cloud à utiliser pour la récupération.
...
Suite à l'enregistrement des paramètres, une tâche est créée et peut être surveillée sous l'onglet **Jobs** . Les paramètres de cloud peuvent être modifiés sous l'onglet **Configure** . Pour modifier l'emplacement cible ou le cloud cible, vous devez supprimer la configuration du cloud, puis reconfigurer ce dernier.
...
* **Nouvelle machine virtuelle**: dans la console VMM, lorsque vous créez une machine virtuelle, vous activez la protection Azure Site Recovery et associez la machine virtuelle au groupe de réplication.
...
4. Une fois un plan de récupération créé, il s’affiche dans la liste de l'onglet **Plans de récupération** .
5. Dans l’onglet **Plans de récupération**, sélectionnez le plan et cliquez sur **Test de basculement**.
...
1. La machine virtuelle de test est créée sur le même hôte que celui sur lequel la machine virtuelle de réplication existe. Elle n'est pas ajoutée au cloud où se trouve la machine virtuelle de réplication.
2. Après la réplication, la machine virtuelle de réplication possède une adresse IP différente de celle de la machine virtuelle principale. Si vous émettez des adresses à partir de DHCP, DNS est mis à jour automatiquement. Si vous n’utilisez pas DHCP et que vous souhaitez vérifier que les adresses sont identiques, vous devez exécuter quelques scripts.
3. Exécutez cet exemple de script pour récupérer l'adresse IP.
   
       $vm = Get-SCVirtualMachine -Name <VM_NAME>
       $na = $vm[0].VirtualNetworkAdapters>
       $ip = Get-SCIPAddress -GrantToObjectID $na[0].id
       $ip.address  
4. Exécutez cet exemple de script pour mettre à jour DNS, en spécifiant l'adresse IP que vous avez récupérée à l'aide de l'exemple de script précédent.
