...
1. Une fois le stockage SAN intégré à VMM, vous créez (approvisionnez) des LUN :
	...
	>[AZURE.NOTE] Une fois que vous avez activé la réplication sur une machine, vous ne devez ajouter de disques durs virtuels pour celle-ci aux numéros d’unités logiques qui ne se trouvent pas dans un groupe de réplication Site Recovery. Si vous le faites, ils ne seront pas détectés par Site Recovery.
