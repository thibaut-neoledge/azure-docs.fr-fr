...
1. Sur la page **Démarrage rapide**, dans **Préparer les serveurs VMM**, cliquez sur **Télécharger le fournisseur Microsoft Azure Site Recovery pour une installation sur les serveurs VMM** pour obtenir la dernière version du fichier d’installation du fournisseur.
...
Où les paramètres sont les suivants :
 ...
 - **/proxyPassword** : paramètre facultatif qui spécifie le mot de passe de l’utilisateur proxy.  


## Étape 4 : Création d’un compte de stockage Azure

1. Si vous ne possédez pas de compte de stockage Azure, cliquez sur **Ajouter un compte de stockage Azure** pour créer un compte.
...
2. Dans la liste des machines virtuelles du cloud, sélectionnez celle que vous voulez protéger.
	...
	Suivez la progression de l’action **Activer la protection** sous l’onglet **Tâches**, y compris la réplication initiale. Lorsque la tâche **Finaliser la protection** s’exécute, la machine virtuelle est prête pour le basculement. Une fois la protection activée et les machines virtuelles répliquées, celles-ci sont affichées dans Azure.
...
4. Sous l’onglet **Configurer** des propriétés de la machine virtuelle, les propriétés réseau suivantes peuvent être modifiées.
...
En savoir plus sur la [définition de plans de récupération](site-recovery-create-recovery-plans.md) et sur le [basculement](site-recovery-failover.md).
