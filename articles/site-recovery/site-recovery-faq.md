...
Cet article contient les questions fréquemment posées sur Azure Site Recovery.
...
### Que peut répliquer Site Recovery ?

- **Machines virtuelles Hyper-V** : Site Recovery peut répliquer toute charge de travail en cours d’exécution sur une machine virtuelle Hyper-V. 
- **Serveurs physiques** : Site Recovery peut répliquer toute charge de travail s’exécutant sur un serveur physique Windows/Linux pris en charge.
- **Machines virtuelles Hyper-V** : Site Recovery peut répliquer toute charge de travail en cours d’exécution sur une machine virtuelle VMware.


### De quoi ai-je besoin dans Hyper-V ?
...
Vous pouvez protéger des serveurs physiques exécutant des systèmes d’exploitation Windows et Linux pris en charge, sur Azure ou sur un site secondaire. [En savoir plus](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment) sur la configuration requise pour le système d’exploitation pour la réplication de site Azure ou secondaire. Notez que les serveurs physiques qui sont répliqués vers le stockage Azure s’exécuteront en tant que machines virtuelles dans Azure lors du basculement. Lorsque vous effectuez une restauration automatique à partir d’Azure vers votre site local, le basculement vers un serveur physique n’est actuellement pas pris en charge. Vous pouvez effectuer la restauration automatique uniquement sur une machine virtuelle exécutée sous VMware.

### Quelles machines virtuelles VMware puis-je protéger ?

Pour protéger les machines virtuelles VMware, vous avez besoin d’un hyperviseur vSphere et de machines virtuelles exécutant les outils VMware. Nous vous recommandons également de disposer d’un serveur VMware vCenter pour gérer les hyperviseurs. [En savoir plus](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment) sur la configuration requise exacte pour la réplication des serveurs VMware et des machines virtuelles vers Azure ou sur un site secondaire.
...
Lors de la réplication normale, les données sont répliquées dans le stockage Azure géo-redondant. Ainsi, vous n’avez aucuns frais de gestion des machines virtuelles IaaS Azure à régler. Lorsque vous exécutez un basculement vers Azure, Site Recovery crée automatiquement les machines virtuelles IaaS Azure. Après cela, vous êtes facturé pour les ressources de calcul que vous utilisez dans Azure.

### Existe-t-il un kit de développement logiciel (SDK) que je peux utiliser pour automatiser les flux de travail de Site Recovery ?
...
### Pour le fournisseur de services, l’identité de mon client est-elle communiquée au service Site Recovery ?
Non. Vos clients n’ont pas besoin d’accéder au portail Site Recovery. Seul l’administrateur du fournisseur de services interagit avec le portail.
