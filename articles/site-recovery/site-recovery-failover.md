...
Le service [Microsoft Azure Site Recovery](site-recovery-overview.md) offre une solution fiable de continuité d’activité et de récupération d’urgence, qui protège vos machines virtuelles et vos serveurs physiques locaux en orchestrant et en automatisant la réplication et le basculement vers Microsoft Azure ou un centre de données secondaire local. Cet article fournit des informations et des instructions relatives à la récupération (basculement ou restauration automatique) des machines virtuelles et des serveurs physiques protégés par Microsoft Azure Site Recovery.
...
Une fois que vous avez configuré la protection des serveurs physiques et des machines virtuelles, la réplication est effectuée vers l’emplacement secondaire. Lorsque la réplication est activée, vous pouvez exécuter des basculements en cas de besoin. Microsoft Azure Site Recovery prend en charge différents types de basculement.

**Type de basculement** | **Motif de l’exécution** | **Détails** | **Processus**
---|---|---|---
**Test de basculement** | Exécutez ce basculement pour valider votre stratégie de réplication ou effectuer un exercice de récupération d’urgence. | <p>Aucune perte de données, ni aucun temps d’arrêt.</p><p>Aucun impact sur la réplication.</p><p>Aucun impact sur votre environnement de production.</p> | <p>Démarrez le basculement.</p><p>Indiquez le mode de connexion des machines de test aux réseaux après le basculement.</p><p>Effectuez un suivi de l’opération sur l’onglet **Tâches**. Les machines de test sont créées ; elles démarrent à l’emplacement secondaire.</p><p>Microsoft Azure : connectez-vous à la machine dans le portail Azure.</p><p>Site secondaire : accédez à la machine sur le même hôte et le même cloud.</p><p>Terminez le test et lancez le nettoyage automatique des paramètres du test de basculement.</p>
**Basculement planifié** | <p>Exécutez ce basculement pour répondre aux exigences de conformité.</p><p>Exécutez ce basculement pour les opérations de maintenance planifiée.</p><p>Basculez les données afin de maintenir les charges de travail en fonctionnement en cas de panne connue (coupure de courant ou phénomène climatique important annoncés au préalable).</p> <p>Effectuez cette opération pour assurer la restauration automatique après le basculement, du site principal vers le site secondaire. | <p>Aucune perte de données.</p><p>Un temps d’arrêt survient pendant l’intervalle d’arrêt de la machine virtuelle sur le site principal, puis de redémarrage de cette machine sur le site secondaire.</p><p>Les machines virtuelles sont affectées, car les machines cibles deviennent des machines sources après le basculement.</p> | <p>Démarrez le basculement.</p><p>Effectuez un suivi de l’opération sur l’onglet **Tâches**. Les machines sources sont arrêtées.</p><p>Les ordinateurs virtuels de réplication démarrent sur le site secondaire.</p><p>Microsoft Azure : connectez-vous à l’ordinateur virtuel de réplication dans le portail Azure.</p><p>Site secondaire : accédez à la machine sur le même hôte et le même cloud.</p><p>Validez le basculement.</p>
**Basculement non planifié** | <p>Exécutez ce basculement lorsqu’un site principal devient inaccessible en raison d’un problème inattendu, tel qu’une attaque de virus ou une panne d’alimentation.</p><p>Vous pouvez exécuter un basculement non planifié même si le site principal n’est pas disponible.<p> | <p>Perte de données en fonction des paramètres de fréquence de réplication.</p> <p>Les données sont mises à jour en fonction de leur dernière synchronisation.</p> | <p>Démarrez le basculement.</p><p>Effectuez un suivi de l’opération sur l’onglet **Tâches**. Le cas échéant, essayez d’arrêter les machines virtuelles et de synchroniser les données les plus récentes.</p><p>Les ordinateurs virtuels de réplication démarrent sur le site secondaire.</p><p>Microsoft Azure : connectez-vous à l’ordinateur virtuel de réplication dans le portail Azure.</p><p>Site secondaire : accédez à la machine sur le même hôte et le même cloud.</p><p>Validez le basculement.</p>
...
Entre un site VMware et Microsoft Azure | Non pris en charge |Ce scénario utilise la réplication continue. Il n’existe donc aucune distinction entre un basculement planifié et un basculement non planifié. Sélectionnez le type **Basculement**. | N/D
...
### Restauration automatique à partir du site secondaire
...
### Restauration automatique à partir de Microsoft Azure
...
---|---|---|---
**Basculement vers Microsoft Azure, sans réseau** | Aucun réseau Microsoft Azure cible n’est sélectionné. | Vérifie que la machine virtuelle de test démarre comme prévu dans Microsoft Azure. | <p>Toutes les machines virtuelles de test d’un plan de récupération sont ajoutées dans un seul service cloud et peuvent se connecter entre elles.</p><p>Les machines ne sont pas connectées à un réseau Microsoft Azure après le basculement.</p><p>Les utilisateurs peuvent se connecter aux machines de test avec une adresse IP publique.</p>
**Basculement vers Microsoft Azure, via un réseau** | Un réseau Microsoft Azure cible est sélectionné. | Vérifie que les machines de test sont connectées au réseau. | <p>Créez un réseau Microsoft Azure isolé du réseau de production Microsoft Azure et configurez l’infrastructure de la machine virtuelle répliquée, afin qu’elle fonctionne comme prévu.</p><p>Le sous-réseau de la machine virtuelle de test est basé sur le sous-réseau auquel la machine virtuelle ayant fait l’objet d’un basculement doit se connecter en cas de basculement, planifié ou non.</p>
**Basculement vers un site VMM secondaire, sans réseau** | Aucun réseau de machines virtuelles n’est sélectionné. | Vérifie que les machines de test sont créées.<p>La machine virtuelle de test est créée sur l’hôte qui héberge déjà l’ordinateur virtuel de réplication. Elle n’est pas ajoutée au cloud sur lequel se trouve l’ordinateur virtuel de réplication.</p></p>| <p>Après basculement, la machine n’est connectée à aucun réseau.</p><p>La machine peut être connectée à un réseau de machines virtuelles après sa création.</p>
...
### Exécuter un test de basculement depuis un site local vers Microsoft Azure
...
## Restauration automatique d’un site secondaire vers un site primaire
...
3. Si vous effectuez la restauration à partir de Microsoft Azure, sélectionnez différents paramètres dans la zone **Synchronisation des données** :
	...
	> [AZURE.NOTE]Nous vous recommandons d’utiliser cette option si Microsoft Azure est exécuté depuis un certain temps (un mois ou plus). En effet, elle effectue la synchronisation sans arrêter les machines virtuelles. Elle ne procède à aucune resynchronisation, mais effectue une restauration automatique complète. Par contre, elle n’exécute aucun calcul de somme de contrôle.

	- **Synchroniser les données uniquement pendant le basculement** : utilisez cette option si Microsoft Azure s’exécute depuis une courte période. Elle est plus rapide, car elle resynchronise les données au lieu d’effectuer une restauration automatique complète. Cette option effectue un calcul de somme de contrôle rapide et télécharge uniquement les blocs modifiés.
...
4. Dans le champ **Confirmer le basculement planifié**, sélectionnez **Créer une machine virtuelle locale si elle n’existe pas**. 
