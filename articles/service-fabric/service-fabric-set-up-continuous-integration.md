...
Avant de pouvoir configurer l’ordinateur de build, vous devez créer un « principal du service » que l’automatisation utilise pour s’authentifier auprès d’Azure. Vous devez également créer un certificat et le charger dans un coffre de clés, car le coffre de clés Azure ne prend pas en charge l’authentification du principal du service. Vous pouvez effectuer ces étapes à partir de tout ordinateur. Votre ordinateur de développement est un bon choix.
...
1.	Installez et mettez à jour le module AzureRM.
    ...
    1.  Lancez une invite de commandes PowerShell.
    1.	Installez le module « AzureRM » à l’aide de la commande `Install-Module AzureRM`.
    1.	Mettez à jour le module « AzureRM » à l’aide de la commande `Update-AzureRM`.
1.	Désactivez (ou activez) la collecte de données Azure.
...
1.	Connectez-vous à Azure PowerShell.
    ...
    1. Dans la boîte de dialogue qui s’affiche, entrez vos informations d’identification Azure.
    1. Exécutez la commande `Get-AzureRmSubscription`.
    1. Recherchez l’abonnement que vous souhaitez utiliser, puis exécutez la commande `Select-AzureRmSubscription -SubscriptionId <id for your subscription>`.
...
1.	Dans une invite de commandes PowerShell administrateur, accédez au répertoire dans lequel vous avez extrait ServiceFabricCIManualScripts.zip.
1.	Choisissez un mot de passe pour le principal du service à l’aide de la commande suivante. N’oubliez pas ce mot de passe, car il sera utilisé comme une variable de build.

    ```
    $password = Read-Host -AsSecureString
    ```
1.	Exécutez le script PowerShell Create-ServicePrincipal.ps1 avec les paramètres suivants :
...
1.	Exécutez le script PowerShell CreateAndUpload-Certificate.ps1 avec les paramètres suivants.
...
2.	Si vous n’avez pas encore de machine, vous pouvez rapidement configurer une machine virtuelle Azure avec Visual Studio 2015 préinstallé. Pour ce faire :
    1.	Connectez-vous au [portail de gestion Azure](http://portal.azure.com).
    1.	Sélectionnez la commande **Nouveau** dans l’angle supérieur gauche de l’écran.
    1.	Sélectionnez **Marketplace**.
    1.	Recherchez **Visual Studio 2015**.
    1.	Sélectionnez **Calculer** > **Machine virtuelle** > **À partir de la galerie**.
    1.	Sélectionnez l’image **Visual Studio Enterprise 2015 avec le Kit de développement logiciel (SDK) Azure 2.7 sur Windows Server 2012 R2**.

        >[AZURE.NOTE]Le Kit de développement logiciel (SDK) Azure n’est pas un composant requis, mais il n’y a aucune image disponible ayant Visual Studio 2015 uniquement installé.

    1.	Suivez les instructions de la boîte de dialogue pour créer votre machine virtuelle. (Il est recommandé de choisir une machine virtuelle série D pour de meilleures performances de disque et d’UC.)
...
1.	Importez le certificat sur votre ordinateur de build. Pour ce faire :
    ...
    1. Ouvrez un compte PowerShell administrateur et exécutez les commandes suivantes à l’aide du mot de passe transmis à GenerateCertificate.ps1 précédemment.
...
1.	Exécutez le gestionnaire de certificats.
    ...
    1. Recherchez **Certificat**.
    1. Sélectionnez **Outils d’administration** > **Gérer les certificats d’ordinateur**.


1.	Accordez au compte de service local l’autorisation d’utiliser votre certificat Automation.
    1.	Sous **Certificats - Ordinateur local**, développez **Personnel**, puis sélectionnez **Certificats**.
    1.	Recherchez votre certificat dans la liste.
    1.	Cliquez avec le bouton droit sur votre certificat, puis sélectionnez **Toutes les tâches** > **Gérer les clés privées**.
    1.	Sélectionnez le bouton **Ajouter**, puis entrez **Service local** et sélectionnez **Vérifier les noms**.
    1.	Sélectionnez le bouton **OK**, puis fermez le gestionnaire de certificats.
...
1.	À partir d’une invite de commandes PowerShell administrateur, exécutez `C:\agent\ConfigureAgent.ps1`. Le script vous invite à entrer les paramètres suivants :
...
1.	Archivez les fichiers obtenus.
...
1.	Créez une définition de build vide. Pour ce faire :
    ...
    1.  Sélectionnez la file d’attente de l’agent dans laquelle vous avez inscrit votre agent de build, et cochez la case **Intégration continue**.
1.	Dans l’onglet **Variables**, créez les variables suivantes avec ces valeurs.
...
1.	Dans l’onglet **Déclencheurs**, sélectionnez les options **Intégration continue** et **Modifications par lots**.
1.	Dans l’onglet **Général**, cliquez sur la file d’attente dans laquelle vous avez inscrit vos agents de build.
1.	Enregistrez la définition de build et nommez-la. (Vous pouvez modifier ce nom ultérieurement si vous le souhaitez.)
...
1.	Dans l’onglet **Build**, sélectionnez la commande **Ajouter une étape de build…**
1.	Sélectionnez **Utilitaire** > **PowerShell**.
1.	Cliquez sur l’icône en forme de crayon en regard du nom de l’étape de build et renommez-la **Supprimer le groupe de ressources de cluster**.
1.	Sélectionnez le bouton **...** en regard de **Nom du fichier script**. Accédez à l’emplacement où vous avez extrait les scripts d’automatisation, puis sélectionnez **RemoveClusterResourceGroup.ps1**.
1.	Pour Arguments, entrez `-ServicePrincipalPassword "$(ServicePrincipalPassword)`.
1.	Sous **Options de contrôle**, cochez la case **Toujours exécuter**.
1.	Enregistrez la définition de build.

### Essayez !

Sélectionnez la commande **Mettre en file d’attente le build** pour démarrer un build. Les builds seront également déclenchés lors de l’archivage.
...
1.	Créez manuellement un cluster de test via le portail de gestion Azure ou Azure PowerShell. Vous pouvez vous rapporter au script ProvisionAndDeploy-SecureCluster.ps1 en guise de référence.
1.	Configurez votre profil de publication pour prendre en charge la mise à niveau de l’application en suivant [ces instructions](service-fabric-visualstudio-configure-upgrade.md).

1.	Remplacez l’étape **Configurer et déployer sur le cluster sécurisé** par une étape qui appelle directement Deploy-FabricApplication.ps1 (et le transmet à votre profil de publication).
1.	Supprimez les deux étapes de build **Supprimer le groupe de ressources de cluster** de votre définition de build.
