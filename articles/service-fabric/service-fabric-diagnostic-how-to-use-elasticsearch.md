...
## <a name="use-the-listeners-library-to-send-diagnostic-data-to-elasticsearch"></a>Utiliser la bibliothèque Listeners pour l’envoi de données de diagnostic à Elasticsearch
...
```
Les valeurs des paramètres `serviceUri`, `userName` et `password` correspondent respectivement à l’adresse de point de terminaison de cluster Elasticsearch, au nom d’utilisateur Elasticsearch et au mot de passe, respectivement. `indexNamePrefix` désigne le préfixe des index Elasticsearch ; la bibliothèque Microsoft.Diagnostics.Listeners crée chaque jour un index pour ses données.
