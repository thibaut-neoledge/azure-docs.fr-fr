...
Une application Azure Service Fabric est une collection de services. Pendant une mise à niveau, Service Fabric compare le nouveau [manifeste d'application](service-fabric-application-model.md#describe-an-application) à la version précédente et détermine les services qui, dans l'application, nécessitent des mises à jour. Fabric Service y parvient en comparant les numéros des versions dans les manifestes de service avec les numéros de version dans la version précédente. Si un service n'a pas changé, ce service n'est pas mis à niveau.
...
De ce fait, les deux versions doivent être mutuellement compatibles. Si ce n'est pas le cas, l'administrateur d'application doit effectuer une mise à niveau échelonnée pour maintenir la disponibilité. L'administrateur effectue cela avant la mise à niveau vers la version finale, en procédant à une mise à niveau avec une version intermédiaire de l'application qui est compatible avec la version précédente.

Les domaines de mise à jour sont spécifiés dans le manifeste de cluster quand vous configurez le cluster. Les domaines de mise à jour ne sont pas censés recevoir les mises à jour dans un ordre particulier. Un domaine de mise à jour est une unité logique de déploiement pour une application. Grâce aux domaines de mise à jour, les services demeurent hautement disponibles pendant une mise à niveau.

Une mise à niveau non propagée est possible si elle est appliquée à tous les nœuds du cluster, ce qui est le cas quand l'application n'a qu'un seul domaine de mise à jour. Cette méthode n'est pas recommandée, car le service est arrêté et indisponible au moment de la mise à niveau. En outre, Azure ne fournit aucune garantie quand un cluster est configuré avec un seul domaine de mise à jour.
...
Pendant une mise à jour effectuée par Service Fabric, les stratégies et vérifications d'intégrité sont indépendantes du service et de l'application. Autrement dit, aucun test spécifique au service n'est exécuté. Par exemple, votre service peut avoir une exigence de débit minimal. Toutefois, comme Service Fabric ne dispose pas des informations permettant de le vérifier, il ne vérifie pas le débit tel que défini pour votre application. Consultez les [articles sur l'intégrité](service-fabric-health-introduction.md) pour connaître les vérifications qui sont effectuées. Les contrôles qui se produisent pendant une mise à niveau incluent des tests pour savoir si le package d'application a été copié correctement, si l'instance a été démarrée, etc.
...
Le mode que nous recommandons pour les mises à niveau est le mode surveillé, qui est le plus couramment utilisé. Le mode surveillé effectue la mise à niveau sur un domaine de mise à jour spécifique, puis, si toutes les vérifications d'intégrité sont satisfaites (suivant la stratégie spécifiée), il passe automatiquement au domaine de mise à jour suivant. Si les vérifications d'intégrité échouent et/ou que les délais sont atteints, la mise à niveau est restaurée pour le domaine de mise à jour, ou le système bascule sur le mode manuel non surveillé (si cette option a été sélectionnée au moment de la mise à niveau).
...
L'organigramme ci-dessous peut vous aider à comprendre le processus de mise à niveau d'une application Service Fabric. En particulier, le flux indique dans quelle mesure les délais, notamment *HealthCheckStableDuration*, *HealthCheckRetryTimeout* et *UpgradeHealthCheckInterval*, déterminent l'échec ou la réussite de la mise à niveau dans un domaine de mise à jour donné.
