...
>[AZURE.NOTE] Ce package Windows Server autonome peut contenir des fonctionnalités qui sont actuellement en version préliminaire et ne sont pas prises en charge pour un usage commercial. Pour afficher la liste des fonctionnalités en version préliminaire, consultez « Fonctionnalités préliminaires incluses dans ce package ». Vous pouvez également [télécharger une copie du CLUF](http://go.microsoft.com/fwlink/?LinkID=733084) maintenant.
...
>[AZURE.NOTE] Les journaux de déploiement sont disponibles localement sur la machine virtuelle/l’ordinateur sur laquelle/lequel vous avez exécuté le script PowerShell CreateServiceFabricCluster. Ils se trouvent dans un sous-dossier nommé DeploymentTraces sous le dossier où vous avez exécuté la commande PowerShell. Pour voir si Service Fabric a été déployé correctement sur un ordinateur, vous pouvez rechercher les fichiers installés dans le répertoire C:\ProgramData et vérifier si les processus FabricHost.exe et Fabric.exe sont mentionnés comme étant en cours d’exécution dans le Gestionnaire des tâches.
...
>[AZURE.NOTE] Avec la nouvelle [version mise à la disposition générale du cluster autonome pour Windows Server (version 5.3.204.x)](https://azure.microsoft.com/blog/azure-service-fabric-for-windows-server-now-ga/), vous pouvez mettre à niveau votre cluster vers des versions ultérieures, manuellement ou automatiquement. Étant donné que cette fonctionnalité n’est pas disponible sur les versions préliminaires, vous devrez créer un cluster à l’aide de la version générale et migrer vos données et applications à partir du cluster de la version préliminaire. Restez connecté pour plus d’informations sur cette fonctionnalité.
...
<!--Image references-->
[Zone approuvée]: ./media/service-fabric-cluster-creation-for-windows-server/TrustedZone.png
