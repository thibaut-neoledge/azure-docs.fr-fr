<properties
   pageTitle="Déploiement d’une application existante dans Azure Service Fabric"
...
Azure Service Fabric permet de déployer des applications existantes. Les applications actuellement exécutées, par exemple, sur un rôle Web Azure ou de travail, peuvent être « empaquetées » afin d’être déployées sur un cluster Service Fabric. Les applications existantes exécutées sur un cluster Service Fabric peuvent bénéficier de fonctionnalités comme la surveillance de l'intégrité et ALM (gestion du cycle de vie des applications). Il s’agit donc d’un scénario important entièrement pris en charge. Ce didacticiel décrit le processus et les concepts de base impliqués dans la sélection et l’empaquetage d'une application existante. Cet article est une vue d'ensemble du processus et nous le compléterons avec des exemples spécifiques sur la façon de sélectionner des applications existantes (par exemple, node.js ou Java) et de les empaqueter afin qu'elles puissent être hébergées sur un cluster Service Fabric.

## Rapide vue d'ensemble des fichiers manifeste d’application et de service

Avant de commencer à apprendre comment empaqueter une application existante, nous devons examiner une brève présentation du modèle de déploiement Service Fabric. Le modèle de déploiement Service Fabric repose principalement sur deux fichiers :


* Manifeste d'application. Le manifeste d'application est utilisé pour décrire l'application et répertorie les services qui la composent ainsi que d’autres paramètres, comme le nombre d'instances, utilisés pour définir la manière dont les services doivent être déployés. Dans l’univers Service Fabric, une application représente l'unité pouvant être mise à niveau. Une application peut être mise à niveau comme une unité unique dans laquelle les défaillances potentielles (et les restaurations potentielles) sont gérées par la plateforme pour garantir que la mise à niveau réussit complètement ou, si elle échoue, ne laisse pas l'application dans un état inconnu/instable.
...
Remarque : vous pouvez utiliser n'importe quel nom arbitraire pour les répertoires Code, Config et Data. Vous devez vous assurer d'utiliser la même valeur dans le fichier ApplicationManifest.
...
* Vous pouvez choisir n'importe quel nom pour `ServiceTypeName` ; la valeur est utilisée dans le `applicationmanifest.xml` pour identifier le service.
* Vous devez spécifier `UserImplicitHost = "true"`. Cet attribut indique à Service Fabric que le service est basé sur une application autonome et que la seule opération nécessaire consiste à la lancer en tant que processus et à surveiller son intégrité.
...
L’élément `Name` sert à spécifier le nom du répertoire dans le package de l'application qui contient le code. `CodePackage` contient également l’attribut `version` qui peut être utilisé pour spécifier la version du code et, potentiellement, pour mettre à niveau le code en utilisant l'infrastructure de gestion du cycle de vie de l’application Service Fabric.
...
* `Arguments`: il spécifie les arguments qui doivent être transmis au fichier exécutable. Il peut s’agir d’une liste de paramètres avec des arguments.
* `WorkingFolder`: il spécifie le répertoire de travail pour le processus qui va être démarré. Vous pouvez spécifier deux valeurs :
	* `CodeBase`: le répertoire de travail sera défini sur répertoire Code dans le package d'application (répertoire `Code` dans la structure ci-dessous)
	* `CodePackage`: le répertoire de travail sera défini sur la racine du package d'application (`MyServicePkg`) ; l’élément `WorkingDirectory` est utile pour définir le répertoire de travail correct afin que des chemins d'accès relatifs puissent être utilisés par l’application ou des scripts d'initialisation. Il existe également une autre valeur que vous pouvez spécifier pour l’élément `WorkingFolder` (`Work`), mais elle n'est pas très utile pour un scénario avec une application existante.
...
Le `SetupEntrypoint` sert à spécifier un fichier exécutable ou de commandes qui doit être exécuté avant le lancement du code du service. Cet élément est facultatif et il n’est donc pas nécessaire de l'inclure si aucune initialisation/configuration n’est requise. Le paramètre Entrypoint est exécuté chaque fois que le service est redémarré. Comme il n’existe qu’un seul paramètre SetupEntrypoint, les scripts d'installation/configuration doivent donc être regroupés dans un même fichier de commandes si l’installation/configuration de l'application requiert plusieurs scripts. À l’instar de l’élément `Entrypoint`, l’élément `SetupEntrypoint` peut exécuter n'importe quel type de fichier : fichier exécutable, fichiers de commandes, applet de commande powershell. Dans l'exemple ci-dessus, l’élément `SetupEntrypoint` est basé sur un fichier de commandes myAppsetup.cmd qui se trouve dans le sous-répertoire des scripts du répertoire Code (en supposant que `WorkingDirectory` est défini sur `Code`).

## Fichier de manifeste d'application

Une fois que vous avez configuré le fichier `servicemanifest.xml`, vous devez apporter des modifications au fichier `applicationmanifest.xml` afin de garantir que le type et le nom corrects du service sont utilisés.
...
* `ServiceTypeName` est utilisé comme un « ID » pour le service. Dans le contexte du portage d'une application, `ServiceTypeName` doit simplement être un identificateur unique de votre service.
