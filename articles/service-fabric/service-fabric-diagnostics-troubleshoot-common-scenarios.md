...
| Erreur | Description |
| --- | --- |
| System.IO.FileNotFoundException | Cette erreur est souvent due à des dépendances d'assembly manquantes. Vérifiez la propriété CopyLocal dans Visual Studio ou dans le GAC pour le nœud.
...
[WAD](service-fabric-diagnostics-how-to-setup-wad-operational-insights.md) peut être configuré pour télécharger le journal d’événements des applications pour tous les nœuds automatiquement.

###RunAsync() or OnActivateAsync()
Si le blocage se produit pendant l'initialisation ou l'exécution de votre type de service ou acteur inscrit, l'exception est interceptée par Service Fabric. Vous pouvez l’afficher à partir des fournisseurs EventSource détaillés dans les étapes suivantes.

## Étapes suivantes

En savoir plus sur les diagnostics existants fournis par Service Fabric :

* [Diagnostics d’acteurs](service-fabric-reliable-actors-diagnostics.md)
