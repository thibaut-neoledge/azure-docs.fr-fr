...
>[AZURE.NOTE]
- Les certificats utilisés dans les clusters qui exécutent des charges de travail de production doivent être créés à l’aide d’un service de certificats Windows Server correctement configuré ou obtenus après d’une [autorité de certification](https://en.wikipedia.org/wiki/Certificate_authority) approuvée. - N’utilisez jamais en production des certificats temporaires ou de test créés à l’aide d’outils tels que MakeCert.exe. - Pour les clusters que vous utilisez uniquement à des fins de test, vous pouvez choisir d’utiliser un certificat auto-signé.
