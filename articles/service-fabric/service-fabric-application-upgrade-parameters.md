...
Cet article décrit les différents paramètres qui s'appliquent pendant la mise à niveau d'une application Service Fabric. Les paramètres incluent le nom et la version de l'application, et permettent de contrôler les délais d'attente et les vérifications d'intégrité appliqués au cours de la mise à niveau et de spécifier les stratégies à appliquer quand une mise à niveau échoue.
...
| FailureAction | La propriété FailureAction décrit l'action effectuée par Service Fabric en cas d'échec de la mise à niveau. L'application peut être restaurée à la dernière version existante avant la mise à jour (Rollback), ou la mise à niveau de l'application est arrêtée au niveau du domaine de mise à niveau actuel et la mise à niveau passe en mode manuel. Les valeurs autorisées sont Rollback et Manual. |
...
| HealthCheckRetryTimeoutSec | Durée (en secondes) pendant laquelle Service Fabric continue d'évaluer l'intégrité avant de déclarer la mise à niveau comme ayant échoué. La valeur par défaut est 600 secondes. Cette durée commence quand HealthCheckWaitDuration est atteint. Dans le délai imparti par HealthCheckRetryTimeout, Service Fabric peut effectuer plusieurs vérifications d'intégrité de l'application. La valeur par défaut est 10 minutes, mais nous vous recommandons de l'adapter à votre application. |
| HealthCheckStableDurationSec | Durée (en secondes) nécessaire pour vérifier que l'application est stable avant de passer au domaine de mise à niveau suivant ou de terminer la mise à niveau. Cette durée d'attente facilite la détection des modifications d'intégrité qui pourraient avoir lieu juste après la fin de la vérification d'intégrité. La valeur par défaut est 0 seconde, mais nous vous recommandons de l'adapter à votre application. |
| UpgradeDomainTimeoutSec | Durée maximale (en secondes) pour mettre à niveau un seul domaine de mise à niveau. Si ce délai est atteint, la mise à niveau s'arrête et effectue l'action spécifiée par UpgradeFailureAction. La valeur par défaut est « never » (infinie), mais nous vous recommandons de l'adapter à votre application. |
| UpgradeTimeout | Délai (en secondes) qui s'applique à l'ensemble de la mise à niveau. Si ce délai est atteint, la mise à niveau s'arrête et UpgradeFailureAction est déclenché. La valeur par défaut est « never » (infinie), mais nous vous recommandons de l'adapter à votre application. |
| UpgradeHealthCheckInterval | Spécifié dans la section ClusterManager du _manifeste_ de _cluster_ (non spécifié dans le cadre de l'applet de commande de mise à niveau), ce paramètre indique la fréquence à laquelle l'état d'intégrité est vérifié (la valeur par défaut est 60 secondes). |
...
| Paramètre | Description |
| --- | --- |
| ConsiderWarningAsError | La valeur par défaut est False. Traiter les événements d'avertissement d'intégrité de l'application comme une erreur pendant l'évaluation de l'intégrité de l'application au cours de la mise à niveau. Par défaut, Service Fabric ne considère pas les événements d'avertissement d'intégrité comme un échec (erreur) ; la mise à niveau peut donc continuer même si des événements d'avertissement se produisent. |
| MaxPercentUnhealthyDeployedApplications | La valeur par défaut et recommandée est 0. Nombre maximal d'applications déployées (voir la [section Intégrité](service-fabric-health-introduction.md)) pouvant être défectueuses avant que l'application ne soit considérée comme défectueuse et que sa mise à niveau échoue. Comme il s'agit de l'intégrité de l'application empaquetée qui se trouve sur le nœud, cela permet de détecter immédiatement un problème pendant la mise à niveau et de déterminer à quel endroit le package d'application déployé sur le nœud est défectueux (blocage, etc.). Dans un cas classique, les réplicas de l'application font l'objet d'un équilibrage de charge sur l'autre nœud, ce qui rend l'application intègre et permet de poursuivre la mise à niveau. En spécifiant une intégrité MaxPercentUnhealthyDeployedApplications stricte, Service Fabric peut détecter rapidement un problème avec le package d'application et provoquer dans la foulée un échec de la mise à niveau. |
