...
4. *Ajouter un nouveau point de terminaison* Le service Web de notation publié lors l’étape 2 ci-dessus a été créé avec un point de terminaison par défaut. Les points de terminaison par défaut sont toujours synchronisés avec l’expérience d’origine. Par conséquent, un modèle formé du point de terminaison par défaut ne peut pas être remplacé. Pour créer un point de terminaison pouvant être mis à jour, visitez le portail Azure et cliquez sur Ajouter un point de terminaison \(plus de détails [ici](machine-learning-create-endpoint.md)\).	
