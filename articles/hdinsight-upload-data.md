...
Vous pouvez accéder au stockage d'objets blob Azure via [AzCopy][], [Azure PowerShell][], la [bibliothèque du client de stockage Windows Azure pour .NET][] ou des outils d'explorateur. Voici certains des outils disponibles :
...
-   Un cluster Azure HDInsight. Pour obtenir des instructions, consultez le didacticiel [Prise en main d'Azure HDInsight][] ou la rubrique [Mise en service de clusters HDInsight][].

## Dans cet article

-   [Téléchargement de données vers le stockage d'objets blob avec AzCopy][]
-   [Téléchargement de données vers le stockage d'objets blob avec Azure PowerShell][]
-   [Téléchargement de données vers le stockage d'objets blob avec Azure Storage Explorer][]
-   [Téléchargement de données vers le stockage d'objets blob avec la ligne de commande Hadoop][]
-   [Importation de données à partir d'une base de données SQL Azure vers le stockage d'objets blob avec Sqoop][]

## <span id="azcopy"></span></a>Téléchargement de données vers le stockage d'objets blob avec AzCopy

AzCopy est un utilitaire de ligne de commande conçu pour simplifier le transfert de données vers et depuis un compte Azure Storage. Vous pouvez l'utiliser comme un outil indépendant ou l'incorporer dans une application existante. [Téléchargement d'AzCopy][].
...
1.  Ouvrez la fenêtre de la console Azure PowerShell, comme indiqué dans la section [Installation et configuration d'Azure PowerShell][].
...
Avant de l'utiliser, vous devez connaître le nom et la clé de votre compte Azure Storage. Pour obtenir des instructions sur l'obtention de ces informations, consultez la section «  Affichage, copie et régénération de clés d'accès de stockage » de la rubrique [Gestion des comptes de stockage][].
...
Pour plus d'informations sur l'utilisation de Sqoop, consultez la rubrique [Utilisation de Sqoop avec HDInsight][].
...
-   [Utilisation de Pig avec HDInsight][]
  ...
  [bibliothèque du client de stockage Windows Azure pour .NET]: /en-us/develop/net/how-to-guides/blob-storage/
