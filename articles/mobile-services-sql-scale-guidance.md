...
Si l'un des points ci-dessus n'est pas confirmé, ajustez vos paramètres de mise à l'échelle sous l'onglet *Scale*. Si cela ne résout pas le problème, passez à l'étape suivante et cherchez si la base de données SQL Azure peut être à l'origine du problème. Les prochaines sections proposent différentes approches pour diagnostiquer le problème.
...
Pour plus d'informations sur l'utilisation de chaque niveau, consultez la rubrique [Raisons d'utiliser les nouveaux niveaux de service](http://msdn.microsoft.com/library/azure/dn369873.aspx#Reasons)
...
Si vous exécutez souvent les instructions WHERE ou JOIN sur certaines colonnes, veillez donc à les indexer. Consultez la section [Création d'un index](#CreatingIndexes) pour plus d'informations.
...
Pour plus d'informations sur les index, consultez la page sur les [annotations d'index dans Entity Framework][]. Pour d'autres conseils sur l'optimisation des index, consultez la page [indexation avancée](#AdvancedIndexing) à la fin de ce document.
...
4. Dans la boîte de dialogue **Se connecter au serveur**, entrez les valeurs suivantes
    - Nom du serveur : *server address you obtained earlier*
    - Authentification: *SQL Server Authentication*
    - Connexion : *login you picked when creating server*
    - Mot de passe : *password you picked when creating server*
...
2. Configurez la connexion en appliquant les valeurs suivantes.
    - Serveur : *should be pre-set to the right value*
    - Base de données : *leave blank*
    - Nom d'utilisateur : *login you picked when creating server*
    - Mot de passe : *password you picked when creating server*
...
- Restreinte : utilise un petit datatype, ou est une [clé composite][Contraintes de clé primaire et de clé étrangère] d'un petit nombre de colonnes restreintes
...
La clé doit être **statique** et **en constante augmentation** pour éviter de devoir conserver l'emplacement physique des enregistrements (ce qui signifie soit déplacer les enregistrements physiquement, soit potentiellement fragmenter le stockage en fractionnant les pages dans lesquelles les enregistrements sont stockés). 
...
L'exemple de requête suivant exécute une jointure sur ces tables pour obtenir la liste des colonnes qui doivent faire partie de chaque index manquant et calcule un 'index advantage' pour déterminer si l'index concerné doit être pris en compte :
...
[Création d'index cluster]: http://technet.microsoft.com/library/ms186342(v=sql.120).aspx
[Création d'index uniques]: http://technet.microsoft.com/library/ms187019.aspx
[Création d'index non cluster]: http://technet.microsoft.com/library/ms189280.aspx
...
[Consignes générales pour la création d'index]: http://technet.microsoft.com/library/ms191195(v=sql.105).aspx 
[Directives pour la conception d'index uniques]: http://technet.microsoft.com/library/ms187019(v=sql.105).aspx
[Instructions sur la conception des index cluster]: http://technet.microsoft.com/library/ms190639(v=sql.105).aspx
