...
Pour plus d'informations sur l'utilisation de chaque niveau, consultez la rubrique [Raisons d'utiliser les nouveaux niveaux de service][Raisons d'utiliser les nouveaux niveaux de service]
...
Si des métriques dépassent 80 % d'utilisation pendant une longue période, ce peut être révélateur d'un problème de performance. Pour plus d'informations sur l'utilisation des bases de données, consultez la rubrique [Présentation de l'utilisation des ressources][Présentation de l'utilisation des ressources].

Si les mesures indiquent que votre base de données supporte une utilisation intensive, envisagez comme première mesure de **mettre à l'échelle la base de données vers un niveau de service plus élevé**. Pour résoudre les problèmes immédiatement et mettre à l'échelle la base de données, vous pouvez utiliser l'onglet **Mise à l'échelle** de votre base de données. Votre facture sera alors plus élevée.
...
3.  Sélectionnez la mesure pour laquelle vous souhaitez définir une alerte et sélectionnez **Ajouter une règle**
...
5.  Spécifiez la valeur à utiliser comme seuil d'alerte. Vous pouvez indiquer **80 %** pour vous ménager un temps de réaction. Veillez également à spécifier une adresse électronique que vous surveillez activement.
    ![Azure Management Portal - SQL Alert Threshold and Email][Azure Management Portal - SQL Alert Threshold and Email]

Pour plus d'informations sur le diagnostic des problèmes SQL, consultez la section [Diagnostic avancé][Diagnostic avancé] à la fin de ce document.
...
Si vous exécutez souvent les instructions WHERE ou JOIN sur certaines colonnes, veillez donc à les indexer. Pour plus d'informations, consultez la section [Création d'index][Création d'index].
...
Le portail de gestion met certaines mesures à disposition avec les niveaux Basic, Standard et Premium. Cependant, avec les niveaux Web et Business, seule la mesure de stockage est disponible via le portail. Heureusement, ces mesures, comme d'autres, sont faciles à obtenir avec la vue de gestion **[sys.resource\_stats][sys.resource\_stats]**, indépendamment du niveau que vous utilisez. Examinez la requête suivante :
...
-   [Documentation de base de données SQL Azure][Documentation de base de données SQL Azure]
-   [Performances et mise à l'échelle de la base de données SQL Azure][Performances et mise à l'échelle de la base de données SQL Azure]
-   [Résolution des problèmes de base de données SQL Azure][Résolution des problèmes de base de données SQL Azure]

### Indexation

-   [Notions fondamentales sur les index][Notions fondamentales sur les index]
-   [Consignes générales pour la création d'index][Consignes générales pour la création d'index]
-   [Directives pour la conception d'index uniques][Directives pour la conception d'index uniques]
-   [Instructions sur la conception des index cluster][Instructions sur la conception des index cluster]
-   [Contraintes de clé primaire et de clé étrangère][clé composite]
-   [Combien coûte cette clé ?][Combien coûte cette clé ?]

### Entity Framework

-   [Considérations sur les performances d'Entity Framework 5][Considérations sur les performances d'Entity Framework 5]
-   [Annotations de données Code First][Annotations de données Code First]
...
<!-- BLOG LINKS -->
  ...
  [annotations d'index dans Entity Framework]: http://msdn.microsoft.com/en-us/data/jj591583.aspx#Index
  [Indexation avancée]: #AdvancedIndexing
  [objet table]: http://msdn.microsoft.com/en-us/library/windowsazure/jj554210.aspx
  ...
  [objet query]: http://msdn.microsoft.com/en-us/library/azure/jj613353.aspx
  [méthode Take]: http://msdn.microsoft.com/en-us/library/vstudio/bb503062(v=vs.110).aspx
  ...
  [Cache Microsoft Azure]: http://azure.microsoft.com/en-us/services/cache/
  ...
  [Éditions SQL Server – Express]: http://www.microsoft.com/en-us/server-cloud/products/sql-server-editions/sql-server-express.aspx
  ...
  [clé composite]: http://msdn.microsoft.com/en-us/library/ms179610(v=sql.120).aspx
  [Création et modification des contraintes PRIMARY KEY]: http://technet.microsoft.com/en-us/library/ms181043(v=sql.105).aspx
  [Création d'index non cluster]: http://technet.microsoft.com/en-us/library/ms189280.aspx
  [Création d'index cluster]: http://technet.microsoft.com/en-us/library/ms186342(v=sql.120).aspx
  [Création d'index uniques]: http://technet.microsoft.com/en-us/library/ms187019.aspx
  ...
  [Résolution des problèmes de base de données SQL Azure]: http://msdn.microsoft.com/en-us/library/azure/ee730906.aspx
  [Notions fondamentales sur les index]: http://technet.microsoft.com/en-us/library/ms190457(v=sql.105).aspx
  [Consignes générales pour la création d'index]: http://technet.microsoft.com/en-us/library/ms191195(v=sql.105).aspx
  [Directives pour la conception d'index uniques]: http://technet.microsoft.com/en-us/library/ms187019(v=sql.105).aspx
  [Instructions sur la conception des index cluster]: http://technet.microsoft.com/en-us/library/ms190639(v=sql.105).aspx
  [Combien coûte cette clé ?]: http://www.sqlskills.com/blogs/kimberly/how-much-does-that-key-cost-plus-sp_helpindex9/
  [Considérations sur les performances d'Entity Framework 5]: http://msdn.microsoft.com/en-us/data/hh949853
  [Annotations de données Code First]: http://msdn.microsoft.com/en-us/data/jj591583.aspx
