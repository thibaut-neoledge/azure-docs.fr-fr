...
- [Power Query : connexion à une base de données Microsoft SQL Server locale][Power-Query-OnPrem-SQL] 
...
- **Se connecter et collecter**. Dans cette étape, les données de différentes sources sont importées dans des hubs de données.
...
### Service lié
Service qui est lié à une fabrique de données Azure. Un service lié peut être :
...
Les tranches permettent aux professionnels de l'informatique de travailler avec un sous-jeu de données globales pendant une période spécifique (par exemple : la tranche exécutée pour la durée (heure) : De 13 à 14H00). Ils peuvent également afficher toutes les tranches de données en aval pendant un certain temps, en interne, et réexécuter une tranche en cas de défaillance.
