<properties 
	pageTitle="Utilisation du stockage de tables (PHP) | Microsoft Azure" 
...
##<a id="ConnectionString"></a>Configuration d'une connexion de stockage Azure
...
##<a id="CreateTable"></a>Procédure : Création d'une table
...
##<a id="RetrieveEntity"></a>Procédure : Extraction d'une seule entité

La méthode **TableRestProxy -> getEntity** vous permet d'extraire une seule entité via une requête portant sur `PartitionKey` et `RowKey`. Dans l'exemple ci-dessous, la clé de partition `tasksSeattle` et clé de ligne " 1 " sont transmises à la méthode **getEntity**.
...
##<a id="RetEntitiesInPartition"></a>Procédure : Extraction de toutes les entités d'une partition

Les requêtes d'entité sont construites à l'aide de filtres (pour plus d'informations, consultez la page [Interrogation de tables et d'entités][filters]). Pour extraire toutes les entités d'une partition, utilisez le filtre " PartitionKey eq *partition_name* ". L'exemple suivant montre comment extraire toutes les entités de la partition `tasksSeattle` en transmettant un filtre à la méthode **queryEntities**.
...
##<a id="RetrieveSubset"></a>Procédure : Extraction d'un sous-ensemble d'entités dans une partition
...
##<a id="RetPropertiesSubset"></a>Procédure : Extraction d'un sous-ensemble de propriétés d'entité
...
##<a id="UpdateEntity"></a>Procédure : Mise à jour d'une entité
...
##<a id="DeleteEntity"></a>Procédure : Suppression d'une entité

Pour supprimer une entité, transmettez le nom de la table et les clés `PartitionKey` et `RowKey` à la méthode **TableRestProxy -> deleteEntity**.
...
##<a id="BatchOperations"></a>Procédure : Traitement par lots d'opérations de table
...
##<a id="DeleteTable"></a>Procédure : Suppression d'une table
