...
adaptée aux appareils mobiles et son déploiement dans Azure. Pour ce didacticiel, vous avez besoin de
[Visual Studio Express 2013 pour le Web][Visual Studio Express 2013]
ou l'édition professionnelle de Visual Studio si vous l'avez déjà
.
...
Dans ce didacticiel, vous ajouterez des fonctionnalités mobiles à l'application simple de listes de conférence fournie dans le [projet de départ][StarterProject]. La capture d'écran suivante illustre les sessions ASP.NET dans l'application terminée telles qu'elles s'affichent dans l'émulateur de navigateur des outils destinés aux développeurs F12 d'Internet Explorer 11.
...
-   Émulateur de navigateur dans les [outils destinés aux développeur F12 d'Internet Explorer 11][EmulatorIE11] (utilisé dans toutes les captures d'écran de navigateur mobile
). Il dispose de présélections de chaîne d'agent utilisateur pour Windows Phone 8, Windows Phone 7 et l'iPad d'Apple.
...
Les projets Visual Studio avec code source C\# sont disponibles pour
cette rubrique :
...
choisissez le bouton **Débloquer**. Le déblocage empêche les avertissements de sécurité
se produisant lorsque vous essayez d'utiliser un fichier *.zip* que vous avez téléchargé
à partir du web.)

4.	Cliquez sur le fichier *Mvc5Mobile.zip* et sélectionnez **Extraire tout** pour
...
11. La boîte de dialogue Sélectionner un site Web existant doit à présent indiquer que vous êtes connecté. Cliquez sur **Nouveau**.

	![][DeployNewWebsite]  

12. Dans le champ **Nom du site**, indiquez le préfixe d'un nom de site unique. Le nom complet de votre site sera au format *&lt;préfixe>*.azurewebsites.net. De plus, dans le champ **Région**, sélectionnez une région. Cliquez ensuite sur **Créer**.
...
*bootstrap.js* et * bootstrap.css*. Le code suivant concerne les deux dernières
instructions de la méthode " RegisterBundles " après la modification :
...
Vous pouvez remarquer que la barre de navigation réductible n'est maintenant qu'une simple liste non triée classique. Cliquez sur **Parcourir par balise** à nouveau, puis cliquez sur **ASP.NET**. Dans la vue de l'émulateur mobile, vous voyez maintenant qu'elle n'est plus adaptée à l'écran, et vous devez faire défiler la page horizontalement afin de voir le côté droit du tableau.
...
Bootstrap n'est pas spécifique d'ASP.NET MVC 5 et vous pouvez tirer parti de ces fonctionnalités dans n'importe quelle application web. Désormais, Bootstrap est intégré au modèle de projet ASP.NET MVC 5. Votre application web MVC 5 peut donc tirer parti de Bootstrap par défaut.

Pour plus d'informations sur Bootstrap, rendez-vous sur le site [Bootstrap][BootstrapSite].

Cette section vous explique comment fournir des vues spécialement adaptées aux appareils mobiles
.
...
Pour commencer, copiez *Views\\Shared\\\_Layout.cshtml* vers *Views\\Shared\\\_Layout.Mobile.cshtml*. Ouvrez *\_Layout.Mobile.cshtml* et remplacez le titre **MVC5 Application** par **MVC5 Application (Mobile)**.

Dans chaque appel " Html.ActionLink " pour la barre de navigation, supprimez " Parcourir par " dans chaque lien *ActionLink*. Le code suivant affiche la balise " <ul class="nav navbar-nav"> " terminée d'un fichier de disposition mobile.
...
Par contre, l'affichage du Bureau n'a pas changé (avec des titres de *\_Layout.cshtml* et de *AllTags.cshtml*).

![][AllTagsMobile_LayoutMobileDesktop]

<h2><a name="bkmk_browserviews"></a>Création de vues de navigateur mobiles</h2>
...
Dans le code, cliquez avec le bouton droit sur " DefaultDisplayMode ", choisissez **Resolve**, puis
choisissez " using System.Web.WebPages; ". Cela ajoute une référence à l'
...
Exécutez l'application. Lancez un émulateur de navigateur mobile, vérifiez que son agent utilisateur est défini sur " iPhone " et parcourez la vue *AllTags*. Si vous utilisez l'émulateur dans les outils destinés aux développeurs F12 d'Internet Explorer 11, configurez l'émulation comme suit :

-   Profil du navigateur = **Windows Phone**
-   Chaîne d'agent utilisateur =  **Custom**
-   Chaîne personnalisée = **Apple-iPhone5C1/1001.525**

La capture d'écran qui suit présente la vue *AllTags* affichée dans l'émulateur des outils destinés aux développeurs F12 d'Internet Explorer 11, avec la chaîne d'agent utilisateur personnalisée (il s'agit d'une chaîne d'agent utilisateur d'un iPhone 5C).

![][AllTagsIPhone_LayoutIPhone]

Dans le navigateur mobile, sélectionnez le lien **Speakers**. En l'absence d'affichage mobile (*AllSpeakers.Mobile.cshtml*), la vue par défaut des intervenants (*AllSpeakers.cshtml*) est affichée à l'aide du mode de disposition mobile (*\_Layout.Mobile.cshtml*). Comme indiqué ci-dessous, le titre **MVC5 Application (Mobile)** est défini dans *\_Layout.Mobile.cshtml*.
...
Lorsque " RequireConsistentDisplayMode " est défini sur " true ", la disposition mobile (*\_Layout.Mobile.cshtml*) est utilisée uniquement pour les affichages mobiles (lorsque le fichier de vue se présente sous la forme ***NomVue**.Mobile.cshtml*). Vous pouvez définir " RequireConsistentDisplayMode " sur " true " si votre disposition mobile ne fonctionne pas bien avec les vues non mobiles. La capture d'écran suivante indique comment la page *Speakers* s'affiche lorsque " RequireConsistentDisplayMode " est défini sur " true " (sans la chaîne " (Mobile) " dans la barre de navigation, dans la partie supérieure).

![][AllSpeakers_LayoutMobileOverridden]

Vous pouvez désactiver le mode d'affichage cohérent dans une vue spécifique en définissant " RequireConsistentDisplayMode " sur " false " dans le fichier de vue. Le balisage suivant du fichier *Views\\Home\\AllSpeakers.cshtml* définit " RequireConsistentDisplayMode " sur " false " :
...
Dans cette section, nous avons vu comment créer des vues et des dispositions mobiles, et comment créer des dispositions et des vues pour des appareils spécifiques tels que l'iPhone. Toutefois, le principal avantage de l'infrastructure d'amorçage (Bootstrap) CSS est la disposition réactive, ce qui signifie qu'une seule feuille de style peut être appliquée sur les navigateurs de bureau, de téléphone et de tablette pour créer une apparence cohérente. Cette section vous présente la façon de tirer parti de Bootstrap et de créer des vues adaptées aux appareils mobiles.
...
Vous pouvez utiliser le style de [groupe de liste liée][] pour améliorer la vue *Speakers*. Dans *Views\\Home\\AllSpeakers.cshtml*, remplacez le contenu du fichier Razor par le code ci-dessous.
...
L'attribut " class="list-group " dans la balise " <div> " applique le style de liste 
Bootstrap et l'attribut " class="input-group-item "
applique le style d'élément de liste Bootstrap à chaque lien.

Actualisez le navigateur mobile. La vue actualisée ressemble à ceci :

![][AllSpeakersFixed]

Avec le style de [groupe de liste liée][] Bootstrap, toute la zone de chaque lien est cliquable, ce qui confère un confort d'utilisation optimal. Basculez vers l'affichage de bureau et jetez un œil à cet aspect cohérent.
...
Remarque : les styles Bootstrap ont été appliqués aux balises " <form> " et " <input> ". L'élément " <span> " ajoute un [glyphicon][] Bootstrap à la search box.
...
Vous devez également inclure le fichier filter.js à vos lots enregistrés. Ouvrez le fichier *App_Start\BundleConfig.cs* et modifiez les premiers lots. Modifiez la première instruction " bundles.Add " (pour le lot **jquery**) de manière à inclure le fichier *Scripts\\filter.js*, comme décrit ci-dessous : 

     bundles.Add(new ScriptBundle("~/bundles/jquery").Include(
                "~/Scripts/jquery-{version}.js",
                "~/Scripts/filter.js"));

Le lot **jquery** est déjà rendu par la vue par défaut *\_Layout*
. Plus tard, vous pouvez utiliser le même code JavaScript pour appliquer la
fonctionnalité de filtre à d'autres vues de liste.
...
Vous pouvez améliorer la vue *Dates* en organisant les valeurs de date et heure
par date. Pour ce faire, vous devez utiliser le style
[panels][] Bootstrap. Remplacez
le contenu du fichier *Views\\Home\\AllDates.cshtml* par le
code suivant :
...
Ce code crée une balise " <div class="panel panel-primary"> " séparée pour
chaque date distincte dans la liste, puis utilise le [groupe de liste liée][] pour les
liens correspondants comme avant. Voici à quoi ressemble le navigateur mobile
...
-   Il utilise le [groupe personnalisé de liste liée][] Bootstrap pour mettre en forme les informations de session de façon verticale. Ainsi, toutes ces informations sont lisibles sur un navigateur mobile (à l'aide de classes telles que list-group-item-text).
...
-   Il utilise les [utilitaires réactifs][] pour masquer les balises de session lorsqu'elles s'affichent dans le navigateur mobile (à l'aide de la classe hidden-xs).
...
Le système de grille Bootstrap que vous avez automatiquement appliqué permet de classer les sessions à la verticale dans le navigateur mobile. En outre, vous remarquerez que les balises ne s'affichent pas. Basculez vers le navigateur de bureau.
...
Dernière étape, vous allez optimiser la vue *SessionByCode* pour l'adapter aux appareils mobiles.

Dans le navigateur mobile, appuyez sur le bouton **Balise**, puis saisissez " asp " dans la
zone de recherche.
...
Ouvrez *Views\\Home\\SessionByCode.cshtml* et remplacez le contenu par
le balisage suivant :
...
Actualisez le navigateur mobile. L'image suivante reflète les changements de code
...
Ce didacticiel vous a montré comment utiliser ASP.NET MVC 5 pour développer
des applications web compatibles avec les appareils mobiles. Vous avez notamment vu les points suivants :
...
-   Utiliser Bootstrap pour créer une disposition web réactive au sein de votre application
    MVC 5
-   Remplacer la disposition, les vues et les vues partielles, de façon globale ou pour une
    vue individuelle
...
[Création de vues de navigateur mobiles]:#bkmk_browserviews
...
[groupe de liste liée]: http://getbootstrap.com/components/#list-group-linked
[glyphicon]: http://getbootstrap.com/components/#glyphicons
[panneaux]: http://getbootstrap.com/components/#panels
[groupe personnalisé de liste liée]: http://getbootstrap.com/components/#list-group-custom-content
