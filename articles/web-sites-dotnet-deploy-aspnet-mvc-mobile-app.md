...
Ce didacticiel aborde les bases de la conception d'une application web ASP.NET MVC 5 adaptée aux appareils mobiles et de son déploiement dans Azure App Service. Pour ce didacticiel, vous devez disposer de 
...
1. Dans la fenêtre Web Platform Installer, cliquez sur **Installer**, puis procédez à l'installation.
...
-   Émulateur de navigateur dans les [Outils de développement F12 d'Internet Explorer 11][EmulatorIE11] (utilisé dans toutes les captures d'écran de navigateur mobile). Il dispose de présélections de chaîne d'agent utilisateur pour Windows Phone 8, Windows Phone 7 et l'iPad d'Apple.
...
-   [Émulateur Opera Mobile][EmulatorOpera]
...
2. 	Dans l'Explorateur Windows, cliquez avec le bouton droit sur le fichier Mvc5Mobile.zip, puis choisissez *Properties*.
...
4.	Cliquez avec le bouton droit sur le fichier *Mvc5Mobile.zip*, puis sélectionnez **Extraire tout** pour décompresser le fichier. 
...
9.	Tapez votre nom de compte Azure, puis cliquez sur **Continuer**.

	<!--todo: ![][DeployUsername] -->

10.	Tapez votre mot de passe, puis cliquez sur **Se connecter**.
...
12. Dans le champ **Nom du site**, indiquez un préfixe unique pour le nom du site. Le nom complet de votre site sera *&lt;préfixe>*.azurewebsites.net. Par ailleurs, dans le champ **Région**, sélectionnez une région. Ensuite, cliquez sur **Créer**.
...
*bootstrap.js* et *bootstrap.css*. Le code ci-après indique les deux dernières instructions de la méthode `RegisterBundles` après la modification :
...
Bootstrap n'est pas propre à ASP.NET MVC 5 et vous pouvez tirer parti de ces fonctionnalités dans n'importe quelle application web. Mais Bootstrap est désormais intégré au
modèle de projet ASP.NET MVC 5. Votre application web MVC 5 peut donc tirer parti de Bootstrap par défaut.

Pour plus d'informations sur Bootstrap, accédez au site
...
Accédez à la page des balises à l'aide d'un navigateur de Bureau et de l'émulateur de navigateur mobile. L'émulateur de navigateur mobile affiche les deux modifications que vous avez effectuées (le titre de *\_Layout.Mobile.cshtml* et le titre de
...
-   Chaîne de l'agent utilisateur =  **Personnalisée**
-   Chaîne personnalisée = **Apple-iPhone5C1/1001.525**

La capture d'écran qui suit présente la vue *AllTags* affichée dans l'émulateur des Outils de développement F12 d'Internet Explorer 11 avec la chaîne d'agent utilisateur personnalisée (il s'agit de la chaîne de l'agent utilisateur d'un iPhone 5C).
...
Vous pouvez utiliser le style Bootstrap [liste groupée avec liens][] pour améliorer la vue *Speakers*. Dans *Views\\Home\\AllSpeakers.cshtml*, remplacez le contenu du fichier Razor par le code ci-dessous.
...
L'attribut `class="list-group"` de la balise `<div>` applique le
style Bootstrap liste, tandis que l'attribut `class="input-group-item"` applique le style Bootstrap élément de liste à chaque lien.

Actualisez le navigateur mobile. L'affichage actualisé ressemble à ceci :

![][AllSpeakersFixed]

Avec le style Bootstrap [liste groupée avec liens][], la totalité de la zone de chaque lien est interactive, ce qui améliore sensiblement l'expérience utilisateur. Basculez vers l'affichage de bureau et observez l'homogénéité de l'apparence.

![][AllSpeakersFixedDesktop]

Certes, l'affichage du navigateur mobile a été amélioré, mais il est tout de même difficile de parcourir la longue liste d'intervenants. Bootstrap est dépourvu d'une fonction de filtre de recherche en natif. Il est possible d'en ajouter une à l'aide de quelques lignes de code. Vous devez commencer par ajouter une zone de recherche à la vue, puis du code JavaScript pour la fonction de filtre. Dans
...
Vous devez également inclure le fichier filter.js à vos lots enregistrés. Ouvrir
*App\_Start\\BundleConfig.cs* et modifiez les premiers lots. Modifiez la première instruction `bundles.Add` (pour le lot **jquery**) en y insérant
*Scripts\\filter.js*, comme indiqué ci-après :
...
Actualisez le navigateur mobile, puis accédez à la vue *AllSpeakers*. Dans la zone de recherche, entrez " sc ". La liste des intervenants doit à présent être filtrée selon vos critères de recherche.
...
Vous pouvez encore améliorer la vue *Dates* en classant les valeurs date-heure par date. Pour effectuer cette opération, vous devez utiliser le style Bootstrap
[panneaux][]. Remplacez le contenu du fichier *Views\\Home\\AllDates.cshtml* par le code suivant :
...
Comme vous pouvez le constater, l'affichage se fait sous forme de tableau, actuellement conçu pour être affiché sur le navigateur de bureau. Toutefois, ce type d'affichage n'est pas adapté à un navigateur mobile. Pour résoudre ce problème, ouvrez
...
Ouvrez *Views\\Home\\SessionByCode.cshtml*, puis remplacez le contenu par le balisage suivant :
...
-   [Bootstrap][BootstrapSite]
-   [Blog officiel Bootstrap (en anglais)][]
-   [Didacticiel Bootstrap Twitter de Tutorial Republic (en anglais)][]
-   [Laboratoire Bootstrap (en anglais)][]
-   [Recommandation du W3C en matière de bonnes pratiques pour la création d'applications Web mobiles (en anglais)][]
...
<!-- Internal Links -->
[Déployer le projet de départ dans une application web Azure]: #bkmk_DeployStarterProject
[Infrastructure CSS Bootstrap]: #bkmk_bootstrap
[Remplacer des vues, des dispositions et des vues partielles]: #bkmk_overrideviews
[Créer des vues spécifiques du navigateur]:#bkmk_browserviews
[Améliorer la liste des intervenants]: #bkmk_Improvespeakerslist
[Améliorer la liste des balises]: #bkmk_improvetags
[Améliorer la liste des dates]: #bkmk_improvedates
[Améliorer la vue SessionsTable]: #bkmk_improvesessionstable
[Améliorer la vue SessionByCode]: #bkmk_improvesessionbycode
...
[Didacticiel Bootstrap Twitter de Tutorial Republic (en anglais)]: http://www.tutorialrepublic.com/twitter-bootstrap-tutorial/
[Laboratoire Bootstrap (en anglais)]: http://www.bootply.com/
[Recommandation du W3C en matière de bonnes pratiques pour la création d'applications Web mobiles (en anglais)]: http://www.w3.org/TR/mwabp/
