...
- Un [compte Azure Storage](storage-create-storage-account.md) avec un conteneur d'objets blob pour stocker la sauvegarde de la base de données. Actuellement, le compte de stockage doit utiliser le modèle de déploiement classique : choisissez par conséquent **Classique** lors de la création d'un compte de stockage. 
...
2.	Cliquez sur **PARCOURIR TOUT**.
3.	Cliquez sur **Comptes de stockage (classique)**.
2.	Cliquez sur le compte de stockage où vous avez stocké le fichier BACPAC.
3.	Cliquez sur **Conteneurs** et sélectionnez le conteneur dans lequel vous avez exporté la base de données pour obtenir les détails de la sauvegarde (vous pouvez télécharger et enregistrer le fichier BACPAC ici).
