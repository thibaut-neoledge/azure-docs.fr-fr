...
- La vignette **Requêtes de base de données** affiche le résumé des performances de requête de votre base de données (utilisation DTU globale et requêtes les plus consommatrices de ressources). Cette vignette vous amène sur **Query Performance Insight**.
...
[SQL Database Advisor](sql-database-index-advisor.md) fournit des recommandations de paramétrage intelligent qui peuvent aider à améliorer les performances de votre base de données.
