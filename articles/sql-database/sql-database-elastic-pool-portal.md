...
Vous pouvez sélectionner à tout moment les bases de données spécifiques à inclure dans le pool. Lorsque vous créez un nouveau pool, Azure recommande les bases de données adaptées pour un pool en les marquant pour l'inclusion. Azure évalue l’historique d’utilisation des bases de données et recommande un pool élastique lorsqu’il est plus rentable que l’utilisation des niveaux de performances pour les bases de données uniques. Si un pool élastique est recommandé, Azure fournit la quantité recommandée de DTU du pool, les paramètres de DTU min/max pour chaque base de données du pool et une liste de bases de données recommandées. Pour qu’une base de données soit considérée comme candidate au pool élastique, elle doit exister au moins depuis 14 jours et ne doit pas être au niveau de tarification Premium (la version préliminaire publique est limitée au niveau Standard uniquement, les bases de données Premium ne sont donc pas considérées comme viables). Vous pouvez ajouter toutes les bases de données disponibles sur le serveur, ou vous pouvez sélectionner des bases de données dans la liste initiale ou en effacer selon vos besoins.
