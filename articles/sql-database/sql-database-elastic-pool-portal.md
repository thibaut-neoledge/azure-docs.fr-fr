<properties 
	pageTitle="Créer et gérer un pool élastique de bases de données SQL" 
...
# Créer et gérer un pool élastique de bases de données SQL
...
Cet article vous montre comment créer un pool élastique de bases de données avec la [version préliminaire du portail](https://portal.azure.com).

Les pools élastiques de bases de données simplifient le processus de création, de maintenance et de gestion des performances et des coûts pour un grand nombre de bases de données.
...
Le niveau de tarification d’un pool élastique de bases de données détermine les fonctionnalités disponibles pour les bases de données élastiques dans le pool, ainsi que le nombre maximal d'eDTU (eDTU MAX) et le stockage (Go) disponibles pour chaque base de données. Pour en savoir plus, consultez la rubrique [Limites relatives aux eDTU et au stockage pour les pools et bases de données élastiques](sql-database-elastic-pool-reference.md#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases).
...
## Étapes suivantes
Après avoir créé un pool élastique de bases de données, vous pouvez gérer des bases de données du pool en créant des tâches élastiques. Les tâches élastiques facilitent l'exécution de scripts T-SQL, quel que soit le nombre de bases de données dans le pool. Pour en savoir plus, consultez [Vue d'ensemble des tâches de base de données élastiques](sql-database-elastic-jobs-overview.md).
