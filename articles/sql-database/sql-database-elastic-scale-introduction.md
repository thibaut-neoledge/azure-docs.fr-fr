...
* Outils de base de données élastique : ces deux outils simplifient le développement et la gestion des solutions de base de données partitionnées. Les outils incluent la [bibliothèque cliente de la base de données élastique](sql-database-elastic-client-overview.md) et l’[outil de fusion et de fractionnement de la base de données élastique](sql-database-elastic-scale-overview-split-and-merge.md). 
...
* [Tâches de base de données élastique](sql-database-elastic-jobs-overview.md) (version préliminaire) : utilisez des tâches pour gérer un grand nombre de bases de données SQL Azure. Une **tâche** est un script T-SQL idempotent exécuté par rapport à un ensemble prédéfini de bases de données. Les échecs sont automatiquement relancés (configurables) et toutes les erreurs sont enregistrées pour le débogage. 
...
6. La **une requête de base de données élastique** vous permet d'écrire une requête qui s'étend sur toutes les bases de données de l'ensemble de partitions.
...
Pour plus de détails sur le pool de bases de données élastique, consultez la rubrique [Considérations sur les prix et performances pour un pool de bases de données élastique](sql-database-elastic-pool-guidance.md), ou créez un pool à l’aide du [didacticiel](sql-database-elastic-pool-portal.md).
