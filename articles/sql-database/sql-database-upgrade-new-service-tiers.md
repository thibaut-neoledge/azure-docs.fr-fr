<properties 
	...
	description="Mise à niveau de base de données Azure SQL Web ou Business vers les nouveaux niveaux de service/de performances de base de données Azure SQL. Mise à niveau de base de données SQL, modification des niveaux de service, modification des niveaux de performances." 
...
Les bases de données Web et Business Azure SQL sont déconseillées et [retirées en septembre 2015](http://msdn.microsoft.com/library/azure/dn741330.aspx). Il est donc temps de commencer à planifier la mise à niveau des bases de données Web ou Business vers les niveaux de service de base, Standard ou Premium.
...
Par exemple, vous devez prendre en compte la durée de conservation des longues sauvegardes, si des fonctions de [géo-réplication Standard ou Active](http://msdn.microsoft.com/library/azure/dn783447.aspx) sont requises, la taille globale maximale de la base de données nécessaire, etc. Ces exigences déterminent votre niveau de service minimal.
...
Dans la mesure où les bases de données Web et Business ne sont associées à aucune limite de ressources/UDBD garantie, nous normalisons les valeurs de pourcentage en termes de quantité de ressources disponibles pour une base de données au niveau de performances S2. Le pourcentage UDBD moyen utilisé par une base de données dans un intervalle donné peut être calculé comme la valeur de pourcentage la plus élevée entre l'utilisation du processeur, les E/S et le journal dans cet intervalle.


Utilisez le portail de gestion pour obtenir une vue d'ensemble du pourcentage d'utilisation UDBD, puis explorez les détails à l'aide de vues système.


### Affichage de la consommation UDBD dans le portail de gestion
Le portail de gestion permet de connaître la consommation UDBD d'une base de données Web ou Business.
Les informations UDBD sont disponibles dans le portail Azure actuel.
...
4. Sélectionnez **Pourcentage UDBD** et cliquez sur la coche en bas pour confirmer.

Après avoir confirmé, vous verrez les données relatives au **pourcentage UDBD** dans le tableau. N'oubliez pas que c'est un pourcentage par rapport aux UDBD dans une base de données de niveau Standard(S2), ce qui correspond à 50 UDBD.
...
Les informations sur la consommation UDBD en termes de niveau S2 de base de données vous permettent de normaliser la consommation actuelle de vos bases de données Web/Business en termes de bases de données du nouveau service et de déterminer le niveau qui leur convient le mieux. Par exemple, si le pourcentage moyen de consommation UDBD indique une valeur de 80 %, cela signifie que la base de données consomme des UDBD à 80 % de la limite d'une base de données au niveau de performances S2. Si des valeurs sont supérieures à 100 % dans la vue **sys.resource_stats**, vous avez besoin d'un niveau de performances supérieur à S2. Par exemple, supposons que vous observez une valeur de pourcentage UDBD maximale de 300 %.  Cela vous indique que vous utilisez trois fois plus de ressources que la quantité disponible dans un niveau S2. Pour déterminer une taille de départ raisonnable, comparez les UDBD disponibles dans un niveau S2 (50 UDBD) à celles des niveaux de taille supérieure (S3P1 = 100 UDBD ou 200 % de S2, P2 = 200 UDBD ou 400 % de S2). Avec une valeur de 300 % de S2, il est souhaitable de commencer par un niveau P2 et de recommencer les tests. 

En fonction du pourcentage d'utilisation UDBD et de l'édition la plus grande correspondant à votre charge de travail, vous pouvez déterminer le niveau de service et le niveau de performances les mieux adaptés à la charge de travail de votre base de données (comme indiqué par le pourcentage UDBD et la puissance UDBD relative de divers [niveaux de performances)](http://msdn.microsoft.com/library/azure/dn741336.aspx). Le tableau suivant indique la correspondance entre le pourcentage de consommation des ressources Web/Business et les niveaux de performances des nouveaux niveaux de service : 

![Resource consumption][4]

> **Remarque :**
> le nombre d'UDBD relatif des différents niveaux de performances est basé sur la charge de travail de [référence d'Azure SQL Database](http://msdn.microsoft.com/library/azure/dn741327.aspx). Dans la mesure où la charge de travail de votre base de données est susceptible de ne pas être égale à celle de référence, vous devez utiliser les calculs ci-dessus pour trouver le nouveau niveau de service adapté à votre base de données Web/Business. Une fois que vous avez déplacé la base de données vers le nouveau niveau, utilisez la procédure décrite dans la section précédente pour valider/ajuster le niveau de service par rapport aux besoins de votre charge de travail.
> 
...
| Article | Description |
|:--|:--|
| [Guide des performances de base de données SQL Azure](http://msdn.microsoft.com/library/dn369873.aspx) | La section 'Tuning Your Application' fournit des recommandations détaillées et des techniques relatives au réglage des performances d'une base de données SQL Azure.|
...
| :---| :---|
| [Portail de gestion Azure](https://manage.windowsazure.com) | Cliquez sur l'onglet **SCALE** de la page du tableau de bord de votre base de données. |
| [Azure PowerShell](http://msdn.microsoft.com/library/azure/dn546726.aspx) | Utilisation de la cmdlet [Set-AzureSqlDatabase](http://msdn.microsoft.com/library/azure/dn546732.aspx). |
| [API REST de gestion de service](http://msdn.microsoft.com/library/azure/dn505719.aspx) | Utilisation de la commande [Update Database](http://msdn.microsoft.com/library/dn505718.aspx).|
| [Transact-SQL](http://msdn.microsoft.com/library/bb510741.aspx) | Utilisation de l'instruction [ALTER DATABASE (Transact-SQL)](http://msdn.microsoft.com/library/ms174269.aspx). |
...
**Données de consommation des ressources :** pour les bases de données De base, Standard et Premium, des données plus granulaires de consommation de ressources sont disponibles via une nouvelle vue de gestion dynamique appelée [sys.dm_ db_ resource_stats](http://msdn.microsoft.com/library/azure/dn800981.aspx) dans la base de données utilisateur. Cette vue offre des informations de consommation des ressources quasiment en temps réel avec une granularité à 15 secondes pour l'heure de fonctionnement précédente. Le pourcentage de consommation UDBD pour un intervalle est calculé comme le pourcentage maximal de consommation sur les dimensions Utilisation du processeur, E/S et journal. Voici une requête qui permet de calculer le pourcentage de consommation UDBD moyen pendant la dernière heure :
...
Consultez la [documentation](http://msdn.microsoft.com/library/dn800981.aspx) pour des détails sur l'utilisation de cette vue de gestion dynamique.  [Le guide de performances Azure SQL Database](http://msdn.microsoft.com/library/azure/dn369873.aspx) explique comment surveiller et optimiser votre application.


- **Alertes :** configurez des 'Alerts' dans le portail de gestion Azure pour vous avertir quand la consommation UDBD d'une base de données mise à jour atteint un certain niveau. Les alertes de base de données peuvent être configurées dans le portail de gestion Azure pour diverses mesures de performances comme le nombre d'UDBD, l'utilisation du processeur, le nombre d'E/S et le journal. 

	Par exemple, vous pouvez configurer une alerte par courrier électronique sur " Pourcentage UDBD " si la valeur moyenne du pourcentage UDBD est supérieure à 75 % pendant les 5 dernières minutes. Consultez [Procédure : réception de notifications d'alerte et gestion des règles d'alerte dans Azure](http://msdn.microsoft.com/library/azure/dn306638.aspx) pour en savoir plus sur la configuration des notifications d'alerte.


- **Passage planifié à un niveau de performances supérieur/inférieur :** si votre application comporte des scénarios spécifiques qui nécessitent davantage de performances à certains moments de la journée/semaine, vous pouvez utiliser [Azure Automation](http://msdn.microsoft.com/library/azure/dn643629.aspx) pour planifier l'augmentation/la réduction de la taille de votre base de données vers un niveau de performances supérieur/inférieur.

	Par exemple, mettez à jour la base de données vers un niveau de performances supérieur pour la durée d'une tâche de maintenance/un traitement par lots hebdomadaire et réduisez-la à la fin de la tâche. Ce type de planification est également utile pour les opérations qui utilisent une grande quantité de ressources volumineuses comme le chargement de données, la recréation d'index, etc. Notez que le modèle de facturation d'Azure SQL Database est basé sur l'utilisation horaire d'un niveau de service/niveau de performances. Cette flexibilité vous permet de planifier des mises à jour à moindre coût.
