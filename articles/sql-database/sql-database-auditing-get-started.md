...
Les outils d'audit permettent et facilitent le respect des normes liées à la conformité, mais ne garantissent pas cette dernière. Pour plus d'informations sur les programmes Azure prenant en charge la conformité aux normes, consultez le [Centre de gestion de la confidentialité Azure](http://azure.microsoft.com/support/trust-center/compliance).
...
La section suivante décrit la configuration de l'audit à l'aide du portail Azure en version préliminaire. Vous pouvez également [configurer l'audit pour votre base de données à partir du portail Azure classique].

Éléments rendus possibles par l'audit de bases de données SQL :
...
> [AZURE.NOTE]Vous pouvez désormais recevoir des alertes proactives sur des activités anormales de la base de données qui peuvent indiquer des menaces de sécurité potentielles à l'aide de la nouvelle fonctionnalité de **détection de menaces**, désormais en version préliminaire. La détection des menaces peut activée et configurée à partir du panneau de configuration d'audit. Pour en savoir plus, consultez [Prise en main de la détection des menaces (en anglais)](sql-database-threat-detection-get-started.md).
...
Pour chaque catégorie d'événement, les audits des opérations **Succès** et **Échec** sont configurées séparément.

Pour d'autres informations sur les activités et les événements audités, consultez la page [Référence sur les formats des journaux d'audit (fichier à télécharger)](http://go.microsoft.com/fwlink/?LinkId=506733).
...
Avant de configurer le processus d'audit, assurez-vous que vous utilisez bien un [« Client de bas niveau »](sql-database-auditing-and-dynamic-data-masking-downlevel-clients.md).


##<a id="subheading-2"></a>Configuration de l'audit pour votre base de données

1. Lancez le [portail Azure en version préliminaire](https://portal.azure.com) à l'adresse https://portal.azure.com. Vous pouvez également lancer la [version classique du portail Azure](https://manage.windowsazure.com/) à l'adresse https://manage.windowsazure.com/. Consultez les détails ci-dessous.

2. Accédez au volet des paramètres de la base de données SQL/du SQL Server que vous voulez auditer. Dans le volet Paramètres, sélectionnez **Audit et détection des menaces (Auditing & Threat Detection)**.

	![Volet de navigation][1]

3. Dans le panneau de configuration de l'audit, activez **ON**.

4. Sélectionnez **Détails du stockage (Storage Details)** pour ouvrir le volet de stockage de journaux d'audit. Sélectionnez le compte de stockage Azure dans lequel les journaux seront enregistrés, ainsi que la période de rétention. **Conseil :** utilisez le même compte de stockage pour toutes les bases de données auditées afin de profiter au mieux des modèles de rapport d'audit.

	![Volet de navigation][2]

5. Cliquez sur **Événements audités (Audited Events)** pour personnaliser les événements à auditer. Dans le panneau **Enregistrement par événement (Logging by event)**, cliquez sur **Succès (Success)** et **Failure (échec)** pour journaliser tous les événements, ou choisissez des catégories d'événements individuelles.
...
Un modèle de rapport préconfiguré est disponible sous forme de [feuille de calcul Excel téléchargeable](http://go.microsoft.com/fwlink/?LinkId=403540) afin de vous aider à analyser rapidement les données de journal. Pour utiliser le modèle sur vos journaux d'audit, il vous faut Excel 2013 ou une version ultérieure et Power Query, téléchargeable [ici](http://www.microsoft.com/download/details.aspx?id=39379).
...
1. Lancez le [portail Azure classique](https://manage.windowsazure.com/) à l'adresse https://manage.windowsazure.com/.
...
La description fournie dans cette section fait référence aux captures d'écran précédentes. Le [portail Azure en version préliminaire](https://portal.azure.com) ou la [version classique du portail Azure](https://manage.windowsazure.com/) peuvent être utilisés.
...
1. Dans le panneau de configuration de l'audit, faites passer **la clé d'accès de stockage** de *Primaire* à *Secondaire*, puis cliquez sur **ENREGISTRER**.
