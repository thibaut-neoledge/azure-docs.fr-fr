<properties
	pageTitle="Base de données SQL (SaaS) comparée à SQL Server dans le cloud | Microsoft Azure"
	description="Découvrez l’option SQL Server cloud qui convient à son application : base de données SQL Azure (SaaS) ou SQL Server dans le cloud dans Azure Virtual Machine."
	services="sql-database, virtual-machines"
	keywords="Cloud SQL Server, SQL Server dans le cloud, base de données SaaS, cloud SQL Server"
	documentationCenter=""
	authors="jeffgoll"
	manager="jeffreyg"
	editor="cjgronlund"/>
...
# Choisissez une option de SQL Server cloud : base de données SQL Azure (SaaS) ou Azure SQL Server dans le cloud sur des machines virtuelles

Microsoft Azure propose deux options pour SQL Server dans le cloud :

* [Base de données SQL Azure](https://azure.microsoft.com/services/sql-database/) : base de données SQL native dans le cloud, ou en mode logiciel en tant que service (software-as-a-service - SaaS).
* [SQL Server sur des machines virtuelles Azure](https://azure.microsoft.com/services/virtual-machines/sql-server/) : SQL Server installé et hébergé dans le cloud, infrastructure en tant que service (IaaS).

Découvrez comment chaque option s’adapte à la plateforme de données Microsoft. Obtenez ensuite l’aide correspondant à l’option SQL Server sur le cloud pour les besoins de votre entreprise. Que vous privilégiiez les économies ou une administration minimale, cet article peut vous aider à déterminer l’approche adaptée en fonction des exigences clients essentielles.
...
L'une des premières choses à comprendre dans toute discussion comparant les bases de données Azure et SQL Server locales est que vous pouvez tout utiliser. La plateforme de données de Microsoft utilise la technologie SQL Server et la rend disponible sur les machines physiques locales, les environnements de cloud privé, les environnements de cloud privé hébergé par un tiers et le cloud public. Cela vous permet de répondre à des besoins spécifiques et différents via une combinaison de déploiements locaux et hébergés dans le cloud, tout en utilisant le même ensemble de produits serveur, d'outils de développement et de savoir-faire dans ces environnements.
   ...
   ![Options SQL Server Cloud : SQL server sur IaaS ou base de données SQL SaaS dans le cloud.][1]
...
##<a name="business"></a>Motivations commerciales pour choisir entre Azure SQL Database ou SQL Server dans les machines virtuelles Azure
