<properties 
	pageTitle="Pool de bases de données élastique Azure SQL (version préliminaire)" 
...
# Pools élastiques de bases de données SQL (version préliminaire)

Si vous êtes un développeur SaaS chargé d'administrer des dizaines, des centaines, voire des milliers de bases de données, un pool élastique simplifie la création, la maintenance et la gestion des performances dans ces bases de données avec un budget contrôlé.
...
Microsoft a créé des pools élastiques spécifiquement pour vous aider à résoudre ce problème.

> [AZURE.VIDEO elastic-databases-helps-saas-developers-tame-explosive-growth]

Un pool de bases de données élastique est un ensemble de ressources disponibles partagé par les bases de données élastiques dans le pool. Vous pouvez ajouter des bases de données au pool ou les en supprimer à tout moment. Ces bases de données élastiques partagent les ressources (exprimées en unités de débit de base de données élastique ou eDTU) et la capacité de stockage du pool, mais chaque base de données élastique utilise uniquement les ressources dont elle a besoin quand elle en a besoin, libérant ainsi les ressources pour d'autres bases de données élastiques si nécessaire. Au lieu surprovisionner des bases de données individuelles et de payer pour des ressources inutilisées, vous allouez globalement à un coût prévisible les ressources du pool élastique. Ceci permet de répartir le coût pour adopter un modèle commercial compétitif et d'adapter la performance de chaque base de données élastique.
...
## Gérer en toute facilité un grand nombre de bases de données grâce à des outils élastiques

En plus de rendre plus efficace l'utilisation des ressources et d'offrir une meilleure  prévisibilité des performances, les pools élastiques facilitent également le développement d'applications SaaS grâce à des outils qui simplifient la création et la gestion de votre couche de données. L'exécution de tâches de maintenance et de modifications dans un large ensemble de bases de données, qui était jusqu'ici un processus complexe et long, n'est plus aujourd'hui qu'une question d'exécution de scripts dans des tâches élastiques. La possibilité de créer et d'exécuter des tâches élastiques élimine en grande partie la lourde charge de travail associée à l'administration de centaines, voire de milliers, de bases de données. Pour en savoir plus sur l'exécution de tâches élastiques, qui permet l'exécution de scripts T-SQL dans toutes les bases de données élastiques dans un pool, consultez [Vue d'ensemble des tâches de base de données élastique](sql-database-elastic-jobs-overview.md).
...
## Fonctionnalités de continuité des activités pour les bases de données élastiques

Dans la version d'évaluation actuelle, les bases de données élastiques (dans la couche standard élastique) prennent en charge la plupart des fonctionnalités disponibles pour les bases de données de niveau Standard.

### Sauvegarde et restauration de bases de données (Limite de restauration dans le temps)

Les bases de données élastiques sont sauvegardées automatiquement par le système et la stratégie de rétention de la sauvegarde est la même que pour les bases de données de niveau Standard. Dans la version préliminaire, les bases de données élastiques dans un pool sont restaurées en tant que nouvelles bases de données élastiques dans le même pool. Les bases de données élastiques supprimées seront toujours restaurées comme bases de données autonomes en dehors du pool en tant que bases de données Standard S0. Vous pouvez effectuer des opérations de restauration de bases de données à l'aide du portail Azure ou par programmation en utilisant l'API REST. La prise en charge des applets de commande PowerShell sera bientôt disponible.
...
Les bases de données pour lesquelles la géo-réplication est déjà activée peuvent être déplacées vers et depuis un pool élastique, et la réplication  fonctionne normalement. Actuellement, dans la version préliminaire, vous ne pouvez pas activer la géo-réplication sur une base de données élastique qui se trouve déjà dans un pool.
