...
- Un fichier .bacpac de la base de données à importer. Le fichier BACPAC doit se trouver dans un conteneur d’objets blob de [Compte Azure Storage (classique)](../storage/storage-create-storage-account.md).
...
Le nom du serveur doit être celui d’un serveur qui existe dans l’abonnement sélectionné à l’étape précédente. Il doit s’agir du serveur dans lequel vous voulez créer la base de données. Notez que l’importation directe d’une base de données dans un pool élastique n’est pas prise en charge. Mais vous pouvez d’abord l’importer dans une base de données unique, puis déplacer cette dernière vers le pool.
...
L’exécution immédiatement après la demande généralement retourne **État : Pending** ou **État : Running**. Exécutez cette opération plusieurs fois jusqu'à ce que le résultat affiche **État : Completed**.
