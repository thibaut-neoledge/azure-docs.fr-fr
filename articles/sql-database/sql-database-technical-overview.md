...
Microsoft Azure SQL Database est une solution DBaaS entièrement gérée et relationnelle du cloud Microsoft (« Azure »). Avec cette offre de plateforme en tant que service, Microsoft gère toutes les applications de correctifs et mises à jour de la base de code SQL en toute transparence et élimine toute la gestion de l’infrastructure sous-jacente. SQL Database partage sa base de code avec le [moteur de base de données Microsoft SQL Server](https://docs.microsoft.com/sql/sql-server/sql-server-technical-documentation). 

Microsoft gère actuellement des millions de bases de données de production exécutant une large gamme d’applications et de charges de travail (des données transactionnelles simples aux applications critiques les plus consommatrices de données nécessitant un traitement de données avancé à une échelle mondiale). Le service SQL Database est actuellement présent dans 38 centres de données du monde entier, et de plus en plus de centres de données passent en ligne régulièrement.  

SQL Database est une base de données relationnelle à usage général qui prend en charge des structures telles que les données relationnelles, JSON, les données spatiales et XML. Cette solution offre des [performances évolutives dynamiques](sql-database-service-tiers.md) et fournit des options telles que les [index columnstore](https://docs.microsoft.com/sql/relational-databases/indexes/columnstore-indexes-overview) pour la génération de rapport et l’analyse extrême, et [OLTP en mémoire](sql-database-in-memory.md) pour le traitement transactionnel extrême. 
