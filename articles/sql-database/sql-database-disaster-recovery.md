...
Vous devez vous assurer que tous les identifiants de connexion utilisés par votre application existent sur le serveur qui héberge votre base de données restaurée. Pour plus d’informations, voir Gestion de la sécurité lors de la récupération d’urgence. Pour plus d’informations, voir [Configuration de la sécurité de la géo-réplication](sql-database-geo-replication-security-config.md).

>[AZURE.NOTE] Si vous utilisez l’option de géo-restauration pour la récupération après panne, vous devez configurer les règles de pare-feu du serveur et les identifiants de connexion au cours du test de récupération d’urgence afin de vous assurer que le serveur principal est toujours disponible pour la récupération de sa configuration. Comme la géo-restauration utilise les sauvegardes de base de données, il se peut que la configuration au niveau du serveur ne soit pas disponible pendant la panne. Après le test, vous pouvez supprimer les bases de données restaurées, mais vous devez faire en sorte que le serveur et sa configuration soient prêts pour le processus de récupération. Pour plus d’informations sur les tests de récupération d’urgence, voir [Exécution de tests de récupération d’urgence](sql-database-disaster-recovery-drills.md).
