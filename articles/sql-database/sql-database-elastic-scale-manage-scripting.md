...
* [Pas à pas : prise en main de la NOUVELLE fonctionnalité en version préliminaire de Microsoft Azure Automation](http://blogs.technet.com/b/keithmayer/archive/2014/04/04/step-by-step-getting-started-with-windows-azure-automation.aspx) 
...
[Inscrivez-vous](http://azure.microsoft.com/services/preview/) et [familiarisez-vous](../automation-create-runbook-from-samples.md) avec le service en version préliminaire Microsoft Azure Automation.
...
6. Sous la forme d’une opération unique, testez le runbook **SetupShardedEnvironment.ps1** pour configurer l’environnement partitionné. 
...
2. Extrayez tout le contenu.
    ...
    ![Extraire tout][1]
3. Obtenez la DLL du client de l’infrastructure élastique (c’est-à-dire Microsoft.Azure.SqlDatabase.ElasticScale.Client.dll) et copiez les fichiers suivants dans votre dossier local « ShardElasticityModule » téléchargé à l’étape 1. Cela peut se faire de deux manières : 1) téléchargez le DLL via le package NuGet de l’infrastructure élastique [link] ou 2) à partir de votre projet Starter Kit de l’infrastructure élastique [link] (doit être généré), accédez à \\bin\\Debug\\ pour obtenir le DLL.
...
1. Cliquez sur **ÉLÉMENTS MULTIMÉDIAS** dans le ruban en haut de l’écran.
...
2. Sous le compte **ShardElasticityExamples**, cliquez sur **ÉLÉMENTS MULTIMÉDIAS** dans le ruban.
3. Cliquez sur **AJOUTER UN PARAMÈTRE** au bas de l’écran.  
4. Cliquez sur **AJOUTER DES INFORMATIONS D’IDENTIFICATION**. 

    ![Ajouter des informations d’identification][9]
...
5. Cliquez sur la flèche dans le coin inférieur droit de la zone. 
    ...
    Remarque : pour utiliser les Runbooks sans modification, utilisez les noms de variables textuels, comme indiqué dans les instructions. Les noms de variables sont référencés par les Runbook. 
...
4. Répétez les étapes 2 et 3 pour les trois Runbooks PowerShell restants (**ProvisionByDate.ps1**, **ProvisionBySize.ps1** et **ReduceServiceTier.ps1**). 
...
4. Cliquez sur **ENREGISTRER**, puis sur **TESTER**.5. Répétez cette opération pour le Runbook **ReduceServiceTier**. 

    Notez que, comme **ProvisionBySize** et **ProvisionByDate** approvisionnent de nouvelles partitions (en utilisant des algorithmes différents), il n’est pas nécessaire d’exécuter **ProvisionByDate** à ce stade.
