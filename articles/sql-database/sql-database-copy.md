---
title: Copie d’une base de données SQL Azure | Microsoft Docs
...
Vous pouvez utiliser les [sauvegardes automatisées de base de données SQL](sql-database-automated-backups.md) Azure pour créer une copie de votre base de données SQL. L’opération de copie effectue une copie de la fin du journal des transactions, puis utilise les sauvegardes complètes, différentielles et de journal des transactions qui font partie des sauvegardes automatisées pour créer une copie cohérente au niveau transactionnel avec la base de données source au moment de la sauvegarde finale du journal des transactions. 
