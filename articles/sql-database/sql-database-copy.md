...
Une fois la copie réussie et avant que les autres utilisateurs ne soient remappés, seule la connexion qui a initié la copie, le propriétaire de la base de données (DBO), peuvent se connecter à la nouvelle base de données. Pour résoudre les connexions à l’issue de l’opération de copie, consultez [Résoudre les connexions](sql-database-copy.md#resolve-logins.md)

## <a name="database-copy-using-the-azure-portal"></a>Copie de base de données à l’aide du portail Azure

Pour copier une base de données à l’aide du portail Azure, ouvrez la page de votre base de données, puis cliquez sur **Copier** sur la barre d’outils. 

   ![Copie de base de données](./media/sql-database-copy/database-copy.png)

## <a name="database-copy-using-powershell"></a>Copie de base de données à l’aide de PowerShell

Pour copier une base de données avec PowerShell, utilisez l’applet de commande [New-AzureRmSqlDatabaseCopy](https://docs.microsoft.com/powershell/module/azurerm.sql/new-azurermsqldatabasecopy). 
...
## <a name="database-copy-using-transact-sql"></a>Copie de base de données à l’aide de Transact-SQL
...
Consultez également la page [Gestion de la sécurité de la base de données SQL Azure après la récupération d’urgence](sql-database-geo-replication-security-config.md) pour en savoir plus sur la gestion des utilisateurs et des connexions lors de la copie d’une base de données vers un autre serveur logique.
