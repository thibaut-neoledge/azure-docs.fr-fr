...
SQL Database ne prend en charge que l’authentification SQL Server. L’authentification Windows (sécurité intégrée) n’est pas prise en charge. Les utilisateurs doivent fournir leurs informations d’identification (nom d’utilisateur et mot de passe) chaque fois qu’ils se connectent à la base de données SQL. Pour plus d’informations sur l’authentification SQL Server, consultez [Choix d’un mode d’authentification](https://msdn.microsoft.com/library/ms144284.aspx) dans la documentation en ligne de SQL Server.

[SQL Database V12](sql-database-v12-whats-new.md) permet aux utilisateurs de s’authentifier dans la base de données à l’aide des utilisateurs déclarés dans celle-ci. Pour plus d’informations, consultez [Utilisateurs de base de données à relation contenant-contenu - Rendre votre base de données portable](https://msdn.microsoft.com/library/ff929188.aspx), [CREATE USER (Transact-SQL)](https://technet.microsoft.com/library/ms173463.aspx) et [Bases de données à relation contenant-contenu](https://technet.microsoft.com/library/ff929071.aspx).
...
- Lors de l’exécution des instructions ``CREATE/ALTER/DROP LOGIN`` et ``CREATE/ALTER/DROP DATABASE`` dans une application ADO.NET, l’utilisation de commandes paramétrées est interdite. Pour plus d’informations, consultez [Commandes et paramètres](https://msdn.microsoft.com/library/ms254953.aspx).
...
Pour plus d’informations sur les connexions et les utilisateurs, consultez [Gestion des bases de données et des connexions dans Azure SQL Database](sql-database-manage-logins.md).
