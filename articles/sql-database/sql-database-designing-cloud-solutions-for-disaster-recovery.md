<properties
   pageTitle="Solutions de récupération d’urgence cloud - géo-réplication de SQL Database | Microsoft Azure"
...
# Concevoir une application pour la récupération d’urgence cloud à l’aide de la géo-réplication dans SQL Database

Découvrez comment utiliser la géo-réplication dans SQL Database pour concevoir des applications en vue d’inclure une solution de récupération d’urgence cloud. Pour la planification de la continuité des activités, vous devez prendre en compte la topologie de déploiement d’applications, le contrat de niveau de service que vous ciblez, la latence du trafic et les coûts. Dans cet article, nous allons examiner les modèles d’application courants et présenter les avantages et les inconvénients de chaque option.
...
Votre stratégie de récupération d’urgence cloud spécifique peut combiner ou étendre ces modèles de conception afin de mieux répondre aux besoins de votre application. Comme mentionné précédemment, la stratégie que vous choisissez sera basée sur le contrat de niveau de service que vous souhaitez proposer à vos clients et sur la topologie de déploiement d’applications. Pour vous aider à prendre votre décision, le tableau ci-dessous compare les choix en fonction de la perte de données estimée ou de l’objectif de point de récupération (RPO) et du temps de récupération estimé (ERT).
