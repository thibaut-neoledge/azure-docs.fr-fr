<properties 
   pageTitle="Conception de solutions cloud pour la récupération d’urgence à l’aide de la géo-réplication de base de données SQL"
...
# Conception d’applications cloud pour la continuité d’activité à l’aide de la géo-réplication

Il existe plusieurs méthodes selon lesquelles vous pouvez tirer parti de la géo-réplication dans une base de données SQL lorsque vous concevez votre application pour la continuité d’activité. Les facteurs qui guident votre conception incluent la topologie de déploiement d’applications, le contrat de niveau de service que vous ciblez, la latence du trafic et les coûts. Dans cet article, nous allons examiner les modèles d’application courants et présenter les avantages et les inconvénients de chaque option.

## Modèle de conception 1 : Déploiement actif / passif pour la récupération d’urgence avec la base de données colocalisée
...
> [AZURE.NOTE]Si vous utilisez la [géo-réplication active](https://msdn.microsoft.com/library/azure/dn741339.aspx), les deux applications de surveillance peuvent être actives et vous pouvez sonder les bases de données primaire et secondaire. Les bases de données primaire et secondaire peuvent servir à détecter une défaillance dans la région secondaire et à alerter lorsque l’application n’est pas protégée.

Le diagramme suivant illustre cette configuration avant une panne.

![La figure 1](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern1-1.png)
...
2. Appeler la base de données secondaire pour [initier le basculement de la base de données](https://msdn.microsoft.com/library/azure/dn509573.aspx). 
...
+ La chaîne de connexion SQL est définie lors du déploiement de l’application dans chaque région et ne change pas après le basculement.
...
> [AZURE.NOTE]Étant donné que ce modèle nécessite un accès en lecture seule sur la région secondaire, il nécessite la [géo-réplication active](https://msdn.microsoft.com/library/azure/dn741339.aspx).
...
+ Il est nécessaire de basculer dynamiquement vers l’application lorsqu’elle est connectée à une base de données en lecture seule.
...
Votre stratégie de récupération d’urgence spécifique peut combiner ou étendre ces modèles afin de mieux répondre aux besoins de votre application. Comme mentionné précédemment, la stratégie que vous choisissez sera basée sur le contrat de niveau de service que vous souhaitez proposer à vos clients et sur la topologie de déploiement d’applications. Pour vous aider à prendre votre décision, le tableau ci-dessous compare les choix en fonction de la perte de données estimée ou de l’objectif de point de récupération (RPO) et du temps de récupération estimé (ERT).
