...
Après avoir créé le groupe de ressources, vous pouvez voir la mention suivante : **ProvisioningState : Opération réussie**.


### Créer un serveur

Les bases de données SQL sont créées dans les serveurs du service Base de données SQL Azure. Exécutez **New-AzureRMSqlServer** pour créer un serveur. Remplacer *ServerName* par le nom de votre serveur. Ce nom doit être unique pour tous les serveurs du service Base de données SQL Azure. Vous obtiendrez une erreur si le nom est déjà utilisé pour un autre serveur. Il est également à noter que l'exécution de cette commande peut prendre plusieurs minutes. Vous pouvez modifier la commande à utiliser n’importe quel emplacement valide de votre choix, mais vous devez utiliser le même emplacement que celui utilisé pour le groupe de ressources créé à l’étape précédente.
...
Voici un script PowerShell permettant de créer une base de données SQL :
