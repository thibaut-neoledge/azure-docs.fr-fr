...
Pour créer un enregistrement DNS, utilisez la commande `az network dns record-set <recordtype> add-record`. (où recordtype désigne le type d’enregistrement, par exemple A, SRV, TXT, etc.). Pour obtenir de l’aide, reportez-vous à `az network dns record-set --help`.
...
Des jeux d’enregistrements sont créés à l’aide de la commande `az network dns record-set create`. Pour obtenir de l’aide, consultez l’article `az network dns record-set create --help`.
...
Pour récupérer un jeu d’enregistrements existant, utilisez `az network dns record-set show`. Pour obtenir de l’aide, consultez l’article `az network dns record-set show --help`.
...
Vous pouvez utiliser `az network dns record-set add-record` à la fois pour créer un enregistrement dans un nouveau jeu d’enregistrements ou pour ajouter un enregistrement à un jeu d’enregistrements existant.
...
Pour supprimer un enregistrement DNS dans un jeu d’enregistrements existant, utilisez `az network dns record-set ? remove-record` (où ? désigne le type d’enregistrement). Pour obtenir de l’aide, consultez l’article `az network dns record-set -h`.
...
Pour modifier un enregistrement CNAME, utilisez `az network dns record-set update` afin d’ajouter la nouvelle valeur de l’enregistrement. Contrairement aux autres types d’enregistrements, un jeu d’enregistrements CNAME ne peut contenir qu’un seul enregistrement.
...
Utilisez `az network dns record-set soa update` pour modifier l’enregistrement SOA pour une zone DNS donnée. Pour obtenir de l’aide, consultez l’article `az network dns record soa update --help`.
...
Vous pouvez utiliser des [métadonnées de jeu d’enregistrements](dns-zones-records.md#tags-and-metadata) pour associer les données spécifiques de l’application à chaque jeu d’enregistrements, comme paires clé-valeur. Pour modifier les métadonnées d’un jeu d’enregistrements, utilisez `az network dns record-set update`. Pour obtenir de l’aide, consultez l’article `az network dns record-set update --help`.
