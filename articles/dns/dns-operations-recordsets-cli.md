...
Des jeux d’enregistrements sont créés à l’aide de la commande `azure network record-set create`. Vous devez indiquer le nom du jeu d’enregistrements, la zone, la durée de vie (TTL) et le type d'enregistrement.
...
Vous allez utiliser `azure network dns-record-set delete-record` pour supprimer une valeur existante à partir d’un jeu d’enregistrements :
...
Des enregistrements peuvent être supprimés d’un jeu d’enregistrements à l’aide de `azure network dns-record-set delete-record`.tez que l’enregistrement qui est supprimé doit correspondre précisément avec un enregistrement existant, sur tous les paramètres.
