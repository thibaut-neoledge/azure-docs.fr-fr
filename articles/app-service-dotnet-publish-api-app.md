...
Cliquez sur l'onglet **Profil** et sur **Applications API Microsoft Azure (version préliminaire)**. 
...
Cliquez sur le bouton **Parcourir** sur la barre latérale et sélectionnez **Applications API**.
...
Une fois le processus de publication terminé, revenez au portail et fermez, puis rouvrez le panneau **Définition d'API**. Vous verrez le nouveau point de terminaison d'API que vous venez de créer et de déployer directement dans votre abonnement Azure.
