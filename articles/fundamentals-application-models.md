<properties umbracoNaviHide="0" pageTitle="Modèle d'application" metaKeywords="Azure, Azure, modèle d'application, modèle d'application Azure, modèle de développement, modèle de développement Azure, service hébergé, service hébergé Azure, rôle web, rôle de travail" description="Découvrez le modèle d'application de service hébergé Azure. Découvrez les concepts clés, les considérations en matière de conception, la définition et la configuration de votre application, et la mise à l'échelle." urlDisplayName="Application Model" headerExpose="" footerExpose="" disqusComments="1" title="Azure Fundamentals - Application Models" authors="timlt" manager="timlt" />
...
2. Utilisez les disques durs virtuels fournis par Microsoft et ses partenaires dans la galerie des machines virtuelles Azure ou sur le site Web open source de Microsoft [VMDepot](http://vmdepot.msopentech.com/). 
...
- **Machines virtuelles pour le développement et les tests.** Les groupes de développement nécessitent généralement des machines virtuelles avec des configurations spécifiques pour créer des applications. Les machines virtuelles Azure fournissent un moyen simple et économique de créer ces machines virtuelles, de les utiliser, puis de les supprimer lorsqu'elles ne sont plus nécessaires.
- **Exécution d'applications dans le cloud.** Pour certaines applications, l'exécution sur le cloud public se justifie par des raisons économiques. Pensez à une application avec des pics de demande importants, par exemple. Il est toujours possible d'acheter un nombre suffisant de machines pour que votre centre de données exécute cette application, mais la plupart de ces machines risquent de demeurer inutilisées la plupart du temps. L'exécution de cette application sur Azure vous permet de ne payer les machines virtuelles supplémentaires que lorsque vous avez besoin, en les arrêtant quand un pic de demande a pris fin. Ou bien, supposons que vous soyez une start-up qui a besoin de ressources informatiques sur demande, rapidement et sans engagement. Encore une fois, Azure peut être le bon choix.
- **Étendre votre centre de données dans le cloud public.** Avec Azure Virtual Network, votre organisation peut créer un réseau virtuel (VNET) qui permet qu'un groupe de machines virtuelles Azure apparaisse comme faisant partie de votre propre réseau local. Cela permet d'exécuter des applications telles que SharePoint ou autres sur Azure, approche qui peut être plus facile à déployer et/ou moins coûteuse que l'exécution dans votre propre centre de données.
- **Récupération d'urgence.** Au lieu de payer continuellement pour un centre de données de sauvegarde qui est rarement utilisé, la récupération d'urgence basée sur IaaS vous permet de ne payer les ressources informatiques que lorsque vous en avez réellement besoin. Par exemple, si votre centre de données principal tombe en panne, vous pouvez créer des machines virtuelles sous Windows Azure pour exécuter les applications essentielles, puis les arrêter lorsqu'elles ne sont plus nécessaires.
...
Comme le montrent ces exemples, vous pouvez utiliser Azure Virtual Machines pour créer et exécuter de nouvelles applications dans le cloud, pour exécuter des applications existantes ou pour d'autres usages. Quelle que soit l'option choisie, l'objectif technologique est le même : fournir une base générale pour l'informatique de cloud public.



<h2><a id="WebSites"></a>Sites Web</h2>
...
Azure Virtual Machines permet d'effectuer toutes ces opérations. Mais la création et la gestion de machines virtuelles brutes exigent certaines compétences et requièrent des efforts. Si vous avez besoin d'implémenter une site web ou une application web, il existe une solution plus simple (et moins chère) : l'approche communément appelée plateforme PaaS (Platform as a Service). Comme le montre la figure 5, Azure fournit PaaS avec les sites Web.
...
Comme la [figure 5](#Fig5) l'illustre, vous pouvez publier du code et autre contenu web dans les sites web de différentes manières. Vous pouvez utiliser FTP, FTPS ou la technologie WebDeploy de Microsoft. Les sites Web prennent également en charge la publication de code provenant des systèmes de contrôle de code source, y compris Git, GitHub, CodePlex, BitBucket, Dropbox, Mercurial, Team Foundation Server et Team Foundation Service basé dans le cloud.


<h2><a id="CloudServices"></a>Services cloud</h2>

Azure Virtual Machines fournit une infrastructure IaaS, tandis que les sites Web Azure fournissent un hébergement Web. La troisième option, Services cloud, fournit la plateforme PaaS (Platform as a Service). Cette technologie est conçue pour prendre en charge des applications évolutives, fiables et dont l'exploitation est peu coûteuse. Elle vise également à libérer les développeurs des soucis liés à la gestion de la plateforme qu'ils utilisent, pour leur permettre de se consacrer entièrement à leurs applications. [La figure 6](#Fig6) illustre le concept.
...
À l'instar des autres options de calcul Azure, Azure Cloud Services s'appuie sur des machines virtuelles. La technologie fournit deux options de machine virtuelle légèrement différentes : les instances de *rôles web* exécutent une variante de Windows Server avec IIS, tandis que les instances de *rôles de travail* exécutent la même variante de Windows Server sans IIS. Une application Cloud Services repose sur une combinaison de ces deux options. 
...
<h2><a id="WhatShouldIUse"></a>Que dois-je utiliser ? Faire un choix</h2>
...
Parfois, aucun modèle d'exécution ne convient à lui seul. Dans de telles situations, il est parfaitement légal de combiner des options. Supposons, par exemple, que vous créez une application dans laquelle vous souhaitez bénéficier des avantages en matière de gestion offerts par les rôles Web Cloud Services, tout en ayant besoin également d'utiliser SQL Server standard pour des raisons de compatibilité ou de performances. Dans ce cas, la meilleure solution consiste à combiner les modèles d'exécution, comme la [Figure 7](#Fig7) l'illustre.
