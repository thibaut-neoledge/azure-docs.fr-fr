<properties pageTitle="Utilisation de la bibliothèque cliente Android pour Mobile Services" description="Découvrez comment utiliser un client Android pour Azure Mobile Services." services="mobile-services" documentationCenter="android" authors="RickSaling" manager="dwrede" editor=""/>
...
Ce guide décrit le déroulement de scénarios courants dans le cadre de l'utilisation du client Android pour Azure Mobile Services.  Les scénarios traités incluent l'interrogation des données, l'insertion, la mise à jour et la suppression des données, l'authentification des utilisateurs, la gestion des erreurs et la personnalisation du client. Si vous débutez avec Mobile Services, suivez le didacticiel [Démarrage rapide de Mobile Services][Prise en main de Mobile Services]. Ces didacticiels de démarrage rapide vous aideront à configurer et à créer votre premier service mobile.
...
- [Procédure : Interrogation des données à partir d'un service mobile]
	...
	- [Procédure : Concaténation de méthodes de requête]
- [Procédure : Insertion de données dans un service mobile]
- [Procédure : Mise à jour des données d'un service mobile]
- [Procédure : Suppression des données d'un service mobile]
- [Procédure : Recherche d'un élément spécifique]
- [Procédure : Utilisation de données non typées]
- [Procédure : Liaison des données dans l'interface utilisateur]
	- [Procédure : Définition de la mise en page]
	- [Procédure : Définition de l'adaptateur]
	- [Procédure : Utilisation de l'adaptateur]
...
Dans le code ci-dessus, remplacez `MobileServiceUrl` et `AppKey` par l'URL et la clé d'application de votre service mobile, dans cet ordre. Tous deux sont disponibles dans le portail de gestion Azure. Pour y accéder, sélectionnez votre service mobile, puis cliquez sur *Dashboard*.

<h2><a name="instantiating"></a>Procédure : Création d'une référence de table</h2>

Le moyen le plus simple pour interroger ou modifier des données dans le service mobile consiste à utiliser le *typed programming model*, puisque Java est un langage fortement typé (nous aborderons plus tard le modèle *untyped*). Ce modèle fournit une sérialisation et une désérialisation transparentes de JSON en utilisant la bibliothèque <a href=" http://go.microsoft.com/fwlink/p/?LinkId=290801" target="_blank">gson</a> lors de l'envoi des données entre le client et le service mobile : le développeur n'a rien à faire, tout est géré par l'infrastructure.

La première chose à faire pour interroger ou modifier des données consiste à créer un objet [MobileServiceTable](http://go.microsoft.com/fwlink/p/?LinkId=296835) en appelant la méthode **getTable** sur le [**MobileServiceClient**](http://dl.windowsazure.com/androiddocs/com/microsoft/windowsazure/mobileservices/MobileServiceClient.html).  Nous allons examiner deux surcharges de cette méthode :
...
Dans le code suivant, *mClient* est une référence à votre client de service mobile.
...
Il existe de nombreux types d'objet de rappel, et celui que vous utilisez dépend de l'opération effectuée (interrogation, modification ou suppression). Les paramètres de la méthode *onCompleted* varient en fonction de l'objet de rappel dont elle fait partie.
...
Le code suivant retourne tous les éléments de la table *ToDoItem* dont le champ *complete* est égal à *false*. *mToDoTable* est la référence à la table de service mobile que vous avez créée précédemment. 
...
Le filtre commence par un appel de méthode [**where**](http://go.microsoft.com/fwlink/p/?LinkId=296867) sur la référence de table. Il est suivi par un appel de méthode [**field**](http://go.microsoft.com/fwlink/p/?LinkId=296869), suivi d'un appel de méthode qui spécifie le prédicat logique. Les méthodes de prédicat possibles incluent [**eq**](http://go.microsoft.com/fwlink/p/?LinkId=298461), [**ne**](http://go.microsoft.com/fwlink/p/?LinkId=298462), [**gt**](http://go.microsoft.com/fwlink/p/?LinkId=298463), [**ge**](http://go.microsoft.com/fwlink/p/?LinkId=298464), [**lt**](http://go.microsoft.com/fwlink/p/?LinkId=298465), [**le**](http://go.microsoft.com/fwlink/p/?LinkId=298466) etc.
...
Vous pouvez, par exemple, filtrer les dates. Vous pouvez comparer le champ de date complète, mais vous pouvez également comparer des parties de la date, avec des méthodes telles que [**year**](http://go.microsoft.com/fwlink/p/?LinkId=298467), [**month**](http://go.microsoft.com/fwlink/p/?LinkId=298468), [**day**](http://go.microsoft.com/fwlink/p/?LinkId=298469), [**hour**](http://go.microsoft.com/fwlink/p/?LinkId=298470), [**minute**](http://go.microsoft.com/fwlink/p/?LinkId=298471) and [**second**](http://go.microsoft.com/fwlink/p/?LinkId=298472). L'extrait de code qui suit ajoute un filtre pour les éléments dont *due date* est égale à 2013.

		mToDoTable.where().year("due").eq(2013)

Vous pouvez appliquer un large éventail de filtres complexes sur les champs de type chaîne avec des méthodes telles que [**startsWith**](http://go.microsoft.com/fwlink/p/?LinkId=298473), [**endsWith**](http://go.microsoft.com/fwlink/p/?LinkId=298474), [**concat**](http://go.microsoft.com/fwlink/p/?LinkId=298475), [**subString**](http://go.microsoft.com/fwlink/p/?LinkId=298477), [**indexOf**](http://go.microsoft.com/fwlink/p/?LinkId=298488), [**replace**](http://go.microsoft.com/fwlink/p/?LinkId=298491), [**toLower**](http://go.microsoft.com/fwlink/p/?LinkId=298492), [**toUpper**](http://go.microsoft.com/fwlink/p/?LinkId=298493), [**trim**](http://go.microsoft.com/fwlink/p/?LinkId=298495), and [**length**](http://go.microsoft.com/fwlink/p/?LinkId=298496). L'extrait de code qui suit filtre les lignes de la table dans lesquelles la colonne *text* commence par " PRI0 ".

		mToDoTable.where().startsWith("text", "PRI0")

Les champs numériques permettent également d'appliquer de nombreux filtres plus complexes avec des méthodes telles que [**add**](http://go.microsoft.com/fwlink/p/?LinkId=298497), [**sub**](http://go.microsoft.com/fwlink/p/?LinkId=298499), [**mul**](http://go.microsoft.com/fwlink/p/?LinkId=298500), [**div**](http://go.microsoft.com/fwlink/p/?LinkId=298502), [**mod**](http://go.microsoft.com/fwlink/p/?LinkId=298503), [**floor**](http://go.microsoft.com/fwlink/p/?LinkId=298505), [**ceiling**](http://go.microsoft.com/fwlink/p/?LinkId=298506), and [**round**](http://go.microsoft.com/fwlink/p/?LinkId=298507). L'extrait de code qui suit filtre les lignes de la table dans lesquelles la colonne  *duration* est un nombre pair.

		mToDoTable.where().field("duration").mod(2).eq(0)


Vous pouvez combiner des prédicats avec des méthodes telles que [**and**](http://go.microsoft.com/fwlink/p/?LinkId=298512), [**or**](http://go.microsoft.com/fwlink/p/?LinkId=298514) and [**not**](http://go.microsoft.com/fwlink/p/?LinkId=298515). Cet extrait de code combine deux des exemples ci-dessus.
...
Le code suivant retourne tous les éléments d'une table  *ToDoItems* triés par ordre croissant sur le champ  *text*. *mToDoTable* est la référence à la table de service mobile créée précédemment.
...
Le premier paramètre de la méthode [**orderBy**](http://go.microsoft.com/fwlink/p/?LinkId=298519) est une chaîne égale au nom du champ de tri.
...
Notez que si vous filtrez à l'aide de la méthode ***where***, la méthode ***where*** doit être appelée avant la méthode ***orderBy***.
...
Le code suivant montre comment renvoyer tous les éléments d'une table  *ToDoItems*, mais n'affiche que les champs *complete* et *text*. *mToDoTable* est la référence à la table de service mobile que nous avons créée précédemment.
...
La méthode [**select**](http://go.microsoft.com/fwlink/p/?LinkId=290689) doit suivre les méthodes telles que [**where**](http://go.microsoft.com/fwlink/p/?LinkId=296296) et [**orderBy**](http://go.microsoft.com/fwlink/p/?LinkId=296313), le cas échéant. Elle peut être suivie de méthodes telles que [**top**](http://go.microsoft.com/fwlink/p/?LinkId=298731).

### <a name="chaining"></a>Procédure : Concaténation de méthodes de requête 
...
Cela peut fonctionner parce que les méthodes de requête utilisées renvoient des objets [**MobileServiceQuery&lt;T&gt;**](http://go.microsoft.com/fwlink/p/?LinkId=298551) sur lesquels d'autres méthodes peuvent être appelées. Pour mettre fin à la série de méthodes et exécuter la requête, appelez la méthode [**execute**](http://go.microsoft.com/fwlink/p/?LinkId=298554).

Voici un exemple de code où *mToDoTable* est une référence à la table *ToDoItem* des services mobiles.
...
+ Caractères de contrôle : [0x0000-0x001F] and [0x007F-0x009F]. Pour plus d'informations, consultez la page [Codes de contrôle ASCII C0 et C1].
...
<h2><a name="updating"></a>Procédure : Mise à jour des données d'un service mobile</h2>
...
Notez que dans le cas de *delete*, l'objet de rappel est une méthode [**TableDeleteCallback**](http://go.microsoft.com/fwlink/p/?LinkId=296858) et la méthode **onCompleted** est légèrement différente dans la mesure où aucune ligne de table n'est renvoyée.
...
<h2><a name="lookup"></a>Procédure : Recherche d'un élément spécifique</h2>
Il est possible de rechercher un élément spécifique par son *id*, à l'inverse de la requête, qui génère une suite d'éléments qui répondent à certains critères. Le code qui suit montre comment effectuer cette recherche, avec *id* = "37BBF396-11F0-4B39-85C8-B319C729AF6D".
...
### <a name="json_instance"></a>Procédure : Création d'une instance de table non typée
...
### <a name="json_insert"></a>Procédure : Insertion dans une table non typée

Le code suivant vous explique comment effectuer une insertion. La première étape consiste à créer un objet [**JsonObject**](http://google-gson.googlecode.com/svn/trunk/gson/docs/javadocs/com/google/gson/JsonObject.html), qui fait partie de la bibliothèque <a href=" http://go.microsoft.com/fwlink/p/?LinkId=290801" target="_blank">gson</a>.
...
### <a name="json_delete"></a>Procédure : Suppression dans une table non typée
...
### <a name="json_get"></a>Procédure : Récupération de toutes les lignes d'une table non typée
...
<h2><a name="binding"></a>Procédure : Liaison des données dans l'interface utilisateur</h2>
...
### <a name="layout"></a>Procédure : Définition de la mise en page
...
### <a name="adapter"></a>Procédure : Définition de l'adaptateur
	
Étant donné que la source de données de notre vue est un tableau de *ToDoItem*, nous créons une sous-classe de notre adaptateur à partir d'une classe *ArrayAdapter&lt;ToDoItem&gt;*. Cette sous-classe produit une vue pour chaque *ToDoItem* à l'aide de la *row_list_to_do* mise en page.
...
Vous devez remplacer la méthode *getView* de l'adaptateur. À cet effet, vous pouvez utiliser cet exemple de code : les détails varient en fonction de votre application.
...
Notez que le deuxième paramètre du constructeur ToDoItemAdapter est une référence à la mise en page. L'appel au constructeur est suivi du code ci-dessous, qui obtient tout d'abord une référence à **ListView**, puis appelle ensuite *setAdapter* afin de se configurer lui-même pour utiliser l'adaptateur que nous venons de créer :

		ListView listViewToDo = (ListView) findViewById(R.id.listViewToDo);
		listViewToDo.setAdapter(mAdapter);


### <a name="use-adapter"></a>Procédure : Utilisation de l'adaptateur
...
### <a name="caching"></a>Procédure : Ajout de code d'authentification à votre application
...
### <a name="caching"></a>Procédure : Mise en cache des jetons d'authentification
...
Le développeur peut maintenant utiliser les rappels de sous-classe et ne plus se soucier de la vérification de l'exception, car elle est gérée de façon centralisée (#2) pour toutes les instances du rappel.
...
### <a name="columns"></a>Procédure : Mappage de noms de client et de serveur différents
...
### <a name="table"></a>Procédure : Mappage de noms de tables différents entre client et service mobile
...
### <a name="conversions"></a>Procédure : Automatisation du mappage des noms de colonnes
...
À cet effet, nous nous servons de la bibliothèque <a href=" http://go.microsoft.com/fwlink/p/?LinkId=290801" target="_blank">gson</a> que la bibliothèque cliente Android utilise en arrière-plan pour sérialiser les objets Java en données JSON, qui sont ensuite envoyées à Azure Mobile Services.

Le code suivant utilise la méthode *setFieldNamingStrategy()*, dans laquelle nous définissons une méthode *FieldNamingStrategy()*. Cette méthode indique de supprimer le premier caractère (un " m "), puis de mettre le caractère suivant en minuscule, ce pour tous les noms de champ. Ce code permet aussi d'obtenir une impression lisible du résultat JSON.
...
### <a name="complex"></a>Procédure : Stockage d'un objet ou d'une propriété de tableau dans une table 
...
[Procédure : Concaténation de méthodes de requête]: #chaining
[Procédure : Liaison des données dans l'interface utilisateur]: #binding
[Procédure : Définition de la mise en page]: #layout
[Procédure : Définition de l'adaptateur]: #adapter
[Procédure : Utilisation de l'adaptateur]: #use-adapter
[Procédure : Insertion de données dans un service mobile]: #inserting
[Procédure : Mise à jour des données d'un service mobile]: #updating
[Procédure : Suppression des données d'un service mobile]: #deleting
[Procédure : Recherche d'un élément spécifique]: #lookup
...
[Procédure : Conception de tests]: #tests
...
[Kit de développement logiciel (SDK) Mobile Services]: http://go.microsoft.com/fwlink/p/?linkid=280126
