...
Azure Cosmos DB fournit les fonctionnalités suivantes pour vous permettre d’écrire facilement des applications à l’échelle planétaire. Ces fonctionnalités sont disponibles par l’intermédiaire des [API REST](https://docs.microsoft.com/rest/api/documentdbresourceprovider/) basées sur le fournisseur de ressources d’Azure Cosmos DB, ainsi que dans le portail Azure.

### <a id="RegionalPresence"></a>Omniprésence régionale 
Azure étend constamment sa présence géographique en ajoutant de nouvelles régions en ligne. Azure Cosmos DB est disponible par défaut dans toutes les nouvelles régions Azure. Cela vous permet d’associer une région géographique à votre compte de base de données Azure Cosmos DB dès qu’Azure ouvre une nouvelle région aux entreprises.

**Azure Cosmos DB est disponible par défaut dans toutes les régions Azure**

![Azure Cosmos DB est disponible par défaut dans toutes les régions Azure](./media/documentdb-distribute-data-globally/azure-regions.png)
...
![Compte de base de données Azure Cosmos DB couvrant 25 régions Azure](./media/documentdb-distribute-data-globally/documentdb-spanning-regions.png)
...
![Configuration des priorités de basculement avec Azure Cosmos DB](./media/documentdb-distribute-data-globally/documentdb-failover-priorities.png)
...
Azure Cosmos DB expose [plusieurs niveaux de cohérence bien définis](documentdb-consistency-levels.md) pris en charge par des contrats SLA. Vous pouvez choisir un modèle de cohérence spécifique (à partir de la liste des options disponibles) en fonction des charges de travail/scénarios. 
...
Vous trouverez plus d’informations sur la configuration des préférences de lecture des API [DocumentDB](../cosmos-db/tutorial-global-distribution-documentdb.md), [Graph](../cosmos-db/tutorial-global-distribution-graph.md), [Table](../cosmos-db/tutorial-global-distribution-table.md) et [MongoDB](../cosmos-db/tutorial-global-distribution-mongodb.md) dans leurs articles respectifs.
...
![Débit configuré en toute flexibilité d’Azure Cosmos DB](./media/documentdb-distribute-data-globally/documentdb-elastic-throughput.png)
...
![Mesures du contrat SLA visible par le client d’Azure Cosmos DB](./media/documentdb-distribute-data-globally/documentdb-customer-slas.png)

## <a id="Next Steps"></a>Étapes suivantes
* Pour mettre en œuvre la réplication mondiale sur votre compte Azure Cosmos DB à l’aide du portail Azure, consultez [Configuration de la distribution mondiale d’Azure Cosmos DB à l’aide de l’API DocumentDB](../cosmos-db/tutorial-global-distribution-documentdb.md).
* Pour savoir comment implémenter des architectures multimaîtres avec Azure Cosmos DB, consultez [Architectures de base de données multimaîtres répliquées de façon globale avec Azure Cosmos DB](documentdb-multi-region-writers.md).
* Pour plus d’informations sur les basculements automatiques et manuels de tâches dans Azure Cosmos DB, consultez [Basculements régionaux automatiques pour la continuité des activités dans Azure Cosmos DB](documentdb-regional-failovers.md).
...
9. [SLA pour Azure Cosmos DB](https://azure.microsoft.com/support/legal/sla/documentdb/v1_1/)
