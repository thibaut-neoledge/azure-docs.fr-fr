---
title: "Connexion d’Azure Cosmos DB à Azure Search à l’aide d’indexeurs | Microsoft Docs"
description: "Cet article vous indique comment utiliser l’indexeur Azure Search avec Azure Cosmos DB en tant que source de données."
services: cosmosdb
documentationcenter: 
author: mimig1
manager: jhubbard
...
---
# <a name="connecting-azure-cosmos-db-with-azure-search-using-indexers"></a>Connexion d’Azure Cosmos DB à Azure Search à l’aide d’indexeurs
Si vous souhaitez améliorer les recherches au niveau de vos données Cosmos DB, utilisez l’indexeur Azure Search pour Cosmos DB. Dans cet article, nous allons vous montrer comment intégrer Azure Search dans Azure Cosmos DB sans avoir à écrire le moindre code pour mettre l’infrastructure d’indexation à jour.

Pour ce faire, vous devez [configurer un compte Azure Search](../search/search-create-service-portal.md) (vous n’avez pas besoin de procéder à la mise à niveau vers la recherche standard), puis appeler l’[API REST Azure Search](https://msdn.microsoft.com/library/azure/dn798935.aspx) afin de créer une **source de données** Cosmos DB et un **indexeur** pour cette dernière.
...
Azure Search prend en charge la création et la gestion de sources de données (dont Cosmos DB) et d’indexeurs qui fonctionnent en s’appuyant sur ces dernières.
...
* **container**:
  ...
  * **name**: obligatoire. Spécifiez l’ID de la collection de l’API DocumentDB à indexer.
...
Félicitations ! Vous venez d’apprendre comment intégrer Azure Search dans Azure Cosmos DB à l’aide de l’indexeur pour Cosmos DB.

* Pour en savoir plus sur Cosmos DB, consultez la [page du service Azure Cosmos DB](https://azure.microsoft.com/services/documentdb/).
