...
> [!NOTE]
> La spécification GeoJSON nécessite que, pour les polygones valides, la dernière paire de coordonnées fournie soit identique à la première, pour créer une forme fermée.
> 
...
Les arguments de polygone dans ST\_WITHIN peuvent contenir un seul cercle, cela signifie que les polygones ne doivent pas contenir de trous.
...
> [!NOTE]
> Si la valeur GeoJSON d'emplacement dans le document est incorrecte ou non valide, elle n’est pas indexée pour les requêtes spatiales. Vous pouvez valider les valeurs d'emplacement à l'aide de ST\_ISVALID et ST\_ISVALIDDETAILED.
> 
