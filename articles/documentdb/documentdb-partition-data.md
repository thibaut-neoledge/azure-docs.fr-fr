...
Le  *range partitioning* *lookup partitioning* et le  *hash partitioning* sont les techniques les plus courantes utilisées pour partitionner des données avec Azure DocumentDB. Généralement, vous désignez un seul nom de propriété JSON dans votre document comme votre clé de partition, par exemple, " timestamp " ou " userID ". Dans certains cas, il peut s'agir plutôt d'une propriété JSON interne ou d'un nom de propriété différent pour chaque type de document distinct.

Examinons ces techniques plus en détail.

## Partitionnement par plage

Dans le partitionnement par plage, l'affectation des partitions est fonction de la présence de la clé de partition dans une certaine plage. Cette méthode est couramment utilisée pour le partitionnement avec des propriétés d' *time stamp* (par exemple, eventTime entre le 1er février 2015 et le 2 février 2015). 
...
- Le **partitionnement par plage** est normalement utilisé dans le contexte des dates, car il offre un mécanisme simple et naturel de vieillissement des partitions par horodatage. Il est également utile lorsque les requêtes sont généralement limitées à une plage de temps, dans la mesure où celle-ci est alignée avec les limites de partitionnement. 
...
- Le **partitionnement par hachage** est utile pour assurer un équilibrage de charge uniforme des demandes afin d'utiliser efficacement le débit et le stockage approvisionnés. L'utilisation d'algorithmes de *consistent hashing* permet de réduire la quantité de données à déplacer lors de l'ajout ou de la suppression d'une partition.
...
### Étapes suivantes
Dans cet article, nous avons introduit certaines techniques courantes pour le partitionnement des données avec DocumentDB et indiqué quand utiliser telle technique ou combinaison de techniques. Commencez avec l'un des [Kits de développement logiciel (SDK) pris en charge](https://msdn.microsoft.com/library/azure/dn781482.aspx) et contactez-nous via les [forums de support MSDN](https://social.msdn.microsoft.com/forums/azure/home?forum=AzureDocumentDB) si vous avez des questions.
