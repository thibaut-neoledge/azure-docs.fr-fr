...
---
# <a name="view-edit-create-and-upload-json-documents"></a>Affichage, modification, création et chargement de documents JSON 
...
## <a name="create-a-json-document"></a>Création d’un document JSON
...
3. Dans le panneau **Document**, tapez ou collez le contenu du document JSON que vous souhaitez créer, puis cliquez sur **Enregistrer** pour valider votre document dans la base de données et la collection spécifiées dans le panneau **Explorateur de documents**.
   ...
   > [!NOTE]
   > Si vous ne fournissez pas de propriété « id », l'Explorateur de documents en ajoute automatiquement une et génère un GUID comme valeur d'ID.
   > 
   > 
...
> [!NOTE]
> La propriété timestamp (_ts) est représentée en interne comme heure d'époque, mais l'Explorateur de documents affiche la valeur dans un format GMT lisible.
> 
> 
...
3. Les paramètres de filtre s’affichent au-dessous de la barre de commandes. Dans les paramètres de filtre, indiquez une clause WHERE et/ou une clause ORDER BY, puis cliquez sur **Filtrer**.
   ...
   > [!IMPORTANT]
   > La fonctionnalité de filtre de l’Explorateur de documents filtre uniquement l’ensemble de documents ***actuellement*** chargé et n’exécute pas de requête sur la collection actuellement sélectionnée.
   > 
   > 
...
3. Cliquez sur le bouton Parcourir pour ouvrir une fenêtre de l’Explorateur de fichiers, sélectionnez un ou plusieurs documents JSON à charger, puis cliquez sur **Ouvrir**.
   ...
   > [!NOTE]
   > L'Explorateur de documents prend actuellement en charge jusqu'à 100 documents JSON par opération de téléchargement.
   > 
   > 
4. Une fois que vous êtes satisfait de votre sélection, cliquez sur le bouton **Télécharger** .  Les documents sont automatiquement ajoutés à la grille de l'Explorateur de documents et les résultats du téléchargement sont affichés à mesure que l'opération progresse. Les échecs d'importation sont signalés pour chaque fichier.
...
5. Une fois l’opération terminée, vous pouvez sélectionner jusqu’à 100 autres documents à charger.

<a id="data-explorer"></a>
## <a name="create-a-document-by-using-data-explorer-preview"></a>Créer un document à l’aide de l’Explorateur de données (version préliminaire)

L’autre méthode pour créer, modifier et interroger des documents dans le portail consiste à utiliser l’Explorateur de données. Pour ouvrir l’Explorateur de données, cliquez sur **Explorateur de données (version préliminaire)** dans la barre de navigation du portail, puis développez le nom de la base de données, développez le nom de la collection, cliquez sur **Documents**, puis sur **Nouveau Document**, comme illustré dans la capture d’écran suivante.
