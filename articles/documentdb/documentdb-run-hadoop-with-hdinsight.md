...
* Un compte DocumentDB, une base de données et une collection de documents. Pour plus d’informations, consultez [Prise en main de DocumentDB][getting-started]. Importez des exemples de données dans votre compte DocumentDB avec [l’outil d’importation de DocumentDB][documentdb-import-data].
* Débit. Les lectures et écritures à partir de HDInsight seront comptabilisées sur vos unités de demande allouées pour vos collections. Pour plus d’informations, consultez [Débit approvisionné, unités de demande et opérations de base de données][documentdb-manage-throughput].
...
Ce didacticiel utilise une action de script à partir du portail Azure pour personnaliser votre cluster HDInsight. Dans ce didacticiel, nous allons utiliser le portail Azure pour créer votre cluster HDInsight. Pour obtenir des instructions sur l’utilisation des applets de commande PowerShell ou du Kit de développement (SDK) .NET HDInsight, consultez l’article [Personnaliser des clusters HDInsight à l’aide d’une action de script][hdinsight-custom-provision].
...
8. Dans le même panneau, spécifiez un **conteneur par défaut** et un **emplacement**. Et cliquez sur **SÉLECTIONNER**.
   ...
   > [!NOTE]
   > Pour de meilleures performances, sélectionnez un emplacement proche de la région de votre compte DocumentDB
   >
   >
...
1. Installez Azure PowerShell. Vous pouvez trouver des instructions [ici][powershell-install-configure].
   ...
   > [!NOTE]
   > Pour les requêtes Hive, vous pouvez également utiliser l'éditeur Hive en ligne de HDInsight. Pour ce faire, connectez-vous au [portail Azure][azure-portal], puis cliquez sur **HDInsight** dans le volet gauche pour afficher la liste de vos clusters HDInsight. Cliquez sur le cluster sur lequel vous souhaitez exécuter des requêtes Hive, puis sur **Console de requête**.
   >
   >
...
2. Nous allons exécuter une tâche MapReduce qui compte le nombre d'occurrences de chaque propriété de document de votre collection DocumentDB. Ajoutez cet extrait de script **après** l’extrait ci-dessus.
   ...
   > [!NOTE]
   > TallyProperties-v01.jar est fourni avec l'installation personnalisée du connecteur Hadoop DocumentDB.
   >
   >
