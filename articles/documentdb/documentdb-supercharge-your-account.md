...
6. De retour dans le panneau **Paramètres**, le **Niveau tarifaire** est défini sur **Standard** et la zone **Débit (unités de requête/s)** est affichée avec une valeur par défaut de 400. Cliquez sur **OK** pour enregistrer vos modifications.
   ...
   > [!NOTE]
   > Définissez le débit entre 400 et 10 000 [unités de requête](documentdb-request-units.md)/seconde (UR/s). Le **Résumé de la tarification** au bas de la page se met à jour automatiquement pour donner une estimation du coût mensuel.
   > 
   > 
