---
title: "Connexion d’Apache Spark à Azure DocumentDB | Microsoft Docs"
description: "Grâce à ce didacticiel, vous en saurez davantage sur le connecteur Spark-Azure DocumentDB, qui vous permet de connecter Apache Spark à Azure DocumentDB pour réaliser des agrégations distribuées et des opérations de sciences des donnés sur un système de base de données Microsoft mutualisé, distribué dans le monde entier et conçu pour le cloud."
...
# <a name="accelerate-real-time-big-data-analytics-with-the-spark-to-documentdb-connector"></a>Accélérer l’analyse en temps réel des données volumineuses grâce au connecteur Spark-DocumentDB

Le connecteur Spark-DocumentDB permet à Azure DocumentDB de jouer le rôle de source d’entrée ou de récepteur de sortie pour les travaux Apache Spark. Connecter [Spark](http://spark.apache.org/) à [DocumentDB](https://azure.microsoft.com/services/documentdb/) accélère votre capacité à résoudre les problèmes liés aux opérations de sciences de données dynamiques, où les données peuvent rapidement être à l’état persistant et interrogées à l’aide de DocumentDB. Le connecteur Spark-DocumentDB tire le meilleur parti des index gérés DocumentDB natifs et active les colonnes pouvant être mises à jour lors de l’exécution d’analyses et de filtres de prédicat sur les données distribuées dans le monde entier en constante évolution, pour les scénarios d’analyse, d’opération de sciences de données et IoT. 

## <a name="download"></a>Télécharger

Commencez en téléchargeant le connecteur Spark-DocumentDB (version préliminaire) depuis le référentiel [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark/) sur GitHub.

## <a name="connector-components"></a>Composants du connecteur

Le connecteur utilise les composants suivants :

* [DocumentDB](http://documentdb.com), un [système de base de données Microsoft mutualisé](documentdb-distribute-data-globally.md), distribué dans le monde entier et conçu pour le cloud. DocumentDB permet à ses utilisateurs d’approvisionner et d’adapter leurs capacités de débit et de stockage partout dans le monde. Le service offre une latence faible garantie au 99e centile, une haute disponibilité garantie à 99,99 %, et [plusieurs modèles de cohérence bien définis](documentdb-consistency-levels.md) aux développeurs.
...
Vous pouvez choisir entre deux approches pour connecter Apache Spark et Azure DocumentDB :
...
Le [Kit de développement logiciel (SDK) pyDocumentDB](https://github.com/Azure/azure-documentdb-python) actuel permet de connecter Spark à DocumentDB comme indiqué dans le schéma suivant :

![Flux de données de Spark vers DocumentDB via pyDocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-pydocumentdb.png)
...
1. La connexion est établie à partir du nœud maître Spark vers le nœud de passerelle DocumentDB via pyDocumentDB.  Notez que l’utilisateur spécifie uniquement des connexions Spark et DocumentDB, le fait qu’il se connecte aux nœuds maître et de passerelle respectifs est transparent pour l’utilisateur.
2. Des requêtes sont envoyées à DocumentDB (via le nœud de passerelle), où elles s’exécutent ensuite sur les partitions de la collection dans les nœuds de données. La réponse à ces requêtes est renvoyée au nœud de passerelle, et ce jeu de résultats est renvoyé au nœud maître Spark.
...
L’opération importante est que la communication entre Spark et DocumentDB est limitée au nœud maître Spark et aux nœuds de passerelle DocumentDB.  Les requêtes sont aussi rapides que la couche de transport entre ces deux nœuds.
...
### <a name="connecting-spark-to-documentdb-via-pydocumentdb"></a>Connexion de Spark à DocumentDB via pyDocumentDB 
À l’inverse du transport des communications pouvant être complexe, l’exécution d’une requête à partir de Spark vers DocumentDB à l’aide de pyDocumentDB est relativement simple.
...
pyDocumentDB est généralement utilisé pour connecter Spark à DocumentDB pour les scénarios dans lesquels :
...
## <a name="spark-to-documentdb-connector"></a>Connecteur Spark-DocumentDB

Le connecteur Spark-DocumentDB utilise le [Kit de développement logiciel (SDK) Azure DocumentDB Java](https://github.com/Azure/azure-documentdb-java) et déplace les données entre les nœuds Worker Spark et DocumentDB comme indiqué dans le schéma suivant :

![Flux de données dans le connecteur Spark-DocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-connector.png)

Le flux de données est le suivant :

1. Une connexion est établie à partir du nœud maître Spark vers le nœud de passerelle DocumentDB pour obtenir le mappage de partitions.  Notez que l’utilisateur spécifie uniquement des connexions Spark et DocumentDB, le fait qu’il se connecte aux nœuds maître et de passerelle respectifs est transparent pour l’utilisateur.
2. Ces informations sont renvoyées au nœud maître Spark.  À ce stade, vous devez être en mesure d’analyser la requête pour déterminer les partitions (et leurs emplacements) auxquelles vous devez accéder dans DocumentDB.
3. Ces informations sont transmises aux nœuds Worker Spark...
4. Cela permet donc aux nœuds Worker Spark de se connecter directement aux partitions DocumentDB pour extraire les données qui sont nécessaires et renvoyer les données aux partitions Spark dans les nœuds Worker Spark.

L’opération importante est que la communication entre Spark et DocumentDB est beaucoup plus rapide, car les données sont déplacées entre les nœuds Worker Spark et les nœuds de données DocumentDB (partitions).

### <a name="building-the-spark-to-documentdb-connector"></a>Création du connecteur Spark-DocumentDB
...
### <a name="connecting-spark-to-documentdb-using-the-connector"></a>Connexion de Spark à DocumentDB à l’aide du connecteur
Alors que le transport des communications est un peu plus complexe, l’exécution d’une requête de Spark vers DocumentDB à l’aide du connecteur est très rapide.
...
### <a name="why-use-the-spark-to-documentdb-connector-implementation"></a>Pourquoi utiliser l’implémentation du connecteur Spark-DocumentDB ?

Le connecteur est généralement utilisé pour connecter Spark à DocumentDB pour les scénarios dans lesquels :
...
* Vous avez une grande quantité de données à transférer entre Apache Spark et DocumentDB.
...
Cette section fournit des exemples pour réaliser des agrégations distribuées et des analyses à l’aide d’Apache Spark et d’Azure DocumentDB.  Notez qu’Azure DocumentDB prend déjà en charge les agrégations, comme indiqué dans [l’article de blog Planet scale aggregates with Azure DocumentDB](https://azure.microsoft.com/blog/planet-scale-aggregates-with-azure-documentdb/) (Agrégats à l’échelle de la planète avec Azure DocumentDB), et voici donc comment passer au niveau supérieur avec Apache Spark.

Notez que ces agrégations font référence au [bloc-notes de connecteur Spark-DocumentDB](https://github.com/Azure/azure-documentdb-spark/blob/master/samples/notebooks/Spark-to-DocumentDB_Connector.ipynb).

### <a name="connecting-to-flights-sample-data"></a>Connexion à des données d’exemple de vols
Pour ces exemples d’agrégations, nous accédons à certaines données de performances de vol stockées dans notre base de données DocumentDB **DoctorWho**.  Pour vous connecter à celle-ci, vous devez utiliser l’extrait de code suivant :
...
Avec cet extrait de code, nous allons également exécuter une requête de base qui transfère l’ensemble de données filtré de DocumentDB à Spark (où ce dernier peut réaliser des agrégats distribués).  Dans ce cas, nous demandons des vols au départ de Seattle (SEA).
...
Dans l’ensemble suivant, nous pouvons facilement exécuter des requêtes **GROUP BY** sur notre base de données DocumentDB :
...
Si vous ne l’avez pas encore fait, téléchargez le connecteur Spark-DocumentDB à partir du référentiel GitHub [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark) et explorez les ressources supplémentaires dans le référentiel :
