...
Azure permet d'utiliser une machine virtuelle pour gérer les tâches nécessitant beaucoup de ressources. Par exemple, une machine virtuelle peut gérer des tâches et fournir des résultats à des ordinateurs clients ou à des applications mobiles. Ce guide vous permettra de créer une machine virtuelle exécutée sur une application Java nécessitant beaucoup de ressources et pouvant être surveillée par une autre application Java.

Ce didacticiel part du principe que vous savez créer des applications console Java, importer des bibliothèques dans votre application Java et générer une archive Java (JAR). Aucune connaissance d'Azure n'est nécessaire.
...
![Résolution du problème du voyageur de commerce](./media/virtual-machines-java-run-compute-intensive-task/WA_JavaTSPSolver.png)

Vous trouverez ci-dessous un exemple d'application Java qui surveille la tâche nécessitant beaucoup de ressources :

![Client du problème du voyageur de commerce](./media/virtual-machines-java-run-compute-intensive-task/WA_JavaTSPClient.png)
...
5.  Dans la boîte de dialogue **Configuration de la machine virtuelle** :
    ...
    3.  Entrez un nom d'administrateur dans le champ **Nom d'utilisateur**. Notez le nom et le mot de passe que vous allez entrer, car vous les utiliserez pour vous connecter à distance à votre machine virtuelle.
...
6.  Dans la boîte de dialogue **Configuration de la machine virtuelle** suivante :
    ...
    3.  Indiquez une région, un groupe d'affinités ou un réseau virtuel. Dans le cadre de ce didacticiel, indiquez une région telle que la **West US**.
    4.  Pour le **Compte de stockage**, sélectionnez **Utiliser un compte de stockage généré automatiquement**.
    5.  Pour le **Groupe à haute disponibilité**, sélectionnez **(Aucun)**.
...
3.  Dans le volet supérieur gauche du portail de gestion, cliquez sur le nœud **Service Bus**, puis sur le bouton **Nouveau**.
     ![Capture d'écran du nœud Service Bus](./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_02_SvcBusNode.jpg)
4.  Dans la boîte de dialogue **Create a new Service Namespace**, entrez un **Espace de noms**, puis vérifiez qu'il est unique en cliquant sur le bouton **Vérifier la disponibilité**.
     ![Capture d'écran Créer un espace de noms](./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_03_CreateNewSvcNamespace.jpg)
...
1.  Dans le volet de navigation gauche, cliquez sur le nœud **Service Bus** pour afficher la liste des espaces de noms disponibles :
     ![Capture d'écran Available Namespaces](./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_04_SvcBusNode_AvailNamespaces.jpg)
2.  Sélectionnez l'espace de noms que vous venez de créer dans la liste affichée :
     ![Capture d'écran Liste d'espaces de noms](./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_05_NamespaceList.jpg)
...
3.  Après le codage, exportez l'application dans une archive Java exécutable (JAR) et créez un package contenant les bibliothèques requises dans le fichier JAR généré. Dans le cadre de ce didacticiel, nous utiliserons le nom **TSPSolver.jar** pour désigner le fichier JAR généré.
        ...
        //  Valeur spécifiant la fréquence de mise à jour sur la console.
        ...
        public static void main(String args[]){
            ...
            try {
                ...
                int numCities = 10;  // À utiliser par défaut si aucune valeur n'est spécifiée sur la ligne de commande. 
                if (args.length != 0) 
                {
                    if (args[0].toLowerCase().compareTo("createqueue")==0)
                    {
                        // Aucun traitement n'intervient en dehors de la création de la file d'attente.
                    ...
                    {
                        // Aucun traitement n'intervient en dehors de la suppression de la file d'attente.
                    ...
                    // Ni création ni suppression de file d'attente.
                    // En partant du principe que la valeur transmise correspond au nombre de villes à résoudre.
...
2.  Exportez l'application dans un fichier JAR exécutable et créez un package contenant les bibliothèques requises dans le fichier JAR généré. Dans le cadre de ce didacticiel, nous utiliserons le nom **TSPClient.jar** pour désigner le fichier JAR généré.
        ...
        public static void main(String[] args) 
        {
     	...
     	"minute." : waitMinutes + " minutes."; 
                    ...
                    // Cette file d'attente doit avoir été préalablement créée.
                    ...
                    while (true)
                    {
                        ...
                        {                        
                            ...
                            // Afficher le message de file d'attente.
                            byte[] b = new byte[200];
                            ...
                            {
                                // Aucun autre traitement n'intervient.
                                date = new Date();
...
> [WACOM.NOTE] Plus le nombre spécifié est important, plus l'exécution du solveur est longue. Par exemple, une exécution portant sur 14 villes peut prendre quelques minutes, et une exécution portant sur 15 villes peut prendre des heures. Au-delà de 16 villes, l'exécution peut prendre des jours (voire des semaines, des mois et des années). Cette lenteur est due à la hausse rapide du nombre de permutations évaluées par le solveur à mesure que le nombre de villes augmente.
