<properties urlDisplayName="Web with Django (Windows)" pageTitle="Didacticiel&nbsp;Application web Python avec&nbsp;Django - Azure" metaKeywords="Azure Django web app, Azure Django virtual machine" description="Didacticiel qui explique comment h&eacute;berger un site web&nbsp;Django dans Azure en utilisant une machine virtuelle Windows&nbsp;Server&nbsp;2012&nbsp;R2&nbsp;Datacenter." metaCanonical="" services="virtual-machines" documentationCenter="Python" title="Application Web Django Hello World" authors="huvalo" solutions="" manager="wpickett" editor="" />
...
Hello World. L'application sera hébergée sur une machine virtuelle Azure.
...
1.  Suivez les instructions disponibles [ici][ici] pour créer une machine virtuelle Azure de la distribution *Windows Server 2012 R2 Datacenter*.
...
## <span id="setup"></span> </a>Configuration de Python et de Django
...
1.  Installez [Web Platform Installer][Web Platform Installer].
2.  Installez Python et WFastCGI au moyen de Web Platform Installer. Cela installe wfastcgi.py dans votre dossier de scripts Python.

    1.  Démarrez Web Platform Installer.
    2.  Tapez WFastCGI dans la barre de recherche.
    3.  Sélectionnez l'entrée WFactCGI pour la version de Python que vous voulez utiliser (2.7 ou 3.4). Remarque : cela installe Python comme dépendance de WFastCGI.

3.  Si vous avez installé Python 2.7, [suivez ces instructions pour installer pip manuellement][suivez ces instructions pour installer pip manuellement] (pip est déjà installé avec Python 3.4).
4.  Installez Django en utilisant pip.
...
2.  Enregistrez le gestionnaire pour ce site.
...
3.  Configurez le gestionnaire de manière à exécuter votre application Django.
...
4.  Configurez PYTHONPATH de manière à permettre à l'interpréteur Python de trouver votre application Django.
...
5.  Spécifiez le gestionnaire WSGI à utiliser par la passerelle FastCGI-WSGI.
...
6.  Les éléments suivants doivent s'afficher :
...
Lorsque vous avez terminé ce didacticiel, arrêtez et/ou supprimez votre machine virtuelle Azure nouvellement créée afin de libérer des ressources pour d'autres didacticiels et éviter l'ajout de charges d'utilisation Azure.
  ...
  [suivez ces instructions pour installer pip manuellement]: https://pip.pypa.io/en/latest/installing.html
