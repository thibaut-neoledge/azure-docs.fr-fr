<properties 
	pageTitle="Concepts liés aux comptes de stockage | Azure" 
	description="Découvrez les concepts liés au compte de stockage." 
	headerExpose="" 
	footerExpose="" 
	services="storage" 
	authors="tamram" 
...
Votre compte Azure Storage est un compte unique et sécurisé grâce auquel vous pouvez accéder aux services de stockage Azure - les services BLOB, de File d'attente, de Table et de fichier. Vos données sont sécurisées au niveau de votre compte de stockage et, par défaut, sont uniquement disponibles pour vous, le propriétaire. 
...
Cet article décrit comment créer un compte de stockage et inclut certaines décisions à prendre en compte pour le créer. Il décrit également la manière de gérer les clés d'accès à votre compte de stockage et la façon de supprimer un compte de stockage.
...
## <a id="create"></a>Procédure : Création un compte de stockage ##
...
3. Dans **URL**, entrez un nom pour votre compte de stockage. Voir [Points de terminaison de compte stockage](#account-endpoints) ci-dessous pour en savoir plus sur la manière dont ce nom sera utilisé pour adresser les objets que vous stockez dans Azure Storage.
...
Vous pouvez aussi configurer un nom de domaine personnalisé à utiliser avec votre compte de stockage. Consultez la rubrique [Configuration d'un nom de domaine personnalisé pour des données d'objets blob dans un compte de stockage](http://azure.microsoft.com/fr-FR/documentation/articles/storage-custom-domain-name/) pour plus d'informations.
...
> [AZURE.WARNING] Il n'existe aucun moyen de restaurer le contenu d'un compte de stockage supprimé. Assurez-vous de sauvegarder tout ce que vous souhaitez enregistrer avant de supprimer le compte.<br />
