...
Avant de commencer, lisez attentivement cette procédure pour comprendre à quel moment et pourquoi effectuer chaque étape. Autant que possible, préparez les informations et autres données de votre entreprise, téléchargez les outils nécessaires et/ou créez les composants techniques avant de lancer le processus de création de l’offre. Cet article devrait vous aider à mieux comprendre ces étapes.
...
> [AZURE.NOTE]Vous devez avoir accès à un ordinateur Windows pour - Exécuter l’outil de validation de certification - Créer l’URL SAS du disque dur virtuel pour l’envoi de la certification de disque dur virtuel
...
- Un développement local nécessite de télécharger un disque dur virtuel et de le développer dans une infrastructure locale. Bien que cela soit possible, nous ne le recommandons pas. Notez que le développement local pour Windows ou SQL implique que vous disposiez des clés de licence locales correspondantes. Vous ne pouvez pas inclure ou installer SQL Server après la création d’une machine virtuelle, et vous devez baser votre offre sur une image SQL approuvée à partir du portail Azure. Si vous décidez d’effectuer un développement local, certaines étapes diffèrent de celle d’un développement dans le cloud. Pour les informations correspondantes, consultez [Création d’une image de machine virtuelle locale](marketplace-publishing-vm-image-creation-on-premise.md).
