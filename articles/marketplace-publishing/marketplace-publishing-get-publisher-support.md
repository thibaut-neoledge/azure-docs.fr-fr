<properties
   pageTitle="Accès au support d'éditeur pour Azure Marketplace | Microsoft Azure"
   description="Comment créer à et soumettre des demandes de support pour Azure Marketplace"
   services="marketplace-publishing"
   documentationCenter="na"
   authors="v-jeana"
   manager="lakoch"
   editor=""/>

<tags
   ms.service="marketplace-publishing"
   ms.devlang="na"
   ms.topic="article"
   ms.tgt_pltfrm="na"
   ms.workload="na"
   ms.date="10/08/2015"
   ms.author="v-jeana; hascipio"/>


# Accès au support d'éditeur pour Azure Marketplace

Microsoft prend en charge un large éventail de produits et services. Il est donc important que votre demande soit dirigée vers l'équipe de support adéquate afin de garantir une réponse appropriée et rapide. Veuillez prendre les scénarios suivants en compte. Ils devraient vous aider à acheminer votre demande vers l'équipe appropriée.

- Si vous êtes un éditeur sur Marketplace et développez encore votre machine virtuelle, vous devez contacter vos équipes Microsoft Onboarding ou Compute.

- Si vous êtes un éditeur sur Marketplace et avez une question posée par un client, veuillez lui indiquer de demander une assistance via les liens de support à l'adresse https://ms.portal.azure.com/

- Si vous êtes un éditeur sur Marketplace et avez une question relative à votre application ou service, poursuivez la lecture ci-dessous.

## Cette article vous explique comment :

- vous connecter à votre compte Microsoft Azure ;
- accéder aux pages de support de l'éditeur ;
- sélectionner le type et la catégorie de problème ;
- demander une assistance ;
- fournir des informations de contact ;
- décrire le problème.

## 1\. Connexion à votre compte Microsoft Azure

Accédez à https://publish.windowsazure.com/ et connectez-vous avec les informations d'identification de votre compte Microsoft.

  ![Écran de connexion][1]

## 2\. Accès aux pages de support de l'éditeur

Dans le menu supérieur droit, sélectionnez « Obtenir de l'aide »

  ![Obtenir de l'aide][2]

## 3\. Sélection du type et de la catégorie de problème

![Type et catégorie de problème][3]

Les types de problèmes disponibles sont les suivants :

  - Account management (Gestion de compte)
  - Reports and Insights (Rapports et analyses)
  - VM Image Certification (Certification d'image de machine virtuelle)
  - Managing Customers (Gestion des clients)
  - Managing Offers (Gestion des offres)
  - Payouts (Paiements)
  - Seller Dashboard (Tableau de bord du vendeur)
  - Autres

## 4\. Demande d'assistance

Après avoir sélectionné le type et la catégorie de problème, cliquez sur le bouton « Start request (Début de la requête) »

![Démarrer le support][4]

## 5\. Indication des informations de contact et description du problème

Décrivez le problème d'une manière aussi complète que possible et fournissez tous les fichiers (par exemple, des captures d'écran, des traces réseau, etc.) qui peuvent être utiles pour résoudre votre problème.

![Décrire le problème][5]

## 6\. Confirmation de la soumission

Une fois votre demande soumise, nous accuserons réception dans les 24 heures et affecterons à votre problème une priorité et une gravité. Notre équipe d'ingénieurs de support technique dédiée aidera à sa résolution en fonction des temps de réponse définis dans notre contrat de niveau de service.

![Confirmation][6] + Si vous avez besoin de nous parler de votre problème, utilisez le numéro de confirmation dans toute correspondance + Vous pouvez vous informer de la progression de votre demande à tout moment

## Étapes suivantes

Pour savoir comment résoudre des problèmes de support spécifiques, visitez le site : [Résolution des problèmes courants dans Azure Marketplace (en Anglais)](marketplace-publishing-support-common-issues.md).

[1]: ./media/marketplace-publishing-get-publisher-support/step1.png
[2]: ./media/marketplace-publishing-get-publisher-support/step2.png
[3]: ./media/marketplace-publishing-get-publisher-support/step3.png
[4]: ./media/marketplace-publishing-get-publisher-support/step4.png
[5]: ./media/marketplace-publishing-get-publisher-support/step5.png
[6]: ./media/marketplace-publishing-get-publisher-support/step6.png

<!---HONumber=Oct15_HO3-->