---
title: "Collecte de données pour la formation d’un modèle : API Machine Learning Recommendations | Microsoft Docs"
...
---
# <a name="collecting-data-to-train-your-model"></a>Collecte de données pour la formation d’un modèle
...
## <a name="catalog-data"></a>Données de catalogue
### <a name="catalog-file-format"></a>Format de fichier de catalogue
Le fichier de catalogue contient des informations sur les articles que vous proposez à votre client.
...
#### <a name="sample-rows-in-a-catalog-file"></a>Exemples de lignes d’un fichier de catalogue
...
#### <a name="format-details"></a>Détails du format
...
#### <a name="uploading-a-catalog-file"></a>Chargement d’un fichier de catalogue
...
> Le nombre d’articles du catalogue pris en charge est limité à 100 000.
> 
> 

## <a name="why-add-features-to-the-catalog"></a>Pourquoi ajouter des caractéristiques au catalogue ?
...
### <a name="features-are-categorical"></a>Les caractéristiques sont catégorielles
...
### <a name="how-manywhich-features-should-i-use"></a>Combien de caractéristiques dois-je utiliser, et lesquelles ?
...
### <a name="when-are-features-actually-used"></a>Quand les caractéristiques sont-elles réellement utilisées ?
...
### <a name="using-product-features"></a>Utiliser les caractéristiques de produit
...
#### <a name="usage-format-details"></a>Détails du format du fichier d’utilisation
Un fichier d’utilisation est un fichier CSV (valeurs séparées par des virgules) dans lequel chaque ligne représente une interaction entre un utilisateur et un article. Chaque ligne est mise en forme de la manière suivante : <br>
...
| Événement |Non |Celui-ci peut avoir l'une des valeurs suivantes :<br>• Click (clic)<br>• RecommendationClick (clic de recommandation)<br>•    AddShopCart (ajout au panier)<br>• RemoveShopCart (suppression du panier)<br>• Purchase |Le type de transaction. |
...
#### <a name="uploading-a-usage-file"></a>Chargement d’un fichier d’utilisation
Consultez la [référence d’API](https://westus.dev.cognitive.microsoft.com/docs/services/Recommendations.V4.0/operations/56f316efeda5650db055a3e2) pour savoir comment charger des fichiers d’utilisation.
Notez que vous devez transmettre le contenu du fichier d’utilisation dans le corps de l’appel HTTP.
...
## <a name="how-much-data-do-you-need"></a>De quelle quantité de données avez-vous besoin ?
La qualité de votre modèle dépend fortement de la qualité et de la quantité de vos données.
Le système apprend lorsque les utilisateurs achètent différents éléments (ce que nous appelons les « co-occurrences »). Pour les versions FBT, il est également important de savoir quels éléments sont achetés dans une même transaction. 
