...
4. Sélectionnez le produit **API Cognitive Services**. Ce produit vous permettra de commencer un abonnement à l’une des API Cognitive Services (Face, Text Analytics, Computer Vision, etc.). Aujourd'hui, nous nous concentrerons sur l'API de recommandations.
...
1. Généralement, vous récupérez ces éléments d'information en interrogeant votre base de données de transactions. Au cas où vous n’en auriez pas à disposition, nous vous proposons des exemples de données fondés sur les données de transactions Microsoft Store.
   ...
   > 
   > 
2. Examinons maintenant le fichier de catalogue. Naviguez jusqu'à l'emplacement où vous avez copié les données. Ouvrez le fichier de catalogue dans le **bloc-notes**.
