<properties urlDisplayName="Use Content from a CDN in Your Web Application" pageTitle="Utilisation de contenu provenant d'un CDN dans votre application web" metaKeywords="Azure tutorial, Azure web app tutorial, ASP.NET, CDN" description="Un didacticiel qui explique comment utiliser le contenu d'un CDN pour améliorer les performances de votre application web." metaCanonical="" services="cdn" documentationCenter=".NET" title="Use Content from a CDN in Your Web Application" authors="cephalin" solutions="" manager="wpickett" editor="tysonn" />
...
-	Un [compte Microsoft Azure](http://azure.microsoft.com/fr-fr/account/) actif. Vous pouvez demander un compte d'évaluation gratuit.
...
2. Lorsque l'état du nouveau compte de stockage est **En ligne**, créez un point de terminaison CDN lié au compte de stockage que vous avez créé. Cliquez sur **Nouveau > Services d'application > CDN > Création rapide**. Sélectionnez le compte de stockage que vous avez créé et cliquez sur **Créer**.
...
4. Vous pouvez alors déjà voir où le point de terminaison CDN se trouve le plus près de vous. Sur mon ordinateur de bureau, l'adresse IP qui répond est **93.184.215.201**. Liez-la à un site tel que [www.ip-address.org](http://www.ip-address.org) : vous constatez que le serveur se trouve à Washington D.C.
	...
	Pour obtenir la liste des emplacements de tous les nœuds CDN, consultez la page [Emplacements des nœuds CDN Azure](http://msdn.microsoft.com/fr-fr/library/azure/gg680302.aspx).
...
6. Dans l'Explorateur de serveurs de Visual Studio 2013, cliquez sur le bouton **Se connecter à Windows Azure**.
...
8.  Lorsque vous êtes connecté, développez **Windows Azure > Stockage > votre compte de stockage**. Cliquez avec le bouton droit sur **Objet blob** et sélectionnez **Créer un conteneur d'objets blob**.
...
12.	Dans un navigateur, accédez à l'URL d'un des objets blob pour tester leur accès public. Par exemple, je peux tester la première image de ma liste téléchargée avec `http://cephalinstorage.blob.core.windows.net/cdn/cephas_lin.png`.
...
13.	Si l'objet blob s'affiche correctement dans votre navigateur, remplacez l'URL à partir de `http://<Nom de votre compte de stockage>.blob.core.windows.net` par l'URL de votre Azure CDN. Dans mon cas, pour tester la première image de mon point de terminaison CDN, j'utilise `http://az623979.vo.msecnd.net/cdn/cephas_lin.png`.
...
1. Téléchargez le scripte à partir de la[page de téléchargement]((http://gallery.technet.microsoft.com/scriptcenter/Upload-Content-Files-from-41c2142a). Enregistrez-la dans le répertoire du projet de votre application ASP.NET.
...
Ce script télécharge tous les fichiers de vos dossiers *\Content* et *\Scripts* dans le compte de stockage et le conteneur spécifiés. Cela présente les avantages suivants :
...
Lorsque le téléchargement du contenu est terminé, vous pouvez lier n'importe quoi dans vos dossiers *\Content* et *\Scripts* dans votre code HTML (par exemple, vos fichiers .cshtml) en utilisant `http://<Nom de votre CDN>.vo.msecnd.net/<Nom du conteneur>`. Voici un exemple de quelque chose que je peux utiliser dans une vue Razor : 

	<img alt="Mugshot" src="http://az623979.vo.msecnd.net/MyMvcApp/Content/cephas_lin.png" />

Pour un exemple d'intégration de scripts PowerShell dans votre configuration de remise continue, consultez la page [Remise continue pour Cloud Services dans Azure](http://azure.microsoft.com/fr-fr/documentation/articles/cloud-services-dotnet-continuous-delivery/). 
...
Vous pouvez d'abord faire cela dans votre script PowerShell pour définir les en-têtes Cache-Control de tous les objets blob. Pour le script de la section [Automatisation du téléchargement du contenu de votre application ASP.NET vers votre point de terminaison CDN](#upload), recherchez l'extrait de code suivant :
...
Avec les services [Sites Web Azure](http://azure.microsoft.com/fr-fr/services/websites/) et [Azure Cloud Services](http://azure.microsoft.com/fr-fr/services/cloud-services/), vous pouvez obtenir la meilleure intégration CDN grâce au [regroupement et à la minimisation ASP.NET](http://www.asp.net/mvc/tutorials/mvc-4/bundling-and-minification). 
...
- Vous devez télécharger manuellement vers le serveur les scripts regroupés dans le stockage d'objets blob. Une solution par programme est proposée par [stackoverflow](http://stackoverflow.com/a/13736433).
