<properties 
	pageTitle="Résolution des problèmes du cache Redis Azure | Microsoft Azure" 
...
### Saturation de la mémoire sur le client
...
### Bande passante dépassée côté client
...
### Taille importante de la demande/réponse
...
### Qu’est-il arrivé à mes données dans Redis ?
...
### Saturation de la mémoire sur le serveur
...
4. Procédez à une [mise à l’échelle](cache-how-to-scale.md) pour augmenter la taille du cache.
...
### Utilisation élevée du processeur / charge importante du serveur
...
Procédez à la [mise à l’échelle](cache-how-to-scale.md) vers une plus grande taille de cache avec davantage de capacités processeur ou recherchez la cause des pics d’utilisation du processeur.

### Bande passante dépassée côté serveur
...
3. Vérifiez que vous utilisez la dernière version en date du [package NuGet StackExchange.Redis](https://www.nuget.org/packages/StackExchange.Redis/). Nous corrigeons en permanence le code pour le rendre plus robuste aux délais d’expiration. Il est donc primordial d’utiliser la dernière version.
