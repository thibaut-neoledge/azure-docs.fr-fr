...
Pour plus d’informations sur les autres fonctionnalités du cache Premium, consultez [Comment configurer la persistance pour un Cache Redis Azure Premium](cache-how-to-premium-persistence.md) et [Comment configurer le clustering pour un Cache Redis Azure Premium](cache-how-to-premium-clustering.md).
...
La configuration de l’intégration de réseau virtuel Cache Redis Azure s’effectue dans le panneau **Réseau virtuel**. Depuis ce panneau vous pouvez sélectionner un réseau virtuel classique existant. Pour utiliser un nouveau réseau virtuel, suivez les étapes fournies dans [Créer un réseau virtuel (classique) à l’aide du portail Azure en version préliminaire](../virtual-network/virtual-networks-create-vnet-classic-pportal.md), puis revenez au panneau **Réseau virtuel de Cache Redis** pour le sélectionner.

>[AZURE.NOTE]Pendant la période d’évaluation du cache Premium, le Cache Redis Azure fonctionne avec les réseaux virtuels classiques. Pour plus d’informations sur la création d’un réseau virtuel classique, consultez [Créer un réseau virtuel (classique) à l’aide du portail Azure en version préliminaire](../virtual-network/virtual-networks-create-vnet-classic-pportal.md).
...
Tapez l’**Adresse IP statique** souhaitée et cliquez sur **OK** pour enregistrer la configuration du réseau virtuel. Si l’adresse IP statique sélectionnée est déjà utilisée, un message d’erreur s’affiche.
