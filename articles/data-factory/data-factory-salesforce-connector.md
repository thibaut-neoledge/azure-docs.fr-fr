...
Pour l’instant, Data Factory permet uniquement de déplacer des données depuis Salesforce vers des [magasins récepteurs pris en charge]((data-factory-data-movement-activities.md#supported-data-stores), mais pas de déplacer des données depuis d’autres magasins de données vers Salesforce.

## Configuration requise
- Vous devez utiliser l’une des éditions suivantes : Developer Edition, Professional Edition, Enterprise Edition ou Unlimited Edition.
...
- Pour copier des données depuis Salesforce vers des magasins de données locaux, la passerelle de gestion des données version 2.0 ou ultérieure doit être installée dans votre environnement local.

## Assistant Copier des données
Le moyen le plus simple de créer un pipeline qui copie les données à partir de Salesforce vers n’importe quel magasin de données récepteur pris en charge consiste à utiliser l’Assistant Copier des données. Consultez la page [Didacticiel : Créer un pipeline à l’aide de l’Assistant de copie](data-factory-copy-data-wizard-tutorial.md) pour une procédure pas à pas rapide sur la création d’un pipeline à l’aide de l’Assistant Copier de données.
...
La définition de **external** sur **true** informe le service Data Factory qu’il s’agit d’un jeu de données externe à la Data Factory et non produit par une activité dans la Data Factory.
...
| username |Spécifiez le nom d’utilisateur du compte d’utilisateur. | Oui |
...
| Propriété | Description | Requis |
| -------- | ----------- | -------- |
| TableName | Nom de la table dans Salesforce. | Non (si la **requête** de **RelationalSource** est spécifiée) | 
