...
3. Copiez cette entrée dans le fichier machine.config dans le dossier v4.0 suivant : <disque système>:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\Config\\machine.config, et remplacez la version par 4.xxx.x.x.
4. Installez « <Chemin d’installation d’ODP.NET>\\11.2.0\\client\_1\\odp.net\\bin\\4\\Oracle.DataAccess.dll » dans le Global Assembly Cache (GAC) en exécutant `gacutil /i [provider path]`.
