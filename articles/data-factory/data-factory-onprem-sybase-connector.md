<properties 
	pageTitle="Connecteur Sybase - Déplacement de données à partir de Sybase" 
	description="En savoir plus sur le connecteur Sybase pour le service Data Factory qui vous permet de déplacer des données depuis une base de données Sybase" 
...
# Connecteur Sybase - Déplacement de données à partir de Sybase 
...
**Remarque :** vous devez utiliser la passerelle pour vous connecter à Sybase même si elle est hébergée sur des machines virtuelles IaaS Azure. Si vous essayez de vous connecter à une instance Sybase hébergée dans le cloud, vous pouvez également installer l'instance de la passerelle dans la machine virtuelle IaaS.
...
1.	Un service lié de type OnPremisesSybase.
...
La définition de « external » : true et la spécification de la stratégie externalData informent Data Factory qu'il s'agit d'une table externe à Data Factory et non produite par une activité dans Data Factory. Notez que le **type** du service lié est défini sur :**RelationalTable**.
...
Comme mentionné dans l’article consacré aux activités de déplacement des données, l’activité de copie convertit automatiquement des types source en types récepteur à l’aide de l’approche en 2 étapes suivante :
