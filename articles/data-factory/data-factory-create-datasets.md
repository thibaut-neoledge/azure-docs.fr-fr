...
Un **service lié** définit les informations nécessaires à Azure Data Factory pour se **connecter** à un **magasin de données** (notamment un compte de stockage Azure et Azure SQL Database) ou au **calcul** (notamment Azure HDInsight et Azure Batch). Le service lié définit le mécanisme (adresse, protocole, schéma d’authentification, etc.) pour accéder au magasin de données ou au calcul.

Pour obtenir la liste des services liés de stockage de données pris en charge, consultez [Sources de données prises en charge](data-factory-data-movement-activities.md#supported-data-stores). Cliquez sur une source de données dans la table pour accéder à la rubrique qui fournit des détails sur la création/configuration d’un service lié pour ce magasin de données.

Pour obtenir la liste des services de calcul pris en charge liés, consultez [Services liés de calcul](data-factory-compute-linked-services.md). Pour comprendre les activités qui utilisent ces services liés, consultez [Activités de transformation des données](data-factory-data-transformation-activities.md).

Un **jeu de données** dans Data Factory représente des structures de données dans un magasin de données représenté par un **service lié de magasin de données**, par exemple, un conteneur d’objets blob dans un compte de stockage Azure, une table dans une base de données SQL Azure). Il peut être utilisé en tant qu’entrée ou sortie d’une activité dans un pipeline. Après avoir créé des jeux de données, vous pouvez utiliser ces derniers avec des activités dans le pipeline. Un jeu de données peut, par exemple, constituer un jeu de données d’entrée/de sortie d’une activité de copie/activité HDInsightHive.

> [AZURE.NOTE] Si vous découvrez tout juste Azure Data Factory, consultez la [Présentation d’Azure Data Factory](data-factory-introduction.md) pour une vue d’ensemble du service Azure Data Factory et le didacticiel [Créer votre première fabrique de données](data-factory-build-your-first-pipeline.md) pour créer votre première fabrique de données. Ces deux articles fournissent des informations de base pour mieux comprendre le présent article.
...
> [AZURE.IMPORTANT] À moins qu’un jeu de données soit généré par Azure Data Factory, il doit être marqué comme étant **externe (external)**. Cela s’applique généralement aux entrées de la première activité d’un pipeline.
