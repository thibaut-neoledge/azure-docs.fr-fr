...
### <a name="capabilities-of-data-management-gateway"></a>Fonctionnalités de la passerelle de gestion de données
...
### <a name="install-gateway-from-download-center"></a>Installer la passerelle à partir du Centre de téléchargement
...
#### <a name="if-you-haven't-already-created-a-logical-gateway-in-the-portal"></a>Si vous n’avez pas déjà créé une passerelle logique dans le portail
...
3. Dans le panneau **Passerelle de données**, cliquez sur **Télécharger et installer la passerelle de données**.
...
4. Dans le panneau **Configurer**, cliquez sur **Recréer une clé**. Dans le message d’avertissement, cliquez sur Oui après l’avoir lu attentivement.
...
### <a name="configure-proxy-server-settings-in-diahost.exe.config"></a>Configurer les paramètres du serveur proxy dans diahost.exe.config
...
### <a name="to-disable/enable-auto-update-feature"></a>Pour activer/désactiver une fonctionnalité de mise à jour automatique
...
5. Une fois que vous êtes satisfait de ce que vous allez charger, cliquez sur **Envoyer des journaux** pour envoyer les journaux des sept derniers jours à Microsoft en vue de la résolution des problèmes. L’état de l’opération d’envoi des journaux devrait s’afficher comme indiqué sur l’image suivante :
...
6. Une fois l’opération terminée, la boîte de dialogue de l’image suivante s’affiche :
...
#### <a name="gateway-cannot-connect-to-cloud-service-through-service-bus"></a>La passerelle ne peut pas se connecter au service cloud via service bus
...
#### <a name="error:-cloud-service-cannot-connect-to-gateway-through-service-bus."></a>Erreur : Le service cloud ne peut pas se connecter à la passerelle via service bus.
Suivez ces étapes pour remettre la passerelle en ligne :
...
## <a name="move-gateway-from-a-machine-to-another"></a>Déplacement d’une passerelle d’une machine vers une autre
...
2. Sélectionnez votre passerelle dans la section **PASSERELLES DONNÉES** du panneau **Services liés**.
...
4. Dans le panneau **Configurer**, cliquez sur **Télécharger et installer la passerelle de données**, puis suivez les instructions pour installer la passerelle de données sur la machine. 
...
6. Dans le panneau **Configurer** du portail, cliquez sur **Recréer une clé** dans la barre de commandes, puis, au message d’avertissement, cliquez sur **Oui**. Cliquez sur le **bouton de copie** en regard du texte de la clé qui copie la clé dans le presse-papiers. La passerelle de l’ancienne machine cesse de fonctionner dès que vous recréez la clé.  
