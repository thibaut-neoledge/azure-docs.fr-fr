...
Cet article présente l’intégration des données entre les magasins de données locaux et les magasins de données cloud, ainsi que le traitement cloud à l’aide de Data Factory. Il s’appuie sur l’article [Activités de déplacement des données](data-factory-data-movement-activities.md) et d’autres articles sur les principaux concepts Data Factory. Vous devez maîtriser les concepts Data Factory tels que les pipelines, les activités, les jeux de données et l’activité de copie.

## Fonctionnement d’Azure Data Factory avec la passerelle de gestion des données 
Et si vous pouviez déplacer vos données en toute transparence entre vos emplacements locaux et le cloud ? Azure Data Factory – un agent installé localement – vous offre les fonctionnalités de gestion de passerelle de données dont vous avez besoin, y compris une application permettant de définir les informations d’identification de la passerelle.

## Rôle de la passerelle de données
...
1.	Les versions de **système d’exploitation** prises en charge sont les suivantes : Windows 7, Windows 8/8.1, Windows 10, Windows Server 2008 R2, Windows Server 2012 et Windows Server 2012 R2. L’installation de la passerelle de gestion des données sur un contrôleur de domaine n’est pas prise en charge actuellement.
...
## Définir les informations d’identification de la passerelle
...
3. Dans le panneau **Configurer**, cliquez sur **Installer directement sur cet ordinateur**. Cette opération télécharge le package d’installation de la passerelle, installe, configure et inscrit la passerelle sur l’ordinateur.
	...
	>  
	> Vous devez en faire autant pour Firefox (installez un complément). Cliquez sur le **bouton de menu** dans la barre d’outils (**trois lignes horizontales** en haut à droite), cliquez sur **Modules complémentaires**, effectuez une recherche sur le mot-clé « ClickOnce », choisissez l’une des extensions ClickOnce, puis installez-la.
...
2.	Remplacez le code JSON du volet droit par le texte suivant :
	...
	Si vous ne spécifiez pas de nom (**fileName**) pour une **table de sortie**, les fichiers générés dans le **folderPath** sont nommés selon le format suivant : Data.<Guid>.txt (par exemple : Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt).
...
1. Lancez le navigateur web sur **l’ordinateur passerelle**, accédez au [portail Azure](http://portal.azure.com), rechercher votre fabrique de données si nécessaire, ouvrez la fabrique de données dans le panneau **DATA FACTORY**, puis cliquez sur **Créer et déployer** pour démarrer l’éditeur Data Factory.
