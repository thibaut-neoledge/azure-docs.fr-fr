...
Sorties | Tables de sortie utilisées par l'activité.<p>// une table de sortie<br/>« sorties » : [ { « nom » : « outputtable1 » } ],</p><p>//deux tables de sortie<br/>« sorties » : [ { « nom » : « outputtable1 » }, { « nom » : « outputtable2 » } ],</p>
...
typeProperties | Les propriétés de la section typeProperties dépendent du type de l'activité. Consultez l'article sur chaque activité pour en savoir plus | No 
policy | Stratégies qui affectent le comportement d'exécution de l'activité. Si aucune valeur n'est spécifiée, les stratégies par défaut seront utilisées. Faites défiler vers le bas pour plus d'informations | No 
...
isPaused | Si la valeur est true, le pipeline ne s’exécute pas. Valeur par défaut = false. Vous pouvez utiliser cette propriété pour activer ou désactiver. | No 
