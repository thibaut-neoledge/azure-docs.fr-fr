...
Cet article vous aide à comprendre les pipelines de données et les activités dans Azure Data Factory et comment les utiliser pour créer des flux de travail pilotés par les données de bout en bout pour votre scénario ou votre entreprise, des recommandations produit personnalisées aux analyses de campagnes marketing.
...
typeProperties | Les propriétés de la section typeProperties dépendent du type de l’activité. Consultez les articles propres à chaque activité pour en savoir plus. | Non
policy | Stratégies affectant le comportement d'exécution de l'activité. Si aucune valeur n'est spécifiée, les stratégies par défaut seront utilisées. Faire défiler pour obtenir plus d’informations | Non
...
isPaused | Si la valeur est « true », le pipeline n’est pas exécuté. Valeur par défaut = false. Vous pouvez utiliser cette propriété pour activer ou désactiver. | Non 
...
| expirationTime | Durée pendant laquelle le pipeline est valide et doit rester configuré. Le pipeline sera automatiquement supprimé une fois le délai d’expiration atteint s’il ne contient aucune exécution active, en échec ou en attente. | Non | 
