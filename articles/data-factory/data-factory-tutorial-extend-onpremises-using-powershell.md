...
2. [Créer le service lié SQL Server](#create-sql-server-linked-service). Dans cette étape, vous créez d’abord une base de données et une table sur votre ordinateur SQL Server local, avant de créer le service lié : **OnPremSqlLinkedService**.
...
4. [Surveiller le pipeline](#monitor-pipeline). Dans cette étape, vous allez surveiller les pipelines, les tables et les tranches de données à l’aide du portail Azure Classic.
...
Vous pouvez maintenant suivre la même procédure que celle présentée dans [Surveillance des pipelines](data-factory-tutorial-using-powershell.md#monitor-pipelines) pour surveiller le nouveau pipeline et les tranches de données de la nouvelle table ADF locale.
