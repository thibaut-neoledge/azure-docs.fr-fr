...
1. Connectez-vous à la machine virtuelle en cliquant sur **Connecter** dans la barre de commandes. Pour plus d'informations, consultez la rubrique [Connexion à une machine virtuelle exécutant Linux][].
...
8.	Toutes les images Linux doivent être *deprovisioned* à l'aide de la commande `waagent`, avec l'option `-deprovision`. Cliquez sur **J'ai exécuté waagent -deprovision sur la machine virtuelle.** pour indiquer que le système d'exploitation est prêt à être converti en image.
...
[Association d'un disque de données avec une machine virtuelle]: storage-windows-attach-disk.md
