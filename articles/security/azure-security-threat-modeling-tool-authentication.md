...
## <a name="a-idstandard-authn-web-appaconsider-using-a-standard-authentication-mechanism-to-authenticate-to-web-application"></a><a id="standard-authn-web-app"></a>Envisager d’utiliser un mécanisme d’authentification standard pour l’application web
...
## <a name="a-idhandle-failed-authnaapplications-must-handle-failed-authentication-scenarios-securely"></a><a id="handle-failed-authn"></a>Les applications doivent gérer les scénarios d’authentification ayant échoué en toute sécurité 
...
## <a name="a-idstep-up-adaptive-authnaenable-step-up-or-adaptive-authentication"></a><a id="step-up-adaptive-authn"></a>Activer l’authentification adaptative ou avancée
...
## <a name="a-idadmin-interface-lockdownaensure-that-administrative-interfaces-are-appropriately-locked-down"></a><a id="admin-interface-lockdown"></a>Garantir que les interfaces d’administration sont correctement verrouillées
...
## <a name="a-idforgot-pword-fxnaimplement-forgot-password-functionalities-securely"></a><a id="forgot-pword-fxn"></a>Implémenter des fonctionnalités de mot de passe oublié en toute sécurité
...
## <a name="a-idpword-account-policyaensure-that-password-and-account-policy-are-implemented"></a><a id="pword-account-policy"></a>Garantir que la stratégie de compte et de mot de passe est implémentée
...
## <a name="a-idcontrols-username-enumaimplement-controls-to-prevent-username-enumeration"></a><a id="controls-username-enum"></a>Implémenter des contrôles pour empêcher l’énumération de nom d’utilisateur
...
## <a name="a-idwin-authn-sqlawhen-possible-use-windows-authentication-for-connecting-to-sql-server"></a><a id="win-authn-sql"></a>Si possible, utiliser l’authentification Windows pour la connexion à SQL Server
...
| Références              | [SQL Server - Choisir un mode d’authentification](https://msdn.microsoft.com/library/ms144284.aspx) |
...
## <a name="a-idaad-authn-sqlawhen-possible-use-azure-active-directory-authentication-for-connecting-to-sql-database"></a><a id="aad-authn-sql"></a>Si possible, utiliser l’authentification Azure Active Directory pour la connexion à SQL Database
...
| Références              | [Connexion à SQL Database avec l’authentification Azure Active Directory](https://azure.microsoft.com/documentation/articles/sql-database-aad-authentication/) |
...
## <a name="a-idauthn-account-pwordawhen-sql-authentication-mode-is-used-ensure-that-account-and-password-policy-are-enforced-on-sql-server"></a><a id="authn-account-pword"></a>Lorsque le mode d’authentification SQL est utilisé, garantir que la stratégie de compte et de mot de passe est appliquée sur SQL Server
...
| Technologies applicables | Générique |
| Attributs              | N/A  |
| Références              | [Stratégie de mot de passe SQL Server](https://technet.microsoft.com/library/ms161959(v=sql.110).aspx) |
...
## <a name="a-idautn-contained-dbado-not-use-sql-authentication-in-contained-databases"></a><a id="autn-contained-db"></a>Ne pas utiliser l’authentification SQL dans des bases de données à relation contenant-contenu
...
| Références              | [Meilleures pratiques de sécurité recommandées avec les bases de données à relation contenant-contenu](http://msdn.microsoft.com/library/ff929055.aspx) |
...
## <a name="a-idauthn-sas-tokensause-per-device-authentication-credentials-using-sas-tokens"></a><a id="authn-sas-tokens"></a>Utiliser les informations d’authentification par appareil à l’aide des jetons SAP
...
## <a name="a-idmulti-factor-azure-adminaenable-azure-multi-factor-authentication-for-azure-administrators"></a><a id="multi-factor-azure-admin"></a>Activer Azure Multi-Factor Authentication pour les administrateurs Azure
...
| Technologies applicables | Générique |
| Attributs              | N/A  |
| Références              | [Présentation d'Azure Multi-Factor Authentication](https://azure.microsoft.com/documentation/articles/multi-factor-authentication/) |
...
## <a name="a-idanon-access-clusterarestrict-anonymous-access-to-service-fabric-cluster"></a><a id="anon-access-cluster"></a>Restreindre l’accès anonyme au cluster Service Fabric
...
| Références              | [Scénarios de sécurité d’un cluster Service Fabric](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security) |
...
## <a name="a-idfabric-cn-nnaensure-that-service-fabric-client-to-node-certificate-is-different-from-node-to-node-certificate"></a><a id="fabric-cn-nn"></a>Garantir que le certificat client à nœud Service Fabric est différent du certificat nœud à nœud
...
## <a name="a-idaad-client-fabricause-aad-to-authenticate-clients-to-service-fabric-clusters"></a><a id="aad-client-fabric"></a>Utiliser AAD pour authentifier les clients sur les clusters Service Fabric
...
| Technologies applicables | Générique |
| Attributs              | Environnement - Azure |
| Références              | [Scénarios de sécurité d’un cluster - Recommandations en matière de sécurité](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#security-recommendations) |
...
## <a name="a-idfabric-cert-caaensure-that-service-fabric-certificates-are-obtained-from-an-approved-certificate-authority-ca"></a><a id="fabric-cert-ca"></a>Garantir que les certificats Service Fabric proviennent d’une autorité de certification approuvée
...
| Technologies applicables | Générique |
| Attributs              | Environnement - Azure |
| Références              | [Certificats X.509 et Service Fabric](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security/#x509-certificates-and-service-fabric) |
...
## <a name="a-idstandard-authn-idause-standard-authentication-scenarios-supported-by-identity-server"></a><a id="standard-authn-id"></a>Utiliser des scénarios d’authentification standard pris en charge par IdentityServer
...
| Technologies applicables | Générique |
| Attributs              | N/A  |
| Références              | [IdentityServer3 - The Big Picture](https://identityserver.github.io/Documentation/docsv2/overview/bigPicture.html) (IdentityServer3 - Vue d’ensemble) |
...
## <a name="a-idoverride-tokenaoverride-the-default-identity-server-token-cache-with-a-scalable-alternative"></a><a id="override-token"></a>Remplacer le cache de jetons IdentityServer par défaut par une solution évolutive
...
| Technologies applicables | Générique |
| Attributs              | N/A  |
| Références              | [Identity Server Deployment - Caching](https://identityserver.github.io/Documentation/docsv2/advanced/deployment.html) (Déploiement d’IdentityServer - Mise en cache) |
...
## <a name="a-idbinaries-signedaensure-that-deployed-applications-binaries-are-digitally-signed"></a><a id="binaries-signed"></a>Garantir que les fichiers binaires de l’application déployée sont signés numériquement
...
## <a name="a-idmsmq-queuesaenable-authentication-when-connecting-to-msmq-queues-in-wcf"></a><a id="msmq-queues"></a>Activer l’authentification lors de la connexion aux files d’attente MSMQ dans WCF
...
| Attributs              | N/A |
| Références              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx) |
...
## <a name="a-idmessage-noneawcf-do-not-set-message-clientcredentialtype-to-none"></a><a id="message-none"></a>WCF - Ne pas définir clientCredentialType du message sur la valeur « aucun »
...
| Références              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx), [Fortify](https://vulncat.fortify.com/en/vulncat/index.html) |
...
## <a name="a-idtransport-noneawcf-do-not-set-transport-clientcredentialtype-to-none"></a><a id="transport-none"></a>WCF - Ne pas définir clientCredentialType du transport sur la valeur « aucun »
...
## <a name="a-idauthn-secure-apiaensure-that-standard-authentication-techniques-are-used-to-secure-web-apis"></a><a id="authn-secure-api"></a>Garantir que les techniques d’authentification standard sont utilisées pour sécuriser les API Web
...
## <a name="a-idauthn-aadause-standard-authentication-scenarios-supported-by-azure-active-directory"></a><a id="authn-aad"></a>Utiliser des scénarios d’authentification standard pris en charge par Azure Active Directory
...
## <a name="a-idadal-scalableaoverride-the-default-adal-token-cache-with-a-scalable-alternative"></a><a id="adal-scalable"></a>Remplacer le cache de jetons ADAL par défaut par une solution évolutive
...
| Technologies applicables | Générique |
| Attributs              | N/A  |
| Références              | [Modern Authentication with Azure Active Directory for Web Applications](https://blogs.msdn.microsoft.com/microsoft_press/2016/01/04/new-book-modern-authentication-with-azure-active-directory-for-web-applications/) (Authentification moderne avec Azure Active Directory pour les applications web), [Using Redis as ADAL token cache](https://blogs.msdn.microsoft.com/mrochon/2016/09/19/using-redis-as-adal-token-cache/) (Utilisation de Redis en tant que cache de jeton ADAL)  |
...
## <a name="a-idtokenreplaycache-adalaensure-that-tokenreplaycache-is-used-to-prevent-the-replay-of-adal-authentication-tokens"></a><a id="tokenreplaycache-adal"></a>Garantir que TokenReplayCache est utilisé pour empêcher la relecture de jetons d’authentification ADAL
...
| Technologies applicables | Générique |
| Attributs              | N/A  |
| Références              | [Modern Authentication with Azure Active Directory for Web Applications](https://blogs.msdn.microsoft.com/microsoft_press/2016/01/04/new-book-modern-authentication-with-azure-active-directory-for-web-applications/) (Authentification moderne avec Azure Active Directory pour les applications web) |
...
## <a name="a-idadal-oauth2ause-adal-libraries-to-manage-token-requests-from-oauth2-clients-to-aad-or-on-premises-ad"></a><a id="adal-oauth2"></a>Utiliser les bibliothèques ADAL pour gérer les demandes de jeton des clients OAuth2 vers AAD (ou Active Directory local)
...
| Technologies applicables | Générique |
| Attributs              | N/A  |
| Références              | [ADAL](https://azure.microsoft.com/documentation/articles/active-directory-authentication-libraries/) |
...
## <a name="a-idauthn-devices-fieldaauthenticate-devices-connecting-to-the-field-gateway"></a><a id="authn-devices-field"></a>Authentifier les appareils se connectant à la passerelle de champ
...
## <a name="a-idauthn-devices-cloudaensure-that-devices-connecting-to-cloud-gateway-are-authenticated"></a><a id="authn-devices-cloud"></a>Garantir que les appareils se connectant à la passerelle de cloud sont authentifiés
...
| Références              | N/A, [Mise en route d’Azure IoT Hub (.NET)](https://azure.microsoft.com/documentation/articles/iot-hub-csharp-csharp-getstarted/), [Mise en route d’Azure IoT Hub (Node)](https://azure.microsoft.com/documentation/articles/iot-hub-node-node-getstarted), [Contrôler l’accès à IoT Hub](https://azure.microsoft.com/documentation/articles/iot-hub-sas-tokens/), [référentiel Git](https://github.com/Azure/azure-iot-sdks/tree/master/node) |
...
## <a name="a-idauthn-credause-per-device-authentication-credentials"></a><a id="authn-cred"></a>Utiliser les informations d’authentification par appareil
...
| Références              | [Contrôler l’accès à IoT Hub](https://azure.microsoft.com/documentation/articles/iot-hub-sas-tokens/) |
...
## <a name="a-idreq-containers-anonaensure-that-only-the-required-containers-and-blobs-are-given-anonymous-read-access"></a><a id="req-containers-anon"></a>Garantir que seuls les conteneurs et blobs requis disposent d’un accès en lecture anonyme
...
| Références              | [Gestion de l’accès en lecture anonyme aux conteneurs et aux objets blob](https://azure.microsoft.com/documentation/articles/storage-manage-access-to-resources/), [Utilisation des signatures d’accès partagé (SAP)](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/) |
...
## <a name="a-idlimited-access-sasagrant-limited-access-to-objects-in-azure-storage-using-sas-or-sap"></a><a id="limited-access-sas"></a>Accorder un accès limité aux objets dans le stockage Azure à l’aide de SAS ou SAP
