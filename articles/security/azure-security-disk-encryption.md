...
4. Azure met à jour le modèle de service de machine virtuelle et la machine virtuelle Iaas est marquée comme déchiffrée. Le contenu de la machine virtuelle n’est plus chiffré au repos.
5. La désactivation du chiffrement ne supprime ni le coffre de clés ni le matériel de clé de chiffrement du client (clés de chiffrement BitLocker pour Windows ou phrase secrète pour Linux).
...
> [AZURE.NOTE] L’authentification par certificat AAD n’est actuellement pas prise en charge sur les machines virtuelles Linux.
