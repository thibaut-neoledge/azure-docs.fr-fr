...
Vous trouverez ci-dessous un exemple de campagne marketing numérique multi-canal globale utilisant App Service Web Apps. Il montre les possibilités offertes par l'utilisation d'App Service Web Apps avec d'autres services pour un investissement technique minimal. **Cliquez sur un élément dans la topographie pour en savoir plus à ce sujet.** 

<object type="image/svg+xml" data="https://sidneyhcontent.blob.core.windows.net/documentation/digital-marketing-notitle.svg" width="100%" height="100%"></object>

> [AZURE.NOTE]
> Ce guide présente certaines des zones et des tâches les plus courantes correspondant à l'exécution d'une campagne marketing numérique dans Azure App Service Web Apps. Toutefois, il existe d'autres solutions courantes que vous pouvez implémenter dans App Service Web Apps. Pour examiner ces solutions, consultez les autres guides relatifs à la [Présence sur le Web à l'international](web-sites-global-web-presence-solution-overview.md) et aux [Applications métier](web-sites-business-application-solution-overview.md).

## Créez de nouvelles ressources ou intégrez des ressources existantes
...
Azure Marketplace fournit des modèles à partir des systèmes de gestion de contenu (CMS) de site web populaires, tels que [Orchard], [Umbraco], [Drupal] et [WordPress]. Vous pouvez créer une application web à l'aide de votre configuration de CMS favorite. Vous pouvez faire votre choix parmi diverses bases de données principales en fonction de vos besoins, y compris [Base de données SQL Azure] et [MySQL].
...
## Utilisez des contenus multimédia enrichis pour atteindre tous les appareils

Enrichissez votre campagne marketing numérique avec d'autres services Azure :

-  Télécharger et diffuser des vidéos globalement avec [Azure Media Services]
-  Envoyer des courriers électroniques aux utilisateurs avec [service SendGrid dans Azure Marketplace]
-  Établir une présence sur les appareils Windows, iOS et Android grâce à [Mobile Services]
-  Envoyer des notifications Push à des millions d'appareils avec [Notification Hub]

## Pensez global

Pensez global en desservant des sites régionaux avec Azure Traffic Manager et en fournissant du contenu de manière ultra rapide avec Azure CDN.

Pour servir des clients internationaux dans leurs régions respectives, utilisez [Azure Traffic Manager] pour router les visiteurs vers les sites régionaux proposant les meilleures performances. Vous pouvez également répartir uniformément la charge du site entre plusieurs copies de votre application web hébergée dans plusieurs régions.

Proposez votre contenu statique de manière ultra rapide et globale aux utilisateurs en [intégrant votre application web avec Azure CDN]. Azure CDN met en cache le contenu statique dans le [nœud CDN] le plus proche de l'utilisateur, ce qui réduit la latence et les connexions à votre application web.
...
La capacité de [mise à l'échelle] d'App Service Web Apps est idéale pour faire face aux charges de travail imprévisibles des campagnes marketing numériques. Mettez votre application web manuellement à l'échelle via le [portail Azure](http://go.microsoft.com/fwlink/?LinkId=529715), par programmation via l'[API de gestion de service] ou des [scripts PowerShell] ou automatiquement via la fonctionnalité de mise à l'échelle automatique. Au niveau **Standard**, la fonctionnalité Échelle automatique vous permet de faire évoluer une application web automatiquement en fonction de l'utilisation du processeur. Elle vous permet dans le même temps d'optimiser la flexibilité et de réduire les coûts en mettant à l'échelle l'application web uniquement lorsque cela est nécessaire en fonction de l'activité des utilisateurs. Pour prendre connaissance des meilleures pratiques, consultez [Les](https://twitter.com/troyhunt) [10 conseils de Troy Hunt sur la mise à l'échelle rapide d'applications web avec Azure].

Rendez votre application web plus réactive avec le [Cache Redis Azure]. Utilisez-le pour mettre des données en cache à partir de bases de données principales et d'autres éléments tels que [l'état de session ASP.NET] et le [cache de sortie].

Maintenez la haute disponibilité de votre application web à l'aide d'[Azure Traffic Manager]. À l'aide de la méthode de **basculement**, Traffic Manager achemine automatiquement le trafic vers un site secondaire en cas de problème sur le site principal.

## Surveillez et analysez

Tenez-vous informé des performances de votre application web avec Azure ou des outils tiers. Recevez des alertes concernant les événements critiques liés à l'application web. Obtenez facilement des informations utilisateur grâce à Application Insight ou à l'analyse de journal web à partir de HDInsight. 

Obtenez un [aperçu rapide] des mesures de performances et des quotas de ressources actuels de l'application web dans le panneau de cette dernière sur le [portail Azure](http://go.microsoft.com/fwlink/?LinkId=529715). Pour avoir une vision globale de votre application en termes de disponibilité, de performances et d'utilisation, utilisez [Azure Application Insights] pour obtenir des informations rapides et importantes sur le dépannage, le diagnostic et l'utilisation. Vous pouvez également utiliser un outil tiers comme [New Relic] afin de fournir des données de surveillance avancées pour vos applications web.

Au niveau **Standard**, surveillez la réactivité de l'application et recevez des notifications par courrier électronique dès que votre application web ne répond plus. Pour plus d'informations, consultez la page [Réception des notifications d'alerte et gestion des règles de création d'alerte dans Azure].

## Autres ressources

- [Documentation App Service Web Apps](/services/app-service/web/)
...
[Service SendGrid dans Azure Marketplace]:sendgrid-dotnet-how-to-send-email.md
...
[intégrant votre application web avec Azure CDN]:cdn-websites-with-cdn.md 
...
[Les]:https://twitter.com/troyhunt
[10 conseils de Troy Hunt sur la mise à l'échelle rapide d'applications web avec Azure]:http://www.troyhunt.com/2014/09/10-things-i-learned-about-rapidly.html
[Cache Redis Azure]:/blog/2014/06/05/mvc-movie-app-with-azure-redis-cache-in-15-minutes/
[l'état de session ASP.NET]:https://msdn.microsoft.com/library/azure/dn690522.aspx
...
[Réception des notifications d'alerte et gestion des règles de création d'alerte dans Azure]:http://msdn.microsoft.com/library/windowsazure/dn306638.aspx
