---
title: "Déployer votre première application web Node.js dans Azure en 5 minutes | Microsoft Docs"
description: "Découvrez la facilité avec laquelle vous pouvez exécuter des applications web dans App Service en déployant un exemple d’application Node.js. Commencez le développement rapidement et visualisez les résultats immédiatement."
...
---
# <a name="deploy-your-first-nodejs-web-app-to-azure-in-five-minutes"></a>Déployer votre première application web Node.js dans Azure en 5 minutes
...
Dans ce didacticiel, vous découvrirez comment déployer votre première application web Node.js dans [Azure App Service](../app-service/app-service-value-prop-what-is.md).
...
* Déployer un exemple de code Node.js
...
- [Azure CLI 1.0](app-service-web-get-started-nodejs-cli-nodejs.md) : notre interface de ligne de commande pour les modèles de déploiement Classique et Resource Manager
- [Azure CLI 2.0 (version préliminaire)](app-service-web-get-started-nodejs.md) : notre interface de ligne de commande nouvelle génération pour le modèle de déploiement Resource Manager
...
## <a name="deploy-a-nodejs-web-app"></a>Déployer une application web Node.js
...
4. Accédez à un répertoire de travail (`CD`) et clonez l’exemple d’application Node.js comme ceci :
...
6. Créez la ressource d’application App Service dans Azure avec un nom d’application unique et l’utilisateur de déploiement que vous avez configuré précédemment. Lorsque vous y êtes invité, spécifiez le numéro de la région souhaitée.
    ...
    ![Créer la ressource Azure pour votre première application web dans Azure](./media/app-service-web-get-started-languages/node-site-create.png)
...
7. Déployez votre exemple de code dans votre application Azure de la même façon que vous transmettez du code avec Git. Lorsque vous y êtes invité, utilisez le mot de passe que vous avez configuré précédemment.
    ...
    ![Transmettez du code pour votre première application web dans Azure](./media/app-service-web-get-started-languages/node-git-push.png)
   
    `git push` injecte non seulement du code dans Azure, mais déclenche également des tâches de déploiement dans le moteur de déploiement. 
    Si vous avez un fichier package.json à la racine de votre projet (référentiel), le script de déploiement restaure les packages requis pour vous. 
...
[Créez, configurez et déployez une application web Node.js Express dans Azure](app-service-web-nodejs-get-started-cli-nodejs.md). En suivant ce didacticiel, vous acquerrez les compétences de base nécessaires pour exécuter n’importe quelle application web Node.js dans Azure. Vous apprendrez notamment à :
...
* Automatiser des tâches à l’aide de NPM
* Obtenir les journaux d’erreurs et de sortie
