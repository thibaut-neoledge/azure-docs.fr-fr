...
De plus, assurez-vous que les informations d'identification importées sont sécurisées. Par exemple, **Azure PowerShell** et l’**interface de ligne de commande interplateforme Azure** stockent les informations importées dans votre **répertoire d’accueil** (*\~* sous les systèmes Linux et OS X et */utilisateurs/nomutilisateur* sous les systèmes Windows.) Pour plus de sécurité, vous pouvez **chiffrer** ces emplacements à l'aide d'outils de chiffrement disponibles pour votre système d'exploitation.
...
Les **paramètres d’application** et **chaînes de connexion** sont configurables via le [portail Azure](http://go.microsoft.com/fwlink/?LinkId=529715) ou des utilitaires tels que PowerShell ou l’interface de ligne de commande interplateforme Azure.
