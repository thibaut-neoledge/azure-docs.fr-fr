...
* Visual Studio 2015 : vous pouvez télécharger l’[édition Community](https://www.visualstudio.com/en-us/products/visual-studio-express-vs.aspx).
...
4. Lorsque le clone local est créé, accédez à *&lt;racine_référentiel>*\ARMTemplates, puis exécutez le script deploy.ps1 avec un suffixe unique, comme indiqué ci-dessous :
...
7. Lorsque le script se termine, revenez en arrière pour accéder à l’adresse du serveur frontal (http://ToDoApp*&lt;votre_suffixe>*.azurewebsites.net/) afin d’afficher l’application qui s’exécute en production.
...
3. Cliquez avec le bouton droit sur  **MultiChannelToDo.Web** > **Ajouter Application Insights Telemetry** > **Configurer les paramètres** > Modifier le groupe de ressources en ToDoApp*&lt;votre_suffixe>* > **Ajouter Application Insights au projet**.
...
5. Ajoutez le code d’instrumentation JS copié à *&lt;racine_référentiel>*\src\MultiChannelToDo.Web\app\Index.cshtml, juste avant la balise de fermeture `<heading>`. Il doit contenir la clé d’instrumentation unique de votre ressource Application Insights.
...
1. Cliquez avec le bouton droit sur **MultiChannelToDo** > **Ajouter Application Insights Telemetry** > **Configurer les paramètres** > Modifier le groupe de ressources en ToDoApp*&lt;votre_suffixe>* > **Ajouter Application Insights au projet**.
...
## <a name="investigate-add-slot-specific-tags-to-your-client-app-metrics"></a>Enquête : Ajouter des balises spécifiques aux emplacements à vos mesures d’application cliente
...
### <a name="add-slot-specific-tags-to-your-server-app-metrics"></a>Ajouter des balises spécifiques aux emplacements à vos mesures d’application serveur
...
1. Ouvrez *&lt;racine_référentiel >*\src\MultiChannelToDo\Global.asax.cs. Ajoutez le bloc de code ci-dessous, juste avant l’accolade fermante d’espace de noms.
...
1. Ouvrez *&lt;racine_référentiel>*\ARMTemplates\ProdAndStagetest.json et trouvez les ressources `appsettings` (recherchez `"name": "appsettings"`). Elles sont au nombre de 4, une pour chaque emplacement.
...
## <a name="update-push-your-updates-to-the-beta-app"></a>Mise à jour : Envoyer vos mises à jour à l’application bêta
Revenons à votre application que vous souhaitez améliorer.

1. Vérifiez que vous êtes à présent dans votre branche bêta

        git checkout beta
2. Dans *&lt;racine_référentiel>*\src\MultiChannelToDo.Web\app\Index.cshtml, trouvez la balise `<li>` et ajoutez l’attribut `style="cursor:pointer"`, comme indiqué ci-dessous.

    ![](./media/app-service-web-test-in-production-controlled-test-flight/07-change-cursor-style-on-li.png)
3. Validez et envoyez vers Azure.
4. Vérifiez que la modification est désormais reflétée dans l’emplacement bêta en accédant à *&lt;votre_suffixe>*-beta.azurewebsites.net/. Si vous ne voyez pas encore la modification, actualisez votre navigateur pour obtenir le nouveau code JavaScript.
...
## <a name="validate-route-traffic-to-the-beta-app"></a>Validation : Acheminer le trafic vers l’application bêta
...
2. Maintenant, accédez à http://ToDoApp*&lt;votre_suffixe>*.azurewebsites.net. 50 % du trafic doit maintenant être redirigé vers l’emplacement bêta.
