...
Enfin, il s’agit de permettre à l’application WordPress d’utiliser des chemins d’accès relatifs. WordPress stocke les informations d’URL dans la base de données. Le déplacement du contenu d’un environnement vers un autre devient alors plus difficile, car vous devez mettre à jour la base de données à chaque fois que vous passez d’un environnement local à un environnement intermédiaire ou d’un emplacement intermédiaire à un environnement de production. Pour réduire les risques de problèmes liés au déploiement de base de données à chaque déploiement d’un environnement vers un autre, utilisez le [complément de liens à la racine relative](https://wordpress.org/plugins/root-relative-urls/) qui peut être installé à l’aide du tableau de bord Administrateur WordPress ou effectuez un téléchargement manuel à partir [ici](https://downloads.wordpress.org/plugin/root-relative-urls.zip).
...
Parcourir et tester votre application web intermédiaire. Si l’on prend en compte un scénario où le thème de l’application web doit être mis à jour, voici l’application web intermédiaire.
...
![Parcourir l’application web intermédiaire avant d’échanger les emplacements](./media/app-service-web-staged-publishing-realworld-scenarios/5wpstage.png)
 ...
 Si tout vous semble correct, cliquez sur le bouton **Échange** des paramètres d’application web intermédiaire pour déplacer le contenu de votre environnement de production. Dans ce cas, vous échangez l’application web et la base de données d’un environnement à l’autre pendant chaque opération d’**échange**.
...
Si vous devez procéder à une **restauration** , vous pouvez accéder aux paramètres de l’application web de production, puis cliquer sur le bouton **Échange** pour basculer l’application web et la base de données de l’emplacement de production à l’emplacement intermédiaire. Il est essentiel de retenir que, si des modifications de base de données sont incluses dans une opération d’**échange** à un moment donné, lors du redéploiement de l’application web intermédiaire suivant, vous devez déployer les modifications apportées à la base de données actuellement référencée par votre application web intermédiaire, et qui peut être la base de données de production ou intermédiaire précédente.
...
### Umbraco
Dans cette section, vous allez apprendre comment un serveur de gestion de contenus Umbraco (CMS) utilise des modules personnalisés pour exécuter des déploiements sur plusieurs environnements DevOps. Cet exemple propose une approche différente de la gestion des environnements de développement multiples.
...
## Références
[Développement logiciel agile avec Azure App Service](app-service-agile-software-development)

[Configurer des environnements intermédiaires pour les applications web dans Azure App Service](web-sites-staged-publishing)
