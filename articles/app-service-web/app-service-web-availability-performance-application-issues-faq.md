...
Pour déterminer si vous utilisez Cache local, ouvrez l’onglet **Paramètres d’application** d’App Service. Si la fonctionnalité Cache local est utilisée, le paramètre d’application `WEBSITE_LOCAL_CACHE_OPTION` est défini sur `Always`. Pour plus d’informations sur Cache local, consultez la [vue d’ensemble de la fonctionnalité Cache local d’App Service](https://docs.microsoft.com/azure/app-service/app-service-local-cache).
