...
- [Azure CLI 2.0](app-service-web-nodejs-get-started.md) : notre interface de ligne de commande nouvelle génération pour le modèle de déploiement Resource Manager
...
4. Passez en mode ASM et connectez-vous à Azure (vous avez besoin de [l’interface de ligne de commande Azure](#prereq)) :
...
5. Assurez-vous que vous vous trouvez toujours dans le répertoire racine de votre application, puis créez la ressource d’application App Service dans Azure avec un nom d’application unique à l’aide de la commande suivante. Par exemple : http://{appname}.azurewebsites.net
    ...
    Suivez les instructions pour sélectionner une région Azure vers laquelle effectuer le déploiement. 
...
8. Enregistrez vos modifications, puis utilisez git pour déployer votre application dans Azure. Lorsque vous y êtes invité, utilisez les informations d’identification utilisateur créées précédemment.
