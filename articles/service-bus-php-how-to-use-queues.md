...
> [WACOM.NOTE] L'[extension OpenSSL](http://php.net/openssl) doit également être installée et activée dans votre installation PHP.
...
> [WACOM.NOTE] Vous pouvez utiliser la méthode **listQueues** sur les objets **ServiceBusRestProxy** pour vérifier s'il existe déjà une file d'attente d'un nom déterminé dans un espace de noms de service.
...
L'exemple ci-dessous montre comment les messages peuvent être reçus et traités avec le mode **PeekLock** (différent du mode par défaut).
    ...
    try {
        // Définissez la méthode de réception sur PeekLock (méthode par défaut : ReceiveAndDelete).
