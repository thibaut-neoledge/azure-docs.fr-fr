<properties" pageTitle="Utilisation de plusieurs clients avec un seul serveur principal de service mobile | Azure Mobile" metaKeywords="mobile services single backend" description="Learn how to use a single mobile service back-end from multiple client apps that target different mobile platforms, including Windows Store and Windows Phone." metaCanonical="" services="mobile-services,notification-hubs" documentationCenter="Mobile" title="Considerations for supporting multiple clients from a single mobile service" authors="glenga" solutions="" manager="dwrede" editor="mollybos" />
...
Pour plus d'informations sur Mobile Services, consultez la rubrique [Centre de développement Mobile Services](/fr-fr/documentation/services/mobile-services/).
...
>[WACOM.NOTE]Notification Hubs ne prend pas en charge actuellement l'utilisation des services WNS pour envoyer des notifications Push aux applications Silverlight Windows Phone 8.1. Vous devez utiliser les services de notifications Push Microsoft (MPNS) pour envoyer des notifications vers les applications Silverlight et Windows Phone 7.0 et 8.0.
...
>[WACOM.NOTE]Une erreur s'est produite pendant la tentative d'envoi d'un message à une plateforme d'appareil native pour laquelle aucune inscription d'appareil n'existe. Cette erreur ne se produit pas lorsque vous envoyez des notifications de modèle.

Les tableaux des sections suivantes renvoient vers les didacticiels relatifs aux clients, qui montrent comment implémenter des notifications Push à partir de services mobiles principaux .NET et JavaScript.  

###Service principal .NET

Dans un service mobile principal .NET, vous envoyez des notifications en appelant la méthode [SendAsync] sur l'objet [PushClient](http://msdn.microsoft.com/fr-fr/library/azure/microsoft.windowsazure.mobile.service.notifications.pushclient.aspx) obtenu de la propriété [ApiServices.Push](http://msdn.microsoft.com/fr-fr/library/azure/microsoft.windowsazure.mobile.service.apiservices.push.aspx). La notification Push envoyée (native ou de modèle) dépend de l'objet spécifique dérivé de [IPushMessage](http://msdn.microsoft.com/fr-fr/library/azure/microsoft.windowsazure.mobile.service.notifications.ipushmessage.aspx) qui est passé à la méthode [SendAsync], comme le montre le tableau suivant : 
...
Quand vous utilisez des inscriptions clientes de modèle plutôt que des inscriptions clientes natives, vous pouvez envoyer la même notification par un appel unique à [SendAsync], en fournissant un objet [TemplatePushMessage] comme suit : 
...
Dans un service mobile principal JavaScript, vous pouvez envoyer des notifications en appelant la méthode **send** sur l'objet propre à la plateforme obtenu à partir de l'[objet push] global, comme le montre le tableau suivant : 
...
|-----|-----|----|----|-----|
|Native|[objet apns](http://msdn.microsoft.com/fr-fr/library/azure/jj839711.aspx)   |[gcm objet](http://msdn.microsoft.com/fr-fr/library/azure/dn126137.aspx)     |[objet wns](http://msdn.microsoft.com/fr-fr/library/azure/jj860484.aspx) | [objet mpns](http://msdn.microsoft.com/fr-fr/library/azure/jj871025.aspx) |
...
+ [**Xamarin**](https://go.microsoft.com/fwLink/p/?LinkID=330242)<br/>Xamarin vous permet de créer des applications complètement natives pour les appareils iOS et Android, dotées d'une interface utilisateur entièrement native et d'un accès à toutes les ressources d'appareil. Les applications Xamarin sont codées à l'aide du langage C#, et non des langages Objective-C et Java. Ceci permet aux développeurs .NET de publier des applications vers iOS et Android et de partager le code depuis des projets Windows. Xamarin fournit une expérience utilisateur entièrement native sur les appareils iOS et Android à l'aide du code C#. Ceci vous permet de réutiliser certains de vos codes Mobile Services provenant d'applications Windows sur des appareils iOS et Android. Pour plus d'informations, consultez la rubrique [Développement Xamarin](#xamarin) ci-dessous. 
	...
	Vous pouvez créer des applications Xamarin dans Xamarin Studio ou Visual Studio 2013. Pour plus d'informations, consultez la page [Développement interplateforme dans Visual Studio](http://msdn.microsoft.com/fr-fr/library/dn771552.aspx).
...
>[WACOM.NOTE]Pour utiliser une authentification cliente unique à l'aide du compte Microsoft dans les applications Windows Runtime et Windows Phone Silverlight, vous devez d'abord inscrire l'application Windows Runtime sur le tableau de bord Windows Store. En effet, une fois que vous créez une inscription Live Connect pour Windows Phone, vous ne pouvez pas en créer une pour Windows Store. Pour plus d'informations sur la marche à suivre, lisez la rubrique **Authentification unique de votre application du Windows Store avec Live Connect** ([Windows Store][Authentification unique Windows Store]/[Windows Phone][Authentification unique Windows Phone]).


<!-- URLs -->
[Portail de gestion Azure]: https://manage.windowsazure.com
[Azure Notification Hubs]: /fr-fr/develop/net/how-to-guides/service-bus-notification-hubs/
[Authentification unique Windows Store]: /fr-fr/develop/mobile/tutorials/single-sign-on-windows-8-dotnet/
[Authentification unique Windows Phone]: /fr-fr/develop/mobile/tutorials/single-sign-on-wp8/
[Didacticiels et ressources]: /fr-fr/develop/mobile/resources/
[Prise en main de Notification Hubs]: /fr-fr/manage/services/notification-hubs/getting-started-windows-dotnet/
[Envoi de notifications interplateformes aux utilisateurs]: /fr-fr/manage/services/notification-hubs/notify-users-xplat-mobile-services/
[Prise en main de Windows dotnet push]: /fr-fr/develop/mobile/tutorials/get-started-with-push-dotnet-vs2012/
[Prise en main de Windows js push]: /fr-fr/develop/mobile/tutorials/get-started-with-push-js-vs2012/
[Prise en main de Windows Phone push]: /fr-fr/develop/mobile/tutorials/get-started-with-push-wp8/
[Prise en main d'iOS push]: /fr-fr/develop/mobile/tutorials/get-started-with-push-ios/
[Prise en main d'Android push]: /fr-fr/develop/mobile/tutorials/get-started-with-push-android/
[Schéma dynamique]: http://msdn.microsoft.com/fr-fr/library/windowsazure/jj193175.aspx
[Utilisation d'un client .NET pour Mobile Services]: fr-fr/documentation/articles/mobile-services-windows-dotnet-how-to-use-client-library/
