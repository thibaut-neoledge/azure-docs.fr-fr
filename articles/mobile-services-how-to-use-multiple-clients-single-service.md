...
+ [Notifications Push interplateformes](#push)
+ [Développement d'applications interplateformes](#xplat-app-dev)
+ [Partage de code dans les projets Visual Studio](#shared-vs)

Pour plus d'informations sur Mobile Services, consultez le [Centre de développement Mobile Services](/fr-fr/documentation/services/mobile-services/).

##<a id="push"></a>Notifications Push multiplateformes
...
Dans un service mobile principal .NET, vous pouvez envoyer des notifications en envoyant la méthode [SendAsync] sur l'objet [PushClient](http://msdn.microsoft.com/fr-fr/library/azure/microsoft.windowsazure.mobile.service.notifications.pushclient.aspx) obtenu de la propriété [ApiServices.Push](http://msdn.microsoft.com/fr-fr/library/azure/microsoft.windowsazure.mobile.service.apiservices.push.aspx) . La notification Push envoyée (native ou modèle) dépend de l'objet [IPushMessage](http://msdn.microsoft.com/fr-fr/library/azure/microsoft.windowsazure.mobile.service.notifications.ipushmessage.aspx) transmis à la méthode [SendAsync], comme indiqué dans le tableau suivant : 
...
##<a id="xplat-app-dev"></a>Développement d'applications multiplateformes
...
>[WACOM.NOTE]La version C# du projet d'application de démarrage rapide du portail partage la page code-behind MainPage.xaml.cs, mais n'utilise pas de modèle de vue. Pour obtenir un exemple de l'application TodoList en tant que projet d'application Windows universelle en C# qui utilise le modèle MVVM, consultez la page [Projet d'application Windows universelle pour Azure Mobile Services à l'aide du modèle MVVM]. 
...
>[WACOM.NOTE][WACOM.NOTE]Pour utiliser l'authentification unique du client avec un compte Microsoft dans des applications Windows Runtime et Windows Phone Silverlight, vous devez d'abord inscrire l'application Windows Runtime sur le tableau de bord Windows Store. En effet, une fois que vous créez une inscription Live Connect pour Windows Phone, vous ne pouvez pas en créer une pour Windows Store. Pour plus d'informations sur la marche à suivre, lisez la rubrique **Authentification unique de votre application Windows Store avec Live Connect** ([Windows Store][SSO Windows Store]/[Windows Phone][SSO Windows Phone]).
...
[Prise en main des notifications Push Windows dotnet]: /fr-fr/develop/mobile/tutorials/get-started-with-push-dotnet-vs2012/
[Prise en main des notifications Push Windows js]: /fr-fr/develop/mobile/tutorials/get-started-with-push-js-vs2012/
