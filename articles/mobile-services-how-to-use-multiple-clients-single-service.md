...
+ [Notifications Push multiplateformes](#push)
+ [Développement d'applications multiplateformes](#xplat-app-dev)
...
Le code suivant envoie une notification Push depuis un service principal .NET vers toutes les inscriptions d'appareils iOS et Windows Store : 
	...
	// Envoyez des notifications Push à tous les appareils iOS et Windows Store inscrits. 
...
Quand vous utilisez des inscriptions clientes de modèle plutôt que des inscriptions clientes natives, vous pouvez envoyer la même notification par un appel unique à [SendAsync], en fournissant un objet [TemplatePushMessage] comme suit : 
	...
	// Créez un nouveau message de modèle et ajoutez le paramètre 'message'.    
	var templatePayload = new TemplatePushMessage();
    templatePayload.Add("message", item.Text);

	// Envoyez une notification Push à toutes les inscriptions de modèle.
...
Le code suivant envoie une notification Push à toutes les inscriptions Android et Windows Phone : 
	...
	// Définissez une notification Push pour GCM.
	var gcmPayload = 
    '{"data":{"message" : item.text }}';

	// Définissez la charge utile pour une notification toast Windows Phone.
	var mpnsPayload = '<?xml version="1.0" encoding="utf-8"?>' +
    '<wp:Notification xmlns:wp="WPNotification"><wp:Toast>' +
    '<wp:Text1>New Item</wp:Text1><wp:Text2>' + item.text + 
    '</wp:Text2></wp:Toast></wp:Notification>';

	//  Envoyez des notifications Push à tous les appareils Android et Windows Phone 8.0 inscrits. 
...
Lorsque vous utilisez des inscriptions clientes de modèle plutôt que des inscriptions clientes natives, vous pouvez envoyer la même notification par un appel unique vers la fonction **send** de l'[objet push global], en fournissant une charge utile de messages de modèle, comme suit : 
	...
	// Créez un nouveau message de modèle avec le paramètre 'message'.    
...
[Prise en main des notifications Push Windows dotne]: /fr-fr/develop/mobile/tutorials/get-started-with-push-dotnet-vs2012/
