<properties urlDisplayName="Active Directory forest" pageTitle="Installation d'une forêt Active Directory sur un réseau virtuel Azure" metaKeywords="" description="Didacticiel qui explique comment créer une forêt Active Directory sur une machine virtuelle d'Azure Virtual Network." metaCanonical="" services="active-directory,virtual-network" documentationCenter="" title="Install a new Active Directory forest in Azure" authors="Justinha"  solutions="" writer="Justinha" manager="TerryLan" editor="LisaToft"  />
...
Pour configurer...  | Localement  | Réseau virtuel Azure	
------------- | -------------  | ------------
**Adresse IP pour le contrôleur de domaine**  | Attribuez une adresse IP statique aux propriétés d'adaptateur réseau   | Exécutez l'applet de commande Set-AzureStaticVNetIP pour attribuer une adresse IP statique
...
2. Créez un réseau virtuel. Cliquez sur <b>Réseaux</b> > <b>Create a virtual network</b>. Utilisez les valeurs du tableau suivant pour compléter l'Assistant. 
	...
	**Espaces d'adressage du réseau virtuel**  | <p>Nom de sous-réseau : entrez le nom du sous-réseau</p><p>Adresse IP de début : <b>10.0.0.0</b></p><p>CIDR : <b>/24 (256)</b></p>
...
1. Cliquez sur <b>Nouveau</b> > <b>Calcul</b> > <b>Machine virtuelle</b> > <b>À partir de la galerie</b>. 
2. Utilisez les valeurs du tableau suivant pour compléter l'Assistant.
	...
	**Configuration de machine virtuelle**  | <p>Date de lancement : date du jour</p><p>Nom de la machine : spécifiez une valeur unique</p><p>Niveau : Standard</p><p>Taille : sélectionnez une taille</p><p>Nom d'utilisateur : entrez un nom. Ce compte d'utilisateur sera membre du groupe Administrateurs intégré. </p><p>Mot de passe : doit comporter au moins 8 caractères, et inclure 3 types parmi les types de caractères suivants :</p><ul><li>une majuscule</li><li>une minuscule</li><li>un chiffre</li><li>un caractère spécial</li></ul>
	**Service cloud**  | <p>Service de cloud computing : <b>créez un service cloud</b></p><p>Nom du service cloud : Acceptez la valeur par défaut</p><p>Région/Groupe d'affinités/Réseau virtuel : sélectionnez le réseau virtuel que vous avez créé</p><p>sous-réseau du réseau virtuel : sélectionnez le sous-réseau que vous avez créé. </p><p>Compte de stockage : <b>Utiliser un compte de stockage généré automatiquement</b></p><p>Groupe à haute disponibilité : <b>Aucun</b></p><p>Points de terminaison : acceptez les valeurs par défaut</p>
	**Agent de machine virtuelle**  | Sélectionnez **Installer l'agent de machine virtuelle**

1. L'adresse IP dynamique attribuée par défaut à la machine virtuelle est valide pendant la durée du service cloud. Mais elle change lorsque la machine virtuelle est arrêtée. Attribuez une adresse IP statique en [exécutant la cmdlet Set-AzureStaticVNetIP Azure PowerShell](http://msdn.microsoft.com/fr-fr/library/windowsazure/dn630228.aspx) afin que l'adresse IP soit conservée si vous devez arrêter la machine virtuelle à un moment donné. 
...
3. Connectez-vous à la machine virtuelle et formatez l'autre disque. 
  4. Cliquez sur <b>Se connecter</b> pour vous connecter à la machine virtuelle, sur <b>Ouvrir</b> pour créer une session RDP, puis à nouveau sur <b>Se connecter</b>.
  4. Remplacez les informations d'identification par les nouveaux nom d'utilisateur et mot de passe spécifiés.
  5. Dans Gestionnaire de serveur, cliquez sur <b>Outils</b> > <b>Gestion de l'ordinateur</b>. 
  6. Cliquez sur <b>Gestion des disques</b> et sur <b>Ok</b> pour initialiser le nouveau disque. 
...
3. Sélectionnez la machine virtuelle et cliquez sur <b>Redémarrer</b> pour que la machine virtuelle configure les paramètres de résolution DNS avec l'adresse IP du nouveau serveur DNS. 
