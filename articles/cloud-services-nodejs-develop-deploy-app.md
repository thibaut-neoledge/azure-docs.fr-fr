...
À la fin de ce guide, vous disposerez d'une application Node.js simple qui s'exécute
dans un service cloud Azure. Les services de cloud computing sont les blocs
de construction des applications cloud extensibles dans Azure. Ils permettent de séparer, de gérer et d'étendre
de façon indépendante le composant frontal et le composant principal de votre application. Les services cloud
...
<p>Si votre sc&eacute;nario ne comporte qu'un simple composant frontal Web, envisagez d'utiliser un <a href="/fr-fr/documentation/articles/web-sites-nodejs-develop-deploy-mac/">site Web Azure l&eacute;ger.</a> Vous pouvez facilement passer &agrave; un service cloud en fonction de l'&eacute;volution de votre site et de vos besoins.</p>
</div>

Dans ce didacticiel, vous allez créer une application Web simple, hébergée dans un rôle Web. Vous
allez utiliser l'émulateur de calcul pour tester votre application en local, puis vous allez la
déployer à l'aide d'outils de ligne de commande PowerShell.
...
3.  Entrez la cmdlet suivante pour créer une solution :
    ...
    ![Le résultat de la nouvelle commande helloworld New-AzureService][]
    ...
    -   **ServiceConfiguration.Cloud.cscfg**,
        **ServiceConfiguration.Local.cscfg** et **ServiceDefinition.csdef** sont des
        fichiers propres à Azure, nécessaires à la publication de votre
        application.
    ...
    -   **deploymentSettings.json** stocke les paramètres locaux utilisés par
        les cmdlets de déploiement Azure PowerShell.

4.  Entrez la commande suivante pour ajouter un nouveau rôle Web à l'aide de la cmdlet
    **Add-AzureNodeWebRole** :
    ...
    ![Résultat de la commande Add-AzureNodeWebRole.][]
...
<b>Remarque</b>
<p>Par d&eacute;faut, si vous n'indiquez pas de nom de r&ocirc;le, un nom est cr&eacute;&eacute;. Vous pouvez indiquer un nom comme premier param&egrave;tre de <b>Add-AzureNodeWebRole</b>. Par exemple&nbsp;: <code data-inline="1">Add-AzureNodeWebRole MyRole</code></p>
</div>
...
5.  Utilisez les commandes suivantes pour accéder au répertoire **WebRole1**, puis ouvrez le fichier **server.js** dans le Bloc-notes.
    ...
    Le fichier **server.js** a été créé par la cmdlet **Add-AzureNodeWebRole** et il contient le code de démarrage suivant. Ce code est similaire à l'exemple « Hello World » du site Web [nodejs.org][], à l'exception des points suivants :
...
Un des outils installés par le Kit de développement logiciel (SDK) Azure
est l'émulateur de calcul, qui vous permet de tester votre application en local. Il simule
l'environnement dans lequel votre application sera exécutée lors de son
déploiement dans le cloud. Effectuez les étapes suivantes pour tester l'application dans l'émulateur.

1.  Fermez le Bloc-notes et revenez à la fenêtre Windows PowerShell.
    Entrez la cmdlet suivante pour exécuter votre service dans l'émulateur :
    ...
    ![Navigateur Web avec la page Web Hello World][]
...
### <span id="download_publishing_settings"></span></a>Téléchargement des paramètres de publication Azure
...
2.  Dans la fenêtre Azure Windows PowerShell, utilisez la cmdlet suivante pour configurer les cmdlets Windows PowerShell pour Node.js afin qu'elles utilisent le profil de publication Azure que vous avez téléchargé :
    ...
    <div class="dev-callout">
<b>Remarque</b>
<p>Apr&egrave;s l'importation des param&egrave;tres de publication, pensez &agrave; supprimer le fichier .publishSettings t&eacute;l&eacute;charg&eacute;, car il contient des informations pouvant &ecirc;tre utilis&eacute;es par d'autres personnes pour acc&eacute;der &agrave; votre compte.</p>
</div>

### Publication de l'application

1.  Publiez l'application à l'aide de la cmdlet **Publish-AzureServiceProject**,
    ...
    ![Résultat de la commande Publish-AzureService.][]
...
4.  Elle crée un **service cloud**, si celui-ci n'existe pas. Le **service cloud** est le conteneur dans lequel votre application est hébergée lorsqu'elle est déployée sur Azure. Pour plus d'informations, consultez la page [Présentation de la création d'un service hébergé pour Azure][].
...
1.  Dans la fenêtre Windows PowerShell, arrêtez le déploiement du service créé dans la section précédente à l'aide de la cmdlet suivante :
    ...
    ![État de la commande Stop-AzureService.][]
...
2.  Pour supprimer le service, utilisez la cmdlet suivante :
    ...
    ![État de la commande Remove-AzureService][]

    <div class="dev-callout">
<strong>Remarque</strong>
<p>La suppression du service ne supprime pas le compte de stockage qui a &eacute;t&eacute; cr&eacute;&eacute; lors de la publication initiale du service. Le stockage utilis&eacute; continuera &agrave; vous &ecirc;tre factur&eacute;. Pour plus d'informations sur la suppression d'un compte de stockage, consultez la page <a href="http://msdn.microsoft.com/fr-fr/library/windowsazure/hh531562.aspx">Suppression d'un compte de stockage d'un abonnement Azure</a>.
</p>
</div>
  ...
  [site Web Azure léger.]: /fr-fr/documentation/articles/web-sites-nodejs-develop-deploy-mac/
  ...
  [A command prompt displaying the commands 'mkdir c:\\node' and 'cd node'.]: ./media/cloud-services-nodejs-develop-deploy-app/getting-started-6.png
  ...
  [Suppression d'un compte de stockage d'un abonnement Azure]: http://msdn.microsoft.com/fr-fr/library/windowsazure/hh531562.aspx
