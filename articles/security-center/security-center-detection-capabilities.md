...
Face au problème, les organisations déploient souvent plusieurs solutions, axées sur la défense du périmètre de l’entreprise ou de points de terminaison en recherchant des signatures d’attaques connues. Ces solutions ont tendance à générer un volume élevé d’alertes basse fidélité, qui nécessitent qu’un analyste les trie et les examine. La plupart des organisations ne disposent pas du temps et de l’expertise nécessaires pour répondre à ces alertes, qui restent alors bien souvent sans réponse. Parallèlement, les pirates ont fait évoluer leurs méthodes pour compromettre les nombreuses défenses basées sur la signature et pour [s’adapter aux environnements de cloud](https://azure.microsoft.com/blog/detecting-threats-with-azure-security-center/). De nouvelles approches sont nécessaires pour identifier plus rapidement les nouvelles menaces, et accélérer la détection et la réaction.
...
Azure Security Center emploie des analyses de sécurité avancées allant bien au-delà des approches simplement basées sur la signature. Les innovations en matière de Big Data et de technologies [d’apprentissage automatique](https://azure.microsoft.com/blog/machine-learning-in-azure-security-center/) sont utilisées pour évaluer des événements sur l’intégralité de l’infrastructure de cloud, détectant des menaces qui seraient impossibles à identifier à l’aide de méthodes manuelles et prédisant l’évolution des attaques. Ces analyses de sécurité comprennent les éléments suivants :

- **Intelligence des menaces intégrée** : recherche les éléments malveillants en exploitant des informations globales concernant des menaces provenant de produits et services Microsoft, de Microsoft Digital Crimes Unit (DCU), de Microsoft Security Response Center (MSRC) et de sources externes.
...
- **Attaques en force brute RDP/SSH entrantes** : vos déploiements peuvent comporter des machines virtuelles prises par un nombre plus ou moins important de connexions quotidiennes. Azure Security Center peut déterminer l’activité de connexion de base de ces machines virtuelles et utiliser l’apprentissage automatique pour définir ce qui ne correspond pas à une activité normale. Si le nombre de connexions, l’heure de connexion, l’emplacement à partir duquel les connexions sont demandées, ou autres caractéristiques relatives à la connexion diffèrent considérablement de la ligne de base, une alerte peut être générée. Là encore, l’apprentissage automatique détermine ce qui est significatif.
...
- **Spécialistes de la sécurité Microsoft** : engagement continu avec les équipes Microsoft spécialisées dans les domaines de la sécurité, tels que l’analyse et la détection d’attaques web.
