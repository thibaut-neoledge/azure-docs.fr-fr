...
Ce forum aux questions répond aux questions concernant le Centre de sécurité Azure.
...
### Qu’est-ce qu’une recommandation de sécurité ?
 Le Centre de sécurité analyse l’état de sécurité de vos ressources Azure. Lorsqu’il identifie des failles de sécurité potentielles, il crée des recommandations. Les recommandations vous guident tout au long du processus de configuration du contrôle. Voici quelques exemples :

- Approvisionnement de logiciels anti-programmes malveillants pour identifier et supprimer les programmes malveillants
- Configuration de [groupes de sécurité réseau](../virtual-network/virtual-networks-nsg.md) et de règles associées pour contrôler le trafic vers les machines virtuelles
- Approvisionnement de pare-feu d’applications web pour se défendre des attaques ciblant vos applications web
...
- Des alertes de sécurité émises par des solutions de sécurité partenaires intégrées, telles que des logiciels anti-programmes malveillants ou des pare-feu d’applications web.

### Comment les autorisations sont-elles gérées dans le Centre de sécurité ?
 Le Centre de sécurité prend en charge l’accès en fonction du rôle. Pour plus d’informations sur le contrôle d’accès en fonction du rôle, consultez [Contrôle d’accès en fonction du rôle Azure Active Directory](../active-directory/role-based-access-control-configure.md).

Lorsque les utilisateurs ouvrent le Centre de sécurité, ils ne peuvent voir que les recommandations et alertes liées aux ressources auxquelles ils ont accès. Autrement dit, ils voient uniquement les éléments liés aux ressources pour lesquelles ils possèdent le rôle de propriétaire, collaborateur ou lecteur, pour l’abonnement ou le groupe de ressources auquel appartient une ressource.

Pour modifier une stratégie de sécurité, vous devez disposer du rôle de propriétaire ou de collaborateur pour l’abonnement concerné.
