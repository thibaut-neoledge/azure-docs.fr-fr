...
1. Dans le menu principal de **Security Center**, sélectionnez **Événements traités**.
2. Le sélecteur d’espaces de travail **Événements traités** peut s’ouvrir. Si vous avez un seul espace de travail, ce sélecteur d’espaces de travail n’apparaît pas. Si vous avez plusieurs espaces de travail, vous devez sélectionner un espace de travail pour afficher les détails de son événement traité. Sélectionnez un espace de travail dans la liste, si vous avez plusieurs espaces de travail.

  ![Liste d’espaces de travail][3]

3. Le tableau de bord **Événements traités** s’ouvre pour afficher les détails de l’événement de l’espace de travail sélectionné. Vous pouvez afficher les événements notables et tous les événements par type.  Dans cet exemple, nous avons sélectionné **Notable events** (Événements notables).
...
Votre espace de travail doit exécuter Log Analytics version 2 et se trouver dans le niveau Standard de Security Center pour utiliser le traitement des événements dans Security Center. Le sélecteur d’espaces de travail **Événements** identifie les espaces de travail qui ne répondent pas à ces exigences.
...
2. **Le tableau de bord Événements** s’ouvre. Sélectionnez **Try the Events Processed dashboard** (Essayer le tableau de bord Événements traités).
...
Dans cet article, vous avez appris comment utiliser le tableau de bord Événements traités de Security Center. Pour en savoir plus sur le fonctionnement du tableau de bord et écrire vos propres requêtes d’événements, consultez :
