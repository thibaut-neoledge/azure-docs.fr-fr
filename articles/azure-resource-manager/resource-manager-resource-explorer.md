...
---
# <a name="use-azure-resource-explorer-to-view-and-modify-resources"></a>Affichage et modification des ressources à l’aide de l’Explorateur de ressources Azure
...
## <a name="view-resources"></a>Afficher les ressources
...
L’exploration d’une base de données nous montre les propriétés de cette base de données. Dans la capture d’écran ci-dessous, nous pouvons voir que la base de données `edition` est `Standard` et que le `serviceLevelObjective` (ou niveau de base de données) est `S1`.
...
## <a name="invoking-the-api-via-powershell"></a>Appel de l’API au moyen de PowerShell
