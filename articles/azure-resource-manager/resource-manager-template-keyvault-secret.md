---
title: "Modèle Resource Manager pour une clé secrète dans un coffre de clés | Microsoft Docs"
...
---
# <a name="key-vault-secret-template-schema"></a>Schéma d’un modèle de clé secrète de coffre de clés
...
| Nom | Valeur |
| --- | --- |
| type |Enum<br />Requis<br />**secrets** (en cas de déploiement comme ressource enfant d’un coffre de clés) ou<br /> **Microsoft.KeyVault/vaults/secrets** (en cas de déploiement comme ressource de niveau supérieur)<br /><br />Le type de ressource à créer. |
...
| Nom |String<br />Requis<br />Mot unique si déployé comme ressource enfant d’un coffre de clés, ou au format **{key-vault-name}/{secret-name}** si déployé comme ressource de niveau supérieur à ajouter à un coffre de clés existant.<br /><br />Nom de la clé secrète à créer. |
| properties |Object<br />Requis<br />[objet propriétés](#properties)<br /><br />Objet qui spécifie la valeur de la clé secrète à créer. |
| dependsOn |Tableau<br />Facultatif<br />Liste séparée par des virgules de noms de ressources ou d’identificateurs de ressources uniques.<br /><br />La collection de ressources dont dépend de ce lien. Si le coffre de clés pour la clé secrète est déployé dans le même modèle, ajoutez le nom du coffre de clés à cet élément pour vous assurer qu'il est d'abord déployé. |
...
| Nom | Valeur |
| --- | --- |
| Valeur |String<br />Requis<br /><br />Valeur de clé secrète à stocker dans le coffre de clés. Lors du passage d'une valeur pour cette propriété, utilisez un paramètre de type **securestring**. |
