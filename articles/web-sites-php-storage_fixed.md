...
Ce didacticiel vous montre comment créer un site Web PHP et utiliser le service de stockage de tables Azure sur le serveur principal. Il part du principe que vous avez installé [PHP](http://www.php.net/manual/en/install.php) et un serveur Web sur votre ordinateur. Les instructions de ce didacticiel s'appliquent à n'importe quel système d'exploitation, notamment Windows, Mac et Linux. À la fin de ce guide, vous disposerez d'un site Web PHP s'exécutant dans Azure et capable d'accéder au service de stockage de tables.
...
1.  [Installez Git.](http://git-scm.com/book/en/Getting-Started-Installing-Git)
    ...
    > [WACOM.NOTE] Sur Windows, vous devez également ajouter l'exécutable Git à votre variable d'environnement PATH.
