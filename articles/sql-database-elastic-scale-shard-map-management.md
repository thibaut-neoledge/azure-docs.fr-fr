<properties title="Shard Map Management" pageTitle="Gestion des cartes de partitions" description="Utilisation de ShardMapManager, l'API de mise à l'échelle flexible pour .NET" metaKeywords="sharding scaling, Azure SQL Database sharding, elastic scale, shardmapmanager" services="sql-database" documentationCenter="" manager="jhubbard" authors="sidneyh@microsoft.com"/>
...
Par exemple, **[0, 100)** inclut tous les entiers supérieurs ou égaux à 0 et inférieurs à 100. Notez que plusieurs plages peuvent pointer vers la même base de données, et que les plages disjointes sont prises en charge (par exemple : [100,200) et [400,600) pointent vers la base de données C dans l'exemple ci-dessous.)
<table>
   <tr>
    <td><b>Plage de clé</b></td>
...
## Remplissage d'une carte de partitions : exemples 
...
Pour plus d'informations, consultez la page [Routage dépendant des données](./sql-database-elastic-scale-data-dependent-routing.md). 
...
* Pour créer ou supprimer des points ou des plages mappés sur les partitions : utilisez **CreateRangeMapping**, **DeleteMapping** et **CreatePointMapping**. 
    
    Vous pouvez mapper différents points ou plages vers la même partition. Ces méthodes affectent uniquement les métadonnées. Elles n'affectent pas les données qui peuvent déjà être présentes dans les partitions. Si vous devez supprimer des données de la base de données par soucis de cohérence avec les opérations **DeleteMapping**, vous devez effectuer ces opérations séparément, mais en utilisant ces méthodes.  
...
* Pour récupérer des mappages en ligne et hors connexion : utilisez **MarkMappingOffline** et **MarkMappingOnline** pour contrôler l'état en ligne d'un mappage. 
