<properties
   pageTitle="Développement de topologies basées sur Java pour Apache Storm sur HDInsight | Azure"
...
À partir de la ligne de commande, utilisez ce qui suit pour créer un nouveau projet Maven nommé **WordCount**.
...
* **src\test\java\com\microsoft\example** : contient des tests pour votre application. Pour cet exemple, nous n'allons pas créer de tests

###Suppression de l'exemple de code

Étant donné que nous allons créer notre application entièrement, supprimez les fichiers d'application et de test générés :
...
> [AZURE.NOTE] Notez la ligne `<scope>provided</scope>` dans la section que nous avons ajoutée. Cela indique à Maven d'exclure storm-core de tous les fichiers Jar que nous créons, étant donné qu'il sera fourni par le système. Ainsi, les packages que vous créez seront plus petits et cela garantit qu'ils utiliseront les bits storm-core inclus sur le Storm du cluster HDInsight.
...
> [AZURE.NOTE] Bien que cette topologie n'utilise qu'un spout, d'autres peuvent en avoir plusieurs, qui alimentent la topologie avec des données provenant de plusieurs sources.
...
> [AZURE.NOTE] Les bolts peuvent tout faire : le calcul, la persistance, la communication avec des composants externes, etc.
...
Pour implémenter la topologie, créez un fichier nommé **WordCountTopology.java** dans le répertoire **src\main\java\com\microsoft\example**. Utilisez les données suivantes comme contenu du fichier.
...
Prenez un moment pour lire les commentaires du code pour comprendre comment la topologie est définie, puis envoyée au cluster.
...
Pendant son exécution, la topologie affichera les informations de démarrage, puis commencer à afficher des lignes similaires à ce qui suit, lorsque des phrases sont émises par le spout et traitées par les bolts.
