<properties urlDisplayName="Get Started with Mobile Services for Xamarin Android apps" pageTitle="Prise en main de Mobile Services pour les applications Xamarin Android - Azure Mobile Services" metaKeywords="" description="Suivez ce didacticiel pour commencer à utiliser Azure Mobile Services pour le développement Xamarin Android." metaCanonical="" services="" documentationCenter="Mobile" title="Get Started with Mobile Services for Xamarin Android apps" authors="donnam" solutions="" manager="dwrede" editor="mollybos" />
...
2. Appuyez sur le bouton **Exécuter** pour générer le projet et démarrer l'application. Vous êtes invité à sélectionner un émulateur ou un périphérique USB connecté. 
	...
	<p>Afin d'exécuter le projet dans l'émulateur Android, vous devez définir au moins un AVD (appareil virtuel Android). Utilisez le gestionnaire AVD pour créer et gérer ces appareils.</p></div>
...
3. Dans l'application, tapez un texte explicite, comme _Suivre le didacticiel_, puis cliquez sur l'icône plus (**+**).
	...
	> [WACOM.NOTE] 
   	> Vous pouvez vérifier le code (se trouvant dans le fichier C# TodoService.cs) qui permet d'accéder au service mobile pour exécuter une requête et insérer des données.
...
<!-- URLs. -->
[Prise en main de la synchronisation de données hors connexion]: /fr-fr/documentation/articles/mobile-services-xamarin-android-get-started-offline-data
