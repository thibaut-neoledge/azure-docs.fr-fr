...
Ce document contient la liste des codecs et formats de fichiers d'entrée et de sortie pris en charge par la version préliminaire publique de l'encodeur **Media Encoder prime Workflow**.
...
**Media Encoder Premium Workflow** prend en charge le sous-titrage codé décrit dans [cette](#closed_captioning) section. 
...
- JPEG2000
- MPEG-2 (jusqu'au profil 422 et haut niveau ; y compris les variantes telles que XDCAM, XDCAM HD, XDCAM IMX, CableLabs(r) et D10)
- MPEG-1
...
- AES (SMPTE 331M et 302M, AES3-2003)
- Dolby(r) Digital (AC3)
- Dolby(r) Digital Plus (E-AC3) jusqu'à 7.1
- AAC (AAC-LC, AAC-HE et AAC-HEv2 ; jusqu'à 5.1)
- MPEG Layer 2
- MP3 (MPEG-1 Audio Layer 3)
- Windows Media Audio

##<a id="closed_captioning"></a>Prise en charge du sous-titrage codé

À l'heure actuelle, le processeur multimédia **Media Encoder Premium Workflow** prend en charge le sous-titrage codé quand il est inséré dans le fichier multimédia source ou d'entrée : essentiellement, ACE-608/ACE-708 transporté comme données utilisateur (messages SEI de flux élémentaires H.264, ATSC/53, SCTE20) ou comme données connexes dans des fichiers MXF/GXF.

En sortie, les options suivantes sont disponibles pour réempaqueter CEA 608/708 dans l'entrée :

- conversion de CEA-608 vers CEA-708
- Transition CEA-608/CEA-708 (incorporé dans des messages SEI de flux élémentaires H.264 ou transporté comme données connexes dans des fichiers MXF)
- SCC
- SMPTE Timed Text (de la source CEA-608 par SMPTE RP2052 ; création de fichiers DFXP incluse)
- Fichier de sous-titre SRT
- Flux de sous-titres DVB
