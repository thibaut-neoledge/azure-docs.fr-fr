<properties
   pageTitle="Prise en main de l’interface de ligne de commande Azure Batch | Microsoft Azure"
...
>[AZURE.TIP] Nous vous recommandons de mettre à jour fréquemment votre installation d’interface de ligne de commande Azure pour tirer parti des améliorations et des mises à jour de service.
...
> [AZURE.NOTE] Le nom du compte Batch doit être unique dans la région Azure dans laquelle le compte est créé. Il doit comporter 3 à 24 caractères, lesquels ne peuvent être que des caractères alphanumériques minuscules. Vous ne pouvez pas utiliser de caractères spéciaux tels que `-` ou `_` dans les noms de compte Batch.
...
>[AZURE.TIP] Dressez la liste de vos clés avec `azure batch account keys list`, et affichez le point terminaison du compte avec `azure batch account show`.
...
>[AZURE.NOTE] Si vous spécifiez un fichier JSON lorsque vous créez une ressource, tous les autres paramètres que vous spécifiez sur la ligne de commande de cette ressource sont ignorés.
...
>[AZURE.TIP] Recherchez dans la [liste des images de machine virtuelle](batch-linux-nodes.md#list-of-virtual-machine-images) les valeurs appropriées pour les options `--image-*`.
