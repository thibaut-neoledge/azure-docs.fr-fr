<properties urlDisplayName="Optimistic concurrency" pageTitle="Gestion des conflits d'écriture dans une base de données lors d'un accès concurrentiel optimiste (Windows Store) | Centre de développement mobile" metaKeywords="" description="Learn how to handle database write conflicts on both the server and in your Windows Store application." metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Handling database write conflicts" authors="wesmc" manager="dwrede" />
...
+ Package NuGet Azure Mobile Services version 1.1.0 ou ultérieure. Pour obtenir la dernière version, procédez comme suit :
	...
	2. Développez **En ligne**, puis cliquez sur **Microsoft et .NET**. Dans la zone de texte de recherche, entrez **Azure Mobile Services**. Cliquez sur **Installer** sur le package NuGet **Azure Mobile Services** .
...
Dans cette section, vous allez mettre à jour l'interface utilisateur TodoList pour autoriser la mise à jour du texte de chaque élément composant un contrôle ListBox. Ce dernier contiendra un contrôle CheckBox et TextBox pour chaque élément de la table de base de données. Vous pourrez mettre à jour le champ texte de TodoItem. L'application gérera l'événement `LostFocus` depuis cette zone de texte pour mettre à jour l'élément dans la base de données.
...
6. Dans l'instance 1 de l'application, mettez à jour le texte du dernier élément en le remplaçant par **Test Write 1**, puis cliquez sur une autre zone de texte de sorte que le gestionnaire d'événements mette à jour la base de données. La capture d'écran ci-dessous montre un exemple.
...
3. Cliquez sur **Script**, puis sélectionnez l'opération **Mise à jour**.
...
7. À noter qu'aucune exception n'a été rencontrée dans l'application, car le script serveur a résolu le conflit. Et comme l'élément n'était pas marqué comme terminé, la mise à jour a abouti. Pour vérifier que la mise à jour a vraiment abouti, cliquez sur **Actualiser** dans l'instance 2 pour réinterroger la base de données.
...
* [Découvrir les notifications Push] 
