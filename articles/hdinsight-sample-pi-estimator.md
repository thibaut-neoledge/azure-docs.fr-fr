...
Cette rubrique explique comment exécuter un programme MapReduce Hadoop qui estime la valeur de la constante mathématique Pi en utilisant Azure PowerShell pour exécuter le programme dans HDInsight. Elle fournit également le code Java utilisé dans le programme MapReduce et permettant d'estimer la valeur de Pi.
...
La liste des autres exemples disponibles pour vous aider à vous habituer à une utilisation rapide de HDInsight pour exécuter des tâches MapReduce se trouve sur la page [Exécution des exemples HDInsight][] où vous trouverez également des liens vers des instructions sur leur exécution.
...
2.  [Code Java du programme MapReduce de l'Estimateur de la valeur de Pi][]
3.  [Résumé][]
4.  [Étapes suivantes][]
...
## <span id="java-code"></span></a>Code Java du programme MapReduce Estimateur de la valeur de Pi
...
Dans ce didacticiel, vous avez appris à exécuter une tâche MapReduce sur HDInsight et à utiliser des méthodes Monte-Carlo qui nécessitent et génèrent des jeux de données volumineux pouvant être gérés par ce service.

Voici le script complet permettant d'exécuter cet exemple dans un cluster HDInsight 2.1 par défaut. Seul le nom du fichier .jar doit être modifié de manière à exécuter l'exemple dans un cluster HDInsight 3.0 : hadoop-examples.jar devient hadoop-mapreduce-examples.jar.)
...
-   [Documentation du Kit de développement logiciel (SDK) Azure HDInsight][]
  ...
  [Code Java du programme MapReduce de l'Estimateur de la valeur de Pi]: #java-code
  [Résumé]: #summary
