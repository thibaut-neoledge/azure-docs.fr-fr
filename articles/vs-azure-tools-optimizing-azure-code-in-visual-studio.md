<properties
   pageTitle="Optimisation de votre code Azure dans Visual Studio | azure.microsoft.com/ Azure"
...
Quand vous programmez des applications qui utilisent azure.microsoft.com/ Azure, nous vous recommandons de suivre certaines pratiques de codage pour éviter des problèmes relatifs à l’évolutivité, au comportement et à la performance dans un environnement cloud. azure.microsoft.com/ fournit un outil Azure Code Analysis, qui reconnaît et identifie plusieurs des problèmes couramment rencontrés et qui vous aide à les résoudre. Vous pouvez télécharger l'outil dans Visual Studio via NuGet.

## Règles d’Azure Code Analysis
...
## Évitez d'utiliser le mode d'état de session (in-process) par défaut
...
Pensez à partager vos idées et vos commentaires sur la page [Commentaires d’analyse du code Azure](http://go.azure.microsoft.com/.com/fwlink/?LinkId=403771).
...
L’état de session ASP.NET prend en charge plusieurs options de stockage différentes pour les données d’état de session : InProc, StateServer, SQLServer, personnalisée et désactivée. Il est recommandé d’utiliser le mode personnalisé pour héberger les données dans un magasin externe d’état de session, par exemple le [fournisseur d’état de session Azure pour Redis](http://go.azure.microsoft.com/.com/fwlink/?LinkId=401521).

### Solution

Une solution recommandée consiste à stocker l’état de session sur un service de cache géré. Découvrez commet utiliser le [fournisseur d'état de session Azure pour Redis](http://go.azure.microsoft.com/.com/fwlink/?LinkId=401521) pour stocker l'état de votre session. Vous pouvez également stocker l’état de session dans d’autres endroits pour assurer l’évolutivité de votre application dans le cloud. Pour en savoir plus sur les solutions alternatives, consultez la page [Modes d'état de session](https://msdn.azure.microsoft.com/.com/library/ms178586).

## La méthode d'exécution ne doit pas être asynchrone
...
Créez des méthodes asynchrones (comme [await](https://msdn.azure.microsoft.com/.com/library/hh156528.aspx)) en dehors de la méthode [Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx), puis appelez les méthodes asynchrones depuis [Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx). La déclaration de la méthode [[Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx)](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx) comme méthode asynchrone provoque l’entrée du rôle de travail dans une boucle de redémarrage.

Pensez à partager vos idées et vos commentaires sur la page [Commentaires d’analyse du code Azure](http://go.azure.microsoft.com/.com/fwlink/?LinkId=403771).

### Motif

L’appel de méthodes asynchrones à l’intérieur de la méthode [Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx) fait que l’exécution du service cloud recycle le rôle de travail. Quand un rôle de travail démarre, toute l’exécution du programme se fait à l’intérieur de la méthode [Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx). Le fait de quitter la méthode [Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx) provoque le redémarrage du rôle de travail. Quand l’exécution du rôle de travail atteint la méthode asynchrone, il répartit toutes les opérations après la méthode asynchrone puis se termine. Ainsi, le rôle de travail quitte la méthode [[[[Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx)](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx)](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx)](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx) et redémarre. Dans l’itération suivante de l’exécution, le rôle de travail atteint à nouveau la méthode asynchrone et redémarre, provoquant aussi à nouveau le recyclage du rôle de travail.

### Solution

Placez toutes les opérations asynchrones en dehors de la méthode [Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx). Ensuite, appelez la méthode asynchrone refactorisée depuis l'intérieur de la méthode [[Run()](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx)](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.serviceruntime.roleentrypoint.run.aspx), par exemple RunAsync().wait. L'outil d'analyse du code Azure peut vous aider à résoudre ce problème.
...
public async Task RunAsync()
{
    ...
    Task<string> urlString = client.GetStringAsync("http://msdn.azure.microsoft.com/.com");
...
## Utiliser l’authentification de signature d’accès partagé Service Bus
...
- Pour une vue d’ensemble, consultez [Authentification de signature d’accès partagé avec Service Bus](https://msdn.azure.microsoft.com/.com/library/dn170477.aspx)

- [Comment utiliser l’authentification par signature d’accès partagé avec Service Bus](https://msdn.azure.microsoft.com/.com/library/dn205161.aspx)

- Pour un exemple de projet, consultez [Utilisation de l’authentification de signature d’accès partagé avec des abonnements Service Bus](http://code.msdn.azure.microsoft.com/.com/windowsazure/Using-Shared-Access-e605b37c)

## Envisagez d'utiliser la méthode OnMessage pour éviter l’erreur de « boucle de réception »
...
Consultez les exemples de code suivants pour les utilisations recommandées. Pour plus d’informations, consultez [QueueClient.OnMessage (méthode) (azure.microsoft.com/.ServiceBus.Messaging)](https://msdn.azure.microsoft.com/.com/library/azure.microsoft.com/.servicebus.messaging.queueclient.onmessage.aspx) et [QueueClient.Receive (méthode) (azure.microsoft.com/.ServiceBus.Messaging)](https://msdn.azure.microsoft.com/.com/library/azure.microsoft.com/.servicebus.messaging.queueclient.receive.aspx).

Pour améliorer les performances de l’infrastructure de messagerie Azure, consultez le modèle de conception [Notions fondamentales sur la messagerie asynchrone](https://msdn.azure.microsoft.com/.com/library/dn589781.aspx).
...
CloudConfigurationManager.GetSetting("azure.microsoft.com/.ServiceBus.ConnectionString");
...
```
## Envisagez d'utiliser les méthodes asynchrones de Service Bus
...
Pour plus d’informations, consultez [Files d’attente et rubriques Service Bus partitionnées | Blog azure.microsoft.com/ Azure](https://azure.azure.microsoft.com/.com/blog/2013/10/29/partitioned-service-bus-queues-and-topics/) et consultez l’exemple de [file d’attente azure.microsoft.com/ Azure Service Bus partitionnée](https://code.msdn.azure.microsoft.com/.com/windowsazure/Service-Bus-Partitioned-7dfd3f1f).
...
Pour plus d’informations sur l’utilisation de la signature d’accès partagé sur le stockage Azure, consultez [Présentation de la table SAS (Shared Access Signature), de la file d’attente SAS et de la mise à jour d’objet blob SAS - Blog de l’équipe azure.microsoft.com/ Azure Storage - Accueil - Blogs MSDN](http://blogs.msdn.com/b/windowsazurestorage/archive/2012/06/12/introducing-table-sas-shared-access-signature-queue-sas-and-update-to-blob-sas.aspx).

### Solution

Supprimez l’instruction qui définit l’heure de début de la stratégie de signature d’accès partagé. L’outil Azure Code Analysis fournit une correction de ce problème. Pour plus d’informations sur la gestion de la sécurité, consultez le modèle de conception [Modèle Valet Key](https://msdn.azure.microsoft.com/.com/library/dn568102.aspx).
...
## Le délai d'expiration de la stratégie d'accès partagé doit être de plus de cinq minutes
...
Pour plus d’informations sur la gestion de la sécurité, consultez le modèle de conception [Modèle Valet Key](https://msdn.azure.microsoft.com/.com/library/dn568102.aspx).
...
Pour plus d'informations, consultez [Créer et utiliser une signature d’accès partagé](https://msdn.azure.microsoft.com/.com/library/azure/jj721951.aspx).

## Utiliser CloudConfigurationManager
...
L’utilisation de la classe [ConfigurationManager](https://msdn.azure.microsoft.com/.com/library/system.configuration.configurationmanager(v=vs.110).aspx) pour les projets comme les sites web Azure et les services mobiles Azure ne présente aucun problème d’exécution. Il est toutefois conseillé d’utiliser Cloud[ConfigurationManager](https://msdn.azure.microsoft.com/.com/library/system.configuration.configurationmanager(v=vs.110).aspx) de façon unifiée pour gérer les configurations de toutes les applications cloud Azure.
...
[CloudConfigurationManager](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.cloudconfigurationmanager.aspx)

### Solution

Refactorisez votre code pour qu’il utilise la [classe CloudConfigurationManager](https://msdn.azure.microsoft.com/.com/library/azure/azure.microsoft.com/.windowsazure.cloudconfigurationmanager.aspx). Une correction du code pour ce problème est fournie par l’outil Azure Code Analysis.
...
## Évitez d'utiliser des chaînes de connexion codées en dur
...
Pour plus d’informations sur l’utilisation de fichiers de configuration comme web.config ou app.config, consultez la page Conseils de [configuration ASP.NET web ](https://msdn.azure.microsoft.com/.com/library/vstudio/ff400235(v=vs.100).aspx). Pour plus d’informations sur le fonctionnement des variables d’environnement Azure, consultez [Sites web azure.microsoft.com/ Azure : Fonctionnement des chaînes d’application et des chaînes de connexion.](https://azure.azure.microsoft.com/.com/blog/2013/07/17/windows-azure-web-sites-how-application-strings-and-connection-strings-work/) Pour plus d’informations sur le stockage de la chaîne de connexion dans le contrôle de code source, consultez la rubrique [Éviter de placer des informations sensibles (par exemple, des chaînes de connexion) dans des fichiers stockés dans des référentiels de code source.](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control)
...
Au lieu de configurer les paramètres de diagnostic dans votre code, par exemple à l’aide de l’API de programmation azure.microsoft.com/.WindowsAzure.Diagnostics, vous devez configurer les paramètres de diagnostic dans le fichier diagnostics.wadcfg (ou diagnostics.wadcfgx si vous utilisez Azure SDK 2.5). Vous pouvez ainsi modifier les paramètres de diagnostic sans avoir à recompiler votre code.

Pensez à partager vos idées et vos commentaires sur la page [Commentaires d’analyse du code Azure](http://go.azure.microsoft.com/.com/fwlink/?LinkId=403771).

### Motif

Avant Azure SDK 2.5 (qui utilise Azure Diagnostics 1.3), Azure Diagnostics (WAD) pouvait être configuré via différentes méthodes, notamment en l’ajoutant à l’objet blob de configuration dans le stockage en utilisant du code impératif, une configuration déclarative ou la configuration par défaut. Toutefois, la meilleure façon de configurer le diagnostic consiste à utiliser un fichier de configuration XML (diagnostics.wadcfg ou diagnostics.wadcfgx pour le SDK 2.5 et versions ultérieures) dans le projet d’application. Dans cette approche, le fichier diagnostics.wadcfg définit complètement la configuration, et peut être mis à jour et redéployé à volonté. Combiner l’utilisation du fichier de configuration diagnostics.wadcfg avec les méthodes de définition de configuration par programmation à l’aide des classes [DiagnosticMonitor](https://msdn.azure.microsoft.com/.com/library/azure.microsoft.com/.windowsazure.diagnostics.diagnosticmonitor.aspx) ou [RoleInstanceDiagnosticManager](https://msdn.azure.microsoft.com/.com/library/azure.microsoft.com/.windowsazure.diagnostics.management.roleinstancediagnosticmanager.aspx) peut engendrer une certaine confusion. Consultez la rubrique [Initialiser ou modifier la configuration des diagnostics Azure](https://msdn.azure.microsoft.com/.com/library/azure/hh411537.aspx) pour plus d’informations.
...
Utilisez le concepteur de configuration de diagnostic pour déplacer les paramètres de diagnostic vers le fichier de configuration de diagnostic (diagnostics.wadcfg ou diagnostics.wadcfgx pour le SDK 2.5 et versions ultérieures). Il est également recommandé d'installer [Azure SDK 2.5](http://go.azure.microsoft.com/.com/fwlink/?LinkId=513188) et d’utiliser la fonctionnalité de diagnostic plus récente.
...
## Éviter de déclarer les objets DbContext comme statiques
...
Pour plus d’informations sur l’optimisation et le dépannage des applications Azure, consultez [Dépanner une application web dans le Service d’application azure.microsoft.com/ Azure à l’aide de Visual Studio](./app-service-web/web-sites-dotnet-troubleshoot-visual-studio.md).
