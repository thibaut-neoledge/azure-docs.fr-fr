...
Dans cette phase de déploiement d'une batterie de serveurs SharePoint 2013 intranet avec les groupes de disponibilité SQL Server AlwaysOn dans des services d'infrastructure Azure, vous configurez deux contrôleurs de domaine dans le réseau virtuel Azure afin que les requêtes web des clients pour les ressources de batterie de serveurs SharePoint puissent être authentifiées dans le réseau virtuel Azure, au lieu d'envoyer ce trafic d'authentification vers votre réseau local à travers la connexion VPN ou ExpressRoute.
...
Utilisez le bloc de commandes PowerShell suivant pour créer les machines virtuelles pour les deux contrôleurs de domaine. Spécifiez les valeurs des variables en supprimant les caractères < and >. Notez que ce jeu de commandes PowerShell utilise les valeurs des tables suivantes :
...
1.	Dans le portail de gestion Azure, cliquez sur **Machines virtuelles** dans le volet gauche.
...
2.	À l'invite de commande Windows PowerShell, utilisez la commande **ping** pour effectuer un test Ping des noms et des adresses des ressources sur le réseau de votre organisation.
...
- sqlservice : un compte d'utilisateur pour l'exécution d'instances SQL.
...
1.	Dans le volet gauche du portail de gestion Azure, cliquez sur **Réseaux**, puis cliquez sur le nom de votre réseau virtuel (Table V – Élément 1 – Colonne Valeur).
...
6.	Dans le volet gauche du portail de gestion Azure, cliquez sur **Machines virtuelles**, puis cliquez sur la colonne **État** située en regard de votre premier contrôleur de domaine.
...
Vous trouverez ci-dessous la configuration résultant de cette phase, avec des espaces réservés de noms d'ordinateur.
