<properties title="Gestion de l'acc&egrave;s concurrentiel dans Microsoft Azure Storage" pageTitle="required" description="Gestion de l'acc&egrave;s concurrentiel pour les services BLOB, de File d'attente, de Table et de Fichier" metaKeywords="Optional" services="Optional" solutions="Optional" documentationCenter="Optional" authors="Tamra Myers" videoId="Optional" scriptId="Optional" />
...
L'extrait de code C# suivant (à l'aide de la bibliothèque de stockage cliente 4.2.0) présente un exemple simple de construction d'une condition d'accès **If-Match AccessCondition** basée sur la valeur ETag obtenue à partir des propriétés d'un objet blob précédemment récupéré ou inséré. Il utilise ensuite l'objet **AccessCondition** lors de la mise à jour de l'objet blob : l'objet **AccessCondition** ajoute l'en-tête **If-Match** à la demande. Si l'objet blob a été mis à jour par un autre processus, le service BLOB renvoie un message d'état HTTP 412 (Échec de la condition préalable). L'exemple en version intégrale est disponible en téléchargement [ici][ici].
...
Copy Blob| Oui| Oui (pour les objets blob source et cible)|
...
-   [Spécification des en-têtes conditionnels pour les opérations du service BLOB][1]
...
-   [Opérations sur les entités][Opérations sur les entités]
...
-   [API REST du service de File d'attente][API REST du service de File d'attente]
...
-   [Gestion des verrouillages de fichiers][Gestion des verrouillages de fichiers]
...
-   [Gestion de l'accès concurrentiel avec Azure Storage - exemple d'application][ici]

Pour plus d'informations concernant Azure Storage, consultez la page :

-   [Page d'accueil de Microsoft Azure Storage][Page d'accueil de Microsoft Azure Storage]
-   [Introduction à Azure Storage][Introduction à Azure Storage]
-   Prise en main du stockage d'[objets blob][objets blob], de [tables][tables] et de [files d'attente][files d'attente]
-   Architecture de stockage – [Windows Azure Storage : Service de stockage sur le cloud à haute disponibilité et à cohérence forte][Windows Azure Storage : Service de stockage sur le cloud à haute disponibilité et à cohérence forte]

  [ici]: http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114
  [Spécification des en-têtes conditionnels pour les opérations du service BLOB]: http://msdn.microsoft.com/fr-fr/library/dd179371.aspx
  [bail]: http://msdn.microsoft.com/fr-fr/library/azure/ee691972.aspx
  [1]: http://msdn.microsoft.com/fr-fr/library/azure/dd179371.aspx
  ...
  [objets blob]: http://azure.microsoft.com/fr-fr/documentation/articles/storage-dotnet-how-to-use-blobs/
  [tables]: http://azure.microsoft.com/fr-fr/documentation/articles/storage-dotnet-how-to-use-tables/
  [files d'attente]: http://azure.microsoft.com/fr-fr/documentation/articles/storage-dotnet-how-to-use-queues/
  [Windows Azure Storage : Service de stockage sur le cloud à haute disponibilité et à cohérence forte]: http://blogs.msdn.com/b/windowsazurestorage/archive/2011/11/20/windows-azure-storage-a-highly-available-cloud-storage-service-with-strong-consistency.aspx
