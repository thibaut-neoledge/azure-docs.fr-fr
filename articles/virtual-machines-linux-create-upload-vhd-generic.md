<properties urlDisplayName="Upload a Linux VHD" pageTitle="Cr&eacute;ation et t&eacute;l&eacute;chargement d'un disque dur virtuel&nbsp;Linux dans Azure" metaKeywords="Azure VHD, uploading Linux VHD" description="Apprenez &agrave; cr&eacute;er et &agrave; t&eacute;l&eacute;charger un disque dur virtuel&nbsp;(VHD)&nbsp;Azure contenant un syst&egrave;me d'exploitation Linux." metaCanonical="" services="virtual-machines" documentationCenter="" title="Cr&eacute;ation et t&eacute;l&eacute;chargement d'un disque dur virtuel contenant un syst&egrave;me d'exploitation Linux" authors="kathydav" solutions="" manager="timlt" editor="tysonn" />
...
**Important** : Le contrat SLA de la plateforme Azure s'applique aux machines virtuelles exécutant le système d'exploitation Linux uniquement lorsqu'une des [distributions approuvées][distributions approuvées] est utilisée. Toutes les distributions Linux fournies dans la galerie d'images Azure sont des distributions reconnues répondant à la configuration requise.

-   [Linux dans les distributions approuvées par Azure][distributions approuvées]
-   [Prise en charge d'images Linux dans Microsoft Azure][Prise en charge d'images Linux dans Microsoft Azure]
...
C'est pourquoi nous recommandons de commencer avec une de nos [distributions Linux approuvées sur Azure][distributions Linux approuvées sur Azure] dans la mesure du possible. Les articles suivants vous montrent comment préparer les diverses distributions Linux approuvées prises en charge dans Azure :
...
-   NUMA n'est pas pris en charge pour les machines virtuelles plus volumineuses à cause d'un bogue dans les versions du noyau Linux antérieures à 2.6.37. Ce problème affecte essentiellement les distributions utilisant le noyau Red Hat 2.6.32 en amont. L'installation manuelle de l'Agent Linux Azure (waagent) désactive automatiquement NUMA dans la configuration GRUB pour le noyau Linux.
...
-   [ata\_piix : reporter des disques dans les pilotes Hyper-V par défaut][ata\_piix : reporter des disques dans les pilotes Hyper-V par défaut]
-   [storvsc : compte des paquets en transit dans le chemin RESET][storvsc : compte des paquets en transit dans le chemin RESET]

## Agent Linux Azure

L'[Agent Linux Azure][Agent Linux Azure] (waagent) est requis pour approvisionner une machine virtuelle Linux dans Azure. La dernière version, les problèmes des fichiers ou l'envoi de requêtes d'extraction sont disponibles dans le [référentiel de l'agent Linux sur GitHub][référentiel de l'agent Linux sur GitHub].
...
-   Vous devez ensuite arrêter la machine virtuelle et télécharger le disque dur virtuel dans Azure.
  ...
  [distributions approuvées]: ../virtual-machines-linux-endorsed-distributions
  [Prise en charge d'images Linux dans Microsoft Azure]: http://support2.microsoft.com/kb/2941892
  [distributions Linux approuvées sur Azure]: ../linux-endorsed-distributions
  ...
  [à cette adresse]: http://go.microsoft.com/fwlink/p/?LinkID=254263&clcid=0x409
  [MAINTAINERS]: https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/tree/MAINTAINERS
  [ata\_piix : reporter des disques dans les pilotes Hyper-V par défaut]: https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/commit/drivers/ata/ata_piix.c?id=cd006086fa5d91414d8ff9ff2b78fbb593878e3c
  ...
  [Agent Linux Azure]: ../virtual-machines-linux-agent-user-guide
  [référentiel de l'agent Linux sur GitHub]: https://github.com/Azure/WALinuxAgent
