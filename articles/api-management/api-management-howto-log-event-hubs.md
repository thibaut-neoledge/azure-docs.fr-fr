...
Nommez la seconde **Réception** et attribuez-lui des autorisations d’**écoute**, puis cliquez sur **Enregistrer**.
...
-	Autorisation : SharedAccessSignature uid=...
	-	Pour obtenir des instructions sur la génération de `SharedAccessSignature`, consultez [Authentification de l’API REST de gestion des API Azure](https://msdn.microsoft.com/library/azure/dn798668.aspx).
...
>[AZURE.NOTE]Pour connaître les autres codes de retour possibles et leurs raisons, consultez [Créer un enregistreur d’événements](https://msdn.microsoft.com/library/azure/mt592020.aspx#PUT). Pour voir comment effectuer d’autres opérations, notamment créer des listes, mettre à jour et supprimer, consultez la documentation de l’entité [Enregistreur d’événements](https://msdn.microsoft.com/library/azure/mt592020.aspx).
...
Pour configurer des stratégies, connectez-vous au [portail Azure](https://manage.windowsazure.com), accédez à votre service de gestion des API et cliquez sur **portail de publication** ou sur **Gérer** pour accéder au portail de publication.
...
Remplacez `logger-id` par le nom de l’enregistreur d’événements de gestion des API que vous avez configuré à l’étape précédente.
