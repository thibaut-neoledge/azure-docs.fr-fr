...
|IMessageBody|As<T\>(preserveContent: bool = false): Where T: string, JObject, JToken, JArray, XNode, XElement, XDocument<br /><br /> Les méthodes `context.Request.Body.As<T>` et `context.Request.Body.As<T>` sont utilisées pour lire un corps de message de demande ou de réponse dans un des formats spécifiés. Notez que, par défaut, lorsque vous accédez au corps du message, le corps du message d’origine est perdu et doit être défini en renvoyant le corps dans l’expression qui lit le corps. Pour conserver le contenu du corps, donnez la valeur `true` au paramètre `preserveContent` lorsque vous accédez au message. La valeur par défaut de `preserveContent` est `false`. Si `preserveContent` a la valeur `true` et qu’un corps est renvoyé par l’expression, le corps renvoyé est utilisé. Pour plus d’informations, consultez la stratégie [Set body](api-management-transformation-policies.md#SetBody).|  
