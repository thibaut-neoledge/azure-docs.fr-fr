...
L'exemple ci-dessous montre comment les messages peuvent être reçus et traités à l'aide de **receive\_subscription\_message()**. Dans l'exemple, un message est d'abord reçu, puis supprimé de l'abonnement « low-messages » par le biais de **:peek\_lock** défini sur **false**. Un autre message de « high-messages » est ensuite reçu, puis supprimé via **delete\_queue\_message()** :
