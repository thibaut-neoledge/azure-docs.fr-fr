...
# Utiliser Power BI Embedded avec REST
...
Mais, vous le voyez, cette solution d’incorporation simple ne répond pas aux besoins de l’application d’un éditeur de logiciels indépendant. La plupart des applications ISV ont besoin de fournir les données à leurs clients, et pas nécessairement aux utilisateurs de leur propre organisation. Par exemple, si vous proposez un service à la société A et à la société B, les utilisateurs de la société A doivent voir uniquement les données de leur propre société, la société A. Autrement dit, l’architecture mutualisée est nécessaire pour fournir les données.
...
Vous pouvez utiliser .NET (C#) ou le Kit de développement logiciel (SDK) Node.js pour créer facilement votre application avec Power BI Embedded. Toutefois, dans cet article, nous expliquerons le flux HTTP (y compris AuthN) de Power BI sans SDK. Si vous comprenez ce flux, vous pouvez générer votre application **avec n’importe quel langage de programmation** et comprendre l’essence même de Power BI Embedded.

## Création d’une collection d’espaces de travail Power BI et obtention de la clé d’accès (approvisionnement)
Power BI Embedded fait partie des services Azure. Seul l’éditeur de logiciels indépendant qui utilise le portail Azure doit payer des frais d’utilisation (par session d’utilisateur à l’heure), et l’utilisateur qui consulte le rapport n’est pas facturé et n’a même pas besoin d’un abonnement Azure. Avant de commencer le développement de notre application, nous devons créer la **collection d’espaces de travail Power BI** avec le portail Azure.
...
## Importer le fichier .pbix dans l’espace de travail
...
## Connectivité de la source de données (et architecture mutualisée des données)
...
> [AZURE.NOTE] Si nous souhaitons utiliser la sécurité au niveau des lignes (RLS) avec Power BI Embedded, nous devons également spécifier un **nom d’utilisateur** et des **rôles** dans les revendications.
...
## Dernière étape : intégration du rapport dans la page web
...
Nous pouvons intégrer le rapport dans notre application web à l’aide du jeton d’application précédent. Si nous examinons l’exemple de code suivant, la première partie est la même que l’exemple précédent. Dans la dernière partie, cet exemple montre **embedUrl** (voir le résultat précédent) dans l’iframe et publie le jeton d’application dans l’iframe.

> [AZURE.NOTE] Vous devez modifier la valeur de l’ID de rapport à votre convenance. En outre, en raison d’un bogue dans notre système de gestion de contenu, la balise iframe dans l’exemple de code est lue littéralement. Supprimez le texte encapsulé de la balise si vous copiez-collez le code suivant.
