<properties
    pageTitle="Configuration de la stratégie SSL et du chiffrement SSL de bout en bout avec la passerelle Application Gateway | Microsoft Azure"
...
# Configuration de la stratégie SSL et du chiffrement SSL de bout en bout avec la passerelle Application Gateway à l’aide de PowerShell
...
> [AZURE.NOTE] Un nom de domaine complet (FQDN) est également une valeur valide pour remplacer une adresse IP dans les serveurs principaux.
...
> [AZURE.NOTE] Cet exemple configure le certificat utilisé pour la connexion SSL. Le certificat doit être au format .pfx et le mot de passe contenir entre 4 et 12 caractères.
...
> [AZURE.NOTE] Le certificat fourni dans cette étape doit être la clé publique du certificat pfx présent sur le serveur principal. Cette étape permet d’ajouter le serveur principal à la liste approuvée par la passerelle d’application.
...
## Désactivation de versions du protocole SSL sur une passerelle Application Gateway existante
