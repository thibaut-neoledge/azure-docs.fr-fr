...
Configurez le paramètre de passerelle Application Gateway « poolsetting01 » pour le trafic du pool principal. Cette étape comporte également une configuration du délai d'expiration de la réponse du pool principal à une requête de passerelle Application Gateway. Lorsqu'une réponse du serveur principal atteint une limite d'expiration, la passerelle Application Gateway annule la demande. Cette réponse est différente du délai d'expiration d'analyse qui s'applique uniquement à la réponse du serveur principal aux contrôles d'analyse.
