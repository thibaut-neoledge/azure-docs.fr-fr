...
Le serveur principal doit répondre aux sondes de la passerelle Application Gateway, qui sont décrites dans la section [Vue d’ensemble de la sonde d’intégrité](application-gateway-probe-overview.md). Les sondes d’intégrité Application Gateway sont uniquement pour HTTP/HTTPS, ce qui implique que chaque serveur principal doit répondre aux sondes HTTP pour qu’Application Gateway puisse acheminer le trafic WebSocket vers le serveur.
...
Autre raison : la sonde d’intégrité principale d’Application Gateway prend en charge uniquement les protocoles HTTP/HTTPS. Si le serveur principal ne répond pas aux sondes HTTP/HTTPS, il est exclu du pool principal et aucune demande, y compris les demandes WebSocket, ne peut atteindre ce serveur principal.
