<properties urlDisplayName="Command Line Administration" pageTitle="Administration d'un service mobile à l'aide d'une invite de commandes - Didacticiel Azure" metaKeywords="" description="Découvrez comment créer, déployer et gérer votre service mobile Azure à l'aide des outils en ligne de commande." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Automate mobile services with command-line tools" authors="glenga" solutions="" manager="dwrede" editor="" />
...
-   [Inscription d'un script dans une opération de table][Register a new table script]
...
+ Si vous ne possédez pas de compte, vous pouvez créer un compte d'évaluation gratuit en quelques minutes. Pour plus d'informations, consultez la rubrique <a href="http://www.windowsazure.com/fr-fr/pricing/free-trial/" target="_blank">Version d'évaluation gratuite d'Azure</a>.
...
* **Linux** : installez la dernière version de [Node.js][nodejs-org] (voir [Installation de Node.js via le gestionnaire de package][install-node-linux]), puis exécutez la commande suivante :
...
Vous pouvez supprimer toutes les informations stockées par la commande <code>Importer</code> en utilisant la commande <code>account clear</code>  :
...
<div class="dev-callout"><strong>Remarque</strong> 
   <p>Si vous ne spécifiez pas le paramètre <code>-q</code> avec <code>-a</code> ou <code>-d</code>, l'exécution est interrompue et vous êtes invité à sélectionner des options de suppression pour votre base de données SQL. Utilisez uniquement le paramètre <code>-a</code> lorsqu'aucun autre service n'utilise la base de données ou le serveur ; sinon, utilisez le paramètre <code>-d</code> pour supprimer uniquement les données appartenant au service mobile supprimé.</p>
</div>
...
La commande suivante télécharge et inscrit une fonction dans une opération sur une table, où <service-name> est le nom du service mobile, <table-name>` est le nom de la table, et <operation> est l'opération de table, qui peut être définie sur" insert ", " read ", " update " ou " delete " :

		azure mobile script upload <service-name> table/<table-name>.<operation>.js

Notez que cette opération charge un fichier JavaScript (.js) à partir de l'ordinateur local. Le nom du fichier doit se composer des noms d'une table et d'une opération et doit être situé dans le sous-dossier table par rapport à l'emplacement où est exécutée la commande. Par exemple, l'opération suivante charge et inscrit un nouveau script insert appartenant à la table TodoItems :
