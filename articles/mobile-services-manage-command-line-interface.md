...
+ Si vous ne possédez pas de compte, vous pouvez créer un compte d'évaluation gratuit en quelques minutes. Pour plus d'informations, consultez la page <a href="http://www.windowsazure.com/pricing/free-trial/" target="_blank">Version d'évaluation gratuite d'Azure</a>.
...
> [AZURE.NOTE] Si vous ne spécifiez pas le paramètre <code>-q</code> avec <code>-a</code> ou <code>-d</code>, l'exécution est interrompue et vous êtes invité à sélectionner des options de suppression pour votre base de données SQL. Utilisez uniquement le paramètre <code>-a</code> lorsqu'aucun autre service n'utilise la base de données ou le serveur ; sinon, utilisez le paramètre <code>-d</code> pour supprimer uniquement les données appartenant au service mobile supprimé.
