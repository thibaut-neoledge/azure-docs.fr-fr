<properties
	pageTitle="Manuel de modèle Cortana Analytics Solution de prévision de la demande d’énergie | Microsoft Azure"
...
>[AZURE.TIP] Pour télécharger un diagramme offrant une vue d’ensemble de l’architecture de ce modèle, consultez [Cortana Analytics Solution Template architecture for demand forecasting of energy](cortana-analytics-architecture-demand-forecasting-energy.md) (Architecture de modèle de Cortana Analytics Solution pour la prévision de la demande en énergie).

## Vue d'ensemble  

Ce document traite de l’utilisation de Cortana Analytics et plus particulièrement, d’Azure Machine Learning (AML) dans l’implémentation et le déploiement de solutions de prévision d’énergie du point de vue de l’entreprise, des données et de la technique. Le document se compose de trois parties principales :
...
-   maîtrise rapide du produit Cortana Analytics Suite,
...
Cortana Analytics et Machine Learning peuvent être très efficaces pour résoudre un problème d’entreprise donné lorsque les critères suivants sont satisfaits :
...
-   Le client a accès à une architecture de données sur le cloud ou à l’**apprentissage automatique basé sur le cloud**, notamment à Azure ML et à d’autres composants de Cortana Analytics Suite.
...
Cortana Analytics Suite fournit un modèle de tarification par répartition, et il est donc inutile d’engager un composant de coût fixe pour cette formule. Cette formule peut être calculée sur une base quotidienne, mensuelle ou annuelle.

Vous trouverez les tarifs de Cortana Analytics Suite et Azure ML [ici](http://azure.microsoft.com/pricing/details/machine-learning/).

### Processus de développement de solution
Le cycle de développement d’une solution de prévision de demande de l’énergie implique généralement 4 phases, dans l’ensemble desquelles nous utilisons les technologies et le service du cloud dans le cadre de Cortana Analytics Suite.
...
### Format de données
Cortana Analytics Suite prend en charge les formats de données les plus courants tels que CSV, TSV, JSON, *etc.*. Il est important que le format de données reste le même pour l’ensemble du cycle de vie du projet.
...
Pour ce faire, Cortana Analytics Suite offre différents moyens de prendre en charge un processus d’ingestion de données. Cette méthode sera expliquée plus loin dans la section **Déploiement** de ce document.
...
Comme vous pouvez le constater, le service web est déployé dans le nuage Cortana Analytics Suite et peut être appelé via le point de terminaison API REST exposé. Différents types de clients résidant dans plusieurs domaines peuvent appeler le service via l’API Web simultanément. Le service web peut également évoluer pour prendre en charge des milliers d’appels simultanés.
...
Pour déployer une solution de prévision de demande d’énergie sur Cortana Analytics, nous devons vous assurer que les composants requis sont définis et configurés correctement.

Le diagramme suivant représente une architecture Cortana Analytics classique qui implémente et gère le cycle de flux de données décrit ci-dessus :
