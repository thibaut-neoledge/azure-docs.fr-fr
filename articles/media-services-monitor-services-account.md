...
**Remarque** La surveillance des données de stockage dans le portail de gestion Azure occasionne des frais supplémentaires. Pour plus d'informations, consultez la page [Storage Analytics et facturation][].

## <span id="configuremonitoring"></span></a> Surveillance d'un compte Media Services

1.  Dans le [portail de gestion][], cliquez sur **Media Services**, puis sur le nom du compte Media Services pour ouvrir le tableau de bord.
...
1.  When you finish the monitoring configuration, click **Save**.
    ...
    Metrics are stored in the storage account in four tables named $MetricsTransactionsBlob, $MetricsTransactionsTable, $MetricsTransactionsQueue, and $MetricsCapacityBlob. Pour plus d'informations, consultez la page [À propos des métriques Storage Analytics][].
