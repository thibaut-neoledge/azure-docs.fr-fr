...
Une spécification de structure de réseau neuronal est composée de trois sections : la **déclaration de constante**, la **déclaration de couche** et la **déclaration de connexion**. Il existe également une section de **déclaration de partage**, qui est facultative. Ces sections peuvent être définies dans n'importe quel ordre.  
...
Pour définir simultanément deux constantes ou plus, mettez les noms d'identificateur et les valeurs associées entre accolades, en les séparant par des points-virgules. Exemple :  

    Const { X = 28; Y = 4; }  

La partie droite de chaque expression d'affectation peut correspondre à un entier, un nombre réel, une valeur booléenne (True ou False) ou une expression mathématique. Exemple :  

	Const { X = 17 * 2; Y = true; }  

##Déclaration de couche
La déclaration de couche est requise. Elle définit la taille et la source de la couche, y compris ses attributs et faisceaux de connexions. L'instruction de déclaration commence par le nom de la couche (couche d'entrée, masquée ou de sortie), suivi de ses dimensions (tuple d'entiers positifs). Exemple :  

	input Data[784];
	hidden Hidden[5,20] from Data all;
	output Result[2] from Hidden all;  

-	Le produit des dimensions est le nombre de nœuds de la couche. Dans cet exemple, il existe deux dimensions de valeur [5,20], ce qui signifie qu'il existe  100 nœuds dans la couche.
...
<!-- CONTENU SUPPRIMÉ JUSQU'À CE QUE CETTE FONCTIONNALITÉ SOIT PRISE EN CHARGE DANS LE PRODUIT
Pour spécifier que le nombre de nœuds d'une couche doit être déterminé automatiquement, utilisez le mot clé **auto**. Le mot clé **auto** peut avoir différents effets, selon la couche :  
...
Une déclaration d'une couche apte à l'apprentissage (couche masquée ou de sortie) peut éventuellement inclure la fonction de sortie (également appelée fonction d'activation), configurée par défaut sur **sigmoid**. Les fonctions de sortie suivantes sont prises en charge :  
...
Immédiatement après avoir défini la couche entraînable, vous devez déclarer les connexions entre les couches définies. Une déclaration de faisceau de connexions commence par le mot clé **from**, suivi du nom de la couche source du faisceau et du type de faisceau de connexion à créer.   
...
-	La valeur de l'élément **KernelShape** indique qu'il s'agit d'une couche de normalisation de même signature, dans laquelle le voisinage est un rectangle de 3x3. 
-	La valeur par défaut du paramètre **Remplissage** est False. La couche de destination n'a donc que 10 nœuds par dimension. Pour inclure un nœud dans la couche de destination correspondant à tous les nœuds de la couche source, ajoutez la chaîne Padding = [true, true, true]; et remplacez la taille de l'élément RN1 par [5, 12, 12].  
...
###Définir un réseau neuronal personnalisé simple : Exemple de modèle " Hello World "
...
-	La première ligne définit la couche d'entrée, appelée " Data ", qui contient 100 nœuds, chacun représentant une fonctionnalité dans les exemples d'entrée.
...
-	La troisième ligne définit la couche de sortie, appelée " O ", qui contient 10 nœuds de sortie. Pour les réseaux neuronaux de classification, il y a un nœud de sortie par classe. Le mot clé **sigmoid** indique que la fonction de sortie est appliquée à la couche de sortie.   
