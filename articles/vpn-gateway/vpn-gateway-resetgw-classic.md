...
Cet article vous guide dans la réinitialisation de votre passerelle VPN Azure à l’aide des applets de commande PowerShell. Ces instructions s’appliquent au modèle de déploiement classique. Actuellement, les passerelles de réseau virtuel qui sont créés dans le modèle de déploiement de Resource Manager ne peuvent pas être réinitialisées.
