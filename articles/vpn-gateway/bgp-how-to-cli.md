---
title: "Configurer BGP sur des passerelles VPN Azure : Resource Manager : CLI | Microsoft Docs"
description: "Cet article vous guide dans la configuration de BGP avec des passerelles VPN Azure à l’aide d’Azure Resource Manager et de CLI."
...
* [Activer BGP pour la passerelle VPN](#enablebgp) (Requis)
  ...
  Vous pouvez ensuite configurer les sections suivantes, ou les deux :
...
### <a name="step-1---create-and-configure-testvnet1"></a>Étape 1 : créez et configurez TestVNet1
...
Créez la passerelle de réseau virtuel pour TestVNet1. BGP requiert l’utilisation d’une passerelle VPN basée sur un itinéraire. Vous avez également besoin du paramètre supplémentaire ’-Asn’ pour définir l’ASN (numéro AS) pour TestVNet1. La création d’une passerelle peut prendre un certain temps (45 minutes ou plus). 

Si vous exécutez cette commande à l’aide du paramètre « --no-wait », vous ne voyez aucun commentaire ni sortie. Le paramètre ’--no-wait’ permet à la passerelle d’être créée en arrière-plan. Cela ne signifie pas que la passerelle VPN termine la création immédiatement.
...
Exécuter la commande suivante et vérifier la section bgpSettings en haut de la sortie
...
Avant de continuer, assurez-vous que vous avez terminé la section [Activer BGP pour la passerelle VPN](#enablebgp) de cet exercice et que vous êtes toujours connecté à Subscription 1. Notez que dans cet exemple, vous créez un nouveau groupe de ressources. Remarquez également les deux paramètres supplémentaires pour la passerelle de réseau local : Asn et BgpPeerAddress.
...
Dans cette étape, vous allez créer la connexion entre TestVNet1 et Site5. Vous devez spécifier le paramètre « --enable-bgp » pour activer le BGP sur cette connexion. 

Dans cet exemple, la passerelle de réseau virtuel et une passerelle de réseau local sont présents dans les différents groupes de ressources. Lorsque les passerelles se trouvent dans différents groupes de ressources, vous devez spécifier l’ID de ressource complet des deux passerelles pour configurer une connexion entre les réseaux virtuels.
...
Les instructions suivantes sont la suite des étapes des précédentes sections. Vous devez terminer la section [Activer BGP pour la passerelle VPN](#enablebgp) pour créer et configurer TestVNet1 et la passerelle VPN avec le protocole BGP.
...
Dans cet exemple, les réseaux virtuels appartiennent au même abonnement. Vous pouvez configurer des connexions de réseau virtuel à réseau virtuel entre différents abonnements. Pour en savoir plus, voir [Configurer une connexion de réseau virtuel à réseau virtuel](vpn-gateway-howto-vnet-vnet-cli.md). Veillez à ajouter l’argument « -EnableBgp $True » lors de la création de connexions pour activer BGP.
...
Créez la passerelle de réseau virtuel pour TestVNet2. Vous devez substituer la valeur par défaut de l’ASN sur vos passerelles VPN Azure. Les ASN des réseaux virtuels connectés doivent être différents pour activer BGP et le routage de transit.
...
Dans l’exemple suivant, la passerelle de réseau virtuel et une passerelle de réseau local sont présentes dans les différents groupes de ressources. Lorsque les passerelles se trouvent dans différents groupes de ressources, vous devez spécifier l’ID de ressource complet des deux passerelles pour configurer une connexion entre les réseaux virtuels. 
...
Créez la connexion TestVNet1 à TestVNet2 et de TestVNet2 à TestVNet1. Veillez à remplacer les ID d’abonnement par les vôtres.
...
Vous établissez la topologie du réseau indiquée dans l’exemple suivant en effectuant les trois sections de cet exercice :
