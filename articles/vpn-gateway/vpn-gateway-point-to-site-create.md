<properties
   pageTitle="Configuration d’une connexion VPN de point à site à un réseau virtuel à l’aide du portail Classic | Microsoft Azure"
...
Cet article s’applique aux connexions de passerelle VPN de point à site à un réseau virtuel créées à l’aide du **modèle de déploiement classique** (Gestion des services) et du portail Classic. Si certaines étapes se réfèrent au portail Azure, nous associons l’article correspondant depuis cette page.
...
La configuration d’une connexion point à site est divisée en 4 sections. L’ordre dans lequel vous configurez chacun de ces éléments étant important, n’ignorez et ne devancez aucune étape.


- La **Section 1** vous guide au cours de la création d’un réseau virtuel et une passerelle VPN.
- La **Section 2** vous aide à créer les certificats utilisés pour l’authentification et à les charger.
- La **Section 3** vous guide à travers les étapes d’exportation et d’installation des certificats de votre client.
- La **Section 4** vous guide à travers les étapes de configuration de votre client VPN.
...
1. Connectez-vous au [portail Azure Classic](https://manage.windowsazure.com/). Notez que ces étapes utilisent le portail Classic, et non le portail Azure.
...
1. Dans la page **Réseaux** du portail Azure Classic, cliquez sur le réseau virtuel que vous venez de créer, puis accédez à la page **Tableau de bord**.

2. Dans le bas de la page **Tableau de bord**, cliquez sur **Créer une passerelle**. Vous voyez apparaître le message suivant : **Souhaitez-vous créer une passerelle pour réseau virtuel « votre\_réseau » ?**. Cliquez sur **Oui** pour initialiser la création de la passerelle. Cette opération peut prendre environ 15 minutes.
...
Si vous souhaitez utiliser un certificat auto-signé, les étapes ci-dessous vous guideront le long du processus. Si vous envisagez d’utiliser une solution de certificat d’entreprise, les étapes présentes dans chaque section seront différentes, mais vous devrez tout de même effectuer les étapes suivantes :

### Partie 1 : identifier ou générer un certificat racine

Si vous n’utilisez pas de solution de certificat d’entreprise, vous devez générer un certificat racine auto-signé. Les étapes décrites dans cette section ont été écrites pour Windows 8. Pour connaître les étapes relatives à Windows 10, référez-vous à l’article [Utilisation des certificats racine auto-signés pour les configurations point à site](vpn-gateway-certificates-point-to-site.md).
...
1. Accédez au dossier Visual Studio Tools et démarrez l’invite de commandes en tant qu’administrateur.

2. La commande figurant dans l’exemple suivant crée et installe un certificat racine dans le magasin de certificats Personnel de votre ordinateur. Elle crée également un fichier *.cer* correspondant que vous chargerez par la suite dans le Portail Azure Classic.

3. Accédez au répertoire dans lequel vous souhaitez stocker le fichier .cer, puis exécutez la commande suivante (*RootCertificateName* correspond au nom de certificat à utiliser). L’exécution de l’exemple suivant sans aucune modification entraîne la création d’un certificat racine et du fichier *RootCertificateName.cer* correspondant.
...
2. Dans le Portail Azure Classic, cliquez dans la page **Certificats** de votre réseau virtuel sur **Télécharger un certificat racine**.

3. Dans la page **Télécharger un certificat**, recherchez le certificat racine .cer, puis cliquez sur la coche correspondante.

### Partie 3 : générer un certificat client

Les étapes ci-dessous servent à générer un certificat client à partir du certificat racine auto-signé. Les étapes décrites dans cette section ont été écrites pour Windows 8. Pour connaître les étapes relatives à Windows 10, référez-vous à l’article [Utilisation des certificats racine auto-signés pour les configurations point à site](vpn-gateway-certificates-point-to-site.md). Si vous utilisez une solution de certificat d’entreprise, suivez les instructions accompagnant la solution que vous utilisez.
...
Pour la connexion au réseau virtuel, vous devrez également configurer votre client VPN. Le client nécessite un certificat client et la configuration appropriée du client VPN pour la connexion. Pour configurer votre client VPN, exécutez les étapes suivantes dans l’ordre :
...
3. Sélectionnez le package de téléchargement correspondant au système d’exploitation client sur lequel il sera installé :
...
### Partie 3 : vérifier la connexion VPN
