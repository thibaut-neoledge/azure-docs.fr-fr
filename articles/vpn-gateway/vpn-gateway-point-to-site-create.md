<properties
   pageTitle="Configuration d’une connexion VPN point à site à un réseau virtuel Azure | Microsoft Azure"
   description="Connectez-vous en toute sécurité à votre réseau virtuel Azure en créant une connexion VPN de point à site. Instructions pour les réseaux virtuels créés à l’aide du modèle de déploiement de la gestion de service (classique)."
...
# Configuration d’une connexion VPN de point à site vers un réseau virtuel


Cet article porte sur les connexions point à site pour les réseaux virtuels créés à l’aide du modèle de déploiement classique (Gestion des services).

**À ce stade, les connexions point à site à un réseau virtuel créé à l’aide d’un modèle de déploiement Azure Resource Manager ne sont pas prises en charge.** Cette page sera mise à jour lorsque cette fonctionnalité sera prise en charge pour le modèle de déploiement du Gestionnaire des ressources.

[AZURE.INCLUDE [vpn-gateway-sm-rm](../../includes/vpn-gateway-sm-rm-include.md)]

La procédure ci-après vous guidera tout au long des étapes de création d’une connexion point à site sécurisée à un réseau virtuel. Bien que la configuration d’une connexion point à site nécessite plusieurs étapes, elle constitue un excellent moyen de disposer d’une connexion sécurisée entre votre ordinateur et votre réseau virtuel sans exiger l’acquisition et la configuration d’un périphérique VPN. La configuration d’un VPN de point à site comprend trois parties principales : le réseau virtuel et la passerelle VPN, les certificats utilisés pour l’authentification, ainsi que le client VPN utilisé pour la connexion au réseau virtuel. L’ordre dans lequel vous configurez chacun de ces éléments étant important, n’ignorez et ne devancez aucune étape.


1. Créer un réseau virtuel et une passerelle VPN
2. Générer et télécharger des certificats
3. Configurer votre client VPN

## 1\. Créer un réseau virtuel et une passerelle VPN


Une connexion de point à site nécessite un réseau virtuel avec une passerelle de routage dynamique. Les étapes suivantes vous guident tout au long des étapes qui suivent :
...
1. Connectez-vous au **portail Azure** (pas le portail en version préliminaire).
...
Les certificats sont utilisés pour authentifier les clients VPN relatifs aux VPN de point à site. Autrefois, vous deviez générer votre propre certificat auto-signé. À présent, vous pouvez utiliser des certificats que vous avez générés à l’aide d’une solution d’entreprise. Vous pouvez télécharger jusqu’à 20 certificats racine vers Azure.
...
4. Accédez au répertoire dans lequel vous souhaitez stocker le fichier .cer, puis exécutez la commande suivante, où *RootCertificateName* correspond au nom à utiliser pour le certificat. L’exécution de l’exemple suivant sans aucune modification entraînera la création d’un certificat racine et du fichier *RootCertificateName.cer* correspondant.
...
### Étape 2 : télécharger le fichier de certificat racine .cert dans le portail Azure
...
Les étapes ci-dessous servent à générer un certificat client à partir du certificat racine auto-signé. Si vous utilisez une solution de certificat d’entreprise, suivez les instructions accompagnant la solution que vous utilisez.
...
### Étape 1 : créer le package de configuration client VPN.
...
Les systèmes d’exploitation clients pris en charge sont les suivants :
 ...
 - Windows 10 (validation en cours)
...
### Étape 2 - Installer le package de configuration VPN sur le client et démarrer la connexion

1. Copiez le fichier de configuration localement sur l’ordinateur que vous souhaitez connecter à votre réseau virtuel, puis double-cliquez sur le fichier .exe. Une fois le package installé, vous pouvez démarrer la connexion VPN. Notez que le package de configuration n’est pas signé par Microsoft. Si vous le souhaitez, vous pouvez signer le package à l’aide du service de signature de votre organisation ou le signer vous-même au moyen de [SignTool](http://go.microsoft.com/fwlink/p/?LinkId=699327). Il vous est possible d’utiliser le package sans le signer. Toutefois, si le package n’est pas signé, un avertissement s’affiche au moment où vous l’installez.
...
### Étape 3 - Vérifier la connexion VPN
...
Pour plus d’informations sur la connectivité de réseau virtuel entre locaux, consultez l’article [À propos des connexions sécurisées entre locaux pour les réseaux virtuels](vpn-gateway-cross-premises-options.md).

Si vous souhaitez configurer une connexion VPN site à site, consultez l’article [Créer un réseau virtuel avec une connexion VPN site à site](vpn-gateway-site-to-site-create.md).
