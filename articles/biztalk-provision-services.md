<properties urlDisplayName="Provision BizTalk Services in management portal" pageTitle="Cr&eacute;ation de services BizTalk dans le portail de gestion | Azure" metaKeywords="Get started Azure biztalk services, provision, Azure unstructured data" description="D&eacute;couvrez comment approvisionner un service BizTalk dans le portail de gestion Azure et comment cr&eacute;er une base de donn&eacute;es SQL et un compte de stockage facultatifs." metaCanonical="http://www.windowsazure.com/fr-fr/manage/services/biztalk-services/provisioning-biztalk-service" services="biztalk-services" documentationCenter="" title="BizTalk Services&nbsp;: approvisionnement &agrave; l'aide du portail de gestion Azure" authors="mandia" solutions="" manager="dwrede" editor="cgronlun" />
...
**Conseil**
Pour vous connecter au portail de gestion Azure, vous devez disposer d'un compte et d'un abonnement Azure. Si vous ne possédez pas de compte, vous pouvez créer un compte d'évaluation gratuit en quelques minutes. Consultez [Version d'évaluation gratuite d'Azure][Version d'évaluation gratuite d'Azure].
...
4.  Entrez les paramètres du service BizTalk.
    ...
    |--------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | **BizTalk Service Name** | Vous pouvez entrer n'importe quel nom, mais soyez spécifique. Voici quelques exemples :                                                                                                                                                                                                                                   
                                 *masociété*.biztalk.windows.net                                                                                                                                                                                                                                                                                           
                                 *masociétémonapplication*.biztalk.windows.net                                                                                                                                                                                                                                                                             
                                 *monapplication*.biztalk.windows.net                                                                                                                                                                                                                                                                                      
                                 « .biztalk.windows.net » est automatiquement ajouté au nom que vous entrez. L'URL obtenue est utilisée pour accéder à votre service BizTalk. Par exemple **https://*monapplication*.biztalk.windows.net**.                                                                                                                |
    | **Édition**              | Si vous êtes en phase de test ou de développement, choisissez **Développeur**. Si vous êtes en phase de production, reportez-vous au document [BizTalk Services : Tableau comparatif des éditions][BizTalk Services : Tableau comparatif des éditions] pour déterminer l'édition (**Premium**, **Standard** ou **De base**) la mieux adaptée à votre scénario d'entreprise. |
    | **Région**               | Sélectionnez la région géographique dans laquelle héberger votre service BizTalk.                                                                                                                                                                                                                                         |
    | **Domain URL**           | **Facultatif**. Par défaut, l'URL du domaine est *YourBizTalkServiceName*.biztalk.windows.net. Un domaine personnalisé peut également être entré. Par exemple, si votre domaine est *contoso*, vous pouvez entrer :                                                                                                       
                                 *MaSociété*.contoso.com                                                                                                                                                                                                                                                                                                   
                                 *MaSociétéMonApplication*.contoso.com                                                                                                                                                                                                                                                                                     
                                 *MonApplication*.contoso.com                                                                                                                                                                                                                                                                                              
                                 *NomServiceBizTalk*.contoso.com                                                                                                                                                                                                                                                                                           |
...
5.  Entrez les paramètres de stockage et de base de données :
    ...
    <p>Créez la base de données des suivis et le compte de stockage de surveillance ou d'archive dans la même région que le service BizTalk.</p>
    </div></td>
    </tr>
    </tbody>
    </table>
...
6.  Entrez les paramètres de base de données :
    ...
    |---------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | **Nom**                   | Disponible lorsque l'option **Create a new SQL Database instance** est sélectionnée dans l'écran précédent.                                                                
                                  Entrez le nom de la base de données SQL que votre service BizTalk doit utiliser.                                                                                           |
    | **Serveur**               | Disponible lorsque l'option **Create a new SQL Database instance** est sélectionnée dans l'écran précédent.                                                                
                                  Sélectionnez un serveur de base de données SQL existant ou créez un serveur de base de données SQL.                                                                        |
    | **Server Login Name**     | Entrez le nom d'utilisateur de connexion.                                                                                                                                  |
    | **Server Login Password** | Entrez le mot de passe de connexion.                                                                                                                                       |
    | **Région**                | Disponible lorsque l'option **Create a new SQL Database instance** est sélectionnée. Sélectionnez la région géographique dans laquelle héberger votre base de données SQL. |
...
<td>
L'abonnement détermine qui peut se connecter au portail de gestion Azure. Le titulaire du compte crée l'abonnement dans [Abonnements Azure][Abonnements Azure].
Le compte Azure peut avoir plusieurs abonnements et peut être géré par toute personne y étant autorisée. Par exemple, le titulaire de votre compte Azure crée un abonnement nommé *BizTalkServiceSubscription* et donne aux administrateurs BizTalk de votre société (par exemple, ContosoBTSAdmins@live.com) accès à cet abonnement. Dans ce scénario, les administrateurs BizTalk se connectent au portail de gestion Azure et disposent des droits d'administrateur complets pour tous les services hébergés dans l'abonnement, y compris Azure BizTalk Services. Comme ils ne sont pas les titulaires du compte Azure, ils n'ont aucun accès aux informations de facturation.
[Gestion des abonnements et des comptes de stockage dans le portail de gestion Azure][Gestion des abonnements et des comptes de stockage dans le portail de gestion Azure] fournit plus d'informations.
...
La mise à l'échelle de la base de données SQL est automatiquement configurée. En général, la mise à l'échelle par défaut est suffisante pour un service BizTalk. La modification de la mise à l'échelle a des conséquences sur la tarification. Consultez [Comptes et facturation dans la base de données SQL Azure][Comptes et facturation dans la base de données SQL Azure]
**Remarques**

-   Lorsque vous créez un serveur et une base de données SQL Azure, les services Azure sont automatiquement activés. Le service BizTalk nécessite l'activation des services Azure.
-   Si vous créez une base de données SQL Azure sur un serveur SQL Azure existant, les règles de pare-feu du serveur ne sont pas modifiées. Il est donc possible que d'autres services Azure ne soient pas autorisés à accéder aux bases de données du serveur.
...
-   Fichiers journaux qui surveillent le service BizTalk. La sortie de la surveillance s'affiche également dans l'onglet Surveillance du portail de gestion Azure.
-   Lors de la création d'un contrat X12 ou AS2 entre des partenaires, vous pouvez activer la fonctionnalité d'archivage pour stocker les propriétés des messages. Ces données sont enregistrées dans le compte de stockage.
 ...
 Pour plus d’informations, consultez [Stockage][Stockage].
 ...
 **Informations importantes sur le certificat SSL**

-   La date d'expiration du certificat doit être comprise dans les 5 prochaines années.
-   Tous les certificats privés exigent un mot de passe. Retenez ce mot de passe. Il est également recommandé de communiquer ce mot de passe à vos administrateurs.
-   Les certificats auto-signés sont utilisés dans des environnements de test/développement. Lorsque vous utilisez des certificats auto-signés, importez le certificat dans votre magasin de certificats personnels et dans le magasin de certificats Autorités de certification racines de confiance.
...
-   **Utilisation avancée de la clé** : Au minimum, Azure BizTalk Services exige l'authentification du serveur.
-   **Nom commun** : Entrez le nom de domaine complet de l'URL de votre service Azure BizTalk. Consultez [Créer un service BizTalk][Création d'un service BizTalk] dans cette rubrique.
...
-   [Hybrid Connections][Connexions hybrides]
