<properties 
	pageTitle="Étape 5 : publication du service web Machine Learning | Microsoft Azure" 
	description="Étape 5 de la procédure pas à pas du développement d’une solution prédictive : publication d’une expérience d’évaluation en tant que service web dans Machine Learning Studio." 
...
# Étape 5 de la procédure pas à pas : publication du service web Azure Machine Learning
...
5.	**Publier le service web**
6.	[Accéder au service web](machine-learning-walkthrough-6-access-web-service.md)

----------

Pour que ce modèle prédictif soit utile à d'autres personnes, nous le publions comme service web sur Azure.

Jusqu’à présent, nous avons réalisé l’expérience avec la formation de notre modèle. Mais le service publié ne va plus être formé ; il va noter l'entrée de l'utilisateur. Par conséquent, nous allons effectuer certaines tâches de préparation, puis publier cette expérience en tant que service web opérationnel accessible par les utilisateurs. Un utilisateur pourra envoyer au service un ensemble de données d’application sur le crédit : le service renverra la prédiction du risque sur le crédit.
...
Nous sommes maintenant prêts à publier ce modèle.

##Convertissez l'expérience d'apprentissage en une expérience de notation

La conversion d’une expérience d’évaluation se déroule en trois étapes :
...
Vous vous demandez peut-être pourquoi nous avons laissé le jeu de données Données de carte de crédit allemande UCI dans l’expérience d’évaluation. Ce service va utiliser les données de l'utilisateur et non le jeu de données d'origine : pourquoi les laisser connectés ?
...
##Test du service web
Dans la page **TABLEAU DE BORD**, cliquez sur le lien **Test** sous **Point de terminaison par défaut**. Une boîte de dialogue vous demande d’entrer les données du service. Les colonnes sont identiques à celles du jeu de données d'origine du risque sur le crédit allemand.
