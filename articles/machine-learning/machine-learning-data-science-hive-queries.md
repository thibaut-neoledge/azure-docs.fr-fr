...
Cette requête Hive suppose que `<datetime field>` est au format par défaut.
...
Dans cette requête, si `<datetime field>` a le format `03/26/2015 12:04:39`, alors `'<pattern of the datetime field>'` doit être `'MM/dd/yyyy HH:mm:ss'`. Pour le tester, les utilisateurs peuvent exécuter :
