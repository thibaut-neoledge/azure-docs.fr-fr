...
Nous pouvons ajouter des en-têtes de colonne en utilisant le module [Éditeur de métadonnées][metadata-editor]. Vous utilisez le module [Éditeur de métadonnées][metadata-editor] pour modifier des métadonnées associées à un jeu de données. Dans ce cas, il peut fournir plus de noms conviviaux pour les en-têtes de colonne.

Pour utiliser l’[Éditeur de métadonnées][metadata-editor], vous spécifiez les colonnes à modifier (dans ce cas, toutes les colonnes), puis l’action à effectuer sur les colonnes (en l’occurrence, modifier les en-têtes de colonne).

1.	Dans la palette des modules, tapez « métadonnées » dans la zone **Rechercher**. L’[Éditeur de métadonnées][metadata-editor] apparaît dans la liste des modules.
2.	Cliquez sur le module [Éditeur de métadonnées][metadata-editor] et faites-le glisser sur le canevas avant de le déposer sous le jeu de données que nous avons ajouté précédemment.
3.	Connectez le jeu de données à l’[Éditeur de métadonnées][metadata-editor] : cliquez sur le port de sortie du jeu de données (le petit cercle en bas du jeu de données), faites glisser vers le port d’entrée de l’[Éditeur de métadonnées][metadata-editor] (le petit cercle en haut du module), puis relâchez le bouton de la souris. Le jeu de données et le module restent connectés même si vous opérez des déplacements sur le canevas.
    ...
    > [AZURE.TIP] Vous pouvez ajouter un commentaire dans un module en double-cliquant sur ce module, puis en saisissant du texte. Ceci peut vous aider à voir d’un seul coup d’œil ce que fait chaque module dans votre expérience. Dans ce cas, double-cliquez sur le module [Éditeur de métadonnées][metadata-editor] et tapez le commentaire « Ajouter des en-têtes de colonne ». Cliquez n’importe où sur le canevas pour fermer la zone de texte. Cliquez sur la flèche vers le bas sur le module pour afficher le commentaire.

4.	Sélectionnez l’[Éditeur de métadonnées][metadata-editor], puis dans le panneau **Propriétés** à droite du canevas, cliquez sur **Lancer le sélecteur de colonne**.
...
1.	Recherchez le module [Fractionner les données][split], faites-le glisser sur le canevas, et connectez-le au dernier module [Éditeur de métadonnées][metadata-editor].
