...
> [AZURE.NOTE]Vous devez exécuter les cmdlets `Switch-AzureMode -Name AzureResourceManager` et `Add-AzureAccount` avant d'exécuter la cmdlet [New-AzureDataFactoryTable](https://msdn.microsoft.com/library/azure/dn835096.aspx), afin de vérifier que les cmdlets Azure PowerShell sont disponibles et que l'abonnement Azure approprié est sélectionné pour l'exécution de la commande. Pour obtenir la documentation relative à ces cmdlets, consultez [Switch-AzureMode](https://msdn.microsoft.com/library/dn722470.aspx) et [Add-AzureAccoun](https://msdn.microsoft.com/library/azure/dn790372.aspx).
 
Les définitions reposant sur JSON dans les tables utilisent les noms suivants :

* le **nom de la table** dans le serveur SQL local est *nyctaxi\_data*
...
Copiez la définition de JSON de la table dans un fichier appelé *onpremtabledef.json* et enregistrez-le dans un emplacement connu (ici supposé comme étant *C:\\temp\\onpremtabledef.json*). Créez la table dans ADF avec la cmdlet Azure PowerShell suivante.
...
Copiez la définition de JSON de la table dans un fichier appelé *bloboutputtabledef.json* et enregistrez-le dans un emplacement connu (ici supposé comme étant *C:\\temp\\bloboutputtabledef.json*). Créez la table dans ADF avec la cmdlet Azure PowerShell suivante.
...
Copiez la définition de JSON de la table dans un fichier appelé *AzureSqlTable.json* et enregistrez-le dans un emplacement connu (ici supposé comme étant *C:\\temp\\AzureSqlTable.json*). Créez la table dans ADF avec la cmdlet Azure PowerShell suivante.

	New-AzureDataFactoryTable -ResourceGroupName adfdsprg -DataFactoryName adfdsp -File C:\temp\AzureSqlTable.json  

##<a name="adf-pipeline"></a>Définition et création du pipeline
...
Copiez cette définition de JSON du pipeline dans un fichier appelé *pipelinedef.json* et enregistrez-le dans un emplacement connu (ici supposé comme étant *C:\\temp\\pipelinedef.json*). Créez le pipeline dans ADF avec la cmdlet Azure PowerShell suivante.
...
##<a name="adf-pipeline-start"></a>Lancez le pipeline
