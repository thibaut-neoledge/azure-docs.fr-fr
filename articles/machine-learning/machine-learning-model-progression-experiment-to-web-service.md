...
Une fois que vous êtes satisfait de votre expérience, vous pouvez la déployer en tant que ***service web Azure classique*** ou ***nouveau service web Azure*** afin de permettre aux utilisateurs d’envoyer de nouvelles données et de recevoir les résultats en retour.
...
Une fois que vous êtes satisfait et que votre expérience prédictive est prête, vous pouvez déployer votre service en tant que service web classique ou nouveau service web basé sur Azure Resource Manager. Pour rendre opérationnel votre modèle en le déployant en tant que *service web classique*, cliquez sur **Déployer le service web** puis sélectionnez **Déployer le service web [classique]**. Pour déployer en tant que *nouveau service web*, cliquez sur **Déployer le service web** puis sélectionnez **Déployer le service web [nouveau]**. Les utilisateurs peuvent maintenant envoyer des données à votre modèle à l’aide de l’API REST du service web et recevoir les résultats en retour. Pour plus d’informations sur la procédure à suivre, consultez [Utilisation d’un service web Azure Machine Learning déployé à partir d’une expérience Machine Learning](machine-learning-consume-web-services.md).
...
|configuration du service web dans Studio (service web classique uniquement)|**Gérer les points de terminaison…**|gestion des points de terminaison dans le portail Azure Classic|
|gestion des points de terminaison dans le portail Azure Classic (service web classique uniquement)|**Modifier dans Studio**|zone de dessin de l’expérience Studio|
...
|&nbsp;|-*ou*- **Déployer le service web [classique]** |Si vous avez configuré l’expérience de reformation pour un déploiement, cette option permet de la déployer en tant que service web classique|
|&nbsp;|-*ou*- **Déployer le service web [Nouveau]** |Si vous avez configuré l’expérience de reformation pour un déploiement, cette option permet de la déployer en tant que nouveau service web|
|Expérience prédictive|**Déployer le service web [classique]** |Déploie l’expérience prédictive sous la forme d’un service web classique|
|Expérience prédictive|**Déployer le service web [Nouveau]** |Déploie l’expérience prédictive sous la forme d’un nouveau service web|
...
|Expérience préparée pour le déploiement|***Déployer le service web [classique]**|Déploie l’expérience en tant que service web, ouvre la page de configuration du service web classique|
|&nbsp;|-*ou*- **Déployer le service web [Nouveau]**| Déploie le service comme un nouveau service web|
