...
La machine virtuelle pour la science des données (DSVM, « Data Science Virtual Machine ») Microsoft est un environnement puissant de développement de la science des données qui vous permet d'effectuer diverses tâches de modélisation et d'exploration des données. L'environnement est déjà généré et fourni avec plusieurs outils d'analyse de données courants qui facilitent la prise en main rapide de votre analyse. La DSVM fonctionne en lien avec de nombreux services Azure et peut lire et traiter les données déjà stockées sur Azure dans Azure SQL Data Warehouse, Azure Data Lake, Azure Storage et DocumentDB. Elle peut également exploiter les outils d'analyse tels qu'Azure Machine Learning et Azure Data Factory.
...
**Conditions préalables** Vous aurez besoin d'un abonnement Azure. Vous pouvez vous inscrire à un essai gratuit [ici](https://azure.microsoft.com/free/). Des instructions de configuration d'une machine virtuelle pour la science des données sur le portail Azure sont disponibles dans [Création d'une machine virtuelle](https://ms.portal.azure.com/#create/microsoft-ads.standard-data-science-vmstandard-data-science-vm).
...
Vous êtes maintenant prêt à ouvrir un projet et à commencer à travailler !

## 2\. Utiliser un Notebook Jupyter pour explorer et modéliser vos données avec Python ou R

Le Notebook Jupyter est un environnement puissant qui fournit un « IDE » de modélisation et d'exploration de données sur navigateur. Vous pouvez utiliser Python 2, Python 3 ou R (Open Source et Microsoft R Server). Pour lancer le notebook Jupyter, cliquez sur l'icône du menu Démarrer ou du bureau intitulée **Jupyter Notebook**. Sur la DSVM vous pouvez également aller à « https://localhost:9999/ » pour accéder au notebook Jupiter. S'il vous demande un mot de passe, utilisez les instructions disponibles sur la [page de documentation DSVM](machine-learning-data-science-provision-vm.md/#how-to-create-a-strong-password-on-the-jupyter-notebook-server) pour créer un mot de passe fort d'accès au notebook Jupyter.

À FAIRE : CAPTURE D'ÉCRAN

Une fois sur le notebook, vous verrez un répertoire contenant quelques exemples de notebooks pré-intégrés à la DSVM. Vous pouvez cliquer sur le notebook pour visualiser le code. Vous pouvez exécuter chaque cellule en appuyant sur **MAJ-ENTRÉE**. Vous pouvez exécuter le notebook en cliquant sur **Cellule** -> **Exécuter**.

Vous pouvez créer un nouveau notebook en cliquant sur l'icône Jupyter (coin supérieur gauche), puis sur le bouton **Nouveau** sur la droite et en sélectionnant la langue du notebook (également appelé noyau). Actuellement, nous prenons en charge Python 2.7, Python 3.5 et R. Le noyau R prend en charge la programmation dans Open source R ainsi que Microsoft R Server, évolutif à l'échelle de l'entreprise. Une fois dans le notebook, vous pouvez explorer vos données, générer le modèle et le tester avec les bibliothèques de votre choix.


## 3\. Générer des modèles avec R et Python et les rendre opérationnels sur Azure Machine Learning

Une fois que vous avez créé et validé votre modèle, l'étape suivante consiste généralement à le déployer en production. Cela permet à vos applications clientes d'appeler les prédictions de modèle en temps réel ou par lots. Azure Machine Learning fournit un mécanisme permettant de faire fonctionner le modèle généré avec R ou Python. Lorsque vous rendez votre modèle opérationnel dans Azure Machine Learning, un service web est exposé. Il permet aux clients d'effectuer des appels REST qui transmettent des paramètres d'entrée et reçoivent des prédictions du modèle en tant que sorties. Si vous n'êtes pas encore inscrit à AzureML, vous pouvez obtenir un accès invité gratuit de 8 heures ou un espace de travail gratuit en vous rendant sur la page d'accueil [AzureML Studio](https://studio.azureml.net/) et en cliquant sur « Démarrer ».

### Générer et rendre opérationnels des modèles créés avec Python  

Téléchargez le notebook intitulé « IrisClassifierPyMLWebService » sur votre Jupyter. Voici un modèle simple créé dans Python à l'aide de SciKit-learn tel qu'il se présente dans le notebook.
...
La méthode utilisée pour déployer vos modèles Python sur AzureML consiste à encapsuler la prédiction d'un modèle dans une fonction et à la décorer avec des attributs fournis par la bibliothèque AzureML indiquant votre ID d'espace de travail AzureML, votre clé d'API, les paramètres d'entrée et les paramètres de retour.
...
REMARQUE : La bibliothèque AzureML n'est actuellement prise en charge que sur Python 2.7.

### Générer et rendre opérationnels des modèles R

Vous pouvez déployer des modèles R générés sur la machine virtuelle pour la science des données ou ailleurs sur Azure ML de la même façon que pour Python. Commencez par créer un fichier settings.json comme indiqué ci-dessous pour fournir votre ID d'espace de travail et votre jeton d'authentification. Vous écrirez ensuite un wrapper pour la fonction de prédiction du modèle. Puis vous lancerez l'appel ```publishWebService``` dans la bibliothèque AzureML, transmettant le wrapper de la fonction. Voici un extrait de code qui peut être utilisé pour publier un modèle sous la forme d'un service web dans Azure ML.
...
Pour utiliser le modèle à partir d'une application cliente, nous utilisons la bibliothèque AzureML pour rechercher le service web publié par nom à l'aide de l'appel d'API `services` afin de déterminer le point de terminaison. Il vous suffit ensuite d'appeler la fonction `consume` et de transmettre la trame de données à prédire. Le code suivant permet d'utiliser le modèle publié comme service web AzureML.
...
Les scientifiques des données peuvent partager des jeux de données volumineux, du code, etc. au sein de leur équipe. La DSVM elle-même a environ 70 Go d'espace disponible. Pour étendre votre espace de stockage, vous pouvez utiliser le service Azure File Service et l'installer sur la DSVM ou y accéder via l'API REST. L'espace maximal du partage Azure File Service est de 5 To et la limite de taille par fichier est de 1 To.
...
Dans Visual Studio, vous pouvez effectuer la même opération de clonage. Consultez la capture d'écran ci-dessous pour accéder aux outils Git et Github dans Visual Studio.

![Git dans Visual Studio](./media/machine-learning-data-science-vm-do-ten-things/VSGit.PNG)


Vous trouverez plus d'informations sur l'utilisation de Git pour travailler avec votre référentiel Github dans plusieurs ressources disponibles sur github.com. Vous pouvez trouver utile l'[aide-mémoire](https://training.github.com/kit/downloads/github-git-cheat-sheet.pdf).
...
- **** Assurez-vous que l'outil AzCopy préinstallé qui se trouve dans ```C:\Program Files (x86)\Microsoft SDKs\Azure\AzCopy\azcopy.exe``` soit ajouté à votre variable d'environnement. Pour plus d'informations sur AzCopy, consultez la [documentation AzCopy](../storage/storage-use-azcopy.md)


- **Lancez Azure Storage Explorer [ici](https://azurestorageexplorer.codeplex.com/).**
...
Exécutez la commande AzCopy dans PowerShell ou dans l'invite de commandes. Voici un exemple d'utilisation de la commande AzCopy :
...
Dans le notebook IPython intitulé **NYC Data wrangling using IPython Notebook and Azure Blob Storage** (Retraitement des données de New-York City à l'aide de Notebook IPython et Azure Blob Storage), vous pouvez utiliser le package pyodbc pour lire les données directement à partir de l'objet blob.
...
Vous pouvez voir que les données sont lues comme une trame de données :
...
- Installez la **passerelle de gestion des données** suivant ce [document](../data-factory/data-factory-move-data-between-onprem-and-cloud.md).

![Azure\_Data\_Gateway\_v2](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Gateway_v2.PNG)


- Partagez le dossier local dans lequel vos données sont stockées avec tout le monde afin que la passerelle de données Azure puisse y accéder.
...
Vous pouvez utiliser **Azure Data Lake Explorer** pour télécharger des données à partir des fichiers locaux vers le stockage Data Lake.

![Azure\_Data\_Lake\_UploadData](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Lake_UploadData.PNG)


**Déplacer des données de la machine virtuelle vers Data Lake : Azure Data Factory**

Azure Data Factory est un service permettant de créer, planifier et gérer les pipelines de données. Nous pouvons utiliser Azure Data Factory pour déplacer des données entre différents espaces de stockage. Créez [Azure Data Factory](https://azure.microsoft.com/services/data-factory/) dans le [portail Azure](http://portal.azure.com) :

![Azure\_Data\_Factory\_Create](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_Create.PNG)

Une fois la création effectuée, vous pouvez générer des pipelines pour déplacer des données entre différents espaces de stockage. Dans **Créer et déployer**, vous pouvez spécifier des groupes de données pour transférer et configurer vos pipelines.
...
Voici les principales étapes pour déplacer des données de la machine virtuelle vers Azure Data Lake. Vous trouverez des informations sur le déplacement des données à l'aide d'Azure Data Factory [ici](../data-factory/data-factory-data-movement-activities.md). Les fichiers JSON ci-dessous seront présents dans votre machine virtuelle pour la science des données.

1. **Créer des services liés**
	- Cliquez sur **Nouveau magasin de données**, puis sélectionnez **Azure Data Lake Storage** et entrez vos informations d'identification et les paramètres dans le fichier JSON.
	- Cliquez sur **Nouveau magasin de données**, puis sélectionnez **Système de fichiers** et entrez vos informations d'identification et les paramètres dans le fichier JSON.
2. **Créer des jeux de données**
	- Cliquez sur **Nouveau jeu de données**, puis sélectionnez **Azure Data Lake** et entrez le nom du **Service lié** et le **chemin d'accès du dossier** dans le fichier JSON.
	- Cliquez sur **Nouveau jeu de données**, puis sélectionnez **Fichier local** et entrez le **schéma du jeu de données**, le nom du **Service lié**, le **nom de fichier** et le **chemin d'accès du dossier** dans le fichier JSON.
3. **Créer des pipelines**
	- Cliquez sur **Nouveau pipeline**, spécifiez les **jeux de données d'entrée et de sortie**, le **type d'activité**, etc. dans le fichier JSON. 
4. **Créer des passerelles de données**
	- Enregistrez votre clé de passerelle et assurez-vous que l'enregistrement soit **Enregistré** et l'état **Démarré**.
	- Cliquez sur **Nouvelle passerelle de données** et renseignez le **nom de la passerelle de données**. La partie **Configurer** sera automatiquement paramétrée si votre passerelle de données a été correctement installée dans les étapes précédentes.
...
Les fichiers JSON utilisés dans les étapes ci-dessus sont :

**Services liés : AzureDataLakeStoreLinkedService**
...
**Services liés : OnPremisesFileServerLinkedService**
...
**Jeux de données : OnPremisesFile**
...
**Jeux de données : vmtodatalake\_tripdata**
...
**Passerelles de données : weiggateway**
...
Une fois le pipeline créé, vous pouvez le consulter dans le **Diagramme** du tableau de bord Azure Data Factory.


![](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_Find_Diagram.PNG)

Cliquez sur la zone **Diagramme** pour afficher le pipeline :

![](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_PipeLine_Diagram.PNG)

Vous pouvez surveiller le pipeline de données dans **Contenu**->**Jeux de données**->**Surveillance**

![](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_Monitor_v2.PNG)


Vous pouvez également vérifier si les données ont été déplacées vers Azure Data Lake à l'aide de **Azure Data Lake Explorer** dans **Visual Studio**.
...
Dans Visual Studio, vous pouvez lire les données à partir d'un objet blob, effectuer certaines manipulations de données et extractions de paramètres et envoyer les données résultantes vers Azure Data Lake ou Azure Blob Storage. Lorsque vous référencez les données de Blob Storage, utilisez ****wasb://** ; lorsque vous référencez les données d'Azure Data Lake, utilisez ****swbhdfs://**
...
- Vous pouvez également suivre cette [procédure pas à pas](machine-learning-data-science-process-hive-walkthrough.md) pour charger les données des taxis new-yorkais vers le cluster HDI. Voici les grandes étapes :
...
Nous allons maintenant préparer des données sous-échantillonnées (1 %) pour la modélisation. Nous pouvons utiliser ces données dans le module lecteur AML.
...
2. Vous verrez les enregistrements JSON importés sous forme de liste
3. Convertissez ensuite la liste en table afin que Power BI fonctionne avec la même
4. Puis développez les colonnes en cliquant sur l'icône Développer (celle avec l'icône « flèche gauche et flèche droite » à droite de la colonne)
5. Vous verrez que cet emplacement est un champ « Enregistrement ». Développez l'enregistrement et sélectionnez uniquement les coordonnées. Une coordonnée est une colonne de liste
6. Ensuite, ajoutez une nouvelle colonne pour convertir la colonne coordonnée de liste en une colonne LatLong séparée par des virgules et concaténant les deux éléments dans le champ de liste de coordonnées à l'aide de la formule ```Text.From([coordinates]{1})&","&Text.From([coordinates]{0})```. 
7. Enfin, convertissez la colonne ```Elevation``` en Decimal et appuyez sur Fermer et appliquer.
...
Nous avons empaqueté plusieurs outils dont nous pensons qu'ils seront en mesure de résoudre une grande partie des besoins d'analyse de données courants et de vous faire gagner du temps en évitant l'installation et la configuration de votre environnement un par un et en ne payant que ce que vous utilisez. Vous pouvez tirer parti des autres services de données et d'analyse Azure comme indiqué précédemment dans cet article pour améliorer votre environnement d'analyse. Nous comprenons que, dans certains cas, vos besoins puissent nécessiter des outils supplémentaires, notamment des outils tiers propriétaires. Vous avez un accès administratif total à la machine virtuelle pour installer les nouveaux outils dont vous avez besoin. Vous pouvez également installer des packages supplémentaires non préinstallés en Python et R. Pour Python, vous pouvez utiliser soit ```conda```, soit ```pip```. Pour R, vous pouvez utiliser ```install.packages()``` dans la console R ou utiliser l'IDE et choisir « Packages -> Installer les packages... ».
