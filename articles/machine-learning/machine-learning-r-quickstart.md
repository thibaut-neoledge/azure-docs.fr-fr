<properties title="Quick start guide to R for Azure Machine Learning Studio" pageTitle="Guide de démarrage rapide avec le langage R pour Azure Machine Learning Studio" description="Utilisation de R dans Azure Machine Learning Studio" metaKeywords="R azure, r machine learning, azure r machin elearning" services="machine-learning" solutions="" documentationCenter="" authors="larryfr" manager="paulettm" editor="cgronlun"  videoId="" scriptId="" />
...
#Guide de démarrage rapide avec le langage R pour Azure ML

Docteur Stephen F Elston.

##  Introduction

Si Azure ML intègre divers modules d'apprentissage automatique et de manipulation de données très efficaces,  le puissant langage R a été décrit comme la lingua franca de l'analyse.  Fort heureusement, l'analyse et la manipulation de données dans Azure ML peuvent être étendues grâce au langage R, qui conjugue à la fois la grande facilité de déploiement d'Azure ML et la souplesse et la capacité d'analyse approfondie du langage R.

Ce document va vous aider à vous lancer rapidement dans l'extension d'Azure ML à l'aide du langage R. Ce guide contient les informations dont vous aurez besoin pour créer, tester et exécuter du code R dans Azure ML. À travers ce guide de démarrage rapide, vous allez créer une solution de prévision complète en utilisant le langage R dans Azure ML.  
...
Les données chronologiques sont des données dont les valeurs ont un index chronologique. L'index chronologique peut être régulier, c'est-à-dire tous les mois ou toutes les minutes, ou irrégulier. Un modèle chronologique repose sur des données chronologiques. Le langage R propose un cadre souple permettant d'analyse de manière étendue les données chronologiques.

Dans ce guide de démarrage rapide, nous allons travailler sur les données de production et de prix des produits laitiers en Californie.  Ces données comportent des informations mensuelles sur la production de plusieurs produits laitiers, ainsi que le prix de la matière grasse du lait, produit de base de référence.

Les données utilisées dans cet article et les scripts R peuvent être [téléchargé ici][download]. À l'origine, ces données ont été synthétisées à partir des informations disponibles sur le site de l'Université du Wisconsin à l'adresse http://future.aae.wisc.edu/tab/production.html.

###	Organisation

À travers plusieurs étapes successives, vous allez apprendrez à créer, tester et exécuter du code R d'analyse et de manipulation de données dans l'environnement Azure ML.  

* Dans un premier temps, nous explorerons les bases de l'utilisation du langage R dans l'environnement Azure ML Studio.

* Après quoi, nous examinerons différents aspects de l'E/S de données, du code R et des graphiques dans l'environnement Azure ML.
...
* [Interaction avec le code R dans ML Studio](#mlstudio)

* [Obtention de données dans et hors du module d'exécution de script R](#scriptmodule)

* [Filtrage et transformation des données](#filtering)

* [Objets de série chronologique et analyse des corrélations](#timeseries)

* [Exemple de série chronologique : prévision saisonnière](#seasonalforecasting)

* [Annexe A - Guide de RStudio](#appendixa)

* [Annexe B - Informations supplémentaires](#appendixb)

##<a id="mlstudio"></a>Interaction avec le code R dans ML Studio

Cette section vous fait découvrir certains principes de l'interaction avec le code R dans l'environnement ML Studio.  Le langage R offre un outil efficace pour créer des modules d'analyse et de manipulation de données personnalisés au sein de l'environnement Azure ML.

Nous utiliserons RStudio pour développer, tester et déboguer du code R à petite échelle.  Ce code sera ensuite coupé et collé dans un module d'exécution de script R (ou " Execute R script ") dans Studio ML, prêt à être exécuté.  

###Module d'exécution de script R

Dans ML Studio, les scripts R sont exécutés dans le module d'exécution de script R.  Un exemple du module d'exécution de script R dans ML Studio est illustré dans la figure 1.

 ![The ML Studio environment showing the Execute R Script module selected][1]

*Figure 1 : environnement ML Studio avec le module d'exécution de script R sélectionné.*

Dans la figure 1, examinons les principaux éléments de l'environnement ML Studio permettant de travailler avec le module d'exécution de script R.
...
- La partie inférieure du volet droit présente certaines propriétés du module d'exécution de script R. Vous pouvez afficher les journaux d'erreurs et de sortie en cliquant aux endroits appropriés de ce volet.

Bien entendu, nous reviendrons plus en détail sur le module d'exécution de script R au fil du présent document.

Lorsqu'il s'agit d'utiliser des fonctions R complexes, je vous recommande de modifier, tester et déboguer dans RStudio. Comme pour tout projet de développement logiciel, développez votre code graduellement et testez-le dans un petit cas de test simple. Ensuite, coupez vos fonctions et collez-les dans la fenêtre de script R du module d'exécution de script R.  Cette approche vous permet de tirer parti de l'IDE de RStudio et de la puissance d'Azure ML.  

####Exécution de code R

Pour exécuter du code R dans le module d'exécution de script R, il suffit d'exécuter l'expérimentation en cliquant sur le bouton **Run** (Exécuter).  Une fois l'exécution terminée, une coche apparaît sur l'icône du module d'exécution de script R.

####Codage R défensif pour Azure ML

Si vous développez du code R pour, par exemple, un service web à l'aide d'Azure ML, vous devez absolument prévoir la façon dont votre code traitera une entrée de données inattendue et les exceptions. Dans un souci de clarté, j'ai évité de surcharger les exemples présentés dans ce document de code relevant de la vérification et de la gestion des exceptions.  Cependant, je vous montrerai par la suite plusieurs exemples de fonctions qui font appel à la fonctionnalité de gestion des exceptions du langage R.  
...
####Débogage et test du code R dans ML Studio

Encore une fois, je vous recommande de tester et de déboguer votre code R à petite échelle dans RStudio.  Cependant, dans certains cas, vous devrez analyser les problèmes de code R dans le module d'exécution de script R lui-même.  De plus, vous aurez tout intérêt à vérifier les résultats dans ML Studio.

La sortie résultant de l'exécution de votre code R sur la plateforme Azure ML se trouve principalement dans le fichier output.log. Vous pourrez trouver des informations complémentaires dans le fichier error.log.  

Si une erreur se produit dans ML Studio pendant l'exécution de votre code R, vous devez dans un premier temps examiner le fichier error.log. Ce fichier peut contenir des messages d'erreur utiles susceptibles de vous aider à cerner l'erreur et à la corriger.  Pour afficher le fichier error.log, cliquez sur **View error log** (Afficher le journal d'erreurs) dans le **volet de propriétés** du module d'exécution de script R contenant l'erreur.

Par exemple, j'ai exécuté le code R ci-dessous, avec une variable y non définie, dans un module d'exécution de script R :
...
Il apparaît ici nécessaire de regarder dans le fichier output.log pour consulter le message d'erreur R. Il convient pour cela de cliquer sur le module d'exécution de script R et sur l'élément **View output.log** (Afficher output.log) dans le **volet de propriétés** à droite. Dans la nouvelle fenêtre de navigateur qui s'ouvre, voici les informations qui s'affichent :
...
####Packages dans ML Studio

Azure ML est fourni avec plus de 350 packages R préinstallés.  Vous pouvez utiliser le code ci-dessous dans le module d'exécution de script R pour extraire la liste des packages préinstallés.
...
Si, pour l'heure, vous ne comprenez pas la dernière ligne de ce code, poursuivez la lecture de ce document.  Vous trouverez un exposé complet sur l'utilisation du langage R dans l'environnement Azure ML.

###	Présentation de RStudio

RStudio est un environnement de développement intégré (IDE) pour R couramment utilisé. C'est l'outil que j'utiliserai pour modifier, tester et déboguer une partie du code R utilisé dans ce guide de démarrage rapide.  Une fois le code R testé et prêt, il suffit de le couper dans l'éditeur RStudio et de le coller dans un module d'exécution de script R de ML Studio.  

Si vous n'avez pas encore installé R sur votre ordinateur de bureau, je vous conseille de le faire dès maintenant. Des versions open source de R sont disponibles en téléchargement gratuit dans la section CRAN (Comprehensive R Archive Network) du site http://www.r-project.org/. Il existe des versions Windows, MacOS et Linux/UNIX. Choisissez un site miroir voisin et suivez les instructions de téléchargement.   Par ailleurs, le section CRAN contient de nombreux packages d'analyse et de manipulation de données fort utiles.
...
Un didacticiel de présentation de RStudio est disponible à l'adresse https://support.rstudio.com/hc/fr-fr/sections/200107586-Using-RStudio.
...
Dans cette section, nous allons voir comment obtenir des données dans et hors du module d'exécution de script R.  Nous examinerons les façons de gérer les différents types de données lus dans et hors du module d'exécution de script R.

Le code complet de cette section se trouve dans le fichier zip téléchargé précédemment.

###Chargement et vérification des données dans ML Studio

####<a id="loading"></a>Chargement du jeu de données

Nous allons commencer par charger le fichier **csdairydata.csv** dans Azure ML Studio.

- Démarrez votre environnement Azure ML Studio.
...
Maintenant que ML Studio contient des données, nous devons créer une expérimentation pour faire l'analyse.  
...
- Dans la zone **Search experiment items** (Rechercher dans les éléments de l'expérimentation) en haut du volet gauche, tapez **Execute R Script** (Exécution de script R).  Le module s'affiche alors dans la liste de recherche.

- Glissez-déplacez le module d'exécution de script R vers votre palette.  

- Reliez la sortie du **jeu de données csdairydata.csv** à la première entrée (**Jeu de données 1**) du module d'**exécution de script R** en partant de la gauche.
...
Cette vue contient de nombreuses informations utiles.  La ligne **Feature Type** indique les types de données qui sont affectés aux colonnes de notre jeu de données par Azure ML Studio.  Les premières lignes de ce jeu de données apparaissent également.  Il est judicieux d'effectuer un contrôle rapide de ce type avant de se lancer dans un travail sérieux.

###	Premier script R

Créons un premier script R simple pour l'expérimenter dans Azure ML Studio.  J'ai créé et testé le script suivant dans RStudio :  
...
Maintenant, je dois transférer ce script dans Azure ML Studio.  Je pourrais simplement effectuer un couper-coller.  Mais je vais ici transférer le script R via un fichier zip.

###	Entrée de données dans le module d'exécution de script R

Penchons-nous sur les entrées du module d'exécution de script R.  En l'occurrence, nous allons examiner les données du secteur laitier californien dans le module d'exécution de script R.  

Il existe trois types d'entrée possibles pour le module d'exécution de script R.  Selon votre application, vous pouvez utiliser une ou toutes ces entrées.  Il est aussi tout à fait envisageable d'utiliser un script R qui ne prenne aucune entrée.  

Passons en revue chacune de ces entrées, de gauche à droite. Pour afficher le nom d'une entrée, il suffit de placer le curseur sur l'entrée en question et de lire l'info-bulle.  

####	Script groupé

L'entrée de script groupé permet de transmettre le contenu d'un fichier zip au module d'exécution de script R.  Vous pouvez utiliser l'une des commandes suivantes pour lire le contenu du fichier zip dans votre code R :
...
> [WACOM.NOTE] Azure ML traite les fichiers contenus dans le zip comme s'ils se trouvaient dans le répertoire src/. Il est donc nécessaire de faire précéder les noms des fichiers du nom de ce répertoire.  

Nous avons déjà abordé la question du chargement des jeux de données dans la section [Chargement du jeu de données](#loading).  Dès lors que vous avez créé et testé le script R présenté dans la section précédente, procédez comme suit :
...
3.	Ajoutez ce dernier aux **jeux de données** dans ML Studio en spécifiant le type **zip**. Le fichier zip doit dès lors figurer dans vos jeux de données.

4.	Glissez-déplacez le fichier zip des **jeux de données** vers le **canevas ML Studio**.

5.	Reliez la sortie de l'icône des **données zip** à l'entrée de **script groupé** du module d'**exécution de script R**.

6.	Tapez la fonction `source()` avec le nom de votre fichier zip dans la fenêtre de code du module d'**exécution de script R**.  Dans mon cas, j'ai tapé `source("src/SimplePlot.R")`.  

7.	Pensez à cliquer sur **Save** (Enregistrer).

Une fois ces étapes effectuées, le module d'exécution de script R exécute le script R du fichier zip quand l'expérimentation est exécutée. À ce stade, votre expérimentation doit être similaire à la figure 5.
...
Exécutez votre expérimentation en cliquant sur le bouton **Run** (Exécuter).  À la fin de l'exécution, cliquez sur le module d'exécution de script R, puis sur **View output log** (Afficher le journal de sortie) dans le volet de propriétés.  Une nouvelle page doit s'ouvrir dans votre navigateur en affichant le contenu du fichier output.log.  Si vous faites défiler l'écran vers le bas, voici ce que vous devez voir :
...
> [WACOM.NOTE] Pratique, cette même sortie imprimée est disponible depuis la sortie du périphérique R du module d'exécution de script R.  Nous évoquerons les sorties du module d'exécution de script R dans la section suivante.  
...
La sortie du périphérique du module d'exécution de script R contient des messages et une sortie graphique.  La sortie et les messages d'erreur standard de R sont envoyés au port de sortie du périphérique R.  
...
*Figure 8 : sortie graphique du port du périphérique R.*  
...
Maintenant que nous pouvons lire les données du secteur laitier californien dans le code R du module d'exécution de script R, nous devons nous assurer que les données contenues dans les colonnes ont un type et un format appropriés.  
...
Si l'on examine les types de données des colonnes que nous avons entrées à la section précédente, on constate que toutes les colonnes sont de type numérique, à l'exception de la colonne intitulée " Month ", qui est de type caractère.  Convertissons-la en facteur et vérifions les résultats.  

J'ai supprimé la ligne qui a créé la matrice de nuage de points et ajouté une ligne convertissant la colonne " Month " en facteur.  Dans mon expérimentation, je vais simplement couper et coller le code R dans la fenêtre de code du module d'exécution de script R.  Une autre possibilité aurait été de mettre à jour le fichier zip et de le télécharger dans Azure ML Studio, mais cette opération nécessite plusieurs étapes.  
...
Le type de la colonne Month doit à présenter indiquer " **Factor w/ 14 levels** " (Facteur à 14 niveaux).  C'est un problème, car une année ne compte que 12 mois. Vous pouvez aussi vérifier que le type qui apparaît dans la **visualisation** du port du jeu de données de résultat est " **Categorical** " (Catégorique).

Le problème, c'est que la colonne " Month " n'a pas été codée de façon systématique.  Dans certains cas, le mois s'intitule Avril et dans d'autres, il apparaît sous forme d'abréviation (Avr.). Il est possible de résoudre ce problème en réduisant la chaîne à 3 caractères.  La ligne de code se présente désormais comme suit :
...
> [WACOM.NOTE] À partir de maintenant, dans cette section, je ne vous montrerai que le code supplémentaire que j'ajoute dans le module d'exécution de script R.  Chaque nouvelle ligne sera ajoutée **avant** la fonction `str()`. J'utilise cette fonction pour vérifier les résultats dans Azure ML Studio.

J'ajoute la ligne suivante à mon code R dans le module d'exécution de script R.
...
Si l'on examine les valeurs contenues dans les aperçus du tableau de données, on note certaines incohérences. Par exemple, produit-on vraiment plus de crème glacée que de lait en Californie ?  Bien sûr que non, au grand dam peut-être des amateurs de glaces. Les unités sont différentes. Le prix est exprimé en unités de livres américaines, le lait en unités de 1 M de livres américaines et la crème glacée en unités de 1 000 gallons américains et le fromage blanc en unités de 1 000 livres américaines. En supposant que le poids de la crème glacée est d'environ 6,5 livres par gallon, nous pouvons facilement effectuer la multiplication de conversion pour que ces valeurs soient toutes exprimées dans les mêmes unités de 1 000 livres.
...
La programmation défensive vise à éviter l'échec d'une seule fonction et d'empêcher la poursuite du traitement.  L'échec soudain d'une analyse au long cours peut s'avérer très contrariante pour les utilisateurs. Pour éviter cette situation, il convient de choisir des valeurs de retour par défaut qui limiteront les nuisances à un traitement en aval.  Un message est également généré pour avertir les utilisateurs qu'un problème s'est produit.

Si vous n'êtes pas rompu à la programmation défensive en langage R, tout ce code peut vous paraitre insurmontable. Je vous accompagnerai tout au long des étapes principales :
...
Commençons notre exemple. Glissez-déplacez un **nouveau module d'exécution de script R** dans votre expérimentation.  Reliez le port de sortie du jeu de données 1 de résultat du module d'exécution de script R existant au port d'entrée du jeu de données 1 du nouveau module d'exécution de script R.
...
La première ligne de code est un peu compliquée et une explication s'impose pour vous aider à la comprendre. Voici ce que l'on a :
...
Notez que les noms de lignes se trouvent dans une colonne du tableau de données. Cela préserve les noms de lignes quand ils sont générés à partir du module d'exécution de script R.

L'exécution du code génère la sortie présentée dans la figure 29 après avoir actionné la commande **Visualize** (Visualiser) au niveau du port du jeu de données de résultat. Les noms de lignes se trouvent dans la première colonne, comme prévu.

![Results output from the correlation analysis][20]

*Figure 29 : résultats générés par l'analyse des corrélations.*
...
Commencez par ajouter un **nouveau module d'exécution de script R** à votre expérimentation.  Reliez la sortie de **jeu de données de résultat** du module d'exécution de script R existant à l'entrée **Jeu de données 1** du nouveau module.  Le résultat doit être similaire à la figure 20.
...
*Figure 22 : graphique chronologique des données de production et de prix des produits laitiers californiens.*
...
Compte tenu de la petite échelle des données, je vais générer le modèle de tendance dans RStudio, puis couper et coller le modèle obtenu dans Azure ML. RStudio offre un environnement interactif pour ce type d'analyse interactive.
...
En guise de test, créons un graphique chronologique à partir des données de production de produits laitiers californiens en affichant la courbe de tendance.  J'ai ajouté le code suivant dans le module d'exécution de script R Azure ML (pas RStudio) pour créer le modèle et tracer un graphique.
...
Créons un autre graphique chronologique à partir des données de production laitière californienne pour voir si le modèle saisonnier fonctionne correctement.  J'ai ajouté le code suivant dans le module d'exécution de script R Azure ML pour créer le modèle et tracer un graphique :
...
L'exécution de ce code dans Azure ML génère le graphique présenté dans la figure 24.
...
1.	Création de projets
	...
	Vous pouvez organiser et gérer votre code R dans les projets avec RStudio.  Vous trouverez la documentation relative à l'utilisation de projets à l'adresse https://support.rstudio.com/hc/fr-fr/articles/200526207-Using-Projects.
...
2.	Modification et exécution de code R
	...
	RStudio offre un environnement intégré qui permet de modifier et d'exécuter du code R.  Vous trouverez la documentation à l'adresse https://support.rstudio.com/hc/fr-fr/articles/200484448-Editing-and-Executing-Code.

3.	Débogage

	RStudio intègre de puissantes fonctionnalités de débogage.  Vous trouverez la documentation relative à ces fonctionnalités à l'adresse https://support.rstudio.com/hc/fr-fr/articles/200713843-Debugging-with-RStudio.

	Les fonctionnalités de résolution des problèmes de point d'arrêt sont documentées à l'adresse https://support.rstudio.com/hc/fr-fr/articles/200534337-Breakpoint-Troubleshooting.

##<a id="appendixb"></a>ANNEXE B - Informations supplémentaires

Si vous ne connaissez pas le langage R, deux présentations sont disponibles sur le site CRAN.
...
- Le document de présentation du langage R de W. N. Venables et. al., plus approfondi, est disponible à l'adresse http://cran.r-project.org/doc/manuals/R-intro.html.
...
- bacoff est un autre ouvrage élémentaire utile.  Le site web d'accompagnement " Quick R " est une ressource utile à l'adresse http://www.statmethods.net/.
...
- Si vous souhaitez examiner plus en profondeur les aspects avancés du langage R, jetez un œil au livre à paraître " Advanced R " de Hadley Wickham.  La version en ligne de ce livre est disponible gratuitement à l'adresse http://adv-r.had.co.nz/.

Vous trouverez dans la section Task Views du site CRAN un catalogue de packages R pour l'analyse des séries chronologiques.: http://cran.r-project.org/web/views/TimeSeries.html. For information on specific time series object packages, you should refer to the documentation for that package.
