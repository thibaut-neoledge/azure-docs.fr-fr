...
Nous utiliserons RStudio pour développer, tester et déboguer du code R à petite échelle. Ce code est ensuite coupé et collé dans un module [Exécuter un script R][execute-r-script] de Machine Learning Studio prêt à être exécuté.  

###<a name="the-execute-r-script-module"></a>Module d’exécution de script R

Dans Machine Learning Studio, les scripts R sont exécutés à l’intérieur du module [Exécuter un script R][execute-r-script]. Un exemple du module [Exécuter un script R][execute-r-script] de Machine Learning Studio est présenté à la figure 1.
...
Dans la figure 1, examinons certains des éléments clés de l’environnement Machine Learning Studio pour travailler avec le module [Exécuter un script R][execute-r-script].
...
- La partie inférieure du volet droit présente certaines propriétés du module [Exécuter un script R][execute-r-script]. Vous pouvez afficher les journaux d'erreurs et de sortie en cliquant aux endroits appropriés de ce volet.

Bien entendu, nous reviendrons plus en détail sur le module [Exécuter un script R][execute-r-script] dans la suite du présent document.

Lorsqu'il s'agit d'utiliser des fonctions R complexes, je vous recommande de modifier, tester et déboguer dans RStudio. Comme pour tout projet de développement logiciel, développez votre code graduellement et testez-le dans un petit cas de test simple. Ensuite, coupez et collez vos fonctions dans la fenêtre de script R du module [Exécuter un script R][execute-r-script]. Cette approche vous permet d’exploiter l’environnement de développement intégré (IDE) de RStudio et la puissance d’Azure Machine Learning.  

####<a name="execute-r-code"></a>Exécution du code R

Pour exécuter tout code R dans le module [Exécuter un script R][execute-r-script], il suffit de cliquer sur le bouton **Exécuter**. Une fois l’exécution terminée, une coche apparaît sur l’icône du module [Exécuter un script R][execute-r-script].
...
Encore une fois, je vous recommande de tester et de déboguer votre code R à petite échelle dans RStudio. Toutefois, dans certains cas, il convient d’analyser les problèmes de code R dans le module [Exécuter un script R][execute-r-script] proprement dit. De plus, vous aurez tout intérêt à vérifier les résultats dans Machine Learning Studio.
...
Si une erreur se produit dans Machine Learning Studio pendant l’exécution de votre code R, vous devez dans un premier temps examiner le fichier error.log. Ce fichier peut contenir des messages d’erreur utiles susceptibles de vous aider à cerner l’erreur et à la corriger. Pour afficher le fichier error.log, cliquez sur **Afficher le journal des erreurs** dans le **volet des propriétés** du module [Exécuter un script R][execute-r-script] contenant l’erreur.

Par exemple, j’ai exécuté le code R ci-dessous, avec une variable y non définie, dans un module [Exécuter un script R][execute-r-script] :
...
Il apparaît ici nécessaire de regarder dans le fichier output.log pour consulter le message d’erreur R. Il convient pour cela de cliquer sur le module [Exécuter un script R][execute-r-script], puis sur l’option **Afficher output.log** dans le **volet des propriétés** à droite. Dans la nouvelle fenêtre de navigateur qui s’ouvre, voici les informations qui s’affichent :
...
Azure Machine Learning est fourni avec plus de 350 packages de langage R préinstallés. Vous pouvez utiliser le code ci-dessous dans le module [Exécuter un script R][execute-r-script] pour extraire la liste des packages préinstallés.
...
- Cliquez sur __+ NEW__ en bas à gauche de votre écran, puis sélectionnez **Jeu de données**.
...
- Cliquez sur __+ NEW__ en bas à gauche, sélectionnez **Experiment** (Expérimentation), puis **Blank Experiment** (Expérimentation à blanc).
...
- Dans la zone **Search experiment items** (Rechercher dans les éléments de l’expérimentation) en haut du volet gauche, tapez [Exécuter un script R][execute-r-script]. Le module s'affiche alors dans la liste de recherche.

- Glissez-déplacez le module [Exécuter un script R][execute-r-script] vers votre palette.  

- Reliez la sortie du jeu de données **csdairydata.csv** à la première entrée en partant de la gauche (**Jeu de données1**) du module [Exécuter un script R][execute-r-script].
...
Examinons les entrées du module [Exécuter un script R][execute-r-script]. Dans cet exemple, nous allons examiner les données du secteur laitier californien dans le module [Exécuter un script R][execute-r-script].  

Il existe trois types d’entrées possibles pour le module [Exécuter un script R][execute-r-script]. Selon votre application, vous pouvez utiliser une ou toutes ces entrées. Il est aussi tout à fait envisageable d’utiliser un script R qui ne prenne aucune entrée.  
...
L’entrée de script groupé permet de transmettre le contenu d’un fichier zip au module [Exécuter un script R][execute-r-script]. Vous pouvez utiliser l’une des commandes suivantes pour lire le contenu du fichier zip dans votre code R :
...
2.  Créez un fichier zip et copiez votre script dans ce fichier. Dans Windows, vous pouvez cliquer avec le bouton droit sur le fichier, sélectionner __Envoyer vers__, puis __Dossier compressé__. Vous créez ainsi un fichier zip contenant le fichier « simpleplot.R ».
...
5.  Reliez la sortie de l’icône des **données zip** à l’entrée de **script groupé** du module [Exécuter un script R][execute-r-script].

6.  Tapez la fonction `source()` avec le nom de votre fichier zip dans la fenêtre de code du module [Exécuter un script R][execute-r-script]. Dans mon cas, j’ai saisi `source("src/simpleplot.R")`.  

7.  Pensez à cliquer sur **Save**(Enregistrer).

Une fois ces étapes effectuées, le module [Exécuter un script R][execute-r-script] exécute le script R du fichier zip lors de l’exécution de l’expérimentation. À ce stade, votre expérimentation doit être similaire à la figure 5.
...
Exécutez votre expérimentation en cliquant sur le bouton **Run** (Exécuter). À la fin de l’exécution, cliquez sur le module [Exécuter un script R][execute-r-script], puis sur **View output log** (Afficher le journal de sortie) dans le volet des propriétés. Une nouvelle page doit s’ouvrir dans votre navigateur en affichant le contenu du fichier output.log. Si vous faites défiler l’écran vers le bas, vous devez voir quelque chose de similaire à ceci :
...
> [AZURE.NOTE] Cette même sortie imprimée est disponible dans la sortie de l’appareil R du module [Exécuter un script R][execute-r-script]. Nous évoquerons les sorties du module [Exécuter un script R][execute-r-script] dans la section suivante.  
...
La sortie de l’appareil du module [Exécuter un script R][execute-r-script] contient des messages et une sortie graphique. La sortie et les messages d'erreur standard de R sont envoyés au port de sortie de l’appareil R.  
...
Maintenant que nous pouvons lire les données du secteur laitier californien dans le code R du module [Exécuter un script R][execute-r-script], nous devons nous assurer que les données contenues dans les colonnes ont un type et un format appropriés.  
...
J’ai supprimé la ligne qui a créé la matrice de nuage de points et ajouté une ligne convertissant la colonne « Month » en facteur. Dans mon expérimentation, je vais simplement couper et coller le code R dans la fenêtre de code du module [Exécuter un script R][execute-r-script]. Une autre possibilité aurait été de mettre à jour le fichier zip et de le télécharger dans Azure Machine Learning Studio, mais cette opération nécessite plusieurs étapes.  
...
> [AZURE.NOTE] À partir de maintenant, dans cette section, je ne vous montrerai que le code supplémentaire que j’ajoute dans le module [Exécuter un script R][execute-r-script]. Chaque nouvelle ligne sera ajoutée **avant** la fonction `str()`. J’utilise cette fonction pour vérifier les résultats dans Azure Machine Learning Studio.

J’ajoute la ligne suivante à mon code R dans le module [Exécuter un script R][execute-r-script].
...
Commençons notre exemple. Glissez-déplacez un **nouveau** module [Exécuter un script R][execute-r-script] dans votre expérimentation. Reliez le port de sortie du jeu de données 1 de résultat du module [Exécuter un script R][execute-r-script] existant au port d’entrée du jeu de données 1 du nouveau module [Exécuter un script R][execute-r-script].
...
Notez que les noms de lignes se trouvent dans une colonne du tableau de données. Cela préserve les noms de lignes quand ils sont générés à partir du module [Exécuter un script R][execute-r-script].
...
Commencez par ajouter un **nouveau** module [Exécuter un script R][execute-r-script] à votre expérimentation. Reliez la sortie du **jeu de données de résultat** du module [Exécuter un script R][execute-r-script] existant à l’entrée **Jeu de données 1** du nouveau module. Le résultat doit être similaire à la figure 20.
...
En guise de test, créons un graphique chronologique à partir des données de production de produits laitiers californiens en affichant la courbe de tendance. J’ai ajouté le code suivant dans le module [Exécuter un script R][execute-r-script] Azure Machine Learning (pas RStudio) pour créer le modèle et tracer un graphique. Ce schéma est illustré dans la Figure 23.
...
Créons un autre graphique chronologique à partir des données de production laitière californienne pour voir si le modèle saisonnier fonctionne correctement. J’ai ajouté le code suivant dans le module [Exécuter un script R][execute-r-script] Azure Machine Learning pour créer le modèle et tracer un graphique.
...
<!--links-->
[annexe a]: #appendixa
[télécharger]: https://azurebigdatatutorials.blob.core.windows.net/rquickstart/RFiles.zip
