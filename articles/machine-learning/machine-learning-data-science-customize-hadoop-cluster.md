<properties 
	pageTitle="Personnaliser des clusters Hadoop pour le processus et la technologie d'analyse avancée | Microsoft Azure" 
...
Cet article explique comment personnaliser un cluster Hadoop HDInsight en installant Anaconda 64 bits (Python 2.7) sur chaque nœud lorsque le cluster est en cours d’approvisionnement dans le service HDInsight. Cette personnalisation prépare le cluster pour une utilisation avec le processus et la technologie d'analyse avancée (ADAPT) utilisés dans Azure Machine Learning. Il indique également comment accéder au nœud principal pour soumettre des tâches personnalisées au cluster.
...
Sur la troisième page de configuration, saisissez un nom d’utilisateur et un mot de passe pour le cluster Hadoop HDInsight. **Ne sélectionnez pas** l'option _Entrer les metastores Hive/Oozie_. Ensuite, cliquez sur la flèche pour passer à la page de configuration suivante.
...
1. Connectez-vous au [**portail de gestion Azure**](https://manage.windowsazure.com/), sélectionnez la valeur **HDInsight** sur la gauche, choisissez le cluster Hadoop dans la liste des clusters, cliquez sur l'onglet **CONFIGURATION** puis cliquez sur l'icône **ACTIVER DISTANT**, dans la partie inférieure de la page.
