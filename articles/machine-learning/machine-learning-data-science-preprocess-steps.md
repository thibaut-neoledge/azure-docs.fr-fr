<properties 
	pageTitle="Prétraiter et nettoyer les données | Azure" 
...
========================================
Les données brutes sont souvent bruitées, peu fiables et incomplètes. Leur utilisation pour la modélisation peut générer des résultats trompeurs. Le prétraitement et le nettoyage des données sont des étapes importantes qui doivent intervenir avant d'utiliser un jeu de données à des fins d'apprentissage automatique. Cet article présente différentes étapes de prétraitement des données, à effectuer avant d'intégrer ces données dans Azure Machine Learning. 

----------------

1. **Pourquoi prétraiter et nettoyer les données ?**
	...
	La qualité des données est essentielle pour obtenir des modèles prédictifs performants. Pour éviter de traiter des données erronées et améliorer la qualité du modèle, il faut impérativement analyser les données, détecter les anomalies le plus tôt possible et déterminer les étapes de prétraitement et de nettoyage appropriées.

2. **Quels sont les critères d'intégrité des données généralement employés ?** 
...
3. **Quelles sont les principales opérations effectuées lors du prétraitement des données ?**

	- Nettoyage des données :  compléter les valeurs manquantes, détecter et supprimer les données bruitées et les aberrations.
	- Transformation des données :  normaliser les données pour réduire le volume et le bruit.
	- Réduction des données :  échantillonner les enregistrements ou les attributs pour faciliter la manipulation des données.
	...
	- Nettoyage du texte : supprimer les caractères intégrés pouvant perturber l'alignement des données (comme les tabulations dans un fichier TSV), les nouvelles lignes qui peuvent casser des enregistrements, etc.	
	
	Les sections suivantes décrivent certaines de ces étapes de prétraitement des données.

4. **Comment gérer les valeurs manquantes ?**
...
5. **Comment normaliser les données ?**
	...
	- Normalisation min-max : adapter linéairement les données à une plage comprise, par exemple, entre 0 et 1.  La valeur minimum est 0 et la valeur maximum est 1.
...
6. **Comment discrétiser les données ?**  
...
8. **Comment nettoyer les données textuelles ?**  
...
>_Data Mining : Concepts et Techniques, 3e édition, Morgan Kaufmann, 2011, Jiawei Han, Micheline Kamber et Jian Pei
