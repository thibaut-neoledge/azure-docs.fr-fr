<properties pageTitle="Ajout d'une pagination aux données (Windows Phone) | Centre de développement mobile" description="Découvrez comment utiliser la pagination pour gérer la quantité de données renvoyées vers votre application Windows Phone à partir de Mobile Services." services="mobile-services" documentationCenter="windows" authors="ggailey777" manager="dwrede" editor=""/>
...
Ce didacticiel s'appuie sur la procédure et l'exemple d'application du didacticiel précédent [Ajout de services mobiles à une application existante](/fr-fr/documentation/articles/mobile-services-dotnet-backend-windows-phone-get-started-data/). Avant de commencer ce didacticiel, vous devez suivre intégralement celui-ci : 
...
Vous voici parvenu à la fin de la série de didacticiels présentant les principes de base de l'utilisation des données dans Mobile Services. Pour plus d'informations, consultez les rubriques Mobile Services suivantes:
