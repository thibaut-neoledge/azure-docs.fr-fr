...
Dans la [version préliminaire du portail de gestion Azure](https://portal.azure.com/), accédez à la partie **Clés** du volet de votre **Compte DocumentDB** afin d'afficher, de copier et de régénérer les clés utilisées pour accéder à votre
compte DocumentDB.
...
1.      Dans la [version préliminaire du portail Azure](https://portal.azure.com/), accédez à votre compte DocumentDB. 
...
2.      Régénérez la clé d'accès principale de votre compte de stockage.
Dans la [version préliminaire du portail de gestion Azure](https://portal.azure.com/), accédez à votre compte DocumentDB.
...
- **Strong (Forte)** - niveau de cohérence qui garantit que les opérations de lecture renvoient toujours la dernière valeur écrite.
...
- **Session (Par session)** - niveau de cohérence qui garantit des lectures monotoniques (vous ne lisez jamais d'anciennes données, puis de nouvelles données, puis d'anciennes données à nouveau), des écritures monotoniques (les écritures sont ordonnées), et que vous lisez les écritures les plus récentes dans n'importe quel point de vue du client.

- **Eventual (Éventuel)** - niveau de cohérence qui garantit que les opérations de lecture lisent toujours un sous-ensemble d'écritures valides, qui finiront par converger.

*Notez que par défaut, les comptes DocumentDB sont configurés avec une cohérence de niveau Session.  Pour plus d'informations sur les paramètres de cohérence de DocumentDB, consultez la section [Niveau de
...
5.      La progression de l'opération peut être contrôlée via le concentrateur de notifications de la version préliminaire du portail de gestion Azure.

*Notez que l'application d'une modification du paramètre de cohérence par défaut à votre compte DocumentDB peut prendre plusieurs minutes.*

## <a id="capacity"></a>Gestion des paramètres de capacité de DocumentDB
Microsoft Azure DocumentDB vous permet d'évoluer avec souplesse au fur et à mesure que les demandes de votre application changent tout au long de son cycle de vie. La mise à l'échelle de DocumentDB est réalisée en augmentant la capacité de votre compte de base de données DocumentDB via la version préliminaire du portail de gestion Azure.

Lorsque vous créez un compte de base de données, il est approvisionné avec un stockage de base de données et un débit réservé. Vous pouvez à tout moment modifier le stockage de base de données et le débit approvisionnés pour votre compte en ajoutant ou en supprimant des unités de capacité via la version préliminaire du portail de gestion Azure. 
...
4.      Cliquez sur **Enregistrer** (notez que l'opération de mise à l'échelle peut prendre plusieurs minutes ; la progression de l'opération peut être contrôlée via le concentrateur de notifications de la version préliminaire du portail de gestion Azure).

 *Notez que la version préliminaire de DocumentDB prend en charge un maximum de 5 unités de capacité par compte DocumentDB.*
 

## <a id="delete"></a>Procédure : Suppression d'un compte DocumentDB
...
1.      Dans la [version préliminaire du portail de gestion Azure](https://portal.azure.com/), accédez au compte DocumentDB à supprimer. 
...
Découvrez la [prise en main de votre compte DocumentDB](http://go.microsoft.com/fwlink/p/?LinkId=402364).
