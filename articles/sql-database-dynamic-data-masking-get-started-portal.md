<properties 
   ...
   description="Comment prendre en main le masquage de données dynamiques de base de données SQL dans le portail Azure" 
...
> [AZURE.SELECTOR]
- [Masquage de données dynamiques - Portail Azure en version préliminaire](sql-database-dynamic-data-masking-get-started.md)
...
> [AZURE.NOTE] Pour configurer le masquage des données dynamiques dans le portail Azure en version préliminaire, consultez [Prise en main du masquage de données dynamiques de base de données SQL (portail Azure en version préliminaire)](sql-database-dynamic-data-masking-get-started.md). 
...
>[Conseil.AZURE] utilisez l'option **Restriction étendue** pour limiter l'accès aux développeurs utilisant un accès direct à la base de données et exécutent des requêtes SQL pour résoudre des problèmes.
...
| **Nombre aléatoire** | **Méthode de masquage qui génère un nombre aléatoire de masquage** selon les frontières sélectionnées et les types de données réelles. Si les limites désignées sont égales, la fonction masquage sera un nombre constant.<br/><br/>![Volet de navigation][Image1] |
| **Texte personnalisé** | **Méthode de masquage qui affiche la première et la dernière lettres** et ajoute une chaîne de remplissage personnalisée entre les deux.<br/>préfixe[remplissage]suffixe<br/><br/>![Volet de navigation][Image2] |
...
4. Tapez les noms d'accès privilégiés qui doivent avoir accès aux données sensibles non masquées.
	...
	>[AZURE.TIP] Afin que la couche d'application puisse afficher des données sensibles pour les utilisateurs d'application privilégiés, ajoutez le nom de connexion de l'application utilisée pour les requêtes ainsi que la base de données. Il est vivement recommandé de limiter le nombre de connexions incluses dans cette liste afin de limiter l'exposition des données sensibles.
