...
* [Présentation d’Azure Resource Health](#what-is-azure-resource-health)
* [À quoi sert Resource Health ?](#what-is-the-resource-health-intended-for)
* [Quels contrôles d’intégrité sont effectués par Resource Health ?](#what-health-checks-are-performed-by-resource-health)
* [Que signifie chaque état Resource Health ?](#what-does-each-of-the-health-status-mean)
* [Que signifie l’état inconnu ? Y a-t-il un problème avec ma ressource ?](#what-does-the-unknown-status-mean-is-something-wrong-with-my-resource)
* [Comment puis-je obtenir de l’aide pour une ressource qui n’est pas disponible ?](#how-can-i-get-help-for-a-resource-that-is-unavailable)
* [Resource Health fait-il la différence entre une indisponibilité causée par des problèmes de plateforme et une mauvaise manipulation de l’utilisateur ?](#does-resource-health-differentiate-between-unavailability-cased-by-platform-problems-versus-something-i-did)
* [Puis-je intégrer Resource Health à mes outils d’analyse ?](#can-i-integrate-resource-health-with-my-monitoring-tools)
* [Où trouver Resource Health ?](#where-do-i-find-resource-health)
* [Resource Health est-il disponible pour tous les types de ressources ?](#is-resource-health-available-for-all-resource-types)
* [Que dois-je faire si ma ressource apparaît comme disponible, alors que ce n’est pas le cas ?](#what-should-i-do-if-my-resource-is-showing-available-but-i-believe-it-is-not)
* [Resource Health est-il disponible pour toutes les régions Azure ?](#is-resource-health-available-for-all-azure-regions)
* [En quoi Resource Health diffère du tableau de bord d’état du service ou des notifications de service de portail Azure ?](#how-is-resource-health-different-from-the-service-health-dashboard-or-the-azure-portal-service-notifications)
* [Dois-je activer Resource Health pour chaque ressource ?](#do-i-need-to-activate-resource-health-for-each-resource)
* [Faut-il activer Resource Health pour mon organisation ?](#do-we-need-to-enable-resource-health-for-my-organization)
* [Resource Health est-il disponible gratuitement ?](#is-resource-health-available-free-of-charge)
* [Quelles sont les recommandations que fournit Resource Health ?](#what-are-the-recommendations-that-resource-health-provides)


## <a name="what-is-azure-resource-health"></a>Présentation d’Azure Resource Health
...
Une fois qu’un problème lié à une ressource a été détecté, Resource Health peut vous aider à diagnostiquer la cause première. Il fournit de l’aide pour résoudre le problème et un support technique si vous avez besoin d’aide supplémentaire concernant des problèmes liés aux services Azure.
...
1. Événements non planifiés, par exemple le redémarrage d’un hôte inattendu
2. Événements planifiés, telles que les mises à jour planifiées du système d’exploitation hôte
3. Événements déclenchés par des actions de l’utilisateur, par exemple le redémarrage d’une machine virtuelle
...
1. Disponible : aucun problème connu sur la plateforme Azure susceptible d’impacter cette ressource
2. Non disponible : Resource Health a détecté des problèmes qui impactent la ressource
3. Inconnu : Resource Health ne peut pas déterminer l’intégrité d’une ressource, car il ne reçoit plus d’informations la concernant. 
...
1.    Action initiée par l’utilisateur
2.    Événement planifié 
3.    Événement non planifié
...
Resource Health est un service conçu pour vous aider à diagnostiquer et à résoudre les problèmes de service Azure qui affectent vos ressources. Bien que vous puissiez utiliser l’API Resource Health pour obtenir par programme l’état d’intégrité, nous vous recommandons d’utiliser des mesures pour analyser vos ressources. Une fois qu’un problème est détecté, Resource Health vous permet de déterminer l’origine du problème et vous guide au fil des actions pour les résoudre. Visitez [Azure Monitor](https://docs.microsoft.com/azure/monitoring-and-diagnostics/) pour en savoir plus sur la façon dont vous pouvez utiliser les mesures pour vérifier vos ressources.
...
1. Accédez à votre ressource. Dans le volet de navigation gauche, cliquez sur **Resource Health**.
2. Accédez au panneau Azure Monitor.  Dans le volet de navigation gauche, cliquez sur **Resource Health**.
3. Cliquez sur le point d’interrogation dans le coin supérieur droit du portail, puis sélectionnez **Aide + Support** pour ouvrir le panneau **Aide et support**. Lorsque le panneau s’ouvre, cliquez sur **Resource Health**
...
## <a name="how-is-resource-health-different-from-the-service-health-dashboard-or-the-azure-portal-service-notifications"></a>En quoi Resource Health diffère du tableau de bord d’état du service ou des notifications de service de portail Azure ?
