<properties 
	pageTitle="CENC avec Multi-DRM et contrôle d’accès : une conception de référence et l’application sur Microsoft Azure et Azure Media Services" 
...
1. Google Widevine
1. FairPlay Apple (non pris en charge par Azure Media Services)
...
1. Couche « DMZ » (en bleu) contenant tous les points de terminaison exposés au public ;
1. Couche Internet Public (en bleu clair) contenant les réseaux de diffusion en continu et des lecteurs avec le trafic transitant par l’Internet public.
...
1. un jeton d’autorisation est créé pour l’utilisateur ;
1. le contenu protégé par DRM (manifeste) est téléchargé sur le lecteur ;
1. le joueur soumet la demande d’acquisition de licence pour les serveurs de licence avec l’ID de clé et le jeton d’autorisation.
...
1. contenu ouvert ;
1. clé de contenu issue de la génération/la gestion de clé ;
1. URL d’acquisition de licence ;
1. liste d’informations fournie par Azure AD.
...
1. AssetDeliveryPolicy contient des spécifications au format streaming, la protection DRM et les URL d’acquisition de licence.
...
1. L’une des revendications contenues dans le jeton JWT est une revendication de type « groupes » qui contient l’ID d’objet de groupe de « EntitledUserGroup ». Cette revendication sera utilisée pour le réussir la « vérification des droits ».
1. Le lecteur charge le manifeste client d’un contenu protégé CENC et « voit » le texte suivant :
	1. ID de clé, 
	1. le contenu est protégé par CENC
	1. URL d’acquisition de licence.
...
1. L’URL de l’**émetteur** URL doit se terminer par **« / »**.  
	...
	**Public** doit correspondre à l’ID client d’application de lecteur et vous devez également ajouter des **« / »** à la fin de l’URL de l’émetteur.

		<add key="ida:audience" value="[Application Client ID GUID]" />
		<add key="ida:issuer" value="https://sts.windows.net/[AAD Tenant ID]/" /> 

	Dans [décodeur JWT](http://jwt.calebb.net/), vous devez trouver la chaîne de caractères **aud** et **iss** comme suit dans le jeton JWT :
...
4. Octroi des autorisations de revendications de membre de groupe. Assurez-vous que vous avez bien ce qui suit dans le fichier de manifeste de l’application AAD
	...
	« groupMembershipClaims » : « Tous » (la valeur par défaut est null.)
...
1. Il est préférable d’éviter les contenus mixtes : les deux doivent utiliser soit HTTP, soit utiliser HTTPS Lors de la lecture de contenu mixte, silverlightSS tech requiert la suppression d’un avertissement. FlashSS tech traite le contenu mixte sans aucun avertissement.
1. Si votre point de terminaison de diffusion a été créé avant août 2014, il ne prend pas en charge HTTPS. Dans ce cas, créez et utilisez un nouveau point de terminaison pour le protocole HTTPS.
...
1.	Un utilisateur est connecté à Azure AD dans l’application web (voir la section [Navigateur web vers application web](active-directory-authentication-scenarios.md#web-browser-to-web-application) ci-dessus).
