<properties 
	pageTitle="Création de ContentKeys avec REST | Microsoft Azure" 
...
Lorsque vous créez une nouvelle ressource (par exemple, avant de [charger des fichiers](media-services-rest-upload-files.md)), vous pouvez spécifier les options de chiffrement suivantes : **StorageEncrypted**, **CommonEncryptionProtected** ou **EnvelopeEncryptionProtected**.

Lors de la fourniture de ressources à vos clients, vous pouvez [configurer les ressources devant être chiffrées dynamiquement](media-services-rest-configure-asset-delivery-policy.md) avec un des deux chiffrements suivants : **DynamicEnvelopeEncryption** ou **DynamicCommonEncryption**.
...
6. Associez l’entité **ContentKey** avec votre entité **Asset** via l’opération $links.
...
##Récupération de ProtectionKey pour ProtectionKeyId
...
L’exemple suivant montre comment créer une **ContentKey** avec un **ContentKeyType** défini pour le chiffrement de stockage (« 1 ») et le **ProtectionKeyType** défini sur « 0 » pour indiquer que l’ID de clé de protection est l’empreinte numérique du certificat X.509.
...
##Association de la ContentKey avec une ressource
