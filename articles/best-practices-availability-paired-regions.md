...
![1Vert](./media/best-practices-availability-paired-regions/1Green.png) **Azure Compute** – Vous devez approvisionner des ressources de calcul supplémentaires à l’avance pour garantir la disponibilité des ressources dans d’autres régions au cours d’un incident. Nous vous recommandons de déployer ces ressources dans une région jumelée pour la récupération d’urgence.

![2Vert](./media/best-practices-availability-paired-regions/2Green.png) **Azure Storage** - Le stockage géo-redondant (GRS, Geo-Redundant Storage) est configuré par défaut quand vous créez un compte de stockage Azure. Avec GRS, vos données sont répliquées trois fois dans la région principale et trois fois dans la région jumelée. GRS autorise uniquement la réplication dans la région jumelée et ne peut pas être configuré différemment. Pour plus d'informations, consultez [Options de redondance du stockage Azure](../storage/storage-redundancy.md).


![3Vert](./media/best-practices-availability-paired-regions/3Green.png) **Bases de données SQL Azure** – Avec la géo-réplication SQL Azure, vous pouvez configurer la réplication asynchrone des transactions vers d’autres serveurs de base de données Microsoft Azure. La géo-réplication standard permet uniquement la réplication asynchrone avec la région jumelée. Avec la géo-réplication Premium, vous pouvez configurer la réplication pour n’importe quelle région du monde ; toutefois, nous vous recommandons de déployer ces ressources dans une région jumelée pour la récupération d’urgence. Pour plus d’informations, consultez la rubrique [Géo-réplication dans la base de données SQL Azure](https://msdn.microsoft.com/library/azure/dn783447.aspx).

![4Vert](./media/best-practices-availability-paired-regions/4Green.png) **Services de gestion indépendante** – Azure Resource Manager (ARM) fournit par nature une isolation logique des composants de gestion de service entre les régions : aucune configuration n’est requise. Cela signifie que des échecs logiques dans une région sont moins susceptibles d’avoir un impact sur une autre.

## Avantages d’une région jumelée

Conformément à la figure 2.

![5Orange](./media/best-practices-availability-paired-regions/5Orange.png) **Isolation physique** – Quand cela est possible, Azure préfère une séparation de 483 kilomètres au moins entre les centres de données d’une paire régionale, même si ce n’est pas pratique, voire impossible dans toutes les régions géographiques. La séparation physique du centre de données réduit la probabilité de catastrophes naturelles, de troubles civils, de coupures de courant ou de pannes de réseau physique affectant les deux régions en même temps. L’isolation est soumise aux contraintes géographiques (étendue, disponibilité de l’infrastructure réseau et de l’alimentation, réglementations, etc.).

![6Orange](./media/best-practices-availability-paired-regions/6Orange.png) **État cohérent de l’application ** – Si vous utilisez un service qui autorise uniquement la réplication sur une région jumelée (tel que le stockage géo-redondant), vous devez vous assurer que tous les autres services ciblent également la région jumelée pour la réplication. Cela garantit un état cohérent de l’application pendant le basculement.
