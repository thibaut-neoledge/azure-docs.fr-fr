...
Le [package NuGet SendGrid](https://www.nuget.org/packages/Sendgrid) permet de facilement obtenir l'API SendGrid et configurer votre application avec toutes les dépendances associées. NuGet est une
extension de Visual Studio incluse dans Microsoft Visual Studio 2012 et qui facilite l'installation et la mise à jour des bibliothèques et outils. 
...
Ajoutez les déclarations d'espace de noms de code suivantes en haut de tous les fichiers C\# dans lesquels vous souhaitez accéder par programmation au service de messagerie SendGrid.
**System.Net** et **System.Net.Mail** sont des espaces de noms .NET Framework inclus, car ils comprennent les types qui sont généralement utilisés avec les API SendGrid.
...
Pour plus d'informations sur l'ensemble des propriétés et méthodes prises en charge par le
type **SendGrid**, consultez [sendgrid-csharp][] sur GitHub.
...
Vous pouvez ajouter des pièces jointes à un message en appelant la méthode **AddAttachment** et en spécifiant le nom et le chemin d'accès du fichier à joindre.
