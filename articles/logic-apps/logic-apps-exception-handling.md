---
title: Gestion des exceptions Logic Apps | Microsoft Docs
...
---
# <a name="logic-apps-error-and-exception-handling"></a>Gestion des erreurs et des exceptions Logic Apps
...
## <a name="runafter-property-to-catch-failures"></a>Propriété RunAfter pour identifier les échecs
...
2. La condition de l’action **Filtrer le tableau** est tout élément `@result()` dont l’état est égal à `Failed`.  Le tableau de tous les résultats d’action de `My_Scope` seront filtrés selon un tableau des résultats d’action ayant échoué.
3. Exécution d’une action **For Each** sur les résultats du **tableau filtré**.  Cette opération effectue une action *pour chaque* résultat d’action ayant entraîné un échec filtré ci-dessus.
