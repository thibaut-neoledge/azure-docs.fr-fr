...
Le déclencheur de demande vous permet d’entrer un schéma JSON pour la charge utile que vous vous attendiez à recevoir. Cela permet au concepteur de générer des jetons pour vous aider à utiliser le contenu de la demande. Si vous n’avez pas de schéma prêt, sélectionnez `Use sample payload to generate schema` pour générer un schéma JSON à partir d’un exemple de charge utile.

![Schéma](./media/logic-apps-http-endpoint/manualtrigger.png)

### <a name="use-parse-json-action"></a>Utilisation d’une action d’analyse JSON
L’action `Parse JSON` vous permet d’analyser un contenu JSON en jetons conviviaux à utiliser dans l’application logique. À l’instar du déclencheur de demande, il vous permet d’entrer ou de générer un schéma JSON pour le contenu que vous souhaitez analyser. Il s’agit d’un outil très efficace pour faciliter nettement l’utilisation des données de Service Bus, DocumentDb, etc.
