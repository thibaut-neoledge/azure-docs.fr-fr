...
-   [Configuration de Pacemaker][Configuration de Pacemaker]
-   [Test][Test]
-   [STONITH][STONITH]
-   [Limites][Limites]
...
L'objectif de cet article est d'explorer et d'illustrer les différentes approches disponibles pour déployer des services Linux hautement disponibles sur Microsoft Azure en explorant la haute disponibilité de MySQL Server. Une vidéo illustrant cette approche est disponible sur [Channel 9][Channel 9].
...
Plusieurs architectures sont possibles pour MySQL, comme NBD Cluster, Percona et Galera, ainsi que plusieurs solutions intermédiaires, notamment au moins une disponible sous forme de machine virtuelle sur [VM Depot][VM Depot]. Tant que ces solutions peuvent se répliquer en monodiffusion, multidiffusion ou en diffusion et tant qu'elles ne reposent pas sur un stockage partagé ou plusieurs interfaces réseau, les scénarios doivent être faciles à déployer sur Microsoft Azure.
...
Nous sommes prêts pour une simulation de basculement automatique. Il existe deux façons de procéder : douce et dure. La méthode douce consiste à utiliser la fonction d'arrêt du cluster : `` crm_standby -U `uname -n` -v on ``. Si on utilise cette fonction sur le maître, l'esclave prend le relais. N'oubliez pas de redésactiver cette fonction (sinon, crm\_mon vous indiquera qu'un nœud est en veille).
...
Un exemple de code pour la ressource est disponible sur [GitHub][GitHub]. Il est nécessaire de changer la configuration du cluster en ajoutant le code suivant à `sudo crm configure` :
