<properties  pageTitle="Planification des tâches de serveur principal avec le Scheduler - Mobile Services" metaKeywords="" description="Use the Windows Azure Mobile Services Scheduler to schedule jobs for your mobile app." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Schedule recurring jobs in Mobile Services" authors="glenga"  solutions="mobile" writer="" manager="dwrede" editor=""  />
...
4. Développez le dossier Modèles, ouvrez le fichier de contexte du modèle de données (nommé <em>service_name</em>Context.cs) et ajoutez la propriété suivante qui renvoie un **DbSet** typé :
