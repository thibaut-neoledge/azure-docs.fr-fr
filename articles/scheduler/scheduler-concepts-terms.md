<properties
 pageTitle="Concepts, terminologie et hiérarchie d'entités de Scheduler"
 description=""
 services="scheduler"
...
|**Gestion de la collection de travaux**|Prise en charge de GET, PUT et DELETE pour la création et la modification des collections de travaux et des travaux qu'elles contiennent. Une collection de travaux est un conteneur pour les travaux et mappe ceux-ci aux quotas et paramètres partagés. Les exemples de quotas, présentés ultérieurement, sont le nombre maximal de travaux et le plus petit intervalle de périodicité <p>PUT & DELETE : `https://management.core.windows.net/{subscriptionId}/cloudservices/{cloudServiceName}/resources/scheduler/jobcollections/{jobCollectionName}`</p><p>GET : `https://management.core.windows.net/{subscriptionId}/cloudservices/{cloudServiceName}/resources/scheduler/~/jobcollections/{jobCollectionName}`</p>
...
Il existe deux types de travaux : les travaux HTTP (y compris les travaux HTTPS qui prennent en charge SSL) et les travaux de file d'attente de stockage. Les travaux HTTP sont idéaux si vous disposez d'un point de terminaison d'une charge de travail ou d'un service existant. Les travaux de file d'attente de stockage vous permettent de publier des messages aux files d'attente de stockage, donc ces travaux sont idéaux pour les charges de travail qui utilisent des files d'attente de stockage.

## L'entité « Travail » en détail

À un niveau de base, un travail planifié comporte plusieurs éléments :

1.  L'action à effectuer lorsque le travail se déclenche  

2.  (Facultatif) L'heure d'exécution du travail

3.  (Facultatif) Quand et à quelle fréquence répéter le travail

4.  (Facultatif) Une action à déclencher en cas d'échec de l'action principale
...
Vous trouverez ci-dessous un exemple détaillé de travail de Scheduler. Les détails sont fournis dans les sections suivantes.
...
1.  Heure de début (« startTime »)  

2.  Action (« action »), qui inclut l'action d'erreur (« errorAction »)
...
6.  Stratégie de nouvelle tentative (« retryPolicy »)

Examinons chacun en détail :

## startTime

« startTime » est l'heure de début et permet à l'appelant de spécifier un décalage de fuseau horaire sur le câble au [format ISO-8601](http://en.wikipedia.org/wiki/ISO_8601).

## action et errorAction

« action » est l'action appelée sur chaque occurrence et décrit un type d'appel de service. L'Action correspond à ce qui sera exécuté selon la planification spécifiée. Actuellement, Scheduler prend en charge les actions HTTP et file d'attente de stockage.
...
« errorAction » est le gestionnaire d'erreurs, l'action appelée lorsque l'action principale échoue. Vous pouvez utiliser cette variable pour appeler un point de terminaison de gestion d'erreurs ou envoyer une notification utilisateur. Cela peut être utilisé pour atteindre un point de terminaison secondaire au cas où le premier n'est pas disponible (par exemple, en cas de sinistre sur le site du point de terminaison) ou pour notifier un point de terminaison de traitement d'erreur. Comme l'action principale, l'action d'erreur peut être une logique simple ou composite basée sur d'autres actions. Pour savoir comment créer un jeton SAS, consultez [Créer et utiliser une signature d'accès partagé](https://msdn.microsoft.com/library/azure/jj721951.aspx).

## recurrence

La récurrence comporte plusieurs parties :

1.  La fréquence (frequency) : minute, heure, jour, semaine, mois, année  

2.  L'intervalle (interval) : intervalle à la fréquence donnée pour la récurrence

3.  La planification prescrite (prescribed schedule) : spécifiez les minutes, heures, jours de la semaine, mois et jours du mois de la récurrence

4.  Le nombre (count) : nombre d'occurrences

5.  L'heure de fin (end time) : aucun travail ne s'exécutera après l'heure de fin spécifiée
...
Pour définir une stratégie de nouvelle tentative, deux paramètres supplémentaires de valeurs peuvent être spécifiés : un intervalle de nouvelle tentative (**retryInterval**) et le nombre de nouvelles tentatives (**retryCount**).

L'intervalle de nouvelle tentative, spécifié avec l'objet **retryInterval**, est l'intervalle entre les nouvelles tentatives. Sa valeur par défaut est de 1 minute, sa valeur minimale est de 1 minute et sa valeur maximale est de 18 mois. Il est défini dans le format ISO-8601. De même, la valeur du nombre de nouvelles tentatives est spécifiée avec l'objet **retryCount**. Il s'agit du nombre de nouvelles tentatives. Sa valeur par défaut est de 5 et sa valeur maximale est de 20. Les deux objets **retryInterval** et **retryCount** sont facultatifs : ils reçoivent leur valeur par défaut si **retryType** est défini sur **fixed** et qu'aucune valeur n'est spécifiée explicitement.
