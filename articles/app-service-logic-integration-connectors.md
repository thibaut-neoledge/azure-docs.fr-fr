...
--- | ---
<ul><li>Application API X12</li><li>Connecteur AS2</li><li>Application API EDIFACT</li><li>Application API de Gestion des partenaires commerciaux</li> | Ces applications API fournissent des capacités B2B (entreprise à entreprise). [La rubrique Connecteurs B2B](app-service-logic-b2b-connectors.md) fournit plus de détails sur ces applications API.
Encodeur de fichier plat BizTalk | Application API d'**Action**. Autorise l'interopérabilité entre des données de fichiers plats (comme Excel, csv) et des données XML. Peut convertir une instance de fichier plat au format XML et vice versa.
Encodeur JSON BizTalk | Application API d'**Action**. Autorise l'interopérabilité entre des données JSON et XML. Peut convertir une instance JSON au format XML et vice versa.
Règles BizTalk | Application API d'**Action**. Vous pouvez implémenter et appliquer une logique métier ou des " règles " pour contrôler une opération commerciale. Les règles sont dynamiques et peuvent changer à tout moment. Les règles BizTalk permettent aux utilisateurs d'activer une logique métier sans écrire de code et peuvent être facilement mises à jour sans affecter les applications.
Transformation BizTalk | Application API d'**Action**. Convertit des données d'un format vers un autre. Vous pouvez utiliser différentes fonctions intégrées pour manipuler des chaînes, effectuer des opérations sur des expressions arithmétiques, mettre en forme la date et l'heure, et ainsi de suite. 
Validateur XML BizTalk | Application API d'**Action**. Valide des données XML par rapport à des schémas XML prédéfinis. Vous pouvez utiliser   des schémas basés sur des instances de fichiers plats, des instances JSON ou des connecteurs existants. 
Extracteur XPath BizTalk | Application API d'**Action**. Rechercher et extraire des données à partir de contenu XML en fonction d'une expression XPath spécifique.
...
> [AZURE.NOTE] Si le code xml d'entrée possède un nœud simple avec un attribut (comme " <authorid= "1">abc</author> "), la sortie JSON de la bibliothèque est { "author": { "@id" : "1", "#text": "abc"}}. Pour gérer l'attribut "Id", une nouvelle clé "#text" est ajoutée pour le contenu du texte du nœud. Pour gérer ce type de nœud, ajoutez une clé constante. Il s'agit du comportement par défaut dans la bibliothèque Newtonsoft.Json. Quand vous insérez cela dans SQL, utilisez "JSONOutput.Author.#text", et non "JsonOutput.Author".
...
[Créer une application logique IAE à l'aide de VETR](app-service-logic-create-EAI-logic-app-using-VETR.md)<br/>
[Transformer des documents XML](app-service-logic-transform-xml-documents.md)<br/>
[Utiliser des règles BizTalk](app-service-logic-use-biztalk-rules.md)<br/>
