<properties urlDisplayName="Get Started" pageTitle="Prise en main de l'&eacute;mulateur HDInsight | Azure" metaKeywords="hdinsight, Azure hdinsight, hdinsight azure, get started hdinsight, emulator, hdinsight emulator" description="D&eacute;couvrez comment utiliser l'&eacute;mulateur HDInsight pour Azure." umbracoNaviHide="0" disqusComments="1" editor="cgronlun" manager="paulettm" services="hdinsight" title="Prise en main de l'&eacute;mulateur HDInsight" author="nitinme" />
...
-   [Installation de l'émulateur HDInsight][Installation de l'émulateur HDInsight]
-   [Exécution de l'exemple de comptage des mots][Exécution de l'exemple de comptage des mots]
-   [Exécution des exemples de prise en main][Exécution des exemples de prise en main]
-   [Connexion au stockage d'objets blob Azure][Connexion au stockage d'objets blob Azure]
-   [Exécution de PowerShell pour HDInsight][Exécution de PowerShell pour HDInsight]
...
1.  Ouvrez Internet Explorer, puis accédez à la [page d'installation de l'émulateur Microsoft HDInsight pour Azure][page d'installation de l'émulateur Microsoft HDInsight pour Azure].
...
8.  Cliquez sur **Quitter** pour fermer la fenêtre Web Platform Installer.
...
Les services liés à l'émulateur HDInsight ne sont pas démarrés par défaut. Pour lancer ces services, à partir de la ligne de commande Hadoop, exécutez **start\_local\_hdp\_services.cmd** sous <system drive\>\\hdp. Pour démarrer automatiquement les services après le redémarrage de l'ordinateur, exécutez **set-onebox-autostart.cmd**.

Pour plus d'informations sur les problèmes connus lors de l'installation et de l'exécution de HDInsight Server, consultez la page [Notes de publication de l'émulateur HDInsight][Notes de publication de l'émulateur HDInsight]. Le journal d'installation se trouve à l'emplacement **C:\\HadoopFeaturePackSetup\\HadoopFeaturePackSetupTools\\gettingStarted.winpkg.install.log**.
...
Le programme MapReduce de comptage des mots a été empaqueté dans le fichier *hadoop-mapreduce-examples-2.4.0.SNAPSHOT.jar*. Celui-ci se situe dans le dossier *C:\\hdp\\hadoop-2.4.0.SNAPSHOT\\share\\hadoop\\mapreduce*.

La tâche MapReduce de comptage des mots prend deux arguments :

-   Un dossier d'entrée. Vous utiliserez *hdfs://localhost/user/HDIUser*.
-   Un dossier de sortie. Vous utiliserez *hdfs://localhost/user/HDIUser/WordCount\_Output*. Le dossier de sortie ne peut pas être un dossier existant, sinon la tâche MapReduce échoue. Si vous souhaitez exécuter la tâche MapReduce une deuxième fois, vous devez soit spécifier un autre dossier de sortie soit supprimer le dossier de sortie existant.
...
Pour plus d'informations sur les commandes Hadoop, consultez la page [Manuel des commandes Hadoop][Manuel des commandes Hadoop].
...
3.  Exécutez la commande suivante pour générer et importer les données vers HDFS :
    ...
    Si vous souhaitez plutôt charger les données vers WASB, consultez la rubrique [Connexion au stockage d'objets blob Azure][Connexion au stockage d'objets blob Azure].
...
4.  Exécutez la commande suivante :
    ...
    <td>
    Le fichier jar se trouve dans le dossier C:\\hdp\\GettingStarted\\Java.
    ...
    <td>
    Le type peut être remplacé par l'un des suivants :
    ...
    <td>
    Le fichier d'entrée peut être remplacé par :
    ...
    <td>
    Il s'agit du nom du dossier de sortie.
...
6.  Exécutez la commande suivante pour exécuter le fichier de script HiveQL **w3ctotalhitsbypage.hql**.
    ...
    <td>
    Vous pouvez remplacer le fichier de script Hive par l'un des suivants :
...
3.  Exécutez la commande suivante pour soumettre la tâche Pig :
    ...
    <td>
    Vous pouvez remplacer le fichier de script Pig Latin par l'un des suivants :
    ...
    <td>
    Vous pouvez remplacer le paramètre par un fichier plus volumineux :
...
> [WACOM.NOTE] Pour plus d'informations sur la façon dont HDInsight utilise le stockage d'objets blob Azure, consultez la rubrique [Utilisation du stockage d'objets blob Azure avec HDInsight][Utilisation du stockage d'objets blob Azure pour HDInsight].

Avant de commencer à suivre les instructions ci-dessous, vous devez créer un compte de stockage. Pour obtenir des instructions, consultez la rubrique [Création d'un compte de stockage][Création d'un compte de stockage].
...
1.  Ouvrez **C:\\hdp\\hadoop-2.4.0.SNAPSHOT\\etc\\hadoop\\core-site.xml** dans le Bloc-notes.
2.  Ajoutez la balise <property\> suivante à côté des autres <property\> balises :
...
Pour plus d'informations sur l'envoi de tâches Hadoop, consultez la page [Envoi de tâches Hadoop par programme][Envoi de tâches Hadoop par programme]. Pour plus d'informations sur les cmdlets PowerShell pour HDInsight, consultez la page [Référence des cmdlets HDInsight][Référence des cmdlets HDInsight].
...
-   [Notes de publication de l'émulateur HDInsight][Notes de publication de l'émulateur HDInsight]
-   [Forum MSDN sur HDInsight][Forum MSDN sur HDInsight]

  [Version de Hadoop dans Azure HDInsight]: ../hdinsight-component-versioning/
  ...
  [page d'installation de l'émulateur Microsoft HDInsight pour Azure]: http://www.microsoft.com/web/gallery/install.aspx?appid=HDINSIGHT
  ...
  [Manuel des commandes Hadoop]: http://hadoop.apache.org/docs/r1.1.1/commands_manual.html
  [Utilisation du stockage d'objets blob Azure pour HDInsight]: ../hdinsight-use-blob-storage/
