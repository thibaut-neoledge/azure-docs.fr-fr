...
Obtenez une présentation de Hadoop, de son écosystème et des données volumineuses dans Azure HDInsight : description de Hadoop dans HDInsight, composants de Hadoop, terminologie commune et scénarios d'analyse de données volumineuses. Et apprenez en plus sur la documentation, les didacticiels et les ressources pour l'utilisation de Hadoop dans HDInsight.
...
* Azure HDInsight déploie et approvisionne les clusters Hadoop dans le cloud, en utilisant **Linux** ou **Windows** en tant que système d'exploitation sous-jacent.
	...
	* **HDInsight sur Windows** - Cluster Hadoop sur Windows Server. Utilisez cette option si vous maîtrisez Windows, migrez à partir d’une solution Hadoop Windows ou souhaitez intégrer des fonctionnalités .NET ou d'autres fonctionnalités de Windows.
	...
	Le tableau suivant présente une comparaison entre les deux :

	Catégorie | HDInsight sur Linux | HDInsight sur Windows 
	---------| -------------------| --------------------
	...
	**Déploiement** | Portail de gestion Azure, ligne de commande interplateforme, Azure PowerShell | Portail de gestion Azure, ligne de commande interplateforme, Azure PowerShell, Kit de développement logiciel (SDK) .NET HDInsight
	**IU du cluster** | Ambari | Tableau de bord du cluster
	**Accès à distance** | Secure Shell (SSH) | Remote Desktop Protocol (RDP)
	

* HDInsight utilise la distribution **HDP (Hortonworks Data Platform)** de Hadoop.

* Apache Hadoop est une infrastructure logicielle pour la gestion et l'analyse des données volumineuses. HDInsight fournit plusieurs configurations pour des charges de travail spécifiques, ou vous pouvez <a href="http://azure.microsoft.com/documentation/articles/hdinsight-hadoop-customize-cluster/" target="_blank">personnaliser des clusters à l'aide d’actions de script</a>.
...
* **[Hive](#hive)** : langage de requête similaire à SQL (Structured Query Language).
...
* Composants Hadoop à la pointe de la technologie. Pour plus d'informations, consultez [
* Nouveautés dans les versions de cluster Hadoop fournies par HDInsight][component-versioning].
...
* Stockage efficace et économique des données avec le stockage d'objets blob Azure, une option compatible Hadoop. Consultez la rubrique [Utilisation du stockage d'objets blob Azure avec Hadoop dans HDInsight](../hdinsight-use-blob-storage.md) pour plus d'informations.
...
* [Prise en main HDInsight sur Linux](../hdinsight-hadoop-linux-get-started.md) : didacticiel de démarrage rapide pour l'approvisionnement de clusters HDInsight Hadoop sous Linux et l'exécution des exemples de requêtes Hive.

* [Approvisionnement de HDInsight sur Linux à l'aide d’options personnalisées](hdinsight-hadoop-provision-linux-clusters.md) : apprenez à approvisionner un cluster HDInsight Hadoop sous Linux à l'aide d’options personnalisées via le portail de gestion Azure, la ligne de commande interplateforme Azure ou Azure PowerShell.

* [Utilisation de HDInsight sur Linux](hdinsight-hadoop-linux-information.md) : obtenez quelques conseils rapides sur l'utilisation de clusters Hadoop Linux approvisionnés sur Azure.
...
* [Prise en main d’Azure HDInsight](../hdinsight-get-started.md) : didacticiel de prise en main rapide pour l'utilisation de Hadoop dans HDInsight.
...
* [Portail de gestion de la base de données SQL](https://msdn.microsoft.com/library/azure/dn771027.aspx) : outil de gestion de base de données léger et simple d'utilisation pour gérer la base de données SQL dans le cloud.

* [Adventure Works pour Base de données SQL](http://msftdbprodsamples.codeplex.com/releases/view/37304) : page de téléchargement de l'exemple de base de données SQL.
...
* <a target="_blank" https://msdn.microsoft.com/library/hh231701.aspx">En savoir plus sur SQL Server Analysis Services</a>.
...
* [Analyse des données de capteur en temps réel avec Storm et HBase dans HDInsight (Hadoop)](hdinsight-storm-sensor-data-analysis.md) : découvrez comment créer une solution qui utilise un cluster Storm dans HDInsight afin de traiter des données de capteur provenant de concentrateurs d'événements Azure, puis qui affiche les données de capteur traitées en tant qu'informations en temps quasi-réel sur un tableau de bord basé sur le web.
