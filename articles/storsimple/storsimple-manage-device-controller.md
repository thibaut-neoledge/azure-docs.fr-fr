...
> - La procédure suivante s'applique uniquement à l’appareil physique StorSimple. Pour plus d'informations sur le démarrage, l’arrêt et le redémarrage de l’appareil virtuel, consultez la rubrique [Utilisation de l’appareil virtuel](storsimple-virtual-device-u1.md#work-with-the-storsimple-virtual-device).

Vous pouvez redémarrer ou arrêter un contrôleur d’appareil unique via :

- le portail de gestion du service StorSimple Manager.
- Windows PowerShell pour StorSimple 

Pour gérer vos contrôleurs d’appareil à partir du portail de gestion, procédez comme suit.

#### Pour redémarrer ou arrêter un contrôleur dans le portail de gestion
...
1. Dans le message de bannière, prenez note du contrôleur auquel vous êtes connecté (contrôleur 0 ou contrôleur 1) et vérifiez s'il est actif ou bien passif (en veille).
...
1. Utilisez la procédure de [redémarrage ou d’arrêt d’un contrôleur](#restart-or-shut-down-a-single-controller) pour identifier et arrêter le contrôleur passif sur votre appareil. Vous pouvez effectuer cette opération dans le portail de gestion ou dans Windows PowerShell pour StorSimple.
...
Cette procédure contient une procédure pas à pas requise pour rétablir les paramètres d’usine de votre appareil Microsoft Azure StorSimple à l'aide de Windows PowerShell pour StorSimple.
...
- **Redémarrer le contrôleur actif** : un message vous indiquera que le redémarrage d'un contrôleur actif entraînera un basculement de l’appareil vers le contrôleur passif. Le contrôleur redémarrera.

- **Arrêter un contrôleur actif** : un message vous indiquera que l’arrêt d’un contrôleur actif entraînera une interruption du service. Vous devrez également appuyer sur le bouton d'alimentation de l’appareil pour activer le contrôleur.
...
- **Arrêter un contrôleur actif** : un message vous indiquera que la poursuite de l'opération entraînera une interruption du service, et que vous devrez appuyer sur le bouton d'alimentation de votre/vos contrôleur(s) pour activer l’appareil. Vous êtes invité à confirmer l’opération.
