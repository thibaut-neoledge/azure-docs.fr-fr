...
2. Accédez à la page **Appareils**.

3. Dans la boîte de dialogue **Créer un appareil virtuel**, spécifiez les éléments suivants.

	![Création d’un appareil virtuel StorSimple](./media/storsimple-virtual-device/StorSimple_CreateVirtualDevice1.png)

	1. **Nom** : nom unique de votre appareil virtuel.

	2. **Réseau virtuel** – Nom du réseau virtuel que vous souhaitez utiliser avec cet appareil virtuel.

	3. **Sous-réseau** – Sous-réseau du réseau virtuel pour une utilisation avec l’appareil virtuel.
...
4. Cliquez sur la coche pour indiquer que vous savez que les données stockées sur l’appareil virtuel sont hébergées dans un centre de données Microsoft. Une machine virtuelle va maintenant être créée. La création d’un appareil virtuel peut prendre 45 minutes à 1 heure. ![Étape de création de l’appareil virtuel StorSimple](./media/storsimple-virtual-device/StorSimple_VirtualDeviceCreating1M.png)
...
Avant de commencer cette procédure, assurez-vous que vous disposez d’une copie de la clé de chiffrement des données de service. Cette clé de chiffrement a été créée lorsque vous avez configuré votre premier appareil StorSimple et que vous avez été invité à l’enregistrer dans un emplacement sécurisé. Si vous n’avez pas de copie de la clé de chiffrement des données de service, vous devez contacter le [Support Microsoft](storsimple-contact-microsoft-support.md) pour obtenir de l’aide.
...
Procédez comme suit pour modifier le Gestionnaire d’instantanés StorSimple lors de l’utilisation avec votre appareil virtuel StorSimple.
...
Pour visionner une vidéo qui décrit comment créer un appareil StorSimple virtuel dans le cloud, cliquez [ici](https://azure.microsoft.com/documentation/videos/create-a-storsimple-virtual-device/).
...
Pour visionner une vidéo décrivant comment restaurer un périphérique physique basculé vers un appareil virtuel dans le cloud, cliquez [ici](https://azure.microsoft.com/documentation/videos/storsimple-and-disaster-recovery/).
