...
5. **Créez votre profil et configurez les paramètres**. Vous pouvez utiliser les API REST, Windows PowerShell ou le Portail Azure pour créer votre profil Traffic Manager et configurer les paramètres. Pour en savoir plus, voir [Configuration des paramètres Traffic Manager](#how-to-configure-traffic-manager-settings). Les étapes suivantes supposent que vous utiliserez l’option **Création rapide** du Portail Azure. 
   - **Créer votre profil Traffic Manager** : pour créer un profil à l’aide de l’option Création rapide du Portail Azure, consultez [Gérer les profils Traffic Manager](traffic-manager-manage-profiles.md).
...
Vous pouvez configurer les paramètres de Traffic Manager à l’aide du Portail Azure, avec les API REST et avec les applets de commande Windows PowerShell.

Même si tous les éléments d’API REST ne sont pas visibles dans le Portail Azure, de nombreux paramètres sont disponibles avec chacune des méthodes. Pour en savoir plus sur l’utilisation des API REST, consultez la page [Opérations pour Traffic Manager (informations de référence sur l’API REST)](http://go.microsoft.com/fwlink/p/?LinkId=313584).
...
Vous pouvez configurer les paramètres suivants dans le Portail Azure :

- **Préfixe DNS** : un préfixe unique que vous créez. Les profils sont affichés par préfixe dans le Portail Azure.
