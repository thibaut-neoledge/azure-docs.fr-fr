...
- Accès en lecture public complet : les données de conteneur et d'objet blob peuvent être lues via une demande anonyme. Les clients peuvent énumérer les objets blob à l'intérieur du conteneur via une demande anonyme, mais ne peuvent pas énumérer les conteneurs dans le compte de stockage.
...
| Opération REST                                         | Autorisation avec accès total en lecture public | Autorisation avec accès en lecture public pour les objets blob uniquement |
|--------------------------------------------------------|-----------------------------------------|---------------------------------------------------|
...
| Get Container Properties                              | Tous                                     | Propriétaire uniquement                                        |
| Get Container Metadata                                 | Tous                                     | Propriétaire uniquement                                        |
...
| Get Block List (blocs validés uniquement)                 | Tous                                     | Tous                                               |
...
| Copy Blob                                              | Propriétaire uniquement                              | Propriétaire uniquement                                        |
...
L'exemple de code suivant crée une stratégie d'accès sur un conteneur puis génère une signature d'accès partagé pour ce conteneur. Cette signature d'accès partagé peut ensuite être communiquée aux clients :
    ...
    // La chaîne de connexion pour le compte de stockage.  Modifiez selon votre compte.
    string storageConnectionString =
       "DefaultEndpointsProtocol=https;" +
       "AccountName=myaccount;" +
       "AccountKey=<account-key>";
    
    Vous pouvez également récupérer les informations du compte de stockage à partir d'un fichier app.config. 
    // C'est une possibilité de stockage et de récupération d'une chaîne de connexion si vous 
    // écrivez une application exécutée localement et non sur Microsoft Azure.
    
    // string storageConnectionString = ConfigurationManager.AppSettings["StorageAccountConnectionString"];
    
    // Créez le compte de stockage avec la chaîne de connexion.
    CloudStorageAccount storageAccount = CloudStorageAccount.Parse(storageConnectionString);
       
    // Créez le client d'objet blob.
    CloudBlobClient blobClient = storageAccount.CreateCloudBlobClient();
    
    // Obtenez une référence au conteneur pour lequel sera créée la signature d'accès partagé.
    CloudBlobContainer container = blobClient.GetContainerReference("mycontainer");
    container.CreateIfNotExists();
    
    // Créez les autorisations du conteneur d'objets blob, notamment une stratégie d'accès partagé 
    // et un paramètre d'accès public. 
    BlobContainerPermissions blobPermissions = new BlobContainerPermissions();
    
    // La stratégie d'accès partagé fournit 
    // un accès en lecture/écriture au conteneur pendant 10 heures.
    blobPermissions.SharedAccessPolicies.Add("mypolicy", new SharedAccessBlobPolicy()
    {
       // Pour garantir que la SAP est valable immédiatement, ne définissez pas d'heure de départ.
       // De cette façon, vous pouvez éviter les défaillances provoquées par les petites différences entre les horloges.
       SharedAccessExpiryTime = DateTime.UtcNow.AddHours(10),
       Permissions = SharedAccessBlobPermissions.Write |
      SharedAccessBlobPermissions.Read
    });
    
    // Le paramètre d'accès public spécifie explicitement que 
    // le conteneur est privé, afin qu'il ne soit pas accessible anonymement.
    blobPermissions.PublicAccess = BlobContainerPublicAccessType.Off;
    
    // Définissez la stratégie d'autorisation du conteneur.
    container.SetPermissions(blobPermissions);
    
    // Obtenez la signature d'accès partagé à partager avec les utilisateurs.
    string sasToken =
...
Un client qui reçoit une signature d'accès partagé peut l'utiliser dans son code pour construire un objet de type [StorageCredentials](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.auth.storagecredentials.aspx). Ces informations d'identification peuvent ensuite servir à construire un objet [CloudStorageAccount](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.cloudstorageaccount.aspx) ou [CloudBlobClient](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.blob.cloudblobclient.aspx) pour utiliser la ressource, comme indiqué dans cet exemple :
    ...
    // Créez les informations d'identifications à l'aide du jeton SAP. Le jeton SAP a été créé dans l'exemple précédent.
    StorageCredentials credentials = new StorageCredentials(sasToken);
    
    // Créez un nouvel objet blob.
    CloudBlockBlob blob = new CloudBlockBlob(blobUri, credentials);
    
    // Téléchargez l'objet blob. 
    // Si l'objet blob n'existe pas encore, il est créé. 
