...
Cet article présente les deux options.  La première option a l’avantage d’utiliser le script utilisé par les développeurs dans Visual Studio, ce qui garantit une cohérence tout au long du cycle de vie.  La deuxième option est une alternative pratique au script intégré.  Les deux procédures partent du principe que vous disposez déjà d’un projet de déploiement Visual Studio contrôlé dans Visual Studio Team Services.
...
La procédure suivante vous guide à travers les étapes nécessaires à la configuration d’un déploiement continu dans Visual Studio Team Services à l’aide d’une tâche unique qui exécute le script PowerShell dans votre projet. 
...
La procédure suivante vous guide à travers les étapes nécessaires à la configuration d’un déploiement continu dans Visual Studio Team Services à l’aide des tâches intégrées.
