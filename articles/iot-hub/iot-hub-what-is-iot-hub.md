...
L’une des plus grandes difficultés des projets IoT consiste à connecter de manière fiable et sécurisée des appareils à l’application principale. Les appareils IoT ont souvent des caractéristiques différentes de celles des autres clients (navigateurs, autres serveurs et applications mobiles) :
...
-   **Un ensemble complet de bibliothèques de périphériques**. Les kits de développement logiciel (SDK) d’appareil Azure IoT sont disponibles et pris en charge pour différents langages et plateformes : C pour les distributions Linux, Windows et RTO. Langages gérés tels que C#, Java et JavaScript.

-   **Protocoles et possibilités d’extension IoT**. Si votre solution ne peut pas utiliser les bibliothèques du périphérique, Azure IoT Hub propose un protocole public qui permet aux appareils d’utiliser les protocoles HTTP 1.1 et AMQP 1.0 natifs. Vous pouvez également étendre Azure IoT Hub afin de permettre la prise en charge de MQTT v3.1.1 avec le composant source de passerelle de protocole Azure IoT ouvert. Vous pouvez exécuter la passerelle de protocole Azure IoT dans le cloud ou sur site, et l’étendre pour prendre en charge des protocoles personnalisés.
...
Outre ces avantages, qui sont généralisés sur de nombreux schémas de communication, Azure IoT Hub vous permet de mettre en œuvre les schémas de communication suivants :

-   **Ingestion appareil vers cloud basée sur un événement.** Les appareils fiables peuvent envoyer des millions d’événements par seconde afin qu’ils soient traités par le moteur de processeur d’événements de chemin réactif ou stockés en vue d’un parcours d’analyse à froid. Azure IoT Hub conserve les données d’événement jusqu’à 7 jours afin de garantir un traitement fiable et absorber les pics de charge.
...
Outre les modèles de communication ci-dessus, vous pouvez implémenter d’autres modèles populaires tels que le chargement ou le téléchargement de fichiers en tirant parti de fonctionnalités spécifiques dans Azure IoT Hub, par exemple, la gestion d’identité de périphérique cohérente, la surveillance et la mise à l’échelle de connectivité.
...
- En général, les appareils se connectent ou établissent des itinéraires uniquement vers des services bien connus auxquels ils sont couplés, par exemple une instance de service Azure IoT Hub.
...
Pour en savoir plus sur Azure IoT Hub, voir :
