...
IoT Hub propose des primitives de messagerie pour communiquer : - [Du cloud vers l’appareil](#c2d) : à partir d’un serveur principal d’applications (*service* ou *cloud*) ; - [De l’appareil vers le cloud](#d2c) : entre un appareil et un serveur principal d’applications.

Les principales propriétés de la fonctionnalité de messagerie IoT Hub sont la fiabilité et la durabilité des messages. Elle active la résilience de la connectivité intermittente côté appareils et des pics de chargement dans le traitement d’événements côté cloud. IoT Hub implémente *au moins une fois* des garanties de remise pour l’envoi de messages de l’appareil vers le cloud et du cloud vers l’appareil.
...
| ConnectionAuthMethod | Propriété définie par IoT Hub sur les messages Appareil vers cloud. Informations sur la méthode d’authentification utilisée pour authentifier l’appareil qui a envoyé le message. Pour plus d’informations, voir [Propriétés de la fonction anti-usurpation d’identité](#antispoofing)|
...
IoT Hub implémente le protocole MQTT v3.1.1 avec le comportement spécifique et les limitations suivantes :
  ...
  * **QoS 2 n’est pas pris en charge** : lorsqu’un client d’appareil publie un message avec **QoS 2**, IoT Hub interrompt la connexion réseau. Lorsqu’un appareil client s’abonne à une rubrique avec **QoS 2**, IoT Hub accorde au maximum le QoS niveau 1 dans le paquet **SUBACK**.
...
Comme expliqué dans la section [Points de terminaison](#endpoints), les messages envoyés de l’appareil vers le cloud sont envoyés via un point de terminaison de l’appareil (**/devices/{deviceId}/messages/events**) et reçus via un point de terminaison du service (**/messages/events**) compatible avec [Event Hubs][lnk-event-hubs]. Par conséquent, vous pouvez utiliser l’intégration et les Kits de développement logiciel (SDK) standard Event Hubs pour recevoir des messages Appareil vers cloud.
...
* Les messages envoyés de l’appareil vers le cloud sont partitionnés dans un ensemble fixe de partitions, qui est défini au moment de la création (voir [Options de configuration Appareil vers cloud](#d2cconfiguration)).
...
Consultez la section [Traitement des messages envoyés de l’appareil vers le cloud][lnk-guidance-d2c-processing] pour en savoir plus sur la meilleure façon de traiter ce type de messages.
...
Comme nous l’avons expliqué dans la section [Points de terminaison](#endpoints), vous pouvez envoyer des messages du cloud vers l’appareil via un point de terminaison d’appareils (**/messages/devicebound**) ; un appareil peut recevoir ces messages via un point de terminaison spécifique à l’appareil (**/devices/{deviceId}/messages/devicebound**).

Chaque message envoyé du cloud vers l’appareil cible un seul appareil, en définissant la propriété **to** sur **/devices/{deviceId}/messages/devicebound**.
...
- Si vous définissez la propriété **Ack** sur **positive**, IoT Hub génère un message de commentaires si et seulement si le message envoyé du cloud vers l’appareil a atteint l’état **Achevé**.
...
**Important** : les opérations de registre d’identité sont prévues pour une utilisation au moment de l’exécution lors de scénarios de gestion d’appareils et d’approvisionnement. La lecture ou la mise à jour de plusieurs identités d’appareils sont prises en charge lors de [tâches d’importation/exportation](#importexport).
