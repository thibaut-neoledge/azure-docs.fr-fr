...
Avec la mise à l'échelle automatique activée, **Performance** s'affiche sur le volet du site Web et l'historique des mises à l'échelle s'affiche dans le graphique :

![Volet Scale avec pourcentage UC](./media/insights-how-to-scale/Insights_ScalePartBladeOn.png)
...
Pour chaque règle, vous pouvez choisir : - Condition - Greater than ou Less than - Threshold - le nombre que cette mesure doit transmettre pour déclencher l'action - Over Past - le nombre de minutes sur lequel la moyenne de cette mesure est calculée - Scale up or down by - la taille de l'action de mise à l'échelle - Cool down - le temps d'attente de cette règle entre la dernière action de mise à l'échelle et la prochaine
