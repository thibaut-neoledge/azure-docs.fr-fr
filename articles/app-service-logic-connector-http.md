...
6.	Vous pouvez maintenant définir la méthode HTTP et l'URL relative sur laquelle vous voulez que l'écouteur déclenche le flux.
...
7.	Pour obtenir l'URI complet, double-cliquez sur l'écouteur HTTP pour afficher ses paramètres de configuration et copiez l'URL de l'hôte de votre application API.
	...
	![][6]
8.	Vous pouvez maintenant utiliser les données reçues dans la requête HTTP dans les autres actions du flux comme suit :
	![][7]
	![][8]
9.	Enfin, pour envoyer une réponse, ajoutez un autre écouteur HTTP et sélectionnez l'action Envoyer une réponse HTTP. Affectez à l'ID de requête la valeur RequestID obtenue de l'écouteur HTTP, puis remplissez le corps de la réponse et l'état HTTP à renvoyer.
	![][9]

## Utilisation de l'action HTTP
L'action HTTP est prise en charge de façon native par les applications logiques et ne requiert pas la création préalable d'une application API pour pouvoir être utilisée. Vous pouvez insérer une action HTTP à n'importe quel endroit dans votre application logique et choisir l'URI, les en-têtes et le corps de l'appel.
L'action HTTP prend en charge plusieurs options de sécurité côté client. Pour les utiliser, consultez l'article [ici](http://aka.ms/logicapphttpauth).
