...
Ce guide présente l'exécution de tâches de programmation courantes avec le
 service de messagerie SendGrid dans Azure. Les exemples sont écrits en utilisant
l'API Node.js. Les scénarios traités incluent la **construction** et l'
**envoi de courriers électroniques**, l'
**ajout de pièces jointes**, l'
**utilisation de filtres**, et la
**mise à jour de propriétés**. Pour plus d'informations sur SendGrid et l'envoi
de courrier électronique, consultez la section [Étapes suivantes][Étapes suivantes].
...
-   [Référencement du module Node.js SendGrid][Référencement du module Node.js SendGrid]
-   [Création d'un courrier électronique][Création d'un courrier électronique]
...
-   [Mise à jour des propriétés de courrier électronique][Mise à jour des propriétés de courrier électronique]
...
Le module SendGrid pour Node.js peut être installé via le gestionnaire
de package node (npm) à l'aide de la commande suivante :

    npm install sendgrid

Après l'installation, vous pouvez exiger la présence du module dans votre application
à l'aide du code suivant :

    var SendGrid = require('sendgrid')

Le module SendGrid exporte les fonctions **SendGrid** et **Email**.
**SendGrid** est chargé d'envoyer le courrier électronique via SMTP ou l'API
Web, alors que **Email** permet d'encapsuler un message électronique.
...
à créer un message électronique à l'aide de la fonction Email, puis à l'envoyer
à l'aide de la fonction SendGrid. L'exemple suivant illustre la création d'un
...
Vous pouvez également spécifier un message HTML pour les clients qui le prennent en charge
en définissant la propriété html. Par exemple :

    html: This is a sample <b>HTML<b> email message.

Définir les propriétés text et html offre une solution de repli intéressante pour
le contenu textuel dans le cas des clients qui ne peuvent pas prendre en charge les messages HTML.

Pour plus d'informations sur l'ensemble des propriétés prises en charge par la fonction Email,
consultez la page [sendgrid-nodejs][sendgrid-nodejs].
...
l'envoyer via SMTP ou l'API Web fournie par SendGrid. Pour plus d'informations
sur les avantages et les différences de chaque API, consultez la page [SMTP et API Web][SMTP et API Web]
dans la documentation SendGrid.

Avant de pouvoir utiliser l'API SMTP ou l'API Web, vous devez initialiser
la fonction SendGrid en spécifiant l'utilisateur (user) et la clé (key) de votre compte SendGrid
comme suit :

    var sender = new SendGrid('user','key');

Le message peut maintenant être envoyé via SMTP ou l'API Web. Les appels
sont pratiquement identiques, avec la transmission du message électronique et d'une fonction
de rappel facultative ; le rappel sert à déterminer la réussite ou
l'échec de l'opération. Les exemples suivants montrent comment envoyer un
message à l'aide de SMTP et de l'API Web.
...
<p>Si les exemples pr&eacute;c&eacute;dents font intervenir un objet de messagerie &eacute;lectronique et
une fonction de rappel, vous pouvez &eacute;galement appeler directement les fonctions send et smtp
en sp&eacute;cifiant directement les propri&eacute;t&eacute;s de message &eacute;lectronique. Par exemple&nbsp;:</p>
...
Il est possible d'ajouter des pièces jointes à un message en spécifiant les noms de fichiers et
les chemins d'accès dans la propriété **files**. L'exemple suivant illustre
l'envoi d'une pièce jointe :
...
<div class="dev-callout">
<strong>Remarque</strong>
<p>Lorsque la propri&eacute;t&eacute; <strong>files</strong> est utilis&eacute;e, le fichier doit &ecirc;tre accessible
via <a href="http://nodejs.org/docs/v0.6.7/api/fs.html#fs.readFile">fs.readFile</a>. Si le fichier que vous souhaitez joindre est h&eacute;berg&eacute; dans Azure Storage, par exemple dans un conteneur d'objets blob, vous devez d'abord copier le fichier dans un stockage local ou un lecteur Azure avant de pouvoir l'envoyer sous forme de pi&egrave;ce jointe &agrave; l'aide de la propri&eacute;t&eacute; <strong>files</strong>.</p>
</div>
...
de filtres. Il s'agit de paramètres que vous pouvez ajouter à un message
électronique pour activer des fonctionnalités spécifiques telles que le
suivi des clics, Google Analytics, le suivi d'abonnement, etc. Pour obtenir une liste exhaustive des filtres,
consultez la page [Paramètres de filtre][Paramètres de filtre].
...
Les exemples suivants montrent les filtres de pied de page, de suivi des clics
et de Twitter :
...
Vous pouvez remplacer certaines propriétés de message électronique en utilisant **set*Property*** ou
en ajouter en utilisant **add*Property***. Par exemple, vous pouvez ajouter
des destinataires supplémentaires en utilisant
...
Pour plus d'informations, consultez la page [sendgrid-nodejs][sendgrid-nodejs].
...
-   Référentiel du module Node.js SendGrid : [sendgrid-nodejs][sendgrid-nodejs]
