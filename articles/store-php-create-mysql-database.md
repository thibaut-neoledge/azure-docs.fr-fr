<properties
	pageTitle="Création et connexion à une base de données MySQL dans Azure"
...
## Création d’une base de données MySQL dans le portail Azure
...
2. Dans le [panneau](azure-portal-overview.md) Nouvelle base de données MySQL, configurez votre nouvelle base de données MySQL comme suit (*panneau* : une page du portail s’ouvre à l’horizontale) :
    ...
    La capture d’écran suivante illustre une manière de configurer votre base de données MySQL. ![Création d’une base de données MySQL dans Azure - configurer](./media/store-php-create-mysql-database/create-db-2-configure.png)

3. Une fois la configuration terminée, cliquez sur **Créer**.
...
1. Configurez l’application Laravel dans votre environnement de développement local pour pointer vers la base de données MySQL. Pour ce faire, ouvrez `.env` à partir du répertoire racine de votre application Laravel, puis configurez les options de la base de données MySQL.
	...
	>[AZURE.NOTE] Dans le panneau **Propriétés**, le nom de votre base de données MySQL peut correspondre ou non à celui indiqué dans le champ **NOM DE BASE DE DONNÉES**. Il est préférable de vérifier le paramètre de base de données dans le champ **CHAÎNE DE CONNEXION**.
	>
...
5. Il vous suffit à présent de répliquer vos paramètres `.env` dans votre application web Azure. Exécutez les commandes suivantes dans l’interface de ligne de commande Azure :
	...
	Pour en savoir plus sur son mode de fonctionnement, consultez la section [Configure the Azure web app](./app-service-web/app-service-web-php-get-started.md#configure) (Configurer l’application web Azure).
