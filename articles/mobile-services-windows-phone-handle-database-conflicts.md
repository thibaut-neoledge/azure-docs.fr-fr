<properties urlDisplayName="Optimistic concurrency" pageTitle="Gestion des conflits d'écriture de base de données avec l'accès concurrentiel optimiste (Windows Store) | Centre de développement mobile" metaKeywords="" description="Learn how to handle database write conflicts on both the server and in your Windows Store application." metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Handling database write conlicts" authors="wesmc" manager="dwrede" />
...
Dans le cadre de ce didacticiel, vous allez ajouter une fonctionnalité à l'application de démarrage rapide afin de gérer les conflits qui se produisent lors de la mise à jour de la base de données TodoItem. Ce didacticiel vous familiarise avec ces étapes de base :
...
3. Dans l'Explorateur de solutions de Visual Studio, ouvrez MainPage.xaml.cs. Ajoutez le gestionnaire d'événements à MainPage pour l'événement `LostFocus` de TextBox, comme indiqué ci-dessous.
...
1. Dans MainPage.xaml.cs, mettez à jour la définition de la classe **TodoItem** avec le code suivant pour inclure la propriété système **__version** autorisant la prise en charge de la détection de conflits d'écriture :
	...
	<pre><code>//Activez l'accès concurrentiel optimiste en récupérant __version
...
3. Dans MainPage.xaml.cs, ajoutez la définition pour la méthode `ResolveConflict()` référencée dans `UpdateToDoItem()`. Notez que pour résoudre le conflit, vous devez définir la version de l'élément local sur la version mise à jour du serveur avant de valider la décision de l'utilisateur. À défaut, le conflit se répétera sans cesse.
...
3. Dans Visual Studio, définissez la zone de liste déroulante de la cible de déploiement sur **Emulator WVGA**.
...
6. Dans l'émulateur de gauche, mettez à jour la propriété `text` du dernier TodoItem sur **Test Write 1**, puis cliquez sur une autre zone de texte pour que le gestionnaire d'événements " LostFocus " mette à jour la base de données. La capture d'écran ci-dessous montre un exemple. 

	![][4]

7. À ce stade, l'élément correspondant dans l'émulateur de droite a une version ancienne et une valeur de texte ancienne. Dans l'émulateur à droite, entrez **Test Write 2** comme propriété de texte. Cliquez sur une autre zone de texte pour que le gestionnaire d'événements " LostFocus " dans l'émulateur de droite tente de mettre à jour la base de données avec l'ancienne version.

	![][5]

8. Comme la version utilisée pour la tentative de mise à jour ne correspond pas à celle du serveur, le Kit de développement logiciel (SDK) Mobile Services génère l'exception `MobileServicePreconditionFailedException`, ce qui permet à l'application de résoudre ce conflit. Pour résoudre le conflit, vous pouvez cliquer sur **ok** pour valider les valeurs de l'application à droite. Vous pouvez aussi cliquer sur **cancel** pour ignorer les valeurs de l'application à droite et conserver celles de l'application à gauche. 
...
5. Modifiez le texte TodoItem du dernier élément dans l'application de l'émulateur de gauche. Cliquez ensuite sur une autre zone de texte pour que le gestionnaire d'événements " LostFocus " mette à jour la base de données.

	![][4]

6. Dans l'émulateur de droite, entrez une autre valeur pour la propriété de texte du dernier TodoItem. Cliquez sur une autre zone de texte pour que le gestionnaire d'événements " LostFocus " dans l'émulateur de droite tente de mettre à jour la base de données avec l'ancienne version.

	![][5]

7. Notez qu'aucune exception n'a été rencontrée dans l'application, car le script serveur a résolu le conflit. Et comme l'élément n'était pas marqué comme terminé, la mise à jour a abouti. Pour confirmer la réussite de la mise à jour, cliquez sur **Actualiser** dans l'application de l'émulateur de gauche pour envoyer une nouvelle requête à la base de données.
...
9. Dans l'application de l'émulateur à droite, essayez de mettre à jour le même texte TodoItem et de déclencher l'événement " LostFocus ". Le script a résolu le conflit ainsi obtenu en refusant la mise à jour, car l'élément était déjà terminé. 
...
Ce didacticiel a présenté l'activation d'une application Windows Phone 8 pour gérer les conflits d'écriture lors de l'utilisation de données dans Mobile Services. Nous vous invitons ensuite à suivre l'un des didacticiels ci-dessous de notre série sur les données :
