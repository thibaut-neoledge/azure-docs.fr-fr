...
2. Dans l'Explorateur de solutions de Visual Studio, ouvrez MainPage.xaml.cs et ajoutez la directive " using " ci-dessous.

		using System.Threading.Tasks;


3. Dans l'Explorateur de solutions de Visual Studio, ouvrez MainPage.xaml.cs. Ajoutez le gestionnaire d'événements dans MainPage pour l'événement de zone de texte " LostFocus " comme indiqué ci-dessous.
...
1. Dans MainPage.xaml.cs, mettez à jour la définition de la classe **TodoItem** avec le code suivant pour inclure la propriété système **__version** autorisant la prise en charge de la détection de conflits d'écriture.
	...
	<pre><code>//Activer l'accès concurrentiel optimiste en récupérant __version
...
6. Dans l'émulateur de gauche, mettez à jour le " text " du dernier TodoItem sur **Test Write 1**, puis cliquez sur une autre zone de texte pour que le gestionnaire d'événements " LostFocus " mette à jour la base de données. La capture d'écran ci-dessous montre un exemple. 

	![][4]

7. À ce stade, l'élément correspondant dans l'émulateur à droite a une version ancienne et une valeur de texte ancienne. Dans l'émulateur de droite, entrez **Test Write 2** comme propriété de texte. Ensuite, cliquez sur une autre zone de texte pour que le gestionnaire d'événements " LostFocus " dans l'émulateur de droite tente de mettre à jour la base de données avec l'ancienne version.
...
6. Dans l'émulateur à droite, entrez une autre valeur pour la propriété de texte du dernier TodoItem. Ensuite, cliquez sur une autre zone de texte pour que le gestionnaire d'événements " LostFocus " dans l'émulateur de droite tente de mettre à jour la base de données avec l'ancienne version.
...
9. Dans l'application dans l'émulateur de droite, essayez de mettre à jour la propriété text du même TodoItem et de déclencher l'événement " LostFocus ". Le script a résolu le conflit ainsi obtenu en refusant la mise à jour, car l'élément était déjà terminé. 
