...
Voici quelques-unes des causes courantes entraînant des problèmes de déploiement ainsi que des conseils de dépannage pour vous aider à résoudre ces problèmes. Une instance de rôle qui ne démarre pas ou qui est recyclée entre les états **Initialisation**, **Occupée** et **Arrêtée** indique un problème au niveau de l’application.
...
Toutes les exceptions générées par les méthodes de la classe [RoleEntryPoint], y compris [OnStart], [OnStop] et [Run], sont des exceptions non gérées. Si une exception non gérée se produit dans une de ces méthodes, le rôle sera recyclé. Si le rôle est recyclé à plusieurs reprises, il peut générer une exception non gérée chaque fois qu'il tente de démarrer.
...
Pour exécuter un rôle web sous SSL, vous devez vous assurer que votre certificat de gestion exporté inclut la clé privée. Si vous utilisez le *gestionnaire de certificats Windows* pour exporter le certificat, veillez à sélectionner l’option *Oui*, exporter la clé privée. Le certificat doit être exporté au format PFX, l’unique format actuellement pris en charge.
