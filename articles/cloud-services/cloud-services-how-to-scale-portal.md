...
>[AZURE.NOTE] Cet article porte essentiellement sur les rôles web et de travail d’un service cloud. Lorsque vous créez directement une machine virtuelle (Classic), elle est hébergée dans un service cloud. Vous pouvez mettre à l’échelle un ordinateur virtuel standard en l’associant à un [groupe à haute disponibilité](../virtual-machines/virtual-machines-windows-classic-configure-availability.md) et en l’activant ou la désactivant manuellement.
...
1. Dans le panneau du service cloud, sélectionnez le nom du service cloud dans la vignette **Rôles et instances**. **IMPORTANT**: veillez à cliquer sur le rôle de service cloud, non sur l’instance de rôle qui se trouve sous le rôle.
