<properties 
	...
	description="En savoir plus sur les meilleures pratiques pour les performances dans Azure." 
...
La plupart des principes de bonne conception d’une base de données s’appliquent également à Azure SQL Database. Il existe un immense corpus de documents expliquant comment concevoir des schémas SQL Server ou Azure SQL Database efficaces. Les références relatives à la conception d’un schéma de base de données SQL sont les suivantes :
...
Lorsque vous prenez une décision relative à l'utilisation du stockage de table Azure, vous devez vous intéresser aux coûts et aux performances. Le stockage de table est beaucoup plus économique que le stockage des mêmes données dans une base de données SQL. Cependant, vous devez réfléchir soigneusement dans quelle mesure les données utilisent les fonctions relationnelles de SQL, comme les jointures, le filtrage des requêtes, etc. Si les données utilisent peu ces fonctionnalités, elles sont un bon candidat pour le stockage dans une table Azure.
...
Pour une vue d'ensemble des fédérations de la base de données SQL, consultez la page [Fédérations dans la base de données SQL](http://go.microsoft.com/fwlink/?LinkId=252668).
...
* Pour un compte complet, écrit avant le lancement des fédérations, consultez la page [Partitionnement avec la base de données SQL](http://go.microsoft.com/fwlink/?LinkId=252678). 
...
Prévoyez de fournir une capacité suffisante pour mettre en cache les objets auxquels vous accédez fréquemment. Dans la base de données SQL, il existe souvent des tables de référence utilisées pour convertir des codes numériques en chaînes de caractères descriptives plus longues. Ces tables incluent souvent des données telles que les noms de pays et ville, les valeurs de code postal valides, les noms des départements au sein de votre entreprise, etc. Pour les petites tables, il peut être judicieux de stocker la totalité de la table dans le cache ; pour les autres, vous pouvez seulement stocker les valeurs les plus fréquemment utilisées. Les gains de performances concernent les requêtes multi-jonctions impliquant ces données : pour chaque valeur trouvée dans le cache, plusieurs accès au disque sont économisés. Pour une introduction et une discussion sur les performances et la mise en cache dans Azure, consultez la page [Présentation du service de mise en cache Azure](http://go.microsoft.com/fwlink/?LinkId=252680). Un billet de blog plus récent sur le sujet est proposé sur la page [Mise en cache Azure – Considérations sur les performances](http://go.microsoft.com/fwlink/?LinkId=252681).

#### Scénario : utilisation de la mise en file d’attente des applications Azure ####
...
Azure dispose de deux technologies de file d’attente différentes : files d’attente de stockage Azure et Service Bus.
...
Pour une comparaison plus complète et détaillée des deux technologies, consultez la page [Files d'attente Windows Azure et files d'attente Windows Azure Service Bus - comparaison et différences](http://go.microsoft.com/fwlink/?LinkId=252682).

Pour une discussion sur les performances de Service Bus, consultez la page [Meilleures pratiques relatives aux améliorations de performances à l’aide de la messagerie répartie Service Bus](http://go.microsoft.com/fwlink/?LinkID=252683).
...
* [Données volumineuses (« Big Data »)](../big-data.md) 
* [Service Azure HDInsight](/documentation/services/hdinsight/)
* [Prise en main du service Azure HDInsight](../hdinsight-get-started.md)
...
Vous pouvez également définir des règles personnalisées. Pour plus d'informations, consultez la page [Bloc applicatif de mise à l’échelle automatique](http://go.microsoft.com/fwlink/?LinkId=252873).
...
* [Surveillance d’une base de données SQL Azure à l’aide de vues de gestion dynamique](http://go.microsoft.com/fwlink/?LinkId=236195) 
