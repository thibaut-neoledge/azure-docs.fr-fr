...
1. Dans le portail de gestion, cliquez sur le déploiement de votre service cloud.
2. Dans le volet **Propriétés** du portail de gestion, vérifiez les ports pour les instances de rôle (sous Points de terminaison d’entrée).
...
1. Dans le portail de gestion, cliquez sur le déploiement de votre service cloud.
2. Dans le volet **Propriétés** du portail de gestion, passez en revue les informations et déterminez si une réparation de service a eu lieu au moment où vous avez observé le redémarrage des rôles.

Les rôles redémarreront également environ une fois par mois pendant les mises à niveau du système d’exploitation hôte et du système d’exploitation invité. Pour plus d’informations, consultez le billet de blog [Redémarrages d’instances de rôles pour cause de mises à niveau du système d’exploitation](http://blogs.msdn.com/b/jarrettr/archive/2012/09/19/role-instance-restarts-due-to-os-upgrades.aspx).
...
1. Dans le portail de gestion, cliquez sur le déploiement de votre service cloud.
2. Dans le volet **Propriétés** du portail de gestion, examinez la valeur **État**. Si elle affiche **Prêt**, vérifiez la **Dernière opération** pour voir si une opération récente empêche l’échange d’adresses IP virtuelles.
...
Cette condition peut indiquer un problème avec votre code d’application, package ou fichier de configuration. Si la valeur est true, vous devez être en mesure de voir l’état changer à des intervalles de quelques minutes et le portail de gestion Azure peut indiquer des états tels que **Recyclage**, **Occupé**, ou **Initialisation**. Cela indique qu’il existe un problème au niveau de l’application qui empêche l’exécution de l’instance de rôle.
...
1. Dans le portail de gestion, cliquez sur l’instance de rôle.
2. Dans le volet **Propriétés** du portail de gestion, considérez les conditions suivantes pour résoudre votre problème :
...
[Données de diagnostic de calcul PaaS Azure]: http://blogs.msdn.com/b/kwill/archive/2013/08/09/windows-azure-paas-compute-diagnostics-data.aspx
