...
1. Dans le portail Azure Classic, cliquez sur le déploiement de votre service cloud.
2. Dans le volet **Propriétés** du portail Azure Classic, vérifiez les ports des instances de rôle (sous Points de terminaison d’entrée).
...
1. Dans le portail Azure Classic, cliquez sur le déploiement de votre service cloud.
2. Dans le volet **Propriétés** du portail Azure Classic, passez en revue les informations et déterminez si une réparation de service a eu lieu au moment où vous avez observé le redémarrage des rôles.
...
1. Dans le portail Azure Classic, cliquez sur le déploiement de votre service cloud.
2. Dans le volet **Propriétés** du portail Azure Classic, examinez la valeur **État**. Si elle affiche **Prêt**, vérifiez la **Dernière opération** pour voir si une opération récente empêche l’échange d’adresses IP virtuelles.
...
Cette condition peut indiquer un problème avec votre code d’application, package ou fichier de configuration. Si la valeur est true, vous devez être en mesure de voir l’état changer à des intervalles de quelques minutes et le portail Azure Classic peut indiquer des états tels que **Recyclage**, **Occupé** ou **Initialisation**. Cela indique qu’il existe un problème au niveau de l’application qui empêche l’exécution de l’instance de rôle.
...
1. Dans le portail Azure Classic, cliquez sur l’instance de rôle.
2. Dans le volet **Propriétés** du portail Azure Classic, tenez compte des conditions suivantes pour résoudre votre problème :
