<properties title="Cr&eacute;ation de votre premi&egrave;re solution de recherche &agrave; l'aide du service Azure&nbsp;Search" pageTitle="Cr&eacute;ation de votre premi&egrave;re solution de recherche &agrave; l'aide du service Azure&nbsp;Search" description="Cr&eacute;ation de votre premi&egrave;re solution de recherche &agrave; l'aide du service Azure&nbsp;Search" metaKeywords="" services="" solutions="" documentationCenter="" authors="Heidist" manager="mblythe" videoId="" scriptId="" />
...
-   [Création d'un index Azure Search][Création d'un index Azure Search]
-   [Exploration de CatalogIndexer][Exploration de CatalogIndexer]
-   [Création d'une application MVC4 avec Azure Search][Création d'une application MVC4 avec Azure Search]
-   [Exploration de AdventureWorksWeb][Exploration de AdventureWorksWeb]
...
-   Un service Azure Search. Vous aurez besoin du nom du service Azure Search et de la clé d’administration. Pour plus d'informations, consultez la section [Prise en main d'Azure Search][Prise en main d'Azure Search].
-   [Projet Adventure Works Azure Search Demo sur CodePlex][Projet Adventure Works Azure Search Demo sur CodePlex]. Dans l'onglet Source, cliquez sur **Télécharger** pour obtenir un fichier .zip de la solution.
...
> [WACOM.NOTE] Si vous recevez des erreurs incluant « Valeur non valide pour la clé 'attachdbfilename' » ou d'autres erreurs concernant des pièces jointes de base de données, il se peut que vous soyez confronté à des conflits de contrôle de compte d'utilisateur. Dans le cadre de cette démonstration, contournez ces erreurs de la manière suivante :
> Copiez la solution dans un dossier nouveau ou existant (comme Temp) qui fournit un accès aux utilisateurs authentifiés.
> Utilisez **Exécuter en tant qu'administrateur** pour lancer Visual Studio.
> Ouvrez la solution, générez-la, puis appuyez sur **F5** pour créer l'index.

Pour vérifier la création de l'index et le téléchargement de documents, accédez au tableau de bord du service de recherche dans la [version préliminaire du portail Azure][version préliminaire du portail Azure]. Dans Utilisation, le nombre d'index doit être augmenté de 1, et vous devez avoir 294 documents, un pour chaque produit de la base de données.

Cliquez sur la vignette **Index** pour afficher la liste des index. Celle-ci change pour afficher le nouveau nombre d'index et de documents.
...
8.  Entrez un terme de recherche, comme « VTT ». Au cours de la frappe, une liste déroulante de requêtes suggérées s'affiche.
...
## Exploration de AdventureWorksWeb
...
2.  **HomeController.cs** inclut deux fonctions principales :
    ...
    **Rechercher** : lorsque l'utilisateur clique sur le bouton de recherche ou choisit une facette, cette fonction est appelée pour récupérer les résultats et les renvoyer pour affichage dans la vue Index.
...
1.  Régénérez l'application et appuyez sur **F5** pour l'exécuter.

2.  Dans la zone de recherche, tapez « Route ». Au bout d'une seconde, une liste d'éléments doit s'afficher sous la zone de texte avec des suggestions parmi lesquelles l'utilisateur peut opérer un choix.
...
-   Examinez l'[API Recherche][API Recherche] qui permet d'effectuer une requête dans Azure Search pour extraire un document spécifique (par exemple, vous pouvez transmettre l'ID de produit).
...
-   Consultez cet autre exemple de code et la vidéo sur la recherche géospatiale : [Channel 9 - Azure Search et données géospatiales][Channel 9 - Azure Search et données géospatiales] et [CodePlex : exemple de recherche géographique Azure Search][CodePlex : exemple de recherche géographique Azure Search]
...
<!--Anchors--> <!--Image references-->
  ...
  [Exploration de AdventureWorksWeb]: #sub-5
  [Étapes suivantes]: #next-steps
  [0]: ./media/search-create-first-solution/AzureSearch_Create1_WebApp.PNG
  [Centre de téléchargement]: http://www.microsoft.com/fr-fr/download/details.aspx?id=40747
  [Prise en main d'Azure Search]: ../search-get-started/
  [Projet Adventure Works Azure Search Demo sur CodePlex]: http://go.microsoft.com/fwlink/p/?LinkID=510972
  ...
  [documentation sur l'API REST Azure Search]: http://msdn.microsoft.com/fr-fr/library/azure/dn798935.aspx
  ...
  [API Suggestions]: http://msdn.microsoft.com/fr-fr/library/azure/dn798936.aspx
  [API Recherche]: http://msdn.microsoft.com/fr-fr/library/azure/dn798929.aspx
  [Channel 9 - Azure Search et données géospatiales]: http://channel9.msdn.com/Shows/Data-Exposed/Azure-Search-and-Geospatial-Data
  [CodePlex : exemple de recherche géographique Azure Search]: http://azuresearchgeospatial.codeplex.com
