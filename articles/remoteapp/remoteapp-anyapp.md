<properties
   pageTitle="Exécutez n’importe quelle application sur n’importe quel appareil avec RemoteApp"
   description="Découvrez comment partager n’importe quelle application avec vos utilisateurs grâce à RemoteApp."
   services="remoteapp"
   documentationCenter=""
   authors="lizap"
   manager="mbaldwin"
...
Grâce à RemoteApp, vos utilisateurs peuvent également utiliser leurs propres appareils Android ou Apple et bénéficier de la même expérience que sur Windows (ou Windows Phone). Pour obtenir ce résultat, votre application Windows est hébergée sur une collection de machines virtuelles Windows via Azure, auxquelles vos utilisateurs peuvent accéder depuis n’importe quel lieu connecté à Internet. Commençons dès maintenant !

Pour ce didacticiel, nous allons partager l’application Access avec tous nos utilisateurs. Comme Access est une base de données, et comme nous voulons que cette base de données soit utile, nous allons effectuer quelques étapes supplémentaires afin de permettre aux utilisateurs d’accéder au partage de données Access.
...
1. Sur le portail de gestion Azure, faites défiler le volet de navigation de gauche jusqu’à ce qu’Azure RemoteApp apparaisse. Ouvrez cette page.
