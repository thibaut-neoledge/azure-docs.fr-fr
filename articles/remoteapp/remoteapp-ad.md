...
Lorsque vous avez configuré le suffixe du domaine UPN dans Azure AD, tous les utilisateurs ouvrant une session dans Azure RemoteApp sont connectés en tant que « utilisateur@<suffixe que vous avez défini> ». Assurez-vous que les utilisateurs peuvent également ouvrir une session avec le même user@suffix dans le domaine local. Dans certains cas vous pouvez configurer un nom de domaine dans Azure AD lors de la spécification d’un suffixe de domaine différent pour l’utilisateur local. Dans ce cas, vos utilisateurs ne peuvent plus se connecter à des ordinateurs ou des ressources appartenant à un domaine via Azure RemoteApp.
