...
**Remarque** Si vous prévoyez de publier votre service cloud depuis Windows Team Foundation Services (TFS), utilisez Quick Create, puis configurez la publication TFS dans **Quick Start** ou dans le tableau de bord. Pour plus d'informations, consultez la page [Livraison continue sur Azure au moyen de Visual Studio Online][TFSTutorialForCloudService] ou **Quick Start**.
...
Le Kit de développement logiciel (SDK) Azure fournit les outils nécessaires à la préparation des fichiers de déploiement. Vous pouvez l'installer depuis la page des [téléchargements Azure](http://www.windowsazure.com/fr-fr/develop/downloads/), dans le langage souhaité pour le développement de votre code.

Si vous découvrez les services cloud, vous pouvez télécharger un exemple de package de services cloud (.cspkg) et de fichier de configuration de service (.cscfg) dans les [exemples de code Azure](http://code.msdn.microsoft.com/windowsazure/).
...
## <span id="quick"></span></a> d'un service cloud avec Quick Create

1.  Dans le [portail de gestion](http://manage.windowsazure.com/), cliquez sur **Nouveau**, sur **Calcul**, sur **Service cloud**, puis sur **Création rapide**.
...
4.  Cliquez sur **Create Cloud Service**.
    ...
    Si un des rôles du service cloud nécessite un certificat pour le chiffrement de données SSL et que le certificat n'a pas été téléchargé sur Azure, vous devez télécharger ce certificat avant de déployer le service cloud. Une fois le certificat téléchargé, les applications Windows qui s'exécutent dans les instances de rôles peuvent y accéder.

## <span id="uploadcertificate"></span></a> d'un certificat pour un service cloud
...
3.  Cliquez sur **Télécharger un certificat**.
     La fenêtre **Télécharger un certificat** s'ouvre.

    ![CloudServices\_AddaCertificate](./media/cloud-services-how-to-create-deploy/CloudServices_AddaCertificate.png)

4.  Dans **Fichier de certificat**, utilisez **Parcourir** pour sélectionner le certificat (fichier .pfx ou .cer) à utiliser.
...
## <span id="deploy"></span></a> d'un service cloud

1.  Si vous n'avez pas installé le Kit de développement logiciel (SDK), cliquez sur **Install Azure SDK** pour ouvrir la page des [téléchargements Azure](http://www.windowsazure.com/fr-fr/develop/downloads/), puis téléchargez le Kit de développement logiciel dans le langage souhaité pour le développement de votre code.
    ...
    > [WACOM.NOTE]
    > Pour les services cloud créés précédemment (appelés *services hébergés*), vous devez vous assurer que les systèmes d'exploitation hôtes sur les machines virtuelles (instances de rôle) sont compatibles avec la version du Kit de développement logiciel (SDK) Azure que vous installez. Pour plus d'informations, consultez les [notes de publication du Kit de développement logiciel (SDK) Azure](http://msdn.microsoft.com/fr-fr/library/windowsazure/hh552718.aspx).

2.  Dans le [portail de gestion](http://manage.windowsazure.com/), cliquez sur **Cloud Services**. Cliquez ensuite sur le nom du service cloud pour ouvrir le tableau de bord, puis cliquez sur **Tableau de bord**.

3.  Cliquez sur **Production** ou sur **Intermédiaire**.
...
4.  Cliquez sur **Télécharger** dans le tableau de bord pour déployer votre service cloud.

    ![CloudServices\_UploadaPackage](./media/cloud-services-how-to-create-deploy/CloudServices_UploadaPackage.png)

5.  Dans **Étiquette du déploiement**, entrez le nom du nouveau déploiement, par exemple MyCloudServicev1.
...
2.  Sous **quick glance**, cliquez sur l'URL du site pour ouvrir le service cloud dans un navigateur Web.
  ...
  [exemples de code Azure]: http://code.msdn.microsoft.com/windowsazure/
  [Configuration d'un certificat SSL sur un point de terminaison HTTPS]: http://msdn.microsoft.com/fr-fr/library/windowsazure/ff795779.aspx
  [Présentation de la configuration d'une connexion Bureau à distance pour un rôle]: http://msdn.microsoft.com/fr-fr/library/windowsazure/gg433010.aspx
  ...
  [CloudServices\_AddaCertificate]: ./media/cloud-services-how-to-create-deploy/CloudServices_AddaCertificate.png
  [CloudServices\_CertificateProgress]: ./media/cloud-services-how-to-create-deploy/CloudServices_CertificateProgress.png
  [notes de publication du Kit de développement logiciel (SDK) Azure]: http://msdn.microsoft.com/fr-fr/library/windowsazure/hh552718.aspx
  [CloudServices\_QuickStartPage]: ./media/cloud-services-how-to-create-deploy/CloudServices_QuickStartPage.png
  [CloudServices\_UploadaPackage]: ./media/cloud-services-how-to-create-deploy/CloudServices_UploadaPackage.png
