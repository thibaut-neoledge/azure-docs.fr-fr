<properties urlDisplayName="How to stage sites on Microsoft Azure" pageTitle="Déploiement intermédiaire dans Sites Web Microsoft Azure" metaKeywords="Microsoft Azure Web Sites, Staged Deployment, Site Slots" description="Découvrez comment utiliser la publication intermédiaire sur Sites Web Microsoft Azure." metaCanonical="" services="web-sites" documentationCenter="" title="Staged Deployment on Microsoft Azure Websites" authors="cephalin"  solutions="" writer="cephalin" manager="wpickett" editor="mollybos"  />
...
Le site doit être exécuté dans le plan d'hébergement **Standard** pour activer plusieurs emplacements de déploiement. 
...
1. Sur la page Démarrage rapide ou dans la section Aperçu rapide de la page Tableau de bord de votre site web, cliquez sur **Ajouter un nouvel emplacement de déploiement**. 
	...
	> [WACOM.NOTE]
	> Si le site web n'est pas déjà en mode **standard**, le message **Vous devez être en mode standard pour activer la publication intermédiaire** s'affiche. À ce moment, vous pouvez sélectionner **Mettre à niveau** et accéder à l'onglet **Mettre à l'échelle** de votre site web, avant de continuer.
...
5. Dans la liste des sites web, cliquez sur la coche à gauche du nom de votre site web pour afficher l'emplacement de déploiement. Il portera le nom du site suivi du nom de l'emplacement de déploiement. 
	
	![Site List with Deployment Slot][SiteListWithStagedSite]
	
4. Lorsque vous cliquez sur le nom de l'emplacement du site de déploiement, une page s'ouvre. Elle contient plusieurs onglets, comme n'importe quel autre site web. <strong><i>your-website-name</i> (<i>deployment-slot-name</i>)</strong> s'affiche en haut de la page du portail pour vous rappeler que vous consultez l'emplacement du site de déploiement.
...
Lorsque vous clonez la configuration depuis un autre emplacement de déploiement, celle-ci est modifiable. Les listes suivantes présentent les éléments de configuration qui changent lorsque vous effectuez des basculements d'emplacements.

**Éléments de configuration modifiés lors du basculement d'emplacement** :
...
**Éléments de configuration non modifiés lors du basculement d'emplacement** :
...
- Plusieurs emplacements de déploiement sont uniquement disponibles pour les sites dans le plan d'hébergement web **Standard**.
...
3. Cliquez sur la coche pour terminer l'opération. Une fois l'opération terminée, les emplacements de site sont basculés.
...
- Pour plus d'informations sur l'installation et la configuration d'Azure PowerShell et sur l'authentification d'Azure PowerShell avec votre abonnement Windows Azure, consultez l'article [Installation et configuration d'Azure PowerShell](http://www.windowsazure.com/fr-fr/documentation/articles/install-configure-powershell).  
...
###Show-AzureWebsite
Une fois les mises à jour de contenu et de configuration appliquées au nouvel emplacement, vous pouvez les valider en accédant à l'emplacement via la cmdlet **Show-AzureWebsite**, comme le montre l'exemple suivant.
