---
title: 'Exemple DMZ : créer un périmètre de réseau pour protéger les réseaux avec un pare-feu, UDR et un groupe de sécurité réseau | Microsoft Docs'
...
> [!TIP]
> La deuxième règle de trafic de l’application autorise n’importe quel port dans cet exemple. Dans un scénario réel le port et les plages d’adresses les plus spécifiques permettent de réduire la surface d’attaque de cette règle.
> 
> 
...
* **Règles de trafic d’application** : elles sont au nombre de deux, la première correspondant au trafic web frontal, et la seconde, pour le trafic de l’ordinateur principal (par exemple, serveur web vers couche de données). La configuration de ces règles dépend de l’architecture du réseau sur lequel sont placés vos serveurs et des flux de trafic (sens du flux de trafic et les ports utilisés).
    ...
    **Remarque** : aucun port n’est affecté dans le champ de la liste cible est, et par conséquent le port entrant 80 (ou 443 pour le Service sélectionné) sera utilisé pour la redirection du serveur web. Si le serveur web est à l’écoute sur un autre port, par exemple, le port 8080, le champ de liste cible peut être mis à jour vers 10.0.1.4:8080 pour permettre aussi de rediriger le port.
    ...
    Dans cette capture d’écran l’élément « < explicite-dest > » est utilisé dans le champ de Destination pour désigner 10.0.2.5 comme destination. Il peut être soit explicite, comme indiqué, ou il peut s’agir d’un objet réseau nommé (comme dans les conditions préalables requises pour le serveur DNS). C’est à l’administrateur du pare-feu qu’il appartient de choisir la méthode qui sera utilisée. Pour ajouter 10.0.2.5 en tant que destination explicite, double-cliquez sur la première ligne vide sous < explicite dest >, puis saisissez l’adresse dans la fenêtre qui s’affiche.
...
#### (Autorisé) Internet vers le serveur web
...
#### (Autorisé) Internet RDP vers le serveur principal
...
#### (Refusé) Internet direct vers le serveur web
...
#### (Refusé) Internet vers le serveur principal
...
#### (Autorisé) Serveur frontal vers Serveur principal
...
#### (Refusé) Accès Internet vers SQL via le pare-feu
...
<!--Image References-->
[1]: ./media/virtual-networks-dmz-nsg-fw-udr-asm/example3design.png "DMZ bidirectionnel avec NVA, NSG et UDR"
[2]: ./media/virtual-networks-dmz-nsg-fw-udr-asm/example3firewalllogical.png "Affichage logique des règles de pare-feu"
[3]: ./media/virtual-networks-dmz-nsg-fw-udr-asm/createnetworkobjectfrontend.png "Créer un objet réseau de serveur frontal"
