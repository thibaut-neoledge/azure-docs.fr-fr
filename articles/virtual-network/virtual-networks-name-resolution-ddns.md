...
[Azure fournit la résolution de noms](virtual-networks-name-resolution-for-vms-and-role-instances.md) pour les machines virtuelles et les instances de rôle. Toutefois, quand votre résolution de noms doit aller au-delà de ceux fournis par Azure, vous pouvez fournir vos propres serveurs DNS. Cela vous permet de personnaliser votre solution DNS pour l’adapter à vos propres besoins. Par exemple, vous pouvez avoir besoin d’une solution DNS pour l’accès à votre contrôleur de domaine Active Directory local.

Comme Azure n’a pas la possibilité (informations d’identification, par exemple) d’inscrire directement les enregistrements dans vos serveurs DNS, d’autres mécanismes sont souvent nécessaires. Voici quelques scénarios courants avec des solutions alternatives.
