...
Découvrez comment créer, modifier ou supprimer une homologation de réseau virtuel. Une homologation de réseau virtuel permet de connecter deux réseaux virtuels situés dans un même emplacement Azure via le réseau principal Azure. Une fois homologués, les deux réseaux virtuels sont toujours gérés comme des ressources distinctes. Les ressources dans chaque réseau virtuel communiquent avec les mêmes latences et bande passante que si elles étaient dans le même réseau virtuel. Si vous n’êtes pas familiarisé avec l’homologation de réseau virtuel, avant d’accomplir les tâches décrites dans cet article, nous vous recommandons de lire [Homologation de réseaux virtuels](virtual-network-peering-overview.md), et de suivre le didacticiel [Créer une homologation de réseau virtuel](virtual-network-create-peering.md).
...
- Si vous utilisez des commandes de l’interface de ligne de commande (CLI) Azure pour accomplir les tâches décrites dans cet article, vous devez [Installer et configurer Azure CLI](/cli/azure/install-azure-cli?toc=%2fazure%2fvirtual-network%2ftoc.json). Assurez-vous que la version la plus récente d’Azure CLI est installée. Pour obtenir de l’aide sur les commandes CLI, entrez `az <command> --help`. Au lieu d’installer l’interface CLI et ses prérequis, vous pouvez utiliser Azure Cloud Shell. Azure Cloud Shell est un interpréteur de commandes Bash gratuit, que vous pouvez exécuter directement dans le portail Azure. L’interface Azure CLI est préinstallée et configurée sur Cloud Shell pour être utilisée avec votre compte. Pour utiliser Cloud Shell, cliquez sur le bouton Cloud Shell **> _** en haut du [portail](https://portal.azure.com). 

## <a name="create-a-peering"></a>Créer une homologation

>[!NOTE]
>La réussite de la création d’une homologation de réseau virtuel est sujette à plusieurs conditions, contraintes et considérations. Avant de créer une homologation, familiarisez-vous avec les [exigences et contraintes](#requirements-and-constraints) et les [autorisations requises](#permissions).
>
