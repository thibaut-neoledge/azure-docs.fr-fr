<properties 
   pageTitle="Résoudre les problèmes relatifs aux groupes de sécurité réseau - Portail | Microsoft Azure"
...
# Résoudre les problèmes relatifs aux groupes de sécurité réseau à l’aide du portail Azure
...
### Afficher les règles de sécurité effectives pour une machine virtuelle
...
6. Le panneau **Obtenir les règles de sécurité effectives** s’affiche, comme illustré dans l’image suivante :
	...
	- **Interface réseau :** l’option *VM1-NIC1* est sélectionnée. Une machine virtuelle peut avoir plusieurs interfaces réseau. Chaque carte réseau peut avoir des règles de sécurité effectives uniques. En cours de dépannage, vous devrez peut-être afficher les règles de sécurité effectives pour chaque carte réseau.
...
7. Cliquez sur l’onglet **Subnet1-NSG** pour afficher les règles effectives du groupe de sécurité réseau appliquées au sous-réseau, comme illustré dans l’image suivante :
	...
	>[AZURE.NOTE]Si la machine virtuelle associée à la carte réseau n’est pas en cours d’exécution, ou si les groupes de sécurité réseau n’ont pas été appliqués à la carte réseau ou au sous-réseau, aucune règle ne s’affiche.

8. Pour modifier les règles du groupe de sécurité réseau, cliquez sur *Subnet1-NSG* dans la section **Groupes de sécurité réseau associés**. Cette opération ouvre le panneau **Subnet1-NSG**. Vous pouvez modifier directement les règles en cliquant sur **Règles de sécurité de trafic entrant**.
...
### Afficher les règles de sécurité effectives pour une interface réseau
...
## Afficher les règles de sécurité effectives pour un groupe de sécurité réseau
...
3. Sélectionnez un groupe de sécurité réseau. Dans l’image suivante, un groupe de sécurité réseau nommé VM1-nsg a été sélectionné.
	...
	>[AZURE.NOTE] Si un groupe de sécurité réseau est appliqué uniquement à un sous-réseau vide, les machines virtuelles ne sont pas répertoriées. Si un groupe de sécurité réseau est appliqué à une carte réseau qui n’est pas associée à une machine virtuelle, cette carte réseau n’est pas répertoriée.
...
>[AZURE.NOTE] S’il ne peut y avoir un seul groupe de sécurité réseau appliqué à un sous-réseau et à une carte réseau, un groupe de sécurité réseau peut être associé à plusieurs cartes réseau et sous-réseaux.
