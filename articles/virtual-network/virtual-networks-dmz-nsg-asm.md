...
6.	Tout le trafic (tous les ports) issu du sous-réseau du serveur frontal à destination du sous-réseau du serveur principal est refusé.

Lorsque ces règles sont associées à chacun des sous-réseaux, si une requête HTTP entrante en provenance d’HTTP arrive d’Internet à destination du serveur web, les 3 règles (autorisation) et les 5 règles (refus) s’appliquent. Cependant, comme la règle 3 a une priorité plus élevée, elle seule s’applique, et la règle 5 n’entre pas en jeu. La requête HTTP est donc autorisée à accéder au serveur web. Si le même trafic tentait d’atteindre le serveur DNS01, la règle 5 (Refus) serait la première à s’appliquer et le trafic ne serait pas autorisé à franchir le serveur. La règle 6 (Refus) bloque la communication du sous-réseau du serveur frontal vers le sous-réseau du serveur principal (excepté le trafic autorisé dans les règles 1 et 4), ce qui protège le réseau du serveur principal en cas d’intrusion sur l’application web du serveur frontal. Le pirate a alors un accès limité au réseau principal « protégé » (uniquement pour les ressources exposées sur le serveur AppVM01).
...
11.	Le sous-réseau du serveur frontal commence le traitement des règles de trafic entrant :
  ...
  2.	La règle du système par défaut autorisant le trafic entre sous-réseaux autorise le trafic. Donc, le trafic est transmis.
...
10.	Aucune règle sur le trafic sortant sur le sous-réseau du serveur principal. Le trafic est autorisé.
11.	Le sous-réseau du serveur frontal commence le traitement des règles de trafic entrant :
  1.	Aucune règle NSG ne s’applique au trafic entrant en provenance du sous-réseau du serveur principal vers le sous-réseau du serveur frontal, et donc aucune des règles NSG ne s’applique.
...
3.	La création d’un nouveau réseau virtuel et de deux sous-réseaux, comme indiqué dans le fichier de configuration du réseau
4.	La génération de 4 machines virtuelles Windows Server
5.	Configurez un groupe de sécurité réseau, notamment :
  -	Création d’un groupe de protection réseau
  -	Renseignement de ce dernier avec des règles
