...
4. <a name="register"></a>Exécutez les commandes PowerShell suivantes (vous ne pouvez pas vous inscrire avec l’interface CLI) afin de vous inscrire pour la version préliminaire :
...
5. Créez la machine virtuelle en exécutant le script suivant sur un ordinateur Linux ou Mac. Le script crée un groupe de ressources, un réseau virtuel (VNet), une carte réseau avec trois configurations IP et une machine virtuelle avec les deux cartes réseau qui lui sont associées. La carte réseau, l’adresse IP publique, le réseau virtuel et les ressources de la machine virtuelle doivent se trouver au même emplacement et correspondre au même abonnement. Bien que les ressources n’aient pas toutes à se trouver dans le même groupe de ressources, c’est le cas dans le script suivant.
    ...
    En plus de créer une machine virtuelle avec une carte réseau présentant trois configurations IP, le script génère : - Un unique disque géré par compte Premium par défaut. Notez que vous pouvez créer ici différents types de disque. Consultez l’article [Créer une machine virtuelle Linux à l’aide d’Azure CLI 2.0](../virtual-machines/virtual-machines-linux-quick-create-cli.md?toc=%2fazure%2fvirtual-network%2ftoc.json) pour plus d’informations.
...
2. Inscrivez-vous pour recevoir la version préliminaire publique. Pour cela, suivez les indications de l’[étape 4](#register), dans la section **Créer une machine virtuelle avec plusieurs adresses IP** de cet article.
...
4. Affichez les ressources d’adresses IP privées et l’adresse IP publique affectées à la carte d’interface réseau en saisissant la commande suivante :
