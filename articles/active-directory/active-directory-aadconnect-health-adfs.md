...
>[AZURE.NOTE] Pour utiliser l’analyse de l’utilisation avec AD FS, vous devez vous assurer que l’audit AD FS est activé. Pour plus d’informations, consultez [Activer l’audit pour AD FS](active-directory-aadconnect-health-agent-install.md#enable-auditing-for-ad-fs).
...
>[AZURE.NOTE] Ce rapport est automatiquement mis à jour toutes les deux heures avec les nouvelles informations collectées durant ce laps de temps. En conséquence, les tentatives de connexion effectuées dans les deux dernières heures peuvent ne pas être incluses dans le rapport.
