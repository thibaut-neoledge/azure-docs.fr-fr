...
## <a name="0----prerequisites"></a>*0.    Configuration requise*
...
* Pour générer et exécuter pour iOS
  ...
  * Xcode 5.x ou supérieur. Téléchargez-le à l’adresse http://developer.apple.com/downloads ou à partir du [Mac App Store](http://itunes.apple.com/us/app/xcode/id497799835?mt=12)
...
2. Dans le coin supérieur droit, cliquez sur votre compte et sous la liste **Répertoire**, choisissez un client Active Directory pour lequel vous disposez des autorisations d’administrateur.
3. Tapez **Inscriptions des applications** dans le filtre de recherche.
4. Cliquez sur **Inscriptions des applications**, puis sur **Ajouter**.
5. Suivez les invites et créez une **application cliente native** (bien que les applications Cordova soient basées sur le langage HTML, l’option `Native Client Application` doit être sélectionnée, car nous créons ici une application cliente native ; sinon, l’application ne fonctionnera pas). Entrez un nom convivial pour l’application, puis sélectionnez « Native » comme type d’application. **l’URI de redirection** est l’URI utilisé pour retourner les jetons à votre application, par exemple `http://MyDirectorySearcherApp` Cliquez sur **Créer** pour créer l’application.
6. Toujours dans le portail Azure, choisissez votre application, cliquez sur **Paramètres** et choisissez **Propriétés**.
7. Recherchez la valeur de l’ID d’application et copiez-la dans le Presse-papiers.
8. Configurez les autorisations pour votre application : dans le menu Paramètres, cliquez sur la section « Autorisations requises », cliquez sur **Ajouter**, sur **Sélectionner une API**, puis sélectionnez « Windows Azure Active Directory » (il s’agit de l’API AADGraph). Cliquez ensuite sur **Sélectionner les autorisations** et sélectionnez « Accéder au répertoire en tant qu’utilisateur connecté ».
...
Cela permet de créer la structure de dossiers et la structure du projet Cordova, en copiant le contenu du projet de démarrage dans le sous-dossier www.
...
## <a name="3-add-code-to-authenticate-users-and-obtain-tokens-from-aad"></a>*3. Ajout de code pour authentifier les utilisateurs et obtenir des jetons d’AAD*
