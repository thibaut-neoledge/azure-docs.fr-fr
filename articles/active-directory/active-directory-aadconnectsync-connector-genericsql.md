...
- **Authentification Windows** : la base de données d’authentification utilisera les informations d’identification Windows pour contrôler l’utilisateur. Le nom d’utilisateur/mot de passe spécifié sera utilisé pour l’authentification auprès de la base de données. Ce compte aura besoin des autorisations pour la base de données.
...
- **Authentification Azure SQL Database** : pour plus d’informations, voir [Connexion à la base de données SQL avec l’authentification Azure Active Directory](..\sql-database\sql-database-aad-authentication.md).
