...
L’incidence de l’installation de l’agent Microsoft Identity Health sur les contrôleurs de domaine ou serveurs AD FS est minime en matière de consommation du processeur, de consommation de la mémoire, d’utilisation de la bande passante réseau et du stockage.
...
> [!NOTE]
> Si l’agent ne parvient pas à communiquer avec Azure, il stocke les données localement, jusqu’à une limite maximale définie. Une fois que l'agent atteint cette limite, s’il n'a pas été en mesure de télécharger les données sur le service, les nouvelles transactions ADFS remplaceront toutes les transactions « mises en cache » sur la base « dernier événement traité ».
> 
> 
...
Si vous devez configurer un serveur proxy lors de l'inscription de l'agent, vous devez modifier vos paramètres de proxy Internet Explorer. <br>
Ouvrez Internet Explorer -> Paramètres -> Options Internet -> Connexions -> Paramètres de réseau local.<br>
Sélectionnez Utiliser un serveur proxy pour votre réseau local.<br>
Sélectionnez Avancé SI vous disposez de ports proxy différents pour les protocoles HTTP et HTTPS sécurisé.<br>
