<properties
	...
	description="Comparaison entre le service Azure AD mis à la disposition générale et la version préliminaire publique du modèle d’application v2.0."
...
Le modèle d’application v2.0 nécessite que vous inscriviez vos applications Microsoft dans un nouvel emplacement : [apps.dev.microsoft.com](https://apps.dev.microsoft.com). Il s’agit du portail où vous pouvez obtenir un ID d’application, personnaliser l’apparence de la page de connexion de votre application, et bien plus encore. Nous allons continuer à ajouter des fonctionnalités à ce portail d’inscription des applications, en nous appuyant largement sur les commentaires reçus pendant la version préliminaire. Notre souhait est que ce portail soit le nouvel emplacement où vous pouvez gérer absolument tout ayant un rapport avec vos applications Microsoft.

Et le meilleur dans tout cela, c’est qu’il est inutile de posséder un abonnement Azure ou Office pour l’utiliser. Il vous suffit de disposer d’un compte Microsoft personnel ou d’un compte professionnel/scolaire.

Notez que pour le moment les applications inscrites dans le nouveau portail d’inscription des applications fonctionnent uniquement avec le modèle d’application v2.0, et que *seules* les applications inscrites dans ce nouveau portail fonctionnent avec le modèle d’application v2.0. Si vous essayez d’utiliser l’une de ces applications avec les services de compte Microsoft ou Azure AD mis à la disposition générale, ou si vous essayez d’utiliser une application existante avec le modèle d’application v2.0, vous pouvez rencontrer des incompatibilités.


## Un ID d’application pour toutes les plateformes
Dans le service Azure AD mis à la disposition générale, vous pouvez avoir inscrit plusieurs applications différentes pour un seul et même projet. Vous avez été contraint d’utiliser des inscriptions d’application distinctes pour vos applications clientes et web natives :
...
Dans le service Azure AD mis à la disposition générale, une application peut se comporter comme une **ressource**, ou un destinataire de jetons. Une ressource peut définir plusieurs **étendues** ou **oAuth2Permissions** qu’elle comprend, permettant ainsi aux applications clientes de demander des jetons pour cette ressource pour un ensemble d’étendues donné. Prenez comme exemple de ressource l’API Graph d’Azure AD :
...
Pour en savoir plus sur les demandes spécifiques émises dans les jetons du modèle d’application v2.0, consultez [Informations de référence sur les jetons du modèle d’application v2.0](active-directory-v2-tokens.md).
