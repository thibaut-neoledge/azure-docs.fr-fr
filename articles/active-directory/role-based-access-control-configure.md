...
![Les ressources Active Directory Azure et les outils de gestion de ressources - diagramme](./media/role-based-access-control-configure/overview.png)

Le rôle RBAC que vous attribuez aux utilisateurs, groupes et applications détermine les ressources que l’utilisateur (ou l’application) peut gérer au sein de cette étendue.

### Rôles intégrés Azure RBAC
...
### Comparaison entre RBAC Azure et Administrateur et coadministrateurs d’abonnement classiques
...
Le modèle d’autorisation plus précis (contrôle d’accès basé sur les rôles Azure - RBAC) est pris en charge uniquement par le portail Azure et les nouvelles API Azure Resource Manager. Les utilisateurs et les applications qui se voient attribuer des rôles RBAC (dans l’étendue abonnement/groupe de ressources/ressource) ne peuvent pas utiliser le portail de gestion classique et les API de gestion des services Azure.
...
### Voir l’accès
Sélectionnez les paramètres d’accès dans la section Téléchargements du panneau **Groupe de ressources**. Le panneau **Utilisateurs** répertorie l’ensemble des utilisateurs, groupes et applications qui peuvent accéder au groupe de ressources. L’accès est attribué sur le groupe de ressources ou hérité d’une affectation sur l’abonnement parent.
...
### Ajout d’un accès
1. Cliquez sur l’icône **Add** (Ajouter) du panneau **Users** (Utilisateurs). ![Panneau Ajouter un accès - sélectionner un rôle - capture d’écran](./media/role-based-access-control-configure/grant-access1.png)
...
4. Recherchez des utilisateurs, groupes et applications dans l’annuaire à l’aide de noms complets, d’adresses de messagerie et d’identificateurs d’objet. ![Panneau Ajouter un accès panneau d’accès - rechercher - capture d’écran](./media/role-based-access-control-configure/grant-access2.png)
...
-	Utilisez `azure role list` pour répertorier les rôles RBAC pouvant être attribués. Utilisez `azure role show` pour examiner les opérations auxquelles ils octroient l’accès.
...
Utilisez les commandes `Get-AzureRmProviderOperation` ou `azure provider operations show` pour répertorier les opérations des fournisseurs de ressources Azure. Vous pouvez également utiliser ces commandes pour vérifier qu’une chaîne d’opération est valide et pour développer les chaînes d’opération génériques.

![PowerShell Get-AzureRMProviderOperation - capture d’écran](./media/role-based-access-control-configure/1-get-azurermprovideroperation-1.png)

![Afficher des opérations du fournisseur Azure CLI - capture d’écran](./media/role-based-access-control-configure/1-azure-provider-operations-show.png)

### Non-actions
Si l’ensemble des opérations que vous souhaitez autoriser est plus simple à exprimer si l’on exclut des opérations spécifiques plutôt que si l’on inclut toutes les opérations à l’exception des opérations à exclure, utilisez la propriété **NotActions** d’un rôle personnalisé. L’accès effectif accordé par un rôle personnalisé est déterminé par l’exclusion des opérations **NotActions** des opérations **Actions**.
...
La propriété **AssignableScopes** d’un rôle personnalisé permet également de contrôler les personnes autorisées à afficher, à mettre à jour et à supprimer le rôle. Voici quelques exemples d’étendues d’affectation valides :
