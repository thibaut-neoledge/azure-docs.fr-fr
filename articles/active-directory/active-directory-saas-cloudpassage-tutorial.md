...
L’objectif de ce didacticiel est de vous montrer comment intégrer CloudPassage à Azure Active Directory (Azure AD). L’intégration de CloudPassage dans Azure AD vous offre les avantages suivants :
...
12. Pour ajouter les attributs utilisateur requis, pour chaque ligne dans le tableau ci-dessous, procédez comme suit :
...
13. Dans la barre d’outils en bas de la page, cliquez sur **Appliquer les modifications**.
...
L’objectif de cette section est de tester la configuration de l’authentification unique Azure AD à l’aide du volet d’accès. Lorsque vous cliquez sur la vignette CloudPassage dans le volet d’accès, vous devez être connecté automatiquement à votre application CloudPassage.
