---
title: "Meilleures pratiques : Gestion des mots de passe Azure AD | Microsoft Docs"
...
## <a name="how-to-get-users-configured-for-password-reset"></a>Configuration des utilisateurs pour la réinitialisation de mot de passe
...
### <a name="what-makes-an-account-configured"></a>Configuration d’un compte
...
### <a name="ways-to-populate-authentication-data"></a>Méthodes permettant de remplir les données d’authentification
...
Vous pouvez également en savoir plus sur la façon dont [les données sont utilisées par la réinitialisation de mot de passe](active-directory-passwords-learn-more.md#what-data-is-used-by-password-reset) et dont [vous pouvez remplir les différents champs d’authentification avec PowerShell](active-directory-passwords-learn-more.md#how-to-access-password-reset-data-for-your-users).

## <a name="what-is-the-best-way-to-roll-out-password-reset-for-users"></a>Quelle est la meilleure façon de déployer la réinitialisation de mot de passe pour les utilisateurs ?
...
Lorsqu’ils cliquent sur **Vérifier maintenant**, les utilisateurs sont dirigés vers le **portail d’inscription à la réinitialisation de mot de passe** à la page [http://aka.ms/ssprsetup](http://aka.ms/ssprsetup), sur laquelle ils sont invités à s’inscrire.  L’inscription à l’aide de cette méthode peut être désactivée en cliquant sur le bouton **Annuler** ou en fermant la fenêtre, mais les utilisateurs reçoivent un rappel à chaque fois qu’ils se connectent s’ils ne se sont pas inscrits.
...
## <a name="sample-training-materials"></a>Exemples de supports de formation
