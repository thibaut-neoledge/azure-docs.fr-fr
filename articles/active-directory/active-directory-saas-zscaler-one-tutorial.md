...
- Vous pouvez autoriser les utilisateurs à être automatiquement connectés à Zscaler One (via l’authentification unique) avec leurs comptes Azure AD.
...
1. Ajouter Zscaler One à partir de la galerie
...
4. Dans la zone de recherche, tapez **Zscaler One**.
...
5. Dans le panneau de résultats, sélectionnez **Zscaler One**, puis cliquez sur le bouton **Ajouter** pour ajouter l’application.
...
Pour que l’authentification unique fonctionne, Azure AD doit connaître l’utilisateur Zscaler One correspondant à l’utilisateur Azure AD. En d’autres termes, il faut établir une relation entre l’utilisateur Azure AD et l’utilisateur Zscaler One qui lui est associé.

Dans Zscaler One, assignez la valeur de **nom d’utilisateur** dans Azure AD comme valeur de **nom d’utilisateur** pour établir la relation.
...
4. **[Création d’un utilisateur de test Zscaler One](#creating-a-zscaler-one-test-user)** pour avoir un équivalent de Britta Simon dans Zscaler One qui est lié à la représentation de l’utilisateur Azure AD.
...
Dans cette section, vous allez activer l’authentification unique Azure AD dans le portail Azure et configurer l’authentification unique dans votre application Zscaler One.

**Pour configurer l’authentification unique Azure AD avec Zscaler One, effectuez les étapes suivantes :**

1. Dans le portail Azure, dans la page d’intégration de l’application **Zscaler One**, cliquez sur **Authentification unique**.
...
3. Dans la section **Domaine et URL Zscaler One**, effectuez les étapes suivantes :
    ...
    > [!NOTE] 
    > Vous devez mettre à jour cette valeur avec l’URL de connexion réelle. Pour obtenir ces valeurs, contactez [l’équipe du support Zscaler One](https://www.zscaler.com/company/contact).
...
6. Pour ouvrir la fenêtre **Configurer l’authentification**, dans la section **Configuration de Zscaler One**, cliquez sur **Configurer Zscaler One**. Copiez l**’URL du service d’authentification unique SAML** à partir de la **section Référence rapide.**

    ![Configurer l’authentification unique](./media/active-directory-saas-zscaler-one-tutorial/tutorial_zscalerone_configure.png) 

7. Dans une autre fenêtre de navigateur web, connectez-vous à votre site d’entreprise Zscaler One en tant qu’administrateur.
...
Pour se connecter à Zscaler One, les utilisateurs d’Azure AD doivent être attribués dans Zscaler One. Dans le cas de Zscaler One, l’approvisionnement est une tâche manuelle.
...
3. Cliquez sur **User Management**.   
     ...
     ![Ajouter](./media/active-directory-saas-zscaler-one-tutorial/ic781036.png "Ajouter")
...
> [!NOTE]
> Vous pouvez utiliser tout autre outil ou n’importe quelle API de création de compte d’utilisateur fournis par Zscaler One pour approvisionner des comptes d’utilisateur Azure AD.
...
**Pour attribuer Britta Simon à Zscaler One, effectuez les étapes suivantes :**
...
En cliquant sur la vignette Zscaler One dans le Panneau d’accès, vous allez en principe être connecté automatiquement à votre application Zscaler One.
