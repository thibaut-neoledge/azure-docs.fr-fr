...
Si aucune approbation n’est requise, les utilisateurs reçoivent instantanément l’application ajoutée à leur volet d’accès Azure AD. Il s’agit de la procédure appropriée si l’application a été configurée pour [Attribution automatique d'utilisateurs](active-directory-saas-app-provisioning.md) ou a été configurée sur [le mode authentification unique par mot de passe « gérée par l’utilisateur »](active-directory-appssoaccess-whatis.md#password-based-single-sign-on) dans lequel l’utilisateur a déjà un compte d’utilisateur et connaît le mot de passe.
