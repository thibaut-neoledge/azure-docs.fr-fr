...
Si vous avez déterminé à partir de la tâche 1 que vous avez un fournisseur d’identité tiers, ou que vous allez en utiliser un pour fournir la fédération avec Azure AD, vous devez connaître les fonctions prises en charge suivantes : - Tout fournisseur SAML 2.0 conforme pour le profil SP-Lite peut prendre en charge l’authentification à Azure AD et aux applications associées - Prise en charge de l’authentification passive, ce qui facilite l’authentification à Outlook Web App, SPO, etc. - Les clients Exchange Online peuvent être pris en charge via le profil client amélioré (ECP) SAML 2.0
...
>[AZURE.NOTE]Pour consulter la liste la plus à jour, lisez l’article http://aka.ms/ssoproviders.
...
>[AZURE.NOTE]Pour les dernières informations concernant les fonctionnalités prises en charge de chaque outil, consultez l’article [Comparaison des outils d’intégration de répertoire](active-directory-aadconnect-get-started-tools-comparison.md).
...
>[AZURE.NOTE]Pour plus d’informations sur les différentes topologies locales et Azure AD avec la synchronisation Azure AD Connect, consultez l’article [Topologies pour Azure AD Connect](active-directory-aadconnect-topologies.md).
