<properties
	pageTitle="Modèle d’application v2.0 - Types d’applications | Microsoft Azure"
	description="Les types d’applications et de scénarios pris en charge par la version préliminaire publique v2.0 du modèle d’application Azure AD."
...
1. L’application dirige l’utilisateur vers le point de terminaison v2.0 pour la connexion.
2. L’utilisateur s’authentifie, saisit son nom d’utilisateur et mot de passe ou toute autre information d’identification.
3. L’utilisateur autorise l’application à agir en son nom, en octroyant des autorisations aux requêtes de l’application.
...
Chacune de ces sept étapes peut varier légèrement en fonction du type d’application conçue. Nous mettons à votre disposition des bibliothèques de ressources qui vous accompagnent dans ce processus. Vous y trouverez des informations sur les [autorisations, consentements et étendues](active-directory-v2-scopes.md),et pourrez consulter des exemples concrets.

## Applications Web
Pour les applications Web (.NET, PHP, Java, Ruby, Python, Node, etc) accessibles via un navigateur, le modèle d’application v2.0 prend en charge la connexion utilisateur à l’aide d’[OpenID Connect](active-directory-v2-protocols.md#openid-connect-sign-in-flow). Dans OpenID Connect, l’application Web reçoit un élément `id_token`, un jeton de sécurité qui vérifie l’identité de l’utilisateur et vérifie les informations le concernant sous la forme de revendications :
...
En plus de la connexion simple, une application de serveur Web peut également nécessiter l’accès à d’autres services Web, comme une API REST. Dans ce cas, l’application de serveur Web peut s’engager dans un flux OpenID Connect et OAuth 2.0, à l’aide du [flux de code d’autorisation OAuth 2.0](active-directory-v2-protocols.md#oauth2-authorization-code-flow). Ce scénario est abordé ci-dessous dans la [section API Web](#web-apis), ainsi que dans notre [rubrique de Prise en main des applications Web-API Web](active-directory-v2-devquickstarts-webapp-webapi-dotnet.md).

## API Web
Vous pouvez utiliser le modèle d’application v2.0 pour sécuriser également les services Web, comme l’API Web Restful de votre application. En lieu et place des jetons id\_token et des cookies de session, les API Web utilisent les jetons d’accès OAuth 2.0 pour sécuriser les données et authentifier les requêtes entrantes. L’appelant d’une API Web ajoute un jeton d’accès dans l’en-tête d’autorisation d’une requête HTTP :
...
Une API Web peut recevoir des jetons d’accès de tous types d’applications, notamment des applications de serveur Web, des applications de bureau et mobiles, des applications de page unique, des démons côté serveur, et même d’autres API Web. Par exemple, examinons le flux complet d’une application de serveur Web appelant une API Web.
