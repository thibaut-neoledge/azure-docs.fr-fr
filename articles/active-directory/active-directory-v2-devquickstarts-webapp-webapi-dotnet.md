...
- connecte les utilisateurs à l’aide d’OpenID Connect, avec l’aide de l’intergiciel OWIN de Microsoft ;
- récupère les jetons d’accès OAuth 2.0 pour une API Web utilisant ADAL ;
- crée, lit et supprime des éléments sur la liste des tâches d’un utilisateur, qui est stockée sur l’API Web et sécurisée par OAuth 2.0.

Ce didacticiel s’intéresse principalement à la récupération et à l’utilisation des jetons d’accès dans une application Web, dont la description complète est disponible [ici](active-directory-v2-flows.md#web-apps). Afin de vous renseigner sur la configuration requise, apprenez dans un premier temps comment [ajouter une connexion de base à une application Web](active-directory-v2-devquickstarts-dotnet-web.md) ou comment [sécuriser de manière appropriée une application Web](active-directory-v2-devquickstarts-dotnet-api.md).

La procédure de base d’appel d’une API Web de liste de tâche est la suivante :

1. Inscription d’une application
2. Connexion de l’utilisateur à l’API Web à l’aide d’OpenID Connect
3. Utilisation d’ADAL afin de récupérer un jeton d’accès lors de la connexion de l’utilisateur
4. Appel de l’API Web de la liste des tâches à l’aide d’un jeton d’accès

Le code associé à ce didacticiel est stocké [sur GitHub](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet).

Pour suivre la procédure, vous pouvez [télécharger la structure de l’application au format .zip](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet/archive/skeleton.zip) ou la cloner :
...
-	Ensuite, ajoutez au projet `TodoList-WebApp` les packages NuGet de l’intergiciel OWIN à l’aide de la console du gestionnaire de package.
