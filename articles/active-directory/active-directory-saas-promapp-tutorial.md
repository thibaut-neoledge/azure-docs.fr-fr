...
L’objectif de ce didacticiel est de vous montrer comment intégrer Promapp dans Azure Active Directory (Azure AD). L’intégration de Promapp dans Azure AD vous offre les avantages suivants :
...
4. Sur la page **Configurer l’authentification unique sur Promapp**, procédez comme suit :
...
L’objectif de cette section est de tester la configuration de l’authentification unique Azure AD à l’aide du volet d’accès. Lorsque vous cliquez sur la vignette Promapp dans le volet d’accès, vous devez être connecté automatiquement à votre application Promapp.
