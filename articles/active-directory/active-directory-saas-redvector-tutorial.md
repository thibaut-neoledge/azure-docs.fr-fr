...
4. Dans la zone de recherche, tapez **RedVector**.

    ![Création d’un utilisateur de test Azure AD](./media/active-directory-saas-redvector-tutorial/tutorial_redvector_search.png)

5. Dans le panneau de résultats, sélectionnez **RedVector**, puis cliquez sur le bouton **Ajouter** pour ajouter l’application.
...
3. **[Création d’un utilisateur de test RedVector](#creating-a-redvector-test-user)** pour obtenir un équivalent de Britta Simon dans RedVector lié à la représentation Azure AD associée.
...
3. Dans la section **Domaine et URL RedVector**, effectuez les étapes suivantes :
...
6. Dans la section **Configuration de RedVector**, cliquez sur **Configurer RedVector** pour ouvrir la fenêtre **Configurer l’authentification**. Copiez l**’URL du service d’authentification unique SAML** à partir de la **section Référence rapide.**

    ![Configurer l’authentification unique](./media/active-directory-saas-redvector-tutorial/tutorial_redvector_configure.png) 

7. Pour configurer l’authentification unique côté **RedVector**, vous devez envoyer le **Certificat (Base64)** téléchargé et **l’URL du service d’authentification unique SAML** à [l’équipe de support technique RedVector](mailto:sso@redvector.com). Celle-ci configure ensuite ce paramètre pour que la connexion SSO SAML soit définie correctement des deux côtés.
