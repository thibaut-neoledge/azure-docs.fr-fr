...
| Paramètre | Description |
| ----------------------- | ------------------------------- |
| id\_token | Jeton `id_token` que l’application a demandé. Vous pouvez utiliser ce jeton `id_token` pour vérifier l’identité de l’utilisateur et démarrer une session avec lui. |
...
Vous pouvez décider de valider l’élément `id_token` dans le code client, mais une pratique courante consiste à envoyer l’élément `id_token` vers un serveur principal, afin d’y appliquer la validation. Une fois que vous avez validé la signature du jeton `id_token`, vous devez vérifier quelques revendications.
