...
Azure Active Directory peut configurer automatiquement les utilisateurs et les groupes sur une application ou un magasin d’identités avec en façade un service web avec l’interface définie dans [Spécification du protocole SCIM](https://tools.ietf.org/html/draft-ietf-scim-api-19). Azure Active Directory peut envoyer des demandes destinées à créer, modifier et supprimer des utilisateurs et des groupes sur ce service web, qui peut alors transformer ces demandes en opérations sur le magasin d’identités cibles.

![][1] *Figure : configuration d’Azure Active Directory pour une banque d’identités arbitraires via un service web*

Cette fonctionnalité peut être utilisée conjointement avec la fonctionnalité « apportez votre propre application » dans Azure AD afin d’activer l’authentification unique et la configuration utilisateur automatique pour virtuellement n’importe quelle application qui fournit une API de configuration.

Fonctionnement :

1.	Azure AD fournit une bibliothèque d’infrastructure de langages commune nommée [Microsoft.SystemForCrossDomainIdentityManagement](https://www.nuget.org/packages/Microsoft.SystemForCrossDomainIdentityManagement/). Les développeurs et les intégrateurs système peuvent utiliser cette bibliothèque pour créer et déployer un point de terminaison de service web SCIM capable de connecter Azure AD au magasin d’identités d’application d’une application.
...
Pour faciliter ce processus, un ensemble d’[exemples de code](https://github.com/Azure/AzureAD-BYOA-Provisioning-Samples/tree/master) est fourni et crée un point de terminaison de service web SCIM et affiche l’approvisionnement de configuration automatique. Exemple : un fournisseur qui gère un fichier avec des lignes de valeurs séparées par des virgules représentant des utilisateurs et des groupes. L’autre exemple est un fournisseur qui opère sur le service de gestion des identités et des accès des services web Amazon.


###Configuration requise
* Visual Studio 2013 ou une version ultérieure
* [Kit de développement logiciel (SDK) Azure pour .NET](https://azure.microsoft.com/fr-FR/downloads/)
...
* [Dans un abonnement Azure avec une version d’évaluation ou sous licence d’Azure AD Premium](https://azure.microsoft.com/fr-FR/services/active-directory/)
...
4.	Sélectionnez **Outils > Library Package Manager > Console du Gestionnaire de Package**, puis exécutez les commandes ci-dessous afin que le projet FileProvisioningAgent résolve les références de la solution :
...
6.	Lancez l’application d’invite de commandes de Windows (en tant qu’administrateur) et utilisez la commande **cd** pour changer le répertoire en votre dossier **\\AzureAD-BYOA-Provisioning-Samples\\ProvisioningAgent\\bin\\Debug**.
7.	Exécutez la commande ci-dessous en remplaçant <ip-address> par l’IP ou le nom de domaine de la machine Windows.

    FileAgnt.exe http://<ip-address>:9000 TargetFile.csv

8.	Dans Windows, sous **Paramètres Windows > Paramètres Internet & réseau**, sélectionnez le **pare-feu Windows > Paramètres avancés**, et créez une **règle de trafic entrant** qui autorise l’accès entrant au port 9000.
...
3.	Sélectionnez l’onglet **Personnalisé** sur la gauche, entrez un nom tel qu’« Application de Test SCIM » et cliquez sur l’icône de coche pour créer un objet d’application. Notez que l’objet d’application créé doit représenter l’application cible de que vous devez configurer et sur laquelle mettre en œuvre l’authentification unique. Il ne s’agit pas d’un simple point de terminaison SCIM.

![][2]

4.	Dans l’écran qui en résulte, sélectionnez le deuxième bouton de configuration Configurer le compte .
5.	Dans la boîte de dialogue, saisissez l’URL côté Internet et le port de votre point de terminaison SCIM. Il devrait ressembler à ce qui suit http://testmachine.contoso.com:9000 ou http://<ip-address>:9000/, où <ip-address> est l’adresse IP exposée à Internet.  
6.	Cliquez sur **Suivant**, puis sur le bouton **Démarrer le test** pour qu’Azure Active Directory essaie de se connecter au point de terminaison SCI. Si les tentatives échouent, les informations de diagnostic seront affichées.  
...
9.	Une fois les utilisateurs et les groupes attribués, cliquez sur l’onglet **Configurer** près du haut de l’écran.
...
**2 :** [les gestionnaires de routeur Express](http://expressjs.com/guide/routing.html) sont disponibles pour les objets de requête node.js d’analyse représentant des appels (comme défini par la spécification SCIM) envoyés à un service web node.js.

##Création d’un point de terminaison SCIM personnalisé

Grâce aux bibliothèques décrites ci-dessus, les développeurs utilisant ces bibliothèques peuvent héberger leurs services dans un assembly Common Language Infrastructure exécutable ou au sein des services Internet (IIS). Voici des exemples de code pour l’hébergement d’un service dans un assembly exécutable, à l’adresse http://localhost:9000 :
...
Les ressources du groupe sont identifiées par l’identificateur de schéma http://schemas.microsoft.com/2006/11/ResourceManagement/ADSCIM/Group. Le tableau 2 ci-dessous montre le mappage par défaut des attributs de groupes dans Azure Active Directory sur les attributs de http://schemas.microsoft.com/2006/11/ResourceManagement/ADSCIM/Group ressources.
...
Figure ![][4] : séquence d’approvisionnement et d’annulation d’approvisionnement utilisateur

**1:** azure Active Directory interroge le service d’un utilisateur avec une valeur d’attribut externalId correspondant à la valeur d’attribut mailNickname d’un utilisateur dans Active Directory de Azure. La requête sera exprimée dans le protocole Http (Hypertext Transfer Protocol) comme suit, jyoung étant un exemple de mailNickname d’utilisateur dans Azure Active Directory :
...
**3 :** pour mettre à jour un utilisateur dont on sait qu’il existe dans un magasin d’identités avec SCIM en façade, Azure Active Directory continue en demandant au service l’état actuel de cet utilisateur avec une demande du type qui suit :
...
**6 :** pour annuler l’approvisionnement d’un utilisateur à partir d’un magasin d’identités avec un service SCIM en façade, Azure Active Directory envoie une demande comme celle-ci :
...
Figure ![][5] : séquence d’approvisionnement et d’annulation d’approvisionnement utilisateur
