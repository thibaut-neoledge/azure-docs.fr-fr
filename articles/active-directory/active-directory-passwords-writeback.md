...
description: "Utilisation d’Azure AD et Azure AD Connect pour la réécriture du mot de passe sur l’annuaire local"
...
1. Nous vérifions le type de mot de passe de l’utilisateur.
    * Si nous constatons que le mot de passe est géré localement
        * Nous vérifions que le service de réécriture fonctionne et est en cours d’exécution. Si c’est le cas, nous laissons l’utilisateur poursuivre
        * Si le service de réécriture n’est pas disponible, nous indiquons à l’utilisateur que son mot de passe ne peut pas être réinitialisé maintenant
...
4. Lorsqu’il clique sur Envoyer, nous chiffrons le mot de passe en texte brut avec une clé publique qui a été créée durant le processus d’installation de la réécriture.
...
DirSync et Azure AD Sync ne sont plus pris en charge pour l’activation de la réécriture du mot de passe. L’article [Mettre à niveau depuis DirSync et Azure AD Sync](connect/active-directory-aadconnect-dirsync-deprecated.md) contient des informations supplémentaires pour faciliter votre transition.
...
1. Pour configurer et activer le journal de la réécriture du mot de passe sur votre serveur Azure AD Connect et démarrer l’Assistant de configuration **Azure AD Connect**.
...
6. Dans l’écran des fonctionnalités facultatives, cochez la case en regard de **Réécriture du mot de passe**, puis cliquez sur **Suivant**.
   ![Activation de la réécriture du mot de passe dans Azure AD Connect][Writeback]
...
Pour plus d’informations concernant les licences, consultez la rubrique [Licences requises pour la réécriture du mot de passe](active-directory-passwords-licensing.md#licenses-required-for-password-writeback) ou les sites suivants
...
La réécriture du mot de passe fonctionne pour les types de mots de passe utilisateur suivants :

* **Utilisateurs cloud uniquement** : la réécriture du mot de passe ne s’applique pas dans cette situation, faute de mot de passe local
...
* **Étape 1 - Chiffrement du mot de passe avec la clé RSA de 2 048 bits** : lorsqu’un utilisateur envoie un mot de passe en vue de la réécriture au niveau local, celui-ci est préalablement chiffré avec une clé RSA de 2 048 bits.
...
*  **Renouvellement automatique des clés tous les 6 mois** : tous les 6 mois de façon automatique ou chaque fois que la réécriture du mot de passe est désactivée/réactivée sur Azure AD Connect, nous renouvelons toutes ces clés afin d’assurer la sécurité et la protection maximales du service.

### <a name="password-writeback-bandwidth-usage"></a>Utilisation de la bande passante de la réécriture du mot de passe

La réécriture du mot de passe est un service de bande passante très basse qui renvoie des demandes à l’agent local uniquement dans les circonstances suivantes :
...
La taille de chacun des messages décrits ci-dessus est généralement inférieure à 1 ko, ce qui signifie que, même sous des charges extrêmes, le service de la réécriture du mot de passe lui-même utilise quelques kilobits par seconde de bande passante. Étant donné que chaque message est envoyé en temps réel, uniquement lorsqu’une opération de mise à jour du mot de passe le requiert, et étant donné que le message est très petit, l’utilisation de la bande passante de la fonctionnalité de réécriture est effectivement trop petite pour produire un impact mesurable réel.
