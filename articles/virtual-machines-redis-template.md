...
Redis est une solution de cache et stockage clé-valeur open-source dont les clés peuvent contenir des structures de données, telles que des chaînes de caractères, des hachages, des listes, des groupes et des groupes triés.  Redis prend en charge un ensemble d'opérations atomiques pour ces types de données. Avec la version de Redis 3.0, Redis Cluster est désormais disponible dans la version stable la plus récente de Redis. Cluster Redis est une implémentation distribuée de Redis dans laquelle les données sont automatiquement partitionnées sur plusieurs nœuds de Redis, avec la possibilité de continuer à fonctionner lorsqu'un sous-ensemble de nœuds rencontre des erreurs.

Le Cache Redis Microsoft Azure est un service de cache Redis dédié, géré par Microsoft. Mais tous les clients Microsoft Azure ne souhaitent pas utiliser le Cache Redis Azure. Certains souhaitent que leur Cache Redis se trouve derrière un sous-réseau dans leurs propres déploiements Azure, tandis que d'autres préfèrent héberger leurs propres serveurs Redis sur des machines virtuelles Linux afin d'être en mesure de tirer pleinement parti de toutes les fonctionnalités de Redis.

Ce didacticiel présente un exemple de modèle Azure Resource Manager (ARM) pour déployer un Cluster Redis sur des machines virtuelles Ubuntu dans un sous-réseau dans un groupe de ressources dans Microsoft Azure. En plus du Cluster Redis 3.0, ce modèle prend également en charge le déploiement de Redis 2.8 avec Redis Sentinel. Notez que ce didacticiel se concentre uniquement sur l'implémentation de Cluster Redis 3.0.

Le cluster Redis est créé derrière un sous-réseau, il n'existe aucun accès IP publique au cluster Redis. Une « jump box » peut être déployée dans le cadre du déploiement. Cette « jump box » est une machine virtuelle Ubuntu également déployée dans le sous-réseau, mais elle *expose* une adresse IP publique avec un port SSH sur lequel vous pouvez exécuter SSH. Vous pouvez ensuite exécuter SSH pour toutes les machines virtuelles Redis dans le sous-réseau depuis la « jump box ».

Ce modèle utilise un concept de « taille de t-shirt » afin de spécifier une configuration « small », « medium » ou « large » du cluster Redis. Lorsque la langue du modèle ARM prend en charge un dimensionnement de modèle plus dynamique, le paramètre peut être modifié pour spécifier le nombre de nœuds de Cluster Redis principaux, les nœuds subordonnés, la taille des machines virtuelles, etc. Actuellement, vous pouvez voir la taille des machines virtuelles et le nombre de maîtres et esclaves définis dans le fichier **azuredeploy.json** dans les variables `tshirtSizeSmall`, `tshirtSizeMedium`, et `tshirtSizeLarge`.
...
## Déployer un cluster Redis avec un modèle Resource Manager
...
Lorsque vous avez terminé, recherchez le dossier **redis-high-availability** dans votre répertoire C:\\Azure\\Templates.

### Étape 2 : (en option) comprendre les paramètres du modèle 

Lorsque vous créez un cluster Redis avec un modèle, vous devez spécifier un ensemble de paramètres de configuration. Pour afficher les paramètres que vous devez spécifier pour le modèle dans un fichier JSON local avant d'exécuter la commande pour créer le cluster Redis, ouvrez le fichier JSON dans un outil ou un éditeur de texte de votre choix.
...
Chaque paramètre contient des détails, tels que le type de données et les valeurs autorisées. Ceci permet la validation des paramètres transmis pendant l'exécution du modèle dans un mode interactif (par exemple, PowerShell ou l'interface de ligne de commande Azure) dans une interface utilisateur à détection automatique, qui peut être construite de manière dynamique par l'analyse de la liste des paramètres nécessaires et leurs descriptions.

### Étape 3-a : déployer un cluster Redis avec un modèle à l'aide de PowerShell
...
Pendant et après le déploiement, vous pouvez vérifier toutes les requêtes effectuées durant le provisionnement, notamment toutes les erreurs survenues.
...
- Sélectionnez le groupe de ressources que vous venez de créer, le volet « Groupe de ressources » s'affiche.
- Dans la section Analyse, sélectionnez le graphique à barres « événements ». Ceci affichera les événements pour votre déploiement.
...
### Étape 3-b : déployer un cluster Redis avec un modèle à l'aide de l'interface de ligne de commande Azure
...
Si vous n'avez pas téléchargé une copie du fichier de modèle, spécifiez un dossier local comme emplacement de fichier et créez-le (par exemple, C:\\Azure\\Templates\\RedisCluster). Indiquez le nom du dossier, puis exécutez les commandes suivantes.
...
Ouvrez le modèle azuredeploy.json dans un éditeur de texte ou dans un outil de votre choix. La section suivante décrit la structure du fichier de modèle et l'objectif de chaque section. Vous pouvez également voir le contenu de ce modèle dans votre navigateur [ici](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/redis-high-availability/azuredeploy.json).
...
Si vous souhaitez personnaliser la taille du déploiement du Cluster Redis, vous pouvez modifier les propriétés des variables `tshirtSizeSmall`, `tshirtSizeMedium` et `tshirtSizeLarge` dans le modèle **azuredeploy.json**.
...
Ce premier exemple présente clairement la façon dont **azuredeploy.json** est organisé comme mécanisme d'orchestration, pour appeler un certain nombre d'autres fichiers de modèle, chacun d'eux étant responsable d'une partie des activités de déploiement requises.

Vous pouvez remarquer en particulier les modèles liés suivants qui sont utilisés pour ce déploiement :
...
- **node-resources.json** : déploie toutes les machines virtuelles de nœuds de Cluster Redis et les ressources connectées (par exemple, des cartes réseau, des adresses IP privées, etc.). Ce modèle déploie également des extensions de machine virtuelle (scripts personnalisés pour Linux) et appelle un script d'interpréteur de commandes pour installer et configurer physiquement Redis sur chaque nœud. Le script à appeler est transmis à ce modèle dans le paramètre `machineSettings` de la propriété `commandToExecute`. Tous les nœuds du cluster Redis sauf un peuvent être déployés et inclus dans un script en parallèle. Un nœud doit être enregistré jusqu'à la fin, car la configuration du cluster Redis ne peut être exécutée que sur un nœud, et elle ne doit être effectuée qu'une fois que tous les nœuds exécutent le serveur Redis. C'est pourquoi le script à exécuter est transmis à ce modèle. Le dernier nœud doit exécuter un script légèrement différent qui n'installera pas uniquement le serveur Redis, mais configurera également le cluster Redis.
...
Comme mentionné précédemment, le dernier nœud doit attendre son provisionnement jusqu'à ce que tous les autres nœuds du cluster Redis aient été configurés avec le serveur Redis s'exécutant sur ceux-ci. Pour cela, il suffit d'avoir dans **azuredeploy.json** une ressource nommée `lastnode-resources` qui dépend de la boucle `copy` nommée `memberNodesLoop` de l'extrait de code de modèle ci-dessus. Une fois la `memberNodesLoop` terminée, les `lastnode-resources` peuvent être provisionnées :
...
Vous pouvez voir que cette ressource dépend de la ressource de machine virtuelle déjà déployée (Microsoft.Compute/virtualMachines/vmMember<X>, avec <X> comme paramètre « machineSettings.machineIndex », qui est l'index de la machine virtuelle qui a été transmise à ce script à l'aide de la fonction « copyindex() ».
...
- définir votre fichier de modèle principal comme point d'orchestration central pour toutes les activités de déploiement spécifiques, en exploitant les liaisons de modèles pour appeler les exécutions de modèles secondaires ;
- créer un fichier de modèle spécifique déployant toutes les ressources partagées entre toutes les autres tâches du déploiement (par exemple, les comptes de stockage, la configuration du réseau virtuel, etc.). Cette méthode peut être régulièrement réutilisée dans des déploiements ayant des exigences similaires en termes d'infrastructure commune ;
...
Pour plus d'informations, consultez [Langue du modèle Azure Resource Manager](https://msdn.microsoft.com/library/azure/dn835138.aspx).
