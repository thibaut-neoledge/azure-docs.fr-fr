...
Analyse des données Twitter avec HDInsight
==========================================

Découvrez comment analyser les données Twitter en utilisant Hive avec HDInsight.
...
-   Un **cluster Azure HDInsight**. Pour obtenir des instructions sur l'approvisionnement des clusters, consultez les rubriques [Prise en main de HDInsight](../hdinsight-get-started/) ou [Approvisionnement de clusters HDInsight](../hdinsight-provision-clusters/). Vous aurez besoin des données suivantes pour suivre ce didacticiel :
  ...
  <tr data-morhtml="true"><td data-morhtml="true">Nom du cluster HDInsight</td><td data-morhtml="true">$clusterName</td><td data-morhtml="true"></td><td data-morhtml="true">Nom du cluster HDInsight.</td></tr>
...
> Le chemin d'accès WASB est le chemin d'accès virtuel. Pour plus d'informations, consultez la rubrique [Utilisation du stockage d'objets blob Azure avec HDInsight](../hdinsight-use-blob-storage/).
...
<table data-morhtml="true" border="1">
<tr data-morhtml="true"><th data-morhtml="true">Fichiers</th><th data-morhtml="true">Description</th></tr>
<tr data-morhtml="true"><td data-morhtml="true">/tutorials/twitter/data/tweets.txt</td><td data-morhtml="true">Donn&eacute;es source pour la t&acirc;che Hive.</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">/tutorials/twitter/output</td><td data-morhtml="true">Donn&eacute;es de sortie pour la t&acirc;che Hive. Le nom de fichier de sortie de la t&acirc;che Hive du fichier par d&eacute;faut est <strong data-morhtml="true">000000_0</strong>. </td></tr>
<tr data-morhtml="true"><td data-morhtml="true">tutorials/twitter/twitter.hql</td><td data-morhtml="true">Fichier de script HiveQL.</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">/tutorials/twitter/jobstatus</td><td data-morhtml="true">&Eacute;tat de la t&acirc;che Hadoop.</td></tr>
</table>

Obtention du flux Twitter
-------------------------

Dans ce didacticiel, vous allez utiliser les [API de diffusion Twitter](https://dev.twitter.com/docs/streaming-apis). L'API de diffusion Twitter spécifique que vous allez utiliser est [statuses/filter](https://dev.twitter.com/docs/api/1.1/post/statuses/filter).
...
Twitter utilise OAuth pour fournir un accès autorisé à son API. OAuth est un protocole d'authentification qui permet aux utilisateurs d'approuver l'application afin qu'elle agisse à leur place sans partager leur mot de passe. Pour plus d'informations, consultez la page [oauth.net](http://oauth.net/) ou l'excellent [Guide du débutant sur OAuth](http://hueniverse.com/oauth/) de Hueniverse.
...
3.  Entrez **Name**, **Description**, **Website**. Vous pouvez créer une URL pour le champ du site Web. Le tableau suivant affiche quelques exemples de valeurs à utiliser :
    ...
    <table data-morhtml="true" border="1">
 <tr data-morhtml="true"><th data-morhtml="true">Champ</th><th data-morhtml="true">Valeur</th></tr>
 <tr data-morhtml="true"><td data-morhtml="true">Nom</td><td data-morhtml="true">MyHDInsightApp</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">Description</td><td data-morhtml="true">MyHDInsightApp</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">Site Web</td><td data-morhtml="true">http://www.myhdinsightapp.com</td></tr>
 </table>
...
Dans ce didacticiel, vous allez utiliser PowerShell pour effectuer un appel de service Web. L'autre outil connu permettant d'effectuer des appels de service Web est [*Curl*](http://curl.haxx.se). Vous pouvez le télécharger [ici](http://curl.haxx.se/download.html).
...
3.  Définissez les neuf premières variables du script :

   <table data-morhtml="true" border="1">
 <tr data-morhtml="true"><th data-morhtml="true">Variable</th><th data-morhtml="true">Description</th></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$storageAccountName</td><td data-morhtml="true">Compte de stockage Azure utilis&eacute; en tant que syst&egrave;me de fichiers du cluster HDInsight par d&eacute;faut. Il s'agit du compte de stockage sp&eacute;cifi&eacute; lors de l'approvisionnement. Consultez la rubrique <a data-morhtml="true" href="#prerequisites">Conditions pr&eacute;alables</a>.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$containerName</td><td data-morhtml="true">Conteneur d'objets blob utilis&eacute; en tant que syst&egrave;me de fichiers du cluster HDInsight par d&eacute;faut. Il s'agit du conteneur sp&eacute;cifi&eacute; lors de l'approvisionnement. Consultez la rubrique <a data-morhtml="true" href="#prerequisites">Conditions pr&eacute;alables</a>.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$destBlobName</td><td data-morhtml="true">Nom de l'objet blob de sortie.  La valeur par d&eacute;faut est <strong data-morhtml="true">tutorials/twitter/data/tweets.txt</strong>. Si vous modifiez la valeur par d&eacute;faut, vous devez mettre &agrave; jour les scripts PowerShell en cons&eacute;quence.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$trackString</td><td data-morhtml="true">Le service Web renvoie les tweets li&eacute;s &agrave; ces mots cl&eacute;s.  La valeur par d&eacute;faut est <strong data-morhtml="true">Azure, WindowsAzure, Cloud, HDInsight</strong>. Si vous modifiez la valeur par d&eacute;faut, vous devez mettre &agrave; jour les scripts PowerShell en cons&eacute;quence.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$lineMax</td><td data-morhtml="true">La valeur d&eacute;termine le nombre de tweets lus par le script. La lecture de 100&nbsp;tweets prend environ trois minutes. Vous pouvez d&eacute;finir un nombre plus important, mais le t&eacute;l&eacute;chargement prendra plus de temps.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$oauth_consumer_key</td><td data-morhtml="true"><strong data-morhtml="true">Cl&eacute;</strong> que vous avez not&eacute;e auparavant en cr&eacute;ant l'application Twitter.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$oauth_consumer_secret</td><td data-morhtml="true"><strong data-morhtml="true">Secret</strong> que vous avez &eacute;crit auparavant pour l'application Twitter.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$oauth_token</td><td data-morhtml="true"><strong data-morhtml="true">Jeton d'acc&egrave;s</strong> que vous avez &eacute;crit auparavant pour l'application Twitter.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$oauth_token_secret</td><td data-morhtml="true"><strong data-morhtml="true">Secret de jeton d'acc&egrave;s</strong> que vous avez &eacute;crit auparavant pour l'application Twitter.</td></tr>
 </table>
...
2.  **Création de la table tweets\_raw Hive**. Cette table structurée Hive temporaire conserve les données en vue d'un traitement ETL ultérieur. Pour plus d'informations sur la partition, consultez la page [Didacticiel Hive](https://cwiki.apache.org/confluence/display/Hive/Tutorial).
...
4.  **Suppression de la table tweets** au cas où la table existe.
5.  **Création de la table tweets**. Avant de pouvoir effectuer une requête sur les jeux de données Tweets à l'aide de Hive, vous devez exécuter un autre processus ETL. Ce dernier définit un schéma de table plus détaillé pour les données que vous avez stockées dans la table « twitter\_raw ».
6.  **Insertion de la table overwrite**. Ce script Hive complexe démarre un ensemble de tâches MapReduce sur le cluster Hadoop. Selon votre jeu de données et la taille de votre cluster, cela peut prendre environ 10 minutes.
7.  **Insertion du répertoire overwrite**. Exécutez une requête et sortez le jeu de données dans un fichier. Cette requête renvoie une liste d'utilisateurs Twitter qui ont envoyé la plupart des tweets contenant le mot « Azure ».
...
3.  Définissez les deux premières variables du script :
 ...
 <tr data-morhtml="true"><td data-morhtml="true">$storageAccountName</td><td data-morhtml="true">Compte de stockage Azure utilis&eacute; en tant que syst&egrave;me de fichiers du cluster HDInsight par d&eacute;faut. Il s'agit du compte de stockage sp&eacute;cifi&eacute; lors de l'approvisionnement. Consultez la rubrique <a data-morhtml="true" href="#prerequisites">Conditions pr&eacute;alables</a>.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$containerName</td><td data-morhtml="true">Conteneur d'objets blob utilis&eacute; en tant que syst&egrave;me de fichiers du cluster HDInsight par d&eacute;faut. Il s'agit du conteneur sp&eacute;cifi&eacute; lors de l'approvisionnement. Consultez la rubrique <a data-morhtml="true" href="#prerequisites">Conditions pr&eacute;alables</a>.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sourceDataPath</td><td data-morhtml="true">Emplacement WASB o&ugrave; les requ&ecirc;tes Hive lisent les donn&eacute;es. Vous n'avez pas besoin de modifier cette variable.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$outputPath</td><td data-morhtml="true">Emplacement WASB o&ugrave; les requ&ecirc;tes Hive sortent les r&eacute;sultats. Vous n'avez pas besoin de modifier cette variable.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$hqlScriptFile</td><td data-morhtml="true">Emplacement et nom du fichier de script HiveQL.</td></tr>
 </table>

4.  Appuyez sur **F5** pour exécuter le script. Si vous êtes confronté à un problème, vous pouvez, pour le contourner, sélectionner toutes les lignes et appuyer ensuite sur **F8**.
5.  Le message « Complete! » doit normalement s'afficher à la fin de la sortie. S'il y a un message d'erreur, il s'affiche en rouge.

Dans le cadre d'une procédure de validation, vous pouvez vérifier le fichier de sortie, **/tutorials/twitter/twitter.hql**, sur votre stockage d'objets blob Azure à l'aide d'un explorateur de stockage Azure ou d'Azure PowerShell. Pour obtenir un exemple de script PowerShell pour répertorier les fichiers, consultez la rubrique [Utilisation du stockage d'objets blob avec HDInsight](../hdinsight-use-blob-storage/#powershell).

Traitement des données à l'aide de Hive
---------------------------------------
...
Exécutez le script PowerShell suivant pour vérifier la sortie de la tâche Hive. Vous devez définir les deux premières variables.
    ...
    Write-Host "Set the variables ... " -ForegroundColor Green
    $storageAccountName = "<WindowsAzureStorageAccountName>"   # Le nom du compte de stockage utilisé pour le système de fichiers par défaut spécifié lors de l'approvisionnement.
    $containerName = "<BlobStorageContainerName>"  # Le conteneur du système de fichiers par défaut a le même nom que le cluster.
    $blob = "tutorials/twitter/output/000000_0" # Le nom de l'objet blob à télécharger.
...
Une fois que les résultats d'analyse ont été placés sur WASB, vous pouvez exporter les données dans la base de données Azure SQL/SQL Server, exporter les données dans Excel à l'aide de Power Query, ou connecter votre application aux données à l'aide du pilote ODBC Hive. Pour plus d'informations, consultez les pages [Utilisation de Sqoop avec HDInsight](../hdinsight-use-sqoop/) ,[Analyse des données sur les retards de vol avec HDInsight](../hdinsight-analyze-flight-delay-data/), [Connexion d'Excel à HDInsight à l'aide de Power Query](../hdinsight-connect-excel-power-query/) et [Connexion d'Excel à HDInsight à l'aide du pilote ODBC Microsoft Hive](../hdinsight-connect-excel-hive-ODBC-driver/).
