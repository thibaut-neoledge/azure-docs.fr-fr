...
HDInsight élimine ce point de défaillance unique avec l'ajout d'un nœud principal secondaire (Head Node1). Les nœuds [ZooKeeper][] (ZK) ont été ajoutés et sont utilisés pour le choix de l'instance responsable des nœuds principaux et pour s'assurer que les nœuds de travail et les passerelles (GW) savent à quel moment doit se faire le basculement vers le nœud principal secondaire (Head Node1) lorsque le nœud principal actif (HeadNode0) devient inactif.
...
La création et l'approvisionnement d'un cluster avec PowerShell sont documentés sur la page [Administration de HDInsight à l'aide de PowerShell][]. La configuration d'un nœud principal de très grande taille nécessite l'ajout du paramètre `-HeadNodeVMSize ExtraLarge` à la cmdlet `New-AzureHDInsightcluster` utilisée dans ce code.
