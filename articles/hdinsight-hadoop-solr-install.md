<properties 
	pageTitle="Utilisation d'une action de script dans HDInsight pour installer Solr sur un cluster Hadoop| Azure" 
...
- [Présentation de Solr](#whatis)
- [Comment installer Solr ?](#install)
- [Comment utiliser Solr dans HDInsight ?](#usesolr)
- [Installation de Solr sur des clusters HDInsight Hadoop à l'aide de PowerShell](#usingPS)
- [Installation de Solr sur des clusters HDInsight Hadoop à l'aide du Kit de développement logiciel (SDK) .NET](#usingSDK) 
...
2. Sur la page **Actions de script** de l'Assistant, cliquez sur **ajouter l'action de script** pour fournir des informations sur l'action de script, comme illustré ci-dessous :
	...
	<table border='1'>
		...
		<tr><td>URI de script</td>
			<td>Indiquez l'URI vers le script qui est appelé pour personnaliser le cluster. Par exemple, <i>https://hdiconfigactions.blob.core.windows.net/solrconfigactionv01/solr-installer-v01.ps1</i></td></tr>
		...
		<tr><td>Paramètres</td>
			<td>Indiquez les paramètres, s'ils sont requis par le script. Le script d'installation de Solr ne nécessite aucun paramètre. Vous pouvez donc laisser ce champ vide.</td></tr>
	</table>	
...
Vous pouvez également utiliser le script pour installer Solr sur HDInsight à l'aide de PowerShell ou du Kit de développement (SDK) HDInsight .NET. Vous trouverez des instructions relatives à ces procédures dans la suite de cette rubrique.

## <a name="usesolr"></a>Comment utiliser Solr dans HDInsight ?
...
1. **Établissez une connexion RDP au cluster HDInsight sur lequel Solr est installé**. Dans le Portail de gestion Azure, activez le Bureau à distance pour le cluster que vous avez créé et sur lequel vous avez installé Solr, puis accédez à distance au cluster. Pour obtenir des instructions, consultez <a href="http://azure.microsoft.com/fr-fr/documentation/articles/hdinsight-administer-use-management-portal/#rdp" target="_blank">Connexion à des clusters HDInsight à l'aide de RDP</a>.

2. **Indexez Solr en téléchargeant des fichiers de données**. Lorsque vous indexez Solr, vous y placez des documents sur lesquels vous devrez peut-être effectuer des recherches. Pour indexer Solr, établissez une connexion RDP au cluster, accédez au Bureau, ouvrez la ligne de commande Hadoop et accédez à **C:\apps\dist\solr-4.7.2\example\exampledocs**. Exécutez la commande suivante : 
...
4. **Recommandé : Sauvegardez les données indexées à partir de Solr dans un objet WASB (Azure Storage Blob) associé au cluster HDInsight**. Il est conseillé de sauvegarder les données indexées à partir de nœuds de cluster Solr sur WASB. Pour ce faire, procédez comme suit :
	...
	3. Compressez le dossier d'instantanés et téléchargez-le vers WASB. À partir de la ligne de commande Hadoop, accédez à l'emplacement du dossier d'instantanés à l'aide de la commande suivante :
...
## <a name="usingPS"></a>Installation de Solr sur des clusters HDInsight Hadoop à l'aide de PowerShell
...
10. Ajoutez le code suivant à la fonction Main() pour utiliser la classe [ScriptAction](http://msdn.microsoft.com/fr-fr/library/microsoft.windowsazure.management.hdinsight.clusterprovisioning.data.scriptaction.aspx) et appeler un script personnalisé pour installer Solr.
