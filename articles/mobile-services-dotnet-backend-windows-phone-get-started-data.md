...
4.  [Mise à jour de l'application Windows Phone pour utiliser le service mobile](#update-app)
5.  [Test de l'application Windows Phone sur le service hébergé localement](#test-locally-hosted)
6.  [Publication du service mobile sur Azure](#publish-mobile-service)
7.  [Test de l'application Windows Phone sur le service hébergé sous Azure](#test-azure-hosted)

Ce didacticiel requiert que Visual Studio 2012 et le [Kit de développement logiciel (SDK) Windows Phone 8](http://go.microsoft.com/fwlink/p/?linkid=268374) soient exécutés sous Windows 8.
...
Ce didacticiel est basé sur l'[application GetStartedWithMobileServices](http://go.microsoft.com/fwlink/p/?linkid=271146) qui correspond à un projet d'application Windows Phone 8 de Visual Studio 2012. L'interface utilisateur de cette application est semblable à l'application générée par le démarrage rapide de Mobile Services, mais les éléments ajoutés sont ici stockés en local dans la mémoire.

1.  Téléchargez la version C\# de l'exemple d'application GetStartedWithMobileServices sur le [site d'exemples de code développeur](http://go.microsoft.com/fwlink/p/?linkid=271146).

	![][1]

2.  Exécutez Visual Studio 2012 avec des privilèges d'administration : cliquez avec le bouton droit sur Visual Studio, puis cliquez sur **Exécuter en tant qu'administrateur**.

3.  Dans Visual Studio 2012, ouvrez le projet téléchargé et examinez le fichier MainPage.xaml.cs.
...
Mise à jour de l'application Windows PhoneMise à jour de l'application Windows Phone pour utiliser le service mobile
--------------------------------------------------------------------------------------------------------------------
...
4.  Dans Visual Studio, ouvrez App.xaml.cs. Collez l'extrait de code au début de la définition de la classe `App`. Ajoutez également l'instruction `using` suivante en haut du fichier et enregistrez celui-ci.
...
Test de l'application Windows Phone localementTest de l'application Windows Phone avec le service hébergé localement
--------------------------------------------------------------------------------------------------------------------
...
1.  Dans Visual Studio, ouvrez App.xaml.cs. Placez en commentaire le code permettant de créer le client `MobileServiceClient` qui se connecte au service mobile hébergé localement. Annulez le commentaire du code permettant de créer le client `MobileServiceClient` qui se connecte à votre service sous Azure. Enregistrez vos modifications dans le fichier.
         ...
         sealed partial class App : Application
         {
             ...
             // Utilisez ce constructeur après la publication sur le cloud
             public static MobileServiceClient MobileService = new MobileServiceClient(
                  "https://todolist.preview.azure-mobile-preview.net/",
                  "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"
             );        
