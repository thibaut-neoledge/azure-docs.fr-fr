---
title: Planifier des tâches de backend dans un service mobile de backend .NET | Microsoft Docs
description: Utilisez le planificateur dans Azure Mobile Services pour définir des tâches de backend .NET qui s’exécutent selon une planification.
...
> * [Backend .NET](mobile-services-dotnet-backend-schedule-recurring-tasks.md)
...
> [!NOTE]
> Ce didacticiel utilise la bibliothèque tierce LINQ to Twitter pour simplifier l’accès d’OAuth 2.0 aux API Twitter v1.1 . Vous devez télécharger et installer le package NuGet LINQ to Twitter pour suivre ce didacticiel. Pour plus d’informations, voir la page [Projet CodePlex LINQ to Twitter].
> 
> 
...
1. Développez le dossier ScheduledJobs et ouvrez le fichier de projet SampleJob.cs.
    ...
    Cette classe, qui hérite de **ScheduledJob**, représente une tâche pouvant être planifiée, dans le portail Azure Classic, pour être exécutée selon une planification fixe ou à la demande.
2. Remplacez le contenu de SampleJob.cs par le code suivant :
    ...
    Dans le code ci-dessus, vous devez remplacer les chaînes *todolistService* et *todolistContext* par l'espace de noms et le DbContext du projet téléchargé, qui sont *nom&#95;service&#95;mobile*Service et *nom&#95;service&#95;mobile*Context, respectivement.
...
2. Cliquez sur **Essayer**, puis cliquez sur **POST jobs/{jobName}**.
    ...
    ![][8]
3. Cliquez sur **Essayer**, tapez `Sample` comme valeur de paramètre **{jobName}**, puis cliquez sur **Envoyer**.
...
2. Dans le [portail Azure Classic], cliquez sur Mobile Services, puis sur votre application.
3. Cliquez sur l’onglet **Scheduler**, puis sur **+Créer**.
   
   > [!NOTE]
   > Lorsque vous exécutez votre service mobile en mode <em>Gratuit</em>, vous ne pouvez exécuter qu’une tâche planifiée à la fois. Aux niveaux payants, vous pouvez exécuter jusqu’à dix travaux planifiés à la fois.
   > 
   > 
4. Dans la boîte de dialogue du planificateur, entrez *Sample* comme **Nom du travail**, définissez l'intervalle et les unités de planification, puis cliquez sur le bouton de vérification.
...
5. Cliquez sur la tâche que vous venez de créer, puis cliquez sur l’onglet **Exécuter une fois** pour tester le script.
   ...
   > [!NOTE]
   > Une demande POST peut encore être utilisée pour démarrer la tâche planifiée. Néanmoins, l’autorisation prend implicitement la valeur utilisateur, ce qui signifie que la requête doit inclure la clé d’application dans l’en-tête.
   > 
   > 
6. (Facultatif) Dans le [portail Azure Classic], cliquez sur Gérer pour la base de données associée à votre service mobile.
   
    ![][6]
7. Dans le portail Azure Classic, exécutez une requête pour afficher les modifications effectuées par l’application. Votre requête sera similaire à la requête suivante, mais utilisera votre nom de service mobile comme nom de schéma au lieu de `todolist`.
