<properties
	pageTitle="Planifier des tâches de backend dans un service mobile de backend .NET | Microsoft Azure"
...
>[AZURE.NOTE]Ce didacticiel utilise la bibliothèque tierce LINQ to Twitter pour simplifier l’accès d’OAuth 2.0 aux API Twitter v1.1 . Vous devez télécharger et installer le package NuGet LINQ to Twitter pour suivre ce didacticiel. Pour plus d’informations, voir la page [Projet CodePlex LINQ to Twitter].
...
4. Développez le dossier Modèles, ouvrez le fichier de contexte du modèle de données (nommé nom\_service*Context.cs), puis ajoutez la propriété suivante qui renvoie un **DbSet typé** :
...
2. Remplacez le contenu de SampleJob.cs par le code suivant :
	...
	Dans le code ci-dessus, vous devez remplacer les chaînes _todolistService_ et _todolistContext_ par l’espace de noms et le DbContext du projet téléchargé, qui sont respectivement *mobile&#95;service&#95;name*Service et *mobile&#95;service&#95;name*Context.
...
4. Dans le [portail Azure Classic], cliquez sur Mobile Services, puis sur votre application.
...
4. (Facultatif) Dans le [portail Azure Classic], cliquez sur Gérer pour la base de données associée à votre service mobile.

    ![][6]

5. Dans le portail Azure Classic, exécutez une requête pour afficher les modifications effectuées par l’application. Votre requête sera similaire à la requête suivante, mais utilisera votre nom de service mobile comme nom de schéma au lieu de `todolist`.
