<properties 
	pageTitle="Gestion des conflits liés aux données hors connexion dans Mobile Services (Windows Phone) | Centre de développement mobile" 
...
Ce didacticiel repose sur l'[exemple de code de gestion des conflits], qui est un projet Windows Phone 8 pour Visual Studio 2012. L'interface utilisateur de cette application est similaire à celle de l'application présentée dans le didacticiel [Prise en main de données hors connexion], si ce n'est qu'il y a une nouvelle colonne de date pour chaque élément TodoItem.
...
4. Dans Visual Studio 2012, appuyez sur **F5** pour générer et exécuter l'application dans le débogueur.
...
La classe `TodoItem` de l'exemple est définie dans MainPage.xaml.cs Notez que la classe comporte l'attribut suivant qui va cibler les opérations de synchronisation sur la base de cette table.

        [DataTable("TodoWithDate")]

Mettez à jour votre base de données pour inclure cette table.

###<a name="dotnet-backend"></a>Mise à jour de la base de données pour les services mobiles de backend .NET 

Si vous utilisez le backend .NET pour votre service mobile, procédez comme suit pour mettre à jour le schéma de votre base de données.
...
3. Dans l'Explorateur de solutions pour Visual Studio, développez le dossier **App_Start** et ouvrez le fichier WebApiConfig.cs.
    ...
    >[AZURE.NOTE]Lors de l'utilisation de l'initialiseur de base de données par défaut, Entity Framework supprime et recrée la base de données lorsqu'il détecte une modification du modèle de données dans la définition du modèle Code First. Pour modifier ce modèle de données et conserver les données existantes dans la base de données, vous devez utiliser les migrations Code First. Pour plus d'informations, consultez la rubrique [Utilisation des migrations Code First pour mettre à jour le modèle de données](mobile-services-dotnet-backend-how-to-use-code-first-migrations.md).
...
5. Dans l'Explorateur de solutions pour Visual Studio, cliquez avec le bouton droit sur votre projet de service mobile de backend .NET, puis sur **Publier** pour publier vos modifications.


### <a name="javascript-backend"></a>Mise à jour de la base de données pour les services mobiles de backend JavaScript

Pour les services mobiles de backend JavaScript, vous allez ajouter une nouvelle table intitulée **TodoWithDate**. Pour ajouter la table **TodoWithDate** pour les services mobiles de backend JavaScript, procédez comme suit.
...
5. Pour afficher l'état actuel de votre base de données, connectez-vous au [portail de gestion Azure], cliquez sur **Mobile Services**, puis cliquez sur votre service mobile.
  ...
  * Si vous utilisez le backend JavaScript pour votre service mobile, cliquez sur l'onglet **Données**, puis cliquez sur la table **TodoWithDate**. Cliquez sur **Parcourir** pour voir que la table reste vide, étant donné que nous n'avons pas envoyé par Push les modifications depuis l'application vers le serveur.

        ![][1]

  *  Si vous utilisez le backend .NET pour votre service mobile, cliquez sur l'onglet **Configurer**, puis cliquez sur votre base de données SQL. Cliquez sur **Gérer** en bas de l'écran pour vous connecter au portail de gestion SQL Azure et afficher votre base de données en exécutant une requête SQL similaire à la suivante.
...
[Prise en main de données hors connexion]: mobile-services-windows-phone-get-started-offline-data.md
[Prise en main des données hors connexion]: mobile-services-windows-phone-get-started-offline-data.md
