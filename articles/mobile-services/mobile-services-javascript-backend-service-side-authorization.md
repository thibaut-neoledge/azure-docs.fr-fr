---
title: Autorisation côté service des utilisateurs d’un service mobile principal JavaScript | Microsoft Docs
...
2. Cliquez sur **Script**, sélectionnez l’opération **Insérer**, remplacez le script existant par la fonction suivante, puis cliquez sur **Enregistrer**.
   ...
   > [!NOTE]
   > Assurez-vous que le [schéma dynamique](https://msdn.microsoft.com/library/azure/jj193175.aspx) est activé. Dans le cas contraire, la colonne *userId* n'est pas ajoutée automatiquement. Ce paramètre est activé par défaut pour un nouveau service mobile.
   > 
   > 
...
1. Remarquez que quand vous exécutez votre application côté client, bien que certains éléments provenant des didacticiels précédents figurent déjà dans la table *TodoItem*, aucun élément n'est renvoyé. Cela se produit parce que les éléments précédents ont été insérés sans la colonne user ID et comportent maintenant des valeurs null. Vérifiez que les nouveaux éléments ajoutés ont une valeur userId associée dans la table *TodoItem*.
