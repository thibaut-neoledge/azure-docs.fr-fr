---
title: Créer un service mobile de backend .NET qui utilise le stockage de tables | Microsoft Docs
...
1. Modifiez comme suit le type de données **TodoItem** de manière à ce qu’il soit issu de **StorageData** au lieu d’**EntityData**.
   ...
   > [!NOTE]
   > Le type **StorageData** a une propriété d’ID qui nécessite une clé composée, laquelle est une chaîne au format *partitionId*,*rowValue*.
   > 
   > 
...
4. Remplacez la méthode **GetAllTodoItems** existante par le code suivant :
...
2. Exécutez l’application cliente de démarrage rapide connectée à votre service mobile.
...
3. Ajoutez de nouveaux éléments pour générer des modifications de la base de données.
