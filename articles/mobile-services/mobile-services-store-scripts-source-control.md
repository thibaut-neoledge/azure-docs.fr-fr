---
title: Stocker le code de votre projet de backend JavaScript dans le contrôle de code source | Microsoft Docs
...
1. Accédez au dossier `.\service` du référentiel Git local, puis ouvrez le fichier package.json dans un éditeur de texte et ajoutez le champ suivant à l’objet **dependencies** :
   
        "node-uuid": "~1.4.3"
   
   > [!NOTE]
   > Cette mise à jour du fichier package.json provoque un redémarrage dans votre service mobile après que la validation est envoyée.
   > 
   > 
2. Maintenant, accédez au sous-dossier .\\service\\table, ouvrez le fichier todoitem.insert.js et modifiez-le comme suit :
...
3. Comme dans la section précédente, tapez la commande suivante dans l'invite de commandes Git :
