...
6. Ajoutez l'extrait de code suivant à la classe MainPage :
    ...
    Cela crée une variable membre pour le stockage de la session Live Connect actuelle et une méthode pour gérer le processus d'authentification. LiveLoginResult contient le jeton d'authentification attribué à Mobile Services pour authentifier l'utilisateur.

7. Dans l'extrait de code ci-dessus, remplacez l'espace réservé `<microsoft-account-client-id>` par l'ID client que vous avez obtenu lors de l'inscription du compte Microsoft pour votre application.
