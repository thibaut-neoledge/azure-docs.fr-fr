---
title: Ajout de Mobile Services à une application existante (WP8) | Microsoft Docs
...
## Vue d'ensemble
Cette rubrique montre comment utiliser Azure Mobile Services pour exploiter des données dans une application Windows Phone 8. Dans ce didacticiel, vous allez télécharger une application qui stocke les données en mémoire, créer un service mobile, intégrer le service mobile à l’application, puis vous connecter au [portail Azure Classic] pour afficher les modifications apportées aux données lors de l’exécution de l’application.
...
1. Téléchargez le projet de l'exemple d'application GetStartedWithData sur le [site d'exemples de code développeur].
   ...
   > [!NOTE]
   > Pour créer une application Silverlight pour Windows Phone 8.1, il vous suffit de remplacer par Windows Phone 8.1 le système d'exploitation cible dans le projet d'application Silverlight pour Windows Phone 8 téléchargé. Pour créer une application Windows Phone Store, téléchargez la [version Windows Phone Store de l'application](http://go.microsoft.com/fwlink/p/?LinkId=397372) du projet de l'exemple d'application GetStartedWithData.
   > 
   > 
...
## <a name="create-service"></a>Création d’un service mobile dans le portail Azure Classic
...
3. Dans le [portail Azure Classic], cliquez sur **Mobile Services**, puis sur le service mobile que vous venez de créer.
...
7. Dans le fichier MainPage.cs, ajoutez les instructions `using` suivantes ou supprimez leurs marques de commentaire :
   
           using Microsoft.WindowsAzure.MobileServices;
        using Newtonsoft.Json;
8. Dans ce dossier DataModel, remplacez la définition de classe **TodoItem** par le code suivant :
