---
title: Ajout de notifications Push à votre application Mobile Services (Xamarin.Forms) - Mobile Services
...
> Pour la version Mobile Apps équivalente de cette rubrique, consultez [Ajouter des notifications Push à votre application Xamarin.Forms](../app-service-mobile/app-service-mobile-xamarin-forms-get-started-push.md).
> 
> 
...
3. [Ajout de notifications Push à votre application Xamarin.Forms.iOS](#iOS)
4. [Ajout de notifications Push à votre application Xamarin.Forms.Android](#Android)
5. [Ajout de notifications Push à votre application Xamarin.Forms.Windows](#Windows)
6. [Mise à jour du script d’insertion de table Azure pour envoyer des notifications Push à toutes les applications](#all-apps)
...
> [!IMPORTANT]
> En raison des exigences liées au service APNS, vous devez déployer et tester les notifications Push sur un appareil compatible iOS (iPhone ou iPad) plutôt que sur un émulateur.
> 
> 
...
1. Dans **AppDelegate**, remplacez l’événement **RegisteredForRemoteNotifications** :
...
2. Dans **AppDelegate**, remplacez l’événement **ReceivedRemoteNotification** :
...
1. Dans **todoitem**, cliquez sur l'onglet **Script** et sélectionnez **Insérer**.
...
3. Ouvrez le fichier projet MainActivity.cs et ajoutez l’instruction using suivante à la classe :
...
1. Dans le projet ToDoAzure.Droid, créez une classe dans le projet appelé `GcmService`.
2. Ajoutez les instructions Using suivantes à la classe **GcmService** :
...
4. Dans le fichier de projet **GcmService.cs**, ajoutez la classe suivante :
...
5. Dans le fichier de projet GcmService.cs, ajoutez le code suivant qui définit la classe **GcmService** :
...
1. Ajoutez le code suivant à la classe **GcmService** qui remplace le gestionnaire d’événements **OnRegistered**.
...
2. Remplacez la méthode **OnMessage** dans **GcmService** par le code suivant :
...
3. Ajoutez les substitutions de méthode suivantes pour **OnUnRegistered()** et **OnError()**, qui sont requis pour la compilation du projet.
...
4. Dans l’application, ajoutez une nouvelle tâche.
5. Faites glisser votre doigt du haut vers le bas de l’écran pour ouvrir le centre de notifications de l’appareil et afficher la notification.
...
2. Ajoutez le code suivant à App.xaml.cs :
   ...
   > [!NOTE]
   > Dans ce didacticiel, le service mobile envoie une notification toast à l’appareil. Lorsque vous envoyez une notification par vignette, vous devez appeler la méthode **BindToShellTile** sur le canal.
   > 
   > 
...
5. Dans l’Explorateur de solutions, développez **Propriétés**, ouvrez le fichier WMAppManifest.xml, cliquez sur l’onglet **Fonctionnalités** et vérifiez que la fonctionnalité **ID\_CAP\_PUSH\_NOTIFICATION** est activée.
...
1. Dans Visual Studio, appuyez sur la touche F5 pour exécuter l’application.
   ...
   > 
   > 
2. Dans l’application, créez une tâche intitulée **Hello push**, puis cliquez immédiatement sur le bouton Démarrer ou sur le bouton Précédent pour quitter l’application.
   ...
   > [!NOTE]
   > Vous ne recevrez pas cette notification si vous vous trouvez encore dans l’application. Pour recevoir une notification toast pendant que l’application est active, vous devez gérer l’événement [ShellToastNotificationReceived](http://msdn.microsoft.com/library/windowsphone/develop/microsoft.phone.notification.httpnotificationchannel.shelltoastnotificationreceived\(v=vs.105\).aspx).
   > 
   > 
