...
À la fin de ce didacticiel, votre service mobile envoie une notification Push chaque fois qu’un utilisateur ajoute une tâche dans l’une des applications. Vous trouverez cet exemple terminé ici : [ exemple de notification Push Xamarin.Forms Azure terminé].
...
2. Dans le portail de gestion, cliquez sur **Mobile Services**, puis sur le service mobile. Cliquez sur l'onglet **Tableau de bord**, puis notez l'**URL du site**. Ensuite, cliquez sur **Gérer les clés** et prenez note de la **Clé de l'application**. Ces valeurs sont nécessaires pour accéder au service mobile à partir de votre code d’application.
...
Vous allez ajouter des notifications Push à l’application iOS à l’aide du service de notifications Push Apple (APNS, Apple Push Notification Service). Vous avez besoin d’un compte Google actif et du [composant client de Google Cloud Messaging].
...
Vous allez ajouter des notifications Push à l’application Android en utilisant le service Google Cloud Messaging (GCM). Vous avez besoin d’un compte Google actif et du [composant client de Google Cloud Messaging].
...
Dans le portail de gestion, cliquez sur l'onglet **Données**, puis sur la table **TodoItem**.
...
Vous pouvez alors ajouter des notifications Push à **MainActivity**.
...
8. Dans le fichier de projet GcmService.cs, ajoutez le code suivant qui définit la classe **GcmService** :
	...
	>[AZURE.NOTE]La classe **GcmServiceBase** implémente les méthodes **OnRegistered()**, **OnUnRegistered()**, **OnMessage()** et **OnError()**. Vous devez substituer ces méthodes dans la classe **GcmService**.
...
[composant client de Google Cloud Messaging]: http://components.xamarin.com/view/GCMClient/
