---
title: Autorisation côté service des utilisateurs d’un service mobile principal .NET | Microsoft Docs
...
1. Dans Visual Studio, ouvrez votre projet mobile, développez le dossier DataObjects, puis ouvrez **TodoItem.cs**. La classe **TodoItem** définit l'objet de données et vous devez ajouter une propriété **UserId** pour le filtrage. Ajoutez la nouvelle propriété UserId suivante à la classe **TodoItem** :
   ...
   > [!NOTE]
   > Pour modifier ce modèle de données et conserver les données existantes dans la base de données, vous devez utiliser les [migrations Code First](mobile-services-dotnet-backend-how-to-use-code-first-migrations.md).
   > 
   > 
...
4. Recherchez la méthode **GetAllTodoItems** et remplacez l'instruction **return** existante par la ligne de code suivante :
...
5. Publiez à nouveau le projet de service mobile dans Azure.
