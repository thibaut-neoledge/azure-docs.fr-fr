...
Il est important de comprendre les différents niveaux de bases de données à votre disposition pour faire le bon choix en fonction des besoins de votre application. La base de données SQL Azure propose deux éditions différentes de base de données et trois différents niveaux de service :

- Éditions Web et Business (supprimées)
- Niveaux de service De base, Standard et Premium

Si les éditions Web et Business sont entièrement prises en charge, leur suppression est programmée pour le 12 septembre 2015, comme indiqué dans le [Forum aux questions sur la disparition des éditions Web et Business](http://msdn.microsoft.com/library/azure/dn741330.aspx). Nous encourageons les nouveaux clients à commencer à utiliser les niveaux de service De base, Standard et Premium en prévision de cette modification. Ils proposent une variété de capacités de surveillance, qui facilitent la compréhension et la résolution des performances des bases de données. Tous les nouveaux services mobiles sont créés à l’aide de l’un des nouveaux niveaux de service.

Pour convertir un service mobile utilisant l’édition Web ou Business en niveau de service De base, Standard et Premium, procédez comme suit.

1. Accédez au [portail Azure Classic].
2. Sélectionnez **+NOUVEAU** dans la barre d’outils, puis **Services de données**, **Base de données SQL**, **Création rapide**.
3. Entrez un nom de base de données et sélectionnez **Nouveau serveur de base de données SQL** dans le champ **Serveur**. Un serveur est créé, qui utilise le nouveau niveau de service De base, Standard ou Premium.
4. Renseignez les autres champs et sélectionnez **Créer une base de données SQL**. Une base de données de 100 Mo est créée avec le niveau Basic.
...
Pour plus d’informations sur l’utilisation de chaque niveau, voir la rubrique [Raisons d’utiliser les nouveaux niveaux de service](http://msdn.microsoft.com/library/azure/dn369873.aspx#Reasons)
...
3. Sélectionnez la mesure pour laquelle vous souhaitez définir une alerte, puis sélectionnez **Ajouter une règle** ![Portail Azure Classic - Alerte SQL][PortalSqlAddAlert].
4. Fournissez un nom et une description pour l’alerte. ![Portail Azure Classic - Nom et description d’alerte SQL][PortalSqlAddAlert2]
5. Spécifiez la valeur à utiliser comme seuil d’alerte. Vous pouvez indiquer **80 %** pour vous ménager un temps de réaction. Veillez également à spécifier une adresse de messagerie que vous surveillez activement. ![Portail Azure Classic - Courrier électronique et seuil d’alerte SQL][PortalSqlAddAlert3]

Pour plus d’informations sur le diagnostic des problèmes SQL, voir la section [Diagnostic avancé](#AdvancedDiagnosing) à la fin de ce document.
...
Si vous exécutez souvent les instructions WHERE ou JOIN sur certaines colonnes, veillez donc à les indexer. Pour plus d’informations, voir la section [Création d’index](#CreatingIndexes).
...
Pour plus d’informations sur les index, voir la page sur les [annotations d’index dans Entity Framework][]. Pour plus d’informations sur l’optimisation des index, voir la section [Indexation avancée](#AdvancedIndexing) à la fin de ce document.
...
- **Implémentez la pagination.** Effectuer des recherches dans la base de données peut aboutir au renvoi d’un grand nombre d’enregistrements au client. Pour minimiser le volume et la latence des opérations, pensez à implémenter la pagination.
    - Par défaut, votre service mobile limite la taille de la page à 50 enregistrements pour les requêtes entrantes, mais vous pouvez demander manuellement jusqu’à 1 000 enregistrements. Pour plus d’informations, voir la section « Renvoi de données dans les pages » pour [Windows Store](mobile-services-windows-dotnet-how-to-use-client-library.md#paging), [iOS](mobile-services-ios-how-to-use-client-library.md#paging), [Android](mobile-services-android-how-to-use-client-library.md#paging), [HTML/JavaScript](mobile-services-html-how-to-use-client-library/#paging) et [Xamarin](partner-xamarin-mobile-services-how-to-use-client-library.md#paging).
...
Pour plus d’informations sur l’optimisation de la conception des requêtes, y compris sur l’analyse des plans de requête, voir la section [Conception avancée des requêtes](#AdvancedQuery) à la fin de ce document.
...
De nombreuses tâches de diagnostic peuvent être effectuées directement dans le **portail Azure Classic**, mais certaines tâches avancées peuvent être réalisées uniquement au moyen de **SQL Server Management Studio** ou du **portail de gestion de base de données SQL**. Nous allons utiliser les vues de gestion dynamique, un ensemble de vues renseignées automatiquement avec des informations de diagnostic sur votre base de données. Cette section fournit un ensemble de requêtes que nous pouvons exécuter dans ces vues pour examiner différentes mesures. Pour plus d’informations, voir la page [Contrôle de Base de données SQL Windows Azure à l’aide de vues de gestion dynamique][].
...
Pour plus d’informations, voir les pages [Contrôle de Base de données SQL Azure à l’aide de vues de gestion dynamique][] et [Vues de gestion dynamique des index manquants](sys-missing-index-stats).
