...
5. Entrez le nombre de **Nœuds de données** à utiliser pour ce cluster, ainsi que la **Région et/ou le réseau virtuel** où le cluster sera créé. Cliquez sur la flèche pour continuer.
	...
	> [WACOM.NOTE] Pour réduire les coûts du cluster utilisé pour cet article, réduisez la **Taille du cluster** sur 1, puis supprimez le cluster lorsque vous n'en avez plus besoin.
...
3. Dans la ligne de commande Hadoop, utilisez la commande suivante pour remplacer les répertoires par le répertoire contenant la commande Storm.

	cd %storm_home%\bin

4. Pour afficher la liste des commandes disponibles, entrez " storm " sans aucun paramètre.
...
2. Ouvrez le lien **Storm UI** situé sur le Bureau. La page web de l'interface utilisateur de Storm s'affiche.  Sous **Résumé de topologie**, sélectionnez l'entrée **wordcount**.

	![storm ui](./media/hdinsight-storm-getting-started/stormui.png)

	Les statistiques de la topologie s'affichent. Elles incluent les composants qui constituent la topologie, les **spouts** et les **bolts**..

5. Sélectionnez le lien **spout** sur la page puis, pour l'une des entrées de la liste **Executors (All time)**, sélectionnez un numéro de **Port** supérieur à 0 dans les colonnes **Émis** et **Transférés**.
...
6. Une page contenant des informations semblables à celles ci-dessous doit s'afficher.
...
* **Exemples de fichiers** : le cluster Storm HDInsight contient plusieurs exemples dans le répertoire **%storm_home%\contrib**. Chaque exemple doit contenir les éléments suivants :
	...
	* L'exemple : par exemple, storm-starter-0.9.1.2.1.5.0-2057-jar-with-dependencies.jar
	...
	> [WACOM.NOTE] Les documents Java se composent de pages web. Une fois l'extraction effectuée, utilisez un navigateur pour afficher le fichier **index.html**.
