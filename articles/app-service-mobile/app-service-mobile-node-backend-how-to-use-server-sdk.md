<properties
	pageTitle="Comment utiliser le kit de développement logiciel (SDK) du serveur principal Node pour Azure Mobile Apps | Azure App Service"
	description="Découvrez comment utiliser le kit de développement logiciel (SDK) du serveur principal Node pour Azure App Service Mobile Apps."
...
# Comment utiliser le kit de développement logiciel Node dans Azure Mobile Apps

Cet article fournit des informations détaillées ainsi que des exemples sur l’utilisation d’un serveur principal Node dans Azure App Service Mobile Apps.
...
Azure App Service Mobile Apps offre la possibilité d’ajouter à une application Web une API Web d’accès aux données optimisé pour les plateformes mobiles. Le kit de développement logiciel (SDK) Azure App Service Mobile Apps est fourni pour les applications Web ASP.NET et NodeJS et prend en charge les opérations suivantes :
...
### <a name="howto-cmdline-basicapp"></a>Créer un serveur principal Node de base à l’aide de la ligne de commande

Chaque serveur principal Node Azure App Service Mobile Apps démarre en tant qu’application ExpressJS. ExpressJS est l’infrastructure de service Web la plus populaire pour le serveur principal Node. Vous pouvez créer une application ExpressJS Node de base de la manière suivante :
...
### <a name="howto-vs2015-basicapp"></a>Créer un serveur principal Node avec Visual Studio 2015

Visual Studio 2015 requiert une extension pour développer l’application Node dans l’IDE. Pour commencer, téléchargez et installez le fichier [Node.js Tools 1.1 pour Visual Studio]. Une fois le programme Node.js Tools pour Visual Studio installé, créez une application Express 4.x :
...
5. Cliquez avec le bouton droit de la souris sur le nœud **npm** et sélectionnez **Installer New npm packages...**.

6. Vous devez actualiser le catalogue npm lorsque vous créez votre première application Node (cliquez sur **Refresh**).
...
9. Ouvrez le fichier _app.js_ pour ajouter la prise en charge du SDK Azure Mobile Apps :

  a. À la ligne 6, ajoutez le code suivant :
  ...
  b. Vers la ligne 27, ajoutez le code suivant :
...
### <a name="howto-publish-to-azure"></a>Publier votre serveur principal Node dans Azure

Microsoft Azure propose plusieurs mécanismes permettant de publier votre serveur principal Node Azure App Service Mobile Apps sur le service Azure. En fonction du contrôle de la source, vous pouvez notamment utiliser les outils de déploiement intégrés à Visual Studio, les outils de ligne de commande et les options de déploiement continu. Pour plus d’informations sur ce sujet, reportez-vous au [Guide de déploiement d’Azure App Service].

Avant le déploiement, vous devriez prendre connaissance des recommandations suivantes pour l’application Node avec Azure App Service :
...
Le kit de développement logiciel Node Server azure-mobile-apps fournit des mécanismes permettant d’exposer les tables de données stockées dans SQL Azure sous la forme d’une WebAPI. Cinq opérations sont fournies.
...
| PUT /tables/\_tablename\_/:id | Mettre à jour un enregistrement existant dans la table |
...
### <a name="howto-sqlexpress-setup"></a>Utiliser SQL Express comme datastore de développement sur votre ordinateur local

Le SDK Node Azure Mobile Apps propose trois options pour traiter les données dès le départ :
...
Le SDK Node Azure Mobile Apps utilise le [package mssql Node] pour établir et utiliser une connexion à SQL Express et SQL Azure. Ce package nécessite l’activation de connexions TCP sur votre instance SQL Express.

   >[AZURE.NOTE]Le pilote memory ne fournit pas un ensemble complet de fonctionnalités à des fins de test. Si vous souhaitez tester localement votre serveur principal, nous vous recommandons d’utiliser un datastore SQL Express et d’utiliser le pilote sql.
...
L’application Node lira la variable d’environnement **SQLCONNSTR\_MS\_TableConnectionString** pour lire la chaîne de connexion de cette base de données. Vous pouvez définir cette variable dans votre environnement. Par exemple, vous pouvez utiliser PowerShell pour définir cette variable d’environnement :
...
La propriété d’accès peut prendre deux valeurs :

  - *authenticated* indique que l’application cliente doit envoyer un jeton d’authentification valide avec la demande
...
## <a name="CustomAPI"></a>API personnalisée
...
### <a name="howto-customapi-basic"></a>Définir une API personnalisée simple
...
Azure App Service fournit plusieurs techniques de débogage et de résolution des problèmes pour les applications Node. Toutes ces techniques sont disponibles.
