...
# <a name="how-to-use-the-android-client-library-for-mobile-apps"></a>Utilisation de la bibliothèque cliente Android pour Mobile Apps
...
Ce guide est axé sur le kit de développement logiciel Android côté client.  Pour plus d’informations sur les kits de développement logiciels côté serveur pour Mobile Apps, consultez les articles sur [l’utilisation du Kit de développement logiciel (SDK) backend .NET][10] ou sur [l’utilisation du Kit de développement logiciel (SDK) Node.js backend][11].
...
- [créez un backend Mobile Apps][13] à utiliser avec votre application Android ;
...
###<a name="<a-name="gradle-build"></a>update-the-gradle-build-file"></a><a name="gradle-build"></a>Mise à jour du fichier de construction Gradle
...
###<a name="<a-name="enable-internet"></a>enable-internet-permission"></a><a name="enable-internet"></a>activer les autorisations Internet.
...
## <a name="the-basics-deep-dive"></a>Présentation approfondie des principes de base

Cette section présente une partie du code de l’application de démarrage rapide qui exécute Azure Mobile Apps.  

###<a name="<a-name="data-object"></a>define-client-data-classes"></a><a name="data-object"></a>Définition des classes de données client
...
Pour savoir comment créer des tables supplémentaires dans votre backend Mobile Apps, consultez [Définir un contrôleur de table][15] (backend .NET) ou [Définir des tables à l’aide d’un schéma dynamique][16] (backend Node.js). Pour un backend Node.js, vous pouvez également utiliser les **Easy Tables** dans le [portail Azure].

###<a name="<a-name="create-client"></a>how-to:-create-the-client-context"></a><a name="create-client"></a>Création du contexte client
...
###<a name="<a-name="binding"></a>how-to:-bind-data-to-the-user-interface"></a><a name="binding"></a>Liaison des données dans l’interface utilisateur
...
#### <a name="<a-name="layout"></a>how-to:-define-the-layout"></a><a name="layout"></a>Procédure de définition de la mise en page
...
#### <a name="<a-name="adapter"></a>how-to:-define-the-adapter"></a><a name="adapter"></a>Procédure de définition de l’adaptateur

Comme la source de données de notre vue est un tableau **ToDoItem**, nous créons une sous-classe de notre adaptateur à partir de la classe **ArrayAdapter&lt;ToDoItem&gt;**. Cette sous-classe produit une vue pour chaque élément **ToDoItem** utilisant la mise en page **row_list_to_do**.
...
### <a name="<a-name="api"></a>the-api-structure"></a><a name="api"></a>Structure de l'API
...
#### <a name="<a-name="use-adapter"></a>how-to:-use-the-adapter"></a><a name="use-adapter"></a>Procédure d’utilisation de l'adaptateur
...
##<a name="<a-name="querying"></a>how-to:-query-data-from-your-mobile-app-backend"></a><a name="querying"></a>Procédure : interrogation des données à partir de votre back-end Mobile Apps
...
### <a name="<a-name="showall"></a>how-to:-return-all-items-from-a-table"></a><a name="showAll"></a>Procédure de renvoi de tous les éléments d’une table
...
### <a name="<a-name="filtering"></a>how-to:-filter-returned-data"></a><a name="filtering"></a>Procédure : filtrage des données renvoyées
...
### <a name="<a-name="sorting"></a>how-to:-sort-returned-data"></a><a name="sorting"></a>Procédure : tri des données renvoyées
...
### <a name="<a-name="paging"></a>how-to:-return-data-in-pages"></a><a name="paging"></a>Procédure : renvoi de données dans les pages
...
### <a name="<a-name="selecting"></a>how-to:-select-specific-columns"></a><a name="selecting"></a>Procédure : sélection de colonnes spécifiques
...
La méthode **select** doit suivre des méthodes telles que **where** et **orderBy**. Elle peut être suivie de méthodes de pagination telles que **top**.

### <a name="<a-name="chaining"></a>how-to:-concatenate-query-methods"></a><a name="chaining"></a>Procédure de concaténation de méthodes de requête

Les méthodes utilisées dans les requêtes de tables de backend peuvent être concaténées. La concaténation des méthodes de requêtes vous permet de sélectionner des colonnes spécifiques de lignes filtrées, qui sont triées et paginées. Vous pouvez créer des filtres logiques complexes.
Chaque méthode de requête retourne un objet de requête. Pour mettre fin à la série de méthodes et exécuter la requête, appelez la méthode **execute** . Par exemple :
...
##<a name="<a-name="inserting"></a>how-to:-insert-data-into-the-backend"></a><a name="inserting"></a>Procédure : insertion de données dans le back-end
...
##<a name="<a-name="updating"></a>how-to:-update-data-in-a-mobile-app"></a><a name="updating"></a>Mise à jour des données dans une application mobile
...
La ligne avec le même **id** est mise à jour.

##<a name="<a-name="deleting"></a>how-to:-delete-data-in-a-mobile-app"></a><a name="deleting"></a>Suppression de données dans une application mobile
...
##<a name="<a-name="lookup"></a>how-to:-look-up-a-specific-item"></a><a name="lookup"></a>Recherche d’un élément spécifique
...
##<a name="<a-name="untyped"></a>how-to:-work-with-untyped-data"></a><a name="untyped"></a>Procédure : utilisation de données non typées
...
### <a name="<a-name="json_instance"></a>how-to:-create-an-instance-of-an-untyped-table"></a><a name="json_instance"></a>Procédure de création d'une instance de table non typée
...
### <a name="<a-name="json_insert"></a>how-to:-insert-into-an-untyped-table"></a><a name="json_insert"></a>Procédure d’insertion dans une table non typée
...
### <a name="<a-name="json_delete"></a>how-to:-delete-from-an-untyped-table"></a><a name="json_delete"></a>Procédure de suppression dans une table non typée
...
### <a name="<a-name="json_get"></a>how-to:-return-all-rows-from-an-untyped-table"></a><a name="json_get"></a>Procédure de récupération de toutes les lignes d'une table non typée
...
##<a name="<a-name="custom-api"></a>how-to:-call-a-custom-api"></a><a name="custom-api"></a>Procédure : appel d’une API personnalisée
...
##<a name="<a-name="authentication"></a>how-to:-add-authentication-to-your-app"></a><a name="authentication"></a>Ajout d’une authentification à votre application
...
### <a name="<a-name="caching"></a>how-to:-add-authentication-code-to-your-app"></a><a name="caching"></a>Procédure d’ajout de code d’authentification à votre application
...
### <a name="<a-name="caching"></a>how-to:-cache-authentication-tokens"></a><a name="caching"></a>Procédure de mise en cache des jetons d'authentification
...
Vous pouvez obtenir un exemple complet illustrant la mise en cache des jetons d’authentification dans la section [Mise en cache de jetons d’authentification][7].
...
## <a name="<a-name="adal"></a>how-to:-authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>Procédure : authentifier des utilisateurs avec la bibliothèque Active Directory Authentication Library
...
* Remplacez **INSERT-AUTHORITY-HERE** par le nom du client dans lequel vous avez approvisionné votre application. Le format doit être le suivant : https://login.windows.net/contoso.onmicrosoft.com. Cette valeur peut être copiée depuis l’onglet Domaine de votre Azure Active Directory dans le [portail Azure Classic].
...
* Remplacez **INSERT-REDIRECT-URI-HERE** par le point de terminaison _/.auth/login/done_ de votre site, en utilisant le modèle HTTPS. Cette valeur doit être semblable à _https://contoso.azurewebsites.net/.auth/login/done_.
...
## <a name="how-to:-add-push-notification-to-your-app"></a>Procédure : ajout d’une notification Push à votre application
...
## <a name="how-to:-add-offline-sync-to-your-app"></a>Procédure : ajout d’une synchronisation hors connexion à votre application
...
##<a name="<a-name="customizing"></a>how-to:-customize-the-client"></a><a name="customizing"></a>Procédure : personnalisation du client

Plusieurs méthodes permettent de personnaliser le comportement par défaut du client.

### <a name="<a-name="headers"></a>how-to:-customize-request-headers"></a><a name="headers"></a>Procédure de personnalisation des en-têtes de requête
...
### <a name="<a-name="serialization"></a>how-to:-customize-serialization"></a><a name="serialization"></a>Procédure : personnalisation de la sérialisation
...
### <a name="<a-name="columns"></a>how-to:-map-different-client-and-server-names"></a><a name="columns"></a>Procédure de mappage de noms de client et de serveur différents
...
### <a name="<a-name="table"></a>how-to:-map-different-table-names-between-the-client-and-the-backend"></a><a name="table"></a>Procédure de mappage de noms de table différents entre le client et le serveur principal

Mappez le nom de la table cliente sur un autre nom de table Mobile Services en utilisant un remplacement de la méthode [getTable()][4] :

    mToDoTable = mClient.getTable("ToDoItemBackup", ToDoItem.class);

### <a name="<a-name="conversions"></a>how-to:-automate-column-name-mappings"></a><a name="conversions"></a>
...
### <a name="<a-name="complex"></a>how-to:-store-an-object-or-array-property-into-a-table"></a><a name="complex"></a>Procédure de stockage d'un objet ou d'une propriété de tableau dans une table

Jusqu'à présent, nos exemples de sérialisation impliquaient des types primitifs comme des entiers et des chaînes.  Les types primitifs sont facilement sérialisés en JSON.  Si vous souhaitez ajouter un objet complexe qui n’est pas automatiquement sérialisé en JSON, nous devons fournir la méthode de sérialisation JSON.  Pour voir un exemple d’exécution d’une sérialisation JSON personnalisée, consultez le billet de blog [Personnalisation de la sérialisation avec la bibliothèque gson dans le client Android Mobile Services][2].
...
[Procédure de concaténation de méthodes de requête]: #chaining
[Liaison des données dans l’interface utilisateur]: #binding
[How to: Define the layout]: #layout
[Procédure de définition de l’adaptateur]: #adapter
[Procédure d’utilisation de l'adaptateur]: #use-adapter
[Procédure : insertion de données dans un service mobile]: #inserting
[How to: update data in a mobile service]: #updating
[Procédure : suppression de données dans un service mobile]: #deleting
...
[Procédure : conception de tests unitaires]: #tests
...
<!-- URLs. -->
[Prise en main d'applications Azure Mobile]: app-service-mobile-android-get-started.md
[Codes de contrôle ASCII C0 et C1]: http://en.wikipedia.org/wiki/Data_link_escape_character#C1_set
[Kit de développement logiciel (SDK) de Mobile Services pour Android]: http://go.microsoft.com/fwlink/p/?LinkID=717033
