...
Dans tous les cas, vous devez inscrire votre application auprès de votre fournisseur d’identité. Le fournisseur d’identité vous communique un ID client et une clé secrète client. Vous devez ensuite configurer l’authentification / autorisation Azure App Service avec l’ID client et la clé secrète client fournis par votre fournisseur d’identité. Pour plus d’informations, suivez les instructions détaillées dans le didacticiel [Ajout d’une fonction d’authentification à votre application].

###<a name="serverflow"></a>Flux serveur
Une fois que vous avez inscrit votre fournisseur d’identité, appelez MobileServiceCleint.[méthode LoginAsync] avec la valeur [MobileServiceAuthenticationProvider] de votre fournisseur. Par exemple, le code suivant initie une connexion de flux serveur via Facebook.
