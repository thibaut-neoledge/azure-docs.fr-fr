...
Dans le cadre des bonnes pratiques liées à l’Infrastructure as Code et à la Configuration as Code, des modèles d’environnement doivent être gérés dans le contrôle de code source. Azure DevTest Labs suit cette pratique et charge tous les modèles Azure Resource Manager directement à partir de vos référentiels GitHub ou VSTS Git. Quelques règles sont à prendre en compte pour organiser vos modèles Azure Resource Manager dans un référentiel :
...
Les étapes suivantes vous guident pour ajouter un référentiel dans votre laboratoire à l’aide du portail Azure. 
...
1. À partir de la liste de paramètres **Configuration et stratégies**, sélectionnez **Référentiels**. Le panneau **Référentiels** liste les référentiels qui ont été ajoutés au laboratoire. Un référentiel nommé `Public Repo` est généré automatiquement pour tous les laboratoires et se connecte au [référentiel GitHub de DevTest Labs](https://github.com/Azure/azure-devtestlab) qui contient plusieurs artefacts de machine virtuelle que vous pouvez utiliser.

    ![Référentiel public](./media/devtest-lab-create-environment-from-arm/public-repo.png)

1. Sélectionnez **Ajouter +** pour ajouter votre référentiel de modèle Azure Resource Manager.
1. Lorsque le second panneau **Référentiels** s’ouvre, entrez les informations nécessaires comme suit :
    ...
    - **Chemins d’accès du dossier**  : en utilisant l’un des deux champs d’entrée, entrez le chemin d’accès du dossier qui commence par une barre oblique - / - et est relatif à votre Url de clonage Git de vos définitions d’artefacts (premier champ d’entrée) ou de vos définitions de modèle Azure.   
        ...
        ![Référentiel public](./media/devtest-lab-create-environment-from-arm/repo-values.png)
...
Une fois qu’un référentiel de modèles Azure Resource Manager a été configuré dans le laboratoire, vos utilisateurs de laboratoire peuvent créer un environnement à l’aide du portail Azure en procédant comme suit :
...
1. Une fois que l’environnement est créé, sélectionnez l’environnement dans la liste **Mes machines virtuelles** pour ouvrir le panneau du groupe de ressources et parcourir les ressources approvisionnées dans l’environnement.
