<properties title="Analyzing Customer Churn using Microsoft Azure Machine Learning" pageTitle="Analyse de l'attrition des clients à l'aide de Microsoft Machine Learning | Azure" description="Étude de cas de développement d'un modèle intégré pour l'analyse et l'évaluation des résiliations" metaKeywords="" services="machine-learning" solutions="" documentationCenter="" authors="pamehta" manager="paulettm" editor="cgronlun" videoId="" scriptId="" />
...
Le schéma suivant montre le prototype que nous avons créé. Il utilise quatre algorithmes d'évaluation différents dans Cloud ML Studio afin de prévoir l'attrition. L'utilisation d'une méthode multi-modèles ne se justifie pas seulement pour créer un classifieur d'ensemble afin d'améliorer l'exactitude, mais également pour se protéger du surajustement et pour améliorer la sélection des caractéristiques prédictives.  
...
Dans le secteur des télécommunications, il est essentiel de s'adresser uniquement aux clients les plus susceptibles d'attrition en leur offrant un service personnalisé ou tout autre traitement spécial. Sur ce plan, la mise en place d'Azure Cloud ML obtient des résultats identiques au modèle SAP.  
...
**LR**|	**BT**|	**AP**|	**SVM**|
--|--|--|--|
Modèle moyen|	Meilleur modèle|	Sous-performant|	Modèle moyen
...
-	Mesures dérivées pour quatre catégories principales :
	-	**Entité (ex. : abonnement)**. Fournir des informations de base sur l'abonnement et/ou le client qui risque de se désabonner.
	-	**Activité**. Permet d'obtenir toutes les informations d'utilisation possibles liées à l'entité. Ex. : nombre de connexions, etc.
	-	**Service client**. Récupérer des informations dans les journaux du service client pour indiquer si l'abonné a rencontré des problèmes ou est entré en contact avec le service client. 
	-	**Données d'entreprise et concurrentielles**. Permettre d'obtenir toutes les informations possibles concernant le client (elles peuvent être indisponibles ou difficiles à obtenir).
-	Utilisez l'ordre d'importance pour sélectionner des caractéristiques. Cela implique qu'un arbre optimisé/aléatoire est toujours une approche prometteuse.  
...
[2] http://fr.wikipedia.org/wiki/Calcul_d%27incertitude   
