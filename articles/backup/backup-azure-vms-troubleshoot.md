...
* Installez la dernière version depuis le référentiel de distribution. Nous **recommandons vivement** d’installer l’agent uniquement par le biais d’un référentiel de distribution. Pour plus d’informations sur le nom du package, reportez-vous à [Référentiel de l’agent Linux](https://github.com/Azure/WALinuxAgent) 
* Pour les machines virtuelles classiques, [mettez à jour la propriété de la machine virtuelle](http://blogs.msdn.com/b/mast/archive/2014/04/08/install-the-vm-agent-on-an-existing-azure-vm.aspx) pour indiquer que l’agent est installé. Cette étape n’est pas requise pour les machines virtuelles Resource Manager.
