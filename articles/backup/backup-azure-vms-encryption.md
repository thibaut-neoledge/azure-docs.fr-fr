...
| -------- | ------------- | ------- |
| Chiffrement | BitLocker ou dmcrypt | Étant donné que le chiffrement se produit dans une couche *différente* par rapport à Azure Backup, peu importe le logiciel de chiffrement utilisé. Cela dit, cette expérience a été validée avec Bitlocker et dmcrypt uniquement.<br><br> Afin de chiffrer les données, une clé est nécessaire. La clé doit également être protégée pour garantir un accès autorisé aux données. |
| Gestion des clés | CloudLink SecureVM<br>ou Azure KeyVault | La clé est essentielle pour chiffrer ou déchiffrer les données. Sans la clé appropriée, les données ne peuvent pas être récupérées. Cela devient *incroyablement* important avec :<br><li>les substitutions de clé<li>la rétention à long terme<br><br>Par exemple, il est possible que la clé utilisée pour sauvegarder des données il y a 7 ans ne soit pas la même clé utilisée aujourd'hui. Sans la clé d'il y a 7 ans, il est impossible d'utiliser les données restaurées à partir de ce moment-là.|
