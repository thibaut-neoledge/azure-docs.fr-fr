...
Azure Backup fournit une infrastructure qui donne aux clients la possibilité de contrôler leur environnement et leur flux de travail de sauvegarde en exécutant un script personnalisé pré/post-script dans le cadre de la sauvegarde de machine virtuelle, garantissant ainsi la cohérence des applications. Le pré-script est appelé avant la prise de l’instantané de machine virtuelle et le post-script est appelé une fois l’instantané de machine virtuelle pris. Pour plus d’informations, consultez [Sauvegarde de machine virtuelle Linux cohérente dans l’application à l’aide de pré/post-scripts](https://docs.microsoft.com/azure/backup/backup-azure-linux-app-consistent). 
> [!NOTE]
> Azure Backup appelle uniquement les pré/post-scripts écrits par le client, la cohérence de l’application doit être garantie par ces scripts qui contrôlent le client. Azure Backup marque le point de récupération comme étant cohérent dans l’application si les pré/post-scripts s’exécutent avec succès. 
>
