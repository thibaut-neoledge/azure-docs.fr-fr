---
title: "Sauvegarde Azure : sauvegarde cohérente des applications des machines virtuelles Linux Azure | Microsoft Docs"
description: "Sauvegarde cohérente des applications des machines virtuelles Linux Azure"
services: backup
...
Cet article traite de l’infrastructure de pré/post-scripts Linux et de la façon dont elle peut être utilisée pour prendre la sauvegarde cohérente des applications des machines virtuelles Linux Azure.

> [!Note]
> L’infrastructure de pré/post-scripts est prise en charge uniquement pour Resource Manager déployé sur des machines virtuelles Linux ; elle n’est pas disponible pour les machines virtuelles classiques ou Windows.
>
...
2. Téléchargez VMSnapshotPluginConfig.json à partir de [github](https://github.com/MicrosoftAzureBackup/VMSnapshotPluginConfig) et copiez-le dans le dossier /etc/azure sur toutes les machines virtuelles à sauvegarder. Créez le répertoire /etc/azure s’il n’existe pas.

3. Copiez le pré-script et le post-script de votre application sur toutes les machines virtuelles à sauvegarder. Vous pouvez copier les scripts à n’importe quel emplacement de la machine virtuelle ; vous devez mettre à jour le chemin d’accès complet des fichiers de script dans le fichier VMSnapshotPluginConfig.json.

4. Vérifiez les autorisations suivantes pour les fichiers :

   - VMSnapshotPluginConfig.json - Autorisation « 600 », c.-à-d. seul l’utilisateur « racine » doit avoir des autorisations de « lecture » et « d’écriture » pour ce fichier ; aucun utilisateur ne doit disposer des autorisations « d’exécution ».
...
5. Configurez VMSnapshotPluginConfig.json conformément aux informations ci-dessous :
...
| Pre-ScriptNotFound |    Le pré-script est introuvable à l’emplacement spécifié dans le fichier de configuration VMSnapshotPluginConfig.json. |    Assurez-vous que ce pré-script est présent au niveau du chemin d’accès spécifié dans le fichier de configuration pour garantir la sauvegarde cohérente des applications.|
| Post-ScriptNotFound |    Le post-script est introuvable à l’emplacement spécifié dans le fichier de configuration VMSnapshotPluginConfig.json. |    Assurez-vous que ce post-script est présent au niveau du chemin d’accès spécifié dans le fichier de configuration pour garantir la sauvegarde cohérente des applications.|
...
| Pre-ScriptTimeout | L’exécution du pré-script de sauvegarde cohérente des applications a expiré. | Vérifiez le script et augmentez le délai d’expiration dans le fichier VMSnapshotPluginConfig.json situé à l’emplacement /etc/azure. |
| Post-ScriptTimeout | L’exécution du post-script de sauvegarde cohérente des applications a expiré. | Vérifiez le script et augmentez le délai d’expiration dans le fichier VMSnapshotPluginConfig.json situé à l’emplacement /etc/azure. |
