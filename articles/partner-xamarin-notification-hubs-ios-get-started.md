...
Ce didacticiel requiert les composants suivants :

+ [XCode 6.0][Installation de Xcode]
...
1. Si vous n'avez pas encore inscrit votre application, accédez au <a href="http://go.microsoft.com/fwlink/p/?LinkId=272456" target="_blank">portail d'approvisionnement iOS</a> dans le centre de développement Apple. Connectez-vous avec votre ID Apple, cliquez sur **Identifiers**, sur **App IDs**, puis sur le signe **+** pour inscrire une nouvelle application.

   	![][105]

2. Entrez le nom de votre application dans **Description**, entrez une valeur dans **Bundle Identifier**, activez l'option " Push Notifications " dans la section " App Services ", puis cliquez sur **Continuer**.
...
4. Cliquez sur **Edit**, faites défiler la page jusqu'au bas de l'écran, puis cliquez sur **Create certificate...** sous la section **Development Push SSL Certificate**.
...
5. Cliquez sur **Choose File**, accédez à l'emplacement où vous avez enregistré le fichier de demande de signature de certificat créé lors de la première tâche, puis cliquez sur **Generate**.
...
6. Une fois le certificat créé par le portail, cliquez sur le bouton **Download**, puis sur **Done**.
  	...
  	![][119]  
   	...
   	Le certificat de signature est téléchargé et enregistré sur votre ordinateur dans le dossier **Téléchargements**.
...
7. Double-cliquez sur le certificat Push téléchargé **aps_development.cer**.
	...
	> [AZURE.NOTE]
	> Il se peut que le nom de votre certificat soit différent. Il portera toutefois le préfixe <strong>Apple Development iOS Push Notification Services:</strong>.
...
<h2><a name="profile"></a>Création d'un profil d'approvisionnement pour l'application</h2>

1. Dans le <a href="http://go.microsoft.com/fwlink/p/?LinkId=272456" target="_blank">portail d'approvisionnement iOS</a>, sélectionnez **Provisioning Profiles**, puis **Tout**. Cliquez ensuite sur le bouton **+** pour créer un profil. L'Assistant **Add iOS Provisioning Profile** s'affiche.

   	![][120]

2. Sous **Development**, sélectionnez **iOS App Development** en tant que type de profil d'approvisionnement, puis cliquez sur **Continue**.

   	![][121]

3. Ensuite, sélectionnez l'ID d'application de l'application de démarrage rapide Mobile Services dans la liste déroulante **App ID**, puis cliquez sur **Continue**.
...
7. Dans Xcode, ouvrez la fenêtre Organizer, sélectionnez la vue Devices, puis **Provisioning Profiles** dans la section **Library** du volet de gauche et importez le profil d'approvisionnement que vous venez de créer.
...
2. Sélectionnez **App Services**, **Service Bus**, **Concentrateur de notification**, puis **Création rapide**.

   	![][27]

3. Tapez un nom pour votre concentrateur de notification, sélectionnez une région, puis cliquez sur **Créer un concentrateur de notification**.

   	![][28]

4. Cliquez sur l'espace de noms que vous venez de créer (en général ***nom du concentrateur de notification*-ns**), puis cliquez sur l'onglet **Configurer** en haut de la page.
...
6. Cliquez sur l'onglet **Configurer** en haut de la page, puis cliquez sur **Télécharger** dans les paramètres de notification d'Apple. Sélectionnez ensuite le certificat **.p12** exporté précédemment ainsi que le mot de passe du certificat. Assurez-vous de choisir si vous souhaitez utiliser la **Production** (pour envoyer des notifications Push aux utilisateurs qui ont acheté votre application depuis le store) ou le service Push **Bac à sable** (sandbox) (au cours du développement).
...
9. Créez la méthode **ProcessNotification()** suivante dans **AppDelegate.cs** :
    ...
    > [AZURE.NOTE] Vous pouvez choisir de remplacer **FailedToRegisterForRemoteNotifications()** afin de gérer les situations impliquant une absence de connexion réseau, etc.
