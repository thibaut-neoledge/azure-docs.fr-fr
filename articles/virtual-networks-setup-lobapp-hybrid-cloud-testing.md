...
1.	Définir l'environnement de cloud hybride pour le test.
...
À partir du portail de gestion Azure, démarrez l'ordinateur DC2, le cas échéant.

Ensuite, créez une machine virtuelle Azure pour SQL1 avec les commandes suivantes via une invite de commandes de niveau administrateur Azure PowerShell sur votre ordinateur local. Avant d'exécuter ces commandes, renseignez les valeurs des variables et supprimez les caractères < et >.
...
1.	Dans le volet gauche du portail de gestion Azure, cliquez sur **Machines virtuelles**, puis sur **En cours d'exécution** dans la colonne État pour SQL1.
...
Ensuite, configurez les règles de pare-feu Windows pour autoriser le trafic pour le test de la connectivité de base et le serveur SQL. À partir d'une invite de commandes de Windows PowerShell de niveau administrateur sur SQL1, exécutez les commandes suivantes.
...
Ensuite, configurez SQL Server 2014 pour utiliser le lecteur F: pour les nouvelles bases de données et les autorisations de compte d'utilisateur.

1.	À partir de l'écran d'accueil, tapez **SQL Server Management**, puis cliquez sur **SQL Server 2014 Management Studio**.
2.	Dans **Se connecter au serveur**, cliquez sur **Connecter**.
3.	Dans le panneau d'arborescence de l'Explorateur d'objets, cliquez avec le bouton droit sur **SQL1**, puis cliquez sur **Propriétés**.
4.	Dans la fenêtre **Propriétés du serveur**, cliquez sur **Paramètres de la base de données**.
5.	Recherchez les **emplacements par défaut de la base de données** et définissez les valeurs suivantes : 
...
7.	Dans le panneau d'arborescence **Explorateur d'objets**, ouvrez **Sécurité**.
...
10.	Sur la page **Rôles du serveur**, cliquez sur **sysadmin**, puis sur **OK**.
