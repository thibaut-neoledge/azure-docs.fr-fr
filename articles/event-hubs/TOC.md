# Vue d'ensemble

## [Qu’est-ce qu’Event Hubs ?](event-hubs-what-is-event-hubs.md)
## [Vue d’ensemble des hubs d’événements](event-hubs-overview.md)
## [Vue d’ensemble de l'API Event Hubs](event-hubs-api-overview.md)
## [Présentation du modèle de sécurité et de l’authentification](event-hubs-authentication-and-security-model-overview.md)


# Prise en main
## [Prise en main des hubs d’événements](event-hubs-csharp-ephcs-getstarted.md)
## [Guide de programmation de concentrateurs d’événements](event-hubs-programming-guide.md)
## [FAQ sur les hubs d'événements](event-hubs-faq.md)

# Procédures
## Exécuter des exemples
### [Avertir les utilisateurs de données reçues à partir des capteurs](event-hubs-sensors-notify-users.md)
### [Exemple d’application Event Hubs](https://code.msdn.microsoft.com/Service-Bus-Event-Hub-286fd097)
### [Exemple d’application de montée en charge du traitement des événements avec Event Hubs](https://code.msdn.microsoft.com/Service-Bus-Event-Hub-45f43fc3)
## Gérer
### [Azure Event Hubs Archive](event-hubs-archive-overview.md)
### [Activer Event Hubs Archive à l’aide d’un modèle](event-hubs-resource-manager-namespace-event-hub-enable-archive.md)
### [Diffuser des données de diagnostics Azure en continu à l’aide d’Event Hubs](event-hubs-streaming-azure-diags-data.md)
### [Créer et déployer un Event Hub à l’aide d’un modèle Resource Manager](event-hubs-resource-manager-namespace-event-hub.md)
### [Utiliser Python pour activer Event Hubs Archive](event-hubs-archive-python.md)

# Référence
## [Quotas et limites](event-hubs-quotas.md)
## [Exceptions de messagerie](event-hubs-messaging-exceptions.md)
## [Java](/java/api)
## [.NET](/dotnet/api)
## [Guide du protocole AMQP1.0](../service-bus-messaging/service-bus-amqp-protocol-guide.md)
## [REST](/rest/api/eventhub)

# les ressources
## [Tarification](https://azure.microsoft.com/en-us/pricing/details/event-hubs/)
## [Parcours d’apprentissage](https://azure.microsoft.com/documentation/learning-paths/event-hubs/)
## [Explorateur Service Bus](https://code.msdn.microsoft.com/Service-Bus-Explorer-f2abca5a)
## [Mises à jour de service](https://azure.microsoft.com/updates/?product=event-hubs)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-eventhub)
## [Vidéos](https://azure.microsoft.com/documentation/videos/index/?services=event-hubs)


<!--HONumber=Nov16_HO4-->


