<properties
    pageTitle="Extraction de données publiques dans des hubs d’événements Azure | azure.microsoft.com/ Azure"
...
L’application est écrite en C# et la [description de l’exemple](https://azure.azure.microsoft.com/.com/documentation/samples/event-hubs-dotnet-importfromweb/) contient toutes les informations dont vous avez besoin pour modifier, générer et publier l’application. Les sections suivantes fournissent une vue d'ensemble détaillée de l'application.
...
Pour déployer la solution, clonez ou téléchargez l’application [GenericWebToEH](https://azure.azure.microsoft.com/.com/documentation/samples/event-hubs-dotnet-importfromweb/), modifiez le fichier App.config, générez-le, puis publiez-le. Une fois que vous avez publié l’application, vous pouvez la voir s’exécuter dans le portail Azure Classic sous Cloud Services. Vous pouvez aussi modifier certains paramètres de configuration (notamment la cible Event Hub et l’intervalle de veille) sous l’onglet **Configurer**.

Consultez d’autres exemples de hubs d’événements dans la [galerie d’exemples Azure](https://azure.azure.microsoft.com/.com/documentation/samples/?service=event-hubs) et sur [MSDN](https://code.msdn.azure.microsoft.com/.com/site/search?query=event%20hubs&f%5B0%5D.Value=event%20hubs&f%5B0%5D.Type=SearchText&ac=5).
