---
title: "Questions fréquente (FAQ) sur Event Hubs | Microsoft Docs"
description: "Questions fréquente (FAQ) sur Event Hubs"
services: event-hubs
...
# <a name="event-hubs-frequently-asked-questions"></a>Questions fréquente (FAQ) sur Event Hubs

## <a name="general"></a>Généralités

### <a name="what-is-the-difference-between-event-hubs-basic-and-standard-tiers"></a>Quelle est la différence entre les niveaux De Base et Standard Event Hubs ?
...
Vous devez sélectionner explicitement les unités de débit Event Hubs via le portail Azure ou les modèles Resource Manager Event Hubs. Les unités de débit s’appliquent à tous les hubs d’événements dans un espace de noms Event Hubs et chaque unité de débit fournit à l’espace de noms les capacités suivantes :
...
* Jusqu’à 2 Mo par seconde d’événements de sortie (événements consommés à partir d’un hub d’événements).
...
### <a name="can-i-use-a-single-amqp-connection-to-send-and-receive-from-multiple-event-hubs"></a>Puis-je utiliser une connexion AMQP unique pour l’envoi et la réception à partir de plusieurs hubs d’événements ?
Oui, à condition que la totalité des hubs d’événements se trouve dans le même espace de noms.
...
N’oubliez pas que le nombre de partitions sur un hub d’événements ne peut pas être modifié après configuration. Avec cela à l’esprit, il est important de réfléchir au nombre de partitions dont vous avez besoin avant de commencer. 
...
Capture est activé lorsque la fonctionnalité de capture est activée pour chaque hub d’événements de l’espace de noms. Event Hubs Capture est facturé à l’heure, par unité de débit achetée. La facturation d’Event Hubs Capture reflète les variations du nombre d’unités de débit sous forme d’incréments d’heure pleine. Pour plus d’informations sur la facturation d’Event Hubs Capture, consultez [Tarification Event Hubs](https://azure.microsoft.com/pricing/details/event-hubs/).
...
Capture utilise un compte de stockage que vous fournissez lorsqu’il est activé sur un hub d’événements. Ce compte de stockage étant le vôtre, les éventuels changements de configurations seront facturés sur votre abonnement Azure.
