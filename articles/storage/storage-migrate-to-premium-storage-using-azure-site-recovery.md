...
5. **Configurer les paramètres de réplication**. Suivez les étapes présentées dans [Configurer les paramètres de réplication](../site-recovery/site-recovery-vmware-to-azure.md) pour vérifier que votre serveur de configuration est correctement associé à la stratégie de réplication que vous créez.
...
7. Les étapes suivantes vous aident à **installer le service Mobilité et activer la réplication**.
    ...
    La machine virtuelle ayant basculé aura deux disques temporaires : l'un de la machine virtuelle principale et l’autre créé lors de l'approvisionnement de machines virtuelles dans la région de récupération. Pour exclure le disque temporaire avant la réplication, installez le service Mobilité avant d’activer la réplication. Pour plus d’informations sur la façon d’exclure le disque temporaire, reportez-vous à la rubrique [Exclure les disques de la réplication](../site-recovery/site-recovery-vmware-to-azure.md).
