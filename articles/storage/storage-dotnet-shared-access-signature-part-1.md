---
title: "Utilisation des signatures d’accès partagé (SAP) | Microsoft Docs"
...
Une signature d’accès partagé (SAP) constitue un moyen efficace pour octroyer aux autres clients un accès limité aux objets blob dans votre compte de stockage, sans exposer votre clé de compte. Dans la première partie de ce didacticiel consacré aux signatures d'accès partagé, nous allons vous présenter un aperçu du modèle SAP et examiner les meilleures pratiques concernant les signatures d'accès partagé.
...
* Une adresse IP ou plage d’adresses IP facultative à partir de laquelle le stockage Azure acceptera la signature d’accès partagé. Par exemple, vous pouvez spécifier une plage d’adresses IP appartenant à votre organisation. Cela fournit une autre mesure de sécurité à votre SAP.
...
Notez que le jeton de la SAP est une chaîne générée côté client (voir la section [Exemples de SAP](#sas-examples) ci-dessous pour obtenir des exemples de code). Le jeton de la SAP généré par la bibliothèque cliente de stockage n’est pas suivi par le stockage Azure. Vous pouvez créer un nombre illimité de jetons de SAP côté client.
...
De nombreuses surcharges de méthode et constructeurs de bibliothèque cliente de stockage proposent un paramètre SAP.  
...
## <a name="best-practices-for-using-sas"></a>Meilleures pratiques d’utilisation des SAP
