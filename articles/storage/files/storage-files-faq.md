...
* **Qu’est-ce que la synchronisation de fichiers Azure ?**  
...
* **Pourquoi utiliser un partage de fichiers Azure plutôt que Disques Azure ?**  
...
* **Je souhaite vraiment que la fonctionnalité *x* soit ajoutée à Azure Files. Pouvez-vous l’ajouter ?**  
...
* **Quelles sont les régions prises en charge pour la synchronisation de fichiers Azure (préversion) ?**  
...
* **J’ai créé un fichier directement dans mon partage de fichiers Azure sur SMB ou par le biais du portail. Combien de temps la synchronisation du fichier prend-elle sur les serveurs dans le groupe de synchronisation ?** 
    Quand vous créez ou modifiez des fichiers dans un partage de fichiers Azure à l’aide du portail Azure, de SMB ou de REST, la synchronisation des modifications sur les serveurs dans le groupe de synchronisation peut prendre jusqu’à 24 heures.

* **Quand un même fichier est modifié sur deux serveurs à peu près au même moment, que se passe-t-il ?**  
...
* **Quand j’inscris un serveur, j’obtiens de nombreuses réponses indiquant que le site web n’est pas approuvé. Pourquoi ?**  
...
* **Le stockage géoredondant (GRS) est-il pris en charge par la synchronisation de fichiers Azure ?**  
...
* **Pourquoi la propriété *Taille sur le disque* pour un fichier ne correspond-elle pas à la propriété *Taille* après l’utilisation de synchronisation de fichiers Azure ?**  
...
* **Comment puis-je savoir si un fichier a été hiérarchisé ?**  
    ...
    2. **Utiliser `fsutil` pour rechercher les points d’analyse sur un fichier.** Comme indiqué dans l’option précédente, un point d’analyse, ou pointeur spécial pour le filtre de système de fichiers de la synchronisation de fichiers Azure (StorageSync.sys), est toujours défini pour un fichier hiérarchisé. Vous pouvez vérifier si un fichier a un point d’analyse avec l’utilitaire `fsutil` depuis une invite de commandes avec élévation de privilèges ou une session PowerShell :
        ...
        > [!Warning]  
        > La commande d’utilitaire `fsutil reparsepoint` permet également de supprimer un point d’analyse. Cette commande pouvant entraîner la perte de données, ne l’exécutez que si l’équipe d’ingénieurs de la synchronisation de fichiers Azure vous le demande. 

* **Un fichier que je souhaite utiliser a été hiérarchisé... Comment puis-je le rappeler sur le disque pour l’utiliser localement ?**  
...
* **Comment puis-je imposer la hiérarchisation d’un fichier ou répertoire ?**  
...
* **La synchronisation de fichiers Azure exclut un fichier ou dossier... Pourquoi ?**
...
* **Je souhaite utiliser la synchronisation de fichiers Azure avec Windows Server 2008 R2, avec Linux ou avec mon périphérique NAS. La synchronisation de fichiers Azure prend-elle en charge cette utilisation ?**
...
* **Azure Files prend-il en charge le contrôle d’accès et l’authentification Active Directory ?**  
    Azure Files ne prend pas en charge l’authentification Azure Files ou les ACL, mais nous y travaillons activement. Pour l’instant, vous pouvez contourner de deux façons l’absence du contrôle d’accès et de l’authentification Active Directory :
...
* **Comment puis-je m’assurer que mon partage de fichiers Azure est chiffré au repos ?**  
    Vous n’avez rien à faire pour activer le chiffrement ; le [chiffrement côté serveur](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json) est désormais activé par défaut pour Azure Files dans la totalité des régions publiques et des clouds nationaux. 

* **Comment donner accès à un fichier spécifique via un navigateur web ?**  
...
* **Quelles sont les stratégies de conformité des données Azure Files prises en charge ?**  
...
* **Dois-je utiliser Azure ExpressRoute pour me connecter à Azure Files ou pour utiliser la synchronisation de fichiers Azure localement ?**  
...
* **Comment sauvegarder mon partage de fichiers Azure ?**  
    Vous pouvez utiliser des instantanés de partage périodiques (préversion) pour la protection contre les suppressions accidentelles. Vous pouvez utiliser AzCopy, RoboCopy ou un outil de sauvegarde tiers capable de sauvegarder un partage de fichiers monté.
...
* **Le niveau de performance est-il affecté ?**
...
* **Existe-t-il des limites quant au nombre d’instantanés de partage ?**
...
* **Puis-je supprimer mon partage, mais pas les instantanés de partage ?**
...
## <a name="billing-and-pricing"></a>Facturation et tarifs
* **Le trafic réseau entre une machine virtuelle Azure et un partage de fichiers est-il considéré comme de la bande passante externe facturée dans le cadre de l’abonnement ?**  
...
* **Les performances étaient lentes quand j’ai essayé de décompresser des fichiers dans Azure Files. Que dois-je faire ?**  
...
* **Est-il possible d’avoir des partages imbriqués, autrement dit, un partage sous un partage ?**  
...
* **Utilisation d’Azure Files avec IBM MQ**  
