...
Ce service nécessite que les disques durs soient chiffrés par BitLocker pour garantir la sécurité des données. Il prend en charge les comptes de stockage classiques présents dans toutes les régions d’Azure public. Vous devez expédier les disques durs à l’un des emplacements pris en charge spécifiés plus loin dans cet article. Dans cet article, vous allez découvrir le service Azure Import/Export et apprendre comment expédier des disques pour copier des données vers et depuis Azure Blob Storage.

> [AZURE.IMPORTANT] Vous pouvez créer et gérer l’importation et l’exportation de travaux à l’aide du portail Azure Classic ou d’API REST pour les comptes de stockage classiques. Actuellement, les comptes de stockage ARM ne sont pas pris en charge.

## Pourquoi dois-je utiliser le service Azure Import/Export ?
...
- Récupération des données : récupérez la grande quantité de données stockées dans Blob Storage et transmettez-la à votre site local.
...
Ces fichiers journaux stockent des informations sur votre travail et le disque, comme le numéro de série du disque et le nom de compte de stockage. Ce fichier journal n’est pas stocké sur le disque. Il est utilisé lors de la création de travaux d’importation. Une procédure détaillée de la création de ces travaux est fournie plus avant dans cet article.

L’outil client n’est compatible qu’avec le système d’exploitation Windows 64 bits. Pour connaître les versions de système d’exploitation prises en charge, consultez [Système d’exploitation](#operating-system).

Téléchargez la dernière version de l’outil client depuis [Outil client Azure Import/Export](http://go.microsoft.com/fwlink/?LinkID=301900&clcid=0x409). Pour plus d’informations sur l’outil Azure Import/Export, consultez [Référence de l’outil WAImportExport](http://go.microsoft.com/fwlink/?LinkId=329032).
...
> [AZURE.IMPORTANT] Les disques durs externes équipés d’un adaptateur USB ne sont pas pris en charge par ce service. N’envoyez pas de disque dur externe. Le disque à l’intérieur du boîtier d’un disque dur externe n’est pas utilisable. Les clés USB ne sont pas prises en charge non plus.
...
Pour les travaux d’importation, vous devez chiffrer les disques avec BitLocker. Vous pouvez utiliser le paramètre /Encrypt dans l’outil client pendant la préparation du disque. L’autre possibilité consiste à activer le chiffrement BitLocker manuellement sur le disque et spécifier la clé de chiffrement dans la ligne de commande de l’outil client pendant la préparation du disque.

Pour les travaux d’exportation, une fois vos données copiées sur le disque, le service active le chiffrement BitLocker avant de vous renvoyer le disque. La clé de chiffrement vous est fournie via le portail Azure Classic.
...
> [AZURE.IMPORTANT] * Si vous utilisez un ordinateur Windows 10 pour préparer votre disque dur, téléchargez la dernière version de l’outil Azure Import/Export.

### Emplacements

Le service Azure Import/Export prend en charge la copie des données vers et depuis tous les comptes de stockage public Azure. Vous pouvez expédier les disques durs à l’un des emplacements suivants. Si votre compte de stockage est dans un emplacement Azure public non spécifié ici, un autre emplacement d’expédition sera indiqué lorsque vous créez le travail sur le portail Azure Classic ou via une API REST.
...
**Retour des disques du centre de données :**

Lorsque vous créez un travail d’importation ou d’exportation, vous devez fournir une adresse pour le retour des disques une fois le travail terminé. Veillez à fournir une adresse de retour valide pour éviter les retards de traitement.

Vous devez également indiquer un numéro de compte de transporteur FedEx ou DHL valide en fonction de l’emplacement, pour le retour des disques. Un numéro de compte FedEx est requis pour les retours de disque à partir des États-Unis et d’Europe. Un numéro de compte DHL est requis pour les retours de disque à partir d’Asie et d’Australie. Vous pouvez créer un compte de transporteur [FedEx](http://www.fedex.com/us/oadr/) (pour les États-Unis et l’Europe) ou [DHL](http://www.dhl.com/) (pour l’Asie et l’Australie) si vous n’en avez pas. Si vous avez déjà un compte de transporteur, vérifiez qu’il est valide.
...
Vous pouvez transférer des données entre votre local site et Azure Blob Storage à l’aide du service Azure Import/Export en créant des travaux et en expédiant des disques durs à un centre de données Azure. Chaque disque dur expédié est associé à un seul travail. Chaque travail est associé à un seul compte de stockage. Consultez la section Conditions préalables pour en savoir plus sur les particularités de ce service comme les types d’objet blob pris en charge, les types de disque, les emplacements et l’expédition.

Dans cette section, nous allons décrire globalement la procédure permettant d’importer et d’exporter des travaux. Ensuite, dans la section Démarrage rapide, nous expliquons comment créer un travail d’importation et d’exportation.
...
- Utilisez l’outil Azure Import/Export pour copier vos données sur un ou plusieurs disques durs et activez le chiffrement BitLocker.  
- Créer un travail d’importation sur le portail Azure Classic ou à l’aide de l’API REST, dans votre compte de stockage classique cible. Si vous utilisez le portail Azure Classic, téléchargez les fichiers journaux du disque.
- Indiquez l’adresse de retour et le numéro de compte du transporteur pour le retour.
...
- Les disques sont renvoyés à l’aide de votre compte de transporteur à l’adresse de retour indiquée dans le travail d’importation.
...
- Créez un travail d’exportation sur le portail Azure Classic ou à l’aide de l’API REST, dans votre compte de stockage classique source.
...
- Les disques sont réceptionnés et traités dans le centre de données Azure. 
- Le chiffrement BitLocker est activé sur les disques et les clés sont disponibles via le portail Azure Classic.  
- Les disques sont renvoyés à l’aide de votre compte de transporteur à l’adresse de retour indiquée dans le travail d’importation.
...
Shipping|Votre tâche a été créée et vous avez fourni vos détails d'expédition. **Remarque** : lorsque le disque est remis au centre de données Azure, l’état « Expédition » peut rester affiché pendant un certain temps. Lorsque le service démarre la copie de vos données, l’état devient « Transfert ». Pour voir un état plus précis de votre disque, utilisez l’API REST. 
...
Le temps de traitement d’un travail d’importation/exportation varie en fonction de différents facteurs, tels que le délai de livraison, le type de travail, le type et la taille des données copiées, la taille des disques fournis. Ce service n’est soumis à aucun contre de niveau de service. Vous pouvez utiliser l’API REST pour assurer le suivi détaillé de la progression du travail. Dans l’option d’affichage des travaux, un pourcentage d’achèvement terminé indique l’état d’avancement de la copie. Contactez-nous si vous avez besoin d’estimer la durée nécessaire à un travail d’importation/exportation.
...
Lorsque vous envoyez des disques à Azure, vous payez le coût d’expédition au transporteur. Lorsqu’Azure vous renvoie les disques, les frais de retour sont facturés au compte de transporteur que vous avez spécifié lors de la création du travail.
...
Pour les sessions de copie suivantes sur le même disque dur, spécifiez le nom du fichier journal. Inutile d’indiquer la clé du compte de stockage et le disque cible, mais désactivez le formatage et le chiffrement du disque. Dans cet exemple, nous copions le dossier « H:\\Photo » et ses sous-répertoires sur le même disque cible, dans le conteneur de stockage appelé « photo ».

	WAImportExport.exe PrepImport /j:FirstDrive.jrn /id:Photo /srcdir:H:\Photo /dstdir:photo/ /MetadataFile:c:\WAImportExport\SampleMetadata.txt

Commande PrepImport de l’outil client Azure Import/Export pour copier un fichier pendant la première session de copie :

	WAImportExport PrepImport /sk:<StorageAccountKey> /csas:<ContainerSas> /t: <TargetDriveLetter> [/format] [/silentmode] [/encrypt] [/bk:<BitLockerKey>] [/logdir:<LogDirectory>] /j:<JournalFile> /id:<SessionId> /srcfile:<SourceFile> /dstblob:<DestinationBlobPath> [/Disposition:<Disposition>] [/BlobType:<BlockBlob|PageBlob>] [/PropertyFile:<PropertyFile>] [/MetadataFile:<MetadataFile>]

Commande PrepImport de l’outil client Azure Import/Export pour copier un fichier pendant les sessions de copie suivantes :

	WAImportExport PrepImport /j:<JournalFile> /id:<SessionId> /srcfile:<SourceFile> /dstblob:<DestinationBlobPath> [/Disposition:<Disposition>] [/BlobType:<BlockBlob|PageBlob>] [/PropertyFile:<PropertyFile>] [/MetadataFile:<MetadataFile>]

**Attention** : par défaut, les données sont importées sous la forme d’objets blob de bloc. Vous pouvez utiliser le paramètre /BlobType pour importer les données sous la forme d’objets blob de page. Par exemple, si vous importez des fichiers de disque dur virtuel qui seront montés comme des disques sur une machine virtuelle Azure, vous devez les importer en tant qu’objets blob de page. Si vous ne savez pas quel type d’objet blob utiliser, vous pouvez spécifier /blobType:auto pour nous permettre de vous aider à déterminer le type approprié. Dans ce cas, tous les fichiers vhd et vhdx sont importés en tant qu’objets blob de page, le reste étant importé sous la forme d’objets blob de bloc.
...
5. 	À l’étape 4, sélectionnez votre transporteur de retour dans la liste, puis entrez votre numéro de compte. Microsoft utilise ce compte pour réexpédier vos lecteurs une fois la tâche d'importation terminée.
...
3.	À l'étape 3, indiquez les données d'objets blob que vous souhaitez exporter de votre compte de stockage vers le ou les lecteurs vides. Vous pouvez choisir d’exporter toutes les données d’objets blob contenues dans le compte de stockage ou indiquer les objets blob ou ensembles d’objets blob à exporter.
	...
	Vous devez indiquer les chemins d’accès aux objets blob dans des formats valides pour éviter les erreurs de traitement. Voici un exemple.
...
**Puis-je utiliser le service Azure Import/Export avec un compte de stockage ARM ?**

Non, vous ne pouvez pas copier les données dans un compte de stockage ARM directement à l’aide du service Azure Import/Export. Le service ne prend en charge que les comptes de stockage classiques. Le compte de stockage ARM sera bientôt pris en charge. Comme alternative, vous pouvez importer des données dans un compte de stockage classique et les faire migrer vers votre compte de stockage ARM à l’aide d’AzCopy ou de CopyBlob.
...
Lors de la préparation du disque, vous pouvez spécifier si les fichiers cibles peuvent être remplacés ou ignorés à l’aide du paramètre /Disposition:< rename|no-overwrite|overwrite>. Par défaut, le service ne remplace pas les objets blob et renomme les nouveaux fichiers.
...
Vous pouvez expédier des disques au centre de données à l’aide d’un transporteur connu comme FedEx, DHL, UPS ou La Poste. Toutefois, pour renvoyer les disques du centre de données, vous devez fournir un numéro de compte FedEx aux États-Unis et dans l’UE, et un numéro de compte DHL en Asie et en Australie.
...
**Pourquoi l’état de mon travail sur le portail Azure Classic indique-t-il Expédition, alors que le site web du transporteur indique que mon colis est livré ?**

Sur le portail, l’état passe d’Expédition à Transfert au début du traitement du disque. Si votre disque est parvenu au site, mais qu’il n’a pas encore été traité, l’état de votre travail indique Expédition.

**Lors de l’expédition de mon disque, l’opérateur demande le nom du contact et le numéro de téléphone du centre de données. Que dois-je indiquer ?**

Le numéro de téléphone vous est fourni lors de la création du travail. Si vous avez besoin du nom d’un contact, contactez-nous à waimportexport@microsoft.com et nous vous fournirons ces informations.
...
[Configuration de l’outil Importation/Exportation d’Azure](https://msdn.microsoft.com/library/dn529112.aspx)
