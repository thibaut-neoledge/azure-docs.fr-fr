...
Vous trouverez ci-dessous un exemple de fichier journal de copie. Dans ce cas, une partie de 64 Ko d’un fichier a été endommagée sur le disque qui a été envoyé pour le travail d’importation. Dans la mesure où il s’agit de la seule erreur indiquée, les autres objets blob du travail ont bien été importés.  
...
L’option `/PathMapFile` vous permet de résoudre ces erreurs. Vous pouvez spécifier le nom du fichier qui contiendra la liste des fichiers que l’outil n’a pas pu identifier correctement. Voici un exemple de ligne de commande qui peut remplir `9WM35C2V_pathmap.txt` :  
