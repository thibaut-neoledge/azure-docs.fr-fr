...
Utilisez la commande d’interface de ligne de commande [az storage account update](https://docs.microsoft.com/cli/azure/storage/account#update) et spécifiez une chaîne vide (`""`) pour la valeur d’argument `--custom-domain` afin de supprimer une inscription de domaine personnalisé.
