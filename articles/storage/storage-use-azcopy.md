<properties 
	pageTitle="Copie ou déplacement des données vers le stockage avec AzCopy | Microsoft Azure" 
...
AzCopy est un utilitaire de ligne de commande conçu pour copier des données vers et à partir d'un stockage de fichiers, d'objets blob et de tables Microsoft Azure. Avec AzCopy, vous pouvez migrer vos données à partir du système de fichiers vers Azure Storage, ou vice versa, à l'aide de commandes simples et avec des performances optimales. Vous pouvez également copier des données d'un objet vers un autre au sein de votre compte de stockage ou entre des comptes de stockage.

> [AZURE.NOTE] Ce guide part du principe que vous avez installé AzCopy 5.0 ou une version ultérieure.

La version préliminaire de la bibliothèque Microsoft Azure Storage Data Movement est désormais disponible au téléchargement via [Nuget](https://www.nuget.org/packages/Microsoft.Azure.Storage.DataMovement). La bibliothèque Data Movement Library est la principale bibliothèque dans AzCopy. Le code source est disponible sur [GitHub](https://github.com/Azure/azure-storage-net-data-movement). Pour plus d'informations, consultez [Présentation de la version préliminaire de la bibliothèque Azure Storage Data Movement](https://azure.microsoft.com/blog/introducing-azure-storage-data-movement-library-preview-2/).

## Téléchargement et installation d’AzCopy

1. Téléchargez la [dernière version d'AzCopy](http://aka.ms/downloadazcopy).
2. Exécutez le programme d’installation. Par défaut, AzCopy est installé dans `%ProgramFiles(x86)%\Microsoft SDKs\Azure\AzCopy\AzCopy.exe` (sur un ordinateur exécutant Windows 64 bits) ou dans `%ProgramFiles%\Microsoft SDKs\Azure\AzCopy\AzCopy.exe` (sur un ordinateur exécutant Windows 32 bits). Cependant, vous pouvez modifier le chemin d’installation à partir de l’Assistant Installation.
...
Un moyen simple de tester AzCopy consiste à télécharger un fichier à partir de votre système de fichiers local vers le stockage d'objets Blob. À partir d'une fenêtre de console, exécutez la commande suivante, en remplaçant les noms des ressources ci-dessous par vos propres noms de ressources valides :
	
	AzCopy /Source:C:\myfolder /Dest:https://myaccount.blob.core.windows.net/mycontainer /DestKey:key /Pattern:abc.txt

Notez que lorsque vous copiez un fichier unique, vous devez spécifier l'option `/Pattern` avec le nom du fichier. D'autres exemples sont disponibles dans les sections ci-dessous.
...
<table>
  <tr>
    ...
    <th>Applicable au stockage de table (O/N)</th>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="auto-style1">Spécifie une signature d’accès partagé (SAP) avec les autorisations de lecture et d’écriture pour la destination (le cas échéant). Ajoutez des guillemets à la SAP, car elle peut contenir des caractères spéciaux de ligne de commande.<br /><br />
        Si la ressource de destination est un conteneur d’objets blob, un partage de fichiers ou une table, vous pouvez spécifier soit cette option suivie du jeton&#160;SAP, soit la SAP comme élément d’URI de l’objet blob, du partage de fichiers ou de la table de destination, sans cette option.<br /><br />
  ...
  <tr>
    ...
    <td>Spécifie si la destination est un objet blob de blocs, un objet blob de pages ou un objet blob d’ajouts. <br /><br />
	Cette option s'applique uniquement lors du téléchargement d'un objet Blob&#160;; sinon, une erreur est générée. Si la destination est un objet blob et si cette option n’est pas spécifiée, AzCopy crée par défaut un objet blob de blocs.</td>
    <td>O</td>
    <td>N</td>
    <td>N</td>
  </tr>
  ...
  <tr>
    ...
    <td>Stocke les messages de statut détaillés dans un fichier journal.
	<br /><br />Par défaut, le fichier journal détaillé est nommé <code>AzCopyVerbose.log</code> dans <code>%LocalAppData%\Microsoft\Azure\AzCopy</code>. Si vous spécifiez un emplacement de fichier existant pour cette option, le journal détaillé est ajouté à ce fichier.</td>
    <td>O</td>
    <td>O<br /></td>
    <td>O<br /></td>
  </tr>
  ...
  <tr>
    ...
    <td>Télécharge uniquement les fichiers qui ont le jeu d’attributs spécifiés.<br /><br />
  ...
  <tr>
    ...
    <td>Exclut les fichiers dont l’un des attributs spécifiés est défini.<br /><br />
        Les attributs disponibles incluent&#160;:  
        <br /><br />
        R&#160;&#160;&#160;Fichiers en lecture seule  
        <br />
  ...
  <tr>
    ...
    <td>Indique le caractère délimiteur utilisé pour délimiter les répertoires virtuels dans un nom d’objet blob.<br /><br />
  ...
  <tr>
    ...
    <td>Spécifie le fichier manifeste pour l’importation et l’exportation de la table.
	<br /><br />
  ...
  <tr>
    <td><b>/SyncCopy</b></td>
    <td>Indique s’il faut copier de manière synchronisée les objets blob ou les fichiers entre deux points de terminaison Azure Storage. <br /><br />
		...
		<br /><br />
		Vous pouvez utiliser cette option pour la copie de fichiers dans le stockage d’objets blob, le stockage de fichiers ou depuis le stockage d’objets blob vers le stockage de fichiers ou vice versa.</td>
    <td>O</td>
    <td>O<br /></td>
    <td>N</td>
  </tr>
  <tr>
    ...
    <td>Spécifie le type de contenu MIME pour les fichiers ou les objets blob de destination. <br /><br />
		AzCopy définit le type de contenu pour un objet blob ou un fichier sur <code>application/octet-stream</code> par défaut. Vous pouvez définir le type de contenu pour tous les objets blob ou les fichiers en spécifiant explicitement une valeur pour cette option. 
		<br /><br />
		Si vous spécifiez cette option sans valeur, AzCopy définit chaque type de contenu d’objet blob ou de fichier en fonction de son extension de fichier.</td>
    <td>O</td>
    <td>O<br /></td>
    <td>N</td>
  </tr>
    ...
    <td>Spécifie le format du fichier de données de table exporté.<br /><br />
...
Pour plus d’informations sur l’utilisation de vos clés d’accès de stockage, consultez [Gérer vos clés d’accès de stockage](storage-create-storage-account.md#manage-your-storage-access-keys).
...
Notez que, si le répertoire virtuel spécifié n'existe pas, AzCopy charge le fichier pour inclure le répertoire virtuel dans son nom (*par exemple*, `vd/abc.txt` dans l'exemple ci-dessus).
...
## Copie des fichiers dans un Azure File Storage avec AzCopy
...
Si la source spécifiée est un partage de fichiers Azure, vous devez soit spécifier le nom exact du fichier (*par exemple*, `abc.txt`) pour copier un seul fichier, soit spécifier l'option `/S` pour copier tous les fichiers contenus dans le partage de façon récursive. Une erreur se produit si vous tentez de spécifier à la fois un modèle de fichier et l'option `/S`.
...
|---------|-----------------------------------------------------------------------------------------------------------------|--------|----------|
| [**V5.0.0**](http://xdmrelease.blob.core.windows.net/azcopy-5-0-0/MicrosoftAzureStorageTools.msi) | **Version actuelle. Inclut toutes les fonctionnalités de V4.2.0. Toutes les fonctionnalités de stockage de fichiers et de tables sont désormais mises à la disposition générale.** | **V6.0.0** | **05-04-2015**
| [V4.2.0](http://xdmrelease.blob.core.windows.net/azcopy-4-2-0-preview/MicrosoftAzureStorageTools.msi) | Inclut toutes les fonctionnalités de V3.2.0. Prend également en charge les SAP pour le partage de fichiers de stockage, la copie asynchrone File Storage, l’exportation d’entités de table au format CSV et la spécification du nom de manifeste pendant l’exportation des entités de table | V5.0.0 | 2015-02-21
| [V3.2.0](http://xdmrelease.blob.core.windows.net/azcopy-3-2-0/MicrosoftAzureStorageTools.msi) | Prend en charge un objet blob d’ajouts et le paramètre MD5 compatible FIPS | V5.0.0 | 2015-02-21
...
Pour plus d’informations sur Azure Storage et AzCopy, voir les ressources suivantes :
...
- [Transfert de données vers le stockage d’objets blob à l’aide du service Microsoft Azure Import/Export](storage-import-export-service.md)
