...
#### Vérifiez que « Utiliser des algorithmes FIPS compatibles pour le chiffrement, le hachage et la signature » est désactivé lorsque vous utilisez AzCopy. Notez que cette option est désactivée par défaut.
Vous pouvez taper `secpol.msc` dans votre fenêtre `Run` et vérifier que ce commutateur est sur la valeur `Security Setting->Local Policy->Security Options->System cryptography: Use FIPS compliant algorithms for encryption, hashing and signing`. Veuillez noter que ce chemin d'accès du paramètre peut être différent sur le système d'exploitation Windows que vous utilisez.
...
| Version | Nouveautés |
|---------|-----------------------------------------------------------------------------------------------------------------|
| **V4.1.0** | **Version préliminaire actuelle. Inclut toutes les fonctionnalités de V3.1.0. Prend en charge la copie de fichiers et des objets blob de façon synchrone et spécifie le type de contenu pour les fichiers ** et les objets blob de destination	
| **V3.1.0** | **Version actuelle. Prend en charge la copie des objets blob de façon synchrone et spécifie le type de contenu pour les objets blob de destination.**
