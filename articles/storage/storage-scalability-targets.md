...
- **Entités** : la clé de partition d’une entité est le nom de la table + la clé de partition, cette dernière correspondant à la valeur de la propriété **PartitionKey** requise et définie par l’utilisateur pour l’entité.
	...
	Grâce au regroupement d’un ensemble d’entités d’une table en une seule partition, il est possible de mener des opérations atomiques par lots sur les entités d’une même partition, car une partition existe sur un serveur unique. Ainsi, si vous souhaitez effectuer des opérations par lot, pensez à regrouper les entités ayant la même clé de partition.

	Par contre, les entités qui se trouvent dans la même table mais qui appartiennent à des partitions différentes peuvent être réparties sur plusieurs serveurs, ce qui permet de disposer d’une table plus extensible.
