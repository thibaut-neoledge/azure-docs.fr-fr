...
1.  Connectez-vous au [portail Microsoft Azure Classic](http://manage.windowsazure.com) en ligne ou au portail Azure.
2.  Dans le portail Azure Classic : **cliquez sur Nouveau**, sur **Data Services**, puis sur **Stream Analytics** et **Création rapide**.

    ![Assistant Création rapide](./media/stream-analytics-create-a-job/1-stream-analytics-create-a-job.png)

    Dans le portail Azure : cliquez sur Nouveau, puis cliquez sur Données + analyse, puis sur Azure Stream Analytics.

    ![Création d’une tâche dans le portail Azure Classic](./media/stream-analytics-create-a-job/4-stream-analytics-create-a-job.png)
...
3.  Spécifiez la configuration souhaitée pour la tâche Stream Analytics.
	...
	- Utilisez **Région** dans le portail Azure Classic ou **Emplacement** dans le portail Azure pour spécifier l’emplacement géographique où vous souhaitez exécuter la tâche.
	- Si vous utilisez le portail Azure Classic, sélectionnez ou créez un compte de stockage à utiliser comme **Compte de stockage de surveillance régionale**. Ce compte de stockage est utilisé pour stocker les données de surveillance de toutes les tâches Stream Analytics en cours d'exécution dans cette région.
...
4.  Une fois les options de la nouvelle tâche Stream Analytics configurées, cliquez sur **Créer une tâche Stream Analytics**. La création de la tâche Stream Analytics peut prendre plusieurs minutes. Pour vérifier l'état, vous pouvez suivre l'avancement dans le concentrateur de notifications.
    ...
    ![Création d’une tâche dans le portail Azure](./media/stream-analytics-create-a-job/5-stream-analytics-create-a-job.png)
...
5.  Le nouveau travail est affiché avec l’état **Créé**. Notez que le bouton **Démarrer** est désactivé. Avant de pouvoir démarrer la tâche, vous devez configurer son entrée, sa requête et sa sortie.
    ...
    ![État d’une tâche dans le portail Azure](./media/stream-analytics-create-a-job/6-stream-analytics-create-a-job.png)
