...
Vous pouvez créer une machine virtuelle dans Azure en chargeant un disque dur virtuel exporté à partir d’un outil de virtualisation local ou d’un autre cloud. L’utilisation de la fonctionnalité [Managed Disks](../storage/storage-managed-disks-overview.md) pour la nouvelle machine virtuelle simplifie la gestion de celle-ci et fournit une meilleure disponibilité lorsque la machine virtuelle est placée dans un groupe à haute disponibilité. Si vous chargez un disque dur virtuel qui sera utilisé pour créer plusieurs machines virtuelles Azure, vous devez tout d’abord généraliser un disque dur virtuel à l’aide de [Sysprep](virtual-machines-windows-generalize-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json). Sysprep supprime les informations propres à la machine et les informations du compte personnel du disque dur virtuel. 
...
> [!IMPORTANT]
> Consultez [Planification de la migration vers Managed Disks](virtual-machines-windows-on-prem-to-azure.md#plan-for-the-migration-to-managed-disks) avant de démarrer la migration vers [Managed Disks](../storage/storage-managed-disks-overview.md).
>
