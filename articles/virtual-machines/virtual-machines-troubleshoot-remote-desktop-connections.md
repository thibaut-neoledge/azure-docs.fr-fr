...
Pour résoudre temporairement ce problème, enregistrez une copie locale du fichier RDP à partir du portail de gestion Azure, puis exécutez cette commande au niveau d’une invite de commande Windows PowerShell pour vous connecter.
...
Voici un exemple de fichier RDP généré par le portail de gestion Azure :
...
Chaque ordinateur Windows dispose d’un groupe local d’utilisateurs du Bureau à distance, qui comporte les comptes et les groupes autorisés à utiliser avec une connexion Bureau à distance. Les membres du groupe Administrateurs local y ont également accès, même si ces comptes ne sont pas répertoriés en tant que membres du groupe local d’utilisateurs du Bureau à distance. Pour les ordinateurs associés à un domaine, le groupe Administrateurs local contient également les administrateurs de domaine du domaine en question.
...
- Vérifiez l’état de la machine virtuelle sur le portail de gestion Azure ou le portail Azure en version préliminaire
...
Lancez ensuite une session PowerShell distante avec ces commandes.
...
La première partie de l’invite de commande indique que vous émettez désormais les commandes PowerShell pour le service cloud contenant la machine virtuelle cible. Le nom de votre service cloud ne sera pas « cloudservice4testing.cloudapp.net ».

Vous pouvez maintenant émettre des commandes PowerShell pour examiner les problèmes supplémentaires cités ci-dessus et corriger la configuration.

### Correction manuelle du service Services Bureau à distance permettant l’écoute du port TCP

Si vous ne parvenez pas à exécuter le [package de diagnostic Azure IaaS (Windows)](https://home.diagnostics.support.microsoft.com/SelfHelp?knowledgebaseArticleFilter=2976864) pour résoudre le problème de **connectivité RDP à une machine virtuelle Azure (redémarrage requis)**, exécutez cette commande à l’invite de session PowerShell distante.
...
Vérifiez que le port affiche désormais la valeur 3389 avec cette commande.

	Get-ItemProperty -Path "HKLM:\System\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" -Name "PortNumber"

Quittez la session PowerShell distante avec cette commande.
