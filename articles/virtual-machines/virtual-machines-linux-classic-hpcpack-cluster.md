...
Pour plus d’informations sur les options de déploiement d’un cluster HPC Pack dans Azure, voir les [Options pour créer et gérer un cluster HPC (High Performance Computing) dans Azure avec Microsoft HPC Pack](virtual-machines-linux-hpcpack-cluster-options.md).
...
3. Exécutez la commande suivante pour déployer le cluster HPC Pack. Cet exemple part du principe que le fichier de configuration se trouve dans E:\HPCDemoConfig.xml
    ...
    >[AZURE.NOTE]Dans cet exemple, le script génère un fichier journal automatiquement, car le paramètre **-LogFile** n’est pas spécifié. Les journaux ne sont pas écrits en temps réel, mais sont collectés à la fin de la validation et du déploiement. Si le processus PowerShell est arrêté pendant l’exécution du script, certains journaux sont perdus.

## <a name="connect-to-the-head-node"></a>Connexion au nœud principal

Une fois le cluster HPC Pack déployé dans Azure, [connectez-vous via le Bureau à distance au nœud principal](virtual-machines-windows-connect-logon.md) en utilisant les informations d’identification du domaine que vous avez fournies au moment où vous avez déployé le cluster (par exemple, *hpc\\clusteradmin*). Le cluster est géré à partir du nœud principal.
...
>[AZURE.NOTE]Le symbole « \` » dans la deuxième commande est un symbole de caractère d’échappement pour PowerShell. « \`, » signifie que « , » (une virgule) fait partie de la commande.
