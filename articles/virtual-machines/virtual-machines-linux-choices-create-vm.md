...
L'interface utilisateur graphique du portail constitue un moyen simple de tester une machine virtuelle, en particulier si vous venez de démarrer avec Microsoft Azure. Pour créer la machine virtuelle, utilisez le [portail Azure Classic](http://manage.windowsazure.com) ou le [portail Azure](http://portal.azure.com). Pour obtenir des instructions générales, consultez [Création d’une machine virtuelle personnalisée][] et sélectionnez une image Linux à partir de la **galerie**. Notez que le [portail Azure Classic](http://manage.windowsazure.com) crée des machines virtuelles en utilisant uniquement le modèle de déploiement classique.
