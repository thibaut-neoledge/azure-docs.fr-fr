...
Suivez les instructions dans [Comment réinitialiser un mot de passe ou SSH pour les machines virtuelles basées sur Linux](virtual-machines-linux-use-vmaccess-reset-password-or-ssh.md) sur la machine virtuelle. Avec ces instructions, vous pouvez :
...
1. Cliquez sur **Parcourir** > **Machines virtuelles** > *Nom de la machine virtuelle*. Pour une machine virtuelle créée dans Azure Resource Manager, cliquez sur **Parcourir** > **Machines virtuelles (v2)** > *Nom de la machine virtuelle*. Le volet d’état de la machine virtuelle doit afficher **En cours d’exécution**. Faites défiler vers le bas pour voir l’activité récente des ressources de calcul, de stockage et réseau.
...
> [AZURE.NOTE]Pour les machines virtuelles créées dans Azure Resource Manager, passez à [Source 4 : groupes de sécurité réseau](#nsg).
...
Si vous ne l’avez pas déjà fait, suivez les instructions données dans [Comment réinitialiser un mot de passe ou SSH pour les machines virtuelles basées sur Linux](virtual-machines-linux-use-vmaccess-reset-password-or-ssh.md) sur la machine virtuelle.
...
Pour signaler un incident, allez sur le [site de support Azure](http://azure.microsoft.com/support/options/), puis cliquez sur **Obtenir du support**.
