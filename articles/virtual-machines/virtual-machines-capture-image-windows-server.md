...
L’image est prête à être utilisée pour créer des machines virtuelles. Pour cela, créez une machine virtuelle en utilisant l’élément de menu **À partir de la galerie**, puis en sélectionnant l’image que vous venez de créer. Pour obtenir des instructions, consultez [Création d’une machine virtuelle à partir d’une image](virtual-machines-create-custom.md).
