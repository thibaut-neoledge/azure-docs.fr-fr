...
Les [groupes identiques de machines virtuelles (VMSS)](virtual-machines-windows-vmss-powershell-creating.md) peuvent être utilisés avec le gestionnaire d’extensions [Configuration d’état souhaité (DSC) Azure](virtual-machines-windows-extensions-dsc-overview.md). Le VMSS est utilisé pour configurer le déploiement de machines virtuelles de façon à satisfaire la charge. La DSC est utilisée pour configurer les machines virtuelles à mesure de leur mise en ligne afin qu’elles exécutent le logiciel en production.
