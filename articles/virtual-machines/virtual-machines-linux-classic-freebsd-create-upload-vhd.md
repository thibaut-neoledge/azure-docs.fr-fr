...
6. Installez l’Agent Azure.
    ...
    >[AZURE.IMPORTANT] Après avoir installé l’Agent Azure, il est judicieux de vérifier qu’il s’exécute :
...
3. Sélectionnez **Services de données** > **Stockage** > **Création rapide**.
	...
	![Créer rapidement un compte de stockage](./media/virtual-machines-linux-classic-freebsd-create-upload-vhd/Storage-quick-create.png)
...
4. Remplissez les champs comme suit :
	...
	![Entrer les détails du compte de stockage](./media/virtual-machines-linux-classic-freebsd-create-upload-vhd/Storage-create-account.png)


5. Sélectionnez **Créer un compte de stockage**. Le compte apparaît à présent sous **stockage**.

	![Compte de stockage correctement créé](./media/virtual-machines-linux-classic-freebsd-create-upload-vhd/Storagenewaccount.png)

6. Ensuite, créez un conteneur pour vos fichiers .vhd téléchargés. Sélectionnez le nom du compte de stockage, puis **Conteneurs**.

	![Détails du compte de stockage](./media/virtual-machines-linux-classic-freebsd-create-upload-vhd/storageaccount_detail.png)

7. Sélectionnez **Créer un conteneur**.

	![Détails du compte de stockage](./media/virtual-machines-linux-classic-freebsd-create-upload-vhd/storageaccount_container.png)
...
### Utilisez la méthode Azure AD pour télécharger un fichier .vhd
...
### Utilisez la méthode par certificat pour télécharger un fichier .vhd
...
3. Une fenêtre de navigateur apparaît et vous invite à télécharger un fichier .publishsettings. Ce fichier contient des informations et un certificat pour votre abonnement Azure.
	...
	![Page de téléchargement du navigateur](./media/virtual-machines-linux-classic-freebsd-create-upload-vhd/Browser_download_GetPublishSettingsFile.png)

3. Enregistrez le fichier .publishsettings.

4. Tapez : `Import-AzurePublishSettingsFile <PathToFile>`, où `<PathToFile>` est le chemin d'accès complet au fichier .publishsettings.
   ...
   Pour plus d’informations sur l’installation et la configuration de PowerShell, consultez la page [Installation et configuration d’Azure PowerShell](../powershell-install-configure.md).
...
## Étape 5 : création d’une machine virtuelle avec le fichier .vhd téléchargé
...
1. Depuis la fenêtre Azure PowerShell utilisée lors de l’étape précédente, tapez :
    ...
    > [AZURE.NOTE]Utilisez Linux comme type de système d’exploitation. La version actuelle d’Azure PowerShell accepte uniquement « Linux » et « Windows » comme paramètre.
...
4. Une fois l’approvisionnement terminé, vous verrez votre machine virtuelle FreeBSD s’exécuter dans Azure.

	![Image FreeBSD dans Azure](./media/virtual-machines-linux-classic-freebsd-create-upload-vhd/freebsdimageinazure.png)
