...
Vous devez rechercher et modifier deux entrées spécifiques dans le fichier **/etc/waagent.conf**. Ces entrées contrôlent l’existence d’un fichier d’échange dédié et la taille de ce fichier. Les paramètres que vous recherchez sont les suivants : - ResourceDisk.EnableSwap=N - ResourceDisk.SwapSizeMB=0 Vous devrez les modifier comme suit : - ResourceDisk.EnableSwap=Y - ResourceDisk.SwapSizeMB={taille en Mo adaptée à vos besoins} Après avoir modifié les entrées, vous devrez redémarrer l’agent waagent ou votre machine virtuelle Linux pour que vos modifications soient prises en compte. Vous pouvez vous assurer que ces modifications ont été implémentées et qu’un fichier d’échange a été créé en utilisant la commande `free` pour visualiser l’espace disponible. L’exemple ci-dessous correspond à la création d’un fichier d’échange de 512 Mo suite à la modification du fichier waagent.conf.
...
- [Stockage Premium : stockage hautes performances pour les charges de travail des machines virtuelles Azure.](../storage/storage-premium-storage-preview-portal.md)
