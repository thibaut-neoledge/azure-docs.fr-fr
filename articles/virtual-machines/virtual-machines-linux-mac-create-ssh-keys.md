---
title: Utiliser des clés SSH sous Linux et Mac | Microsoft Docs
...
## <a name="quick-command-listing"></a>Liste des commandes rapides
Dans les exemples de commandes suivants, remplacez les valeurs entre &lt; et &gt; par les valeurs de votre propre environnement.  Commencez par modifier des répertoires, `cd ~/.ssh/` afin que toutes vos clés ssh soient créées dans ce répertoire.
...
Entrez la phrase secrète pour azure_fedora_id_rsa :
...
Cette commande crée une paire de clés SSH (chiffrées) protégées par un mot de passe, à l’aide du protocole RSA 2048 bits. Elle fait l’objet d’un commentaire pour faciliter son identification.  Commencez par modifier des répertoires, `cd ~/.ssh/` afin que toutes vos clés ssh soient créées dans ce répertoire.
...
## <a name="walkthrough-of-ssh-keygen"></a>Procédure pas à pas de ssh-keygen
Chaque étape est expliquée en détail.  Commencez par passer au répertoire `~/.ssh` et exécutez `ssh-keygen`.
...
### <a name="example-`~/.ssh/config`-file:"></a>Exemple de fichier `~/.ssh/config` :
```bash
# Azure Keys
Host fedora22
