...
1. Installez Ruby dans votre environnement de développement. En fonction de votre système d'exploitation, les étapes peuvent varier.
	...
	* **Apple OS X** - Il existe plusieurs distributions pour OS X. Ce didacticiel a été validé sur OS X en utilisant [Homebrew](http://brew.sh/) pour installer **rbenv** et **ruby-build**. Vous trouverez des informations d'installation à la page [https://github.com/sstephenson/rbenv/](https://github.com/sstephenson/rbenv/).
...
1. Depuis la ligne de commande ou la session terminal, créez une application Rails nommée « blog_app » à l'aide la commande suivante :
	...
	Cette commande permet de créer un répertoire nommé **blog_app** et le remplit avec les fichiers et sous-répertoires requis par une application Rails.
...
2. Passez les répertoires vers le répertoire **blog_app**, puis utilisez la commande suivante pour créer une structuration de blog de base :
...
2. Ouvrez votre navigateur web et accédez à http://localhost:3000/. Une page similaire à celle ci-dessous doit s'afficher :

	![page rails par défaut][default-rails]

	Cette page est une page de bienvenue statique. Pour visualiser les formulaires générés par la commande de génération de modèles automatique, accédez à http://localhost:3000/posts. Une page similaire à celle ci-dessous doit s'afficher :
...
##<a id="copy"></a>Copier l’application sur la machine virtuelle
...
> [AZURE.NOTE]Si vous utilisez Windows comme environnement de développement, vous pouvez utiliser un utilitaire tel que **pscp** pour la fonctionnalité pscp. Vous pouvez télécharger pscp sur la [page de téléchargement de PuTTY](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html).

Les paramètres utilisés pour cette commande présentent l'effet suivant :

* **-r** : copie de manière récursive le contenu du répertoire spécifié et de tous les sous-répertoires.
...
* **user@vmdns** : adresse de la machine sur laquelle copier les fichiers et compte d’utilisateur avec lequel se connecter.
...
##<a id="start"></a>Installer les maillons et démarrer Rails
...
4. Dans la boîte de dialogue **ADD ENDPOINT**, cliquez sur la flèche dans la partie inférieure gauche afin d'accéder à la deuxième page, puis saisissez les informations suivantes :
...
6. Un message indiquant **UPDATE IN PROGRESS** doit s'afficher. Une fois que ce message disparaît, le point de terminaison est actif. Vous pouvez désormais tester votre application en accédant au nom DNS de votre machine virtuelle. Le site web qui apparaît doit être semblable à l'illustration ci-dessous :
	...
	![page des publications][blog-rails-cloud]
