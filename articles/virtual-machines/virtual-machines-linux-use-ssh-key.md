<properties 
	pageTitle="Utilisation de SSH pour se connecter à des machines virtuelles Linux | Microsoft Azure" 
...
## Génération de clés compatibles avec Microsoft Azure sous Linux ##
...
2. Utilisez `openssl` pour générer un certificat X509 avec une paire de clés RSA 2 048 bits. Répondez aux quelques questions posées par `openssl` (vous pouvez également les laisser vides). Le contenu de ces champs n'est pas utilisé par la plateforme :
...
## Se connecter à une machine virtuelle Microsoft Azure à partir de Linux
...
Nombre d’utilitaires incluent `openssl` pour Windows. Voici quelques exemples :
...
2.	Exécutez Git Shell à partir du menu Démarrer > Tous les programmes > GitHub, Inc
	...
	**Remarque :** l’erreur suivant peut être générée lors de l’exécution des commandes `openssl` ci-dessus :
