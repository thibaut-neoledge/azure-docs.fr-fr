...
|**$rgName**|**Nom du groupe de ressources**: le nom du groupe de ressources qui est associé à la machine virtuelle SQL.|« myrgname »|

### Activation d'AKV Integration avec PowerShell
L’applet de commande **New-AzureVMSqlServerKeyVaultCredentialConfig** crée un objet de configuration pour la fonctionnalité d’intégration de coffre de clés Azure. **Set-AzureVMSqlServerExtension** configure cette intégration avec le paramètre **KeyVaultCredentialSettings**. Les étapes suivantes indiquent comment utiliser ces commandes.
