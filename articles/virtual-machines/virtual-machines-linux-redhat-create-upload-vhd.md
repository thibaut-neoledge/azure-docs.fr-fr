...
6. Inscrivez votre abonnement Red Hat pour installer des packages à partir du référentiel RHEL en exécutant la commande suivante :
   ...
   # <a name="sudo-subscription-manager-register---auto-attach---usernamexxx---passwordxxx"></a>sudo subscription-manager register --auto-attach --username=XXX --password=XXX
7. Modifiez la ligne de démarrage du noyau dans votre configuration grub pour y inclure les paramètres de noyau supplémentaires pour Azure. Pour cela, ouvrez `/etc/default/grub` dans un éditeur de texte et modifiez le paramètre **GRUB_CMDLINE_LINUX**. Par exemple :
...
10. Ajoutez des modules de Hyper-V dans initramfs :  
    ...
    Modifiez `/etc/dracut.conf` en y ajoutant le contenu suivant : add_drivers+=”hv_vmbus hv_netvsc hv_storvsc”
...
7. Inscrivez votre abonnement Red Hat pour installer des packages à partir du référentiel RHEL en exécutant la commande suivante :
   ...
   # <a name="subscription-manager-register---auto-attach---usernamexxx---passwordxxx"></a>subscription-manager register --auto-attach --username=XXX --password=XXX
8. Modifiez la ligne de démarrage du noyau dans votre configuration grub pour y inclure les paramètres de noyau supplémentaires pour Azure. Pour cela, ouvrez `/etc/default/grub` dans un éditeur de texte et modifiez le paramètre **GRUB_CMDLINE_LINUX**. Par exemple :
...
4. Inscrivez votre abonnement Red Hat pour installer des packages à partir du référentiel RHEL en exécutant la commande suivante :
   ...
   # <a name="sudo-subscription-manager-register---auto-attach---usernamexxx---passwordxxx"></a>sudo subscription-manager register --auto-attach --username=XXX --password=XXX
5. Modifiez la ligne de démarrage du noyau dans votre configuration grub pour y inclure les paramètres de noyau supplémentaires pour Azure. Pour cela, ouvrez `/etc/default/grub` dans un éditeur de texte et modifiez le paramètre **GRUB_CMDLINE_LINUX**. Par exemple :
...
4. Ouvrez les paramètres de la machine virtuelle : a.  Attachez un nouveau disque dur virtuel à la machine virtuelle. Veillez à sélectionner **Format VHD** et **Taille fixe**.
...
### <a name="disk-io-freeze"></a>Blocage des E/S du disque
