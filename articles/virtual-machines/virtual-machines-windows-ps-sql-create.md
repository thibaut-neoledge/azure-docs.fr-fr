<properties
    pageTitle="Créer une machine virtuelle SQL Server dans Azure PowerShell (Resource Manager) | Microsoft Azure"
...
# Approvisionner une machine virtuelle SQL Server à l’aide d’Azure PowerShell (Resource Manager)
...
modèle de déploiement classique. Si vous avez besoin de la version classique de cette rubrique, consultez la page [Créer une machine virtuelle SQL Server dans Azure PowerShell (classique)](virtual-machines-windows-classic-ps-sql-create.md).
...
- [Azure PowerShell](../powershell-install-configure.md) 1.0.0 ou version ultérieure (ce didacticiel a été écrit avec la version 1.0.4).
...
Utilisez les variables suivantes pour définir l’image à utiliser pour la machine virtuelle. Dans cet exemple, l’image d’évaluation SQL Server 2016 RC3 est utilisée.
...
Avec le modèle de déploiement Resource Manager, le premier objet que vous créez est le groupe de ressources. Nous allons utiliser l’applet de commande [New-AzureRmResourceGroup](https://msdn.microsoft.com/library/mt678985.aspx) pour créer un groupe de ressources Azure et ses ressources avec le nom et l’emplacement du groupe de ressources définis par les variables que vous avez déjà initialisées.
...
>[AZURE.NOTE] Vous pouvez définir des propriétés supplémentaires dans la configuration de sous-réseau de réseau virtuel à l’aide de cette applet de commande, mais cela sort du cadre de ce didacticiel.
...
Vous pouvez maintenant afficher cette machine dans le portail Azure pour [voir son adresse IP publique et son nom de domaine complet](virtual-machines-windows-portal-sql-server-provision.md#Connect).

## Exemple de script

Le script suivant contient le script PowerShell complet pour ce didacticiel. Il présuppose que vous avez déjà configuré l’abonnement Azure à utiliser avec les commandes **Add-AzureRmAccount** et **Select-AzureRmSubscription**.
