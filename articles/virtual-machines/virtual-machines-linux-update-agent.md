...
Pour mettre à jour votre [agent Linux Azure](https://github.com/Azure/WALinuxAgent) sur une machine virtuelle Linux dans Azure vous devez déjà disposer de :
...
Pour Oracle Linux, assurez-vous que le `Addons`référentiel est activé. Modifiez le fichier `/etc/yum.repo.d/public-yum-ol6.repo`(Oracle Linux 6) ou `/etc/yum.repo.d/public-yum-ol7.repo`(Oracle Linux), modifiez la ligne `enabled=0` à `enabled=1` sous **[ol6\_addons]** ou **[ol7\_addons]** dans ce fichier.

Puis, installez la dernière version de l'agent Linux Azure, tapez :
...
###Pour la version 2.0.x, utilisez :
 ...
 Copier le nouveau fichier exécutable dans/usr/sbin /
  ...
  S'il s'agit de la nouvelle installation d'Azure Linux Agent, exécutez ce qui suit :
 
    #sudo /usr/sbin/waagent -install -verbose

###Pour la version 2.1.x, utilisez :

Vous devrez peut-être installer d'abord le package `setuptools`, disponible [ici](https://pypi.python.org/pypi/setuptools). Exécutez ensuite la commande suivante :
