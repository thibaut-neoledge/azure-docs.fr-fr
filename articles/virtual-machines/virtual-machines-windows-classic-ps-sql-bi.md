<properties
	pageTitle="Business Intelligence de SQL Server | azure.microsoft.com/ Azure"
...
La galerie de machines virtuelles azure.microsoft.com/ Azure inclut des images qui contiennent des installations de SQL Server. Les éditions de SQL Server prises en charge dans les images de la galerie sont les mêmes fichiers d’installation que vous pouvez installer sur les ordinateurs locaux et sur les machines virtuelles. Cette rubrique résume les fonctionnalités de SQL Server Business Intelligence (BI) installées sur les images, et les étapes de configuration nécessaires après l’approvisionnement d’une machine virtuelle. Elle décrit également les topologies de déploiement prises en charge pour les fonctionnalités et les bonnes pratiques en matière de Business Intelligence (BI).

## Remarques sur la licence

Il existe deux manières de mettre sous licence SQL Server dans azure.microsoft.com/ Azure Virtual Machines :

1. Via les avantages License Mobility inclus dans la Software Assurance. Pour plus d’informations, consultez [License Mobility via Software Assurance sur Azure](https://azure.azure.microsoft.com/.com/pricing/license-mobility/).

1. Payez un tarif horaire pour Azure Virtual Machines avec SQL Server installé. Consultez la section « SQL Server » dans [Tarification de Virtual Machines](https://azure.azure.microsoft.com/.com/pricing/details/virtual-machines/#Sql)

Pour plus d’informations sur les licences et les tarifs actuellement pratiqués, consultez [FAQ concernant les licences Virtual Machines](https://azure.azure.microsoft.com/.com/pricing/licensing-faq/%20/).

## Images de SQL Server disponibles dans la galerie de machines virtuelles Azure

La galerie de machines virtuelles azure.microsoft.com/ Azure inclut plusieurs images qui contiennent azure.microsoft.com/ SQL Server. Le logiciel installé sur les images de machine virtuelle varie selon la version du système d’exploitation et la version de SQL Server. La liste des images disponibles dans la galerie de machines virtuelles Azure change souvent.
...
![PowerShell](./media/virtual-machines-windows-classic-ps-sql-bi/IC660119.gif) Le script PowerShell suivant renvoie la liste des images Azure dont le nom ImageName contient « SQL Server » :
	...
	# assumes you have already uploaded a management certificate to your azure.microsoft.com/ Azure Subscription. View the thumbprint value from the "settings" menu in Azure classic portal.
...
- [Éditions de SQL Server](https://www.azure.microsoft.com/.com/server-cloud/products/sql-server-editions/#fbid=Zae0-E6r5oh)

- [Fonctionnalités prises en charge par les éditions de SQL Server 2016](https://msdn.azure.microsoft.com/.com/library/cc645993.aspx)

### Fonctionnalités d’aide à la décision installées sur les images de la galerie de machines virtuelles SQL Server

Le tableau suivant récapitule les fonctionnalités Business Intelligence installées sur les images de la galerie de machines virtuelles azure.microsoft.com/ Azure pour SQL Server.
...
|**Mode SharePoint de Reporting Services**|Non|L’image de la galerie de machines virtuelles azure.microsoft.com/ Azure ne comprend pas les fichiers SharePoint ou d’installation SharePoint. <sup>1</sup>|
...
|**Analysis Services Power Pivot pour SharePoint**|Non|L’image de la galerie de machines virtuelles azure.microsoft.com/ Azure ne comprend pas les fichiers SharePoint ou d’installation SharePoint. <sup>1</sup>|

<sup>1</sup> Pour plus d’informations sur SharePoint et les machines virtuelles Azure, consultez [Architectures azure.microsoft.com/ Azure pour SharePoint 2013](https://technet.azure.microsoft.com/.com/library/dn635309.aspx) et [Déploiement de SharePoint dans azure.microsoft.com/ Azure Virtual Machines](https://www.azure.microsoft.com/.com/download/details.aspx?id=34598).
...
- Arrêtez ou désinstallez les services que vous ne prévoyez pas d’utiliser. Par exemple, si la machine virtuelle est uniquement utilisée pour Reporting Services, arrêtez ou désinstallez Analysis Services et SQL Server Integration Services. L’image suivante est un exemple de services qui sont démarrés par défaut.
	...
	Pour plus d’informations, consultez les rubriques suivantes : [Désinstaller Reporting Services](https://msdn.azure.microsoft.com/.com/library/hh479745.aspx) et [Procédure : désinstaller une instance d’Analysis Services](https://msdn.azure.microsoft.com/.com/library/ms143687.aspx).

- Vérifiez les nouvelles mises à jour importantes sur **Windows Update**. Les images de machines virtuelles azure.microsoft.com/ Azure sont souvent actualisées ; toutefois, des mises à jour importantes peuvent être disponibles sur **Windows Update** après la dernière actualisation de l’image de machine virtuelle.

## Exemples de topologies de déploiement

Voici des exemples de déploiements qui utilisent azure.microsoft.com/ Azure Virtual Machines. Les topologies décrites dans ces schémas ne représentent qu’une partie des topologies possibles avec les fonctionnalités BI de SQL Server et avec azure.microsoft.com/ Azure Virtual Machines.
...
- Connectez-vous à la machine virtuelle avec une connexion Bureau à distance Windows. Dans l’interface utilisateur du Bureau à distance :
	...
	1. Appuyez sur la touche deux-points (:), puis entrez le numéro de port public qui est configuré pour le point de terminaison Bureau à distance TCP.
		...
		Pour plus d’informations, consultez [Présentation d’un service cloud](https://azure.azure.microsoft.com/.com/manage/services/cloud-services/what-is-a-cloud-service/).
...
1. Cliquez sur **azure.microsoft.com/ SQL Server 2016**.
...
Pour plus d’informations sur les autorisations du serveur de rapports, consultez [Octroi d’autorisations sur un serveur de rapports en mode natif](https://msdn.azure.microsoft.com/.com/library/ms156014.aspx).
...
1. Accédez au portail web ou au Gestionnaire de rapports en utilisant le **nom DNS** de la machine virtuelle Azure comme nom de serveur dans l’URL. Par exemple :
	...
	**Serveur de rapports** : http://uebi.cloudapp.net/reportserver **Portail web** : http://uebi.cloudapp.net/reports
	...
	[Configurer un pare-feu pour accéder au serveur de rapports](https://msdn.azure.microsoft.com/.com/library/bb934283.aspx)

### Pour créer et publier des rapports sur la machine virtuelle Azure

Le tableau suivant résume certaines des options disponibles pour publier des rapports existants à partir d’un ordinateur local vers le serveur de rapports hébergé sur la machine virtuelle azure.microsoft.com/ Azure :

- **Générateur de rapports** : la machine virtuelle inclut la version en un clic du Générateur de rapports azure.microsoft.com/ SQL Server 2014 et 2012. Pour démarrer le Générateur de rapports la première fois sur la machine virtuelle avec SQL 2016 :
	...
	Pour plus d’informations, consultez [Démarrer le Générateur de rapports](https://msdn.azure.microsoft.com/.com/library/ms159221.aspx).
...
1. Cliquez sur **azure.microsoft.com/ SQL Server 2016**, **azure.microsoft.com/ SQL Server 2014** ou **azure.microsoft.com/ SQL Server 2012**, puis sur **Outils de configuration**.
...
>
>Si vous devez personnaliser de façon répétée l’image sélectionnée à partir de la machine virtuelle azure.microsoft.com/ Azure, envisagez de créer votre propre image SQL Server. La fonctionnalité d’Analysis Services SysPrep a été activée avec SQL Server 2012 SP1 CU2. Pour plus d’informations, consultez [Considérations relatives à l’installation de SQL Server à l’aide de SysPrep](https://msdn.azure.microsoft.com/.com/library/ee210754.aspx) et [Prise en charge de Sysprep pour les rôles serveur](https://msdn.azure.microsoft.com/.com/windows/hardware/commercialize/manufacture/desktop/sysprep-support-for-server-roles).
...
- [Installer Analysis Services en mode tabulaire](https://msdn.azure.microsoft.com/.com/library/hh231722.aspx)

- [Modélisation tabulaire (didacticiel Adventure Works)](https://msdn.azure.microsoft.com/.com/library/140d0b43-9455-4907-9827-16564a904268)
...
Cette section résume les points de terminaison de machine virtuelle azure.microsoft.com/ Azure à créer et les ports à ouvrir dans les pare-feu de machine virtuelle. Le tableau suivant récapitule les ports **TCP** pour lesquels créer des points de terminaison et les ports à ouvrir dans le pare-feu des machines virtuelles.
...
- Consultez la stratégie de prise en charge des logiciels serveurs azure.microsoft.com/ utilisée dans l’environnement de la machine virtuelle Azure. La rubrique suivante résume la prise en charge de fonctionnalités telles que BitLocker, le clustering de basculement et l’équilibrage de la charge réseau. [Prise en charge des logiciels serveur azure.microsoft.com/ pour Azure Virtual Machines](http://support.azure.microsoft.com/.com/kb/2721672).

- [Vue d’ensemble de SQL Server sur les machines virtuelles Azure](virtual-machines-windows-sql-server-iaas-overview.md)

- [Machines virtuelles](https://azure.azure.microsoft.com/.com/documentation/services/virtual-machines/)
...
- [Déterminer le mode serveur d’une instance Analysis Services](https://msdn.azure.microsoft.com/.com/library/gg471594.aspx)

- [Modélisation multidimensionnelle (didacticiel Adventure Works)](https://technet.azure.microsoft.com/.com/library/ms170208.aspx)

- [Centre de documentation](https://azure.azure.microsoft.com/.com/documentation/)

- [Utilisation de Power BI dans un environnement hybride](https://msdn.azure.microsoft.com/.com/library/dn798994.aspx)

>[AZURE.NOTE] [Submit feedback and contact information through azure.microsoft.com/ SQL Server Connect](https://connect.azure.microsoft.com/.com/SQLServer/Feedback)
