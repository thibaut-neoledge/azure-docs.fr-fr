<properties
	pageTitle="SharePoint Server 2010 sur des machines virtuelles Azure | Microsoft Azure"
	description="Découvrez la valeur de SharePoint sur les machines virtuelles Azure, le processus pas à pas et les scénarios pris en charge pour l’utilisation de SharePoint 2010."
...
> [AZURE.NOTE]Pour en savoir plus sur le déploiement de SharePoint 2013 dans Microsoft Azure, consultez les pages [Architectures Microsoft Azure pour SharePoint 2013](https://technet.microsoft.com/library/dn635309.aspx) et [Batteries de serveurs SharePoint hébergés dans des services d'infrastructure Azure](virtual-machines-sharepoint-infrastructure-services.md).

## Machines virtuelles SharePoint sur Azure

La flexibilité de SharePoint 2010 prend en charge la plupart des charges de travail d’un déploiement Azure Virtual Machines. Azure Virtual Machines est optimal pour FIS (SharePoint Server pour sites Internet) et les scénarios de développement. Les charges de travail SharePoint principales sont également prises en charge. Si une organisation souhaite gérer et contrôler sa propre implémentation SharePoint 2010 tout en exploitant les options de la virtualisation dans une infrastructure cloud, Azure Virtual Machines est idéal pour le déploiement.

L'offre Azure Virtual Machines est hébergée et gérée dans le cloud. Elle fournit une flexibilité de déploiement et réduit le coût en limitant les dépenses d'investissement dues à l'approvisionnement en matériel. Grâce à une plus grande souplesse d’infrastructure, les organisations peuvent déployer SharePoint Server en quelques heures au lieu de plusieurs jours ou semaines. Microsoft Azure Virtual Machines permet également aux organisations de déployer des charges de travail SharePoint dans le cloud en utilisant le modèle du paiement à l’utilisation. Lorsque les charges de travail SharePoint s’accroissent, une organisation peut rapidement développer son infrastructure. Ensuite, lorsqu’elles diminuent, l’organisation peut renvoyer les ressources qui ne sont plus requises, en ne payant que ce qui a été utilisé.
...
2. Pour créer une machine virtuelle avec le système d'exploitation de base, sur le portail Azure, cliquez sur **Nouveau > Calcul > Machine virtuelle > À partir de la galerie**.
...
4. La boîte de dialogue **Virtual machine configuration** s’affiche. Fournissez les informations suivantes :
	...
	- Sélectionnez la taille appropriée. Pour un environnement de production (serveur et base de données d’application SharePoint), nous vous recommandons d’utiliser A3 (4 cœurs, mémoire 7 Go) ou une taille supérieure.
...
- Utilisez l’outil SQL Server System Preparation (SysPrep) pour installer SQL Server sur la machine virtuelle avec le système d’exploitation de base (comme indiqué ci-dessus aux étapes 1 à 7). Pour plus d'informations, accédez à la page [Installation de SQL Server 2012 à l'aide de SysPrep](http://msdn.microsoft.com/library/ee210664.aspx).

- Utilisez l’invite de commandes pour installer SQL Server. Pour plus d'informations, consultez la page [Installer SQL Server 2012 à partir de l'invite de commandes](http://msdn.microsoft.com/library/ms144259.aspx#SysPrep).

- Utilisez le support et votre clé de licence SQL Server afin d’installer SQL Server sur la machine virtuelle avec le système d’exploitation de base (tel que représenté dans les étapes 1 à 7).
...
Étape 2. Configurez les serveurs SharePoint en créant une autre machine virtuelle avec le système d’exploitation de base (tel que représenté ci-dessus dans les étapes 1 à 6). Pour créer un serveur SharePoint sur cette machine virtuelle, sélectionnez l’une des options suivantes :
...
Étape 3. configurez SharePoint. Dès que chaque machine virtuelle SharePoint est prête, configurez SharePoint Server sur chaque serveur à l’aide de l’une des options suivantes :
...
Étape 4. Une fois le script terminé, connectez-vous à la machine virtuelle à l’aide du tableau de bord de machine virtuelle.
...
Cette section aborde quelques scénarios clients majeurs relatifs aux déploiements de SharePoint à l’aide de Microsoft Azure Virtual Machines.
...
1. Mettre en service :
	- Configurez une connexion VPN entre les éléments locaux et Azure à l’aide d’Azure Virtual Network. Pour plus d'informations, consultez la page [Présentation du réseau virtuel](http://msdn.microsoft.com/library/jj156007.aspx).
...
Pour implémenter un site Web SharePoint entièrement personnalisé sur Azure, une organisation doit déployer un domaine Active Directory dans le cloud et configurer de nouvelles machines virtuelles dans ce domaine. Ensuite, une machine virtuelle exécutant SQL Server 2012 doit être créée et configurée dans le cadre d’une batterie de serveurs SharePoint. Enfin, la batterie de serveurs SharePoint doit être créée, équilibrée en charge et connectée à Active Directory et SQL Server (figure 6).
...
3. Déployer une batterie de serveurs SharePoint multiserveur :
	- Créez un réseau virtuel. Pour plus d'informations, consultez la page [Présentation du réseau virtuel](http://msdn.microsoft.com/library/jj156007.aspx).
