...
Le service Gestion des API (version préliminaire) aide les organisations à publier des API pour des développeurs externes, partenaires et internes, afin de dévoiler le potentiel de leurs données et services. Toutes les entreprises cherchent à étendre leurs opérations en tant que plateforme numérique, en créant de nouveaux canaux, en trouvant de nouveaux clients et en suscitant un engagement supérieur auprès de ceux existants. Gestion des API offre les compétences principales pour assurer un programme d'API réussi à travers l'engagement du développeur, les perspectives commerciales, l'analyse, la sécurité et la protection.
...
-   **Développeurs** - Les développeurs sont les clients qui génèrent des applications grâce à vos API. Les développeurs bénéficient d'un accès au [portail des développeurs][Portail des développeurs] et génèrent des applications qui appellent les opérations d'une API.
...
Vous pouvez personnaliser l'aspect de votre portail des développeurs en ajoutant du contenu personnalisé, en personnalisant des styles et en ajoutant votre marque.
  ...
  [Stratégies Gestion des API]: ../api-management-howto-policies
  [Création et configuration de paramètres produit avancés]: ../api-management-howto-product-with-rules
