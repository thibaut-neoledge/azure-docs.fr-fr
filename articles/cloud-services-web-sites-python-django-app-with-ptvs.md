<properties urlDisplayName="Django with Python Tools for Visual Studio 2.0" pageTitle="Création d'applications Django avec Python Tools for Visual Studio 2,0" metaKeywords="" description="Découvrez comment utiliser les outils Python pour Visual Studio pour créer une application Django qui stocke les données dans une base de données SQL ou une instance de base de données MySQL et peut être déployée vers un site web ou un service cloud." metaCanonical="" services="web-sites,cloud-services" documentationCenter="Python" title="Creating Django applications with Python Tools 2.0 for Visual Studio" authors="huvalo" solutions="" manager="wpickett" editor="" />
...
<p>Dans ce didacticiel, nous allons utiliser Python Tools 2.0 for Visual Studio (PTVS) pour créer une application Django. Cette application permettra aux utilisateurs de voter lors de sondages. Nous allons d'abord utiliser une base de données sqlite3 locale, puis passer à un serveur SQL Server ou une base de données MySQL sur Azure. Nous verrons comment activer l'interface d'administration de Django et l'utiliser pour ajouter des sondages à notre base de données. Nous utiliserons également l'interpréteur de commandes Django intégré à Visual Studio.  Enfin, nous déploierons notre application dans un site web Azure, puis un service cloud Azure.</p>
...
Ce didacticiel se concentre sur PTVS et Azure. Pour plus d'informations sur Django et l'application de sondage de ce didacticiel, consultez la page [https://www.djangoproject.com/](https://www.djangoproject.com/).
...
- Visual Studio et le Kit de développement logiciel (SDK) Azure :
  ...
  - L'interpréteur de commandes VS 2013 gratuit intégré **Le Kit de développement logiciel (SDK) Azure ne prend pas en charge l'interprétateur de commandes intégré** : vous pouvez développer, déboguer et exécuter des applications Django en local en utilisant l'interpréteur de commandes intégré, mais vous ne pourrez pas publier dans des sites web ou des services cloud Azure avec Visual Studio.
...
**Remarque :** pour exécuter correctement cette application avec un site web Azure ou un service cloud, vous devez utiliser la version officielle de CPython 2.7 disponible sur [python.org](http://www.python.org/download/). Les autres versions peuvent fonctionner, mais ne sont pas officiellement prises en charge.

## Téléchargement d'un projet existant

Pour passer à la suite de ce didacticiel, veuillez [télécharger le code source de ce projet](http://download-codeplex.sec.s-msft.com/Download?ProjectName=pytools&DownloadId=783376).
...
Python Tools for Visual Studio prend en charge les environnement virtuels Python.  Nous allons créer un projet Django et utiliser un environnement virtuel pour installer nos dépendances.  Cette méthode est recommandée pour configurer les projets publiés dans les sites web Azure ou les services cloud.
...
1. Créez un environnement virtuel.  Dans l'Explorateur de solutions, cliquez avec le bouton droit sur **Environnements Python**, puis sélectionnez **Ajouter un environnement virtuel**.

	![Add Virtual Environment](./media/cloud-services-python-create-deploy-django-app/django-tutorial-002-add-virtual-env.png)

1. Sélectionnez Python 2.7 comme interpréteur Python de base, puis acceptez le nom **env** par défaut. PTVS installe pip et/ou virtualenv si vous ne l'avez pas déjà fait.

1. Cliquez avec le bouton droit sur **env**, puis cliquez sur **Installer le package Python** : **django**

	![Install Django](./media/cloud-services-python-create-deploy-django-app/django-tutorial-003-install-django.png)

1. L'installation de Django peut prendre un peu de temps, car il contient beaucoup de fichiers. Vous pouvez afficher la progression de l'installation dans la fenêtre de résultat.
...
1. Cliquez avec le bouton droit sur **env**, puis cliquez sur **Installer le package Python** : **pytz** (facultatif, mais recommandé, utilisé par Django pour la prise en charge des fuseaux horaires)
...
1. Sélectionnez le **nœud du projet**, **Ajouter**->**Application Django**, avec le nom **polls**. Cela crée un dossier pour l'application, qui contient un code réutilisable pour les fichiers d'application les plus communément utilisés.

	![Add Django App](./media/cloud-services-python-create-deploy-django-app/django-tutorial-005-add-django-app.png)

1. Dans **tutorial/settings.py**, ajoutez le code suivant à **INSTALLED\_APPS** :

		'polls',

1. et supprimez les commentaires de **INSTALLED\_APPS**:

		'django.contrib.admin',

1. Remplacez **tutorial/urls.py** par le code suivant :
...
1. Remplacez **polls/models.py** par le code suivant :
...
1. Remplacez **polls/views.py** par le code suivant :
...
1. Ajoutez un nouveau fichier Python **polls/urls.py** avec le code suivant :
...
1. Créez un élément **polls/admin.py** avec le code suivant :
...
1. Sous le dossier **polls/templates**, créez un dossier nommé **polls**.

1. Déplacez le fichier **polls/templates/index.html** vers le répertoire **polls/templates/polls** en utilisant un glisser-déposer ou un copier/coller.

1. Remplacez **polls/templates/polls/index.html** par le balisage suivant :
...
1. Créez un modèle HTML Django **polls/templates/polls/detail.html** avec le code suivant :
...
1. Créez un modèle HTML Django **polls/templates/polls/results.html** avec le code suivant :
...
1. Votre projet doit maintenant se composer des fichiers suivants :
...
1. Dans **tutorial/settings.py**, ajoutez l'importation suivante en haut du fichier :

        from os import path

1. Après cette commande, toujours en haut du fichier, ajoutez la définition suivante :

		PROJECT_ROOT = path.dirname(path.abspath(path.dirname(__file__)))

1. Remplacez la section DATABASES par le code suivant :
...
1. Cliquez avec le bouton droit sur le nœud du projet et sélectionnez **Django**->**Base de données de synchronisation Django**.  Une fenêtre interactive pour la gestion de Django s'affiche.  Comme la base de données n'existe pas pour le moment, vous devez créer vos informations d'identification d'administrateur.  Entrez un nom d'utilisateur et un mot de passe. L'adresse électronique est facultative.

	![Django Sync DB](./media/cloud-services-python-create-deploy-django-app/django-tutorial-007-sqlite3.png)

1. Démarrez le site web en appuyant sur F5 ou CTRL-F5. Ceci démarre le serveur de développement Django, ainsi que votre navigateur web. L'URL racine du site web affiche l'index des sondages, mais pour le moment, la base de données n'en contient pas.

	![Web Browser](./media/cloud-services-python-create-deploy-django-app/django-tutorial-008-dev-server.png)

1. Accédez à **http://localhost:{port}/admin**. Vous pouvez obtenir le numéro de port à partir de la fenêtre de console du serveur de développement. Connectez-vous en utilisant les informations d'identification créées à l'étape précédente.

	![Add Poll](./media/cloud-services-python-create-deploy-django-app/django-tutorial-009-admin-login.png)

1. Utilisez l'interface d'administration pour ajouter un ou deux sondages.  Ne passez pas trop de temps à ajouter des sondages à la base de données locale. Nous le ferons plus sérieusement lorsque nous basculerons plus tard vers une base de données du cloud.

	![Poll Index](./media/cloud-services-python-create-deploy-django-app/django-tutorial-009-admin-add-poll.png)

1. Accédez à **http://localhost:{port}/**.  Un index des sondages ajoutés s'affiche.

	![](./media/cloud-services-python-create-deploy-django-app/django-tutorial-010-index.png)

1. Cliquez sur l'un des sondages pour accéder à la page de vote.

	![Poll Detail](./media/cloud-services-python-create-deploy-django-app/django-tutorial-011-detail.png)

1. Envoyez votre vote : vous êtes redirigé vers la page des résultats, où vous pouvez constater que le compteur des votes a augmenté.
...
1. Dans **tutorial/settings.py**, recherchez l'affectation de **STATIC_ROOT** et remplacez-la par :

        STATIC_ROOT = path.join(PROJECT_ROOT, 'static').replace('\\','/')

1. Sous le dossier **polls**, créez un dossier nommé **static**.

1. Sous le dossier **polls/static**, créez un dossier nommé **polls**.

1. Sous le dossier **polls/static/polls**, créez un dossier nommé **images**.

1. Ajoutez un nouveau fichier de feuille de style **polls/static/polls/style.css** avec le code suivant :
...
1. Ajoutez une image au dossier **polls/static/polls/images** et nommez-la **background.jpg**.

1. Votre projet doit maintenant se composer des fichiers suivants :

	![Solution Explorer](./media/cloud-services-python-create-deploy-django-app/django-tutorial-013-solution-explorer.png)

1. Modifiez l'en-tête de tous les modèles pour qu'ils se réfèrent à la feuille de style, en utilisant le balisage suivant :
...
1. Exécutez de nouveau le site web.  Les pages d'index, de sondage et de résultats utilisent la feuille de style que vous venez de créer, avec du texte en bleu sombre et une image d'arrière-plan.
...
1. Démarrez le débogage en utilisant la touche **F5**. Visual Studio s'arrête dans le fichier de modèle.

1. Ouvrez la **fenêtre Variables locales** à partir de **Débogage->Fenêtres->Variables locales** pour accéder à la variable **latest_poll_list** et à sa valeur.

1. La touche **F10** vous permet de vous déplacer, comme dans un code Python standard.  Dans la boucle for, inspectez la valeur de **poll** :
...
Vous pouvez également créer une machine virtuelle et installer un serveur de base de données.  Consultez les instructions [ici](http://www.windowsazure.com/fr-fr/manage/linux/common-tasks/mysql-on-a-linux-vm/) pour savoir comment configurer MySQL sur une machine virtuelle Linux Azure.
...
1. Commencez par créer un serveur pour héberger la base de données.  Sélectionnez **SERVEURS** puis **AJOUTER**.

1. Dans l'onglet **CONFIGURATION** du nouveau serveur, vous pouvez voir votre adresse IP. ﻿À côté de cette adresse, cliquez sur **﻿﻿﻿AJOUTER AUX ADRESSES IP AUTORISÉES**.
...
1. Ensuite, vous allez créer la base de données. Sous l'onglet **BASES DE DONNÉES**, cliquez sur **AJOUTER** dans la barre du bas.

1. Dans Visual Studio, vous allez installer les packages requis pour accéder aux bases de données SQL Server depuis Django dans l'environnement virtuel.

1. Cliquez avec le bouton droit sur **env**, puis cliquez sur **Installer le package Python** : **pyodbc** utilise **easy\_install**.

1. Cliquez avec le bouton droit sur **env**, puis cliquez sur **Installer le package Python** : **django-pyodbc-azure** utilise **pip**.

1. Modifiez **tutorial/settings.py** et remplacez la définition de **BASES DE DONNÉES** par la suivante, en remplaçant les valeurs **NOM**, **UTILISATEUR**, **MOT DE PASSE** et **HÔTE** par celles répertoriées dans le panneau de configuration ClearDB :
...
1. Synchronisez la base de données et créez les informations d'identification d'administration, comme pour la base de données sqlite3 locale.
...
Dans le Magasin Azure, vous pouvez ajouter différents services à votre compte, notamment une base de données MySQL.  Vous pouvez créer un compte d'évaluation gratuitement, et les petites bases de données sont gratuites jusqu'à une certaine taille.

1. Dans le portail de gestion Azure, sélectionnez **NOUVEAU**->**MAGASIN**->**SERVICES D'APPLICATION**->**Base de données MySQL ClearDB**.  Créez une base de données avec le plan Gratuit.

1. Ensuite, installez les packages requis pour accéder aux bases de données MySQL depuis Django dans l'environnement virtuel.  Cliquez avec le bouton droit sur **env**, puis sur **Install Python Package** : **mysql-python** utilise **easy\_install**.
...
1. Cliquez avec le bouton droit sur le nœud du projet et sélectionnez **Django** -> **Ouvrir l'interpréteur de commandes Django**.

1. Cette fenêtre interactive vous permet d'accéder à vos bases de données en utilisant vos modèles.  Entrez le code suivant pour ajouter un sondage à la base de données :
...
1. L'analyse statique des modèles offre une vue limitée de l'API complète.  Dans la fenêtre interactive, vous pouvez appliquer IntelliSense aux objets en direct. Cette méthode est excellente pour explorer l'API.  Voici quelques opérations à essayer dans la fenêtre interactive :
...
1. Démarrez le site.  Vous pouvez normalement voir le sondage que vous avez ajouté avec l'interpréteur de commandes Django.
...
- [Site Web](http://www.windowsazure.com/fr-fr/services/web-sites/)
- [Service cloud](http://www.windowsazure.com/fr-fr/services/cloud-services/)
- [Machine virtuelle](http://www.windowsazure.com/fr-fr/services/virtual-machines/)
...
1. Vous devez d'abord créer un site web.  Dans le portail de gestion Azure, cliquez sur **NOUVEAU**->**COMPUTE**->**SITE WEB**->**CRÉATION RAPIDE**.  Sélectionnez un nom disponible.

1. Une fois le site créé, téléchargez le profil de publication.
...
1. Dans Visual Studio, cliquez avec le bouton droit sur le nœud du projet, puis sélectionnez **Publier**.

	![Web Site Publish](./media/cloud-services-python-create-deploy-django-app/django-tutorial-020-website-publish.png)

1. Importez le fichier de profil de publication de site web téléchargé précédemment.

1. Acceptez la configuration par défaut, puis cliquez sur **Publier** pour démarrer la publication.

1. Une fois celle-ci terminée, un navigateur web affiche le site web publié.

	![Web Site Browser](./media/cloud-services-python-create-deploy-django-app/django-tutorial-020-website.png)

### Azure Cloud Services

1. Cliquez avec le bouton droit sur le nœud du projet, puis sélectionnez **Ajouter un projet de service cloud** ou **Convertir -> Convertir en projet de service cloud Azure** (vous pouvez voir l'une ou l'autre de ces options, en fonction de votre version de Visual Studio).  Ceci ajoute à la solution un nouveau projet portant le suffixe .Azure. Ce nouveau projet est marqué comme étant le projet de démarrage de la solution.
...
1. Démarrez le débogage en appuyant sur **F5**, puis l'application s'exécute et se déploie dans l'émulateur de calcul. Vérifiez que l'interface d'administration fonctionne et que vous pouvez voter dans les sondages.

	![Compute Emulator](./media/cloud-services-python-create-deploy-django-app/django-tutorial-021-cloudservice-emulator.png)

1. Vous pouvez maintenant redémarrer Visual Studio si vous ne voulez pas continuer l'exécution en tant qu'administrateur.
...
1. Vous allez maintenant procéder à la publication dans un service cloud Azure. Cliquez avec le bouton droit sur le projet de service cloud **tutorial.Azure**, puis cliquez sur **Publier**.
	...
	**Remarque :** veillez à sélectionner **Publier** sur le projet de service cloud. La boîte de dialogue **Publish Azure Application** s'affiche, permettant de publier dans un service cloud Azure.  Si vous sélectionnez **Publier** sur le projet Django, la boîte de dialogue **Publier le site web** s'affiche, permettant de publier dans un site web Azure.  

	![Cloud Service Publish](./media/cloud-services-python-create-deploy-django-app/django-tutorial-021-cloudservice-publish.png)

1. Vous devez importer votre fichier d'abonnement Azure.  Cliquez sur le lien [Se connecter pour télécharger les informations d'identification](https://manage.windowsazure.com/publishsettings/index?client=vs&schemaversion=2.0) pour le télécharger depuis le portail Azure.

1. Dans la page Paramètres, dans la zone de liste modifiable du service cloud, sélectionnez **Créer** pour créer un service cloud. Vous pouvez utiliser n'importe quel nom disponible.
...
1. Acceptez la configuration par défaut, puis cliquez sur **Publier**. Cette opération dure plus longtemps que la publication dans un site web, car le système doit configurer une machine virtuelle pour le service cloud. Vous pouvez afficher la progression de la publication dans la fenêtre Journal des activités Azure :
...
1. Une fois l'opération terminée, dans la fenêtre Journal des activités Azure, cliquez sur l'URL du site web pour ouvrir un navigateur web.

	![Cloud Service Browser](./media/cloud-services-python-create-deploy-django-app/django-tutorial-021-cloudservice.png)

## Conclusion

Dans ce didacticiel, vous avez développé une application Django avec [Python Tools for Visual Studio](http://pytools.codeplex.com).  Vous avez utilisé 3 types de base de données : sqlite3, SQL Server et MySQL.  Finalement, vous avez publié l'application dans un site web et un service cloud Azure.
