...
Prise en main de Media Services
===============================
...
Un projet C\# Visual Studio qui contient le code pour ce didacticiel est disponible ici : [Télécharger](http://go.microsoft.com/fwlink/?linkid=253275).
...
-   Un compte Media Services dans un abonnement Azure nouveau ou existant. Pour plus d'informations, consultez la page [Création d'un compte Media Services](http://go.microsoft.com/fwlink/?LinkId=256662).
...
-   Installez les bibliothèques **Kit de développement logiciel (SDK) Azure pour .NET**, **Kit de développement logiciel (SDK) d'Azure Media Services pour .NET** et **Services de données WCF 5.0 pour OData V3**, puis ajoutez les références à votre projet à l'aide du package [NuGet windowsazure.mediaservices](http://nuget.org/packages/windowsazure.mediaservices). La section suivante montre comment installer et ajouter ces références.
...
5.  Créez un dossier sur votre ordinateur local et nommez-le supportFiles (dans cet exemple, supportFiles est situé sous le répertoire du projet MediaServicesGettingStarted). Le [Projet](http://go.microsoft.com/fwlink/?linkid=253275) qui accompagne cette procédure pas à pas contient le répertoire supportFiles. Vous pouviez copier le contenu de ce répertoire dans votre dossier supportFiles.
...
7.  Ajoutez les variables de chemin d'accès de niveau classe suivantes. Le chemin d'accès **\_supportFiles** doit pointer sur le dossier que vous avez créé lors d'une étape précédente.
         ...
         // Chemin d'accès des fichiers de support de base  Mettez à jour ce champ afin de pointer vers le chemin d'accès de base  
         // pour le dossier des fichiers de support local que vous avez créé. 
         private static readonly string _supportFiles =
                     Path.GetFullPath(@"../..\supportFiles");
            
         // Chemins d'accès aux fichiers de support (dans le chemin d'accès de base ci-dessus). Vous pouvez utiliser 
         // les exemples de fichiers multimédias fournis dans le dossier « supportFiles » ou 
         // fournir les chemins d'accès à vos propres fichiers multimédias ci-dessous pour exécuter ces exemples.
...
9.  Ajoutez la variable de niveau classe suivante qui est utilisée comme référence statique au contexte de serveur.
         ...
         // Champ pour le contexte de service.
         private static CloudMediaContext _context = null;

Obtention du contexte Media Services
------------------------------------
...
Dans le fichier Program.cs, ajoutez le code suivant en tant que premier élément de votre méthode **Main**. Ce code utilise les valeurs de nom et de clé de votre compte Media Services à partir du fichier app.config pour créer une instance de contexte de serveur. L'instance est attribuée à la variable **\_context** que vous avez créée au niveau classe.
    ...
    // Obtenez le contexte de service.
    _context = new CloudMediaContext(_accountName, _accountKey);

Création d'un élément multimédia et téléchargement d'un fichier
---------------------------------------------------------------

Le code de cette section permet d'effectuer les opérations suivantes :

1.  Création d'un élément multimédia vide Lorsque vous créez des éléments multimédias, vous pouvez spécifier trois options différentes pour les chiffrer.
...
1.  Ajoutez la définition de méthode **CreateEncodingJob** suivante à votre classe. Cette méthode montre comment accomplir plusieurs tâches requises pour un travail d'encodage :
	...
	{    
	    ...
	    // Obtenez une référence de processeur multimédia, puis transmettez-lui le nom du processeur    
		// à utiliser pour la tâche spécifique.
        IMediaProcessor processor = GetLatestMediaProcessorByName("Azure Media Encoder");    

		// Créez une tâche avec les informations d'encodage à l'aide d'une chaîne de présélection.
        ITask task = job.Tasks.AddNew("My encoding task",
	        processor,
	        "H264 Broadband 720p",        Microsoft.WindowsAzure.MediaServices.Client.TaskOptions.ProtectedConfiguration);
	    // Spécifiez l'élément multimédia d'entrée à encoder.
        task.InputAssets.Add(asset);
	    // Ajoutez un élément multimédia de sortie pour contenir les résultats du travail. 
        // Cette sortie est spécifiée en tant que AssetCreationOptions.None, 
	    // ce qui signifie que l'élément multimédia de sortie n'est pas chiffré. 
        task.OutputAssets.AddNew("Output asset",        	AssetCreationOptions.None);    
		// Utilisez le gestionnaire d'événements suivant pour vérifier la progression du travail.  
	    ...
	    // (Facultatif) Journalisez les informations du travail. Cela affiche les informations du travail de base?    
		// sur la console et les enregistre dans un fichier JobDetails-{JobId}.txt 
	    // de votre dossier de sortie.
        LogJobDetails(job.Id);
	   
		 // Vérifiez l'exécution du travail et attendez la fin du travail. 
        Task progressJobTask = job.GetExecutionProgressTask(CancellationToken.None);
	    progressJobTask.Wait();

	    // **********
	    // Code facultatif.  Le code qui suit n'est pas nécessaire pour 
	    // un travail d'encodage, mais il montre comment accéder aux éléments multimédias  
	    // qui sont la sortie d'un travail, soit par la création d'URL vers ?    
		// l'élément multimédia sur le serveur, soit par téléchargement. 
        // **********

	    // Obtenez la référence d'un travail mis à jour.
        job = GetJob(job.Id);

	    // Si l'état du travail est Error, la méthode de 
	    // gestion des événements pour la progression du travail doit enregistrer les erreurs dans le journal.  Ici nous vérifions 
	    // l'état de l'erreur et quittons si besoin.
    ...
    {        
	    ...
	    // Obtenez une référence à l'élément multimédia de sortie à partir du travail.
        IAsset outputAsset = job.OutputMediaAssets[0];
	    IAccessPolicy policy = null;
	    ILocator locator = null;

	    // Déclarez une stratégie d'accès pour les autorisations sur l'élément multimédia. 
        // Vous pouvez appeler une méthode de création sync ou async. 
        policy =
	        _context.AccessPolicies.Create("My 30 days readonly policy",?            TimeSpan.FromDays(30),
            AccessPermissions.Read);

		// Créez un localisateur SAS pour activer l'accès direct à l'élément multimédia 
	    // dans le stockage d'objets blob. Vous pouvez appeler une méthode de création sync ou async.  
        // Vous pouvez définir le paramètre startTime facultatif sur 5 minutes 
	    // avant maintenant pour compenser la différence de temps ?    // entre l'horloge du client et celle du serveur. 

        locator = _context.Locators.CreateLocator(LocatorType.Sas, outputAsset,
	        policy,
	        DateTime.UtcNow.AddMinutes(-5));
	
	    // Créez une liste des URL SAS pour chaque fichier de l'élément multimédia. 
        List<String> sasUrlList = GetAssetSasUrlList(outputAsset, locator);
	
	    // Écrivez la liste des URL dans un fichier local. Vous pouvez utiliser les URL SAS enregistrées 
	    // pour accéder directement aux fichiers dans l'élément multimédia.
        if (sasUrlList != null)
    {
    ...
    {            
        ...
        // (Facultatif) Téléchargez la sortie sur l'ordinateur local.
...
3 .  Ajoutez les méthodes d'assistance suivantes à la classe. Elles sont nécessaires pour prendre en charge la méthode **CreateEncodingJob**. Ce qui suit est un résumé des méthodes d'assistance.
  -   La méthode **GetLatestMediaProcessorByName** renvoie un processeur multimédia approprié pour gérer l'encodage, le chiffrement ou d'autres tâches de traitement connexes. Vous créez un processeur multimédia à l'aide du nom de chaîne approprié pour le processeur à créer. Les chaînes pouvant être transmises à la méthode pour le paramètre mediaProcessor sont : **Azure Media Encoder**, **Azure Media Packager**, **Azure Media Encryptor** et **Storage Decryption**.
		...
		{
    		// Les chaînes pouvant être transmises à la    
			// méthode pour le paramètre mediaProcessor :
            //   Azure Media Encoder?    
...
-   Lorsque vous exécutez des travaux, vous avez généralement besoin de faire appel à une méthode de suivi de la progression du travail. L'exemple de code suivant définit un gestionnaire d'événements StateChanged. Ce dernier suit la progression du travail et fournit l'état mis à jour, selon l'état. Le code définit également la méthode LogJobStop. Cette méthode d'assistance journalise les détails de l'erreur.
		...
		{        case JobState.Finished:
                    Console.WriteLine();
		            Console.WriteLine("********************");            Console.WriteLine("Job is finished.");            Console.WriteLine("Please wait while local tasks or downloads complete...");
                ...
                case JobState.Error:
                    // Effectuez une conversion de type de l'expéditeur en tant que travail.
                    IJob job = (IJob)sender;
		            // Affichez ou journalisez les détails de l'erreur si besoin.
                    LogJobStop(job.Id);
		            break;
		        default:
                    break;
		 ...
		 }private static void LogJobStop(string jobId)
		 {
		    ...
		    builder.AppendLine("Job State: " + job.State.ToString());    builder.AppendLine("Job started (server UTC time): " + job.StartTime.ToString());
		    builder.AppendLine("Media Services account name: " + _accountName);
		    // Journalisez les erreurs du travail si elles existent.  
            if (job.State == JobState.Error)
	     {       
         ...
         {
                builder.AppendLine("  Task Id: " + task.Id);                builder.AppendLine("    Error Code: " + detail.Code);                builder.AppendLine("    Error Message: " + detail.Message + "\n");
          }
          }    
		  }    
		  builder.AppendLine("***************************\n");
		  // Écrivez la sortie dans un fichier local et dans la console. Le modèle 
	     // pour un fichier de sortie d'erreur est :  JobStop-{JobId}.txt    string outputFile = _outputFilesFolder + @"\JobStop-" + JobIdAsFileName(job.Id) + ".txt";
		...
		private static void LogJobDetails(string jobId)
		{
		    ...
		    // Écrivez la sortie dans un fichier local et dans la console. Le modèle 
		    // pour un fichier de sortie d'erreur est :  JobDetails-{JobId}.txt    string outputFile = _outputFilesFolder + @"\JobDetails-" + JobIdAsFileName(job.Id) + ".txt";
		    WriteToFile(outputFile, builder.ToString());
		    Console.Write(builder.ToString());
		}
...
-   Après avoir encodé des éléments multimédias dans Media Services, vous pouvez accéder aux éléments multimédias de sortie qui résultent d'un travail d'encodage. Cette procédure pas à pas vous montre deux façons d'accéder à la sortie d'un travail d'encodage :
    ...
    La méthode GetAssetSasUrlList crée une liste des URL SAS de tous les fichiers dans un élément multimédia.
		...
		{
    		// Déclarez une liste pour contenir toutes les URL SAS.
            List<String> fileSasUrlList = new List<String>();
		
		    // Si des fichiers sont dans l'élément multimédia, créez une liste des URL pour 
		    // chaque fichier dans l'élément multimédia et renvoyez-la. 
            foreach (IAssetFile file in asset.AssetFiles)
	    {
		    ...
		    // Renvoyez la liste des URL SAS.
            return fileSasUrlList;
		}

		// Créez et renvoyez une URL SAS pour un seul fichier dans un élément multimédia. 
        static string BuildFileSasUrl(IAssetFile file, ILocator locator)
		{
		    // Prenez le chemin d'accès au localisateur, ajoutez le nom du fichier, puis créez 
		    // une URL SAS complète pour accéder à ce fichier. Il s'agit du seul 
			// code nécessaire pour créer l'URL complète.
            var uriBuilder = new UriBuilder(locator.Path);
		    uriBuilder.Path += "/" + file.Name;

		    // Facultatif :  écrit le locator.Path de l'élément multimédia et 
		    // l'URL SAS complète du fichier
		    Console.WriteLine("Locator path: ");
		    Console.WriteLine(locator.Path);?    Console.WriteLine();
...
-   La méthode **DownloadAssetToLocal** télécharge chaque fichier de l'élément multimédia dans un dossier local. Dans cet exemple, comme l'élément multimédia a été créé avec un fichier multimédia d'entrée, la collection de fichiers d'élément multimédia de sortie contient deux fichiers : le fichier multimédia encodé (un fichier .mp4) et un fichier .xml avec les métadonnées sur l'élément multimédia. La méthode télécharge les deux fichiers.
		...
		{
		    // Cette méthode montre comment télécharger un élément multimédia unique. 
            // Toutefois, vous pouvez faire une itération dans la collection  OutputAssets
	        // et télécharger tous les éléments multimédias s'ils sont nombreux. 

            // Obtenez une référence au travail. 
            IJob job = GetJob(jobId);   
			// Obtenez une référence au premier élément multimédia de sortie. Si plusieurs éléments multimédias de sortie sont présents,
		    // vous pouvez faire une itération et gérer chacun d'entre eux.
            IAsset outputAsset = job.OutputMediaAssets[0];
	    ...
	    {
        	// Utilisez le gestionnaire d'événements suivant pour vérifier la progression du téléchargement.
	    ...
	    return outputAsset;?}??static void DownloadProgress(object sender, DownloadProgressChangedEventArgs e)
		{
...
-   Les méthodes d'assistance GetJob et GetAsset demandent et renvoient une référence à un objet travail et à un objet élément multimédia avec les ID spécifiés. Vous devez utiliser un type de requête LINQ similaire pour renvoyer des références aux autres objets Media Services sur le serveur.
		...
		{
		    // Utilisez une requête Linq select pour obtenir une référence 
		    // mise à jour par Id. 
            var jobInstance =
		        from j in _context.Jobs
		        where j.Id == jobId
		        select j;
		    // Renvoyez le référence du travail en tant qu'Ijob. 
