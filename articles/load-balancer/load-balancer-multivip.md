...
Toutefois, il existe des scénarios dans lesquels vous pouvez avoir besoin de plusieurs adresses IP virtuelles comme point d’entrée vers le même service cloud. Par exemple, votre service cloud peut héberger plusieurs sites Web qui nécessitent une connexion SSL à l’aide du port SLSL 443 par défaut, chaque site étant hébergé pour un autre client ou locataire. Dans ce scénario, vous devez avoir une adresse IP publique différente pour chaque site Web. Le diagramme suivant présente un hébergement Web mutualisé typique nécessitant plusieurs certificats SSL sur le même port public.
...
La commande ci-dessus affiche un résultat semblable à l’exemple ci-dessous :
...
Le script ci-dessus affiche un résultat semblable à l’exemple ci-dessous :
