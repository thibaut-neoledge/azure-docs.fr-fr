...
<properties
   pageTitle="Prise en main de l’équilibrage de charge accessible sur Internet | Microsoft Azure"
   description="Obtenez votre premier jeu d’équilibrage de charge accessible sur Internet pour vos machines virtuelles ou vos services cloud."
...
## Configuration d’un équilibrage de charge accessible sur Internet pour les machines virtuelles
...
Suivez les étapes ci-dessous pour configurer le jeu d’équilibrage de la charge pour les machines virtuelles :

1. Dans le portail de gestion Azure, cliquez sur Machines virtuelles, puis sur le nom d’une machine virtuelle du jeu d’équilibrage de la charge.
...
4.	Dans la page Spécifier les détails du point de terminaison :
	- Dans Nom, tapez le nom du point de terminaison ou sélectionnez-en un dans la liste des points de terminaison prédéfinis pour les protocoles communs.
...
8.	Dans le portail de gestion, cliquez sur Machines virtuelles, sur le nom d’une machine virtuelle supplémentaire du jeu d’équilibrage de la charge, sur Points de terminaison, puis sur Ajouter.
...
## Configuration d’un équilibrage de charge accessible sur Internet pour les services cloud
...
Vous pouvez utiliser le Kit de développement logiciel (SDK) Azure pour .NET 2.5 pour mettre à jour votre service cloud. Les paramètres de point de terminaison des services cloud sont définis dans [service definition](https://msdn.microsoft.com/library/azure/gg557553.aspx).csdef.
...
### Vérification de l’état d’intégrité de l’équilibrage de charge pour les services cloud
...
L’équilibrage de charge combine les informations du point de terminaison et de la sonde pour créer une URL sous la forme de http://{DIP de VM}:80/Probe.aspx pour interroger l’intégrité du service.

Le service voit que la même adresse IP y accède régulièrement. Il s’agit de la requête de la sonde d’intégrité en provenance de l’hôte du nœud sur lequel la machine virtuelle s’exécute. Le service doit répondre avec un code d’état HTTP 200 pour que l’équilibrage de charge suppose que le service est sain. Tout autre code d’état HTTP (par exemple, 503) met directement la machine virtuelle hors service.
