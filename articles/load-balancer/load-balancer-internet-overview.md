...
>[AZURE.NOTE] Azure Load Balancer fournit un trafic de réseau de distribution par hachage sur plusieurs instances de machines virtuelles suivant les paramètres par défaut (pour en savoir plus sur la distribution par hachage, consultez les [fonctionnalités de l’équilibreur de charge](load-balancer-overview.md#load-balancer-features) . Si vous cherchez une affinité de session, consultez la rubrique [Mode de distribution de l’équilibreur de charge](load-balancer-distribution-mode.md).
