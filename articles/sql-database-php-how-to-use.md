<properties urlDisplayName="SQL Database" pageTitle="Utilisation d'une base de données SQL (PHP) - Guides de fonctionnalités Azure" metaKeywords="Base de données SQL Azure PHP, Base de données SQL PHP" description="Découvrez comment créer une base de données SQL Azure et comment s'y connecter à partir de PHP." metaCanonical="" services="sql-database" documentationCenter="PHP" title="How to Access Azure SQL Database from PHP" authors="tomfitz" solutions="" manager="wpickett" editor="mollybos" videoId="" scriptId="" />
