...
-   Windows PowerShell, version 3.0 ou ultérieure. Pour trouver la version de Windows PowerShell requise, tapez : `$PSVersionTable` et vérifiez que vous êtes équipé de `PSVersion` 3.0 au minimum. Pour installer une version plus récente, accédez à la page [Windows Management Framework 3.0](http://www.microsoft.com/en-us/download/details.aspx?id=34595) ou [Windows Management Framework 4.0](http://www.microsoft.com/en-us/download/details.aspx?id=40855).

-   Azure PowerShell, version 0.8.0 ou ultérieure. Pour installer la dernière version et l'associer à votre abonnement Azure, accédez à la page [Installation et configuration d'Azure PowerShell](http://www.windowsazure.com/en-us/documentation/articles/install-configure-powershell/).
...
-   **Azure** : comprend les cmdlets traditionnelles de gestion de ressources spécifiques (comptes de stockage, sites Web, bases de données, machines virtuelles et services de médias). Pour plus d'informations, consultez la page [Cmdlets de gestion des services Azure](http://msdn.microsoft.com/en-us/library/jj152841.aspx).

-   **AzureResourceManager** : comprend les cmdlets de création, de gestion et de déploiement des ressources Azure pour un groupe de ressources. Pour plus d'informations, consultez la page [Cmdlets du gestionnaire des ressources Azure](http://go.microsoft.com/fwlink/?LinkID=394765).

-   **AzureProfile** : comprend les cmdlets communes aux deux modules, comme Add-AzureAccount, Get-AzureSubscription et Switch-AzureMode. Pour plus d’informations, consultez la page [Cmdlets de profil Azure](http://go.microsoft.com/fwlink/?LinkID=394766).

> [ WACOM.NOTE] Le module Azure Resource Manager est actuellement disponible dans une version préliminaire. Il peut ne pas fournir les mêmes fonctionnalités de gestion que le module Azure Service Management.
...
Si vous utilisez Azure PowerShell, les cmdlets des modules Azure et Azure Profile sont importées par défaut. Pour passer au module Azure Resource Manager, utilisez la cmdlet Switch-AzureMode. Elle retire le module Azure de votre session et importe les modules Azure Resource Manager (et Azure Profile).
...
Pour obtenir de l’aide sur la cmdlet Switch-AzureMode, tapez : `Get-Help Switch-AzureMode` ou reportez-vous à [Switch-AzureMode](http://go.microsoft.com/fwlink/?LinkID=394398).
...
Création d'un groupe de ressources pour un site Web et une base de données
==========================================================================

Cette section du didacticiel vous accompagne tout au long du processus de création et de déploiement d'un groupe de ressources pour un site Web avec une base de données SQL.
...
Le modèle Microsoft.WebSiteSQLDatabase.0.1.0-preview1.json constitue une approche intéressante. Pour obtenir plus d’informations sur un modèle de la galerie, utilisez le paramètre **Identity**. La valeur du paramètre Identity est l'identité du modèle.
...
La cmdlet renvoie un objet contenant des informations beaucoup plus détaillées (avec un descriptif, notamment) sur le modèle.
    ...
    <p>Le service Sites Web Azure permet un développement et un déploiement flexibles et sécurisés, 
    ainsi que des options de mise à l'échelle pour les applications Web de toutes tailles. Exploitez 
    vos outils existants pour créer et déployer des applications sans les contraintes 
    de gestion d'une infrastructure.</p>
...
La section **resources** du modèle répertorie les ressources que le modèle crée. Ce modèle crée un serveur de base de données SQL et une base de données SQL, une batterie de serveurs et un site Web, ainsi qu'un certain nombre de paramètres d'administration.
...
Il a suffi de quelques étapes seulement pour créer et déployer les ressources requises pour un site Web complexe. Le modèle de la galerie a fourni la quasi-totalité des informations dont nous avions besoin pour effectuer cette tâche. Et, avantage non négligeable : la tâche est facilement automatisée.
...
-   [Cmdlets Azure Resource Manager](http://go.microsoft.com/fwlink/?LinkID=394765&clcid=0x409) : apprenez à utiliser les cmdlets dans les modules AzureResourceManager.
...
-   [Blog Hey, Scripting Guy!](http://blogs.technet.com/b/heyscriptingguy/) : bénéficiez des conseils et astuces de la communauté.
-   [Utilisation de l'interface de ligne de commande interplateforme Azure avec le gestionnaire des ressources](http://www.windowsazure.com/en-us/documentation/articles/xplat-cli-azure-resource-manager/) : découvrez d'autres méthodes permettant d'automatiser les opérations du gestionnaire des ressources.
