<properties urlDisplayName="iOS Media Player Framework" pageTitle="Utilisation d'iOS Media Player Framework avec Azure Media Services" metaKeywords="" description="Apprenez &agrave; utiliser la biblioth&egrave;que Media Services iOS Media Player Framework pour cr&eacute;er des applications riches et dynamiques." metaCanonical="" services="media-services" documentationCenter="" title="Utilisation d'Azure Media Services iOS Media Player Framework" authors="juliako" solutions="" manager="dwrede" editor="" />
...
![structure d'exemple de code HLS][structure d'exemple de code HLS]
...
-   Le contrôle **Player time** affiche deux heures (`Label:playerTime`), par exemple 00:23/02:10. Ici, 00:23 est l'heure de lecture actuelle et 02:10 la durée totale du média.
...
-   **clipEndMediaTime** représente l'heure de fin de lecture de la vidéo. Si **clipEndMediaTime**=100, la lecture de la vidéo se termine à la 100e seconde du clip vidéo.
    \*Nous avons planifié **MediaTime** en indiquant à l'infrastructure **appendContentClip**. Dans l'exemple ci-dessus, `[NSURL URLWithString:url]` donne l'URL du contenu principal et la planification de ce média est définie grâce à **withMedia** :
...
-   Toute la durée **LinearTime.duration** doit être sur 0 lors de la planification d'une publicité.
-   Lorsque **clipEndMediaTime** est plus long que la durée de la publicité, celle-ci se termine après et aucune exception n'est levée. Il vous est recommandé de vérifier si la durée naturelle de la publicité ne dépasse pas la durée de rendu (**clipEndMediaTime**) pour éviter de perdre une occasion de diffuser la publicité.
-   Les publicités en pre-roll, mid-roll et post-roll sont prises en charge. Les pre-rolls peuvent uniquement être planifiés au tout début d'un contenu. Par exemple, vous ne pouvez pas planifier un pre-roll pour un deuxième contenu dans un scénario RCE (rough cut editing).
-   Les publicités permanentes et celles à affichage unique sont prises en charge et peuvent être utilisées avec les publicités en pre-roll, en mid-roll ou en post-roll.
-   Les publicités peuvent être au format .Mp4 ou HLS.
...
-   La propriété **mediaTime** indique la durée d'apparition de la publicité. (**clipBeginMediaTime** est l'heure de début d'une publicité et **clipEndMediaTime** définit la fin de la publicité.) Dans l'exemple de code ci-dessus, nous planifions une publicité de 5 secondes qui s'étend de la 0e à la 5e seconde de la durée de la publicité.
...
-   Pour le premier clip, **appendTo** a la valeur -1. Lorsque nous appelons `[framework scheduleClip:adpodInfo1 atTime:adLinearTime forType:PlaylistEntryType_Media andGetClipId:&adIndex]`, `adIndex` reçoit une valeur unique qui indique la fin de ce premier clip dans l'ad pod. Ensuite, pour le deuxième clip du pod publicitaire, alignez le début de la deuxième publicité avec la fin de la première en définissant **appendTo** comme `adpodInfo2.appendTo = adIndex;`, ce qui précise que la position de fin du premier clip correspond à l'emplacement du début du deuxième.
...
### Pour plus d'informations, consultez la rubrique [wiki de l'infrastructure Azure Media Player][wiki de l'infrastructure Azure Media Player].
  ...
  [wiki de l'infrastructure Azure Media Player]: https://github.com/WindowsAzure/azure-media-player-framework/wiki
