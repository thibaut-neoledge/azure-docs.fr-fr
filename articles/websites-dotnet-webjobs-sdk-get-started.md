<properties pageTitle="Prise en main du Kit de développement logiciel (SDK) Azure WebJobs" metaKeywords="Azure tutorial, Azure WebJobs tutorial, Azure multi-tier tutorial, MVC tutorial, Azure blobs tutorial, Azure queues tutorial, Azure storage tutorial" description="Découvrez comment créer une application multiniveau avec ASP.NET MVC et Azure. Le serveur client s'exécute dans un site web et le serveur principal s'exécute comme une tâche Web. L'application utilise Entity Framework, la base de données SQL, ainsi que les files d'attente et objets blobs du stockage Azure." metaCanonical="" services="web-sites,storage" documentationCenter=".NET" title="Get Started with the Azure WebJobs SDK" authors="tdykstra" solutions="" manager="wpickett" editor="mollybos" />
...
Les tâches web WebJobs s'exécutent dans le contexte d'un site web et ne sont pas mises à l'échelle séparément. Par exemple, si vous avez une instance de site web Standard, vous ne pouvez avoir qu'une seule instance en cours d'exécution du processus en arrière-plan. Elle utilise des ressources du serveur (processeur, mémoire, etc.) qui seraient disponibles d'une autre manière pour distribuer du contenu web. 

Si le trafic varie pendant la journée ou la semaine et si le traitement principal dont vous avez besoin peut attendre, vous pouvez planifier l'exécution de vos tâches web WebJobs aux heures de faible trafic. Si la charge est alors encore trop importante pour cette solution, vous pouvez envisager d'autres environnements pour votre programme principal. Exemples :

* Exécution du programme en tant que tâche web WebJob dans un site web séparé dédié à cette tâche. Vous pouvez alors mettre à l'échelle votre site web principal indépendamment de votre site web frontal.
...
Ce didacticiel explique comment exécuter le programme frontal dans un site web et le programme principal en tant que tâche web WebJob dans le même site web. Pour plus d'informations sur le choix du meilleur environnement pour votre scénario, consultez la rubrique [Comparaison entre Sites Web Azure, Azure Cloud Services et Azure Virtual Machines](/fr-fr/documentation/articles/choose-web-site-cloud-service-vm/).

[WACOM.INCLUDE [install-sdk-2013-only](../includes/install-sdk-2013-only.md)]

Les instructions du didacticiel ont été écrites à l'aide de la prochaine version préliminaire de [Visual Studio 2013 Update 4](http://go.microsoft.com/fwlink/?LinkID=510328). La seule différence pour Visual Studio 2013 Update 3 tient à la section Création intégrale d'un projet de tâche web WebJob : avec Update 4, les packages du Kit de développement logiciel (SDK) WebJobs sont automatiquement inclus dans le projet ; sans Update 4, vous devez les installer manuellement.
...
1. Téléchargez et décompressez la [solution terminée][téléchargement].
...
2. Ouvrez le fichier d'application *Web.config* dans le projet ContosoAdsWeb.
	...
	La chaîne de connexion de stockage est un exemple avec des espaces réservés pour la clé d'accès et le nom du compte de stockage. Vous allez le remplacer par une chaîne de connexion qui a le nom et la clé de votre compte de stockage.  
	...
	La chaîne de connexion de stockage est nommée AzureWebJobsStorage, car il s'agit du nom que le Kit de développement logiciel (SDK) WebJobs utilise par défaut. Nous utilisons ce nom ici pour qu'il ne vous reste plus qu'à définir une valeur de chaîne de connexion dans l'environnement Azure.
...
4. Dans la fenêtre **Propriétés**, cliquez sur **Clés de compte de stockage**, puis cliquez sur le bouton de sélection.
...
8. Remplacez la chaîne de connexion de stockage du fichier *Web.config* par la chaîne de connexion que vous venez de copier. Veillez à sélectionner tous les éléments entre guillemets, sans inclure les guillemets avant le collage.

4. Ouvrez le fichier *App.config* dans le projet ContosoAdsWebJob.

	Ce fichier comporte deux chaînes de connexion de stockage : une pour les données de l'application et une pour la journalisation. Pour ce didacticiel, vous allez utiliser le même compte pour les deux. Les chaînes de connexion ont des espaces réservés pour les clés de compte de stockage.
...
&lt;/configuration&gt;</pre>
	...
	Par défaut, le Kit de développement logiciel (SDK) WebJobs recherche les chaînes de connexion AzureWebJobsStorage et AzureWebJobsDashboard. Vous pouvez également [stocker la chaîne de connexion comme vous le souhaitez et la transmettre explicitement à l'objet " JobHost "](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#config).

1. Remplacez les chaînes de connexion de stockage par la chaîne de connexion que vous avez copiée précédemment.
...
2. Pour démarrer la tâche web WebJob principale de l'application, cliquez avec le bouton droit sur le projet ContosoAdsWebJob dans l'**Explorateur de solutions**, puis sur **Débogage** > **Démarrer une nouvelle instance**.
...
2. Entrez des données de test et sélectionnez une image à télécharger, puis cliquez sur **Créer**.
	...
	Entre-temps, après une brève attente, un message dans la fenêtre d'application console indique qu'un message de file d'attente a été reçu et traité.   
...
3. Lorsque ces messages s'affichent dans la fenêtre d'application console, actualisez la page Index pour afficher la vignette.

	![Index page](./media/websites-dotnet-webjobs-sdk-get-started/list.png)

4. Cliquez sur l'option **Détails** correspondant à votre publicité pour afficher l'image en taille réelle.
...
* Déployez vers un site web Azure. Visual Studio crée automatiquement un site web Azure et une instance de base de données SQL.
...
3. À l'étape **Profil** de l'Assistant **Publication du site web**, cliquez sur **Sites web Microsoft Azure**.
...
1. Entrez un **Nom d'utilisateur de base de données** et un **Mot de passe de base de données** d'administrateur. 

	Si vous avez sélectionné **Nouveau serveur de base de données SQL**, vous ne devez pas entrer un nom et un mot de passe existant ici, mais une nouvelle paire nom/mot de passe que vous allez choisir maintenant et utiliser ultérieurement quand vous accèderez à la base de données. Si vous avez sélectionné un serveur créé auparavant, vous devez entrer le mot de passe du compte d'utilisateur administratif déjà créé.
...
2. Dans l'étape de **connexion** de l'Assistant **Publication web**, cliquez sur **Suivant**.

	![Connection step](./media/websites-dotnet-webjobs-sdk-get-started/connstep.png)	

3. À l'étape **Paramètres**, sélectionnez **Utiliser cette chaîne de connexion au moment de l'exécution**, puis cliquez sur **Suivant**.
...
4. À l'étape **Aperçu**, cliquez sur **Démarrer l'aperçu**.
	...
	La fenêtre de l'aperçu indique que les fichiers binaires et de configuration du projet de tâche web WebJob sont copiés dans le dossier *app_data\jobs\continuous* du site web.
...
7. Dans l'**Explorateur de serveurs**, cliquez avec le bouton droit sur votre site web sous le nœud **Sites Web**, puis cliquez sur **Afficher les paramètres**.

	La fenêtre **Site Web Azure** s'ouvre dans l'onglet **Configuration**.

9. Remplacez le nom de la chaîne de connexion DefaultConnection par ContosoAdsContext.

	Azure a automatiquement créé cette chaîne de connexion quand vous avez créé le site avec une base de données associée ; il a donc la valeur correcte de la chaîne de connexion. Vous remplacez simplement le nom par celui que votre code recherche.

9. Ajoutez deux chaînes de connexion nommées AzureWebJobsStorage et AzureWebJobsDashboard. Définissez le type sur Personnalisé et configurez la valeur de la chaîne de connexion avec celle que vous avez utilisée auparavant pour les fichiers *Web.config* et *App.config*. Vérifiez que vous incluez la chaîne de connexion complète, et pas uniquement la clé d'accès, sans guillemet.
...
12. Après l'arrêt du site web, cliquez à nouveau à l'aide du bouton droit, puis cliquez sur **Démarrer le site web**.

	La tâche web WebJob démarre automatiquement quand vous publiez, mais elle s'arrête quand vous modifiez la configuration. Pour la redémarrer, vous pouvez redémarrer le site ou la tâche web WebJob dans le portail de gestion Azure. Il est généralement conseillé de redémarrer le site après une modification de la configuration. 
...
11.	Actualisez la page après quelques secondes : la vignette s'affiche.
	...
	Si la miniature ne s'affiche pas, la tâche web WebJob n'a peut-être pas démarré automatiquement. Dans ce cas, accédez à l'onglet Tâches web dans le 
...
3. Cliquez sur l'URL dans la colonne Journaux de votre tâche web.
	...
	Un nouvel onglet ouvre le tableau de bord du Kit de développement logiciel (SDK) WebJobs dans le navigateur. Le tableau de bord indique que la tâche web WebJob est en cours d'exécution et affiche la liste des fonctions de votre code que le Kit de développement logiciel (SDK) a déclenchées.
...
11. Cliquez avec le bouton droit sur le projet web (et non sur la solution ou le projet de bibliothèque de classes), puis cliquez sur **Ajouter** > **Nouveau projet de tâche web Azure WebJobs**.

	![New Azure WebJob Project menu selection](./media/websites-dotnet-webjobs-sdk-get-started/newawjp.png)	

1. Dans la boîte de dialogue **Ajouter une tâche web Azure WebJobs**, entrez ContosoAdsWebJob pour le **Nom du projet** et le **Nom de la tâche web WebJob**. Laissez **Mode d'exécution de la tâche web WebJob** sur **Exécuter en continu**.

2.  Cliquez sur **OK**.
  
	Visual Studio crée une application console configurée pour se déployer comme une tâche web WebJob quand vous déployez le projet web. Pour cela, il a effectué les tâches suivantes après la création du projet :

	* ajout d'un fichier *webjob-publish-settings.json* dans le dossier des propriétés du projet de tâche web WebJob ;
	...
	* installation du package NuGet Microsoft.Web.WebJobs.Publish dans le projet de tâche web.
	...
	Pour plus d'informations sur ces modifications, consultez la rubrique [Déploiement de tâches web WebJobs en utilisant Visual Studio](/fr-fr/documentation/articles/websites-dotnet-deploy-webjobs/).

### Ajout de packages NuGet

Le nouveau modèle de projet pour un projet de tâche web WebJob installe automatiquement le package NuGet du Kit de développement logiciel (SDK) WebJobs [Microsoft.Azure.WebJobs](http://www.nuget.org/packages/Microsoft.Azure.WebJobs) et ses dépendances. 

Une des dépendances du Kit de développement logiciel (SDK) WebJobs installée automatiquement dans le projet WebJobs est la bibliothèque cliente Azure Storage (SCL, Storage Client Library). Toutefois, vous devez l'ajouter au projet web pour travailler avec des objets blob et des files d'attente.
...
Pour ajouter des fichiers à un projet ou à un dossier, cliquez avec le bouton droit sur ce dernier, puis cliquez sur **Ajouter** > **Élément existant**. Sélectionnez les fichiers et cliquez sur **Ajouter**. Si un message vous demande si vous souhaitez remplacer les fichiers existants, cliquez sur **Oui**.
...
Vous pouvez maintenant générer, exécuter et déployer l'application en suivant les instructions fournies précédemment dans ce didacticiel. Cependant, avant cela, arrêtez la tâche web toujours en cours d'exécution dans le premier site web dans lequel vous l'avez déployée. Sinon, cette tâche web WebJob traite les messages de file d'attente créés localement ou par l'application en cours d'exécution dans un nouveau site web, car ils utilisent tous le même compte de stockage.

## <a id="code"></a>Révision du code de l'application

Les sections suivantes présentent le code utilisé avec le Kit de développement logiciel (SDK) WebJobs et des objets blob et des files d'attente Azure Storage. Pour le code propre au Kit de développement logiciel (SDK) WebJobs, consultez la [section Program.cs](#programcs).
...
Le code appelé par la méthode " Application_Start " crée un conteneur d'objets blob *images* et une file d'attente *images* s'ils n'existent pas déjà. Quand vous commencez à utiliser un nouveau compte de stockage, le conteneur d'objets blob et la file d'attente nécessaires sont créés automatiquement.

Le code a accès au compte de stockage en utilisant la chaîne de connexion de stockage du fichier *Web.config* ou l'environnement d'exécution Azure.
...
Du code similaire obtient une référence à la file d'attente *blobnamerequest* et crée une file d'attente. Dans ce cas, aucune modification des autorisations n'est nécessaire. Plus loin dans ce didacticiel, la section [ResolveBlobName](#resolveblobname) explique pourquoi la file d'attente dans laquelle l'application web écrit est utilisée uniquement pour obtenir les noms des objets blob et non pour créer des miniatures.
...
Les fichiers *Create.cshtml* et *Edit.cshtml* spécifient l'encodage de forme qui permet au contrôleur d'obtenir l'objet " HttpPostedFileBase ".
...
Quand la tâche web WebJob démarre, la méthode " Main " appelle " Initialize " pour instancier le contexte de base de données Entity Framework. Elle appelle ensuite la méthode du Kit de développement logiciel (SDK) WebJobs pour commencer l'exécution dans un seul thread des fonctions déclenchées dans le thread actif.
...
Le Kit de développement logiciel (SDK) WebJobs appelle cette méthode quand un message de file d'attente est reçu. Cette méthode crée une miniature et place son URL dans la base de données.
...
* L'attribut " QueueTrigger " indique au Kit de développement logiciel (SDK) WebJobs d'appeler cette méthode quand un nouveau message est reçu dans la file d'attente thumbnailrequest.
...
* Les deux attributs " Blob " fournissent des objets liés à des objets blob : un à l'objet blob d'image existant et l'autre à un nouvel objet blob de vignette que la méthode crée. 
	...
	Les noms des objets blob proviennent des propriétés de l'objet " BlobInformation " reçues dans le message de file d'attente (" BlobName " et " BlobNameWithoutExtension "). Pour profiter de toutes les fonctionnalités de la bibliothèque cliente de stockage, vous pouvez faire appel à la classe " CloudBlockBlob " pour utiliser des objets blob. Si vous voulez réutiliser du code écrit pour utiliser des objets " Stream ", vous pouvez faire appel à la classe " Stream ". 

>[WACOM.NOTE] 
>* Si votre site web s'exécute sur plusieurs machines virtuelles, ce programme s'exécute sur chaque machine, qui attend des déclencheurs et essaie d'exécuter les fonctions. Dans certains scénarios, certaines fonctions traitent deux fois les mêmes données ; ces fonctions doivent donc être idempotentes (écrites de façon que, si elles sont appelées de manière répétitive avec les mêmes données d'entrée, elles ne produisent pas des résultats en double).
>* Pour plus d'informations sur la façon d'implémenter l'arrêt progressif, consultez la rubrique [Arrêt progressif](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#graceful).   
...
Pour plus d'informations, consultez la rubrique [Ressources recommandées pour Azure WebJobs](http://go.microsoft.com/fwlink/?LinkId=390226).
