---
title: "Configuration de la connectivité SSL dans votre application pour se connecter en toute sécurité à la base de données Azure pour MySQL | Microsoft Docs"
...
Par défaut, le service de base de données doit être configuré pour exiger des connexions SSL lors de la connexion au serveur Azure Database pour MySQL.  Il est recommandé d’éviter de désactiver l’option SSL chaque fois que possible. 
...
De nombreuses applications courantes qui utilisent des services de base de données MySQL, notamment Wordpress, Drupal et Magento, n’activent pas le protocole SSL par défaut lors de l’installation.  La connectivité SSL doit être activée après l’installation ou via les commandes CLI spécifiques à l’application.  Si votre serveur MySQL applique des connexions SSL et que l’application associée n’est pas correctement configurée, cette application risque de ne pas pouvoir se connecter à votre serveur de base de données.  Consultez la documentation de votre application pour savoir comment activer les connexions SSL.
...
### <a name="download-the-certificate-file-from-the-certificate-authority-ca"></a>Téléchargement du fichier de certificat émis par l’autorité de certification (CA) 
Le certificat nécessaire pour communiquer via le protocole SSL avec votre serveur Azure Database pour MySQL se trouve [ici](https://www.digicert.com/CACerts/BaltimoreCyberTrustRoot.crt).  Téléchargez le fichier de certificat sur votre disque local (pour ce didacticiel, nous utiliserons **c:\ssl**).
...
Les sections suivantes décrivent la procédure que vous pouvez utiliser, depuis un PC Linux ou Windows, selon votre système d’exploitation favori (vous ne devez exécuter qu’une seule méthode).
...
Les bibliothèques OpenSSL sont fournies dans le code source directement à partir d[’OpenSSL Software Foundation](http://www.openssl.org).  Les instructions suivantes vous guident tout au long des étapes d’installation d’OpenSSL sur votre PC Linux.  Pour ce guide, nous afficherons les commandes pour Ubuntu 12.04 et versions ultérieures.

Ouverture d’une session terminal et installation d’OpenSSL
```bash
...
```
Entrez le répertoire où les fichiers ont été extraits.  Par défaut, le résultat devrait ressembler à ceci.

```bash
cd openssl-1.1.0e
```
Configurez OpenSSL en exécutant la commande suivante : si vous souhaitez placer les fichiers dans un autre dossier que /usr/local/openssl, veillez à modifier les valeurs suivantes comme il convient.
...
Le fichier CA racine sera téléchargé au format **.cer**. Vous devez utiliser OpenSSL pour convertir le fichier de certificat en un fichier **.pem**.  Pour ce faire, exécutez l’outil de ligne de commande openssl.exe puis la commande suivante : 
