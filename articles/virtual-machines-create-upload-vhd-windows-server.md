...
> [AZURE.NOTE] Lorsque vous créez une machine virtuelle reposant sur une image, vous pouvez personnaliser les paramètres du système d'exploitation selon les applications que vous prévoyez d'exécuter sur la machine virtuelle. Cette configuration est enregistrée pour cette machine virtuelle et n'a pas d'incidence sur l'image. Pour obtenir des instructions, consultez le guide [Création d'une machine virtuelle personnalisée](http://azure.microsoft.com/documentation/articles/virtual-machines-windows-tutorial/).

##Configuration requise##
Cet article part du principe que vous disposez des éléments suivants :

1. **Un abonnement Azure** : si vous n'en possédez pas, vous pouvez créer un compte d'évaluation gratuit en quelques minutes. Pour plus d'informations, consultez la page [Création d'un compte Azure](http://azure.microsoft.com/develop/php/tutorials/create-a-windows-azure-account/).  

2. **Microsoft Azure PowerShell** : le module Microsoft Azure PowerShell est installé et configuré de façon à utiliser votre abonnement. Pour télécharger le module, consultez la page [Téléchargements Microsoft Azure](http://azure.microsoft.com/downloads/). Un didacticiel relatif à l'installation et la configuration du module est disponible [ici](http://azure.microsoft.com/documentation/articles/install-configure-powershell/). Vous utiliserez la cmdlet [Add-AzureVHD](http://msdn.microsoft.com/library/azure/dn495173.aspx) pour charger le disque dur virtuel.
...
6. À l'issue de ces étapes, la nouvelle image apparaît sous l'onglet **Images**. 
	...
	Cette nouvelles image est maintenant disponible dans **Mes Images** lorsque vous créez une machine virtuelle. Pour obtenir des instructions, consultez le didacticiel [Création d'une machine virtuelle exécutant Windows Server](http://azure.microsoft.com/documentation/articles/virtual-machines-windows-tutorial/).
