...
Cet article fait partie d'une série qui présente la programmation Azure Media Services. La rubrique précédente s'intitulait [Gestion des éléments multimédias](http://go.microsoft.com/fwlink/?LinkID=301815&clcid=0x409).
...
// Téléchargez l'élément multimédia de sortie de la tâche spécifiée vers un dossier local.
static IAsset DownloadAssetToLocal( string jobId, string outputFolder)
{
    // Cette méthode illustre comment télécharger un élément multimédia unique. 
    // Vous pouvez toutefois lancer une itération dans l'ensemble OutputAssets
    // et télécharger tous les éléments multimédias s'il y en a plusieurs. 

    // Obtenez une référence au travail. 
    IJob job = GetJob(jobId);

    // Obtenez une référence pour le premier élément multimédia en sortie. S'il y en avait plusieurs, 
    // vous pouvez itérer et gérer chaque élément.
    IAsset outputAsset = job.OutputMediaAssets[0];

    // Créez un localisateur SAS pour télécharger l'élément multimédia
    ...
    {
        // Utilisez le gestionnaire d'événements suivant pour suivre la progression du téléchargement.
...
Cette rubrique vous a indiqué comment télécharger un élément multimédia depuis Azure Storage. Pour plus d'informations sur les autres méthodes possibles pour fournir des éléments multimédias, consultez la page [Fourniture de contenu de diffusion en continu](http://go.microsoft.com/fwlink/?LinkID=301942).
