<properties
   pageTitle="Déploiement continu d’Azure Automation DSC avec Chocolatey | Microsoft Azure"
...
# Déploiement continu sur les machines virtuelles IaaS à l’aide d’Azure Automation DSC et Chocolatey
...
Métadonnées : cet article a été rédigé en septembre 2015. La version Visual Studio utilisée est celle de 2015. [PowerShell Tools for Visual Studio](http://adamdriscoll.github.io/poshtools/) est installé pour faciliter l’utilisation de PowerShell grâce à la coloration syntaxique et à la fonctionnalité Intellisense. Azure Automation DSC étant actuellement au stade de version préliminaire publique, vous risquez de constater des modifications après son passage à l’état de disponibilité générale. La [source complète](https://github.com/sebastus/ARM/tree/master/CDIaaSVM) pour cet exemple se trouve sur GitHub.
...
Vraisemblablement, vous le faites déjà au départ, ou du moins la majeure partie. Créer un nuspec, le compiler et le stocker dans un serveur NuGet n’a rien de compliqué. Et vous gérez déjà des machines virtuelles. L’étape suivante du déploiement continu suppose de configurer le serveur d’extraction (une fois), d’y enregistrer vos nœuds (une fois) et d’y créer et stocker la configuration (au début). Au fur et à mesure que les packages sont mis à niveau et déployés dans le référentiel, vous devez actualiser la configuration dans le serveur d’extraction (et répéter l’opération si besoin).

Si vous ne commencez pas par un modèle ARM, il n’y a aucun problème. Il existe des applets de commande PowerShell conçues pour vous aider à enregistrer vos machines virtuelles auprès du serveur d’extraction et de tous les autres serveurs.
...
![Exemple de la PowerShell Gallery](./media/automation-dsc-cd-chocolatey/cchoco.png)

Il existe aussi une approche manuelle. La structure de dossier d’un module d’intégration PowerShell pour un ordinateur Windows est un peu différente de celle à laquelle s’attend le serveur Pull Azure Automation DSC. Cette différence nécessite une légère modification de votre part. Mais il n’y a là rien de compliqué, et vous n’avez à effectuer cette opération qu’une seule fois par ressource (sauf si vous souhaitez effectuer une mise à niveau ultérieurement).

-   Installez le module dont vous avez besoin sur votre station de travail, comme suit :
    -   Installez [Windows Management Framework v5](http://aka.ms/wmf5latest) 
...
-   Placez le fichier zip dans un emplacement http accessible 
...
L’exemple fourni exécute ces étapes pour cChoco et xNetworking.
...
Ces étapes génèrent une nouvelle configuration de nœud nommée « ISVBoxConfig.isvbox ». Le nom est créé en tant que « configurationName.nodeName ».

## Étape 5 : affectation de la configuration du nœud à votre nœud

Pour cette étape, vous devez interroger le serveur d’extraction pour obtenir l’identifiant de la nouvelle machine virtuelle après son enregistrement. Pour ce faire, vous devez utiliser l’applet de commande Get-AzureAutomationDscNode. Elle comporte quelques filtres qui peuvent probablement fonctionner, mais en règle générale, vous allez chercher à limiter le résultat via un filtre qui extraira les machines virtuelles avec un modèle de nom. Si vous créez uniquement des machines virtuelles d’un rôle donné, vous pouvez toutes les récupérer, auquel cas la propriété NodeConfigurationName sera ConfigureLCMforAAPull.isvbox. Il s’agit du nom de configuration de toute machine virtuelle récemment enregistrée. Une fois que vous obtenez la liste des identifiants, voici l’applet de commande :

    Set-AzureAutomationDscNode ` 
        -ResourceGroupName MY-AUTOMATION-RG -AutomationAccountName MY-AUTOMATION-ACCOUNT ` 
        -NodeConfigurationName ISVBoxConfig.isvbox -Id $theId

## Étape 6 : où placer toutes ces données PowerShell ?

Lorsque vous écrivez des modèles ARM personnalisés, Visual Studio génère un script PowerShell qui appelle le modèle. Vous pouvez ajouter à ce script les applets de commande utilisées pour importer votre configuration, pour la compiler, pour obtenir l’identifiant de vos machines virtuelles, etc. Consultez la source pour obtenir un exemple sur GitHub. Pour les éléments qui sont exécutés à chaque mise à jour de votre package, intégrez les commandes nécessaires dans votre pipeline de l’intégration continue.

## Étape 7 : création et gestion des métadonnées de packages
...
Dans cet exemple, nous partons d’une machine virtuelle provenant d’une image Windows 2012 R2 générique de la galerie Azure. Vous pouvez la démarrer à partir de n’importe quelle image stockée, puis la modifier avec la configuration DSC.
