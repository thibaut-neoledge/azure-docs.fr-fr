...
# Mon premier Runbook textuel

> [AZURE.SELECTOR]
- [Graphical](automation-first-runbook-graphical.md)
- [Textual](automation-first-runbook-textual.md)

Ce didacticiel vous familiarise avec la création d’un [Runbook textuel](automation-powershell-workflow.md) dans Azure Automation. Nous commencerons par un simple Runbook que nous testerons et publierons tout en expliquant comment suivre l'état de la tâche du Runbook. Puis nous modifierons le Runbook pour gérer les ressources Azure, en démarrant dans ce cas une machine virtuelle Azure. Enfin, nous renforcerons le Runbook en ajoutant des paramètres de Runbook.
...
3. Nommez le Runbook *MyFirstRunbook-Textual*.
4. Dans ce cas précis, nous allons créer un [Runbook textuel](automation-powershell-workflow.md) basé sur PowerShell Workflow. Par conséquent, sélectionnez **Flux de travail PowerShell** dans le champ **Type de Runbook**.<br>![Nouveau Runbook](media/automation-first-runbook-textual/new-runbook.png)
...
2. Cliquez sur **Volet de test** pour ouvrir le volet de test.<br> ![Volet de test](media/automation-first-runbook-textual/runbook-edit-toolbar-test-pane.png)
2. Cliquez sur **Démarrer** pour démarrer le test. Ce doit être la seule option activée.
3. Une [tâche de Runbook](automation-runbook-execution) est créée et son état apparaît. L’état initial de la tâche est *Mis en file d’attente* pour indiquer que la tâche attend qu’un Runbook Worker du cloud devienne disponible. La tâche prend ensuite l’état *Démarrage en cours* lorsqu’un Worker sélectionne la tâche, puis l’état *En cours d’exécution* lorsque le Runbook commence à s’exécuter.  
...
3. Faites défiler la page vers la droite pour visualiser le volet **MyFirstRunbook-Textual**. Les options de la partie supérieure nous permettent de démarrer le Runbook, de planifier son démarrage à un moment ultérieur ou de créer un [Webhook](automation-webhooks.md) afin de le démarrer par le biais d’un appel HTTP. 
...
7.  Lorsque le Runbook prend l’état *Terminé*, cliquez sur **Sortie**. Le volet **Sortie** s’ouvre et nous présente notre message *Hello World*.<br> ![Résumé des tâches](media/automation-first-runbook-textual/job-pane-output.png)  
...
9.  Cliquez sur **Tâches** afin d’ouvrir le volet des tâches pour ce Runbook. Il répertorie toutes les tâches créées par ce Runbook. Une seule tâche devrait apparaître, car nous n’avons exécuté la tâche qu’une seule fois.<br> ![Travaux](media/automation-first-runbook-textual/runbook-control-jobs.png) 
...
1. Après la ligne *Add-AzureAccount*, tapez *Start-AzureVM -Name ’NomVM’ -ServiceName ’NomServiceVM’* en fournissant le nom et le nom de service de la machine virtuelle à démarrer. <br> ![Authentifier](media/automation-first-runbook-textual/start-azurevm.png) 
9. Enregistrez le Runbook, puis cliquez sur **Volet de test** afin de tester le Runbook.
...
13.  Cliquez sur **Démarrer** pour démarrer le Runbook. Renseignez les valeurs **VMName** et **VMServiceName** pour la machine virtuelle que vous allez démarrer.<br> ![Démarrage du runbook](media/automation-first-runbook-textual/start-runbook-input-params.png) 
