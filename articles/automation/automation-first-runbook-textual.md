...
4. Dans ce cas précis, nous allons créer un [Runbook PowerShell Workflow](automation-runbook-types.md#powerShell-workflow-runbooks). Par conséquent, sélectionnez **Powershell Workflow** dans le champ **Type de runbook**.<br>![Nouveau Runbook](media/automation-first-runbook-textual/new-runbook.png)
...
9.  Cliquez sur **Tâches** pour ouvrir le volet Tâches pour ce runbook. Il répertorie toutes les tâches créées par ce Runbook. Une seule tâche doit apparaître, car nous n’avons exécuté la tâche qu’une seule fois.<br> ![Travaux](media/automation-first-runbook-textual/runbook-control-jobs.png) 
...
9. Enregistrez le runbook, puis cliquez sur **Volet de test** pour le tester.
