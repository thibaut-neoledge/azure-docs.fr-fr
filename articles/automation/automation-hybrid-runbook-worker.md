...
Si vous ne disposez pas déjà d’un espace de travail Operations Management Suite, créez-en un à l’aide des instructions mentionnées dans la section [Configuration de votre espace de travail Operational Insights](../operational-insights/operational-insights-onboard-in-minutes.md). Vous pouvez utiliser un espace de travail existant si vous en avez déjà un.
...
Utilisez le bouton **-Détaillé** avec **Add-HybridRunbookWorker** pour recevoir des informations détaillées concernant l’installation.
...
Vous pouvez supprimer Runbook Worker hybride d’un ordinateur local en exécutant l’applet de commande **Remove-HybridRunbookWorker** sur cet ordinateur. Utilisez le bouton **-Détaillé** pour afficher un journal détaillé du processus de suppression.
...
>[AZURE.NOTE]Les Runbooks Workers hybrides peuvent uniquement exécuter les [runbooks graphiques et Workflow PowerShell](automation-runbook-types.md). Actuellement, vous ne pouvez pas démarrer un [runbook PowerShell](automation-runbook-types.md) sur un Runbook Worker hybride.
