...
Avant de commencer les étapes de ce didacticiel, [installez l’interface de ligne de commande Azure](../cli-install-nodejs.md).
...
1. **Se connecter à Microsoft Azure**  
Dans votre interface de ligne de commande (Bash, Terminal, invite de commandes), passez en mode Resource Manager, puis [connectez-vous avec votre ID professionnel ou scolaire](../xplat-cli-connect.md#scenario-1-azure-login-with-interactive-login). Suivez les invites pour une expérience de connexion interactive à votre compte Azure.
...
3. **Déployer un compte de stockage dans le nouveau groupe de ressources**  
Il s’agit du compte de stockage dans lequel le modèle est stocké. Créez un compte de stockage nommé **vmsstestsa**.
...
11. Ajoutez la ressource autoscaleSettings qui définit le mode d’ajustement du groupe à échelle identique en fonction de l’utilisation du processeur sur les machines virtuelles du groupe.
    ...
    * **metricName**  
    Cette valeur est la même que celle du compteur de performances que nous avons défini dans la variable wadperfcounter. Grâce à cette variable, l’extension Diagnostics collecte le compteur **Processor\PercentProcessorTime**.
...
> [!NOTE]
> Vous pouvez également utiliser la capacité du portail à déployer les ressources. Utilisez ce lien : https://portal.azure.com/#create/Microsoft.Template/uri/<link to VM Scale Set JSON template>
