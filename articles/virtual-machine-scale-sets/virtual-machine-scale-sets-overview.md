...
Dans les pages de détail correspondant à ces modèles, un bouton est associé à la fonction de déploiement de portail. Pour déployer le groupe identique, cliquez sur le bouton et renseignez ensuite tous les paramètres requis dans le portail. Si vous ne savez pas si une ressource prend en charge les majuscules ou les casses mixtes, il est préférable d’utiliser des lettres en minuscules ou des chiffres dans les valeurs de paramètre. La vidéo [Dissection des modèles de groupes de machines virtuelles identiques](https://channel9.msdn.com/Blogs/Azure/VM-Scale-Set-Template-Dissection/player) fournit un exemple pratique.
