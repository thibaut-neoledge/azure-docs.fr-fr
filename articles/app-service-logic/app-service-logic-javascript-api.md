...
Les entrées de l’action sont les suivantes : - **Expression JavaScript** - Il s’agit de l’expression qui est évaluée. Vous devez inclure l’instruction `return` pour obtenir un contenu quel qu’il soit. - **Objet de contexte** - Objet facultatif pouvant être transmis dans le déclencheur. Vous pouvez définir autant de propriétés que vous le souhaitez, mais l’entité de niveau supérieur doit être un objet, par exemple `{ "bar" : 0}`.
