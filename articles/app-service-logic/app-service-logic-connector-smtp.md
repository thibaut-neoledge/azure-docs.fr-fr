...
Le connecteur SMTP vous permet de vous connecter à un serveur SMTP et d'exécuter une action pour envoyer un message électronique avec des pièces jointes.

- Avec l'action " Envoyer le message électronique " du connecteur SMTP, vous pouvez envoyer un message électronique à une ou plusieurs adresses électroniques. 
...
2.	Accédez à " Web et mobilité > API Apps " et recherchez " Connecteur SMTP ".
3.	Configurez le connecteur SMTP comme suit :
	...
	- **Nom** : indiquez le nom de votre connecteur SMTP.
	- **Paramètres du package**
		- **Nom d'utilisateur** : spécifiez le nom d'utilisateur permettant de se connecter au serveur SMTP.
		- **Mot de passe** : spécifiez le mot de passe permettant de se connecter au serveur SMTP.
		...
		- **Activer SSL** : spécifiez " true " pour utiliser SMTP sur un canal SSL/TLS sécurisé.
...
7.	Vous pouvez maintenant utiliser le connecteur SMTP dans le flux. Sélectionnez l'action " Envoyer le message électronique " et configurez les propriétés d'entrée comme suit :
	...
	- **Pièces jointes** - Pièces jointes à envoyer avec le message électronique. Il contient les champs suivants :
		- Contenu (chaîne)
		- Encodage de transfert de contenu (num) ("none"|"base64")
