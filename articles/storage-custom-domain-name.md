...
> [AZURE.NOTE]	Les procédures décrites ici s'appliquent aux comptes de stockage Azure. Pour les services cloud, consultez la rubrique <a href = "/fr-FR/develop/net/common-tasks/custom-dns/">Configuration d'un nom de domaine personnalisé pour un service cloud Azure</a> ; pour les sites web, consultez la rubrique <a href="/fr-FR/develop/net/common-tasks/custom-dns-web-site/">Configuration d'un nom de domaine personnalisé pour un site web Azure</a>. 

> [AZURE.NOTE]	Un compte de stockage Premium ne peut pas être mappé à un nom de domaine personnalisé. Consultez la page [Stockage Premium : stockage haute performance pour les charges de travail de machines virtuelles Azure](http://go.microsoft.com/fwlink/?LinkId=521898) pour plus d'informations sur les comptes de stockage Premium.
...
<table border="1" cellspacing="0" cellpadding="5" style="border: 1px solid #000000;">
	<tbody>
		...
		<tr>
			<td>Blob</td>
			<td><strong>URL par défaut :</strong>http://mystorageaccount.blob.core.windows.net/mycontainer/myblob<br /><strong>URL du domaine personnalisé :</strong>
		...
		<tr>
			...
			<strong>URL de domaine personnalisé :</strong>http://www.contoso.com/myblob
			<br/>ou<br />
...
2.  Dans l'onglet **Stockage**, cliquez sur le nom du compte de stockage dont vous souhaitez mapper le domaine personnalisé.

3.  Cliquez sur l'onglet **Configurer**.

4.  Au bas de l'écran, cliquez sur **Gérer les domaines** pour afficher la boîte de dialogue **Gérer le domaine personnalisé**. Le texte figurant en haut de la boîte de dialogue contient des informations sur la manière de créer l'enregistrement CNAME. Pour cette procédure, ignorez le texte qui fait référence au sous-domaine **asverify**.

5.  Connectez-vous au site web du bureau d'enregistrement de votre DNS et accédez à la page de
    gestion DNS. Vous pouvez trouver ceci dans une section telle que **Domain
...
7.  Créez un enregistrement CNAME et indiquez un alias de sous-domaine tel que **www** ou **photos**. Puis
    fournissez un nom d'hôte, à savoir votre point de terminaison de service BLOB, au format **mystorageaccount.blob.core.windows.net** (où **mystorageaccount** correspond au nom de votre compte de stockage). Le nom d'hôte à utiliser est indiqué dans le texte de la boîte de dialogue **Gérer le domaine personnalisé**.
...
4.  Au bas de l'écran, cliquez sur **Gérer les domaines** pour afficher la boîte de dialogue **Gérer le domaine personnalisé**. Le texte figurant en haut de la boîte de dialogue contient des informations sur la manière de créer l'enregistrement CNAME à l'aide du sous-domaine **asverify**.
...
7.  Créez un enregistrement CNAME et indiquez un alias de sous-domaine qui inclut le sous-domaine asverify. Par exemple, le sous-domaine que vous spécifiez sera au format **asverify.www** ou **asverify.photos**. Puis
    fournissez un nom d'hôte, à savoir votre point de terminaison de service BLOB, au format **asverify.mystorageaccount.blob.core.windows.net** (où **mystorageaccount** correspond au nom de votre compte de stockage). Le nom d'hôte à utiliser est indiqué dans le texte de la boîte de dialogue **Gérer le domaine personnalisé**.
...
9.	Activez la case à cocher **Avancé : utilisez le sous-domaine 'asverify' pour pré-enregistrer mon domaine personnalisé**. 
