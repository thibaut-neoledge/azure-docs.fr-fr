...
# Prise en main de Hadoop avec Hive dans HDInsight sous Linux (version préliminaire)

> [AZURE.SELECTOR]
- [Windows](hdinsight-get-started.md)
- [Linux](hdinsight-hadoop-linux-get-started.md)

Ce didacticiel vous permet de vous familiariser rapidement avec Azure HDInsight sous Linux en vous montrant comment approvisionner un cluster HDInsight Hadoop sous Linux et exécuter une requête Hive pour extraire des informations utiles à partir des données non structurées.
...
Supposons que vous disposiez d'un vaste jeu de données non structurées et que vous souhaitiez y exécuter des requêtes pour extraire des informations significatives. Voici comment obtenir ce résultat :
   ...
   ![Étapes du didacticiel « Prise en main de Hadoop avec Hive dans HDInsight » illustrées : création d'un compte, approvisionnement d’un cluster, interrogation des données et analyse dans le Tableau.](./media/hdinsight-hadoop-linux-get-started/HDI.Linux.GetStartedFlow.png)
...
- Un abonnement Azure. Pour plus d'informations sur l'obtention d'un abonnement, consultez les pages <a href="http://azure.microsoft.com/pricing/purchase-options/" target="_blank">Modes d’achat d’Azure</a>, <a href="http://azure.microsoft.com/pricing/member-offers/" target="_blank">Offres spéciales membres</a> ou <a href="http://azure.microsoft.com/pricing/free-trial/" target="_blank">Version d'évaluation gratuite</a>.
...
2. Cliquez sur **NOUVEAU** dans le coin inférieur gauche, pointez sur **DATA SERVICES**, pointez sur **STOCKAGE**, puis cliquez sur **CRÉATION RAPIDE**.
	...
	![Portail Azure où vous pouvez utiliser Création rapide pour configurer un nouveau compte de stockage.](./media/hdinsight-hadoop-linux-get-started/HDI.StorageAccount.QuickCreate.png)
...
2. Cliquez sur **NOUVEAU** sur le côté inférieur gauche, sur **DATA SERVICES**, sur **HDINSIGHT**, puis sur **HADOOP SOUS LINUX**.
	...
	![Création d'un cluster Hadoop dans HDInsight.](./media/hdinsight-hadoop-linux-get-started/HDI.QuickCreateCluster.png)
...
2. Ouvrez PuTTY. Dans **Catégorie**, cliquez sur **Session**. Dans l’écran **Options de base pour votre session PuTTY**, entrez l'adresse SSH de votre serveur HDInsight dans le champ **Nom d'hôte (ou adresse IP)**. L'adresse SSH est le nom de votre cluster, suivie de **-ssh.azurehdinsight.net**. Par exemple, **myhdinsightcluster-ssh.azurehdinsight.net**.
	...
	![Connexion à un cluster HDInsight sous Linux à l'aide de PuTTY](./media/hdinsight-hadoop-linux-get-started/HDI.linux.connect.putty.png)

3. Pour enregistrer les informations de connexion pour une utilisation ultérieure, entrez un nom pour cette connexion sous **Sessions enregistrées**, puis cliquez sur **Enregistrer**. La connexion sera ajoutée à la liste des sessions enregistrées.
...
2. À l'aide de l'interface CLI, entrez les instructions suivantes pour créer une nouvelle table nommée **log4jLogs** à l'aide de l'exemple de données déjà disponible sur le cluster :
	...
	- **SELECT** : sélectionne un nombre de toutes les lignes dont la colonne t4 contient la valeur [ERROR]. 
	...
	Notez que la sortie contient **[ERROR] 3**, car trois lignes contiennent cette valeur.
...
3. Utilisez les instructions suivantes pour créer une nouvelle table « interne » nommée **errorLogs** :
	...
	- **CREATE TABLE IF NOT EXISTS** : crée une table, si elle n'existe pas déjà. Dans la mesure où le mot clé **EXTERNAL** n'est pas utilisé, il s'agit d'une table interne, qui est stockée dans l'entrepôt de données Hive et gérée entièrement par Hive. Contrairement aux tables externes, la suppression d’une table interne entraîne également la suppression des données sous-jacentes.
...
4. Pour vérifier que seules les lignes contenant [ERROR] dans la colonne t4 ont été stockées dans la table **errorLogs**, utilisez l'instruction suivante afin de retourner toutes les lignes d’**errorLogs** :
	...
	Les données retournées doivent correspondre aux journaux [ERROR].


## <a name="nextsteps"></a>Étapes suivantes
Dans ce didacticiel, vous avez appris à approvisionner un cluster Hadoop Linux avec HDInsight et à exécuter une requête Hive sur ce cluster à l'aide de SSH. Pour en savoir plus, consultez les articles suivants :
