...
1.  [Inscription à New Relic à l'aide de l'Azure Store](#sign-up)
2.  [Installation du module New Relic](#install-module)
3.  [Activation de New Relic Developer Analytics pour le service mobile](#enable-service)
4.  [Surveillance du service mobile dans le tableau de bord New Relic](#monitor)
...
Installation du module New Relic
--------------------------------
...
1.  Si ce n'est pas déjà fait, suivez les étapes indiquées dans le didacticiel [Stockage de scripts serveur dans un contrôle de code source](/en-us/develop/mobile/tutorials/store-scripts-in-source-control/) pour activer le contrôle de code source pour votre service mobile, cloner le référentiel et installer [Node Package Manager (NPM)](http://nodejs.org/).
...
2.  Accédez au dossier `.\service` de votre référentiel Git local, puis, à partir de l'invite de commandes, exécutez la commande suivante :
    ...
    NPM installe le [module New Relic](https://npmjs.org/package/newrelic) dans le sous-répertoire `\newrelic`.
...
3.  Ouvrez un outil en ligne de commande Git, tel que **GitBash** (Windows) ou **Bash** (Unix Shell), puis tapez la commande suivante dans l'invite de commandes Git :
    ...
    Ceci télécharge le nouveau module `newrelic` sur votre service mobile.

Ensuite, vous allez activer la surveillance New Relic de votre service mobile dans le [portail de gestion](https://manage.windowsazure.com/).

Activation de New Relic Developer Analytics pour le service mobile
------------------------------------------------------------------

1.  Dans le [portail de gestion](https://manage.windowsazure.com/), sélectionnez votre service mobile, puis cliquez sur l'onglet **Configurer**.
...
Surveillance du service mobile dans le tableau de bord New Relic
----------------------------------------------------------------
...
2.  Attendez quelques minutes que les données soient traitées, puis accédez au tableau de bord New Relic.
    ...
    Une fois votre abonnement New Relic acheté en tant que module complémentaire, sélectionnez-le dans le [portail de gestion](https://manage.windowsazure.com/), puis cliquez sur **Gérer**.
...
-   Pour des informations de tarification, consultez la [page New Relic dans l'Azure Store](/en-us/gallery/store/new-relic/new-relic/).
-   Pour plus d'informations sur l'utilisation de New Relic, consultez la page [Vue d'ensemble des applications](https://docs.newrelic.com/docs/applications-dashboards/applications-overview) dans la documentation de New Relic.
