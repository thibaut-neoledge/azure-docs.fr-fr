...
> [!NOTE]
> Les étapes de cet article partent du principe que vous utilisez un système Windows avec le client SSH PuTTY. Si vous utilisez un système Linux, Unix, OS X ou Windows qui fournit la commande `ssh`, consultez la rubrique [Utilisation de SSH avec Hadoop Linux sur HDInsight depuis Linux, Unix ou OS X](hdinsight-hadoop-linux-use-ssh-unix.md).
...
Historiquement, Windows n’a jamais fourni de client SSH. PuTTY est un client SSH graphique qui peut être installé sur Windows.
...
## <a name="connect-to-a-linux-based-hdinsight-cluster"></a>Connexion à un cluster HDInsight sous Linux
