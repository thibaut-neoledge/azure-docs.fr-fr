...
* **[Version préliminaire des clusters joints à un domaine](../domain-joined/apache-domain-joined-introduction.md)** : cluster joint à un domaine Active Directory pour que vous puissiez contrôler l’accès et assurer la gouvernance des données.
...
* [Prise en main de Hadoop dans HDInsight](apache-hadoop-linux-tutorial-get-started.md): didacticiel de démarrage rapide pour l’approvisionnement de clusters HDInsight Hadoop et l’exécution d’exemples de requêtes Hive.
* [Prise en main de Spark dans HDInsight](../spark/apache-spark-jupyter-spark-sql.md): didacticiel de démarrage rapide pour la création d’un cluster Spark et l’exécution de requêtes Spark SQL interactives.
* [Utilisation de R Server sur HDInsight](../r-server/r-server-get-started.md): commencez à utiliser R Server dans HDInsight Premium.
