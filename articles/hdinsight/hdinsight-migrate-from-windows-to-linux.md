<properties
pageTitle="Effectuer la migration de HDInsight Windows vers HDInsight Linux | Azure"
...
### Copier des données dans l’environnement de test
...
> [AZURE.IMPORTANT] Les alertes Ambari indiquent qu’un problème se pose *peut-être* et non pas qu’un problème se pose *vraiment*. Par exemple, vous pouvez recevoir une alerte indiquant que HiveServer2 n’est pas accessible, même si vous pouvez y accéder normalement.
>
...
| Commande `hive` à partir du Bureau à distance | [Beeline](hdinsight-hadoop-use-hive-beeline.md) ou [Hive à partir d’une session SSH](hdinsight-hadoop-use-hive-ssh.md) |
...
### Activités .NET personnalisées Azure Data Factory
