...
## API REST
L'URI de base pour l'API REST d'Ambari sur HDInsight est https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME, où **CLUSTERNAME** est le nom de votre cluster.
...
L’exemple suivant utilise cURL pour effectuer une demande GET sur l’API REST. Remplacez **PASSWORD** par le mot de passe de l’administrateur du cluster. Remplacez **CLUSTERNAME** par le nom du cluster :
...
2. Récupérez la configuration du composant et de la balise à l’aide de la commande suivante. Remplacez **spark-thrift-sparkconf** et **INITIAL** par le composant et la balise dont vous souhaitez récupérer la configuration.
   ...
   * Il récupère le contenu du tableau `.items[]` et l’ajoute à l’élément **desired\_config**.
