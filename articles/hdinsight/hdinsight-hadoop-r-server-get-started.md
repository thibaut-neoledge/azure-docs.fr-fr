...
4. Choisissez **Configuration de cluster**. Dans le panneau **Configuration de cluster** , sélectionnez les options suivantes :
...
5. Sélectionnez **Informations d’identification**, puis saisissez une valeur dans les champs **Nom de connexion au cluster** et **Mot de passe de connexion au cluster**.
...
6. Sélectionnez **Source de données** pour sélectionner une source de données comme emplacement principal du système de fichiers HDFS utilisé par le cluster. Sélectionnez un compte de stockage Azure nouveau ou existant ou un compte de stockage Data Lake existant.

   1. Si vous sélectionnez un compte de stockage Azure, sélectionnez un compte de stockage existant en choisissant **Sélectionner un compte de stockage**, puis en spécifiant le compte, ou créez un compte à l’aide du lien **Créer** dans la section **Sélectionner un compte de stockage**.
      ...
      Si l’option Nouveau compte de stockage a été sélectionnée, sélectionnez **Emplacement** pour sélectionner la région dans laquelle le compte de stockage sera créé.
...
7. Sélectionnez **Niveaux de tarification du nœud** pour afficher des informations sur les nœuds qui seront créés pour ce cluster. Sauf si vous savez que vous aurez besoin d’un cluster plus grand, conservez le nombre de nœuds Worker par défaut `4`. Le coût estimé du cluster s'affiche dans le panneau.
...
8. Sélectionnez le **Groupe de ressources** à utiliser en sélectionnant un groupe de ressources existant ou en en créant un nouveau. Si vous sélectionnez **Créer**, saisissez le nom du nouveau groupe de ressources. Une coche verte s’affiche pour indiquer si le nouveau nom de groupe est disponible.

   ![Panneau Niveaux de tarification du nœud](./media/hdinsight-getting-started-with-r/useexistingrg.png)

   

9. Après avoir vérifié vos sélections, passez à la création du cluster. Pour ce faire, sélectionnez **Épingler au tableau d’accueil**, puis **Créer**. Le cluster est créé et la vignette correspondante ajoutée au tableau d’accueil de votre portail Azure.

   Remarquez qu’il existe également un lien vers les **Options d’automatisation**. Cliquer sur ce lien permet d’afficher les scripts qui peuvent être utilisés pour automatiser la création d’un cluster avec la configuration sélectionnée. Ces scripts sont également disponibles dans l’entrée du Portail Azure de votre cluster, une fois celui-ci créé.
