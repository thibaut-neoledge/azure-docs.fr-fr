...
2. Sélectionnez l'onglet **Configurations** .
    ...
    ![Sélectionnez l’onglet Configurations](media/hdinsight-troubleshoot-spark/update-config-2.png)
...
3. Définissez les configurations Spark suivantes sur les valeurs appropriées, qui ne doivent pas dépasser 90 % de la mémoire et des cœurs disponibles. Les valeurs doivent correspondre à la mémoire requise pour l’application Spark : 
    ...
    Pour obtenir la mémoire totale utilisée par tous les exécuteurs, exécutez la commande suivante : 
    
    ```apache
    spark.executor.instances * (spark.executor.memory + spark.yarn.executor.memoryOverhead) 
    ```
    Pour obtenir la mémoire totale utilisée par le pilote, exécutez la commande suivante :
