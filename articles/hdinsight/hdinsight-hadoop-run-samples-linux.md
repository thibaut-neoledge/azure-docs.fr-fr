...
1. Générer 10 Go de données, qui seront stockées dans le stockage du cluster HDInsight par défaut sous **wasbs:///example/data/10GB-sort-input** :
	...
	`-Dmapred.map.tasks` indique à Hadoop le nombre de tâches de mappage à utiliser pour cette tâche. Les deux derniers paramètres indiquent la tâche pour créer 10 Go de données et pour les stocker sur **wasbs:///example/data/10GB-sort-input**.
