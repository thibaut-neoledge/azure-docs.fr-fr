---
title: "Qu’est-ce que R sur HDInsight ? Présentation de R Server sur HDInsight | Microsoft Docs"
description: "Qu’est-ce que R Server sur HDInsight et comment utiliser R Server pour créer des applications pour l’analyse du Big Data."
services: hdinsight
documentationcenter: 
author: jeffstokes72
manager: jhubbard
...
---
# <a name="overview-of-r-server-on-hdinsight"></a>Vue d’ensemble : R Server sur HDInsight (version préliminaire)
...
Pour inclure R Server dans un cluster HDInsight, vous devez créer un cluster Hadoop ou Spark à l’aide du portail Azure. Les deux types de clusters utilisent la même configuration incluant R Server sur les nœuds de données d’un cluster, et un nœud de périmètre comme zone d’accueil pour l’analyse basée sur R Server. Pour une description détaillée de la création d’un cluster, voir [Prise en main de R Server sur HDInsight](hdinsight-hadoop-r-server-get-started.md) .
