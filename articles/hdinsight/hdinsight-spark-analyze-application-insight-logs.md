...
* Maîtrise de la création d’un cluster HDInsight sous Linux. Si vous ne maîtrisez pas la création d’un cluster, consultez la rubrique [Créer Spark sur HDInsight](hdinsight-apache-spark-jupyter-spark-sql.md) pour plus d’informations.
  ...
  > [!NOTE]
  > Ce document ne fournit aucune procédure détaillée concernant la création d’un cluster. Au lieu de cela, il fait référence à d’autres documents qui contiennent des informations sur la création d’un cluster capable d’accéder aux données de télémétrie.
  > 
  > 
...
* **Emplacement** : le compte de stockage doit figurer dans la même région que HDInsight. Cela réduit la latence lors de l’accès aux données et évite les frais d’acheminement qu’entraîne le déplacement des données entre différentes régions.
...
3. Dans le premier champ (appelé **cellule**) de la page, entrez les informations suivantes :
...
5. Une nouvelle cellule est créée sous la première. Entrez les informations suivantes dans la nouvelle cellule. Remplacez **CONTAINER** et **STORAGEACCOUNT** par le nom du compte de stockage Azure et le nom du conteneur d’objets blob que vous avez utilisés lors de la configuration de l’exportation continue d’Application Insights.
   
        %%bash
        hdfs dfs -ls wasb://CONTAINER@STORAGEACCOUNT.blob.core.windows.net/
   
    Utilisez **MAJ+ENTRÉE** pour exécuter cette cellule. Vous devez voir un résultat similaire à ce qui suit :
...
6. Dans la prochaine cellule, entrez les informations suivantes. Remplacez **WASB\_PATH** par le chemin d’accès de l’étape précédente.
    ...
    Un nouveau tableau de données est créé à partir des fichiers JSON exportés via processus d’exportation continue. Utilisez **MAJ+ENTRÉE** pour exécuter cette cellule.
...
1. Dans le [portail Azure](https://portal.azure.com), sélectionnez votre Spark sur le cluster HDInsight. Dans la section **Liens rapides**, sélectionnez **Tableaux de bord de Cluster**, puis **Notebook Jupyter** dans le panneau du tableau de bord du cluster.
    ...
    ![Les tableaux de bord du cluster](./media/hdinsight-spark-analyze-application-insight-logs/clusterdashboards.png)
2. Dans l’angle supérieur droit de la page Jupyter, sélectionnez **Nouveau**, puis **Scala**. Un nouvel onglet de navigateur s’ouvre, contenant un notebook Jupyter basé sur Scala.
3. Dans le premier champ (appelé **cellule**) de la page, entrez les informations suivantes :
