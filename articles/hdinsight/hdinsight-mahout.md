<properties
	pageTitle="Génération de recommandations à l’aide de Mahout et de HDInsight basé sur Windows | Microsoft Azure"
...
* Installation de Mahout sur les clusters HDInsight 3.0 et 2.0
	...
	> [AZURE.NOTE]Mahout est fourni avec la version HDInsight 3.1 des clusters. Si vous utilisez une version antérieure de HDInsight, consultez la section [Installation de Mahout](#install) avant de continuer.
...
##<a name="recommendations"></a>Génération de recommandations avec Windows PowerShell
...
De façon pratique, [GroupLens Research][movielens] fournit des données d’évaluation de films dans un format compatible avec Mahout.

1. Téléchargez l’archive [MovieLens 100k][100k], qui contient 100 000 évaluations de 1000 utilisateurs sur 1700 films.
...
3. Téléchargez le fichier __u.data__ vers __example/data/u.data__ sur votre cluster HDInsight. La commande suivante utilise PowerShell pour télécharger les données. Pour d'autres façons de télécharger des fichiers, consultez la page [Téléchargement de données pour les tâches Hadoop dans HDInsight][upload].
    ...
    Cela télécharge le fichier __u.data__ vers __example/data/u.data__ sur le stockage par défaut de votre cluster. Vous pouvez ensuite accéder à ces données à l’aide de l’URI __wasb:///example/data/u.data__ des tâches HDInsight.

###Exécution de la tâche

Utilisez le script Windows PowerShell suivant pour exécuter une tâche à l’aide du moteur de recommandation Mahout avec le fichier __u.data__ téléchargé :
...
> [AZURE.NOTE]Les tâches Mahout ne suppriment pas les données temporaires créées lors du traitement de la tâche. C’est la raison pour laquelle le paramètre `--tempDir` est spécifié dans la tâche donnée en exemple : il isole les fichiers temporaires dans un répertoire spécifique.

La tâche Mahout ne retourne pas la sortie dans STDOUT. Au lieu de cela, elle est stockée dans le répertoire de sortie spécifié en tant que __part-r-00000__. Le script télécharge ce fichier dans __output.txt__, dans le répertoire actif sur votre station de travail.
...
Bien que le résultat généré puisse être utilisé dans une application, il n’est pas très lisible. Certains autres fichiers extraits précédemment vers le dossier __ml-100k__ peuvent être utilisés pour résoudre `movieId` en un nom de film, ce dont se charge le script PowerShell suivant :
...
Pour utiliser ce script, vous devez avoir déjà extrait le dossier __ml-100k__. Voici un exemple d’exécution du script :
...
2. Téléchargez les fichiers vers __example/data__. Pour ce faire, vous pouvez utiliser le script suivant. Remplacez __CLUSTERNAME__ par le nom du cluster HDInsight. Remplacez FILENAME par le nom du fichier à télécharger.
...
1. La version de Mahout à utiliser dépend de la version de HDInsight de votre cluster. Vous trouverez la version de cluster à l’aide de la commande [Azure PowerShell][aps] suivante :
  ...
  * __Pour HDInsight 3.0__, vous devez [générer Mahout à partir de la source][build] et spécifier la version Hadoop fournie par HDInsight. Installez les éléments requis répertoriés sur la page de génération, téléchargez la source, puis utilisez la commande ci-après pour créer les fichiers jar de Mahout :
...
2. Téléchargez le fichier jar vers __example/jars__ dans le stockage par défaut de votre cluster. Dans le script suivant, remplacez CLUSTERNAME par le nom de votre cluster HDInsight et FILENAME par le chemin du fichier __mahout-coure-0.9-job.jar__.
