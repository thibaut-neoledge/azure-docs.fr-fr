---
title: "Disponibilité des clusters Hadoop dans HDInsight | Microsoft Docs"
...
## <a name="configure-head-node-size"></a>Configuration de la taille du nœud principal
...
Pour le Kit de développement logiciel (SDK), le scénario est similaire. La création et l’approvisionnement d’un cluster avec le Kit de développement logiciel (SDK) sont documentés sur la page [Utilisation du Kit de développement logiciel (SDK) HDInsight .NET](hdinsight-provision-clusters.md). La configuration d’un nœud principal de très grande taille nécessite l’ajout du paramètre `HeadNodeSize = NodeVMSize.ExtraLarge` à la méthode `ClusterCreateParameters()` utilisée dans ce code.
...
* [Connexion à des clusters HDInsight à l’aide de RDP](hdinsight-administer-use-management-portal.md#connect-to-clusters-using-rdp)
* [Utilisation du Kit de développement logiciel (SDK) HDInsight .NET](hdinsight-provision-clusters.md)
