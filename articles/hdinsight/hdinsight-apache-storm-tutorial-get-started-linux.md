...
> [!NOTE]
> Les étapes décrites dans cet article créent un cluster HDInsight Linux. Pour savoir comment créer un cluster Storm Windows sur HDInsight, consultez le [Didacticiel Apache Storm : prise en main de l’exemple Storm Starter à l’aide de l’analyse de données sur HDInsight](hdinsight-apache-storm-tutorial-get-started.md)
...
* **Des connaissances en SSH et SCP**. Pour plus d’informations sur l’utilisation de SSH et SCP avec HDInsight, consultez les articles suivants :
    ...
    * **Clients Linux, Unix ou OS X** : consultez la page [Utilisation de SSH avec Hadoop dans HDInsight sous Linux](hdinsight-hadoop-linux-use-ssh-unix.md)
    
    * **Clients Windows** : consultez la page [Utilisation de SSH (PuTTY) avec Hadoop Linux dans HDInsight à partir de Windows](hdinsight-hadoop-linux-use-ssh-windows.md)
...
1. Cliquez sur l’image suivante pour ouvrir le modèle dans le portail Azure.         
    ...
    Le modèle se trouve dans un conteneur d’objets blob public, disponible à l’adresse *https://hditutorialdata.blob.core.windows.net/armtemplates/create-linux-based-storm-cluster-in-hdinsight.json*. 

2. Dans le panneau __Déploiement personnalisé__, entrez les valeurs suivantes :
   
    * __Groupe de ressources__ : groupe de ressources dans lequel le cluster est créé.

    * **Nom du cluster** : nom du cluster Hadoop.

    * __Nom d’utilisateur __ et __mot de passe__ : le nom de connexion par défaut est admin.
    
    * __Nom d’utilisateur du SSH__ et __mot de passe__ : utilisateur et mot de passe permettant de se connecter au cluster à l’aide de SSH.

    * __Emplacement__ : emplacement géographique du cluster.
...
1. Ouvrez un navigateur web sur l’adresse https://CLUSTERNAME.azurehdinsight.net/stormui, où **CLUSTERNAME** est le nom de votre cluster. L’interface utilisateur de Storm s’ouvre.
