...
Dans la liste des affichages, sélectionnez l’ __Affichage Hive__.
...
2. Pour démarrer la requête, utilisez le bouton **Exécuter** situé sous la feuille de calcul. Il devient orange et le texte affiche **Arrêter**.

3. Une fois la requête terminée, l’onglet **Résultats** affiche les résultats de l’opération. Le texte suivant est le résultat de la requête :
...
> [!IMPORTANT]
> Tez n’est pas utilisé pour résoudre toutes les requêtes. De nombreuses requêtes peuvent être résolues sans utiliser Tez. 
...
À partir de l’onglet Requête, vous pouvez éventuellement enregistrer des requêtes. Une fois enregistrée, vous pouvez réutiliser la requête à partir de l’onglet __Requêtes Enregistrées__.
