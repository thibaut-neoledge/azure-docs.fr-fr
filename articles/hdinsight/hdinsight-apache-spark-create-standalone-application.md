<properties 
	pageTitle="Créer une application Scala autonome à exécuter sur un cluster HDInsight Spark | Microsoft Azure" 
...
1. Démarrer IntelliJ IDEA, puis à partir de l'écran d’accueil, cliquez sur **Configurer**, puis sur **Plug-in**.

	![Activer le plug-in Scala](./media/hdinsight-apache-spark-create-standalone-application/enable-scala-plugin.png)

2. Dans l'écran suivant, cliquez sur **Installer le plug-in JetBrains** dans le coin inférieur gauche. Dans la boîte de dialogue **Parcourir les plug-in JetBrains** qui s'ouvre, recherchez Scala, puis cliquez sur **Installer**.
...
5. Supprimer le fichier **MySpec.Scala** dans **src\\test\\scala\\com\\microsoft\\spark\\example**. Ce fichier est inutile pour l'application.

6. Si nécessaire, renommez les fichiers source et de test nommés par défaut. Dans le volet gauche dans IntelliJ IDEA, accédez à **src\\main\\scala\\com.microsoft.spark.example**. Cliquez avec le bouton droit sur **App.scala**, puis cliquez sur **Refactoriser**, sur Renommer un fichier. Dans la boîte de dialogue, indiquez le nouveau nom de l'application, puis cliquez sur **Refactoriser**.
...
8. Mettez à jour le fichier source Scala pour inclure le code de votre application. Ouvrez et remplacez le code existant par le code suivant et enregistrez les modifications.
