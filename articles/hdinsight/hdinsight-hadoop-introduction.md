...
HDInsight fournit des configurations de cluster pour des charges de travail spécifiques : Hadoop, HBase ou Storm. Vous pouvez également <a href="http://azure.microsoft.com/documentation/articles/hdinsight-hadoop-customize-cluster/" target="_blank">personnaliser les clusters avec des actions de script</a>.
...
* [Approvisionnement de HDInsight sur Linux à l’aide d’options personnalisées](hdinsight-hadoop-provision-linux-clusters.md) : apprenez à approvisionner un cluster Hadoop HDInsight sous Linux à l’aide d’options personnalisées via le portail de gestion Azure, l’interface de ligne de commande Azure ou Azure PowerShell.
