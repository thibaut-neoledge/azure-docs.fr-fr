---
title: Traitement des événements de hubs d’événements avec Storm sur HDInsight | Microsoft Docs
...
> [!NOTE]
> Lors de l’utilisation du projet terminé, vous devez utiliser le **Gestionnaire de Package NuGet** pour restaurer les packages requis par cette solution.
> 
> 
...
La version la plus récente du fichier **eventhubs-storm-spout-0.9-jar-with-dependencies.jar** est fournie dans le projet [HDInsight Storm examples](https://github.com/hdinsight/hdinsight-storm-examples) dans le dossier **lib**. Pour télécharger le fichier, appliquez l'une des méthodes suivantes.
...
1. Dans le [portail Azure Classic](https://manage.windowsazure.com), sélectionnez **NOUVEAU** > **APP SERVICES** > **SERVICE BUS** > **EVENT HUB** > **CRÉATION PERSONNALISÉE**.
2. Sur l’écran **Ajouter un nouvel hub d’événements**, entrez un **nom de hub d’événements**, sélectionnez la **région** dans laquelle créer le hub, puis créez un espace de noms ou sélectionnez-en un existant. Cliquez sur la **flèche** pour continuer.
   ...
   > 
   > 
3. Sur l’écran **Configurer un hub d’événements**, entrez les valeurs pour **Nombre de partitions** et **Conservation des messages**. Pour cet exemple, entrez 8 pour le nombre de partitions et 1 pour la conservation des messages. Notez le nombre de partitions, car vous en aurez besoin ultérieurement.
4. Une fois le hub d’événements créé, sélectionnez l’espace de noms, puis **Event Hubs**. Enfin, sélectionnez le hub d’événements que vous avez créé.
...
2. Entrez un **nom** pour le compte de stockage, sélectionnez un **emplacement**, puis cliquez sur la **coche** pour créer le compte de stockage.
   ...
   > [!NOTE]
   > Vous devez sélectionner le même **emplacement** que celui de votre hub d’événements et de serveur Storm sur HDInsight pour réduire la latence et les coûts.
   > 
   > 
...
4. Ouvrez Visual Studio. Dans le menu **Affichage**, sélectionnez **Cloud Explorer**. Dans **Cloud Explorer**, développez **Comptes de stockage**, puis le compte de stockage créé précédemment.
   
    ![Cloud Explorer](./media/hdinsight-storm-develop-csharp-event-hub-topology/createtablestorage.png)
5. Cliquez avec le bouton droit sur **Tables** pour votre compte de stockage, puis sélectionnez **Créer une table**. Lorsque vous y êtes invité, saisissez **événements** comme nom de votre table. Enregistrez le nom ; vous en aurez besoin ultérieurement.
...
2. Ouvrez le fichier **Program.cs** et ajoutez le code suivant code immédiatement après la ligne `TopologyBuilder topologyBuilder = new TopologyBuilder("EventHubWriter" + DateTime.Now.ToString("yyyyMMddHHmmss"));`.
...
4. Immédiatement après le code précédent, ajoutez le code suivant :
...
> [!NOTE]
> Par défaut, cette topologie va créer un seul processus de travail, ce qui est suffisant pour notre exemple. Si vous l’adaptez pour un cluster de production, vous devriez ajouter le code suivant pour modifier le nombre de processus de travail :
> 
> 
...
1. Dans l'**Explorateur de solutions**, ouvrez **Spout.cs** et ajoutez le code suivant en haut du fichier :
   ...
   > [!NOTE]
   > Le package JSON.NET doit être déjà installé, comme requis par l'infrastructure SCP.NET utilisée pour les topologies C# Storm.
   > 
   > 
...
3. Remplacez la méthode **NextTuple** par le code suivant :
...
4. Enregistrez le fichier **Spout.cs**.
...
2. Dans l’écran **Nouveau projet**, développez **Installé** > **Modèles**, puis sélectionnez **HDInsight**. Dans la liste des modèles, sélectionnez **Application Storm**. En bas de l’écran, entrez **EventHubReader** comme nom d’application.
...
2. Sélectionnez **Outils**, **Gestionnaire de package NuGet**, puis **Console du Gestionnaire de package**. Lorsque la console s’affiche, utilisez la commande suivante pour installer les packages Azure Storage.
   
        NuGet install WindowsAzure.Storage
3. Dans les propriétés du projet, sélectionnez **Paramètres**, puis sélectionnez **Ce projet ne contient pas de fichier de paramètres par défaut. Cliquez ici pour en créer un.**
4. Entrez les paramètres suivants. Utilisez les informations du hub d’événements et du compte de stockage que vous avez créés précédemment dans la colonne **Valeur**.
   ...
   | eventHubName |string |Application |
   | EventHubPartitionCount |int |Application |
   | StorageConnection |(Chaîne de connexion) |Application |
...
2. Ouvrez le fichier **Program.cs** et ajoutez le code suivant code immédiatement après la ligne`TopologyBuilder topologyBuilder = new TopologyBuilder("EventHubReader" + DateTime.Now.ToString("yyyyMMddHHmmss"));` :
...
4. Ajoutez le code suivant juste après le code précédent :
...
> [!NOTE]
> Par défaut, cette topologie va créer un seul processus de travail, ce qui est suffisant pour notre exemple. Si vous l’adaptez pour un cluster de production, vous devez ajouter le code suivant pour modifier le nombre de processus de travail :
> 
> 
...
1. Dans l’**Explorateur de solutions**, cliquez avec le bouton droit sur le projet **EventHubReader** et sélectionnez **Ajouter**, puis **Classe**. Nommez la nouvelle classe **Devices.cs**.
...
4. Immédiatement après le code précédent, ajoutez le code suivant :
    ...
    Cela permet la connexion à la table Azure Storage que vous avez créée précédemment à l’aide de la chaîne de connexion stockée dans `TableName`.
5. Recherchez la méthode **Execute** et remplacez-la par la méthode suivante :
   ...
   > [!IMPORTANT]
   > Le composant EventHubSpout attend, pour chaque tuple, un accusé de réception provenant des composants en aval comme ce bolt. Si aucun accusé de réception n’est reçu, le EventHubSpout supposera que le traitement du tuple a échoué.
   > 
   > 

À ce stade, vous disposez d’une topologie complète qui lit des données du hub d’événements et qui les stocke dans Table Storage, dans la table que vous avez créée précédemment.
