...
> [AZURE.NOTE] Le spout et le bolt ont été soumis pour inclusion dans le projet Apache Storm. Pour plus d’informations, consultez la rubrique [STORM-583: Initial check-in for storm-event hubs](https://github.com/apache/storm/pull/336/files) dans GitHub.
...
4. Ouvrez Visual Studio. Dans le menu __Affichage__, sélectionnez __Cloud Explorer__. Dans __Cloud Explorer__, développez __Comptes de stockage__, puis le compte de stockage créé précédemment.
...
> [AZURE.NOTE] Par défaut, cette topologie va créer un seul processus de travail, ce qui est suffisant pour notre exemple. Si vous l’adaptez pour un cluster de production, vous devriez ajouter le code suivant pour modifier le nombre de processus de travail :
