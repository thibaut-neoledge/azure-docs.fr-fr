...
* Pour exécuter des requêtes Hive à l’aide de SSH sur des clusters HDInsight joints à un domaine, consultez l’article [Use SSH with Linux-based Hadoop on HDInsight from Linux, Unix, or OS X (Utiliser SSH avec Hadoop sur Linux sur HDInsight à partir de Linux, Unix ou OS X)](hdinsight-hadoop-linux-use-ssh-unix.md).
