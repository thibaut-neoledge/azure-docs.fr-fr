...
> [AZURE.NOTE] Une autre option pour définir des flux de travail avec HDInsight consiste à utiliser Azure Data Factory. Pour en savoir plus sur Azure Data Factory, voir [Utilisation de Pig et Hive avec Data Factory][azure-data-factory-pig-hive].
...
> [AZURE.NOTE] Pour obtenir la liste des versions d’Oozie prises en charge sur les clusters HDInsight, voir [Nouveautés des versions de cluster fournies par HDInsight][hdinsight-versions].
...
> [AZURE.NOTE] Le répertoire `-p` a provoqué la création de tous les répertoires dans le chemin d’accès s’ils n’existent pas déjà. Le répertoire **data** sera utilisé pour contenir les données utilisées par le script **useooziewf.hql**.
...
1. Une fois que l’éditeur nano est ouvert, entrez les informations suivantes comme contenu du fichier :
    ...
    - **RunSqoopExport**: cette action exporte les données créées à partir du script Hive vers la base de données SQL à l’aide de Sqoop. Elle n’est exécutée que si l’action **RunHiveScript** a abouti.
        ...
        > [AZURE.NOTE] Pour plus d’informations sur le workflow Oozie et sur l’utilisation d’actions de workflow, voir la [documentation Apache Oozie 4.0][apache-oozie-400] (pour HDInsight version 3.0) ou la [documentation Apache Oozie 3.3.2][apache-oozie-332] (pour HDInsight version 2.1).
...
2. Une fois que l’éditeur nano est ouvert, utilisez les éléments suivants comme contenu du fichier :
    ...
    * Remplacez toutes les instances de **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** par la valeur que vous avez reçue précédemment.
    ...
    > [AZURE.WARNING] Vous devez utiliser le chemin d’accès WASB complet, avec le conteneur et le compte de stockage inclus dans le chemin d’accès. L’utilisation du format court (wasbs:///) provoque l’échec de l’action RunHiveScript lorsque le travail démarre.
