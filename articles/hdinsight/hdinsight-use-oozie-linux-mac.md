<properties
	pageTitle="Utilisation des workflows Hadoop Oozie dans HDInsight basé sur Linux | Microsoft Azure"
...
# Utilisation d’Oozie avec Hadoop pour définir et exécuter un flux de travail dans HDInsight
...
> [AZURE.NOTE] Une autre option pour définir des flux de travail avec HDInsight consiste à utiliser Azure Data Factory. Pour en savoir plus sur Azure Data Factory, consultez la page [Utilisation de Pig et Hive avec Data Factory][azure-data-factory-pig-hive].
...
1. Une action Hive exécute un script HiveQL pour extraire des enregistrements à partir de la table **hivesampletable** incluse avec HDInsight. Chaque ligne de données décrit un accès depuis un appareil mobile spécifique. Le format d’enregistrement ressemble à ce qui suit :
		...
		8       18:54:20        fr-FR   Android Samsung SCH-i500        California     United States    13.9204007      0       0
		23      19:19:44        fr-FR   Android HTC     Incredible      Pennsylvania   United States    NULL    0       0
		23      19:19:46        fr-FR   Android HTC     Incredible      Pennsylvania   United States    1.4757422       0       1
...
> [AZURE.NOTE] Le paramètre `-p` a provoqué la création de tous les répertoires dans le chemin d’accès s’ils n’existent pas déjà. Le répertoire **data** sera utilisé pour contenir les données utilisées par le script **useooziewf.hql**.
...
1. Une fois que l’éditeur nano est ouvert, utilisez les éléments suivants comme contenu du fichier :
...
3. Exécutez les commandes suivantes pour copier **useooziewf.hql** dans **wasbs:///tutorials/useoozie/useooziewf.hql** :
...
1. Une fois que l’éditeur nano est ouvert, entrez les informations suivantes comme contenu du fichier :
	...
	- **RunHiveScript** : il s’agit de l’action de démarrage, qui exécute le script Hive **useooziewf.hql**
...
3. Utilisez la commande suivante pour copier le fichier **workflow.xml** dans **wasbs:///tutorials/useoozie/workflow.xml** :

		hdfs dfs -copyFromLocal workflow.xml /tutorials/useoozie/workflow.xml

##Création de la base de données

Suivez les étapes du document [Création d’une base de données SQL](../sql-database/sql-database-get-started.md) afin de créer une base de données. Lorsque vous créez la base de données, utilisez __oozietest__ comme nom de base de données. Notez également le nom utilisé pour le serveur de base de données ; vous en aurez besoin dans la section suivante.
...
2. Une fois que l’éditeur nano est ouvert, utilisez les éléments suivants comme contenu du fichier :
	...
	* Remplacez toutes les instances de **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** par la valeur que vous avez reçue précédemment.
	...
	> [AZURE.WARNING] Vous devez utiliser le chemin d’accès WASB complet, avec le conteneur et le compte de stockage inclus dans le chemin d’accès. L’utilisation du format court (wasbs:///) provoque l’échec de l’action RunHiveScript lorsque le travail est démarré.
	...
	* Remplacez **serverName**, **adminLogin** et **adminPassword** par les informations de votre base de données SQL Azure.
	...
	> [AZURE.NOTE] L’entrée **oozie.wf.application.path** définit l’emplacement du fichier workflow.xml, qui contient le flux de travail exécuté par ce travail.
...
> [AZURE.IMPORTANT] Lorsque vous utilisez la commande Oozie, vous devez utiliser le nom de domaine complet pour le nœud principal HDInsight. Ce nom de domaine complet est uniquement accessible à partir du cluster, ou, si le cluster se trouve sur un réseau virtuel Azure, à partir des autres ordinateurs sur le même réseau.
...
4. Pour démarrer le travail, utilisez la commande suivante :
...
5. Une fois le travail terminé correctement, vous pouvez vérifier que les données ont été générées et exportées vers la table de base de données SQL en utilisant les commandes suivantes :
...
5. Sélectionnez un travail pour afficher plus d’informations sur celui-ci.
	...
	![Informations sur le travail](./media/hdinsight-use-oozie-linux-mac/jobinfo.png)
...
> [AZURE.NOTE] Il peut se passer plusieurs minutes avant que HDInsight reconnaisse que l'utilisateur a été ajouté au groupe.
