...
> 
> Ce document inclut également des informations sur l’utilisation d’Azure PowerShell et du Kit de développement logiciel (SDK) .NET HDInsight. Les extraits de code fournis pour ces derniers sont basés sur des commandes qui utilisent Azure Service Management (ASM) pour travailler avec HDInsight et qui sont **déconseillées**. Ces commandes seront supprimées d’ici au 1er janvier 2017.
> 
> Pour obtenir une version de ce document qui utilise le portail Azure, ainsi que des extraits de code du Kit de développement logiciel (SDK) .NET HDInsight et PowerShell qui recourent à Azure Resource Manager (ARM), consultez [Customize HDInsight clusters using Script Action](hdinsight-hadoop-customize-cluster.md) (Personnaliser des clusters HDInsight à l’aide d’une action de script).
> 
> 
...
Les scripts d'action de script peuvent être utilisés à partir du portail Azure Classic, d'Azure PowerShell ou du Kit de développement logiciel (SDK) .NET HDInsight.
...
**Depuis le portail Azure Classic**
