...
> [!IMPORTANT]
> Linux est le seul système d’exploitation utilisé sur HDInsight version 3.4 ou supérieure. Pour plus d’informations, consultez [l’article sur le contrôle de version de HDInsight](hdinsight-component-versioning.md#hdi-version-32-and-33-nearing-deprecation-date).

##<a name="notes-for-02282017-release-of-spark-21-on-hdinsight-36-preview"></a>Notes relatives à la version du 28/02/2017 de Spark 2.1 sur HDInsight 3.6 version préliminaire
...
## <a name="notes-for-10262016-release-of-r-server-on-hdinsight"></a>Notes relatives à la version du 26/10/2016 de R Server sur HDInsight
...
* Le prix du type de cluster R Server est désormais basé sur le prix de niveau Standard, plus le coût supplémentaire R Server. Le niveau Premium est désormais réservé aux fonctionnalités Premium disponibles sur différents types de cluster, et ne sera pas utilisé pour le type de cluster R Server. Cette modification n’affecte pas tarification effective de R Server. Elle modifie uniquement la façon dont les frais sont présentés sur la facture. Tous les clusters R Server existants continueront de fonctionner et les modèles ARM continueront de fonctionner jusqu’à l’avis de désapprobation. **Il est cependant recommandé de mettre à jour vos déploiements de script pour utiliser le nouveau modèle ARM.**

## <a name="notes-for-08302016-release-of-r-server-on-hdinsight"></a>Notes relatives à la version du 30/08/2016 de R Server sur HDInsight
...
## <a name="notes-for-08172016-release-of-r-server-on-hdinsight"></a>Notes relatives à la version du 17/08/2016 de R Server sur HDInsight
...
| --- | --- | --- | --- | --- |
| Modifications apportées aux clusters HDInsight 3.4 |Les valeurs par défaut des configurations hive suivantes sont modifiées pour optimiser les performances  <ul><li>`hive.vectorized.execution.reduce.enabled=true`</li><li>`hive.tez.min.partition.factor=1f`</li><li>`hive.tez.max.partition.factor=3f`</li><li>`tez.shuffle-vertex-manager.min-src-fraction=0.9`</li><li>`tez.shuffle-vertex-manager.max-src-fraction=0.95`</li><li>`tez.runtime.shuffle.connect.timeout= 30000`</li></ul> |de diffusion en continu |Tout |N/A |
