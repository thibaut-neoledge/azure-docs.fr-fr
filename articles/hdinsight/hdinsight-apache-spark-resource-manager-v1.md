...
# Gérer les ressources du cluster Apache Spark dans HDInsight Windows (version préliminaire)

> [AZURE.NOTE] HDInsight fournit désormais des clusters Spark sur Linux. Pour plus d’informations sur la gestion des ressources pour un cluster Spark sur HDInsight Linux, consultez [Gérer les ressources pour le cluster Apache Spark dans Azure HDInsight (Linux)](hdinsight-apache-spark-resource-manager.md).
