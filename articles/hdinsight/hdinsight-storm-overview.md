...
Apache Storm sur HDInsight est un cluster géré intégré dans l‘environnement Azure. Storm et les autres composants Hadoop sur HDInsight sont basés sur la plateforme HDP (Hortonworks Data Platform), tandis que le système d’exploitation du cluster correspond à Ubuntu (distribution Linux). Cela permet de créer une plateforme compatible avec les outils et services les plus courants dans l’écosystème Hadoop.
...
* Vous pouvez utiliser le langage de votre choix ; il prend en charge les composants Storm écrits en **Java**, en **C#** et en **Python**.
...
* Il s’intègre avec d’autres services Azure, notamment le concentrateur d’événements, le réseau virtuel Azure, la base de données SQL, le stockage Blob et DocumentDB.
...
* __Connectivité Secure Shell__ : vous pouvez accéder aux nœuds principaux de votre cluster HDInsight via internet, à l’aide de SSH. Cela vous permet d’exécuter des commandes directement sur le cluster.

  Pour en savoir plus, voir [Utilisation de SSH avec Hadoop Linux sur HDInsight depuis Linux, Unix ou OS X](hdinsight-hadoop-linux-use-ssh-unix.md).

* __Connectivité web__ : les clusters HDInsight fournissent l’interface utilisateur web d’Ambari. Elle vous permet de surveiller, configurer et gérer facilement des services sur votre cluster. Storm sur HDInsight fournit également l’interface utilisateur Storm, qui permet de surveiller et gérer l’exécution de topologies Storm à partir de votre navigateur.
...
* __Intégration de Visual Studio__ : le logiciel Data Lake Tools pour Visual Studio (anciennement appelé HDInsight Tools pour Visual Studio) inclut des modèles de projet pour la création de topologies Storm en C#, ainsi que des outils pour Storm sur HDInsight. Vous pouvez créer, déployer, surveiller et gérer vos topologies C# depuis Visual Studio.
...
* __Intégration avec d’autres services Azure__
  ...
  * Pour le développement en __Java__, Microsoft s’appuie sur les composants Storm existants pour intégrer d’autres services Azure, lorsque cela est possible. Dans certains cas, un composant ou une solution spécifiques aux services peuvent être requis.
    ...
    * __Stockage Blob Azure__ (lorsqu’il est utilisé en tant que stockage pour HDInsight) : les topologies basées sur Java peuvent accéder au stockage Blob Azure associé au cluster à l’aide du bolt Storm-HDFS, avec un schéma d’URI `wasb://`.
  ...
  * Pour le développement en __C#__, vous pouvez généralement utiliser le Kit de développement logiciel (SDK) .NET pour le service Azure. Dans certains cas, ce kit peut s’appuyer sur des infrastructures qui ne sont pas disponibles sous Linux (système d’exploitation hôte pour HDInsight versions 3.4 et plus.) Dans ce cas, vous pouvez utiliser les composants Java à partir de votre solution C#.
    ...
    * Des exemples d’utilisation de la __base de données SQL__, __DocumentDB__, __EventHub__ et __HBase__ sont inclus en tant que modèles dans Azure Data Lake Tools pour Visual Studio. Voir [Développement de topologies C# pour Apache Storm sur HDInsight à l’aide des outils Hadoop pour Visual Studio](hdinsight-storm-develop-csharp-visual-studio-topology.md) pour en savoir plus.
...
Les exemples storm-starter sont situés dans le répertoire ** /usr/hdp/current/storm-client/contrib/storm-starter** répertoire sur votre cluster HDInsight.
