...
* Téléchargez et installez [Microsoft Visual Studio Express pour Windows Desktop](https://www.visualstudio.com/products/visual-studio-express-vs.aspx) sur votre station de travail. Vous utiliserez l'outil makecert du package pour créer votre certificat.  
...
### <a name="configure-a-point-to-site-vpn-connection-to-the-azure-virtual-network"></a>Configurer une connexion VPN de point à site au réseau virtuel Azure
...
9. Cliquez sur **OUI** pour confirmer la modification. Attendez que le système ait terminé d'appliquer la modification avant de passer à la procédure suivante.
...
L’une des méthodes pour créer un certificat X.509 consiste à utiliser l'outil de création de certificats (makecert.exe) fourni avec [Microsoft Visual Studio Express pour Windows Desktop](https://www.visualstudio.com/products/visual-studio-express-vs.aspx).
...
3. La commande suivante dans l'exemple ci-dessous crée et installe un certificat racine dans le magasin de certificats Personal sur votre station de travail, ainsi que le fichier .cer correspondant que vous téléchargerez plus tard dans le portail Azure Classic.
   ...
   > [!NOTE]
   > Comme vous avez créé un certificat racine permettant de générer des certificats clients, il peut être utile d'exporter ce certificat avec sa clé privée et de l'enregistrer à un emplacement sûr à partir duquel il pourra être récupéré.
   >
   >
...
6. La configuration par défaut installe les packages de base et standard.  Cliquez sur **Suivant**.
...
3. Dans le menu **Drivers** (Pilotes), cliquez sur **New Driver** (Nouveau pilote).
4. Entrez les informations suivantes :
   ...
   * **Class Name**: org.apache.phoenix.jdbc.PhoenixDriver
     ...
     > [!WARNING]
     > N'utilisez que des minuscules dans l'exemple d'URL. Vous pouvez utiliser le quorum complet zookeeper au cas où l’un d’eux est inactif.  Les noms d’hôte sont zookeeper0, zookeeper1 et zookeeper2.
     >
     >
...
2. Dans le menu **Aliases** (Alias), cliquez sur **New Alias** (Nouvel alias).
...
1. Cliquez sur l’onglet **SQL** juste à côté de l’onglet **Objects** (Objets).
...
* [Configurer la réplication HBase dans HDInsight](hbase/apache-hbase-replication.md): découvrez comment configurer la réplication HBase entre deux centres de données Azure.
