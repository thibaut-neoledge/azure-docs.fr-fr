...
* Ils peuvent être limités de manière à **s’exécuter sur certains types de nœuds uniquement**, par exemple des nœuds principaux ou des nœuds de travail (worker).
  ...
  > 
  > 
* Son état peut être **persistants** ou **ad hoc**.
  
    Les scripts **persistants** sont appliqués aux nœuds worker et sont automatiquement exécutés sur les nœuds créés lors de la montée en charge d’un cluster.
...
* Sont exécutés avec des **privilèges au niveau de la racine** sur les nœuds du cluster.
...
| Nom | Script |
| --- | --- |
| **Ajouter un compte Azure Storage** |https://hdiconfigactions.blob.core.windows.net/linuxaddstorageaccountv01/add-storage-account-v01.sh. Consultez [Appliquer une action de script sur un cluster en cours d’exécution](#apply-a-script-action-to-a-running-cluster). |
...
* Pour obtenir des instructions sur la création de modèles, consultez [Création de modèles Azure Resource Manager](../resource-group-authoring-templates.md).
...
4. Dans le panneau Ajouter une action de script, entrez les informations suivantes.
   ...
   * **URI de SCRIPT** : l’URI du script. Dans cet exemple : `https://hdiconfigactions.blob.core.windows.net/linuxaddstorageaccountv01/add-storage-account-v01.sh`
   ...
   * **PERSISTANT** : cochez cette entrée si vous souhaitez rendre le script persistant afin de l’appliquer aux nœuds worker créés lors de la montée en charge du cluster.
...
### Appliquer une action de script sur un cluster en cours d’exécution à l’aide de l’API REST
