...
Ici, vous pouvez voir que la version actuelle du nœud est `1.7.7` et que la version `1.8.1` est disponible.
...
Utilisez la commande `az aks upgrade` pour mettre à niveau les nœuds du cluster. Les exemples suivants mettent à jour le cluster vers la version `1.8.1`.
