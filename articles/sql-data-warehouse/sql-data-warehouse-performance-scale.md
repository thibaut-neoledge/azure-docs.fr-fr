...
Vous pouvez également obtenir le même résultat à l’aide d’Azure Powershell en important le module AzureRM.Sql et en utilisant le code suivant :
...
L’interruption et la reprise de votre capacité de calcul s’effectuent via le [portail Azure Classic][], via des API REST ou par le biais d’Azure PowerShell. L’interruption annule l’ensemble des activités en cours ou en attente. Le moment venu, vous pouvez relancer vos ressources de calcul en quelques secondes.

Pour suspendre et reprendre le service à l’aide d’Azure Powershell, vous devez d’abord importer le module AzureRM.Sql comme suit :

```Powershell
Import-Module AzureRM.Sql
```

Le code ci-dessous indique comment effectuer une pause à l’aide d’Azure PowerShell :

```Powershell
Suspend-AzureRmSqlDatabase –ResourceGroupName "ResourceGroup11" –ServerName "Server01" –DatabaseName "Database02"
```

Il est également très simple de reprendre le service à l’aide d’Azure PowerShell :

```Powershell
Resume-AzureRmSqlDatabase –ResourceGroupName "ResourceGroup11" –ServerName "Server01" –DatabaseName "Database02"
```

Pour en savoir plus sur l’utilisation d’Azure PowerShell, consultez [Utilisation des applets de commande PowerShell et des API REST avec SQL Data Warehouse][].
...
[Utilisation des applets de commande PowerShell et des API REST avec SQL Data Warehouse]: sql-data-warehouse-reference-powershell-cmdlets.md
