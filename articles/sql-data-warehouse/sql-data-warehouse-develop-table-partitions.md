...
Pour dimensionner votre base de données SQL Server actuelle au niveau de la partition, utilisez une requête semblable à ce qui suit :
...
Créez la table en tant que table partitionnée, mais sans ajouter de valeurs de partition :
...
Appliquez à la table la commande `SPLIT` lors du processus de déploiement :
