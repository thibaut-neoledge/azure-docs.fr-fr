...
## Réduire les coûts avec les opérations de suspension et de mise à l’échelle
...
Voir aussi [Transactions][Transactions], [Optimizing transactions][Optimizing transactions] \(Optimisation des transactions)
...
Voir aussi [Manage table statistics][Manage table statistics] \(Gérer les statistiques de table), [CREATE STATISTICS][CREATE STATISTICS], [UPDATE STATISTICS][UPDATE STATISTICS]
...
Consultez aussi [Load data][Load data] \(Charger des données), [Guide d’utilisation de PolyBase][Guide d’utilisation de PolyBase], [Modèles et stratégies de chargement Azure SQL Data Warehouse][Modèles et stratégies de chargement Azure SQL Data Warehouse], [Téléchargement de données avec Azure Data Factory][Téléchargement de données avec Azure Data Factory], [Move data with Azure Data Factory][Move data with Azure Data Factory] \(Déplacer des données avec Azure Data Factory, [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT], [Create table as select (CTAS)][Create table as select (CTAS)]

## Charger, puis interroger les tables externes
...
Voir aussi [Table overview][Table overview] \(Vue d’ensemble des tables), [Table distribution][Table distribution] \(Distribution de table), [Selecting table distribution][Selecting table distribution] \(Sélection d’une distribution de table), [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
Voir aussi [Transactions][Transactions], [Optimizing transactions][Optimizing transactions] \(Optimisation des transactions), [Partitionnement de table][Partitionnement de table], [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE], [Create table as select (CTAS)][Create table as select (CTAS)]
...
Voir aussi [Table overview][Table overview] \(Vue d’ensemble des tables), [Table data types][Table data types] \(Types de données des tables), [CREATE TABLE][CREATE TABLE]
...
Voir aussi [Temporary tables][Temporary tables] \(Tables temporaires), [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
Voir aussi [Gestion de la concurrence et des charges de travail][Gestion de la concurrence et des charges de travail], [sys.dm\_pdw\_waits][sys.dm\_pdw\_waits]
