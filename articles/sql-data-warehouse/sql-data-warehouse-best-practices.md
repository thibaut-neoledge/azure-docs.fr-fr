...
Voir aussi [Interrompre des ressources de calcul][Interrompre des ressources de calcul], [Reprendre des ressources de calcul][Reprendre des ressources de calcul], [Mettre à l’échelle des ressources de calcul][Mettre à l’échelle des ressources de calcul]
...
Voir aussi [Transactions][Transactions], [Optimisation des transactions][Optimisation des transactions]
...
Voir aussi [Gestion des statistiques sur les tables][Gestion des statistiques sur les tables], [CREATE STATISTICS][CREATE STATISTICS], [UPDATE STATISTICS][UPDATE STATISTICS]
...
Voir aussi [Charger des données][Charger des données], [Guide d’utilisation de PolyBase][Guide d’utilisation de PolyBase], [Stratégies et modèles de chargement Azure SQL Data Warehouse][Stratégies et modèles de chargement Azure SQL Data Warehouse], [Charger des données avec Azure Data Factory][Charger des données avec Azure Data Factory], [Déplacer des données avec Azure Data Factory][Déplacer des données avec Azure Data Factory], [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT], [Create table as select (CTAS)][Create table as select (CTAS)]
...
Voir aussi [Guide d’utilisation de PolyBase][Guide d’utilisation de PolyBase]
...
Voir aussi [Vue d’ensemble des tables][Vue d’ensemble des tables], [Distribution des tables][Distribution des tables], [Sélection de la distribution des tables][Sélection de la distribution des tables], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]

## <a name="do-not-overpartition"></a>Ne pas créer trop de partitions
...
Voir aussi [Partitionnement de table][Partitionnement de table]
...
Voir aussi [Transactions][Transactions], [Optimisation des transactions][Optimisation des transactions], [Partitionnement de table][Partitionnement de table], [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE], [Create table as select (CTAS)][Create table as select (CTAS)]
...
Voir aussi [Vue d’ensemble des tables][Vue d’ensemble des tables], [Types de données des tables][Types de données des tables], [CREATE TABLE][CREATE TABLE]
...
Voir aussi [Index de table][Index de table], [Guide des index columnstore][Guide des index columnstore], [Reconstruction des index columnstore][Reconstruction des index columnstore]
...
Voir aussi [Gestion de la concurrence et des charges de travail][Gestion de la concurrence et des charges de travail]
...
Voir aussi [Gestion de la concurrence et des charges de travail][Gestion de la concurrence et des charges de travail], [sys.dm_pdw_waits][sys.dm_pdw_waits]
...
Voir aussi [Surveiller votre charge de travail à l’aide de vues de gestion dynamique][Surveiller votre charge de travail à l’aide de vues de gestion dynamique], [LABEL][LABEL], [OPTION][OPTION], [sys.dm_exec_sessions][sys.dm_exec_sessions], [sys.dm_pdw_exec_requests][sys.dm_pdw_exec_requests], [sys.dm_pdw_request_steps][sys.dm_pdw_request_steps], [sys.dm_pdw_sql_requests][sys.dm_pdw_sql_requests], [sys.dm_pdw_dms_workers], [DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN], [sys.dm_pdw_waits][sys.dm_pdw_waits]

## <a name="other-resources"></a>Autres ressources
Consultez également notre article [Dépannage][Dépannage] concernant les problèmes courants et leurs solutions.

Si vous ne trouvez pas ce que vous recherchez dans cet article, essayez d’utiliser la fonction de recherche de documents située sur le côté gauche de cette page pour rechercher tous les documents relatifs à Azure SQL Data Warehouse.  Le [Forum MSDN Azure SQL Data Warehouse][Forum MSDN Azure SQL Data Warehouse] a été créé pour vous permettre de poser des questions à d’autres utilisateurs et au groupe de produits SQL Data Warehouse.  Nous suivons activement ce forum pour vous assurer que vos questions sont traitées par un autre utilisateur ou un membre de notre équipe.  Si vous préférez poser vos questions sur Stack Overflow, nous avons également un [Forum Azure SQL Data Warehouse Stack Overflow][Forum Azure SQL Data Warehouse Stack Overflow].

Enfin, utilisez la page des [commentaires relatifs à Azure SQL Data Warehouse][commentaires relatifs à Azure SQL Data Warehouse] afin de faire des demandes de fonctionnalités.  L’ajout de vos demandes ou la confirmation des autres demandes nous permet vraiment de hiérarchiser les fonctions.
...
[Types de données des tables]: ./sql-data-warehouse-tables-data-types.md
...
[Gestion des statistiques sur les tables]: ./sql-data-warehouse-tables-statistics.md
...
[Chargement des données]: ./sql-data-warehouse-overview-load.md
[Déplacer les données avec Azure Data Factory]: ../data-factory/data-factory-azure-sql-data-warehouse-connector.md
...
[Sélection d’une distribution de tables]: https://blogs.msdn.microsoft.com/sqlcat/2015/08/11/choosing-hash-distributed-table-vs-round-robin-distributed-table-in-azure-sql-dw-service/
