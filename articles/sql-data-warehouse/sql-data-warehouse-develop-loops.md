...
Toutefois, avant de vous lancer, vous devez vous poser la question suivante : « Puis-je réécrire ce curseur afin d’utiliser des opérations basées sur un jeu ? ». Dans de nombreux cas, la réponse est « oui ». Bien souvent, la meilleure solution est justement d’effectuer cette opération. Une opération basée sur un jeu s’exécute généralement beaucoup plus rapidement qu’une méthode itérative de type ligne par ligne.

Les curseurs à avance rapide et en lecture seule peuvent facilement être remplacés par des constructions en boucle. Voici un exemple illustrant cette méthode de manière assez simple : L’exemple de code met à jour les statistiques de chaque table dans la base de données. En effectuant une itération sur les tables dans la boucle, nous sommes en mesure d’exécuter chaque commande dans l’ordre.
...
Deuxièmement, initialisez les variables requises pour exécuter la boucle :
