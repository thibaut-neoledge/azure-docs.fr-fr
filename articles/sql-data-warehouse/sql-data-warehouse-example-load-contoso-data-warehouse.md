<properties
   pageTitle="Chargement de l’entrepôt de données Contoso Retail dans SQL Data Warehouse | Microsoft Azure"
   description="Découvrez comment charger l’entrepôt de données Contoso Retail dans SQL Data Warehouse à l’aide de PolyBase pour optimiser les performances"
...
# Chargement de l’entrepôt de données Contoso Retail

Dans cet article, vous allez apprendre à charger rapidement des données dans le schéma de l’entrepôt de données Contoso Retail.

Ce jeu de données vous offre une autre source de données qui vous permet de tout apprendre sur Azure SQL Data Warehouse.

Le jeu de données Contoso a une taille d’environ Go. Les données brutes sont conservées dans un stockage Azure Blob Storage auquel nous accéderons directement pour charger les données dans SQL Data Warehouse.

Ce didacticiel vous apprendra à effectuer les opérations suivantes :

1. Configurer PolyBase pour charger des données à partir d’un conteneur public de stockage d’objets blob
2. Importer des données à l’aide de [CTAS][]
3. Régénérer (avec [REBUILD][]) les tables afin d’optimiser les index columnstore
4. Créer des statistiques sur les colonnes clés

## Configuration de PolyBase

La première étape lors du chargement des données consiste à configurer les scripts requis par PolyBase :

### Créer la source de données externe

Comme les données sont conservées dans un conteneur public, la première tâche consiste à créer la source de données externe :
...
Dans ce cas, il est inutile de créer une clé principale ou des informations d’identification incluses dans l’étendue de la base de données, car le conteneur de stockage d’objets blob est configuré comme public, ce qui signifie que toute personne peut le lire et l’énumérer.

> [AZURE.IMPORTANT] Si vous choisissez de rendre publics vos conteneurs de stockage d’objets blob Azure, n’oubliez pas que des frais d’acheminement s’appliquent lorsque des données quittent le centre de données.

### Créer un objet au format de fichier externe

Le format de fichier fournit les informations requises pour analyser les fichiers texte stockés dans le stockage d’objets blob.
...
Dans notre cas, les données sont non compressées et séparées par des barres verticales.

## Créer les tables externes

Une fois la connectivité au stockage d’objets blob Azure configurée, nous pouvons créer les tables externes.

Par souci de clarté, commençons par créer un schéma :
...
Maintenant, passons à la création des tables externes. N’oubliez pas que tout ce que nous faisons ici fournit le mappage des types de données. Il faut associer cette définition aux métadonnées pour localiser les fichiers.

Ne soyez pas surpris : ce script est long !
...
## Charger les données avec CTAS

Pour charger les données, il est préférable d’utiliser les tables externes fortement typées que nous venons de créer. En l’occurrence, des instructions [CTAS][] simples suffiront.

Toutefois, commençons par créer un schéma pour charger nos tables.
...
Pour charger les données, nous utilisons l’instruction [CTAS][]. Notez qu’il y a une instruction [CTAS][] par table.
...
Vous pouvez suivre la progression de votre charge à l’aide de la vue de gestion dynamique (DMV) `[sys].[dm_pdw_exec_requests]`. Comme les instructions CTAS ci-dessus ont un `label`, vous pouvez également suivre la progression à l’aide du `label`. Pour plus d’informations, consultez l’article sur les [libellés][].

## Reconstruire les index

Enfin, pour vérifier que les tables sont au format columnstore compressé, nous pouvons lancer une opération de reconstruction sur la table. Ceci convertit les groupes de lignes ouverts ou fermés au format columnstore compressé.
...
Pour plus d’informations sur la vérification de l’état de votre index columnstore clustérisé, consultez l’article [gérer les index columnstore][].

## Créer des objets de statistiques

Pour créer des statistiques sur toutes les colonnes, vous pouvez utiliser le code de la procédure stockée `prc_sqldw_create_stats` décrit dans l’article sur les [statistiques][].

Voici une partie de la sortie couvrant les dimensions complètes et toutes les colonnes de jointure dans l’entrepôt de données Contoso Retail. Il s’agit d’un bon point de départ.

Vous pouvez ajouter d’autres statistiques dans d’autres colonnes ultérieurement.

Là encore, ce script est assez long.
...
Toutes les données sont maintenant chargées dans Azure SQL Data Warehouse. Bon travail !

Vous pouvez maintenant interroger les tables à l’aide de requêtes comme celle ci-dessous :
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/fr-FR/library/mt203887.aspx
[REBUILD]: https://msdn.microsoft.com/fr-FR/library/ms188388.aspx
