...
Cet article fournit les propriétés et les schémas des événements. Les événements se composent de cinq propriétés de chaîne et d’un objet **données** obligatoires. Les propriétés sont communes à tous les événements, quel que soit le serveur de publication. L’objet **données** contient des propriétés spécifiques à chaque serveur de publication. Dans les rubriques du système, ces propriétés sont spécifiques au fournisseur de ressources, comme le stockage ou les concentrateurs d’événements.

Les événements sont envoyés à Azure Event Grid sous forme de tableau qui peut contenir plusieurs objets d’événement. S’il n’existe qu’un seul événement, la longueur du tableau est de 1. 
...
| subject | string | Chemin d’accès à l’objet de l’événement, défini par le serveur de publication. |
...
Actuellement, les événements Event Hubs sont générés uniquement lorsqu’un fichier est automatiquement envoyé vers un volume de stockage à l’aide de la fonctionnalité Capture.
...
Cet exemple d’événement montre le schéma d’un événement Event Hubs déclenché lorsqu’un fichier est enregistré via la fonctionnalité Capture. 
...
La charge utile de données de vos événements personnalisés est définie par vous, sous la forme d’un fichier JSON correctement formaté. Les données de niveau supérieur doivent contenir les mêmes champs que les événements de ressources standard définis. Lors de la publication d’événements dans les rubriques personnalisées, vous devez envisager de modéliser l’objet de vos événements pour faciliter les opérations de routage et de filtrage.
...
* Pour découvrir comment créer un abonnement Event Grid, consultez l’article [Schéma d’abonnement à Event Grid](subscription-creation-schema.md).
