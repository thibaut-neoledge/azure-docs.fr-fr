...
##S'inscire à GCM et activer le service GCM
...
Suivez cette procédure pour activer GCM dans votre compte. Ne lisez pas la section **Obtaining an API Key** et revenez à cette page au lieu de suivre la procédure de Google.
...
> [AZURE.IMPORTANT] Le **numéro de projet** ne doit pas être confondu avec l'**ID du projet**. L'ID du projet peut désormais être différencié (c'est un nom dans les nouveaux projets). Ce que vous devez intégrer dans le SDK Engagement est le **numéro de projet**. Celui-ci se trouve dans le menu **Overview** de la [Console développeurs de Google].
...
Pour cela, ajoutez le code suivant au fichier  `AndroidManifest.xml`, à l'intérieur de la balise `<application/>`j:
...
-   Accédez à APIs & auth -\> Credentials. Cliquez sur CREATE NEW KEY dans la section Public API access.
...
[Console développeur de Google]:https://cloud.google.com/console
