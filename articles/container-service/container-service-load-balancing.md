...
Dans cet article, nous allons configurer un serveur web frontal que vous pouvez faire monter en puissance afin de fournir des services derrière Azure Load Balancer.
...
Deux couches d’équilibrage de charge sont présentes dans un cluster Azure Container Service : Azure Load Balancer pour les points d’entrée publics (ceux qu’atteignent les utilisateurs finaux) et l’équilibreur de charge Marathon (marathon-lb) sous-jacent, qui achemine les demandes entrantes aux instances du conteneur qui les traitent. L’équilibreur de charge marathon-lb s’adapte dynamiquement au fur et à mesure que les conteneurs fournissant le service sont mis à l’échelle.

## Équilibreur de charge Marathon

L’équilibreur de charge Marathon se reconfigure dynamiquement en fonction des conteneurs que vous avez déployés. Il est également résistant à la perte d’un conteneur ou d’un agent ; le cas échéant, Mesos redémarre simplement le conteneur ailleurs et reconfigure l’équilibreur de charge Marathon.

Pour installer l’équilibreur de charge Marathon, exécutez la commande suivante à partir de votre ordinateur client :
...
Les éléments clés sont les suivants :
  * Configurez la valeur de HAProxy\_0\_VHOST sur le nom de domaine complet de l’équilibrage de charge pour vos agents. Il s’agit de la forme `<acsName>agents.<region>.cloudapp.azure.com`. Par exemple, si vous créez un cluster Container Service avec le nom `myacs` dans la région `West US`, le nom de domaine complet serait `myacsagents.westus.cloudapp.azure.com`. Vous pouvez également le trouver en recherchant l’équilibreur de charge avec le terme « agent » dans le nom lorsque vous parcourez les ressources du groupe de ressources que vous avez créé pour votre service de conteneur dans le [portail Azure](https://portal.azure.com).
...
Copiez cet élément JSON dans un fichier appelé `hello-web.json` et utilisez-le pour déployer un conteneur :
...
Pour ce faire, consultez la section consacrée aux [hôtes virtuels](https://mesosphere.com/blog/2015/12/04/dcos-marathon-lb/) , qui fournit un moyen d’associer des domaines à des chemins d’accès marathon-lb spécifiques.
...
Découvrez [ce billet de blog sur l’équilibrage de charge](https://mesosphere.com/blog/2015/12/04/dcos-marathon-lb/) pour plus d’informations sur l’équilibreur de charge Marathon.
