...
1. Créez une machine virtuelle Azure avec Ubuntu 16.04 LTS. 
2. Installez Jenkins en suivant ces [instructions](https://wiki.jenkins-ci.org/display/JENKINS/Installing+Jenkins+on+Ubuntu).
3. Un didacticiel plus détaillé est disponible sur [howtoforge.com](https://www.howtoforge.com/tutorial/how-to-install-jenkins-with-apache-on-ubuntu-16-04).
4. Mettez à jour le groupe de sécurité réseau Azure pour autoriser le port 8080, puis recherchez l’adresse IP publique au port 8080 pour gérer Jenkins dans votre navigateur.
5. Le mot de passe administrateur initial pour Jenkins est stocké dans /var/lib/jenkins/secrets/initialAdminPassword.
6. Installez Docker sur l’ordinateur Jenkins via ces [instructions](https://docs.docker.com/cs-engine/1.13/#install-on-ubuntu-1404-lts-or-1604-lts). Ainsi, les commandes Docker peuvent être exécutées dans les travaux Jenkins.
7. Configurez les autorisations de Docker pour autoriser Jenkins à accéder au point de terminaison.

    ```bash
    sudo chmod 777 /run/docker.sock
    ```
8. Installez la CLI `kubectl` sur Jenkins. Des informations supplémentaires sont disponibles dans l’article [Installation et configuration de kubectl](https://kubernetes.io/docs/tasks/kubectl/install/).
...
1. Copiez le fichier de configuration `kubectl` sur l’ordinateur Jenkins.
...
2. Vérifiez via Jenkins que le cluster Kubernetes est accessible.
