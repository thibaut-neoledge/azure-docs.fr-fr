...
Créez un tunnel SSH (Secure Shell), puis accédez à http://localhost/. Cela charge l'interface utilisateur Web de DC/OS et affiche des informations le cluster, notamment les ressources utilisées, les agents actifs, et les services en cours d’exécution.
...
Pour déployer un nouveau conteneur à l'aide de Marathon, cliquez sur le bouton **Créer une application** et entrez les informations suivantes dans le formulaire. Cliquez sur **Créer une application** quand vous souhaitez commencer.
...
Si vous revenez à l’application DC/OS (http://localhost/)), vous voyez maintenant qu’une tâche, dans ce cas de figure un conteneur formaté Docker, est en cours d’exécution sur le cluster DC/OS.
