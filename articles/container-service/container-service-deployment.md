...
6. Cliquez sur **OK** lorsque vous souhaitez continuer.
    ...
    ![Créer un déploiement 4](media/acs-portal4-new.png)  <br />
...
7. Si **Kubernetes** est sélectionné dans la liste déroulante, vous devrez entrer un ID client et la clé secrète client du principal du service. Pour en savoir plus, consultez [À propos du principal du service pour un cluster Kubernetes](container-service-kubernetes-service-principal.md) (en anglais).
    ...
    ![Créer un déploiement 4.5](media/acs-portal10.PNG)  <br />
...
7. Dans le panneau des paramètres **Azure Container Service**, entrez les informations suivantes :
    ...
    * **Nombre d’agents** : pour Docker Swarm et Kubernetes, il s’agit du nombre initial d’agents dans le jeu de mise à l’échelle d’agent. Pour DC/OS, ce sera le nombre initial d’agents dans un jeu de mise à l’échelle privé. En outre, un jeu de mise à l’échelle public est créé et contient un nombre prédéterminé d’agents. Le nombre d’agents dans ce jeu de mise à l’échelle public est déterminé par le nombre de maîtres créés dans le cluster : un agent public pour un maître, et deux agents publics pour trois ou cinq maîtres.
...
10. Cliquez sur **Acheter** pour entamer le processus de déploiement.
...
1. Sélectionnez l’un des modèles GitHub suivants pour déployer un cluster DC/OS, Docker Swarm ou Kubernetes. 
...
2. Assurez-vous que la CLI Azure a bien été liée à un abonnement Azure. Pour ce faire, vous pouvez utiliser la commande suivante :
...
3. Configurez les outils de l’interface de ligne de commande Azure pour qu’ils utilisent Azure Resource Manager.

    ```bash
    azure config mode arm
    ```

4. Créez un groupe de ressources Azure et un cluster Container Service avec la commande suivante, où :
...
1. Sélectionnez l’un des modèles suivants pour déployer un cluster DC/OS, Docker Swarm ou Kubernetes. Notez que ces deux modèles sont identiques, à l’exception de la sélection de l’orchestrateur par défaut.
...
4. Si vous effectuez un déploiement vers un nouveau groupe de ressources, vous devez commencer par créer ce dernier. Pour créer un groupe de ressources, utilisez la commande `New-AzureRmResourceGroup` et spécifiez un nom de groupe de ressources ainsi qu’une région de destination :
