...
**[L’infrastructure Spring]** est une solution open source qui permet aux développeurs Java de créer des applications d’entreprise. Un des projets les plus connus créé sur cette plateforme est [Spring Boot], qui fournit une approche simplifiée pour la création d’applications Java autonomes.
...
1. Étape facultative : Si vous voulez exécuter le serveur Tomcat incorporé sur le port 80 au lieu du port par défaut 8080, (par exemple si vous voulez tester votre projet Spring Boot localement), vous pouvez configurer le port en procédant comme suit :
...
1. Quand le panneau **Créer un registre de conteneurs** s’affiche, entrez votre **Nom du registre** et votre **Groupe de ressources**, choisissez **Activer** pour **l’utilisateur administrateur** puis cliquez sur **Créer**.
...
>
> Dans ce cas, il peut être nécessaire de se connecter à Azure à partir de la ligne de commande de Docker. Par exemple :
>
...
1. Quand le panneau **Application web sur Linux** s’affiche, entrez les informations suivantes :
...
> [!NOTE]
>
> Azure mappera automatiquement les demandes Internet au serveur Tomcat incorporé qui s’exécute sur les ports standard 80 ou 8080. Cependant, si vous avez configuré votre serveur Tomcat incorporé pour qui s’exécute sur un port personnalisé, vous devez ajouter une variable d’environnement à votre application web qui définit le port pour votre serveur Tomcat incorporé. Pour ce faire, procédez comme suit :
>
> 1. Accédez au [portail Azure] et connectez-vous.
> 
...
[L’infrastructure Spring]: https://spring.io/
