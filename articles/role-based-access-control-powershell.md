...
Le contrôle d'accès basé sur un rôle (RBAC) dans l'API de la version préliminaire du portail Azure et l'API du gestionnaire de ressources Azure permet une gestion très fine de l'accès à votre abonnement. Cette fonctionnalité vous permet d'accorder l'accès aux utilisateurs, groupes et principaux du service Active Directory en leur affectant certains rôles avec une étendue spécifique.
...
-   Windows PowerShell, version 3.0 ou 4.0. Pour connaître votre version de Windows PowerShell, entrez :`$PSVersionTable` et vérifiez que la valeur de `PSVersion` est 3.0 ou 4.0. Pour installer une version compatible, voir la section [Windows Management Framework 3.0][Windows Management Framework 3.0] ou [Windows Management Framework 4.0][Windows Management Framework 4.0].

-   Azure PowerShell, version 0.8.8 ou ultérieure. Pour installer la dernière version et l'associer à votre abonnement Azure, accédez à la page [Installation et configuration d'Azure PowerShell][Installation et configuration d'Azure PowerShell].
...
-   [Utilisation de Windows PowerShell avec le gestionnaire des ressources][Utilisation de Windows PowerShell avec le gestionnaire des ressources]
...
Pour plus d'informations, voir la section [Utilisation de Windows PowerShell avec le gestionnaire de ressources][Utilisation de Windows PowerShell avec le gestionnaire des ressources].
...
Pour plus d'informations, consultez la section [Installation et configuration d'Azure PowerShell][Installation et configuration d’Azure PowerShell].
...
-   Qui souhaitez-vous affecter au rôle : vous pouvez utiliser les applets de commande Azure Active Directory suivants pour savoir quels utilisateurs, groupes et principaux du service sont inclus dans votre locataire AD.
...
-   Quelle étendue vous souhaitez affecter : il existe trois niveaux d'étendue
    ...
    -   L'abonnement actuel
    -   Un groupe de ressources ; pour obtenir la liste des groupes de ressources, entrez `PS C:\> Get-AzureResourceGroup`; Get-AzureResourceGroup</code>
    -   Une ressource ; pour obtenir la liste des ressources, entrez `PS C:\> Get-AzureResource`; Get-AzureResource</code>
...
Et donc, lorsque vous tentez d'exécuter un autre applet de commande tel que `New-AzureResourceGroup`, vous recevez une erreur d’accès refusé si vous ne disposez pas de l'autorisation requise.
...
-   [Cmdlets Azure Resource Manager][Cmdlets Azure Resource Manager] : apprenez à utiliser les cmdlets (applets de commande) dans le module AzureResourceManager.
