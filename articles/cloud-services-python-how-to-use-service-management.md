...
Ce guide vous explique comment effectuer des tâches courantes de gestion des services par programme à partir de Python. La classe **ServiceManagementService** du [Kit de développement logiciel Azure pour Python][download-SDK-Python] prend en charge l'accès par programme à la plupart des fonctionnalités liées à la gestion de service disponible dans le [portail de gestion][management-portal] (comme **la création, la mise à jour et la suppression de services cloud, de déploiements, de services de gestion de données, de machines virtuelles et de groupes d'affinités**). Ces fonctionnalités peuvent être utiles pour la création d'applications nécessitant un accès par programme à la gestion des services. 

## Sommaire

* [Présentation de la gestion des services][]
* [Concepts][]
* [Procédure : Connexion à la gestion des services][]
* [Procédure : Affichage de la liste des emplacements disponibles][]
* [Procédure : Création d'un service cloud][]
* [Procédure : Suppression d'un service cloud][]
* [Procédure : Création d'un déploiement][]
* [Procédure : Mise à jour d'un déploiement][]
* [Procédure : Transfert des déploiements entre les environnements intermédiaire et de production][]
* [Procédure : Suppression d'un déploiement][]
* [Procédure : Création d'un service de stockage][]
* [Procédure : Suppression d'un service de stockage][]
* [Procédure : Création d'un groupe d'affinités][]
* [Procédure : Suppression d'un groupe d'affinités][]
* [Procédure : Affichage de la liste des systèmes d'exploitation disponibles][]
* [Procédure : Création d'une image du système d'exploitation][]
* [Procédure : Suppression d'une image du système d'exploitation][]
...
L'API de gestion des services fournit un accès par programme aux fonctionnalités de gestion des services disponibles par le biais du [portail de gestion][management-portal]. Le Kit de développement logiciel (SDK) Azure pour Python vous permet de gérer vos services cloud, comptes de stockage et groupes d'affinités.

Pour utiliser l'API de gestion de Service, vous devrez [créer un compte Azure](http://azure.microsoft.com/pricing/free-trial/). 
...
## <a name="Connect"> </a>Procédure : Connexion à la gestion des services
...
> [AZURE.NOTE] À partir du Kit de développement logiciel (SDK) Azure pour Python v0.8.0, il est maintenant possible d'utiliser des certificats créés avec OpenSSL sous Windows.  Ceci nécessite Python 2.7.4 ou version ultérieure.
...
Une fois que vous avez créé le certificat, vous devez télécharger le fichier `.cer` sur Azure via l'action Télécharger de l'onglet Paramètres dans le [portail de gestion][management-portal].

Après avoir obtenu votre ID d'abonnement, créé un certificat et téléchargé le fichier `.cer` sur Azure, vous pouvez vous connecter au point de terminaison de gestion Azure en transmettant l'ID d'abonnement et l'emplacement du certificat dans votre magasin de certificats **Personnel** vers **ServiceManagementService** (là encore, remplacez *AzureCertificate* par le nom de votre certificat) :
...
Vous pouvez utiliser [OpenSSL](http://www.openssl.org/) pour créer votre certificat de gestion.  En fait, vous devez créer deux certificats, un pour le serveur (un fichier `.cer`) et un pour le client (un fichier `.pem`). Pour créer le fichier `.pem`, exécutez ceci :

	`openssl req -x509 -nodes -days 365 -newkey rsa:1024 -keyout mycert.pem -out mycert.pem`

Pour créer le certificat `.cer`, exécutez ceci :
...
Après avoir créé ces fichiers, vous devez télécharger le fichier `.cer` sur Azure via l'action Télécharger de l'onglet Paramètres du [portail de gestion][management-portal], et vous devrez noter l'emplacement dans lequel vous avez enregistré le fichier `.pem`.
...
## <a name="ListAvailableLocations"> </a>Procédure : Affichage de la liste des emplacements disponibles
...
Vous pouvez répertorier tous les services hébergés pour votre abonnement grâce à la méthode **list\_hosted\_services** :
...
Une fois que vous avez créé un service cloud, vous pouvez déployer votre code sur le service en utilisant la méthode **create\_deployment**.

## <a name="DeleteCloudService"> </a>Procédure : Suppression d'un service cloud

Vous pouvez supprimer un service cloud en transmettant le nom du service à la méthode **delete\_hosted\_service** :
...
## <a name="CreateDeployment"> </a>Procédure : Création d'un déploiement
...
* **package_url** : URL du package de déploiement (fichier .cspgk). Le fichier de package doit être stocké dans un compte de stockage d'objets blob Azure sous le même abonnement que le service hébergé sur lequel le package est téléchargé. Vous pouvez créer un package de déploiement au moyen des [applets de commande Azure PowerShell] ou de l'[outil de ligne de commande cspack].
...
L'exemple suivant crée un nouveau déploiement `v1` for a hosted service called `myhostedservice` :
...
Vous pouvez accéder à des propriétés de déploiement en utilisant les méthodes **get\_deployment\_by\_slot** ou **get\_deployment\_by\_name**. L'exemple suivant extrait un déploiement en spécifiant son emplacement. Il effectue également une itération dans toutes les instances du déploiement :
...
## <a name="UpdateDeployment"> </a>Procédure : Mise à jour d'un déploiement

Un déploiement peut être mis à jour à l'aide de la méthode **change\_deployment\_configuration** ou **update\_deployment\_status**.

La méthode **change\_deployment\_configuration** vous permet de télécharger un nouveau fichier de configuration de service (`.cscfg`), qui changera n'importe quel paramètre de service (y compris le nombre d'instances dans un déploiement). Pour plus d'informations, consultez la rubrique [Schéma de configuration de service Azure (.cscfg)]. L'exemple suivant indique comment télécharger un nouveau fichier de configuration de service :
...
La méthode **update\_deployment\_status** vous permet de définir un état de déploiement sur RUNNING ou SUSPENDED. L'exemple suivant montre comment définir l'état sur RUNNING pour un déploiement nommé `v1` of a hosted service called `myhostedservice` :
...
## <a name="MoveDeployments"> </a>Procédure : des déploiements entre les environnements intermédiaire et de production

Azure fournit deux environnements de déploiement : intermédiaire et de production. Généralement, un service est déployé dans l'environnement intermédiaire pour être testé avant son déploiement dans l'environnement de production. Lorsqu'il est temps de promouvoir le service de l'environnement intermédiaire à l'environnement de production, vous pouvez le faire sans redéployer le service. Pour cela, intervertissez les déploiements (Pour plus d'informations sur l'échange de déploiements, consultez la rubrique [Déploiement d'un service Azure]).

L'exemple suivant indique comment utiliser la méthode **swap\_deployment** pour intervertir deux déploiements (avec les noms `v1` et `v2`). Dans l'exemple, avant l'appel de **swap\_deployment**, le déploiement `v1` se trouve dans l'emplacement de production, et le déploiement `v2` dans l'emplacement intermédiaire. Après avoir appelé **swap\_deployment**, `v2` est en production et `v1` est en transit.  
...
## <a name="DeleteDeployment"> </a>Procédure : Suppression d'un déploiement
...
Vous pouvez répertorier vos comptes de stockage et leurs propriétés en utilisant la méthode **list\_storage\_accounts** :
...
## <a name="DeleteStorageService"> </a>Procédure : Suppression d'un service de stockage
...
## <a name="CreateAffinityGroup"> </a>Procédure : Création d'un groupe d'affinités
...
Vous pouvez répertorier les groupes d'affinités et inspecter leurs propriétés en appelant la méthode **list\_affinity\_groups** :
...
## <a name="DeleteAffinityGroup"> </a>Procédure : Suppression d'un groupe d'affinités
	
Vous pouvez supprimer un groupe d'affinités en transmettant le nom du groupe à la méthode **delete\_affinity\_group**. Notez qu'avant de pouvoir supprimer un groupe d'affinités, ce dernier doit être dissocié de tout service (ou les services utilisant le groupe d'affinités doivent être supprimés).
...
## <a name="ListOperatingSystems"> </a>Procédure : Affichage de la liste des systèmes d'exploitation disponibles

Pour répertorier les systèmes d'exploitation qui sont disponibles pour l'hébergement de services, utilisez la méthode **list\_operating\_systems** :
...
## <a name="CreateVMImage"> </a>Procédure : Création d'une image du système d'exploitation
...
Pour répertorier les images de système d'exploitation qui sont disponibles, utilisez la méthode **list\_os\_images**. Cela inclut toutes les images de plateforme et les images utilisateur :
...
## <a name="DeleteVMImage"> </a>Procédure : Suppression d'une image du système d'exploitation

Pour supprimer une image de l'utilisateur, utilisez la méthode **delete\_os\_image** :
...
## <a name="CreateVM"> </a>Procédure : Création d'une machine virtuelle

Pour créer une machine virtuelle, vous devez d'abord créer un [service cloud](#CreateCloudService).  Créez ensuite le déploiement de la machine virtuelle en utilisant la méthode **create\_virtual\_machine\_deployment** :
...
## <a name="DeleteVM"> </a>Procédure : Suppression d'une machine virtuelle
...
Le service cloud peut ensuite être supprimé à l'aide de la méthode **delete\_hosted\_service** :
...
-   Consultez la référence MSDN suivante : [Services cloud][]
...
[Procédure : des déploiements entre les environnements intermédiaire et de production]: #MoveDeployments
...
[Procédure : Affichage de la liste des systèmes d'exploitation disponibles]: #ListOperatingSystems
[Procédure : Création d'une image du système d'exploitation]: #CreateVMImage
[Procédure : Suppression d'une image du système d'exploitation]: #DeleteVMImage
[Procédure : Création d'une machine virtuelle]: #CreateVM
[Procédure : Suppression d'une machine virtuelle]: #DeleteVM
[Étapes suivantes]: #NextSteps
[management-portal]: https://manage.windowsazure.com/
[svc-mgmt-rest-api]: http://msdn.microsoft.com/library/windowsazure/ee460799.aspx


[download-SDK-Python]: https://www.windowsazure.com/fr-fr/develop/python/common-tasks/install-python/
[service cloud]:http://windowsazure.com/fr-fr/documentation/articles/cloud-services-what-is
[package de service]: http://msdn.microsoft.com/library/windowsazure/jj155995.aspx
[applets de commande Azure PowerShell]: https://www.windowsazure.com/fr-fr/develop/php/how-to-guides/powershell-cmdlets/
[outil de ligne de commande cspack]: http://msdn.microsoft.com/library/windowsazure/gg432988.aspx
[Déploiement d'un service Azure]: http://msdn.microsoft.com/library/windowsazure/gg433027.aspx
[service de stockage]: https://www.windowsazure.com/fr-fr/manage/services/storage/what-is-a-storage-account/
[azure-blobs]: https://www.windowsazure.com/fr-fr/develop/python/how-to-guides/blob-service/
[azure-tables]: https://www.windowsazure.com/fr-fr/develop/python/how-to-guides/table-service/
[azure-queues]: https://www.windowsazure.com/fr-fr/develop/python/how-to-guides/queue-service/
[schéma de configuration de service Azure (.cscfg)]: http://msdn.microsoft.com/library/windowsazure/ee758710.aspx
[Services cloud]: http://msdn.microsoft.com/library/windowsazure/jj155995.aspx
[Machines virtuelles]: http://msdn.microsoft.com/library/windowsazure/jj156003.aspx
