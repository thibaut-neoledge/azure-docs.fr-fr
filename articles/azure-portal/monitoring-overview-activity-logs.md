...
Le journal d’activité est différent des [journaux de diagnostic](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md), qui correspondent à tous les journaux émis par une ressource. Ces journaux fournissent des informations sur l’opération de cette ressource, plutôt que sur les opérations sur cette ressource.
...
* [Créez une alerte via e-mail ou webhook qui déclenche un événement de journal d’activité.](../monitoring-and-diagnostics/insights-auditlog-to-webhook-email.md)
* [Enregistrez-le dans un **compte de stockage** pour l’archivage ou l’inspection manuelle](../monitoring-and-diagnostics/monitoring-archive-activity-log.md). Vous pouvez spécifier la durée de rétention (en jours) à l’aide des **profils de journal**.
...
* [Diffusez-le en continu vers un **Event Hub**](../monitoring-and-diagnostics/monitoring-stream-activity-logs-event-hubs.md) pour qu’un service tiers ou une solution d’analyse personnalisée (p. ex. PowerBI) l’ingère.
