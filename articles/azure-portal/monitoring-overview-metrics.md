...
Pour une description plus détaillée à l’aide de l’API REST d’Azure Monitor, consultez [Procédure pas à pas d’utilisation de l’API REST d’Azure Monitor](../monitoring-and-diagnostics/monitoring-rest-api-walkthrough.md).
...
Vous pouvez configurer cela via les modèles Resource Manager, [PowerShell](../monitoring-and-diagnostics/insights-powershell-samples.md), [le CLI](../monitoring-and-diagnostics/insights-cli-samples.md) ou [les API REST](https://msdn.microsoft.com/library/dn931943.aspx).
...
Vous pouvez voir une liste détaillée de tous les services pris en charge et leurs mesures dans [Mesures d’Azure Monitor - Mesures prises en charge par type de ressource](../monitoring-and-diagnostics/monitoring-supported-metrics.md).

## Étapes suivantes
Consultez les liens dans cet article. De plus, consultez :

* à propos des [mesures courantes pour la mise à l’échelle automatique](../monitoring-and-diagnostics/insights-autoscale-common-metrics.md)
* comment [créer des règles d’alerte](../monitoring-and-diagnostics/insights-alerts-portal.md)
