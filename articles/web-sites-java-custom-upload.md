<properties urlDisplayName="Upload custom Java website" pageTitle="Télécharger un site web Java personnalisé vers Azure" metaKeywords="" description="Ce didacticiel vous montre comment télécharger un site web Java personnalisé vers Azure." metaCanonical="" services="web-sites" documentationCenter="Java" title="Upload a custom Java website to Azure" videoId="" scriptId="" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" />
...
Notez qu'Azure permet de créer des sites web Java en utilisant l'interface utilisateur de configuration du portail Azure, ainsi que la galerie d'applications Azure, comme indiqué dans la page [Prise en main de Sites Web Azure et de Java
...
**startupTimeLimit** (par défaut = 10 secondes) : durée pendant laquelle **HttpPlatformHandler** attend que l'exécutable/le script démarre un processus d'écoute du port.  Si cette limite de temps est dépassée, **HttpPlatformHandler** arrête le processus et tente de le redémarrer un certain nombre de fois en fonction de la valeur **startupRetryCount**.
