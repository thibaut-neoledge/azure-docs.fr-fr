...
> [WACOM.NOTE] Pour exécuter les exemples de code .NET de ce guide, la bibliothèque cliente de stockage .NET Azure 4.x ou une version ultérieure est nécessaire. Celle-ci est disponible via [NuGet][NuGet].
...
Pour vous préparer à utiliser PowerShell, téléchargez et installez les applets de commande PowerShell Azure. Consultez la rubrique [Installation et configuration d’Azure PowerShell][Installation et configuration d’Azure PowerShell] pour des instructions sur l'installation et le point d'installation.
...
1.  Tout d'abord, créez une machine virtuelle Azure en suivant les instructions de la rubrique [Création d'une machine virtuelle exécutant Windows Server][Création d'une machine virtuelle exécutant Windows Server].
2.  Ensuite, connectez-vous à distance à la machine virtuelle en suivant les instructions de la rubrique [Connexion à une machine virtuelle exécutant Windows Server][Connexion à une machine virtuelle exécutant Windows Server].
...
À présent que vous avez appris les bases du stockage de fichiers, suivez ces liens
...
-   Pour plus d'informations sur les autres options de stockage de données dans Azure, consultez d'autres guides de fonctionnalités.
    ...
    -   Utilisez le [stockage de files d'attente][stockage de files d'attente] pour stocker les messages de façon fiable.
  ...
  [Affectation de noms et références aux partages, répertoires, fichiers et métadonnées]: http://msdn.microsoft.com/en-us/library/azure/dn167011.aspx
  [page de la version préliminaire de Microsoft Azure]: /en-us/services/preview/
  ...
  [espaces de noms System.IO]: http://msdn.microsoft.com/en-us/library/gg145019(v=vs.110).aspx
  [bibliothèque cliente de stockage .NET Azure]: http://go.microsoft.com/fwlink/?LinkID=390731&clcid=0x409
  [Référence de l'API REST du service de fichiers]: http://msdn.microsoft.com/en-us/library/azure/dn167006.aspx
