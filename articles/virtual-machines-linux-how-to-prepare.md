...
1.  Il est recommandé de modifier /etc/sysconfig/network/dhcp (ou son équivalent) de DHCLIENT\_SET\_HOSTNAME="yes" vers DHCLIENT\_SET\_HOSTNAME="no"

2.  Sous /etc/sudoers, placez la ligne suivante en commentaire (si elle est présente) :

        Defaults targetpw

3.  Vérifiez que le serveur SSH est installé et configuré pour démarrer au moment prévu.
...
6.  Vérifiez que l'agent Linux Azure est exécuté au démarrage :
