...
6. Modifiez la ligne de démarrage du noyau dans Grub afin d'y inclure les paramètres suivants. Ceci permet également d'assurer que tous les messages de la console sont envoyés vers le premier port série, ce qui peut simplifier les problèmes de débogage pour la prise en charge d'Azure :

		console=ttyS0 earlyprintk=ttyS0 rootdelay=300

7.	Il est recommandé de modifier /etc/sysconfig/network/dhcp (ou son équivalent) de DHCLIENT\_SET\_HOSTNAME="yes" vers DHCLIENT\_SET\_HOSTNAME="no"
...
4. Sur l'interpréteur de commandes, exécutez la commande '`zypper lr`'. Si cette commande renvoie
   ...
   Si la commande renvoie « No repositories defined. Use the 'zypper addrepo' command to 
   add one or more repositories », cela signifie que les référentiels doivent être réactivés :
...
9.	Il est recommandé de modifier /etc/sysconfig/network/dhcp (ou son équivalent) de DHCLIENT\_SET\_HOSTNAME="yes" vers DHCLIENT\_SET\_HOSTNAME="no"
...
14. Vérifiez que l'agent Linux Azure est exécuté au démarrage :
...
1.	Vous devez vous assurer, au choix, que vous utilisez un noyau qui incorpore les derniers pilotes LIS pour Hyper-V ou que vous les avez compilés correctement (ils sont disponibles en open source). Les pilotes sont disponibles [ici](http://go.microsoft.com/fwlink/p/?LinkID=254263&clcid=0x409)
