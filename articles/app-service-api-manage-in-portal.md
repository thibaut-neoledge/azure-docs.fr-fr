...
Dans Azure App Service, une application API est une [application web](app-service-web-overview.md) qui dispose de fonctionnalités supplémentaires pour l'hébergement de services web. Dans le portail Azure en version préliminaire, il existe un panneau **Application API** pour gérer les fonctionnalités spécifiques à l'API et un panneau **Hôte d'application API** pour gérer l'application web sous-jacente.
...
Vous utilisez le panneau **Hôte d'application API** pour toutes les tâches applicables à n'importe quelle application web. Pour plus d'informations, consultez [Gérer les applications web dans le portail](web-sites-manage.md).
