<properties
   pageTitle="Implémentation d’un plan de test JMeter pour Elasticsearch | Microsoft Azure"
...
# Implémentation d’un plan de test JMeter pour Elasticsearch

Cet article fait [partie d’une série](guidance-elasticsearch.md).

Les tests de performances effectués par rapport à Elasticsearch ont été implémentés à l’aide de plans de test JMeter, ainsi que de code Java incorporé en tant que test JUnit pour effectuer des tâches telles que le chargement de données sur le cluster. Les plans de test et le code JUnit sont décrits dans les documents [Paramétrage des performances d’ingestion des données avec Elasticsearch sur Azure][] et [Paramétrage de l’agrégation de données et des performances de requête dans Elasticsearch sur Azure][].
...
Vous pouvez incorporer du code complexe dans un plan de test JMeter en créant un ou plusieurs tests JUnit. Vous pouvez écrire un test JUnit à l’aide d’un IDE Java tel qu’Eclipse. Le document [Déploiement d'un échantillon de JUnit JMeter pour le test de performances de Elasticsearch][] fournit des informations sur la façon de configurer un environnement de développement approprié.
...
[Paramétrage des performances d’ingestion des données avec Elasticsearch sur Azure]: guidance-elasticsearch-tuning-data-ingestion-performance.md
[Déploiement d'un échantillon de JUnit JMeter pour le test de performances de Elasticsearch]: guidance-elasticsearch-deploying-jmeter-junit-sampler.md
[Paramétrage de l’agrégation de données et des performances de requête dans Elasticsearch sur Azure]: guidance-elasticsearch-tuning-data-aggregation-and-query-performance.md
