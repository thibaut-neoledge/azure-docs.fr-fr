...
DocumentDB est un service de base de données NoSQL orienté documents, entièrement géré et évolutif.  Avec DocumentDB, il n'est pas nécessaire de louer des machines virtuelles, de déployer des logiciels, de surveiller les bases de données ou de se soucier de la récupération d'urgence. DocumentDB est surveillé en continu par les ingénieurs Microsoft afin de fournir une disponibilité, des performances et une protection des données de qualité.  

Vous pouvez démarrer avec DocumentDB en créant un compte de base de données via la [version préliminaire du portail de gestion Azure](https://portal.azure.com/). DocumentDB est proposé sous forme d'unités empilables de capacité de stockage et de débit SSD. Vous pouvez mettre à l'échelle DocumentDB en toute flexibilité pour répondre aux besoins de votre application en matière de performances et de stockage. 
...
> [AZURE.NOTE] Chaque collection peut évoluer en termes de stockage et de débit, jusqu'à la [limite](http://azure.microsoft.com/documentation/articles/documentdb-limits/) maximale prise en charge. Le débit est réparti uniformément entre toutes les collections jusqu'à la limite maximale par collection.
...
En tant qu'abonné Azure, vous pouvez approvisionner un ou plusieurs comptes de base de données DocumentDB. Chaque compte de base de données offre un quota de ressources d'administration, avec des bases de données, des utilisateurs et des autorisations. Ces ressources sont soumises à [des quotas et des limites](http://azure.microsoft.com/documentation/articles/documentdb-limits/). Si vous avez besoin de ressources d'administration supplémentaires, contactez le support technique.   
...
##<a name="ProvStorage"></a>Stockage approvisionné et débit en tant qu'unités de capacité
...
##<a name="Consistency"></a>Choix du niveau de cohérence et du débit
...
-	Les niveaux de cohérence Session (Par session) et Eventual (Éventuel) fournissent environ 2 000 demandes de lecture de documents et 500 insertions/remplacements/suppressions de documents.
-	Les niveaux de cohérence Strong (Forte) et Bounded staleness (En fonction de l'obsolescence) fournissent environ 1 200 demandes de lecture de documents et 500 insertions/remplacements/suppressions de documents. Avec le niveau Bounded staleness (En fonction de l'obsolescence), les insertions/remplacements/suppressions ont une latence sensiblement inférieure à celle du niveau Strong (Forte).  

Pour obtenir des instructions sur la modification de votre niveau de cohérence dans la version préliminaire du portail de gestion Azure, consultez la section Gestion des paramètres de cohérence DocumentDB dans la page [Gestion d'un compte DocumentDB](../documentdb-manage-account/).
...
Pour obtenir des instructions sur la gestion de votre compte DocumentDB dans la version préliminaire du portail Azure, consultez la section Gestion des paramètres de cohérence DocumentDB dans la page [Gestion d'un compte DocumentDB](../documentdb-manage-account/).

Pour obtenir des instructions sur la surveillance des niveaux de performance dans la version préliminaire du portail Azure, consultez la page [Surveillance d'un compte DocumentDB](../documentdb-monitor-accounts/).
