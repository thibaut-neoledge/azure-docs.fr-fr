...
7.  Entrez un nom d'administrateur dans le champ **Nom d'utilisateur**. Notez le nom et le mot de passe que vous allez entrer, car vous les utiliserez pour vous connecter à distance à votre machine virtuelle.
...
7.  Cliquez sur la coche. Le système crée l'espace de noms de service et l'active. Vous devrez peut-être attendre plusieurs minutes afin que le système approvisionne des ressources pour votre compte.
    ...
    ![Capture d'écran illustrant la création](./media/virtual-machines-dotnet-run-compute-intensive-task/ClickCreate.png)
...
2.  Sélectionnez l'espace de noms que vous venez de créer dans la liste affichée :
     ![Capture d'écran Liste d'espaces de noms](./media/virtual-machines-dotnet-run-compute-intensive-task/NamespaceList.png)

3.  Cliquez sur **Clé d'accès**.
     ![Bouton clé d'accès](./media/virtual-machines-dotnet-run-compute-intensive-task/AccessKey.png)
...
1.  Sur votre ordinateur de développement (qui n'est pas forcément celui où se trouve la machine virtuelle que vous avez créée), téléchargez le [Kit de développement logiciel (SDK) Azure pour .NET](http://www.windowsazure.com/en-us/develop/net/).
...
7.  Compilez l'application. **TSPSolver.exe** est créé dans le dossier **bin** de votre projet (**bin\\release** ou **bin\\debug**, en fonction de ce que vous ciblez : version Release ou de débogage). Vous allez copier ultérieurement cet exécutable et Microsoft.ServiceBus.dll dans votre machine virtuelle.
        ...
        class Program
        {
            // Valeur spécifiant la fréquence pour fournir une mise à jour à la console.
            ...
            static void Main(string[] args)
            {
                try
                {
                    ...
                    int numCities = 10; // Utilisez la valeur par défaut, si aucune valeur n'est spécifiée
                    ...
                    if (args.Count() != 0)
                    {
                        ...
                        {
                            // Aucun traitement n'intervient autre que la création de la file d'attente.
                        ...
                        if (args[0].ToLower().CompareTo("createqueue") == 0)
                        {
                            // Aucun traitement n'intervient autre que la suppression de la file d'attente.
                            namespaceManager = NamespaceManager.CreateFromConnectionString(connectionString);
                        ...
                        // Ni création ou suppression d'une file d'attente.
...
1.  Sur votre ordinateur de développement, créez une application console .NET à l'aide de **TSPClient** en tant que nom de projet. Assurez-vous que l'infrastructure cible est définie sur .**NET Framework 4** (et non **.NET Framework 4 Client Profile**). Elle peut être définie après la création d'un projet comme suit : dans le menu de Visual Studio, cliquez successivement sur **Projets**, sur **Propriétés** et sur l'onglet **Application**, puis définissez la valeur pour **Framework cible**.
2.  Ajoutez la bibliothèque Microsoft ServiceBus. Dans l'Explorateur de solutions de Visual Studio, cliquez avec le bouton droit sur **TSPSolver**, cliquez sur **Ajouter une référence**, cliquez sur l'onglet **Parcourir**, accédez à **C:\\Program Files\\Microsoft SDKs\\Windows Azure.NET SDK\\2012-06\\ref** et sélectionnez **Microsoft.ServiceBus.dll** en tant que référence.
        ...
        class Program
        {
            ...
            static void Main(string[] args)
            {
                ...
                try
                {
                    ...
                    int waitMinutes = 3;  // Utilisez la valeur par défaut, si aucune valeur
                    // n'est spécifiée dans la ligne de commande.
         ...
         "minute" : waitMinutes.ToString() + " minutes";
                    ...
                    while (true)
                    {
                        ...
                        if (message != null)
                        {
                            ...
                            {
                                // Indique un problème. Déverrouillez le message de la file d'attente.
                                message.Abandon();
                                throw e;
                            }
                        }
