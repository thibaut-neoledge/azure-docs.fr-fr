...
La section Utilisation quotidienne du fichier montre les détails d’utilisation qui affectent les taux de facturation. Le tableau suivant répertorie les conditions et les descriptions présentées dans cette section. 
...
|Région du compteur|Identifie l’emplacement du centre de données pour certains services dont le prix est basé sur cet emplacement. |
|Unité |Identifie l’unité dans laquelle le service est facturé. Par exemple, Go, heures, 10 000 s. |
|Quantité consommée |Quantité de la ressource consommée ce jour-là. |
|Emplacement de la ressource |Identifie le centre de données dans lequel la ressource est en cours d’exécution. |
|Service consommé |Service de plateforme Azure que vous avez utilisé. |
|Groupe de ressources |Groupe de ressources dans lequel la ressource déployée est en cours d’exécution. Pour plus d’informations, consultez [Présentation d’Azure Resource Manager](../azure-resource-manager/resource-group-overview.md). |
|ID de l’instance |Identificateur pour la ressource. L’identificateur contient le nom que vous avez spécifié pour la ressource lors de sa création. Nom de la ressource ou ID de ressource complet. Pour plus d’informations, consultez la page [API Azure Resource Manager](/rest/api/resources/resources). |
|Tags |Balise que vous affectez à la ressource. Utilisez des balises pour regrouper les enregistrements de facturation. Par exemple, vous pouvez utiliser des balises pour répartir les coûts par département qui utilise la ressource. Les services qui prennent en charge l’émission de balises sont les services de machine virtuelle, de stockage et de réseau approvisionnés à l’aide de [l’API Azure Resource Manager](/rest/api/resources/resources). Pour plus d’informations, voir [Organisation des ressources Azure à l’aide de balises](http://azure.microsoft.com/updates/organize-your-azure-resources-with-tags/). |
