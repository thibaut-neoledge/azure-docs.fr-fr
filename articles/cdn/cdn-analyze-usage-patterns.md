...
- TCP\_HIT : cet état est signalé lorsqu'une requête est traitée directement du POP au client. Une ressource est immédiatement traitée à partir d'un POP lorsqu'il est mis en cache sur le serveur POP le plus proche du client et que sa durée de vie est valide. La durée de vie est déterminée par les en-têtes de réponse suivants :
...
Ce rapport indique le pourcentage de requêtes mises en cache traitées directement à partir de la mémoire cache. Le rapport fournit les informations suivantes :
