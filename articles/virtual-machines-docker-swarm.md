...
<p />
> En outre, cette rubrique utilise docker avec swarm et l'interface de ligne de commande multiplateforme *without* avec docker-machine pour montrer comment les différents outils fonctionnent ensemble tout en demeurant indépendants. docker-machine possède des commutateurs **--swarm** qui vous permettent d'utiliser docker-machine pour ajouter directement des nœuds à un essaim. Pour obtenir un exemple, consultez la documentation [docker-machine](https://github.com/docker/machine). Si l'exécution de docker-machine sur des machines virtuelles Azure ne vous est pas familière, consultez [Utilisation de docker-machine avec Azure](https://azure.microsoft.com/documentation/articles/virtual-machines-docker-machine/).
...
Quand vous avez terminé, vous devez être en mesure d'utiliser **azure vm list** pour voir vos machines virtuelles Azure :
...
> [AZURE.NOTE] Pour dissiper tout doute, nous utilisons notre installation docker locale pour nous connecter à la machine virtuelle **swarm-master** dans Azure et l'instruction **swarm-master** pour télécharger, installer et exécuter la commande **create**, qui retourne l'id de cluster que nous utilisons ultérieurement à des fins de découverte.
<p />
...
Pour chaque nœud, répertoriez les informations sur les points de terminaison à l'aide de l'interface de ligne de commande multiplateforme. Ci-dessous, nous effectuons cela pour l'hôte docker **swarm-node-1** afin d'obtenir le port docker du nœud.
