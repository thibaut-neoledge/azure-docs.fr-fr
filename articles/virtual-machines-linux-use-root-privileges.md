...
1. **Clé SSH et mot de passe OU mot de passe uniquement** : la machine virtuelle a été déployée soit avec un certificat (fichier `.CER`) et un mot de passe, soit avec seulement un nom d'utilisateur et un mot de passe. Dans ce cas,  `sudo` demande le mot de passe de l'utilisateur avant d'exécuter la commande.
...
Dans ce cas, l'utilisateur **n'est pas** invité à fournir un mot de passe. Une fois la touche `<entrée>` activée,  `sudo` exécute la commande avec les privilèges  `root`.
