...
Si vous découvrez l'univers des applications API, voici quelques articles pour démarrer.

1. [Créer une application API](app-service-dotnet-create-api-app.md) : créer une application API de toutes pièces ou convertir un projet web API existant en une application API. 
2. [Déployer une application API](app-service-dotnet-deploy-api-app.md) : déployer une application API dans votre abonnement Azure.
3. [Déboguer une application API](app-service-dotnet-remotely-debug-api-app.md) : utiliser Visual Studio pour déboguer à distance une application API s'exécutant dans Azure.

En outre, tous les extraits de code de cette rubrique sont copiés depuis l'[exemple de code de l'application API FileWatcher](http://go.microsoft.com/fwlink/?LinkId=534802).
