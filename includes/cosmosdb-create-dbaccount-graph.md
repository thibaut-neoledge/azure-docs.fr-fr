...
2. Dans le menu de gauche, cliquez sur **Nouveau**, cliquez ensuite sur **Bases de données**, puis cliquez sur **Azure Cosmos DB**.
   ...
   ![Capture d’écran du portail Azure, mettant en surbrillance l’option Plus de services, et Azure Cosmos DB](./media/cosmosdb-create-dbaccount-graph/create-nosql-db-databases-json-tutorial-1.png)
...
3. Dans le panneau **Nouveau compte**, indiquez la configuration souhaitée pour le compte Azure Cosmos DB. 
    ...
    Dans ce guide de démarrage rapide, nous allons programmer sur l’API Graph, vous choisirez donc **Gremlin (graphique)** lors du remplissage du formulaire. Cependant, si vous avez des données de document provenant d’une application du catalogue, des données clé/valeur (table) ou des données migrées d’une application MongoDB, sachez qu’Azure Cosmos DB peut fournir une plate-forme de service de base de données hautement disponible et distribuée dans le monde entier pour toutes vos applications essentielles.
    ...
    API|Gremlin (graphique)|Nous allons programmer sur [l’API Graph](../articles/cosmos-db/graph-introduction.md) plus loin dans cet article.|
...
5. Dans la barre d’outils, cliquez sur **Notifications** pour surveiller le processus de déploiement.
    ...
    ![Notification du début de déploiement](./media/cosmosdb-create-dbaccount-graph/azure-documentdb-nosql-notification.png)

6.  Lorsque le déploiement est terminé, ouvrez le nouveau compte à partir de la mosaïque Toutes les ressources. 

    ![Compte DocumentDB sur la mosaïque Toutes les ressources](./media/cosmosdb-create-dbaccount-graph/azure-documentdb-all-resources.png)