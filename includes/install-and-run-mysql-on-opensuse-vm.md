...
6.  Après l'exécution, vous pouvez vous connecter à MySQL :
    ...
    Entrez le mot de passe racine MySQL (que vous avez modifié à l'étape précédente) pour ouvrir une invite de commandes vous permettant d'écrire des instructions SQL pour interagir avec la base de
    données.
...
11. Après l'installation de MySQL, vous devez configurer un point de terminaison pour que MySQL soit accessible à distance. Connectez-vous au [portail de gestion Azure][1]. Dans le portail Azure, cliquez sur **Virtual Machines**, puis sur le nom de la nouvelle machine virtuelle et sur **Endpoints**.
    
    ![Points de terminaison](./media/install-and-run-mysql-on-opensuse-vm/LinuxVmAddEndpoint.png)

12. Cliquez sur **Add Endpoint** en bas de la page.
![Points de terminaison](./media/install-and-run-mysql-on-opensuse-vm/LinuxVmAddEndpoint2.png)
...
![Points de terminaison](./media/install-and-run-mysql-on-opensuse-vm/LinuxVmAddEndpointMySQL.png)
...
15. Vous avez réussi à configurer MySQL, à créer une base de données et un utilisateur. Pour plus d'informations sur MySQL, consultez la [documentation MySQL][2].
