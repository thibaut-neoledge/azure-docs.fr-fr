...
| --- | --- | --- | --- |
| columnDelimiter |Caractère utilisé pour séparer les colonnes dans un fichier. |Un seul caractère est autorisé. La valeur par défaut est virgule (,). <br/><br/>Pour utiliser un caractère Unicode, reportez-vous à [Unicode Characters](https://en.wikipedia.org/wiki/List_of_Unicode_characters) (Caractères Unicode) pour obtenir le code correspondant. Par exemple, vous pouvez envisager d’utiliser un caractère non imprimable rare qui n’existe probablement pas dans vos données : spécifiez « \u0001 » qui représente le début d’en-tête. |Non |
