...
* [Étape 2 : Créer un point de terminaison CDN pour le compte de stockage](#Step2)
* [Étape 3 : Accéder au contenu du CDN](#Step3)
* [Étape 4 : Supprimer le contenu du CDN](#Step4)
...
2.  Dans le coin inférieur gauche, cliquez sur **Nouveau**. Dans la boîte de dialogue **Nouveau**, sélectionnez **Services de données**, cliquez sur **Stockage**, puis **Création rapide**.

    La boîte de dialogue **Créer un compte de stockage** apparaît.
...
5.  Dans la liste déroulante **Groupe de régions ou d'affinités**, sélectionnez un groupe de régions ou d'affinités pour le compte de stockage. Sélectionnez un groupe d'affinités plutôt qu'une zone pour que vos services de stockage soient hébergés dans le même centre de données que les autres services Azure que vous utilisez. Ce choix peut améliorer les performances et n'entraîne aucun frais supplémentaire.  
    ...
    **Remarque :** Pour créer un groupe d'affinités, ouvrez la zone **Paramètres** du portail de gestion, cliquez sur **Groupes d'affinités**, puis cliquez sur **Ajouter un groupe d'affinités** ou **Ajouter**. Vous pouvez également créer et gérer des groupes d'affinités par le biais de l'API de gestion des services Azure. Pour plus d'informations, consultez la page [Opérations sur les groupes d'affinités].

6. Dans la liste déroulante **Abonnement**, sélectionnez l'abonnement à utiliser avec le compte de stockage.
7.  Cliquez sur **Créer un compte de stockage**. Le processus de création du compte de stockage peut durer quelques minutes.
8.  Pour savoir si le compte de stockage a bien été créé, regardez si le compte est affiché dans les éléments répertoriés pour **Stockage** avec l'état **En ligne**.
...
3. Dans la liste déroulante **Domaine d'origine**, sélectionnez le compte de stockage créé au cours de la section précédente dans la liste des comptes de stockage disponibles. 
...
-   S'il s'agit d'un objet blob Azure, vous pouvez le supprimer du conteneur public. -   Vous pouvez changer le statut du conteneur de public à privé. Consultez [Restreindre l'accès aux objets blob et aux conteneurs](http://msdn.microsoft.com/fr-fr/library/dd179354.aspx) pour plus d'informations.
...
La capacité à purger immédiatement le contenu n'est actuellement pas prise en charge sur le portail de gestion Azure. Contactez le [support Azure](http://azure.microsoft.com/fr-fr/support/options/)  si vous avez besoin de purger immédiatement du contenu. 

## Ressources supplémentaires

-   [Comment créer un groupe d'affinités dans Azure]
-   [Procédure : gestion des comptes de stockage pour un abonnement Azure]
-   [à propos de l'API de gestion du service]
-   [mappage du contenu CDN à un domaine personnalisé]

  [création d'un compte de stockage]: http://msdn.microsoft.com/fr-fr/library/windowsazure/hh264518.aspx
  [emplacements des nœuds CDN Azure]: http://msdn.microsoft.com/fr-fr/library/windowsazure/gg680302.aspx
  [portail de gestion Azure]: https://manage.windowsazure.com/
  [plan de facturation]: /fr-fr/pricing/calculator/?scenario=full
  [inscription d'un nom de sous-domaine personnalisé pour accéder à des objets blob dans Azure]: http://msdn.microsoft.com/fr-fr/library/windowsazure/ee795179.aspx
  [création d'un groupe d'affinités dans Azure]: http://msdn.microsoft.com/library/azure/ee460798.aspx
  [vue d'ensemble du CDN Azure]: http://msdn.microsoft.com/fr-fr/library/windowsazure/ff919703.aspx
  ...
  [à propos de l'API de gestion du service]: http://msdn.microsoft.com/fr-fr/library/windowsazure/ee460807.aspx
