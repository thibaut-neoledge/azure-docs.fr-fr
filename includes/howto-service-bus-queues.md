...
Les files d'attente Service Bus prennent en charge un modèle de **communication de messagerie répartie** m. Lorsque vous utilisez des files d'attente, les composants d'une application distribuée ne ccommuniquent pas directement entre eux, ils échangent plutôt des messages via a une file d'attente, qui agit comme un intermédiaire (broker). Un producteur de messages (expéditeur) hremet un message à la file d'attente, puis continue son traitement.
De manière asynchrone, un consommateur de message (destinataire) extrait le message de la qfile d'attente, puis le traite. Le producteur n'a pas à attendre une réponse fdu consommateur pour pouvoir continuer à traiter et à envoyer d'autres mmessages. Les files d'attente offrent une remise de messages **Premier entré, premier sorti (FIFO)** tà un ou plusieurs consommateurs simultanés. Autrement dit, les messages sont en général rreçus et traités par les récepteurs dans l'ordre dans lequel ils ont été aajoutés à la file d'attente et chaque message est reçu et traité par oun seul consommateur de message.
...
L'espace de noms que vous avez créé apparaît alors dans le portail de gestion. tSon activation peut prendre un peu de temps. Patientez jusqu'à ce que l'état soit **Actif** avant cde poursuivre.
