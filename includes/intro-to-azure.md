...
## Sommaire
**Services de base** - [Composants d’Azure](#components) - [Portail de gestion](#portal) - [Calcul](#compute) - [Gestion des données](#data) - [Mise en réseau](#networking) - [Services de développement](#DevService)

**Services supplémentaires** - [Identité et accès](#identity) - [Mobilité](#mobile) - [Sauvegarde](#backup) - [Messagerie et intégration](#messaging) - [Assistance au calcul](#ComputeAssist) - [Performances](#Performance) - [Big Compute et données volumineuses](#BigStuff) - [Média](#media) - [Commerce](#commerce)

[Prise en main](#start)

<h2><a id="components"></a>Composants d’Azure</h2>

Azure regroupe les services en catégories sur le portail de gestion et sur différentes aides visuelles telles que l’[Infographie Microsoft Azure](http://azure.microsoft.com/documentation/infographics/azure/ "Infographie Microsoft Azure"). Le portail de gestion vous permet de gérer la plupart des services dans Azure (mais pas tous).
...
![Composants d’Azure](./media/intro-to-azure/AzureComponentsIntroNew780.png) **Figure : Azure fournit des services d’application accessibles via Internet, exécutés depuis les centres de données Azure.**

<h2><a id="portal"></a>Portail de gestion</h2>
...
La mission de base d’une plateforme de cloud est d’exécuter des applications. Azure propose les options suivantes :

1. Azure Virtual Machines vous permet de contrôler votre machine virtuelle, système d'exploitation compris. 
2. Sites Web Azure propose toute une gamme d'applications, d'infrastructures et de modèles pour vous aider à créer des applications web volumineuses et évolutives, à mettre rapidement en place des sites web, puis à en gérer efficacement le développement, les tests et le fonctionnement.
3. Azure Cloud Services est une option PaaS (Platform-as-a-Service) conçue pour créer des applications très évolutives et fiables, mais offrant davantage de flexibilité que Sites Web Azure. 

Chacun des modèles d’exécution d’Azure a son propre rôle à jouer.
...
![Azure Virtual Machines](./media/intro-to-azure/VirtualMachinesIntroNew.png) **Figure : Azure Virtual Machines offre un contrôle total des instances de machine virtuelle dans le cloud.**
...
### Sites web
![Sites Web Azure](./media/intro-to-azure/AzureWebsitesIntroNew.png) **Figure : Sites Web Azure exécute une application de site Web dans le cloud sans gestion du serveur Web sous-jacent.**
...
Sites Web Azure a été conçu pour répondre aux besoins des entreprises, des développeurs et des agences de design web. Les entreprises bénéficient d'une solution simple à gérer, évolutive, hautement sécurisée et très disponible pour leurs sites web. Pour configurer un site web, nous vous recommandons de commencer avec Sites Web Azure, puis de passer à Azure Cloud Services lorsque vous avez besoin d'une fonctionnalité non proposée par Sites Web Azure. Reportez-vous à la section « Compute » pour accéder à différents liens qui vous aideront à faire votre choix parmi les options disponibles.

### Services cloud
![Azure Cloud Services](./media/intro-to-azure/CloudServicesIntroNew.png) **Figure : Azure Cloud Services permet d’exécuter un code personnalisé hautement évolutif dans un environnement PaaS (Platform as a Service).**
...
![Base de données SQL Azure Storage](./media/intro-to-azure/StorageAzureSQLDatabaseIntroNew.png) **Figure : la base de données SQL Azure fournit un service de base de données relationnelle géré dans le cloud.**
...
### Tables
![Tables Azure Storage](./media/intro-to-azure/StorageTablesIntroNew.png) **Figure : les tables Azure permettent de stocker des données via la technologie NoSQL.**
...
### Objets blob
![Objets blob d'Azure Storage](./media/intro-to-azure/StorageBlobsIntroNew.png) **Figure : les objets blob Azure fournissent des données binaires non structurées.**
...
### Azure File Services
![Azure File Services](./media/intro-to-azure/FileServiceIntroNew.png) **Figure : Azure File Services fournit des chemins SMB \serveur\partage aux applications exécutées dans le cloud.**
...
### ExpressRoute
![ExpressRoute](./media/intro-to-azure/ExpressRouteIntroNew.png) **Figure : le service ExpressRoute utilise une instance d’Azure Virtual Network, mais au lieu de passer par le réseau Internet public, il achemine les connexions à l’aide de lignes dédiées plus rapides.**
...
### Traffic Manager
![TrafficManager](./media/intro-to-azure/TrafficManagerIntroNew.png) **Figure : Azure Traffic Manager vous permet d’acheminer le trafic global vers votre service sur la base de règles intelligentes.**
...
<h2><a id="DevService"></a>Services de développement</h2>
...
<h2><a id="identity"></a>Identité et accès</h2>
...
![Azure Multi-Factor Authentication](./media/intro-to-azure/MFAIntroNew.png) **Figure : Multi-Factor Authentication permet à votre application de vérifier plusieurs formes d’identification.**
...
Même si vous êtes en mesure de concevoir l'infrastructure d'une application mobile à l'aide de Virtual Machines, Cloud Services ou Sites Web, vous gagnerez un temps précieux en utilisant les services d'Azure, car vous n'aurez pas à créer les composants de service sous-jacents.

### Mobile Services
![MobileServices](./media/intro-to-azure/MobileServicesIntroNew.png) **Figure : Mobile Services fournit la fonctionnalité généralement requise par les applications qui font office d’interface avec les appareils mobiles.**
...
### Notification Hubs
![NotificationHubs](./media/intro-to-azure/NotificationHubsIntroNew.png) **Figure : Notification Hubs fournit la fonctionnalité généralement requise par les applications qui font office d’interface avec les appareils mobiles.**
...
Notification Hubs a été conçu pour fonctionner avec n'importe quelle infrastructure. Vous pouvez utiliser Azure Mobile Services avec une infrastructure personnalisée située dans le cloud et exécutée auprès de n'importe quel fournisseur, ou avec une infrastructure locale.

**Scénarios relatifs à Notification Hubs** Si vous concevez un jeu mobile dans lequel les joueurs jouent à tour de rôle, vous pouvez être amené à notifier le joueur 2 que la joueuse 1 a terminé son tour. Si vous n'avez rien d'autre à faire, vous pouvez vous contenter d'utiliser Mobile Services. Mais si 100 000 utilisateurs jouent à votre jeu et que vous souhaitez envoyer une offre ponctuelle à chacun d’eux, mieux vaut utiliser Notification Hubs.
...
<h2><a id="messaging"></a>Messagerie et intégration</h2>
...
### Files d’attente
![Azure Service Bus Relay](./media/intro-to-azure/QueuesIntroNew.png) **Figure : les files d’attente permettent d’associer des éléments d’une application et facilitent la mise à l’échelle.**
...
![Azure Service Bus Relay](./media/intro-to-azure/ServiceBusRelayIntroNew.png) **Figure : Service Bus Relay permet la communication entre des applications situées de chaque côté d’un pare-feu.**
...
![Rubriques Azure Service Bus](./media/intro-to-azure/ServiceBusTopicsSubsIntroNew.png) **Figure : les rubriques d’Azure Service Bus permettent à différentes applications de poster des messages et à d’autres applications de s’abonner pour recevoir des messages répondant à des critères spécifiques.**
...
<h2><a id="Performance"></a>Performances</h2>
...
### Azure Caching
![Azure Caching](./media/intro-to-azure/AzureCacheIntroNew.png) **Figure : une application Azure peut mettre des données en cache dans la mémoire, voire les fractionner entre différents rôles de travail.**
...
### CDN
![Azure CDN](./media/intro-to-azure/CDNIntroNew.png) **Figure : les copies d’un objet blob peuvent être mises en cache sur des sites du monde entier.**
...
<h2><a id="BigStuff"></a>Big Compute et données volumineuses</h2>

### HDInsight (Hadoop)
![HDInsight](./media/intro-to-azure/HDInsightIntroNew.png) **Figure : HDInsight permet le traitement en bloc de gros volumes de données.**
...
![Azure Media Services](./media/intro-to-azure/MediaServicesIntroNew.png) **Figure : Media Services est une plateforme pour les applications fournissant des vidéos ou d’autres médias à des clients du monde entier.**
...
![Azure Commerce](./media/intro-to-azure/CommerceIntroNew.png) **Figure : Azure Marketplace et Azure Store vous permettent de rechercher et d’acheter des applications et ensembles de données commerciales Azure afin de les utiliser avec vos applications Azure.**
