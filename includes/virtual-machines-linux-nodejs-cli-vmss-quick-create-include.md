...
À présent, créez maintenant votre groupe de machines virtuelles identiques à l’aide de la commande `azure vmss quick-create`. L’exemple suivant crée un groupe identique Linux nommé `myVMSS` avec cinq instances de machine virtuelle dans le groupe de ressources nommé `myResourceGroup` :
