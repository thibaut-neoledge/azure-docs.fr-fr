...
4.  Dans le fichier ToDoActivity.java, ajoutez la définition suivante pour la méthode `cacheUserToken`.
...
5.  ans le fichier ToDoActivity.java, remplacez la méthode `authenticate` par la méthode suivante qui utilise un cache de jeton. Modifiez le fournisseur de connexion si vous souhaitez utiliser un compte autre que Microsoft.
