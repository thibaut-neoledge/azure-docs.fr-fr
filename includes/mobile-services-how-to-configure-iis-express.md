...
5. Ouvrez la console du Pare-feu Windows et créez une règle de port pour autoriser les connexions au port. Pour plus d'informations sur la création d'une règle de port via le Pare-feu Windows, consultez la rubrique [Ajout d'une nouvelle règle de port via le Pare-feu Windows].
    ...
    >[AZURE.NOTE]Si votre ordinateur de test est associé à un domaine, les exceptions de pare-feu peuvent être contrôlées par une stratégie de domaine. Dans ce cas, vous devez contacter votre administrateur de domaine afin de bénéficier d'une exemption de port sur votre ordinateur.
    ...
    >[AZURE.NOTE]Une fois le service testé localement, vous devez supprimer la règle de Pare-feu Windows que vous avez créée.
