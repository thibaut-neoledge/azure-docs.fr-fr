...
> [!NOTE]
> Azure propose deux modèles de déploiement : [Azure Resource Manager][resource-manager-overview] et classique. Cet article utilise Resource Manager, solution recommandée par Microsoft pour les nouveaux déploiements.
> 
> 
...
* **Groupe de ressources.** Un [*groupe de ressources*][resource-manager-overview] est un conteneur réunissant les ressources associées. Créez un groupe de ressources pour héberger les ressources de cette machine virtuelle.
...
Azure propose un large éventail de ressources et de types de ressources, et cette architecture de référence peut être déployée de différentes manières. Pour plus d’informations sur le déploiement de cette architecture comme illustré ci-dessus, consultez la section [Déploiement de la solution](#solution-deployment) à la fin de ce document. Si vous choisissez de créer votre propre architecture de référence, vous devez suivre ces recommandations, sauf si vous avez un besoin spécifique qui n’est pas abordé dans une recommandation.

### <a name="vm-recommendations"></a>Recommandations pour les machines virtuelles

Bien qu’Azure propose de nombreuses tailles de machine virtuelle, nous recommandons les gammes DS et GS, car ces tailles de machine prennent en charge l’offre [Stockage Premium][premium-storage]. Choisissez l’une de ces tailles de machine virtuelle, sauf si vous disposez d’une charge de travail spécialisée tel qu’un système de calcul hautes performances. Pour plus d’informations, consultez l’article [Tailles des machines virtuelles][virtual-machine-sizes]. 

Si vous déplacez une charge de travail vers Azure, commencez par choisir la taille de machine virtuelle qui correspond le mieux à vos serveurs locaux. Mesurez ensuite les performances de votre charge de travail réelle en termes de processeur, de mémoire et d’opérations d’entrée/sortie par seconde du disque, puis ajustez la taille si nécessaire. Si vous avez besoin de plusieurs cartes réseau (NIC) pour votre machine virtuelle, notez que le nombre maximal de NIC disponibles est une fonction de la [taille de machine virtuelle][vm-size-tables].   

Lorsque vous approvisionnez la machine virtuelle et d’autres ressources, vous devez spécifier un emplacement. En général, choisissez un emplacement le plus proche possible de vos utilisateurs internes ou de vos clients. Sachez toutefois que certaines tailles de machine virtuelle ne sont pas disponibles dans tous les emplacements. Pour plus d’informations, consultez l’article [Disponibilité des produits par région][services-by-region]. Pour visualiser la liste des tailles de machine virtuelle disponibles à un emplacement donné, exécutez la commande d’interface de ligne de commande (CLI) Azure suivante :
...
Approvisionnez suffisamment de comptes de stockage pour le nombre de machines virtuelles que vous prévoyez de déployer. Chacune de vos machines virtuelles nécessite un disque de système d’exploitation et un disque temporaire, et selon les [limites de disque pour les comptes de stockage][storage-account-limits], un compte de stockage Premium peut prendre en charge entre 1 et 20 machines virtuelles. Si vous choisissez d’ajouter des disques de données à votre machine virtuelle, ces derniers ne sont pas formatés lors de la création. Vous devrez vous connecter à la machine virtuelle pour formater les disques de données.

Si vous avez un grand nombre de disques de données, n’oubliez pas les limites d’E/S du compte de stockage. Pour plus d’informations, consultez la section [Limites du nombre de disques de machine virtuelle][vm-disk-limits].
...
* Utilisez une [adresse IP statique][static-ip] si vous avez besoin d’une adresse IP non modifiable &mdash; par exemple, pour créer un enregistrement A dans DNS ou pour ajouter l’adresse IP à une liste approuvée.
* Vous pouvez également créer un nom de domaine complet (FQDN) pour l’adresse IP. Vous pouvez inscrire un [enregistrement CNAME][cname-record] dans le DNS qui pointe vers FQDN. Pour plus d’informations, consultez l’article [Créer un nom de domaine complet dans le portail Azure][fqdn].
...
Vous pouvez ajuster une machine virtuelle en [modifiant sa taille][vm-resize]. Pour évoluer horizontalement, placez deux ou plusieurs machines virtuelles dans un groupe à haute disponibilité derrière un équilibreur de charge. Pour plus d’informations, consultez l’article [Exécution de plusieurs machines virtuelles Windows sur Azure][multi-vm].
...
**Arrêt d’une machine virtuelle.** Azure établit une distinction entre les états « arrêté » et « désalloué ». Vous êtes facturé lorsqu’une machine virtuelle est arrêtée, mais non lorsqu’une machine virtuelle est désallouée.
...
Un [déploiement][github-folder] pour une architecture de référence implémentant ces meilleures pratiques est disponible. Cette architecture de référence comprend un réseau virtuel, un groupe de sécurité réseau et une machine virtuelle unique.

Il existe plusieurs façons de déployer cette architecture de référence. Le moyen le plus simple consiste à suivre les étapes ci-dessous : 
...
Si vous devez modifier le déploiement afin de répondre à vos besoins, suivez les instructions de [readme][github-folder]. 
