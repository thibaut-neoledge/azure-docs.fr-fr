# Créer une application avec un client mobile et web dans Azure App Service
...
Dans cette section, vous allez créer une application web à l'aide d'un exemple de solution d'application web. Vous allez modifier cet exemple pour utiliser le nom du même schéma de base de données et la même chaîne de connexion que l'application mobile.
    ...
    > [AZURE.NOTE] Avant d'effectuer ces étapes, assurez-vous d'avoir initialisé la base de données du serveur principal de votre application mobile en lui connectant un client, faute de quoi l'application web ne pourra pas se connecter à la même base de données.

1. Dans le [portail Azure](https://portal.azure.com/), créez une application web utilisant le même groupe de ressources et le même plan d'hébergement que votre application mobile.
...
3. Dans le projet d'API web, modifiez le fichier MultiChannelToDoContext.cs. Dans OnModelCreating, mettez à jour le nom du schéma pour qu'il soit identique au nom de votre application mobile :
...
4. À présent, vous allez obtenir la chaîne de connexion de l'application mobile à partir du portail Azure :
    ...
    - Dans le portail, sélectionnez votre application mobile, puis cliquez sur la partie libellée **Code utilisateur**.
...
3. Dans l'Assistant **Publication du site web**, sélectionnez **Azure Web App** comme cible de publication, puis créez une application web sans base de données. 
...
[Créer une application mobile]: app-service-mobile-dotnet-backend-xamarin-ios-get-started-preview.md
