...
2. Copiez les fichiers `.jar` du dossier `mobileservices` du Kit de développement logiciel (SDK) dans le dossier `libs` du projet GetStartedWithData.

3. Dans l'Explorateur de package d'Eclipse, cliquez avec le bouton droit sur le dossier `libs`, puis sur **Actualiser**. Les fichiers .jar copiés s'affichent.
...
10. Recherchez la classe *ProgressFilter* au bas du fichier et supprimez les marques de commentaire. Cette classe affiche un indicateur 'loading' lorsque *MobileServiceClient* exécute des opérations réseau.
...
13. Dans la méthode **onCreate**, annulez les marques de commentaire sur les lignes de code suivantes qui définissent la variable **MobileServiceClient** :
  	...
  	Cela crée une nouvelle instance de *MobileServiceClient* qui est utilisée pour accéder à votre service mobile. Cela crée également l'instance de *MobileServiceTable*, utilisée pour le stockage des données de proxy dans le service mobile.
