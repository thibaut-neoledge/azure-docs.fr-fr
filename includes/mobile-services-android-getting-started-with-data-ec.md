...
2. Copiez les fichiers `.jar` depuis le dossier `mobileservices` du Kit de développement logiciel (SDK) dans le dossier `libs` du projet GetStartedWithData.
...
8. Enregistrez le fichier et le projet indiquera des erreurs de build. Recherchez les trois emplacements restants où la variable `toDoItemList` est utilisée et commentez les sections indiquées. Cela supprime complètement la liste en mémoire.
...
10. Recherchez la classe *ProgressFilter* en bas du fichier et supprimez les marques de commentaire sur cette dernière. Cette classe affiche un indicateur « chargement » lorsque *MobileServiceClient* exécute des opérations réseau.
