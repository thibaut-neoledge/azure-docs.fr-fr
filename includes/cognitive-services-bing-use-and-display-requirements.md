Ces conditions d’utilisation et d’affichage s’appliquent à votre implémentation du contenu et aux informations associées (par exemple, les relations, les métadonnées et les autres signaux) disponibles au moyen d’un recours aux API Recherche web Bing, Recherche d’images, Recherche de vidéos et Recherche d’actualités, ainsi qu’aux API Vérification orthographique Bing et Suggestion automatique Bing. Des détails sur l’implémentation associés à ces conditions sont disponibles dans la documentation pour des fonctionnalités et des résultats spécifiques.

## <a name="1--bing-spell-check-api-and-bing-autosuggest-api--you-must-not"></a>1.  API VÉRIFICATION ORTHOGRAPHIQUE BING ET API SUGGESTION AUTOMATIQUE BING.  Vous ne devez pas :

- copier, stocker ou mettre en cache des données provenant des API Vérification orthographique Bing ou Suggestion automatique Bing ; ni utiliser les données provenant des API Vérification orthographique Bing ou Suggestion automatique Bing dans le cadre de Machine Learning ou d’une activité algorithmique similaire pour former, évaluer ou améliorer des services nouveaux ou existants que vous ou des parties tierces pourriez proposer.
...
## <a name="2--bing-web-search-image-search-news-search-and-video-search-apis-the-search-apis"></a>2.  API RECHERCHE WEB BING, RECHERCHE D’IMAGES, RECHERCHE D’ACTUALITÉS ET RECHERCHE DE VIDÉOS (« API DE RECHERCHE ») :

**Définitions.** Les définitions suivantes s’appliquent aux sections 2 à 7 des conditions d’utilisation et d’affichage :
...
**Expérience de recherche Internet.** Toutes les données renvoyées dans les réponses sont utilisables uniquement dans les expériences de recherche Internet. Une expérience de recherche Internet signifie que le contenu affiché, le cas échéant :
...
## <a name="3-advertising"></a>3. Publicité.
La publicité (qu’elle soit fournie par Microsoft ou par un autre fournisseur) ne doit pas être affichée avec les réponses (i) des API de recherche d’images, d’actualités ou de vidéos ; ou (ii) avec des réponses filtrées ou principalement (ou exclusivement) limitées aux résultats de type image, actualités et/ou vidéos à partir d’autres API de recherche.

## <a name="4-branding"></a>4. Personnalisation.
Vous attribuerez chaque réponse (ou partie d’une réponse) affichée à Microsoft, comme décrit à l’adresse https://go.microsoft.com/fwlink/?linkid=833278, sauf indication écrite contraire de Microsoft pour votre usage particulier.


## <a name="5--transferring-responses"></a>5.  Transfert des réponses.
Si vous permettez à un utilisateur de transférer une réponse à un autre utilisateur, par exemple via une application de messagerie ou à une publication sur les réseaux sociaux, les éléments suivants s’appliquent :
...
Vous ne devez pas copier, stocker ou mettre en cache les données des réponses. Toutefois, pour permettre la continuité de l’accès au service et le rendu de données, vous pouvez conserver les résultats uniquement dans les conditions suivantes :
