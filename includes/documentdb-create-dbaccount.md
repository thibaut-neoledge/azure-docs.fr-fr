...
5.	Une fois le compte DocumentDB créé, il est prêt à être utilisé avec les paramètres par défaut dans le portail en ligne. Notez que la cohérence par défaut du compte DocumentDB est définie sur **Par session**. Pour ajuster le paramètre de cohérence par défaut, cliquez sur l’**icône de clé** située sur la **barre Essentials**, puis cliquez sur l’entrée **Cohérence par défaut** dans le panneau **Tous les paramètres**.
