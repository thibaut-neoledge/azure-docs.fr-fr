...
Si vous n'envisagez pas d'utiliser un nom de domaine personnalisé, mais le domaine \*.azurewebsites.net attribué à votre site web par Azure (par exemple, contoso.azurewebsites.net), votre site est déjà sécurisé par un certificat fourni par Microsoft. Vous pouvez utiliser **<https://mywebsite.azurewebsites.net>** pour accéder au site de manière sécurisée. Cependant, \*.azurewebsites.net est un domaine partagé, et comme tous les domaines partagés, il n'est pas aussi sécurisé que si vous utilisiez un domaine personnalisé avec votre propre certificat.
...
5.  À partir de la ligne de commande, un interpréteur de commandes ou une session terminal, utilisez la commande suivante pour convertir **myserver.key** et **myserver.crt** en **myserver.pfx**, le format requis par Sites Web Azure :
	...
	<b>Remarque</b>
	<p>Si l&rsquo;autorit&eacute; de certification utilise des certificats interm&eacute;diaires, vous devez les installer avant d&rsquo;exporter le certificat lors de la prochaine &eacute;tape. Ces certificats sont g&eacute;n&eacute;ralement fournis sous forme de t&eacute;l&eacute;chargement distinct par l&rsquo;autorit&eacute; de certification, dans plusieurs formats adapt&eacute;s &agrave; diff&eacute;rents types de serveurs. S&eacute;lectionnez la version fournie au format PEM (extension de fichier .pem).</p>
	<p>La commande suivante montre comment cr&eacute;er un fichier .pfx incluant des certificats interm&eacute;diaires, qui sont contenus dans le fichier <b>intermediate-cets.pem</b>&nbsp;:</p>
	<pre><code>openssl pkcs12 -export -out myserver.pfx -inkey myserver.key -in myserver.crt -certfile intermediate-cets.pem
	</code></pre>
	</div>
...
Avant de suivre les étapes de cette section, vous devez avoir associé un nom de domaine personnalisé à votre site web Azure. Pour plus d’informations, consultez la rubrique [Configuration d’un nom de domaine personnalisé pour un site Web Azure][Configuration d'un nom de domaine personnalisé pour un site web Azure].
...
## Certificats SubjectAltName (facultatif)
...
7.  À partir de la ligne de commande, un interpréteur de commandes ou une session terminal, utilisez la commande suivante pour convertir **myserver.key** et **myserver.crt** en **myserver.pfx**, le format requis par Sites Web Azure :
...
5.  Exportez le certificat à partir du gestionnaire des services Internet. Pour plus d’informations sur l’exportation du certificat, consultez la page [Exporter un certificat de serveur (IIS 7)][]. Le fichier exporté sera téléchargé ultérieurement dans Azure pour être utilisé avec votre site web Azure.
    ...
    <div class="dev-callout"> 
	<b>Remarque</b>
	<p>Au cours de l&rsquo;exportation, veillez &agrave; s&eacute;lectionner l&rsquo;option <strong>Oui, exporter la cl&eacute; priv&eacute;e</strong>, qui permet d&rsquo;inclure la cl&eacute; priv&eacute;e dans le certificat export&eacute;.</p>
	</div>

    <div class="dev-callout"> 
	<b>Remarque</b>
	<p>Lors de l&rsquo;exportation, veillez &agrave; s&eacute;lectionner les options <strong>Inclure tous les certificats dans le chemin d&rsquo;acc&egrave;s de certification</strong> et <strong>Exporter toutes les propri&eacute;t&eacute;s &eacute;tendues</strong>. Ainsi, les certificats interm&eacute;diaires seront inclus dans le certificat export&eacute;.</p>
	</div>

[][6]

## Certificats auto-signés (facultatif)
...
3.  Pour exporter le certificat dans un fichier .PFX pouvant être téléchargé sur un site web Azure, utilisez la commande suivante :
  ...
  [Certificat SubjectAltName]: #bkmk_subjectaltname
  [Certificats auto-signés]: #bkmk_selfsigned
  [Obtention d’un certificat à l’aide du gestionnaire des services Internet]: #bkmk_iismgr
  [documentation de référence Certreq]: http://technet.microsoft.com/library/cc725793.aspx
  ...
  [inclure tous les certificats et les propriétés étendues]: ./media/configure-ssl-web-site/waws-certwiz2.png
  [spécifier un mot de passe]: ./media/configure-ssl-web-site/waws-certwiz3.png
  [indiquer le chemin d’accès au fichier]: ./media/configure-ssl-web-site/waws-certwiz4.png
  [3]: bkmk_standardmode
  ...
  [boîte de dialogue upload certificate]: ./media/configure-ssl-web-site/ssluploaddlg.png
  [SNI (Indication du nom du serveur)]: http://en.wikipedia.org/wiki/Server_Name_Indication
