# <a name="data-management-and-business-analytics"></a>Gestion des données et analyse marketing
...
## <a name="a-nameblobablob-storage"></a><a name="blob"></a>Stockage d'objets blob
« Blob » est un diminutif pour « Binary Large OBject » (« Objets volumineux binaires ») : c’est une collection d’informations binaires. Tout en étant simples, les objets blob sont utiles. [figure 1](#Fig1) illustre les bases du stockage d’objets blob Azure.
...
&lt;*StorageAccount*&gt; correspond à un identificateur unique attribué lors de la création d’un compte de stockage, tandis que &lt;*Container*&gt; et &lt;*BlobName*&gt; correspondent aux noms d’un conteneur spécifique et d’un objet blob dans ce conteneur. 
...
## <a name="a-namedbinvmarunning-a-dbms-in-a-virtual-machine"></a><a name="dbinvm"></a>Exécution d'un SGBD (système de gestion de base de données) dans une machine virtuelle
...
### <a name="a-namedatasyncasql-data-sync"></a><a name="datasync"></a>Synchronisation des données SQL
...
### <a name="a-namedatarptasql-data-reporting-using-virtual-machines"></a><a name="datarpt"></a>Génération de rapports de données SQL à l'aide de machines virtuelles
Lorsqu’une base de données contient des données, un utilisateur peut être amené à générer des rapports à l’aide de celles-ci. Azure peut exécuter SQL Server Reporting Services (SSRS) dans les machines virtuelles Azure, ce qui revient à exécuter SQL Server Reporting Services en local. SSRS permet de générer des rapports sur les données stockées dans une base de données SQL Azure.  [figure 5](#Fig5) montre le fonctionnement du processus.
...
## <a name="a-nametblstoratable-storage"></a><a name="tblstor"></a>Stockage de table
