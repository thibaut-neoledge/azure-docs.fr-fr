...
3.  Ouvrez le fichier de projet MainPage.xaml.cs et remplacez la méthode **OnNavigatedTo** par le code suivant :
    ...
    Dans ce code, vous devez remplacer le nom de la classe push généré (`todolistPush`) par le nom de classe généré par l'Assistant, qui a généralement le format suivant : `mobile_servicePush`.
