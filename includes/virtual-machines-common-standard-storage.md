...
Pour conserver des copies géoredondantes de vos captures instantanées, vous pouvez copier des captures instantanées d’un compte de stockage localement redondant vers un compte de stockage Standard géoredondant à l’aide des opérations AzCopy ou Copy Blob. Pour plus d’informations, consultez [Transfert de données avec l’utilitaire de ligne de commande AzCopy](../articles/storage/common/storage-use-azcopy.md) et [Copie d’un objet blob](/rest/api/storageservices/Copy-Blob).
