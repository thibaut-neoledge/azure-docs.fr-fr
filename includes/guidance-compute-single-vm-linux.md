...
- Pour évoluer horizontalement, placez deux ou plusieurs machines virtuelles dans un groupe à haute disponibilité derrière un équilibreur de charge. Pour plus d’informations, consultez [Exécution de plusieurs machines virtuelles sur Azure][multi-vm].
...
Le script de commandes Bash suivant exécute des commandes [Azure CLI][azure-cli] pour déployer une seule instance de machine virtuelle ainsi que les ressources de stockage et le réseau associés, comme indiqué dans le précédent schéma.
...
Afin d’appliquer le [contrat SLA pour machines virtuelles][vm-sla], vous devez déployer au moins deux instances dans un groupe à haute disponibilité. Pour plus d’informations, consultez [Exécution de plusieurs machines virtuelles sur Azure][multi-vm].
