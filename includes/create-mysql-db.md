Ce guide explique comment utiliser [ClearDB] pour créer une base de données MySQL à partir de Microsoft Azure Store, et comment créer une base de données MySQL en tant que ressource liée lors de la génération d’un [site web Azure][waws]. [ClearDB] est un fournisseur de services database-as-a-service à tolérance de panne permettant d’exécuter et de gérer des bases de données MySQL dans les centres de données Azure et de s’y connecter à partir d’une application.  

> [!NOTE]
> Lors de la création d'une base de données MySQL pendant le processus de création du site web, vous pouvez créer seulement une base de données gratuite. La création d'une base de données MySQL à partir du Magasin Azure vous permet de créer une base de données gratuite ou de choisir une des options payantes.
> 
> 

## <a name="how-to-create-a-mysql-database-from-the-azure-store"></a>Création d’une base de données MySQL à partir de l’Azure Store
...
2. Cliquez sur l’option **+NOUVEAU** figurant au bas de la page, puis sélectionnez **MARKETPLACE**.
...
## <a name="how-to-create-a-mysql-database-as-a-linked-resource-for-azure-website"></a>Création d’une base de données MySQL comme ressource liée pour un site web Azure
Pour créer une base de données MySQL en tant que ressource liée lors de la génération d’un [site web Azure][waws], procédez comme suit :

1. Connectez-vous au [portail de gestion Azure][portal].
2. Cliquez sur l’option **+NOUVEAU** figurant en bas de la page, puis sélectionnez **CALCUL**, **SITE WEB** et **CRÉER AVEC BASE DE DONNÉES**.
