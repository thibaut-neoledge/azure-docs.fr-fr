...
2. Cliquez sur **Ajouter** en bas de la page. Dans la boîte de dialogue **Créer un conteneur de volumes**, procédez comme suit :
  ...
  5. Indiquez une **clé de chiffrement du stockage cloud** contenant entre 8 et 32 caractères. L’appareil utilise cette clé pour accéder aux données chiffrées. Dans le champ **Confirmer la clé de chiffrement du stockage cloud**, entrez de nouveau la clé de chiffrement du stockage cloud pour la confirmer. Vous ne pouvez pas modifier le paramètre de chiffrement après avoir créé le conteneur de volumes. 
...
Le conteneur de volumes est enregistré et le nouveau conteneur de volumes apparaît sur la page **Conteneurs de volumes**.
