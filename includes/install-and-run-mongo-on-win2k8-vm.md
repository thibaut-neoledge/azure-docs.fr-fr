...
2. Sélectionnez le bouton **Outils** dans le coin supérieur droit. Dans **Options Internet**, sélectionnez l’onglet **Sécurité**, l’icône **Sites de confiance**, puis cliquez sur le bouton **Sites**. Ajoutez \__https://*.mongodb.org_ à la liste des sites de confiance.
...
> [AZURE.NOTE] Le port 27017 est le port par défaut utilisé par MongoDB. Vous pouvez le modifier via le paramètre _--port_ au démarrage du serveur mongod.exe. Veillez à donner le même numéro de port au pare-feu et au point de terminaison « Mongo » dans les instructions ci-dessus.
