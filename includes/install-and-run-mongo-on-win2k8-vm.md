...
2. Sélectionnez le bouton **Outils** dans le coin supérieur droit.  Dans **Options Internet**, sélectionnez l’onglet **Sécurité**, puis l’icône **Sites de confiance**, et cliquez sur le bouton **Sites**. Ajoutez le site _https://\*.mongodb.org_ à la liste des sites de confiance.
...
> [AZURE.NOTE] Le port 27017 est le port par défaut utilisé par MongoDB. Vous pouvez le modifier en spécifiant le paramètre `--port` au démarrage du serveur mongod.exe. Veillez à donner le même numéro de port au pare-feu et au point de terminaison « Mongo » dans les instructions ci-dessus.
